...
function WndBuy:updateMoney()
	...
	self.m_tMoneyId = {[1]=0,[70]=0}
	self.m_tNeedMoney = {[1]=0,[70]=0}
	...
	WZLog("WndBuy:updateMoney", Serialize(self.m_tNeedMoney))
	...
	if CacheCenter:getGameParam().isUseTicket == "0" then
        GetElement(self.m_root,"costText2",WZUILabelTTF):setText(string.format(LocalStrings.SHOPBUY1, tostring(self.m_tMoneyId[70]), GDatatab_item["id_70"].name))
	end
    GetElement(self.m_root,"costShow3",WZUILabelTTF):setText(self.m_tNeedMoney[1])
	GetElement(self.m_root,"costShow1",WZUILabelTTF):setText(self.m_tNeedMoney[70])
	GetElement(self.m_root,"costShow2",WZUILabelTTF):setText(self.m_tNeedMoney[70])
	...
	if self.m_tNeedMoney[70] == 0 then
...
function WndBuy:onGived()
    ...
    if WndShop:selSexIsSame() then
        ...
        WndShopGiven:showWnd(2,self.propData,self.propPrice,self.selSex)
    else
...
function WndBuy:onGive()
    ...
    if curVip >= needVip then
        ...
        local mallId,count = WndBuy:getPropIdAndIndex()
        WndShopGiven:showWnd(1,self.propData,self.propPrice,self.selSex)
    else
...
function WndBuy:moneyCanBuy() 
	...
	if checked == 1 then
        if JudgeMoneyIsEnough(1,self.m_tNeedMoney[1]+self.m_tNeedMoney[70],nil,nil,43, nil, nil, nil, nil, self, self.canBuy) then
            self:canBuy()
			return
		else
			return	
            ...
            if CacheCenter:getGameParam().isUseTicket == "0" then
                if not JudgeMoneyIsEnough(70,self.m_tNeedMoney[70],nil,nil,43, nil, nil, nil, nil, self, self.canBuy) then
				    return
                end
            end
            self:canBuy()
...
function WndBuy:canBuy()
	...
	ProtocolProcessorWndShop:send_MALL_BuyItems(VansPriceID, VansID, checked, 0, self.selSex)
end
