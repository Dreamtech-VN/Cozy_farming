...
function WndPurchase:OnCellCallBack()
	...
	elseif self.costId == 70 then
		GetElement(self.m_root,"conCost1",WZUIContainer):setVisible(false)
		GetElement(self.m_root,"conCost2",WZUIContainer):setVisible(true)
		GetElement(self.m_root,"costText2",WZUILabelTTF):setText(string.format(LocalStrings.SHOPBUY1, tostring(1), GDatatab_item["id_70"].name))
	else
...
function WndPurchase:showBuyInterface(nBuyType,id,sender,callbackFunction,nZorder,other,shopData, buyType,hideGiven,shopId, specialOffer, ownerId)
	...
	CacheCenter:getShopItems(function(t,shopItemList)
		...
		local inShop = (WndShop.m_root ~= nil and WndBagMain.m_root == nil)
		if inShop and (WndShop.leftIndex == 3 or WndShop.leftIndex == 4) then
			local leftTag = 1
			local tag = WndShop.leftIndex
        	if tag == 2 then
        	    leftTag = WndShop.dressTopIndex
        	elseif tag == 3 then
        	    leftTag = WndShop.propTopIndex
        	elseif tag == 4 then
        	    leftTag = WndShop.limitTopIndex
			end
        	for k,v in pairs(self.tShopItemList) do
                ...
                if shopId then
                    if v.shopItemId == id and v.id == shopId and mainType == WndShop.leftIndex and subType == leftTag and v.isOnSale == true then
                        tData = v
                        isOnSale = v.isOnSale
                        break
                    end
                else
            	    if v.shopItemId == id and mainType == WndShop.leftIndex and subType == leftTag and v.isOnSale == true then
            	        tData = v
            	        isOnSale = v.isOnSale
            	        break
            	    end
		...
		elseif inShop and (WndShop.leftIndex == 1) then
			...
			WZLog("购买路径2")
		elseif inShop and (WndShop.leftIndex == 2) then
        	for k,v in pairs(self.tShopItemList) do
                if shopId then
                    if v.shopItemId == id and v.id == shopId then
                        tData = v
		...
		elseif inShop and (WndShop.leftIndex == 5) then
        	for k,v in pairs(self.tShopItemList) do
				...
				if CacheCenter:getPlayerItemById(id) == nil then
                    if shopId then 
                        if v.id == shopId and mainType == WndShop.leftIndex then
                            tData = v
                            isOnSale = v.isOnSale
                            break
                        end
        	...
        	for k,v in pairs(self.tShopItemList) do
                ...
                if shopId then
            	    ...
            	    if v.shopItemId == id and mainType ~= 5 and mainType ~= 4 then
            	        tData = v
            	        isOnSale = v.isOnSale
            	        break
            	    end
        ...
        --商品未上架，弹出提示
        if isOnSale == false and tData.basicInfo.sale_again == 0 then
        ...
        -- 初始化标题
        local str = {LocalStrings.SHOP_DESC10,LocalStrings.SHOP_DESC11,LocalStrings.PURCHASE }
        ...
        local isHideGiven = tData.transaction == -1 and true or false
...
function WndPurchase:BuyResult()
    ...
    local isDress = WndShop:judgePropIsDress(self.selData)
    --时装、购买第一个、限购 关闭购买窗口
    if isDress or curData.index == 0 or self.selData.mainType == 4 then 
...
function WndPurchase:onGiven()
    ...
    WndShopGiven:showWnd(2,{self.selData},cost,sex)
end
...
function WndPurchase:onclickBuy(element)
    ...
    if self.selData.initData.basicInfo.main_type ~= 31 then
    	if itemSex ~= CacheCenter:getPlayerInfo().sex and itemSex ~= 2 then
...
function WndPurchase:_canBuy1()
    ...
    local sex = isDress and WndShop:getShopSelSex() or 2
    WndShopGiven:showWnd(1,{self.selData},cost,sex)
end
...
function WndPurchase:_canBuy2()
	...
	if self.specialOffer then
    	ProtocolProcessorWndShop:send_MALL_BuyItems(count, mallId, checked, 1, nKidSex, self.m_nOwnerId)
	else
    	ProtocolProcessorWndShop:send_MALL_BuyItems(count, mallId, checked, 0, nKidSex, self.m_nOwnerId)
	end
