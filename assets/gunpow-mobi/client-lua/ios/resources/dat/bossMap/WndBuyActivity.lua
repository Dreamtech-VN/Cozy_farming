...
function WndBuyActivity:onBuyBtnFive(element)
    ...
    if CacheCenter:getPlayerInfo().vipLevel < tonumber(buyGoldVipLimited) then
        MsgBoxManager:showConfirmCancelBox(string.format(LocalStrings.MULTI_SWEEP_TIP, tonumber(buyGoldVipLimited)), self, self.needMoreDiamondCallBack, MSGBOXLEVEL_HIGH,nil)
        return 
    end
    -- body
...
function WndBuyActivity:_setDisplayItem()
    ...
    if self.m_tBaseData.nType == 0 then    --购买金币
        imgTitle:setFile("ui/common/common_icon_zhaocaimao.png")
    ...
    elseif self.m_tBaseData.nType == 1 then  --购买活力
        imgTitle:setFile("ui/common/common_icon_bchl.png")
    ...
    elseif self.m_tBaseData.nType == 13 then
        imgTitle:setFile("ui/digGem/common_icon_maijingkuang.png")
        ...
        GetElement(self.m_root, "txtGoldLimitNum_WndBuyActivity", WZUILabelTTF):setText(sGoldLimitNum)
    elseif self.m_tBaseData.nType == 15 then
        imgTitle:setFile("ui/common/common_icon_goumaitouzi.png")
    ...
    elseif self.m_tBaseData.nType == 16 or self.m_tBaseData.nType == 17 then
        ...
        if self.m_tBaseData.nType == 16 then
            imgTitle:setFile("ui/common/common_icon_goumaishengshui.png")
            imgGemBg:setFile("ui/family/bg/common_pic_maishengshui.png")
        elseif self.m_tBaseData.nType == 17 then
            imgTitle:setFile("ui/common/common_icon_goumaiqishi.png")
...
function WndBuyActivity:_updateConsumePrg(nCurCost)
    ...
    if ftxtConsumeValue then
        ftxtConsumeValue:setShowText(string.format(LocalStrings.BUYACTIVITY_RETURN, tCostItemData.icon, nCurCost, self.m_nCousumeMax, returnNum, tCostItemData.icon))
    end
end
