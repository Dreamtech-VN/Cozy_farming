...
function YDBabyAnimation:createAnimation(bBoy)
	...
	obj.m_hasFrontEfect = false
	obj.m_hasBackEffect = false
	obj.m_currentAction = ""
...
function YDBabyAnimation:loadEffect()
    ...
    local animName = self.m_sAninName .. "_body_" .. sIndex .."_effect"   --combatboy_body_036_effect  combat_body_036_effect
	local animNameCommon = "combat_body_" .. sIndex .."_effect"
    local effectPath = "player_sp/effect/" .. sIndex .. "/" .. animNameCommon
	if not WZDataFile:getInstance():checkFileExist(effectPath .. ".json") and not WZDataFile:getInstance():checkFileExist(effectPath .. "2.json") then 
		effectPath = "player_sp/effect/" .. sIndex .. "/" .. animName
	end
	print(effectPath)
    self.m_hasFrontEfect = false
	self.m_hasBackEffect = false
	if WZDataFile:getInstance():checkFileExist(effectPath .. ".json") then 
		self.m_hasFrontEfect = true
	end
	if WZDataFile:getInstance():checkFileExist(effectPath .. "2.json") then 
		self.m_hasBackEffect = true
	end
	if not self.m_hasFrontEfect  and not self.m_hasBackEffect then
        return
    end
	local bodyNode = self:_getBodyNode()
	if bodyNode == nil then return end
	if self.m_hasFrontEfect then 
		local tmp = SkeletonAnimation:createWithFile(effectPath .. ".json",effectPath .. ".atlas",1)
		bodyNode:bindSlot(self.m_sAninName .. "_effect_1",tmp)
	end 
	if self.m_hasBackEffect then 
		local tmp = SkeletonAnimation:createWithFile(effectPath .. "2.json",effectPath .. "2.atlas",1)
		bodyNode:bindSlot(self.m_sAninName .. "_effect_2",tmp)
	end
end
...
function YDBabyAnimation:_changeSpineWeaponDisplay(sName,sIndex)
	...
	local path = "player_sp/" .. self.m_sAninName .. "/weapon/" .. displayName
