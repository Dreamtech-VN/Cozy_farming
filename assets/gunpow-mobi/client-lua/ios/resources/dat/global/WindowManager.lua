...
function WindowManager:checkWindow(bAction,wndElement)
	...
	if directorSize and directorSize.width<= 960 and directorSize.height<=640 then
		return false
	end
	--如果 窗口满屏或是提示框或是聊天框就不弹动画
	if absSize == nil or absSize.width >= 960 or absSize.height >= 640 or absSize.width == 0 then
		return false
...
function WindowManager:addWindow(wndElement,wndLuaObj,bShowAll,bAction,bNoShelter,bBlackBg)
    WZLog("WindowManager:addWindow",tostring(wndElement),tostring(wndLuaObj),tostring(bShowAll),tostring(bAction))
...
function WindowManager:addTeachShow(element, aniName, isArmature, actionName, teachGroupId, teachStepId, isBattle, text, offset, mainId)
    ...
    for i,name in ipairs (aniName) do
    	...
    	local pos = GlobalMethod:ccp(480+offset[i][1],400+offset[i][2])
...
function WindowManager:isFullWindow(WndObj)
    ...
    if WndObj == WndShop then return true end
    if WndObj == WndBagMain then return true end
    if WndObj == WndBless then return true end
    if WndObj == WndFriends then return true end
    if WndObj == WndRankList then return true end
    if WndObj == WndStarSoul then return true end
    if WndObj == WndMounts then return true end
    if WndObj == WndPets then return true end
    if WndObj == WndShop then return true end
    if WndObj == WndStrengthen then return true end
    if WndObj == WndPetRaffle then return true end
    if WndObj == WndEquipmentLottery then return true end
    if WndObj == WndCard then return true end
    if WndObj == WndWakeup then return true end
    if WndObj == WndDigGem then return true end
    if WndObj == SceneRuneLockDraw then return true end
    if WndObj == WndCopyEntry then return true end
    if WndObj == WndRuneBook then return true end
    if WndObj == WndChallengeEntrance then return true end
    if WndObj == SceneCommunity then return true end
    if WndObj == SceneCommunityTotem then return true end
    if WndObj == SceneMemberList then return true end
    if WndObj == WndMaster then return true end
    if WndObj == WndAscending then return true end
    if WndObj == WndFootMark then return true end
    if WndObj == WndEquipmentLottery then return true end
    if WndObj == WndMatchmaking then return true end
