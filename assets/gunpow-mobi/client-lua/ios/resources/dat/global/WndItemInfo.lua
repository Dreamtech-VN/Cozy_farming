...
local stateWidth = {490,385,390}
local stateDescLen = {325,250,250}
...
function WndItemInfo:onPhantom() 
	...
	ProtocolProcessorPhantom:send_SHAPE_UseItem(self.m_tEquip.playerItemId )
	self:_onCloseClick()
end
...
function WndItemInfo:_createLabel(desc)
	...
	txt:setStrokeColor(ccc3(79,60,48))
	txt:setStrokeSize(4)
...
function WndItemInfo:_showItem(pt)
	...
	--创建物品图片
	local img
	if self.m_tEquip.basicInfo.id >= 2001 and self.m_tEquip.basicInfo.id <= 2154 then
	...
	if self.m_tEquip.basicInfo.id >= 1135 and self.m_tEquip.basicInfo.id <= 1146 then
		img:setScale(0.6)
	end
	...
	if maintype == 25 and subtype == 3 then
		...
		img:setScale(0.8)
	elseif maintype == 25 and subtype == 1 then
	--	img:setScaleX(0.6)
	end
...
function WndItemInfo:_showItemName()
	...
	if self.m_tEquip.basicInfo.main_type == 5 then
		local prefix = {LocalStrings.LEVELSTATE1,LocalStrings.LEVELSTATE2,LocalStrings.LEVELSTATE3,LocalStrings.LEVELSTATE4}
		--时装的名字
		local u_type = self.m_tEquip.basicInfo.use_type
		local value = self.m_tEquip.lastTime
		if u_type == 0 then--num
		...
		tData = self:_checkExp(value1,value2,value3,self:_getItemNameColor(),nil,ccc3(55,236,237))
	...
	elseif self.m_tEquip.basicInfo.level == nil then
		...
		tData = self:_checkExp(value1,value2,value3,self:_getItemNameColor(),c2,ccc3(55,236,237))
		sName = "txtItemName_WndItemInfo"
		sName = name or string.format(sName,i)
	else
		...
		value3 = ""
		tData = self:_checkExp(value1,value2,value3,ccc3(255,227,116),ccc3(255,255,255),nil)
		sName = "txtItemName_WndItemInfo"
	end

	self:_createColorTxt(tData,sName,ccp(x,y)) 
end
...
function WndItemInfo:_showItemExplain()
	...
	txtDesc:setColor(ccc3(195,171,148))
	...
	--描述超过2行时增加总高度
	local extraHeight = (math.ceil(ChineseStringLen(self.m_tEquip.basicInfo.desc)/DESCROWLEN) - 2) * 16
...
function WndItemInfo:_showPacks()
	if self.m_tEquip == nil or self.m_tEquip.basicInfo == nil or self.m_tEquip.basicInfo.main_type ~= 3 then return end
	...
	local sub_type = self.m_tEquip.basicInfo.sub_type
	local txt = self:_createTTF(desc[sub_type+1],GlobalMethod:ccp(x,y),nil,GlobalMethod:ccc3(255,227,116),20,nil,GlobalMethod:CCSize(365,0))
	if ProjConfig.LANGUAGE == "pt" or ProjConfig.LANGUAGE == "es" or ProjConfig.LANGUAGE == "en" then
		txt:setFontSize(16)
	end
	self.m_root:addChild(txt)
	...
	local quality = {"ui/common/frame_green.png","ui/common/frame_bule.png","ui/common/frame_violet.png",
		"ui/common/frame_orange.png","ui/common/common_scale9_beibaodi1.png"}
	local countLimit = {}
	...
	local column = 7
	local gridScale = 0.75
	local rowCount = math.ceil(#giftList/column)
	self.m_nGiftNum = #giftList
	for i=1,#giftList do
		local isUse = false
		local tData = GDatatab_item["id_"..giftList[i].id]
		local relativePosition = GlobalMethod:ccp(0.196*((i-1)%column)+0.166,y-0.12-math.floor((i-1)/column)*0.32)
		--创建底图
		...
		--创建物品图片
		local img
		if tData.id >= 2001 and tData.id <= 2154 then
			img = WZUISpine:create()
	        img:setLoop(true)
	        img:setTouchEnable(false)
	        img:setFileJson("ui/ui_qifu.json")
	        img:setFileAtlas("ui/ui_qifu.atlas")
	        img:setAnimationName(tData.icon)
		...
		if tData.main_type == 9 then
			...
			local relativePosition = GlobalMethod:ccp(0.196*((i-1)%column)+0.22,y-0.05-math.floor((i-1)/column)*0.32)
		...
		if main_type ~= 5 and sub_type == 0 or sub_type == 2 then
	...
	self.m_nLineH = self.m_nLineH + 63.5*rowCount
...
function WndItemInfo:_showItemPro()
	...
	if main_type == 20 or main_type == 25 then return end
	...
	local tData = self:_getPropertyData()
...
function WndItemInfo:_showSuit()
	...
	local quality = {"ui/common/frame_green.png","ui/common/frame_bule.png","ui/common/frame_violet.png","ui/common/frame_orange.png","ui/common/common_scale9_beibaodi1.png"}
	local equipList
	local itemSuitNum = 0
	...
	if itemSuitNum < 3 then
    	freeLabel:setShowText(string.format(LocalStrings.BAGTIP5,3,itemSuitNum,3))
		attrList = "suit_3"
	...
	if ProjConfig.LANGUAGE == "th" or ProjConfig.LANGUAGE == "es" or ProjConfig.LANGUAGE == "pt" then
		freeLabel:setMaxWidth(680)
	end

	for i,data in pairs(suitID[attrList]) do
		local sName = string.format("txtSuitPro%d_WndItemInfo",i)
		self:_createColorTxt({value1=ATTR_TITLE[data[1]]..":",font1=fontSize,color1=ccc3(255,227,116),value2=data[2],font2=fontSize,color2=ccc3(255,236,193)},sName,ccp(x,y),360)
	...
	if itemSuitNum < 3 then
    	freeLabel:setShowText(string.format(LocalStrings.BAGTIP5,5,itemSuitNum,5))
		attrList = "suit_5"
	elseif itemSuitNum < 5 then
    	freeLabel:setShowText(string.format(LocalStrings.BAGTIP5,5,itemSuitNum,5))
		attrList = "suit_5"
	elseif itemSuitNum < 6 then
    	freeLabel:setShowText(string.format(LocalStrings.BAGTIP5,6,itemSuitNum,6))
		attrList = "suit_6"
	else
	...
	for i,data in pairs(suitID[attrList]) do
		local sName = string.format("txtSuitPro%d_WndItemInfo",i)
		self:_createColorTxt({value1=ATTR_TITLE[data[1]]..":",font1=fontSize,color1=ccc3(255,227,116),value2=data[2],font2=fontSize,color2=ccc3(255,236,193)},sName,ccp(x,y))
		y = y - 25/self:_getWinSize().height
		self.m_nLineH = self.m_nLineH + 23 
	end
...
function WndItemInfo:_showStar()
	...
	if self.m_tEquip.basicInfo.quality == 4 then
		...
		self.m_nLineH = self.m_nLineH + 38
	else
		for i=1,starLevel do
...
function WndItemInfo:_showStone()
	...
	for i,data in pairs(tData) do
		if data.itemId ~= nil then
			...
			if tItem.main_type == 6 and tItem.sub_type == 5 then
...
function WndItemInfo:_showBtn()
	...
	if self.m_tEquip.tBtnList then
		local dddv = 0
	elseif self.m_bButton == false then
		return
	end
	WZLog("按钮")
	...
	if self.m_nState ~= nil and self.m_nState > 0 then
		WZUIContainer:luaTo(self.m_root):setContentSize(CCSize(stateWidth[self.m_nState],WZUIContainer:luaTo(self.m_root):getAbsContentSize().height))
	end
	...
	self.m_root:addChild(con)
	--WZLog("按钮数据",Serialize(self.m_tBtnData))
...
function WndItemInfo:_getBtnData()
	...
	elseif main_type == MAIN_MATERIAL_TYPE and self.m_tOther and self.m_tOther.interface == INTERFACE_BAG then
		...
		table.insert(tData,temp)
		return tData
	elseif main_type == MAIN_PROPS_TYPE and (sub_type == 0 or sub_type == 1 or sub_type == 2 or sub_type == 14 or sub_type == 15) then--2使用
		local tData = {}
		local temp = {}
		temp.desc = LocalStrings.USE
		temp.clickFun = "onApply"
	...
	elseif main_type == 23 then--23足迹
		...
		table.insert(tData,temp)
		return tData
	elseif main_type == 24 then--24变性卡
		local tData = {}
		local temp = {}
		temp.desc = LocalStrings.USE
