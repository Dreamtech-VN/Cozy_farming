...
CellDialog = {
	-- 请在这里定义和初始化全局成员变量
	DIMG = "ui/teach/dialog_box_4.png",
...
--@note		Order为默认值CellDialog.ZORDER
function CellDialog:createDialog(sText,dir,nShowTime,tBackSender,tBackFunction,nOffsetX,nOffsetY,nMaxWidth)
	local tNewObj = self:_new()
	assert(tNewObj, "CellDialog table create failed!")
	tNewObj:_init()
	local element = WZUIContainer:create()
	assert(element, "CellDialog element create failed!")
	...
	tNewObj.m_nDir = dir or CellDialog.DIR_RIGHT
	tNewObj.m_tBackSender = tBackSender

	element:setLuaObjectIndex(tNewObj)

	element:setZOrder(CellDialog.ZORDER)
...
--@return	#2, 表的引用，也可以用element:getLuaObjectIndex()
--@note		Order为默认值CellDialog.ZORDER
function CellDialog:addDialog(tSender,tParent,sText,dir,nShowTime,tBackSender,tBackFunction,nOffsetX,nOffsetY,nMaxWidth,nScale,bJump,tTxtColor,bIsUpdatePos,tFollowObj, zOrder, tag, textLength, isNeedClick, isOriScale, isScaleAction, isTalkMode)
    --WZLog("CellDialog:addDialog one")
	local tNewObj = self:_new()
	assert(tNewObj, "CellDialog table create failed!")
	tNewObj:_init()
    ...
    if false then
        ...
        element = WZUISystem:getInstance():createElement("CellDialog")
        assert(element, "CellDialog element create failed!")
        element:setLuaObjectIndex(tNewObj)
    ...
    --WZLog("CellDialog:addDialog", tNewObj.m_bIsUpdatePos)
	return element,tNewObj
end
-------------------------------------公有方法模块End----------------------------------------
