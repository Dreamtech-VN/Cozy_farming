...
function WndDressUp:actionCallback( )
    ...
    elseif isEndTeach41 ~= true and step41 < 5 and CacheCenter:getPlayerInfo().level == 9 then
        TeachGroup1:startGroupLevelUp(nil, nil, true, nil, {41,4,self.m_root})
    elseif isEndTeach41 ~= true and step41 < 6 and CacheCenter:getPlayerInfo().level == 9 then
...
function WndDressUp:onExit(element)
    ...
    if WndTask.m_root and WndTask.m_tTaskList and WndTask.m_tTaskList.tMainTask and WndTask.m_tTaskList.tMainTask[1].nId == TeachGroup1.TASK_ID_10 and WndTask.m_tTaskList.tMainTask[1].nTaskStatus == 0 then
        TeachGroup1:endTeachStep({32,5})
        TeachGroup1:startGroup({32,6,WndTask.m_tListItem.m_root})
    end
    ...
    elseif isEndTeach41 ~= true and step41 > 0 and step41 < 5 and CacheCenter:getPlayerInfo().level == 9 and WndEquipmentLottery.m_root then
        TeachGroup1:endTeachStep({41,4})
        TeachGroup1:startGroup({41,5,WndEquipmentLottery.m_root})
    elseif isEndTeach41 ~= true and step41 > 0 and step41 < 6 and CacheCenter:getPlayerInfo().level == 9 and WndEquipmentLottery.m_root then
        TeachGroup1:endTeachStep({41,6})
    elseif isEndTeach41 ~= true and CacheCenter:getPlayerInfo().level == 9 and WndEquipmentLottery.m_root then
    ...
    if isEndTeach35 ~= true and TeachGroup1:isTaskTeachFinish(10203) and (msgCount <= 0 or (msgCount == 1 and MsgBoxManager:_getCurHighestPriorityMsg().nStatus == MSGBOXSTATUS_DONE)) then
        if GlobalGame.g_tWndBottomBarObj and CacheCenter:getPlayerInfo().level <= 8 then
            TeachGroup1:startGroup({35,1,GlobalGame.g_tWndBottomBarObj.m_root})
        end
...
function WndDressUp:onDressUp(element)
	...
	if self.m_tData.nRiseFighting == nil or self.m_tData.nRiseFighting == -1 then
		...
		if tonumber(CacheCenter:getPlayerInfo().level) >= tonumber(self.m_tData.basicInfo.use_level) then
...
function WndDressUp:show(tMsg)
    ...
    if CacheCenter.m_tPlayerInfo.level <= 3 then
