...
-------------------------------------公有方法模块Begin--------------------------------------
--@brief	设置背包玩家缓存信息
function CacheCenter:setPlayerInfo(id, name, sex, title, guildName, position, level, exp, maxExp, vipLevel, winNum, playNum, fighting, mateName, signature, vigor, maxVigor, guildId, property, strongSuitId, starSuitId, mosaicSuitId, petMessage, mountsMessage, fashionProperty, fashionFighting, tournamentLevel, tournamentIntegral, itemSuitId, itemSuitNum, segmentLevel, totemLevel, lovelLevel, loveSkill, moralityLevel, masterName, vipExp, segmentExp, rankMatchMessage, guildLevel, buyTimesPS, headScul, snsValue, starsoulId, spaceSex, giftNum, allMountsMessage, marryFlag, teamId, prayIds, xlId, xlExp, shapeId, shapeLevel, showShape, awakeSoulLevel, awakeStep, itemSuitId2, itemSuitNum2, homeLevel, sheerLuxury, footMark, shapeSkillId, awakeSkillId, runeItemId, runeItemNum, obtainNum, cardMessage, bgId, showMes, coupleMes, childMes, careBuffProp, careToday, headSculStatus, thumbUpNum, badgeInfo)
	ADINDEX = 999999
    ...
    WZLog("CacheCenter:setPlayerInfo", bgId, showMes, self.m_tPlayerInfo.badgeInfo, Serialize(self.m_tPlayerInfo.petInfo))
end

--@brief	设置背包玩家物品列表缓存信息
function CacheCenter:setPlayerItems(itemId, lastNum, lastTime, isUse, data, playerItemId, disappearTime, color)
	...
	--WZLog("CacheCenter:setPlayerItems",Serialize(self.m_tPlayerItemList))
...
--@brief	设置商城商品列表信息
function CacheCenter:setShopItems(id, itemId, itemName, isHot, isNew, isVip, discount, mainType, moneyId, floorPrice, agingPrice, limitLeave,isOnSale,transaction,ad,newad, moneyId2, suit, isPromotion, discountTime)
	self.m_tShopItems = {}
	WndShop.m_tPromotion = {}
	for i=0,id:size()-1 do 
		WZLog("CacheCenter:setShopItems1",id:get(i),itemId:get(i),itemName:get(i),limitLeave:get(i),agingPrice:get(i),discount:get(i),mainType:get(i),isOnSale:get(i))
		local tTempItem = {}
		...
		if tTempItem.basicInfo.main_type == 5 and tTempItem.isOnSale and tTempItem.isNew then
...
--@brief	设置公会信息
function CacheCenter:setGuildInfo(guildId, guildName, guildLevel, prestige, members, desc, setting, totemLevel, schoolLevel, storeLevel, newApply, id, level, post, donate, totalDonate, buyDonate, totemPayTime, sex, weekDonate, lastDonate, allDonate, vipLevel, examine, joinVipLevel, guildwarStage, qualification, presidentInfo)
	self.m_tGuildInfo = {}
...
--@brief	设置师徒信息
function CacheCenter:setMasterInfo(hasMaster, pupil, moralityLevel, moralityExp, baishiLevel, addVigor, message, num, lastTime, taskfinish)
	WZLog("拜师等级",baishiLevel,type(baishiLevel), taskfinish)
	self.m_tMasterInfo = {}
    ...
    if SceneCity.m_root and taskfinish == 1 then
        SceneCity:updateRedDotBuilding("master", true, GlobalMethod:ccp(150,308), 1/0.6)
    elseif SceneCity.m_root and taskfinish == 0 and GlobalGame.g_tRedPointList.master ~= true then
        SceneCity:updateRedDotBuilding("master", false, GlobalMethod:ccp(150,308), 1/0.6)
    end
	...
	if SceneLeagueMain.m_root ~= nil or SceneRoom.m_root ~= nil or SceneBossRoom.m_root ~= nil or SceneBattle.m_root ~= nil or SceneBattleLoading.m_root ~= nil or WindowManager:isHaveTeachTouchLayer() == true or WndTeachTalk.m_root ~= nil then return end

	if WndMaster ~= nil and WndMaster.m_root == nil then
		local wndMaster = WndMaster:createElement()
		WindowManager:addWindow(wndMaster,WndMaster)
	end
	if WndMasterHall ~= nil and WndMasterHall.m_root ~= nil then
		WndMasterHall:update()
	end
	if WndMaster.m_root ~= nil then
		WndMaster:updateRedPoint()
	end
end
...
function CacheCenter:setMailList(id, theme, time, status, sendId, attachment, headId, faceId, cost, content, attachments, sexs,senderName,recvName,color)
	self.m_tMailList = {}
	local isMark = false
	for i = 0 , id:size() - 1 do
		local temp = {}
...
function CacheCenter:setGameParam(name, value)
	...
	WZLog("maxMountsUpgradeLevel::::",self.m_tGameParam["gameMaxLevel"])
...
function CacheCenter:setMultiCopyData(resetTime, mapId, passTime, starLevel)
...
function CacheCenter:setRankListInfo(ranking, playerId, name, faceId, headId, sex, level, param1, param2, param3, param4, param5, param6, param7, rankType, trendRank, vipLevel, param8, headColor, param9)
    ...
    for i = 0,dataLen-1 do
        ...
        if rankType == 12 or rankType == 22 then
...
-- isMentoring：是否是师徒关系
function CacheCenter:setFriendList(playerId, playerName, level, sex, online, fighting, send, friendType, faceItemId, headItemId,appTimer,isMentoring, sendGift, couple, friendNum, isOnlineRemind, vipLevel, offlineTime, serverId, headColor, chum, applychum, mentoringNum, spaceVisitState)
...
--添加好友列表
function CacheCenter:addFriendList(playerId, playerName, level, sex, online, fighting, send, friendType, faceItemId, headItemId,appTimer, isMentoring, friendNum, sendGift, vipLevel, serverId, headColor, mentoringNum)
...
--好友动态列表
function CacheCenter:setDynamicFriendList(playerId, playerName, typeList, date, acceptType,sendType, vigor, friendNum, vigorNum, idonly)	
	self.m_nHaveVigor = vigorNum
...
function CacheCenter:removeBestFriendList(playerId)
	...
	WndFriends:removeBestFriendSuccess()
end
...
function CacheCenter:OperationFriendOK(playerId, acceptType,vigorNum, sendType, friendNum)
	...
	WndFriends:RefreshInterface(nil,nil,itemIdx)
...
function CacheCenter:setFriendDataList(playerId, playerName, level, sex ,faceItemId ,headItemId,isMentoring,playerFighting, friendNum, isOnline, vipLevel, tournamentLevel, serverId, headColor, isChum, mentoringNum, couple)
	self.m_tFriendList = {}
	for i=0,playerId:size()-1 do 
		local level , reinc = ChangeLevelReinc(level:get(i))
		local temp = {}
...
function CacheCenter:_setMoneyList()
	...
	for i=1,#list do
		...
		WZLog("CacheCenter:_setMoneyList",list[i].id,self.m_tMoneyList.diceCoin)
	end
	WZLog("CacheCenter:_setMoneyList",self.m_tMoneyList.blueDiamond,self.m_tMoneyList.gold, self.m_tMoneyList.card, self.m_tMoneyList.ticket)
end
...
function CacheCenter:addMark(name,index,nTag)
   ...
   -- name  = "btnFriend_WndOwnCity"
	--if WndBottomBar.m_root then
    ...
    if elementObj then
		...
		if name == "btnMail_WndOwnCity" or name == "btnFriend_WndOwnCity" then
		    ...
		    if AllBtn then 
		    	local allred = elementObj:getLuaObjectIndex().m_bRed or CacheCenter:getRedState("btnFriend") or CacheCenter:getRedState("btnMail")
		    	if elementObj:getLuaObjectIndex().m_nMoveDirection == 0 then
		    		allred = CacheCenter:getRedState("btnFriend") or CacheCenter:getRedState("btnMail")
		    	else
...
function CacheCenter:updateRedPoint(pos,node,btnName,note)
    ...
    local rightBtnName = {  "btnPlayer_WndBottomBar","btnItem_WndBottomBar","btnStrong_WndBottomBar",
         ...
         "btnFriend_WndBottomBar","btnRune_WndBottomBar","btnFootMark_WndBottomBar"}
    ...
    elseif pos == "right" then
        ...
        if btnName == "btnChat" then
	   	...
	   	elseif btnName == "btnPet" or btnName == "btnFootMark" or btnName == "btnMount" then
        	if (SceneCity.m_tWndBottomBar and node == SceneCity.m_tWndBottomBar) then
		        position = ccp(54,53)
		    else
	            position = ccp(54,53)
	        end
        elseif (SceneCity.m_tWndBottomBar and node == SceneCity.m_tWndBottomBar) then
	        position = ccp(45,53)
	    end
    end
    ...
    if btnName then
        ...
        if curData then
            ...
            if name[curData.index] == "btnTask_WndBottomBar" and (SceneCity.m_tWndBottomBar and node == SceneCity.m_tWndBottomBar) then
            	if ProjConfig.LANGUAGE == "tr" then
            		setRedPoint(btn,curData.state,ccp(95,10),scale)
        		else
	                setRedPoint(btn,curData.state,ccp(85,10),scale)
	            end
            else
        	...
        	if name[curData.index] == "btnItem_WndBottomBar" then
            	...
            	if btn then
            		WZLog("CacheCenter:updateRedPointbtnItem1", CacheCenter:getRedState("btnItem"), CacheCenter:getSkillRed())
            		local state = CacheCenter:getRedState("btnItem") or CacheCenter:getSkillRed()
            		setRedPoint(btn,state, nil, scale)
            	end
        	end

        	if name[curData.index] == "btnPractice_ExtendUp_WndBottomBar" or name[curData.index] == "btnPlayer_WndBottomBar" then
            	local btn = GetElement(node,"btnPlayer_WndBottomBar",WZUIButton)
            	if btn then
            		WZLog("CacheCenter:updateRedPointbtnBag1", CacheCenter:getRedState("btnBag"), CacheCenter:getRedState("btnPractice_ExtendUp"))
            		local state = CacheCenter:getRedState("btnBag") or CacheCenter:getRedState("btnPractice_ExtendUp")
            		setRedPoint(btn,state, nil, scale)
            	end
        	end
        	...
        	if name[curData.index] == "btnPet_WndBottomBar" or name[curData.index] == "btnMount_WndBottomBar" or name[curData.index] == "btnFootMark_WndBottomBar" then
            	...
            	if btn then
            		...
            		local state = CacheCenter:getRedState("btnMount") and CheckButtonOpen(ISLAND_RIGHT_MOUNT, false) or CacheCenter:getRedState("btnFootMark") and CheckButtonOpen(ISLAND_RIGHT_FOOTMARK, false) or GlobalGame.g_tRedPointList.petFetter
        ...
        for k,v in pairs(data) do
            ...
            if name[v.index] == "btnTask_WndBottomBar" and (SceneCity.m_tWndBottomBar and node == SceneCity.m_tWndBottomBar) then
                setRedPoint(btn,v.state,ccp(80,25), scale)
        	...
        	if name[v.index] == "btnPractice_ExtendUp_WndBottomBar" or name[v.index] == "btnPlayer_WndBottomBar" then
            	local btn = GetElement(node,"btnPlayer_WndBottomBar",WZUIButton)
            	if btn then
            		WZLog("CacheCenter:updateRedPointbtnBag1", CacheCenter:getRedState("btnBag"), CacheCenter:getRedState("btnPractice_ExtendUp"))
            		local state = CacheCenter:getRedState("btnBag") or CacheCenter:getRedState("btnPractice_ExtendUp")
        	...
        	if name[v.index] == "btnPet_WndBottomBar" or name[v.index] == "btnMount_WndBottomBar" or name[v.index] == "btnFootMark_WndBottomBar" then
            	local btn = GetElement(node,"btnPet_WndBottomBar",WZUIButton)
            	if btn then
            		WZLog("CacheCenter:updateRedPointbtnBag Pet", CacheCenter:getRedState("btnPet"), CacheCenter:getRedState("btnMount"), CacheCenter:getRedState("btnFootMark"))
            		local state = CacheCenter:getRedState("btnMount") and CheckButtonOpen(ISLAND_RIGHT_MOUNT, false) or CacheCenter:getRedState("btnFootMark") and CheckButtonOpen(ISLAND_RIGHT_FOOTMARK, false) or GlobalGame.g_tRedPointList.petFetter
    ...
    if pos == "right" then
    	for k,v in pairs(data) do
            if (v.state or CacheCenter:getSkillRed()) and v.index ~= 7 and (SceneCity.m_tWndBottomBar == nil or node ~= SceneCity.m_tWndBottomBar or (v.index ~= 6 and v.index ~= 11)) then
...
--@brief 	设置邀请码列表的数据
function CacheCenter:setInviteData(myInviteCode, friendIdList, nameList, pictureList, faceIdList, levelList, vipLevelList, sex, lineStatusList, serverIdList, taskIdList, conditions, currCount, statusList, writeFlag, headColor)
	-- body
...
function CacheCenter:updateInviteTask(id, status, count, currCount)
	...
	if self.m_nInviteMark > 0 then
		self:addMark("btnFriend_WndOwnCity",1)
		WndFriends:showInviteMark(true)
	end
	--排序任务
	table.sort(self.m_tInviteTaskList, sortInviteTask)
	--WZLog("CacheCenter:updateInviteTask 111", Serialize(self.m_tInviteTaskList))
	--只刷新邀请码任务
...
function CacheCenter:refreshInviteTask(id, status)
	...
	if self.m_nInviteMark > 0 then
		self:addMark("btnFriend_WndOwnCity",1)
	elseif self.m_nInviteMark == 0 and self.m_nDailyMark == 0 then
...
--@brief 	新增邀请码好友
function CacheCenter:addInviteFriend(friendId, name, headId, faceId, level, vipLevel, sex, lineStatus, serverId, headColor)
	-- body
	if self.m_tInviteFriends == nil then
		self.m_tInviteFriends = {}
	end
...
--私聊缓存
function CacheCenter:addPriChatCache(iMainChannel, iSendID, sSendName, iRecvID, sRecvName, sMsgContent, tm, vipLevel,playerHead,playerFace,playerSex,headScul,serviceId,headColor,senderLevel,rtime,bubbleId, playerTitle, playerPvpLevel)
	WZLog("CacheCenter:addPriChatCache")
	local temp = {}
...
function CacheCenter:setSkill(unlockSkill, skillNum, useSkill, openLevel, logtype, logskillId, mes)
	WZLog("CacheCenter:setSkill", WndSkillProp.upTip, WndSkillProp.learnTip, type(WndSkillProp.upTip), type(WndSkillProp.learnTip))
...
--@breif 	更新新手定推礼包数据
function CacheCenter:updateNewUserPackageList(funcId, pushInfo, lastNum, endTime, originPrice)
	-- body
	...
	if not bExist then
		...
		elseif funcId == 76 and WndCard.m_root then
			...
			relationPosition = GlobalMethod:ccp(0.5, 0.6)
			bShowAll = false
		end
...
function CacheCenter:updateFootMarkRedPoint()
    ...
    CacheCenter:setRedState("btnFootMark",isRed)
    GlobalGame:getBtnRedPointEvent():dispatcher()
end

--@brief  拥有足迹的数据列表
function CacheCenter:setFootMarkData(footMarkId, upgradeLevel, advancedLevel, advancedBlessingValue, property, upgradeBlessingValue, fighting, remainingTime, useFootmark)
...
function CacheCenter:updateFootMarkInfoOK(footmarkId, upgradeLevel, advancedLevel, advancedBlessingValue, property, fighting, originType, remainingTime, upgradeBlessingValue)
...
--@brief 	设置好友黑名单数据
function CacheCenter:setFriendBlacklistData(playerId, playerName, level, sex ,faceItemId ,headItemId,isOnline, vipLevel, serverId, headColor)
	--body
	self.m_tFriendBlacklist = {}
	for i = 0, playerId:size()-1 do 
		local level , reinc = ChangeLevelReinc(level:get(i))
