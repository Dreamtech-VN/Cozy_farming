...
--@brief	发送预先获取的协议
function PrefetchCache:sendPrefetchProtocal()
	WZLog ("PrefetchCache:sendPrefetchProtocal")
	--获取公会列表
	--ProtocolProcessorSceneCommunity:send_COMMUNITY_GetCommunityListNew(1)
	--获取任务列表
	--ProtocolProcessorWndTask:send_TASK_GetTaskList()  --modify by wuweidong
	--会员信息
	--ProtocolProcessorWndVip:send_VIP_GetVipInfo( )
	--等级礼包
	--ProtocolProcessorWndVip:send_VIP_GetVipRewardsInfo( 1 ) --等级礼包
	--每日礼包
	--ProtocolProcessorWndVip:send_VIP_GetVipRewardsInfo( 2 ) --每日礼包
end
	...
	if ProjConfig.LANGUAGE == "vn" then
		if m_tTaskData.script[1][1] == 996 or m_tTaskData.script[1][1] == 997 then 
			...
			elseif g_loginType == "facebook" then
				...
				if m_tTaskData.id ~= 1340001024 and m_tTaskData.id ~= 1350001024 then
					bIsAdd = false
				end
			end
		end
