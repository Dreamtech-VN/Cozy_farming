...
function MsgBoxManager:showConfirmBox(sMsgBody, tCallbackLuaObj, fCallbackFunc, nLevel, tCustomUIConfig, bIsOnlyOneButton, RedAttText, isMust, fCallbackCancel)
...
function MsgBoxManager:showConfirmCancelBox(sMsgBody, tCallbackLuaObj, fCallbackFunc, nLevel, tCustomUIConfig, sTicketMark)
...
--@brief	弹出签到
function MsgBoxManager:showGameSignIn(msg)
	WZLog("MsgBoxManager:showGameSignIn")
	local tMsg = MsgData:create()
	if tMsg == nil then
		return
	end
	...
	tMsg.nId = self.m_nIdTag
	tMsg.nLevel = MSGBOXLEVEL_NORMAL
	tMsg.nType = MSGBOXTYPE_GAMESIGNIN     
	tMsg.nStatus = MSGBOXSTATUS_INIT
	tMsg.tData = msg
	self:_pushMsg(tMsg)
	return tMsg.nId
...
function MsgBoxManager:showTipBox(sMsgBody, nDisappearDelay, tCallbackLuaObj, fCallbackFunc, nLevel, tCustomUIConfig,nOrder,GrayRender, isRemoveOne)
	if sMsgBody == nil then
		WZLog("MsgBoxManager:showConfirmBox: sMsgBody is nil")
		return
	end
...
function MsgBoxManager:_showMsg(tMsg)
	...
	elseif tMsg.nType == MSGBOXTYPE_GAMESIGNIN then
		WndGameSingIn:show(tMsg)
	elseif tMsg.nType == MSGBOXTYPE_WELFARE then
		WndWelfare:showInterface(1, nil, tMsg)
