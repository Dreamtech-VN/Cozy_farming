...
function GetFailCopyUi()
	...
	local gTable = {}
	--按优先级排序
	for k, v in pairs(GDataSheet1) do
...
function SplitStringWithSeparator(s, sSeparator, sSeparator2, isNumber)
	...
	-- --WZLog("SplitStringWithSeparator one", s, sSeparator)
	-- while true do
        -- local isOther = false
        -- local nFindLastIndex = string.find(s, sSeparator, nFindStartIndex)
        -- local nFindLastIndex2 = sSeparator2 and string.find(s, sSeparator2, nFindStartIndex) or 99999
        -- if nFindLastIndex and nFindLastIndex > nFindLastIndex2 or nFindLastIndex == nil and nFindLastIndex2 and nFindLastIndex2 ~= 99999 then
            -- isOther = true
            -- nFindLastIndex = nFindLastIndex2
        -- end

        -- --WZLog("SplitStringWithSeparator two", tostring(isOther), nFindLastIndex, nFindLastIndex2)
	   -- if not nFindLastIndex then
           -- nSplitArray[nSplitIndex] = isNumber and tonumber(string.sub(s, nFindStartIndex, string.len(s))) or string.sub(s, nFindStartIndex, string.len(s))
           -- break
	   -- end

	   -- nSplitArray[nSplitIndex] = isNumber and tonumber(string.sub(s, nFindStartIndex, nFindLastIndex - 1)) or string.sub(s, nFindStartIndex, nFindLastIndex - 1)
        -- local strLen = string.len(sSeparator)
        -- if sSeparator == "%." then strLen = 1 end
        -- if isOther == true then
            -- strLen = string.len(sSeparator2)
        -- end
        -- --WZLog("SplitStringWithSeparator three", tostring(nSplitArray[nSplitIndex]), strLen, nFindStartIndex, nSplitIndex)

	   -- nFindStartIndex = nFindLastIndex + strLen
...
function createImage(icon,pt,size,bOrigin)
	if icon == nil then
		return
	end
	...
	image:setAnchorPoint( ccp( 0.5 , 1 ) ) 	--图片锚点
...
function SetCloseButtonPos(closeElement)
	...
	local conClosePosX = (screenWidth - (screenWidth-960*scale)/2 - 76 )/scale
...
function JumpByUIId(nUIMainId , nUISubId, taskId, nJumpTypeIndex, bActivityJump)
	...
	if GlobalGame.g_nCurrentUIChannelId == nUIMainId and nUIMainId ~= 92 then  --判断跳转的界面是否已打开
        ...
        --[[
        TeachGroup1:startGroup({3,6,WndSingleCopy.m_oCurPage})
        TeachGroup1:startGroup({5,13,WndSingleCopy.m_oCurPage})
        TeachGroup1:startGroup({8,10,WndSingleCopy.m_oCurPage})
        TeachGroup1:startGroup({9,10,WndSingleCopy.m_oCurPage})
        TeachGroup1:startGroup({1,4,WndSingleCopy.m_oCurPage})
        --]]
    ...
    if nUIMainId == Chat_Channel_Single_Copy_Hall then
        --[[
        TeachGroup1:startGroup({3,6,WndSingleCopy.m_oCurPage})
        TeachGroup1:startGroup({5,13,WndSingleCopy.m_oCurPage})
        TeachGroup1:startGroup({8,10,WndSingleCopy.m_oCurPage})
        TeachGroup1:startGroup({9,10,WndSingleCopy.m_oCurPage})
        TeachGroup1:startGroup({1,4,WndSingleCopy.m_oCurPage})
        --TeachGroup1:startGroup({32,7,WndSingleCopy.m_oCurPage})
        --]]
    end
...
function GetMaxExpByLevel(nLevel)
    ...
    return GDatatab_player_upgrade["id_"..nLevel].exp
end
...
function UpdatePlayerFigure(playerArmNode,playerEquipData,sex,headColor,bodyColor)
    ...
    if not wing then playerArmT:setWing(0) end
...
function SplitStoryTalk(s)
    ...
    for i=1, #storyList do
        ...
        for k=1, #strList do
            ...
            if type(s) == "string" then
                stringFormat = stringFormat .. ([[<T C="255,236,193" S="22">]]..s..[[</T>]])
...
function ShowPanelNullTip( objNode, sText, color, strokeColor, fontSize, position, type)
    ...
    if type and type == 1 then
    	txtNoData:setDimensions(GlobalMethod:CCSize(500,0))
    elseif type and type == 2 then
    	txtNoData:setDimensions(GlobalMethod:CCSize(300,0))
    end
...
function CheckButtonShow(nBtnId)
	...
	if nBtnId == 113 then
		if tostring(ProjConfig:getChannelId()) == "8888" or tostring(ProjConfig:getChannelId()) == "53" or tostring(ProjConfig:getChannelId()) == "75" or tostring(ProjConfig:getChannelId()) == "275" or tostring(ProjConfig:getChannelId()) == "68" then
	        return false
	    end
    end
...
--@param 	1:判断角色名2:判断公会名3:判断战队名4:判断套装名
function JudgeResultInClientForInputText(nUseType, txt)
    ...
    if nInputTxtLen > 12 and nUseType == 1 then
        result = 5
    end
    if nInputTxtLen > 16 and nUseType == 2 then
    	result = 8
    end
    if nInputTxtLen > 10 and nUseType == 3 then
    	result = 9
    end
    if nInputTxtLen > 8 and nUseType == 4 then
    	result = 10
    end
...
function JudgeMoneyIsEnough(moneyType, moneyCnt, text, nType, nWindowId, tCell, func, tCustomUIConfig, nBoxType, tCell2, func2)
	...
	--处理坐骑兑换卡
	elseif tData.main_type == 2 and tData.sub_type == 11 then
        local pCnt = CacheCenter:getPlayerItemCountById(moneyType)
        local itemInfo = GDatatab_item["id_"..moneyType]
        WZLog("-----------------card info----------------",pCnt,moneyCnt)
        if pCnt < moneyCnt then
...
function judgeNotEnoughJump(tCell, callFunc)
    ...
    local nSweetNum = getBagItemCount(102)
    --判断是否有甜甜圈
    if nSweetNum > 0 then
        g_nCurVigor = CacheCenter:getPlayerInfo().vigor
        MsgBoxManager:showConfirmCancelBox(LocalStrings.EAT_SOME_SWEETS, tCell, WndOpenChest.showUseInterface, nil, nil)
    else
        --判断是否有剩余的购买次数
...
--@brief    计算时装战力
function caculateClothesFighting(extraInfo)
    -- body
	for i=1,20 do if extraInfo[tostring(i)] == nil then extraInfo[tostring(i)] = 0 end end
    local nFighting = 0 
...
function ToChangeFreeText(str,fontColor,sc,ss,se,fontSize,channel)
	...
	if #tempTable > 0 then --有表情
		for i,v in ipairs(tempTable) do --循环拼接富文本
			...
			for j,k in ipairs(WndChat.FACEIMASK) do
				if tempSt == k then
...
--@brief    添加关系图标
function AddRelationIcon(rootNode, nMate, nBestFriend, nMentoring, tData, parentNode, relationPt)
    -- body
    --关系图标
    local nTempNum = 0 
    ...
    if nTempNum > 0 then
        ...
        conTemp:setAbsContentSize(GlobalMethod:CCSize(nTempNum * 50,50))
        local nIndex = 0
        local nIconX = 0
        --婚姻
        if nMate > 0 then    
            local element, objNew = CreateRelationIcon(1, tData, parentNode)
            nIconX = (1 + 2*nIndex)/(nTempNum * 2)
            element:setRelativePosition(GlobalMethod:ccp(nIconX, 0.5))
...
function ShowRedEnvelopesRain(redEnvelopeType)
	...
	if (ENVELOPES == nil or #ENVELOPES == 0) and (#g_tRedPackList == 0 or SceneCity.m_root == nil) then return end
...
--@param id:配表的id
function addWeChatBtn(element,id,post,scale)
...
function trailerAnimCall1(info)
	...
	elseif isCity then
		...
		if UIObj.m_nMoveDirection ~= 0 then
			WZLog("trailerAnimCall10", tostring(UIObj.m_root), tostring(icon))
			WZLog("trailerAnimCall11", tostring(GetElement(UIObj.m_root, "imgTrailer_WndBottomBar", WZUIImage)))
			GetElement(UIObj.m_root, "imgTrailer_WndBottomBar", WZUIImage):setFile(icon)
			layer = GetElement(UIObj.m_root, "conAll_WndBottomBar", WZUIContainer)
			anim = GetElement(UIObj.m_root, "conTrailer_WndBottomBar", WZUIContainer)
...
function getNewOnlineRewardState2(rewardItems, rewardCounts, count, target, status)
	WZLog("getNewOnlineRewardState2", WndGameActivity.m_bNewOnlineState, count)
	if WndGameActivity.m_bNewOnlineState == 2 or (WndGameActivity.m_nCurrentSelectTypeId == g_tGameActivityTypes.ACTIVITY_TYPE_5009) then
...
function CreateLimitPackage(funcId, parentNode, relativePoint, bShowAll)
	...
	if curPackage then 
	    ...
	    if ProjConfig.LANGUAGE == "vn" then
			...
			--txtBtn:setRelativePosition(GlobalMethod:ccp(0.3,0))
			end
		end
	end
end
...
--@brief 判断是否iponex
function IsIphoneX()
	if WZDeviceInfo:systemName() == "iPhone10,3" or WZDeviceInfo:systemName() == "iPhone10,6" then
       return true
    end
...
function IsShowPunishTime(bShowPunishTime)
    ...
    if GlobalGame.g_pvpPunishTime == nil then
        ProtocolProcessorGlobal:send_ROOM_CheckPwPunish()
        return true
    elseif GlobalGame.g_pvpPunishTime > 0 then
        ...
        if curSystemTime - GlobalGame.g_pvpPunishTimeCurServiceT < GlobalGame.g_pvpPunishTime then
        	if bShowPunishTime then
        		...
        		WndSuspension:showByTime(curTime)
        	end
            return true
        else
...
function OpenPartner(index)
	if index == nil then
		...
		index = isRedPet and 1 or isRedMount and 2 or isRedFoot and 3 or nil
