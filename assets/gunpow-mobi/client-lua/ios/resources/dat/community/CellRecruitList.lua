...
function CellRecruitList:_update()
	...
	--等级
	GetElement(self.m_root,"txtLv_CellRecruitList",WZUILabelAtlasFont):setText(self.m_nLevel)
	...
	--头像
	local imgHead = CellHead:show(GetElement(self.m_root,"conHead_Cell",WZUIContainer),self.headId,self.faceId,self.m_nImgSex,nil,nil,self.vipLevel, self.headColor)
...
function CellRecruitList:_adaptLanguage_vn()
	WZLog("CellRecruitList:_adaptLanguage_vn")
	GetElement(self.m_root,"ttfFighting",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.49617,0.34))
end
