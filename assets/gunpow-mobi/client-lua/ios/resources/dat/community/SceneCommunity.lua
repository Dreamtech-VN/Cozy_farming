...
function SceneCommunity:createCommunityBtn(sInputName)
	...
	if CacheCenter:getGameParam().isUseTicket == "0" then
		if not JudgeMoneyIsEnough(70, tonumber(CacheCenter:getGameParam().guildCreateDiam), nil, nil, Chat_Channel_Guild_Create, nil, nil, nil, nil, self, self.clickSureMoney) then
			return 
		end
	else
		if not JudgeMoneyIsEnough(1, tonumber(CacheCenter:getGameParam().guildCreateDiam), nil, nil, Chat_Channel_Guild_Create, nil, nil, nil, nil, self, self.clickSureMoney) then
			return 
		end
	end
	self:clickSureMoney()
