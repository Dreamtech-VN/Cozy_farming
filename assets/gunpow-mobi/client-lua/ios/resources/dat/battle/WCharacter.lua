...
function BuffBody:new(info, owner, user)
	local buffBody = {}
	...
	buffBody.m_nCanRemove = tonumber(info and info.disperse) or 0 --0不能,1能
	buffBody.m_nTimeIntervalValue = tonumber(info and info.interval) or 0
	buffBody.m_nTimeDurationValue = tonumber(info and info.duration) or 0
...
function WCharacter:hide(isView)
    ...
    for id,buff in pairs (hero.m_tBuffChangeStateList) do
        ...
        WZLog("WCharacter:hide two", effect, tostring(buff.m_tAnim),tostring(buff.m_tAnimRange))
    end
    --幽灵目标选中框
...
function WCharacter:setRepulse(repulse)
    --do return end
    ...
    if math.abs(repulse) < 10 then
        repulse = repulse * 10/math.abs(repulse)
    end
    if math.abs(repulse) > 30 then
        repulse = repulse * 30/math.abs(repulse)
    end

    local dx = math.floor(repulse)
    local speed = Vector2:create(dx,15)
...
--@brief 颠倒伤害
function WCharacter:getHurtReverse(hurt)
    hurt = math.ceil(hurt)
    local hurtReverse = 0
...
--@return 属性值
function WCharacter:getHurtAddPercent()
    local value = nil
    if self.m_tAttributeChangeStateList.m_nHurtAddPercent ~= nil then
        value = self.m_tAttributeChangeStateList.m_nHurtAddPercent.value / 100
    end
    value = value or 0
...
--@return 属性值
function WCharacter:getBeHurtAddPercent()
    local value = nil
...
function WCharacter:isRobot()
    local isRobot = self:getBattleId() ~= WBattleGlobal:getCurrent():getMyBattleId() and self.m_bCanControl == true
	return isRobot
end
...
function WCharacter:getRadiusForBulletExplode()
    ...
    WZLog("WCharacter:getRadiusForBulletExplode", tostring(self.m_fRadiusForBulletExplodeChange), tostring(self.m_fRadiusForBulletExplode), exScale)
    if self.m_fRadiusForBulletExplodeChange then
        return math.floor(self.m_fRadiusForBulletExplodeChange * exScale)
    else
        return math.floor(self.m_fRadiusForBulletExplode * exScale)
    end
end
...
function WCharacter:getRectForBulletExplodeBomb(skillId)
    ...
    if skillId and skillId > -1 then
        ...
        if isChangeType == "table" then
            explodeBomb = {x=skillInfo.boom_scope[1][1] * scaleW,y=skillInfo.boom_scope[1][2]*scaleH}
        else
            explodeBomb = {x = self.m_fRectForBulletExplodeBomb.x * scaleW, y = self.m_fRectForBulletExplodeBomb.y*scaleH}
        end
    ...
    elseif self.m_fRectForBulletExplodeBombChange then
        explodeBomb = {x = self.m_fRectForBulletExplodeBombChange.x * scaleW, y = self.m_fRectForBulletExplodeBombChange.y*scaleH}
    else
        explodeBomb = {x = self.m_fRectForBulletExplodeBomb.x * scaleW, y = self.m_fRectForBulletExplodeBomb.y*scaleH}
    end
...
function WCharacter:showHurt()
    ...
    --[[
	if not self.m_bIsHurt or self:isDead() or self:getHp() <= 0 then
		self:setHurtType(0)
		...
		if #self:getHurtValueList() > 0 then
            ...
            if self:getHp() <= 0 and self.m_bIsDeadHurt == nil and self:isDead() ~= true then
            	...
            	if (self.m_bIsOldAnim == nil or self.m_bIsOldAnim == true) and self.m_tHurtValue[1] > 0 then
	                self:getAnimation():play(hurtAnim,1)
...
function WCharacter:hurtEffectHandle(hurt)
    hurt = self:getHurtReverse(hurt)
...
function WCharacter:markHurt(nHurtValue,tShootHero,isSkillHurt,isPetHurt,isBuffHurt,hurtRatio)
    local offHurt = self:isNeedOffHurt(tShootHero,nHurtValue)
    
    --当前回合伤害记录(单独伤--治疗弹使用 ,全部伤害--吸血)
    if nHurtValue ~= -1 and not isPetHurt and not isBuffHurt then
        if tShootHero:getType() == 0 or tShootHero.m_nMonsterType == MonsterType.BUFF_TOTEM or tShootHero.m_nMonsterType == MonsterType.TREAT_TOTEM then
    ...
    if nHurtValue ~= -1 and not isBuffHurt and not isPetHurt then
        ...
        if not self.m_bShowOffSkill then
            ...
            WZLog("WCharacter:markHurt 111111",Serialize(offSkillList))
	...
	if nHurtValue ~= -1 and nHurtValue ~= 0 then
        ...
        if WBattleGlobal:getCurrent():isSingleStage() then
            ...
            if tShootHero ~= nil and tShootHero:getBattleId() == self:getBattleId() and nHurtValue >= self:getHp() then
        ...
        if nHurtValue < 0 then
            ...
            WZLog("BattleMsgBossMapSkill:_changeAttribute 11", nHurtValue, recoveryAddPercent)
        end
...
function WCharacter:getCTBSpeed()
    ...
    WZLog("WCharacter:getCTBSpeed", self.m_nAgility,speed)
    return speed
end
...
function WCharacter:updateBuffByCTB(dt, updateCTB_time)
    ...
    if dt ~= nil then
		for id,buff in pairs (self.m_tBuffChangeStateList) do
			...
			if buff.m_nTimeIntervalValue > -1 and 
                count > buff.m_nTakeEffectCount and 
                buff.m_nTakeEffectCount <= totalCount then
				buff.m_nTakeEffectCount = buff.m_nTakeEffectCount + count2
				if buff.m_nType == BuffType.BLOOD or buff.m_nType == BuffType.POISON or buff.m_nType == BuffType.BLOOD_BOSS or buff.m_nType == BuffType.SHAPE_RECOVERY then
                    for i=1, count2, 1 do
                        WZLog("WCharacter:updateBuffByCTB three-6", buff.m_nTakeEffectCountReal, buff.m_nTakeEffectCount, i, totalCount, count, count2)
...
function WCharacter:_setRemainHP()
    ...
    --             WZLog("WCharacter:_setRemainHP removeAnime",tostring(isCurAttackToFrozen))
            ...
            if curRoundAction then
                if curRoundAction.round ~= WBattleGlobal:getCurrent().m_nStartRoundTimes then
                    ...
                    if self:getBattleId() == WndBattleHud.m_tMyHero:getBattleId() then
                        ...
                        WBattleGlobal:getCurrent():endCurRound(self:getBattleId(),18.0,nil,true)
                    else
                        WBattleGlobal:getCurrent():endCurRound(self:getBattleId(),18)
                    end
                end
            else
...
function WCharacter:getIsImmunityByPetSkill(effectType,effectParam, effectParam2)
    ...
    -- local listAll = {self.m_tImmunityBuffList, self.m_tImmunityBuffList2, self.m_tImmunityBuffList3}
    -- for k,list in ipairs(listAll) do
...
function WCharacter:getIsImmunityListByPetSkill(effectType,effectParam, effectParam2)
    ...
    -- local listAll = {self.m_tImmunityBuffList, self.m_tImmunityBuffList2, self.m_tImmunityBuffList3}
    -- for k,list1 in ipairs(listAll) do
        for _,data in pairs(self.m_tImmunityBuffList) do
            local skillId = data.skillId
            local effectData = data.effectData
            WZLog("WCharacter:getIsImmunityByPetSkill",effectType,effectParam,tostring(effectParam2),effectData.type,effectData.param)
...
function WCharacter:addImmunityPetSkill(skillId,effectData)
    -- local skill1 = self.m_tImmunityBuffList[skillId]
    -- local skill2 = self.m_tImmunityBuffList2[skillId]
    -- local skill3 = self.m_tImmunityBuffList3[skillId]
    -- if not skill1 then
    --     self.m_tImmunityBuffList[skillId] = effectData
    -- elseif not skill2 then
    --     self.m_tImmunityBuffList2[skillId] = effectData
    -- elseif not skill3 then
    --     self.m_tImmunityBuffList3[skillId] = effectData
    -- end
...
function WCharacter:setLeftDirection(isLeft)
    ...
    if isLeft then
        --设置为朝右方向
        if self:getType() == 0 or self.m_bIsGuaiWithSuit then
