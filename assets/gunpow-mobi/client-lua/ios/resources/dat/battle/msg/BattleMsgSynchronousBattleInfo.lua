...
function BattleMsgSynchronousBattleInfo:init()
    ...
    if self.m_tData then
		...
		, self.m_tData.explodePlayerId, self.m_tData.explodeSkillId, self.m_tData.explodePosNum, self.m_tData.explodePosX, self.m_tData.explodePosY, self.m_tData.finishPercent, self.m_tData.roundNum, self.m_tData.killCount, self.m_tData.onlineStatus, self.m_tData.battleInfo)
    end
end
