...
--@brief    消息数据表
BattleMsgPlayerReadyFly = {
    m_sName = "BattleMsgPlayerReadyFly",
    SHOOT_LEAST_DIS = 5,
...
function BattleMsgPlayerReadyFly:init()
    BattleMsgPlayerReadyFly.isRun = true
    self.m_cacheLast = {}
    self.m_cacheLast.last_speed = {x = 0,y = 0}
    self.m_cacheLast.last_five_point = {}
    --print("fmj BattleMsgPlayerReadyFly:init")

    local loop = SceneBattle:getBattleLoop()

    if loop:getBattleStatus() == BattleLoop.S_NORMAL then
        loop:setBattleStatus(BattleLoop.S_PLAYER_READY_FLY)
...
function BattleMsgPlayerReadyFly:process(dt)
    ...
    if ((hasMsg and TeachGroup1.ISBATTLE_MYTURN ~= true) or touch1 == nil or touch:getTouchPoint(1) == nil) then
        -- --print("BattleMsgPlayerReadyFly:process", tostring(hasMsg), tostring(touch1))
        return true
    end
    ...
    --WZLog("BattleMsgPlayerReadyFly:process 2",loop:getBattleStatus(),BattleLoop.S_PLAYER_READY_FLY)
    if TeachGroup1.ISBATTLE_MYTURN == true or loop:getBattleStatus() == BattleLoop.S_PLAYER_READY_FLY then
        --WZLog("BattleMsgPlayerReadyFly:process 3",WBattleGlobal:getCurrent():getMyHero():getBattleId(),WBattleGlobal:getCurrent():getMyBattleId())
        if TeachGroup1.ISBATTLE_MYTURN == true or WBattleGlobal:getCurrent():getMyHero():getBattleId() == WBattleGlobal:getCurrent():getMyBattleId() then
            --WZLog("BattleMsgPlayerReadyFly:process 4",touch:getTouchStatus(1),BattleTouch.TOUCH_NONE,touch:getTouchStatus(2),BattleTouch.TOUCH_NONE)
            if touch:getTouchStatus(1) ~= BattleTouch.TOUCH_NONE and touch:getTouchStatus(2) == BattleTouch.TOUCH_NONE then
                --WZLog("BattleMsgPlayerReadyFly:process 5",touch:getTouchStatus(1),BattleTouch.TOUCH_END)
                if touch:getTouchStatus(1) ~= BattleTouch.TOUCH_END then
                    --描绘
                    --WZLog("BattleMsgPlayerReadyFly:process 1",touch:getTouchStatus(1))
                    ...
                    if hero:getUseBigSkill() and TeachGroup1.ISBATTLE_MYTURN ~= true then
                        ...
                        -- WZLog("BattleMsgPlayerReadyFly:process X",hero.m_nRotatePre)
                    ...
                    -- --print("BattleMsgPlayerReadyFly:process 1", angle,speedX,speedY, startPos.x, startPos.y)
                    ...
                    return false
                elseif self.m_bIsCancelDis == nil then
                    --根据点击坐标计算
                    ...
                    if BattleCommon:pointDis(pressPoint,touchPoint) > BattleMsgPlayerReadyFly.SHOOT_LEAST_DIS then
...
function BattleMsgPlayerReadyFly:done()
    ...
    if SceneBattle.m_pointsLine.m_bisWrong == true then
...
function BattleMsgPlayerReadyFly:calSpeed()
    --WZLog("BattleMsgPlayerReadyFly:calSpeed zero")
    local pressPoint = GlobalMethod:ccp( self.m_tPressPoint.x , self.m_tPressPoint.y )
    ...
    local center = SceneBattle:getFrontLayer():convertToNodeSpace(GlobalMethod:ccp(480,320))
    local aPos = WBattleGlobal:getCurrent():getMyHero():getAnimation():getPosition()
    ...
    pointX,pointY = pressPoint.x - touchPoint.x , -(pressPoint.y - touchPoint.y)

    local length = math.sqrt(pointX * pointX + pointY * pointY)

    pointX = pointX / length
    pointY = pointY / length
    if length < BattleMsgPlayerReadyFly.SHOOT_POWER_MIN then
        length = BattleMsgPlayerReadyFly.SHOOT_POWER_MIN
    elseif length > BattleMsgPlayerReadyFly.SHOOT_POWER_MAX then
        length = BattleMsgPlayerReadyFly.SHOOT_POWER_MAX
    end
    local rate = (length - BattleMsgPlayerReadyFly.SHOOT_POWER_MIN) / (BattleMsgPlayerReadyFly.SHOOT_POWER_MAX - BattleMsgPlayerReadyFly.SHOOT_POWER_MIN)
    local rateX = rate
    ...
    local shootPowerX = (rate + rateX) * BattleMsgPlayerReadyFly.SHOOT_POWER_BASE * scale
    local shootPowerY = rate * BattleMsgPlayerReadyFly.SHOOT_POWER_BASE * scale
    ...
    WZLog("BattleMsgPlayerReadyFly:calSpeed one","speedX =",speedX,"speedY =",speedY,"pointX =",pointX,"pointY =",pointY,"length =",length,"rate =", rate,"rateX =", rateX, "perX =", perX,"scale =", scale,"shootPowerX =", shootPowerX,"shootPowerY =",shootPowerY,"touchPoint.x =",touchPoint.x,"touchPoint.y =",touchPoint.y,"pressPoint.x =",pressPoint.x,"pressPoint.y =",pressPoint.y)
