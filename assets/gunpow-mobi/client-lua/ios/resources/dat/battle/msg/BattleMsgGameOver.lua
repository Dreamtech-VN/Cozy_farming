...
BattleMsgGameOver = {
	...
	WIN_BARIMAGE = "battle/hud/battle_hud_sd.png",
	LOSE_BARIMAGE = "battle/hud/battle_hud_sb.png",
...
function BattleMsgGameOver:sendSingleProcessor()
    ...
    if WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_SINGLE) then
        ...
        local nCostTime = g_singleCopyEndTime - g_singleCopyStartTime
        if nCostTime < 0 then
            nCostTime = 0
        end
        g_tSingCopyOver.costTime = nCostTime
        local tIslandConfig = json.decode(CacheCenter:getGameParam().landlordConfig)
        local nRoundScore = tIslandConfig.roundScore - nRound * tIslandConfig.everyRoundReduce
        if nRoundScore < 0 then
            nRoundScore = 0
        end
        local nHpScore = math.ceil(nHp*100/nMaxHp * tIslandConfig.hpRatio)
        local nTimeScore = tIslandConfig.timeScore - math.floor((nCostTime/tIslandConfig.everyNsReduce) * tIslandConfig.second2ReduceScore)
        if nTimeScore < 0 then
            nTimeScore = 0
        end
        local nScore = nRoundScore + nHpScore + nTimeScore
        WZLog("BattleMsgGameOver:send single", nScore)
        g_tSingCopyOver.score = nScore
        if self.m_bWin then
            local verifyData = self:_getVerifyData()
            tVerify.verify = verifyData
            tVerify.costTime = nCostTime
            tVerify.score = nScore
            local mapId = WBattleGlobal:getCurrent().m_tMakePairOk.mapId
    ...
    elseif WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_DAILY) then
            ...
            if tData.isWin then
                ...
                elseif tData.fightId == 3 then
                    ...
                    ProtocolProcessorSingleMap:send_SINGLEMAP_ChallengeSuccess(tData.mapId, data, COPYTYPE_DAILY)
            else
    ...
    elseif WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_TOWER) then
        ...
        local hp = towerData.pass_hp
        local cnt = towerData.pass_round
        ...
        if curHp >= hp and curCnt <= cnt then
            ...
            local tVerify = {hp=nHp*100/nMaxHp, round=nRound }
            tVerify.verify = verifyData
            ProtocolProcessorSingleMap:send_SINGLEMAP_ChallengeSuccess(data.levelId, json.encode(tVerify), COPYTYPE_TOWER)
        else
            WBattleGlobal.getCurrent().m_bIsWin = false
        end
    --英雄塔
...
function BattleMsgGameOver:process()
    ...
    if TeachGroup1.ISFIRSTBATTLE ~= true and WBattleGlobal:getCurrent():isSingleStage() and g_bSingCopyOver == false and WBattleGlobal:getCurrent().m_bIsWin and TeachGroup1.ISFIRSTBATTLE ~= true then
...
function BattleMsgGameOver:story()
    if AutoRunBattleConst.AUTO_RUN_BATTLE or WBattleGlobal:getCurrent():isReplayGame() then
        return true
    end
...
function BattleMsgGameOver:done()
    ...
    if WBattleGlobal:getCurrent():isSingleStage() then
        ...
        if WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_SINGLE) then
            ...
            if self.m_bWin then
                ...
                -- ProtocolProcessorSingleMap:send_SINGLEMAP_ChallengeSuccess(WBattleGlobal:getCurrent().m_tMakePairOk.mapId,
                --     json.encode(tVerify),
                --     WBattleGlobal:getCurrent().m_tMakePairOk.mapType)
            else
        ...
        elseif WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_TOWER) then
            ...
            WndTowerSettlement:showWindow(data)
        elseif WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_DAILY) then
             if g_tSingCopyOver ~= nil and g_tSingCopyOver.rewardId ~= nil then 
        ...
        --     local data = self:_getHeroTowerEndData()
        --     if g_tSingCopyOver ~= nil and g_tSingCopyOver.rewardId ~= nil then 
        --         data.rewardId = g_tSingCopyOver.rewardId
        --     end
        --     if g_tSingCopyOver ~= nil and g_tSingCopyOver.rewardCount ~= nil then 
    ...
    elseif nBattleType == BattleConstants.g_nBATTLE_TYPE_NORMAL then
        ...
        elseif WBattleGlobal:getCurrent():isHeroTowerStage() then
            ...
            WndTowerSettlement:showWindow(self.m_tSettlementData)
            return
        end
...
function BattleMsgGameOver:_getVerifyDataRoundHurtString(data)
    ...
    for battleId,hurtRecord in pairs(data.hurtRecord) do
        ...
        if battleId == heroBattleId then
            self.m_nRemainHp = self.m_nRemainHp + playerAdd + buffHurt + petHurt
        end
        local hurtListStr = ""
        if hurtRecord.playerHurts then
            for i,hurtInfo in pairs(hurtRecord.playerHurts) do
                local tmpHurtStr = tostring(hurtInfo.shootId)..splitII..tostring(hurtInfo.hurtRatio)..splitII..tostring(hurtInfo.hurtAdd)..splitII..tostring(hurtInfo.hurtHp)
                if hurtListStr == "" then
        ...
        local str = tostring(battleId)..splitII..tostring(playerAdd)..splitII..tostring(buffHurt)..splitII..tostring(petHurt)..splitII..tostring(petRatio)..splitII..hurtListStr
        if result == "" then
            result = str
        else
            result = result ..split..str
