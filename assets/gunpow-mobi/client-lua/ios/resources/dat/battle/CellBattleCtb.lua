...
function CellBigBattleCtb:setCharacter(tCharacter)
	...
	if self:getRoot() then
		...
		local sTxt = [[<T S="20" C="255,237,191">LV:%s</T><BR/><T S="20" C="255,237,191">%s</T>]]
		...
		if WBattleGlobal:getCurrent().m_nBattleType == BattleConstants.g_nBATTLE_TYPE_NORMAL and WBattleGlobal:getCurrent().m_tMakePairOk.battleChannle == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_PW then
			sTxt = [[<T S="20" C="255,237,191">ID:%d</T><BR/><T S="20" C="255,237,191">%s</T>]]
			element:setShowText( string.format(sTxt, tCharacter:getId(), tCharacter:getPlayerName()) )
		else
			element:setShowText( string.format(sTxt,tCharacter:getLevel(),tCharacter:getPlayerName()) )
		end
