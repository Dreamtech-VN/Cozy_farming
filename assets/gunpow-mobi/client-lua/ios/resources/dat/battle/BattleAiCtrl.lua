...
function BattleAiCtrl:endCurRround()
    WBattleGlobal:getCurrent():endCurRound(WBattleGlobal:getCurrent():getMyBattleId(),77,nil,nil,true)
end

function BattleAiCtrl:getDtTime()
...
function BattleAiCtrl:updateAction(dt)
    ...
    elseif action == BattleAiAction.ACTION_FIND_WAY then
        ...
        elseif rand <= 3 and hero:isInBuffState(EffectTypeConfig.LIMIT_FLY) ~= true and (BattleHeroUse:heroUse(hero:getBattleId(),BattleHeroUse.USE_FLY) == true or (hero.m_nRemainUseItemCount > 0 and self:getAiHelper():isCanUseItem(BattleHeroUse.ITEM_FLY) and BattleHeroUse:heroUse(hero:getBattleId(),BattleHeroUse.USE_ITEM,BattleHeroUse.ITEM_FLY) == true)) then
    ...
    elseif action == BattleAiAction.ACTION_BIG_SKILL then
        if WBattleGlobal:getCurrent():isArenaPWStage() or WBattleGlobal:getCurrent():isHeroTowerStage() then
        ...
        --大招可用时使用大招，否则使用普通技能
        if hero.m_bIsGuaiWithSuit ~= true and self.m_bUseBigSkill == false and hero:isInBuffState(EffectTypeConfig.LIMIT_USE_SKILL) ~= true and BattleHeroUse:heroUse(hero:getBattleId(),BattleHeroUse.USE_BIGSKILL) then
...
function BattleAiCtrl:_moveFar(tPos)
	...
	if hero:getPF() > 0 then
        ...
        WZLog("BattleAiCtrl:_move two", movetime, math.abs(self.m_nDistanceMove), hero:getPF(), hero:getPosition().x, SceneBattle:getFrontLayerSize().width - 100, face)
		...
		ProtocolProcessorBattleInterface:send_BATTLE_PlayerMove(nBattleId, nPlayerId, movetime, movestep, sPos.x, sPos.y)
	end
end
    ...
    while canMove ~= true and count < 10 do
        ...
        if movetime > 0 then
            ...
            if canMove == true then
                ...
                ProtocolProcessorBattleInterface:send_BATTLE_PlayerMove(nBattleId, nPlayerId, movetime, movestep, sPos.x, sPos.y, 0)
                break
            end
        else
            break
...
function BattleAiCtrl:_moveRobot(tPos)
    ...
    while canMove ~= true and count < 10 do
        count = count + 1
        movetime = movetime + offset
        WZLog("BattleAiCtrl:_move two", count, movetime, hero:getPF(), hero:getPosition().x, SceneBattle:getFrontLayerSize().width - 100, face, tostring(isChangeDir))
