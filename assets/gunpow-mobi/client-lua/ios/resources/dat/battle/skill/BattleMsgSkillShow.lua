...
function BattleMsgSkillShow:getSkillConfig()
    ...
    if type(skillConfig.boom_scope) == "table" then
        self.m_tOwner.m_fRectForBulletExplodeBombChange = {x=skillConfig.boom_scope[1][1],y=skillConfig.boom_scope[1][2]}
    end
...
function BattleMsgSkillShow:addBuff(targetType,buffId)
    ...
    if WBattleGlobal:getCurrent():isHostControl() then
        ...
        ProtocolProcessorSceneBattle:send_BATTLE_BuffChange(WBattleGlobal:getCurrent():getBattleId(), self.m_tOwner:getBattleId(), 0, buffId, idList)
    end
