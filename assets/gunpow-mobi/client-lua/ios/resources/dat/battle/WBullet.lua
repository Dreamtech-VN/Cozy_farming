...
--@param	tChara:子弹所属人物
function WBullet:buildBullet(tPos, tSpeed, tAcceleration, tChara, isPenetrateMap, isSpatter)
	local bullet = {}
    ...
    if false and isBigSkill then
        ...
        if bullet.m_ownerChara.m_bIsMonster then
            local type = GDatatab_shape_skins["id_" .. bullet.m_ownerChara.m_nMonsterId].bullet_prefix
            if type then
                if type == 0 then
	...
	WZLog("WBullet:buildBullet", strWeapon)

    WZLog("WBullet:buildBullet one", tostring(isBigSkill), tostring(bigSkillNumber),tostring(tChara.m_bIsCureBomb))
...
function WBullet:updatePosition()
    ...
    WBattleGlobal:getCurrent():cleanMyBulletFog(self, 2)
end
...
function WBullet:explode(isNeedDigHole)
    ...
    elseif self.m_bIsNoHole == nil then
        table.insert(WBattleGlobal:getCurrent().m_tExplodeInfoCurRound, {id=self:getOwnerChara():getBattleId(), skillId=WBattleGlobal:getCurrent().m_nSkillBeUseCurRound or -1, x=x, y=y})
    end
end
...
function WBullet:DigHole()
    ...
    table.insert(WBattleGlobal:getCurrent().m_tExplodeInfoCurRound, {id=self:getOwnerChara():getBattleId(), skillId=WBattleGlobal:getCurrent().m_nSkillBeUseCurRound or -1, x=self.m_mover:getMoverPosition().x, y=self.m_mover:getMoverPosition().y,width=self:getOwnerChara():getRectForBulletExplodeBomb().x,height=self:getOwnerChara():getRectForBulletExplodeBomb().y})
	if self:getOwnerChara():getCanDigHole() and (self:getOwnerChara():getRectForBulletExplodeBomb().x > 0 or self:getOwnerChara():getRectForBulletExplodeBomb().y > 0) then
		if WBattleGlobal:getCurrent():isFog() then
            WBattleGlobal:getCurrent():cleanMyBulletFog(self, 3, 0, 0, self:getOwnerChara():getRectForBulletExplodeBomb().x, self:getOwnerChara():getRectForBulletExplodeBomb().y)
        end
...
function WBullet:checkCollision()
	...
	--英雄碰撞
        ...
        WZLog("WBullet:checkCollision four-1", tostring(self.m_tIsReflectList))
...
function WBullet:checkCollisionWithCharacterList(pos,raduis,charaList)
	...
	for id,chara in ipairs(charaList) do
        WZLog("WBullet:checkCollisionWithCharacterList one", id, tostring(chara:isDead()))
...
function WBullet:checkCollisionWithReflect(pos,raduis,charaPos,chara)
    ...
    if self.m_tIsReflectInitList == nil then
        ...
        WZLog("WBullet:checkCollisionWithReflect three-1")
    end
    ...
    if self.m_tIsReflectInitList[index] == nil or self.m_tIsReflectInitList[index] == true then
        if isReflect then
            ...
            isReflectInit = false
            WZLog("WBullet:checkCollisionWithReflect four-1", index)
        else
            self.m_tIsReflectInitList[index] = false
            WZLog("WBullet:checkCollisionWithReflect four-2", index)
        end
    end
    ...
    if self.m_tIsReflectPreList[index] == true then
        if isReflect then
            ...
            isReflect = false
            WZLog("WBullet:checkCollisionWithReflect five-1", index)
        else
            self.m_tIsReflectPreList[index] = false
            WZLog("WBullet:checkCollisionWithReflect five-2", index)
        end
    end
    ...
    if self.m_tIsReflectPreList[index] == false then
        if isReflect then
            self.m_tIsReflectPreList[index] = true
            WZLog("WBullet:checkCollisionWithReflect five-3", index)
        else
            self.m_tIsReflectPreList[index] = false
            WZLog("WBullet:checkCollisionWithReflect five-4", index)
        end
    end
    ...
    if self.m_tIsReflectPreList[index] == nil then
        if isReflect then
            self.m_tIsReflectPreList[index] = true
            WZLog("WBullet:checkCollisionWithReflect five-5", index)
        else
            self.m_tIsReflectPreList[index] = false
            WZLog("WBullet:checkCollisionWithReflect five-6", index)
        end
    end
	...
	if collisionRang ~= nil then
		for i,rang in pairs(collisionRang) do
			if rang.m_nType == 0 then
                ...
                WZLog("WBullet:checkCollisionWithRang one", i, tostring(isColl), pos.x, pos.y, charaPos.x, charaPos.y, raduis, charaRaidus)
                if isColl then
                    return true, dis
                end
...
--@return	#1:受伤的人物列表
--@return	#2:受伤值
function WBullet:checkHurt(isNotcalculate)
    WZLog("WBullet:checkHurt zero", tostring(self.m_tCheckHurtWithSkillPos))
	local tHurtCharas = {}
	local tHurtValues = {}
    local tDistance = {}
    local tCritType = {}
	...
	for i,charaList in ipairs(self.m_tCollisionCharacters) do
		for id,chara in ipairs(charaList) do
			if not chara:isDead() and not chara.m_bOffHurt then
...
--@param	chara:英雄
--@return	#1：伤害,#2：距离
function WBullet:getHurt(chara,isNoHurt, dis,id,isNotcalculate)
...
--@param4   宠物攻击系数
--@return	#1：伤害
function WBullet:calculateHurt(distance,shootHero,targetHero,petCoef,isNotcalculate, isPetCrit)
    ...
    local hurtRatio = shootHero:getHurtAddPercent() or 0
    ...
    reduceAcctak = reduceAcctak + reduceAcctak * (targetHero:getBeHurtAddPercent() or 0)

    local hurtType, critRateA, randNumber = self:getHurtType(shootHero,targetHero, isPetCrit)
    if isPetCrit then
    ...
    --WZLog("WBullet:calculateHurt ZERO-4",hurt, targetHero:getBeHurtAddPercent() or 0, shootHero:getHurtAddValue() or 0)
    ...
    if isNotcalculate == nil then
        WZLog("WBullet:calculateHurt end", "shootHeroID", shootHero:getBattleId(), "targetHeroId", targetHero:getBattleId(),
            ...
            "\nhurtRatio", shootHero:getHurtAddPercent() and shootHero:getHurtAddPercent() + 1 or 1,
            "\nhurtRatioB", shootHero:getHurtMulPercent(),
            "\nbHurtRatio", targetHero:getBeHurtAddPercent() and targetHero:getBeHurtAddPercent() + 1 or 1,
            "\n攻击方暴击", critRate,
            "\n攻击方敏捷", agility,
    ...
    if not petCoef then
        ...
        if not petCoef then
            recordRatio = recordRatio * hurtRatio2
        end
        ...
        recordRatio = recordRatio * distancePercent * (1 + (targetHero:getBeHurtAddPercent() or 0))
        ...
        -- WZLog("WBullet recordRatio-4",recordRatio)
...
function WBullet:getHurtType(shootHero,targetHero,isPetCrit)
    ...
    if true then
        ...
        if (randNumber <= critRate or isPetCrit) and shootHero:getCanSuperHit() then
            hurtType = 2
