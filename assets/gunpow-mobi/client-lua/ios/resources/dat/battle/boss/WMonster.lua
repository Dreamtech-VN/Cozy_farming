...
function WMonster:getHeroNearPos(distance, pos)
    ...
    for i ,v in pairs(tHeroList) do
        ...
        local bossPos = pos
        if (not v:isDead() and v:getHp() > 0) and math.abs(heroPos.x - bossPos.x) <= distance then
...
function WMonster:getHeroNearBoss(nDistance)
    ...
    for i ,v in pairs(tHeroList) do
        ...
        if (not v:isDead() and v:getHp() > 0) and math.abs(heroPos.x - bossPos.x) <= nDistance then
            bIsHeroNearBoss = true
            table.insert(heroNearBossList,v)
        end
    end
...
function WMonster:getNearestPlayer()
	...
	for i,player in pairs(tPlayerList) do
		...
		if (not player:isDead() and player:getHp() > 0) and nDisToPlayer < nNearestDis then
...
function WMonster:getFarestPlayer()
	...
	for i,player in pairs(tPlayerList) do
		local nDisToPlayer = math.abs(player:getAnimation():getPosition().x - guaiPos.x)
		if (not player:isDead() and player:getHp() > 0) and nDisToPlayer > nFarestDis then
...
--@brief	获得随机的一个玩家
function WMonster:getRandomPlayer()
...
function WMonster:getHpMaxPlayer()
    ...
    for i ,v in pairs(tHeroList) do
        if (not v:isDead() and v:getHp() > 0) and (hpMaxPlayer == nil or v:getHp() > hpMaxPlayer:getHp()) then
            hpMaxPlayer = v
        end
    end
...
function WMonster:getHpMinPlayer()
    ...
    for i ,v in pairs(tHeroList) do
        if (not v:isDead() and v:getHp() > 0) and (hpMinPlayer == nil or v:getHp() < hpMinPlayer:getHp()) then
            hpMinPlayer = v
        end
    end
...
function WMonster:adjustDirect(tPos)
    WZLog("WMonster:adjustDirect", self:getBattleId() , tostring(self.m_bIsLeftFlip))
