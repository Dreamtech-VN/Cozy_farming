...
function WNewMonsterAI:startRound()
    ...
    if not WBattleGlobal:getCurrent():isReplayGame() and self:isCanControl() then--WBattleGlobal:getCurrent():getCurrentCharacterId() == self.m_tBoss:getBattleId() then
...
function WNewMonsterAI:run(dt)
    ...
    ----WZLog("WNewMonsterAI:run nine", self.m_tBoss:getId(), self.m_tBoss:getBattleId(), self.m_nCurStatus, self.m_nAiActionCount)
    if self:isCanControl() == true and self.m_nCurStatus == -1 then
        ...
        -- WZLog("WNewMonsterAI:nextRound check",self.m_nAiActionCount)
