--WBattleMachineRoundStartCollisionHurt.lua
--@brief    回合开始伤害机关
--@date     2016/11/26
--@note     

WBattleMachineRoundStartCollisionHurt = {
...
--@return   新建的表实例对象
function WBattleMachineRoundStartCollisionHurt:new(type,battleId,templateId,camp,bronPos)
    WZLog("WBattleMachineRoundStartCollisionHurt:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineRoundStartCollisionHurt,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineRoundStartCollisionHurt })
    tNewObj.m_nBattleId = battleId
    tNewObj.m_nPlayerId = templateId
    tNewObj.m_nCamp = camp
    tNewObj.m_nMonsterType = type
    
    tNewObj.m_bIsInCtb = false--不参与ctb轮转
    tNewObj.m_bOffHurt = true --不参与伤害计算
    tNewObj.m_bOffCollision = true --不参与子弹碰撞计算

    tNewObj:checkAniFileIndex()
    ...
    tNewObj:setPosition(Vector2:create(firstPos.x, firstPos.y + WBattleMachineRoundStartCollisionHurt.m_nAnimaOffY))
    
    return tNewObj
end


function WBattleMachineRoundStartCollisionHurt:checkAniFileIndex()
    self.m_tAniFileIndex =  "machine_7001"
    if self.m_nMonsterType == MonsterType.BOSS_CAGE then
        self.m_tAniFileIndex =  "machine_7001"
    elseif self.m_nMonsterType == MonsterType.BOSS_MAGMA then
        self.m_tAniFileIndex =  "machine_8001"
    elseif self.m_nMonsterType == MonsterType.BOSS_POISON then
        self.m_tAniFileIndex =  "machine_9001"
    end
end
...
--@brief 获得配置id
function WBattleMachineRoundStartCollisionHurt:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineRoundStartCollisionHurt:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
    ...
    -- WZLog("WBattleMachineRoundStartCollisionHurt:update",tostring(self:getAnimation():isCurrentAnimationDone()),self:getNormalAnimationName())
    if self:getAnimation():isCurrentAnimationDone() == true and self.m_bAutoStandAction then
        -- WZLog("WBattleMachineRoundStartCollisionHurt:update-2")
...
--@return   #2:碰撞的人物列表
function WBattleMachineRoundStartCollisionHurt:checkCollision()
    WZLog("WBattleMachineRoundStartCollisionHurt:checkCollision")
    ...
    if #collisionList > 0 then
        WZLog("WBattleMachineRoundStartCollisionHurt:checkCollision-two",#collisionList)
        BattleMethod:waitForSkillHurt(self,collisionList)
    end
end

--@brief    初始化基础动画
function WBattleMachineRoundStartCollisionHurt:initAnim()
    WZLog("WBattleMachineRoundStartCollisionHurt:initAnim",self:getConfig().aniFileId)
...
--出现动画
function WBattleMachineRoundStartCollisionHurt:playAppearAction()
    self:getAnimation():play(self:getAppearAnimationName(), false)
end

--@brief 设置碰撞
function WBattleMachineRoundStartCollisionHurt:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineRoundStartCollisionHurt:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineRoundStartCollisionHurt:setScale(scale)
...
function WBattleMachineRoundStartCollisionHurt:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineRoundStartCollisionHurt:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineRoundStartCollisionHurt:getAnimation()
    return self.m_anim
end

function WBattleMachineRoundStartCollisionHurt:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or 1
end

--@brief    添加碰撞矩形
function WBattleMachineRoundStartCollisionHurt:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineRoundStartCollisionHurt:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineRoundStartCollisionHurt:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineRoundStartCollisionHurt:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineRoundStartCollisionHurt:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineRoundStartCollisionHurt:setPosition(tPos)
...
--@brief 
function WBattleMachineRoundStartCollisionHurt:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineRoundStartCollisionHurt:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineRoundStartCollisionHurt:getConfig()
...
function WBattleMachineRoundStartCollisionHurt:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineRoundStartCollisionHurt:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineRoundStartCollisionHurt:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineRoundStartCollisionHurt:getShootPos(target)
...
--@brief
--@return
function WBattleMachineRoundStartCollisionHurt:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineRoundStartCollisionHurt:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineRoundStartCollisionHurt:getAppearAnimationName()
    return self:getAnimationName("skill")
end

--@brief    获取待机动画
function WBattleMachineRoundStartCollisionHurt:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineRoundStartCollisionHurt:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineRoundStartCollisionHurt:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineRoundStartCollisionHurt:setDead(bDead, note)
    WZLog("WBattleMachineRoundStartCollisionHurt:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineRoundStartCollisionHurt:doSuicide()
...
--@brief 获取死亡标记
function WBattleMachineRoundStartCollisionHurt:isDead()
    return self.m_bIsDead
end

--@brief 死亡过程
function WBattleMachineRoundStartCollisionHurt:updateDeadAct()
    if self:isServerDead() and self:getAnimation():isCurrentAnimationDone() == true then
        WZLog("WBattleMachineRoundStartCollisionHurt:updateDeadAct")
        WBattleGlobal:getCurrent():removeMachine(self.m_nBattleId)
    end
end
--@--brief wbattleglobal调用
function WBattleMachineRoundStartCollisionHurt:destroy()
    WZLog("WBattleMachineRoundStartCollisionHurt:destroy")
    if self:getAnimation():getAnimNode() then
        self:getAnimation():getAnimNode():removeFromParentAndCleanup(false)
        WZLog("WBattleMachineRoundStartCollisionHurt:release")
...
function WBattleMachineRoundStartCollisionHurt:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineRoundStartCollisionHurt:_init()
...
--@brief 解析动画名字
function WBattleMachineRoundStartCollisionHurt:_setAnimConfig()
...
function WBattleMachineRoundStartCollisionHurt:_addBossName()
...
--@brief 
function WBattleMachineRoundStartCollisionHurt:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineRoundStartCollisionHurt:getPlayerNameIcon()
    return self.m_tBossName
end

--@brief    清理人物名称信息的显示
function WBattleMachineRoundStartCollisionHurt:clearPlayerNameIcon()
   if self.m_tBossName ~= nil then
...
function WBattleMachineRoundStartCollisionHurt:addCollisionCharas(tCharas)
