--WBattleMachineTreatTotem.lua
--@brief    场景机关
--@date     2015/04/06
--@note     场景机关

WBattleMachineTreatTotem = {
...
--@return   新建的表实例对象
function WBattleMachineTreatTotem:new(battleId,templateId,camp,bronPos)
    WZLog("WBattleMachineTreatTotem:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineTreatTotem,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineTreatTotem })
...
--@brief 读取怪物模板配置
function WBattleMachineTreatTotem:parseMonsterData()
    local monsterData =  BossData["id_"..self.m_nPlayerId]
    if self.m_nCamp ~= WBattleGlobal:getCurrent():getMyHero():getCamp() then
         self.m_tAniFileIndex =  "machine_1003"
    else
        self.m_tAniFileIndex =  "machine_1002"
    end
...
--@brief 获得配置id
function WBattleMachineTreatTotem:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineTreatTotem:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
...
--@brief ctb刷新
function WBattleMachineTreatTotem:updateBuffByCTB(dt, updateCTB_time)
    if self:isDead() then
        return
    end
...
function WBattleMachineTreatTotem:doEffect()
    WZLog("WBattleMachineTreatTotem:doEffect")
    ...
    if WBattleGlobal:getCurrent():isHostControl() then
        WZLog("WBattleMachineTreatTotem:doEffect send")
...
--@brief    初始化基础动画
function WBattleMachineTreatTotem:initAnim()
    WZLog("WBattleMachineTreatTotem:initAnim")
...
--@brief 设置碰撞
function WBattleMachineTreatTotem:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineTreatTotem:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineTreatTotem:setScale(scale)
...
function WBattleMachineTreatTotem:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineTreatTotem:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineTreatTotem:getAnimation()
    return self.m_anim
end

function WBattleMachineTreatTotem:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or 1
end

--@brief    添加碰撞矩形
function WBattleMachineTreatTotem:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineTreatTotem:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineTreatTotem:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineTreatTotem:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineTreatTotem:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineTreatTotem:setPosition(tPos)
...
--@brief 
function WBattleMachineTreatTotem:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineTreatTotem:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineTreatTotem:getConfig()
...
function WBattleMachineTreatTotem:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineTreatTotem:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineTreatTotem:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineTreatTotem:getShootPos(target)
...
--@brief
--@return
function WBattleMachineTreatTotem:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineTreatTotem:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineTreatTotem:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineTreatTotem:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineTreatTotem:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineTreatTotem:setDead(bDead, note)
    WZLog("WBattleMachineTreatTotem:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineTreatTotem:doSuicide()
...
--@brief 获取死亡标记
function WBattleMachineTreatTotem:isDead()
    return self.m_bIsDead
end

--@brief 死亡过程
function WBattleMachineTreatTotem:updateDeadAct()
    ...
    -- WZLog("WBattleMachineTreatTotem:updateDeadAct 00000", type(bIsServerDead), type(bIsDone), type(bHdDone), bIsServerDead, bIsDone, bHdDone)
...
function WBattleMachineTreatTotem:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineTreatTotem:_init()
...
--@brief 解析动画名字
function WBattleMachineTreatTotem:_setAnimConfig()
...
function WBattleMachineTreatTotem:_addBossName()
...
--@brief 
function WBattleMachineTreatTotem:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineTreatTotem:getPlayerNameIcon()
    return self.m_tBossName
end

--@brief    清理人物名称信息的显示
function WBattleMachineTreatTotem:clearPlayerNameIcon()
   if self.m_tBossName ~= nil then
