...
function ProtocolProcessorSceneBossBattle:regAll()
    ...
    --@brief	获取房间状态成功
    self:regProtocolCallbackFunction( Protocol.MAIN_WORLDBOSS, Protocol.WORLDBOSS_GetRoomStateOk, "ProtocolProcessorSceneBossBattle:parse_WORLDBOSS_GetRoomStateOk", "iiivivsviiiiiiiiiii")
    --@brief	发送结算信息
    self:regProtocolCallbackFunction( Protocol.MAIN_WORLDBOSS, Protocol.WORLDBOSS_SendSettlementInfo, "ProtocolProcessorSceneBossBattle:parse_WORLDBOSS_SendSettlementInfo", "iiibis")
    --@brief	发送当前回合的信息成功
...
--@brief	获取房间状态成功
function ProtocolProcessorSceneBossBattle:parse_WORLDBOSS_GetRoomStateOk(mapId, bossBloodMax, bossBloodCurrent,rankPlayerId, rankPlayerName, rankHurt, hurt, cdTime, accelerateCost, inspire, bossLevel, myRank, dimaCDTime, goldCDTime,bossIsDead,openTime)
    -- mapId : 地图id
...
function ProtocolProcessorSceneBossBattle:parse_BOSSMAPBATTLE_AddBuff(userId,playerId, buffId)
    ...
    -- table.insert(WBattleGlobal:getCurrent().m_tBuffAddPlayerList, {round=round,playerId=playerId,buffId=buffId,userId=userId})
end
...
function ProtocolProcessorSceneBossBattle:parse_BOSSMAPBATTLE_CanStartCurRound(battleId, wind,  currentId, attackRate, battleRand,playerIds, nowHP, nowSP,oldCTB,newCTB,updateCTB_time)
	...
	--         WZLog("ProtocolProcessorSceneBossBattle:parse_BATTLE_CanStartCurRound zero-1", WBattleGlobal:getCurrent().m_nTurnTimes)

	--         for i, buffPlayer in pairs (WBattleGlobal:getCurrent().m_tBuffAddSkillPlayerList) do
	--             WZLog("ProtocolProcessorSceneBossBattle:parse_BATTLE_CanStartCurRound one-1", i, Serialize(buffPlayer))

	--             local hero = WBattleGlobal:getCurrent():getCharacterWithId(buffPlayer.playerId)

 --                if hero and hero:isDead() ~= true and hero:getHp() > 0 and buffPlayer.isMapBuff ~= true then
 ...
 --                                    WZLog("ProtocolProcessorSceneBossBattle:parse_BATTLE_CanStartCurRound one-2",buff.m_nID)
 --                                    break
 --                                end
 --                            end
 --                        end
 ...
 --                                buff:removeAnime()
 --                                hero.m_tBuffChangeStateList[id] = nil
 --                                WZLog("ProtocolProcessorSceneBossBattle:parse_BATTLE_CanStartCurRound one-3",buff.m_nID)
 --                                break
 --                            end
 --                        end
 --                    end
 --                end
	...
	--     if WBattleGlobal:getCurrent().m_tBuffAddPlayerList then
	--         WZLog("ProtocolProcessorSceneBossBattle:parse_BATTLE_CanStartCurRound zero-0", WBattleGlobal:getCurrent().m_nTurnTimes,WBattleGlobal:getCurrent().m_nBuffAddRound, WBattleGlobal:getCurrent().m_nBuffAddId)

	--         for i, buffPlayer in pairs (WBattleGlobal:getCurrent().m_tBuffAddPlayerList) do
	--             WZLog("ProtocolProcessorSceneBossBattle:parse_BATTLE_CanStartCurRound one-0", i, Serialize(buffPlayer))
