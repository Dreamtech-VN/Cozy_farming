...
--@brief	发送当前回合的信息
function ProtocolProcessorBattleInterface:send_BATTLE_SendCurRoundInfo(battleId, turn, playerIds, postionX, postionY, angle, face, explodePlayerId, explodeSkillId, explodePosX, explodePosY, battleInfo)
	...
	if self:_getIsUseNormalProtocol() then
		ProtocolProcessorSceneBattle:send_BATTLE_SendCurRoundInfo(battleId, turn, playerIds, postionX, postionY, angle, face, explodePlayerId, explodeSkillId, explodePosX, explodePosY, battleInfo)
	else
		ProtocolProcessorSceneBossBattle:send_BOSSMAPBATTLE_SendCurRoundInfo(battleId, turn, playerIds, postionX, postionY, angle, face, explodePlayerId, explodeSkillId, explodePosX, explodePosY, battleInfo )
	end
end
...
--@brief	角色移动
function ProtocolProcessorBattleInterface:send_BATTLE_PlayerMove(battleId, playerId, movecount, movestep, curPositionX, curPositionY)
	...
	if WBattleGlobal:getCurrent():isRelink() or WBattleGlobal:getCurrent():isAudience() or WBattleGlobal:getCurrent():isSingleStage() or WBattleGlobal:getCurrent().m_bIsWaitSynchronousBattle then WZLog("正在重连或观战或单人副本,不能发协议") return end
	if self:_getIsUseNormalProtocol() then
		ProtocolProcessorSceneBattle:send_BATTLE_PlayerMove(battleId, playerId, movecount, movestep, curPositionX, curPositionY )
	else
...
--@brief	使用技能和道具
function ProtocolProcessorBattleInterface:send_BATTLE_SkillEquip(battleId, playerId, item_id, targetIds, param, uniqueId)
	...
	if self:_getIsUseNormalProtocol() then
		param = param or ""
		ProtocolProcessorSceneBattle:send_BATTLE_SkillEquip(battleId, playerId, item_id, viPlayerId, param, uniqueId or 0)
	else
		param = param or ""
...
--@brief	发射
function ProtocolProcessorBattleInterface:send_BATTLE_Shoot(battleId, playerId, speedx, speedy, leftRight, startX, startY, playerCount, playerIds, curPositionX, curPositionY, curPositionR, curPositionD, attackCount )
    ...
    --attackCount = nil
	if self:_getIsUseNormalProtocol() then
		ProtocolProcessorSceneBattle:send_BATTLE_Shoot(battleId, playerId, speedx, speedy, leftRight, startX, startY, playerCount, playerIds, curPositionX, curPositionY, curPositionR, curPositionD, attackCount)
	else
...
--@brief	发射完成
function ProtocolProcessorBattleInterface:send_BATTLE_Hurt(battleId, playerId, PlayerIds, hurtvalue, distance)
	if WBattleGlobal:getCurrent():isReplayGame() then
		return
	end
	...
	if WBattleGlobal:getCurrent():checkCheat() ~= true then
		if self:_getIsUseNormalProtocol() then
			ProtocolProcessorSceneBattle:send_BATTLE_Hurt(battleId, playerId, PlayerIds, hurtvalue, distance )
		else
			ProtocolProcessorSceneBossBattle:send_BOSSMAPBATTLE_Hurt(battleId, playerId, PlayerIds, hurtvalue, distance)
		end
	end
end
...
function ProtocolProcessorBattleInterface:send_BATTLE_LoadingPercent(battleId, currentPlayerId, percent)
	WZLog("send_BATTLE_LoadingPercent",battleId, playerId, currentPlayerId, percent)
	if WBattleGlobal:getCurrent():isReplayGame() then
		return
	end
	if WBattleGlobal:getCurrent():isRelink() or WBattleGlobal:getCurrent():isAudience() or WBattleGlobal:getCurrent():isSingleStage() or WBattleGlobal:getCurrent().m_bIsWaitSynchronousBattle then WZLog("正在重连或观战或单人副本,不能发协议") return end
