		...
		if self:isOutOfScene()==false then
			if self:isCanControl() == true then
				if not self:isDead() and SceneBattle:getBattleLoop():getBattleStatus() ~= BattleLoop.S_PLAYER_FLY then
					ProtocolProcessorBattleInterface:send_BATTLE_OutOfScene(WBattleGlobal:getCurrent().m_tMakePairOk.battleId, self:getBattleId() ,WBattleGlobal:getCurrent():getCurrentCharacter():getBattleId(), self:getType(), WBattleGlobal:getCurrent():getCurrentCharacter():getType() )
