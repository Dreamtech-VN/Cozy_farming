--WndFriendGiftData.lua
--@brief	WndFriendGift的数据模块
--@date		2016/03/31
--@author	Tianxiang_Xu
--@note		好友礼物列表
...
function WndFriendGift:_unInit()
	self.m_root = nil
...
--@note		请仅用此方法创建场景
function WndFriendGift:createElement()
	local element = WZUISystem:getInstance():createElement("WndFriendGift")
	assert(element, "WndFriendGift element create failed!")
	self:_init()
	return element
end

--@brief 	设置可作为礼物数据列表
function WndFriendGift:setData()
	-- body
	local tGiftList = CacheCenter:getFriendGiftList()
	WZLog("WndFriendGift:setData", Serialize(tGiftList))
...
function WndFriendGift:setFriendId(friendID, friendliness)
	-- body
...
--@brief 	更新好友度
--@param 	friendliness: 好友度
function WndFriendGift:resetFriendliness(friendID, friendliness)
	-- body
