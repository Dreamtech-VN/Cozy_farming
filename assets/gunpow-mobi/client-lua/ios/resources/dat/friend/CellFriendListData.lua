...
function CellFriendList:_unInit()
	self.m_root = nil
	self.m_tFriend = nil 
	self.m_tBackFun = nil 
...
--@note		请仅用此方法创建场景
function CellFriendList:createElement()
	local tNewObj = self:_new()
	assert(tNewObj, "CellFriendList table create failed!")
	tNewObj:_init()

	local element = WZUIContainer:create()
    element:setUseAbsSize(true)
    element:setName("__CellFriendList")
...
function CellFriendList:setCellData(tFriend, nSelect)
	self.m_tFriend = {}
...
--@param	tCell:表名
--@param	backFun:回调函数
function CellFriendList:setBackFun(tCell , backFun)
...
--@brief 	界面类型
function CellFriendList:setUIIndex( index )
	self.m_nInterface = index
end

--@brief 	设置是否为陌生人
function CellFriendList:setIsStranger( isStranger )
