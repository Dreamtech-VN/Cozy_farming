...
function WndFriends:_addTop()
    ...
    tNewObj:setTopData("ui/common/common_icon_haoyoutitle.png", WndFriends, WndFriends.onCloseClick, true, false, false, "WndFriends")
...
function WndFriends:onEnterTransitionDidFinish(element)
    ...
    local checkTheme6 = GetElement(self.m_root,"checkTheme6_WndFriends",WZUICheckBox)
    checkTheme4_WndFriends:setRelativePosition(GlobalMethod:ccp(0.5, 0.85))
    checkTheme2_WndFriends:setRelativePosition(GlobalMethod:ccp(0.5, 0.55))
    checkTheme3_WndFriends:setRelativePosition(GlobalMethod:ccp(0.5, 0.25))
    checkTheme5_WndFriends:setRelativePosition(GlobalMethod:ccp(0.5, -0.05))
    checkTheme6:setRelativePosition(GlobalMethod:ccp(0.5, -0.35))
    ...
    conCheck2_WndFriends:setRelativePosition(GlobalMethod:ccp(0.5, 0.55))
    local conCheck3_WndFriends = GetElement(self.m_root, "conCheck3_WndFriends", WZUIContainer)
    conCheck3_WndFriends:setRelativePosition(GlobalMethod:ccp(0.5, 0.25))
    ...
    conCheck5_WndFriends:setRelativePosition(GlobalMethod:ccp(0.5, -0.05))
    local conCheck6 = GetElement(self.m_root, "conCheck6_WndFriends", WZUIContainer)
    conCheck6:setRelativePosition(GlobalMethod:ccp(0.5, -0.35))
    ...
    conCheck2Sel_WndFriends:setRelativePosition(GlobalMethod:ccp(0.568, 0.55))
    local conCheck3Sel_WndFriends = GetElement(self.m_root, "conCheckTheme3Sel_WndFriends", WZUIContainer)
    conCheck3Sel_WndFriends:setRelativePosition(GlobalMethod:ccp(0.568, 0.25))
    local conCheck4Sel_WndFriends = GetElement(self.m_root, "conCheckTheme4Sel_WndFriends", WZUIContainer)
    conCheck4Sel_WndFriends:setRelativePosition(GlobalMethod:ccp(0.568, 0.85))
    local conCheck5Sel_WndFriends = GetElement(self.m_root, "conCheckTheme5Sel_WndFriends", WZUIContainer)
    conCheck5Sel_WndFriends:setRelativePosition(GlobalMethod:ccp(0.568, -0.05))
    local conCheck6Sel = GetElement(self.m_root, "conCheckTheme6Sel_WndFriends", WZUIContainer)
    conCheck6Sel:setRelativePosition(GlobalMethod:ccp(0.568, -0.35))

    if not CheckButtonShow(83) then
        checkTheme5_WndFriends:setVisible(false)
        conCheck5_WndFriends:setVisible(false)
        --如果邀请不显示，则黑名单上移
        checkTheme6:setRelativePosition(GlobalMethod:ccp(0.5, -0.05))
        conCheck6:setRelativePosition(GlobalMethod:ccp(0.5, -0.05))
        conCheck6Sel:setRelativePosition(GlobalMethod:ccp(0.568, -0.05))
    end
    --End Add 
    if CacheCenter.m_nDailyMark == 1 then
        self:_setCheckBoxSelVisible(false, false, true, false, false, false)
    ...
    elseif CacheCenter.m_nInviteMark > 0 and checkTheme5_WndFriends:isVisible() then
        self:_setCheckBoxSelVisible(false, false, false, false, true, false)
        local tbconFriend = WZUITableContainer:luaTo(self.m_root:getChildElement("tbconFriend_WndFriends"))
        tbconFriend:setVisible(false)

        self:clear()
        self.m_nCheckIndex = INVITE_INDEX
        self:_showButtonByIndex(INVITE_INDEX)
        self:_showFriendCount(false)     --显示好友数量
        ...
        --获取相关数据
        self:setInviteTaskData()
    elseif CacheCenter:getFriendCount()>0 and self.m_nOpenLayerIndex == nil then 
        self:_setCheckBoxSelVisible(false, true, false, false, false, false)
        ...
        self:_showRecvGift(false)
        self:_showButton(true,true, true)

        self.m_bIsResetFriends = true
        self:createLoading()
        ProtocolProcessorWndFriends:send_FRIEND_GetFriendInfoList( )
	else 
        self:_setCheckBoxSelVisible(false, false, false, true, false, false)
...
function WndFriends:onClickRanking(element)
    ...
    self:_setCheckBoxSelVisible(true, false, false, false, false, false)
    ...
    elseif tag == FRIEND_INDEX then--审批界面按钮回调
        ...
        if wnd then
            ...
            for i,data in pairs(tTempFriend) do 
                if data.type == 1 then
                    if data.serverId ~= nil and data.serverId == CacheCenter:getPlayerInfo().serverId then
...
function WndFriends:onClickFriend(element)
    ...
    WZLog("***** WndFriends:onClickFriend *****")
    self:_setCheckBoxSelVisible(false, true, false, false, false, false)
...
function WndFriends:onFriendFind(element)
    ...
    self.m_nCurPageIndex = 1
    self.m_nFriendsTableIndex = 0
    GetElement(self.m_root, "tbconFriend_WndFriends", WZUITableContainer):cleanTable()
...
function WndFriends:onClickBlacklist(element)
    -- body
    if self.m_nCheckIndex == BLACKLIST_INDEX then
        return 
    end
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN2)

    self:_setCheckBoxSelVisible(false, false, false, false, false, true)

    self:clear()
    self.m_nCheckIndex = BLACKLIST_INDEX
    self:_showButtonByIndex(BLACKLIST_INDEX)
...
function WndFriends:onShowFriend(element, delta)
    ...
    for i = 1, self.m_nCurNeedLoadNum do
        if self.m_tFriend[self.m_nCurLoadIndex] then
...
function WndFriends:onShowInvite()
    ...
    if self.m_sMyInviteCode then
        local sCodeFormat = [[<T C="255,227,116" S="24" P="1">%s</T><T C="255,236,193" S="24" P="1">%s</T>]]
...
--@brief    邀请红点
function WndFriends:showInviteMark(bShow)
    if self.m_root == nil then
        return
    end
    
    local conMark = WZUIContainer:luaTo(self.m_root:getChildElement("conInviteMark_WndFriends"))
    conMark:setVisible(bShow)
end
...
--@brief    赠送礼物成功后处理函数
function WndFriends:giveGiftOk(result)
    --body
    WZLog("***** WndFriends:giveGiftOk *****")
    if WndFriends.m_root == nil then
        return
    end
    --关闭礼物界面
    -- if WndFriendGift.m_root then
    --     WindowManager:removeWindow(WndFriendGift.m_root, WndFriendGift, true)
    -- end
    if result > 0 then
...
function WndFriends:_updateInviteFriends()
    ...
    if self.m_tInviteFriends == nil or #self.m_tInviteFriends == 0 then
        ...
        if ProjConfig.LANGUAGE == "en" then
            ShowPanelNullTip(conLeft, LocalStrings.INVITE_CODE_ATT4,nil,nil,22,nil,2)
        elseif ProjConfig.LANGUAGE == "pt" or ProjConfig.LANGUAGE == "es" then
            ShowPanelNullTip(conLeft, LocalStrings.INVITE_CODE_ATT4,nil,nil,20,nil,2)
...
function WndFriends:_showButton(bShowA,bShowB,bShowC)	
    ...
    if bShowC then
        ...
        btn1:setRelativePosition(GlobalMethod:ccp(0.09,0.5))
    else
...
function WndFriends:_showMultiLanguage()
    ...
    self:_showTTFText("txtCheck5_WndFriends",LocalStrings.INVITE)--邀请
...
--@brief    解除蜜友关系成功处理
function WndFriends:removeBestFriendSuccess()
    --body
    if self.m_root == nil then return end 
    ...
    while cellElement do
        ...
        if cellItem then
            ...
            if cellObj then
                local nFriendId = cellObj:getFriendId() 
                if self.m_tClickFriendData.id == nFriendId then 
                    local tData = cellObj:getFriendData()
                    tData.bBestFriend = 0 
                    cellObj:resetFriendData(tData)
                    break 
                end
            end
        end
...
function WndFriends:_setCheckBoxSelVisible(bCommunityVisible, bFriendsVisible, bDynamicVisible, bRecommendVisible, bInvideVisible, bBlackListVisible)
    -- body
    local conFriendList = GetElement(self.m_root,"conFriendList_WndFriends",WZUIContainer)
    ...
    if bInvideVisible == true then
        ...
        GetElement(self.m_root, "conMyCode_WndFriends", WZUIContainer):setVisible(true)

        addWeChatBtn(conBottomBtn, 1, GlobalMethod:ccp(0.45,0.5))
    ...
    --显示本服好友和跨服好友复选框
    GetElement(self.m_root, "conCheckBox_WndFriends", WZUIContainer):setVisible(bFriendsVisible)
    ...
    GetElement(self.m_root, "conCheckTheme6Sel_WndFriends", WZUIContainer):setVisible(bBlackListVisible)
    GetElement(self.m_root, "txtBlacklistAtt_WndFriends", WZUILabelTTF):setVisible(bBlackListVisible)
    GetElement(self.m_root, "conMidBottom_WndFriends", WZUIContainer):setVisible(not bBlackListVisible)
    GetElement(self.m_root, "editFriendFind_WndFriends", WZUIEditBox):setText("")
    GetElement(self.m_root, "btnCancelFind_WndFriends", WZUIButton):setVisible(false)
    self.m_bIsFindFriend = false
end
...
function WndFriends:_adaptLanguage_en()
    ...
    local txtCheck6 = GetElement(self.m_root, "txtCheck6_WndFriends", WZUILabelTTF)
    if txtCheck6 then
        txtCheck6:setFontSize(20)
    end
...
function WndFriends:_adaptLanguage_pt(  )
    ...
    local txtCheck6 = GetElement(self.m_root, "txtCheck6_WndFriends", WZUILabelTTF)
    txtCheck6:setFontSize(18)
    txtCheck6:setRelativePosition(GlobalMethod:ccp(0.55,0.5))
...
function WndFriends:_adaptLanguage_vn()
    ...
    GetElement(self.m_root,"txtNum_WndFriends",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.36,0.5))
    -- for i=1,2 do
    --     GetElement(self.m_root,"txtCheckBox"..i.."_WndFriends",WZUILabelTTF):setFontSize(19)
    -- end
end
