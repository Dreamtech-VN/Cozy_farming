...
function WndBottomBar:getTime(type,time)
    ...
    if SceneCity.m_root then
        -- local state = GlobalGame.g_tRedPointList["EquipLove"] or GlobalGame.g_tRedPointList["btnPet"]
        -- local btnName = GetElementWithoutAssert(SceneCity.m_root, "building11" , WZUIImage)
        -- SceneCity:setRedPoint(btnName, nil, GlobalMethod:ccp(100,40), nil)

        SceneCity:updateRedDotBuilding("btnPet", GlobalGame.g_tRedPointList["btnPet"], GlobalMethod:ccp(100,40))
        WndSummonEntrance:updateRedPoint(nil, GlobalGame.g_tRedPointList["btnPet"])
    end
end
...
function WndBottomBar:_update()
    ...
    local w,h = 264,60 * row + 20 * (row +1) + 4

    self.m_nMoveTime = h / (264/0.15)
    ...
    imgNor:setFile("ui/common/common_scale9_di68.png")
    ...
    for i,v in ipairs(self.m_tBtnsInfo) do
        ...
        if conBtn then
            if isOpen then
                ...
                local offsetX = 0.8 - 0.3 * (column-1)
...
--@brief    点击展开按钮后的响应方法
function WndBottomBar:onClickMore(element)
    WZLog("WndBottomBar:onClickMore")
...
function WndBottomBar:endMoveVerticalBar(sender, levelUp, isTrailerAnim)
    ...
    elseif isFinish19 ~= true and CheckButtonOpen(28,false) then
        ...
        tryTeach = true
    elseif isFinish43 ~= true and finishStep43 >= 0 and CacheCenter:getPlayerInfo().level == 24 then
        isTeach = TeachGroup1:startGroupLevelUp(levelUp, false, nil, isTrailerAnim, {42,2, self.m_root})
        tryTeach = true
    elseif isFinish42 ~= true and finishStep42 >= 0 and CacheCenter:getPlayerInfo().level == 19 then
