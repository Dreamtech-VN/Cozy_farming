...
function CellTrainingReward:_init()
...
function CellTrainingReward:_unInit()
	self.m_root = nil
	self.m_tItem = nil			--数据列表
	self.m_tBackFun = nil 		--回调函数列表
	self.m_tStone = nil 		--石头列表
...
--@note		请仅用此方法创建场景
function CellTrainingReward:createElement()
	local tNewObj = self:_new()
	assert(tNewObj, "CellTrainingReward table create failed!")
	tNewObj:_init()
	local element = WZUISystem:getInstance():createElement("CellTrainingReward")
	assert(element, "CellTrainingReward element create failed!")
...
--@note		判断物品项是否有物品
function CellTrainingReward:isItemNil()
...
--@nType=18:衣橱时装格子
--@nType=19:圣光系统格子
function CellTrainingReward:setCellTrainingReward(tData,nType)
    self.m_tItem = {}
	self.m_tItem = tData
    self.m_nType = nType
...
--@param tCell:父节点
--@param backFun：回调函数
function CellTrainingReward:setItemClickFun(tCell,backFun)
...
--@brief	设置物品Id
function CellTrainingReward:setItemId(id)
	self.m_nId = id
end

--@brief	获取物品Id
function CellTrainingReward:getItemId()
	return self.m_nId
end

--@brief	设置来源的Tag
function CellTrainingReward:setFromTag(tag)
	self.m_nFromTag = tag
end

--@brief	获取来源的Tag
function CellTrainingReward:getFromTag()
	return self.m_nFromTag
end

--@brief	设置使用状态
function CellTrainingReward:setUseData(bUse)
	self.m_tItem.isUse = bUse
end

--@brief	获取数据表
--@return   格子数据表
function CellTrainingReward:getData()
    return self.m_tItem
end

function CellTrainingReward:setTag(tag)
	self.m_nTag = tag
end

function CellTrainingReward:getTag()
	return self.m_nTag
end
...
--@return	新建的表实例对象
function CellTrainingReward:_new( )
	local tNewObj = {}
...
--@brief	宝石列表
function CellTrainingReward:_setStoneData(tItemData)
    ...
    if tData ~= nil then
        ...
        if tData.hpStone and tData.hpStone > 0 then--特殊宝石等级
