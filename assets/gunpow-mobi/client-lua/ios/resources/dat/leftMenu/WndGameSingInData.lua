...
function WndGameSingIn:GetSingInDataOK( days, sign ,vipSign , nCurrentTime , monthReward, reissueTimes, todaySignNum)
	...
	if self.reissueTimes > 4 then
		self.reissueTimes = 4
	end
	local t = os.date("*t",self.m_nTime)
  	self.m_currentDay = t.yday
  	self.m_currentYear = t.year
	WZLog("WndGameSingIn:GetSingInDataOK == ", days, sign, vipSign, todaySignNum, self.m_currentDay)
