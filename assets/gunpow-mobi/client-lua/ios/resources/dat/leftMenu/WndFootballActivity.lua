...
--@breif   对列表进行刷选
function WndFootballActivity:_sortListItem( ItemCount )
	local m_tNewItem = {}
	local m_tNormalItem = {}
	for i=1,ItemCount do
...
function WndFootballActivity:_updateTitle( )
    ...
    elseif self.m_nCurrentSelectTypeId == g_tGameActivityTypes.ACTIVITY_FOOTBALL_QUIZ then
        imgTitle:setFile("ui/football/football_text_002.png")
    end
end
...
-- function WndFootballActivity:sendProtocolGetReward(activityId,rewardId,dayIndex)
--     WZLog("WndFootballActivity:sendProtocolGetReward =",activityId,rewardId,dayIndex)
...
-- function WndFootballActivity:updateImage(closeStats)
--     WZLog("WndFootballActivity:updateImage ",self.m_nCurrentSelectTypeId)
--     local imgPeople = GetElement(self.m_root,"imgPeople_WndFootballActivity",WZUIImage)
...
-- function WndFootballActivity:onClickGoWishing(element)
--     WZLog("WndFootballActivity:onClickGoWishing")
--     SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--@brief    点击前往按钮回调
-- function WndFootballActivity:onClickGoTo(element)
--     WZLog("WndFootballActivity:onClickGoTo")
--     SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
