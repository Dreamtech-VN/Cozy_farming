...
function WndLoveLottery:onEnter(element)
    ...
    self.m_tGiftName = tempTable.name
	self.m_tGiftIcon = tempTable.icon
	self.m_tGiftNum = tempTable.num
	self.m_tGiftId = tempTable.id

	WZLog("WndLoveLottery:onEnter loveItems=",Serialize(tempTable))
	self.m_ncount =1
    self:_updateTable()
    self:_setTime()
...
--@brief    弹窗动画完成后的回调
function WndLoveLottery:actionCallback_close(element,data)
...
--@brief    弹窗动画完成后的回调
function WndLoveLottery:actionCallback(element, data)
...
function WndLoveLottery:setBtnLotteryEnable(bTouchEnable)
	local btnLottery = self.m_root:getChildElement("btnLottery_WndLoveLottery")
...
function WndLoveLottery:onClickRaffle(element)
	WZLog("WndLoveLottery:onClickRaffle loveNum ")
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
function WndLoveLottery:sureToUseDiamondInstead()
    ...
    ProtocolProcessorSceneLottery:send_LOTTERY_ReceiveReward(rewardType)
end
...
function WndLoveLottery:_adaptLanguage_vn(  )
    ...
    txtDrawTip:setDimensions(GlobalMethod:CCSize(170))
end

function WndLoveLottery:_adaptLanguage_en(  )
	local txtDrawTip = GetElement(self.m_root,"txtDrawTip_WndLoveLottery",WZUILabelTTF)
    txtDrawTip:setScale(0.7)
    txtDrawTip:setDimensions(GlobalMethod:CCSize(160,0))
end

function WndLoveLottery:_adaptLanguage_tr()
    local txtTenT = GetElement(self.m_root,"txtTenT_WndLoveLottery",WZUILabelTTF)
    txtTenT:setScale(0.8)
...
function WndLoveLottery:_adaptLanguage_es(  )
	local txtTenT = GetElement(self.m_root,"txtTenT_WndLoveLottery",WZUILabelTTF)
	txtTenT:setDimensions(GlobalMethod:CCSize(130,0))
	txtTenT:setScale(0.6)

    local txtDrawTip = GetElement(self.m_root,"txtDrawTip_WndLoveLottery",WZUILabelTTF)
    txtDrawTip:setScale(0.6)
    txtDrawTip:setDimensions(GlobalMethod:CCSize(180,0))
end
