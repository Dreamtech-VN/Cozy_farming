...
--@brief    点击觉醒之晶按钮回调
function CellWakeupTask:onClickWakeupCoin(element)
    -- body
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
function CellWakeupTask:_setTaskDetail()
    ...
    local tFinishPoint = {GlobalMethod:ccp(0.98,0.633051), GlobalMethod:ccp(0.98,0.376554), GlobalMethod:ccp(0.98,0.108757)}
    local tUnFinishPoint = {GlobalMethod:ccp(0.88,0.633051), GlobalMethod:ccp(0.88,0.376554), GlobalMethod:ccp(0.88,0.108757)}
    --展示图
        ...
        if txtTarget then
            if tTaskList[i].target == 1 then
                ...
                if tonumber(tTaskList[i].target) <= tonumber(tTaskList[i].complete) then
                    ...
                    txtTarget:setColor(GlobalMethod:ccc3(255,236,193))
                    if tTaskList[i].basicInfo.script[1][1] == 0 then
                        txtTarget:setRelativePosition(tFinishPoint[i])
                    else
                        txtTarget:setRelativePosition(tUnFinishPoint[i])
...
function CellWakeupTask:_createBaseProperty()
    ...
    local sPropertyFormat = [[<T C="195,171,148" S="20" P="1">%s: </T><T C="99,255,95" S="20" P="1">+%d</T>]]
