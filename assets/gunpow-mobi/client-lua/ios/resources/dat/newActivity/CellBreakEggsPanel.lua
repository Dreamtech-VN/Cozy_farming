...
function CellBreakEggsPanel:_showCurRechargeInfo()
    ...
    if prgCurRecharge then 
        prgCurRecharge:setPercentage(math.floor(100 * math.mod(self.m_nTotalDiamond, self.m_needDiamond)/self.m_needDiamond))
    end
    --进度数字
    local txtCurValue = GetElement(self.m_root, "txtCurValue_CellBreakEggsPanel", WZUILabelTTF)
    if txtCurValue then 
        txtCurValue:setText(math.mod(self.m_nTotalDiamond, self.m_needDiamond) .. "/" .. self.m_needDiamond)
    end
    --充值概况
