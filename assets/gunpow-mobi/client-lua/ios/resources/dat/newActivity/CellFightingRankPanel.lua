...
function CellFightingRankPanel:showWindow()
    -- body
    if self.activityId == g_tGameActivityTypes.ACTIVITY_NEWSERVER_FIGHTINGRANK then
        self.m_nStartTime, self.m_nEndTime = WndGameActivity:getActivityTime(g_tGameActivityTypes.ACTIVITY_NEWSERVER_FIGHTINGRANK)
    elseif self.activityId == g_tGameActivityTypes.ACIVIITY_RECHARGERANK then
...
function CellFightingRankPanel:_showBK()
    ...
    if imgBK then 
        if self.activityId == g_tGameActivityTypes.ACTIVITY_NEWSERVER_FIGHTINGRANK then
            imgBK:setFile("ui/gameActivity/activity_pic_dyjzlzw.png")
        elseif self.activityId == g_tGameActivityTypes.ACIVIITY_RECHARGERANK then
...
function CellFightingRankPanel:_adaptLanguage_vn(  )
    GetElement(self.m_root, "txtTime_CellFightingRankPanel", WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.412,0.5))
end
