...
function CellCostActivityItem:setTargetInfo( txtMsg )
	...
	if conTitle_CellCostActivityItem~=nil then
		local start_pos = GlobalMethod:ccp(0.025,0.5)
		local str_info_1 = string.format(TitleMsg[1],m_tTargetArrays[index])
		local info_1_label = self:createTTF(str_info_1,start_pos,GlobalMethod:ccp(0,0.5),22,nil,GlobalMethod:ccc3(255,236,193),GlobalMethod:ccc3(105,65,46))
		conTitle_CellCostActivityItem:addChild(info_1_label,0,90)
        local txtSize = info_1_label:getLabelContentSize().width
		local pos = txtSize + 4
		local info_2_label = self:createTTF(tostring(txtMsg),GlobalMethod:ccp(start_pos.x+pos/480,start_pos.y),GlobalMethod:ccp(0,0.5),22,nil,GlobalMethod:ccc3(255,227,116),GlobalMethod:ccc3(105,65,46))
        ...
        if ProjConfig.LANGUAGE ~= "pt" then
    		local info_3_label = self:createTTF(TitleMsg[2],GlobalMethod:ccp(pos_end/480,start_pos.y),GlobalMethod:ccp(0,0.5),22,nil,GlobalMethod:ccc3(255,236,193),GlobalMethod:ccc3(105,65,46))
...
function CellCostActivityItem:setButtonState( state )
	...
	if state == -1 or state == 1 then 
        ...
        txt_buttonName:setStrokeColor(GlobalMethod:ccc3(79,60,48))
...
function CellCostActivityItem:createTTF(desc,pt,anchor,font,Align,color,sclolor)
    desc = desc or ""
    font = font or 22
...
function CellCostActivityItem:_adaptLanguage_pt(  )
    GetElement(self.m_root,"imgArrow_CellCostActivityItem",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.75,0.5))
end

function CellCostActivityItem:_adaptLanguage_th(  )
    GetElement(self.m_root,"imgArrow_CellCostActivityItem",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.55,0.5))
end

function CellCostActivityItem:_adaptLanguage_vn(  )
    GetElement(self.m_root,"imgArrow_CellCostActivityItem",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.65,0.5))
end
-------------------------------------语言适配End--------------------------------------------