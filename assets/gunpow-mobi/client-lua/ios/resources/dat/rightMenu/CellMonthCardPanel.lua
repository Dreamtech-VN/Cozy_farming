...
function CellMonthCardPanel:onRechargeEvent(element)
    ...
    WZLog("CellMonthCardPanel:onRechargeEvent nLimiteDays", nLimiteDays)
    --月卡剩余时间大于0不再操作
...
function CellMonthCardPanel:showWindow()
    ...
    WZLog("CellMonthCardPanel:showWindow", self.newRechargeType)
...
function CellMonthCardPanel:_staticText()
    ...
    if showPrice then
        txtCost:setRelativePosition(GlobalMethod:ccp(0.5,0.8))
    ...
    if self.m_nCardActivityState == 0 then 
        ...
        if self.m_nBuyCardTimes <= 0 then 
            ...
            txtCost:setRelativePosition(GlobalMethod:ccp(0.25,0.8))
