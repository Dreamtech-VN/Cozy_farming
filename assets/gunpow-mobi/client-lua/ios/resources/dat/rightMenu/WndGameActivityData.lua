...
--@note		请仅用此方法创建场景
function WndGameActivity:createElement()
...
--@brief    获得列表成功
function WndGameActivity:GetActivityListInfoOK( activityId, title, startTime, endTime, serverTime , types, type2)
    --body
    ...
    for i=1,#activityId do
        WZLog("WndGameActivity:GetActivityListInfoOK1", activityId[i], title[i],type2[i],types[i],serverTime,endTime[i])
                ...
                if types[i]>0 then 
                    if types[i] ~= g_tGameActivityTypes.ACTIVITY_FIRSTRECHARGE and types[i] ~= g_tGameActivityTypes.ACTIVITY_FIRSTRECHARGE2 then
                        if types[i] == g_tGameActivityTypes.ACTIVITY_RECHARGELEVEL then
                            ...
                            WZLog("WndGameActivity:GetActivityListInfoOK2", self.m_tListItem[index].title, startTime[i], endTime[i], types[i])
    ...
    WZLog("WndGameActivity:GetActivityListInfoOK 111", Serialize(self.m_tListItem))
    ...
    if temp ~= nil then 
        table.insert( self.m_tListItem, 1, temp)
    end
    self:_closeLoading()
...
function WndGameActivity:GetMonthFightingListOK_newServer(session, playerId, rank, worshipTimes, totlaWorshipTimes, fighting, name, faceId, headId, sex, level, vipLevel, headColor, bodyId, bodyColor, windId, crossServer, rewardRank, reward)
    ...
    if g_tGameActivityTypes.ACTIVITY_NEWSERVER_FIGHTINGRANK == self.m_nCurrentSelectTypeId or g_tGameActivityTypes.ACIVIITY_RECHARGERANK == self.m_nCurrentSelectTypeId
        ...
        or g_tGameActivityTypes.ACIVIITY_CROSS_CONSUMERANK == self.m_nCurrentSelectTypeId or g_tGameActivityTypes.ACTIVITY_FLOWER_LIST == self.m_nCurrentSelectTypeId then 
...
--@brief 获取奖励成功
function WndGameActivity:GetRewardOk(rewardItems,rewardCount,ntype)
    if self.m_root == nil then return end
	...
	elseif ntype == g_tGameActivityTypes.ACTIVITY_TOTALFIRSTRECHARGE or g_tGameActivityTypes.ACTIVITY_CUMULATIVECOST == ntype or g_tGameActivityTypes.ACTIVITY_STRENGTHEN == ntype or g_tGameActivityTypes.ACTIVITY_CONTINUERECHARGE == ntype or g_tGameActivityTypes.ACTIVITY_TYPE_NOVICE_ACCUMULATIVE == ntype or g_tGameActivityTypes.ACTIVITY_CUMULATIVECOST_TICKET == ntype 
            ...
            or g_tGameActivityTypes.ACTIVITY_ATHLETIC_VICTORY == ntype or g_tGameActivityTypes.ACTIVITY_RANKING_VICTORY == ntype or g_tGameActivityTypes.ACTIVITY_PET_UPGRADE == ntype or g_tGameActivityTypes.ACTIVITY_MOUNT_UPGRADE == ntype or g_tGameActivityTypes.ACTIVITY_EQUIPMENT_CALL == ntype or g_tGameActivityTypes.ACTIVITY_PET_QUAIL == ntype  or g_tGameActivityTypes.ACTIVITY_CONTINUOUS_LOGIN == ntype  or g_tGameActivityTypes.ACTIVITY_CHANNEL_RECHARGE == ntype then 
    ...
    elseif ntype == g_tGameActivityTypes.ACTIVITY_EXCHANGE or ntype == g_tGameActivityTypes.ACTIVITY_NEW_EXCHANGE or ntype == g_tGameActivityTypes.ACIVIITY_OLD_EXCHANGE then
    ...
    WZLog("WndGameActivity:GetRewardOk", Serialize(CacheCenter.m_tActivityItemRedDotList))
