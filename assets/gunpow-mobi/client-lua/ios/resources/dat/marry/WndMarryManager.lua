...
--@brief	初始化方法
--@note		注册结婚协议
function WndMarryManager:initManager()
...
function WndMarryManager:showMarryHoll()
    WZLog("WndMarryManager:showMarryHoll")
...
function WndMarryManager:getMarryStatusOK(marryStatus, coupleId, coupleName, weddingType,wedTime)
	WZLog("-------WndMarry:getMarryStatusOK---------")
    ...
    if self.m_bRefreshData == false then
		if WndMarryHoll.m_root == nil and SceneCity.m_root ~= nil then 
			local wndMarryHoll = WndMarryHoll:createElement()
			if wndMarryHoll ~= nil then 
				WindowManager:addWindow(wndMarryHoll,WndMarryHoll,nil,nil,nil,false)
			end  
		end 
        if SceneCity.m_root ~= nil then
            WndMarryHoll:onShowItemInfo(self.m_tMarryStatus.marryStatus,self.m_tMarryStatus.wedTime)
        end
    end
    self.m_bRefreshData = false
end
...
--@note  获取婚姻状况成功和获取求婚信成功为互斥协议，有求婚信是则服务器会返回获取求婚信成功
function WndMarryManager:getMarryLetterOK(marryRecordId, sendPlayerName, marryType,sendTime,headIds,faceIds,playerLevels,playerIds,headColors)
