...
function SceneKidHome:_init()
	...
	self.m_tIsRoleMove = {false, false, false, false} 			--是否在移动
...
--@brief 	外部接口
function SceneKidHome:showInterface(playerId)
	-- body
	if self.m_root then 
...
--@brief 	设置家园数据
function SceneKidHome:setData(hostPlayerId, homeName, comfirtValue, nConceiveTime, nServantTime, bHavedServant, configId, flipStatus, indexX, indexY, decoration, playerId, playerName, sex, headId, faceId, bodyId, wingId, headColor, bodyColor, kidId, kidName, kidState, kidSex, kidLevel, kidHeadId, kidFaceId, kidBodyId, kidGrowValue, kidCareValue, kidPlayCar, kidNextCheckTime, careBuffToday, bearChildId, kidFight, kidProp, touch)
	-- body
	self:_stopLoading()
	...
	WZLog("SceneKidHome:setData *****", self.m_sHomeName, self.m_nComfirtValue, self.m_nConceiveTime, self.m_nServantTime, self.m_bHavedServant)
...
--	self:setRoleStandArea(15, 0)
	...
	--生成随机位置
	self.m_tIsRoleMove = {false, false, false, false}
...
--@brief 	雇佣佣人后更新孩子数据
function SceneKidHome:updateKidData(childId, childName, childStatus, childSex, childLevel, childHeadId, childFaceId, childBodyId, growthValue, careValue, playCar, nextCheckTime, childFight, childProp, touch)
	-- body
	if self.m_root == nil then return end 
...
function SceneKidHome:_getAbsPosition(indexX, indexY, tBasicData)
	-- body
...
function SceneKidHome:_judgeCanPutBuilding(indexX, indexY, tBasicData, tData)
	-- body
	local bCanPut = true 
	if tBasicData.type == 2 or tBasicData.type == 3 then return end 
...
--@brief 	清除掉新建层中的建筑
function SceneKidHome:_cleanBuildingInNewLayer()
	-- body
	local conForNewBuild = GetElement(self.m_root, "conForNewBuild_kidSceneMap", WZUIContainer)
	conForNewBuild:removeAllChildrenWithCleanup(true)
		...
		for j,k in ipairs(cellList) do
			if k.listSortIdMin == v[1] and k.lineSortIdMin == v[2] and  k.listSortIdMax == v[3] and k.lineSortIdMax == v[4] then
				if k.configId > 0 then 
					if k.configId >= 999996 and k.configId <= 9999999 then 
...
--@brief 	寻路
--@param 	nIndex : 1->Host, 2->Mate, 3->Kid1, 4->Kid2
...
function SceneKidHome:reGenerateOneGrid()
	-- body
	local nIndexX = math.random(1, KID_MAP_ROW)
	local nIndexY = math.random(1, KID_MAP_ROW)
	local bFind = false 
