...
function SceneKidHome:onEnterTransitionDidFinish(element)
    ...
    self.m_tRandomTime = GetRandomNum(4, 10, 3)
...
--@brief    初始化家园底图
function SceneKidHome:_initMap()
    -- body
    ...
    for i = 1, KID_MAP_ROW do
        ...
        for j = 1, KID_MAP_ROW do
            ...
            if math.mod(i + j, 2) == 0 then
                imgMap:setFile("ui/kid/kidicon/kid_floor01.png")
            else
                imgMap:setFile("ui/kid/kidicon/kid_floor01.png")
            end
            ...
            if self.m_nTouchInBuildingState == 3 then
                if math.abs(pt.x - self.m_startPoint.x) <= 10 and math.abs(pt.y - self.m_startPoint.y) <= 10 then
                    ...
                    if not WndKidOperate.m_bIsClickFunc then 
                    	...
                    	elseif self.m_clickInfo.tData.basicData.type == 2 or self.m_clickInfo.tData.basicData.type == 3 then  --窗户和装饰
                            self.m_clickInfo.tCell:setArrowVisible(false)
	                    end
...
function SceneKidHome:tobuildNewBuilding(tData)
    -- body
    WZLog("SceneKidHome:tobuildNewBuilding", tData.basicData.id, self.m_clickInfo.tData.tempIndexX, self.m_clickInfo.tData.tempIndexY, tData.flipStatus)
    --新建建筑，停掉角色和孩子的跑动动画
    self:stopRoleRun(1)
...
--@brief    新建建筑
function SceneKidHome:cancelTobuildNewBuilding()
    -- body
    self.m_clickInfo = nil 
...
function SceneKidHome:onClickBuildingCallBack(element, tCell, tData)
    ...
    elseif self.m_clickInfo and (self.m_clickInfo.tData.basicData.type == 2 or self.m_clickInfo.tData.basicData.type == 3) then
...
function SceneKidHome:_createOrnaments()
	...
	if conForBuilding:getChildByTag(2222) then
		conForBuilding:removeChildByTag(2222, true)
	end
		...
		if self.m_tUsingOrnaments[i] > 0 then
	        ...
	        if celElement and tNewObj then
				...
				if tBasicInfo.sub_type == 3 then    --窗户
	            	...
	            	celElement:setAbsPosition(GlobalMethod:ccp(716, 415))
				elseif tBasicInfo.sub_type == 4 then --饰品
	            	celElement:setTag(2222)
	            	celElement:setAbsPosition(GlobalMethod:ccp(583, 511))
				end
...
function SceneKidHome:_refreshOperateBuildingState(pt, bIsTouchEnd)
    ...
    if self.m_clickInfo.tData.basicData.type == 2 or self.m_clickInfo.tData.basicData.type == 3 then return end
...
function SceneKidHome:createCouple()
	-- body
	local conForBuilding = GetElement(self.m_root, "conForBuilding_kidSceneMap", WZUIContainer)
	if conForBuilding:getChildByTag(666) then
		conForBuilding:removeChildByTag(666, true)
	end
	if conForBuilding:getChildByTag(667) then
		conForBuilding:removeChildByTag(667, true)
	end
...
function SceneKidHome:createServant()
    ...
    if self.m_bHavedServant == 1 and self.m_nServantTime > 0 then
    	...
    	if conForServant:getChildByTag(333) then
    		conForServant:removeChildByTag(333, true)
    	end
...
function SceneKidHome:createKidAni()
	...
	if conForServant:getChildByTag(777) then
		conForServant:removeChildByTag(777, true)
	end
	if conForServant:getChildByTag(778) then
		conForServant:removeChildByTag(778, true)
	end
    ...
    -- if #self.m_tKidData == 1 then
    --     table.insert(tKidGridIndex, {20,2})
    -- elseif #self.m_tKidData == 2 then
    --     table.insert(tKidGridIndex, {19,1})
    --     table.insert(tKidGridIndex, {21,3})
    -- end
    ...
    if self.m_nTimeCaculate >= 1 and self.m_tRandomTime then 
        ...
        for i = 1, #self.m_tRandomTime do
            if self.m_tRandomTime[i] > 0 then 
                ...
                self.m_tRandomTime[i] = math.random(3, 10)
                if self.m_tIsRoleMove[i] == false then 
                    ...
                    if bFindPath then 
                        local tTargetGrid = self:reGenerateOneGrid()
