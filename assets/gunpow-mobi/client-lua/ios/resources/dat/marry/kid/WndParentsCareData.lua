...
--@brief 	外部调用接口
function WndParentsCare:showInterface(nType)
	-- body
...
--@brief 	设置数据
function WndParentsCare:setData(tData, nMateSelKidId)
	-- body
	self.m_tData = tData 
...
--@brief    收到被离婚数据
function WndParentsCare:receiveDivorceData(fatherDevote, motherDevote, ownerId, childId, sex, childName, headId, faceId, level, otherSelectChildId, childFight, childProp)
    -- body
...
function WndParentsCare:receiveDivorceResult(playerId, playerName, childId, childName, sex, headId, faceId, level, childFight, childProp)
    -- body
