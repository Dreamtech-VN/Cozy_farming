...
function WndParentsCare:setKidFightingAddAtt(nAttIndex)
	...
	if self.m_nType == 1 then
		...
		txtExplanation:setVisible(true)
		if txtExplanation then
			...
			txtExplanation:setText(string.format(LocalStrings.KID_TEXT7, nFighting, nHours))
		end
		--关爱消耗
		local ids, num = SplitItemString(self.m_careCost)
	...
	elseif self.m_nType == 3 then--离婚
		...
		txtExplanation:setText(sContent)
	elseif self.m_nType == 4 then
		local sContent = ""
...
function WndParentsCare:showKidInfo()
	...
	elseif self.m_nType == 5 then
		...
		if #self.m_tData == 1 then
			...
			if conKidInfo then
				conKidInfo:setRelativePosition(GlobalMethod:ccp(0.5, 0.675))
			end
			GetElement(self.m_root, "imgSel1_WndParentsCare", WZUIImage):setVisible(false)
		else
			GetElement(self.m_root, "imgSel1_WndParentsCare", WZUIImage):setVisible(false)
...
function WndParentsCare:setKidHead()
	...
	local kidHead = CellHead:show(conHead, self.m_tData[1].headId, self.m_tData[1].faceId, self.m_tData[1].sex, nil, nil, nil, nil, nil, nil, nil, true)
	kidHead:setScale(1)
