...
--@brief	关闭窗口
--@param	element:按钮的引用
function WndMarryHoll:onCloseBtn(element)
	WZLog("WndMarryHoll:onCloseBtn(element)")
...
function WndMarryHoll:onMarryPurposeBtn(element)
    ...
    if tPlayer then
    	if tPlayer.zsLevel ~= nil and tPlayer.zsLevel == 1 then
    	    ...
    	    WindowManager:addWindow(wndMarry, WndMarry,true,nil,nil,false)
...
function WndMarryHoll:onShowItemInfo(nTag,nMyWedTime)
