...
function WndEquipmentLottery:onEnter(element)
	...
	if IsIphoneX() then
		CreateLimitPackage(11, conMiddle, GlobalMethod:ccp(0.07,0.55), true)
	else
		CreateLimitPackage(11, conMiddle, GlobalMethod:ccp(0.053,0.55), true)
	end
	...
	local bbbb = string.format(LocalStrings.SEND_DRAW_COUNT,10)
	local tempFreeText 
	if self.isUseTicket == "0" then
		tempFreeText= string.format(LocalStrings.DRAW_TIP_FREE_TEXT,self.m_tNums[3],GDatatab_item["id_70"].icon,sssss,GDatatab_item["id_2"].icon,bbbb)
	else
		tempFreeText= string.format(LocalStrings.DRAW_TIP_FREE_TEXT,self.m_tNums[3],GDatatab_item["id_1"].icon,sssss,GDatatab_item["id_2"].icon,bbbb)
	end
	...
	if fragmentCount >= tonumber(self.m_tNums[1]) then
		...
		local bbbb = string.format(LocalStrings.SEND_DRAW_COUNT,1)
		local tempS
		if self.isUseTicket == "0" then
			--imgFaffle2:setFile(GDatatab_item["id_70"].icon)
			tempS = string.format(LocalStrings.DRAW_TIP_FREE_TEXT,self.m_tNums[2],GDatatab_item["id_70"].icon,ss,GDatatab_item["id_2"].icon,bbbb)
		else
			--imgFaffle2:setFile(GDatatab_item["id_1"].icon)
			tempS = string.format(LocalStrings.DRAW_TIP_FREE_TEXT,self.m_tNums[2],GDatatab_item["id_1"].icon,ss,GDatatab_item["id_2"].icon,bbbb)
		end
...
function WndEquipmentLottery:_adaptLanguage_cn()
    WZLog("WndEquipmentLottery:_adaptLanguage_cn ")
    local txtFreeLuckDraw = GetElement(self.m_root,"txtFreeLuckDraw_WndEquipmentLottery",WZUILabelTTF)
    txtFreeLuckDraw:setFontSize(26)
end
...
function WndEquipmentLottery:_adaptLanguage_vn()
	...
	GetElement(self.m_root,"imgVertical1_WndEquipmentLottery",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.578869,0.5))
	GetElement(self.m_root,"imgVertical2_WndEquipmentLottery",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.578869,0.5))
	GetElement(self.m_root,"imgVertical3_WndEquipmentLottery",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.578869,0.5))
	...
	local txtTime1 = GetElement(self.m_root,"txtTime1_WndEquipmentLottery",WZUILabelTTF)
	txtTime1:setRelativePosition(GlobalMethod:ccp(0.453499,0.0821609))
	...
	txtRaffle:setRelativePosition(GlobalMethod:ccp(0.2773,0.5875))
	local txtT = GetElement(self.m_root,"txtT_WndEquipmentLottery",WZUILabelTTF)
	txtT:setScale(0.8)
	...
	GetElement(self.m_root,"txtDiamondCount_WndEquipmentLottery",WZUILabelTTF):setScale(0.6)
	GetElement(self.m_root,"txtDiamondCount2_WndEquipmentLottery",WZUILabelTTF):setScale(0.6)
