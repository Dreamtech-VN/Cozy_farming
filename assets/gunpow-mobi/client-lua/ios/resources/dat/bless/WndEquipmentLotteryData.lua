...
function WndEquipmentLottery:updateUIInfo(leaveTime,lotteryTime,bActivity,tenLotteryTime)
	...
	if self.m_bUpdate or not conMiddle:isVisible() then
		...
		WZLog("WndEquipmentLottery:updateUIInfo")
		if isEndTeach41 ~= true and step41 < 3 and CacheCenter:getPlayerInfo().level == 9 then
		    TeachGroup1:startGroup({41,3,self.m_root})
		elseif isEndTeach41 ~= true and step41 < 5 and CacheCenter:getPlayerInfo().level == 9 and GetElement(self.m_root,"conRSEEE_WndEquipmentLottery",WZUIContainer):isVisible() == false then
...
function WndEquipmentLottery:raffleSuccess(itemId)
	...
	if self.m_root ~= nil then
	    ...
	    if itemCount > 0 then
	    	...
	    	local spRSE =  GetElement(conRSEEE,"spRSE_WndEquipmentLottery",WZUISpine)
	    	spRSE:setVisible(true)
	    	spRSE:setFileAtlas("ui/ui_zbcj.atlas")
	        spRSE:setFileJson("ui/ui_zbcj.json")

	        conRSEEE:enableSchedule("spineEvent1",1.3)

	    	if itemCount > 1 and self.m_nRaffleType == 3 then
	    		self.m_bPlayingSpine = true
	    		spRSE:play("box3_open",false)
	    	else
	    		if self.m_nRaffleType == 1 then
	    			self.m_bPlayingSpine = true
	    			spRSE:play("box1_open",false)
	    		elseif self.m_nRaffleType == 2 then
	    			self.m_bPlayingSpine = true
	    			spRSE:play("box2_open",false)
	    		end
	    	end
	    end
	end
