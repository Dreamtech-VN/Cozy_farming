...
function ProtocolProcessorBossMap:regAll() 
     ...
     self:regProtocolCallbackFunction( Protocol.MAIN_BOSSMAPROOM, Protocol.BOSSMAPROOM_EnterRoomOk, "ProtocolProcessorBossMap:parse_BOSSMAPROOM_EnterRoomOk", "issiiiivbvivivsvivbvivivivivsvivivivivsvsvivissssssvivivivivivivivivivi")

    --@brief	返回房间列表
    self:regProtocolCallbackFunction( Protocol.MAIN_BOSSMAPROOM, Protocol.BOSSMAPROOM_GetRoomListOk, "ProtocolProcessorBossMap:parse_BOSSMAPROOM_GetRoomListOk", "ivivivivsvivbvivs")
...
-------------------------------------客户端到服务器协议发送方法模块--------------------------------------
--@brief	创建房间
function ProtocolProcessorBossMap:send_BOSSMAPROOM_CreateRoom(bossMapId, passWord )
	WZLog("send_BOSSMAPROOM_CreateRoom", bossMapId)
...
function ProtocolProcessorBossMap:send_BOSSMAPROOM_SelectRoom(roomId, passWord, mapId,roomChannel,assist )
	...
	sender:writeInt(assist) --是否助战（1：否）
	SendProtocol(sender,false) --true:showLoading
end
...
function ProtocolProcessorBossMap:parse_BOSSMAPROOM_EnterRoomOk(roomId, passWord, roomName, playerNumMode, mapId, wnersId, playerNum, seatUsed, playerId, serviceId, playerName, playerLevel, playerReady, playerSex, playerEquipment, playerEquipmentLevel, vipLevel, player_title, qualifyingLevel, zsleve, playerStar, playerFighting, pet, extranInfo, playerHeadColour, playerBodyColour, mentoringStr, coupleStr, chumStr, coupleNum, chumNum, mentoringNum, winNum, playNum, tournamentExp, skillId, matchLevel, matchscore, joinTimes, winTimes, continuousWinTimes,assist)
	...
	-- assist ：是否助战（0为助战）
	WZLog("ProtocolProcessorBossMap:parse_BOSSMAPROOM_EnterRoomOk",Serialize(VectorToTable(assist)))
	...
	if localData.map_num == 0 then
		...
		if inroom then
			...
			SceneMarryCopy:setMerryRoomData(
				roomId, passWord, roomName, playerNumMode, mapId, wnersId, playerNum, seatUsed, playerId,serverId,
		...
		SceneBossRoom:receiveEnterRoomOk(
			roomId, passWord, roomName, playerNumMode, mapId, wnersId, playerNum,VectorToTable(seatUsed),VectorToTable(playerId),VectorToTable(serverId),
			VectorToTable(playerName),VectorToTable(playerLevel),VectorToTable(playerReady),VectorToTable(playerSex),VectorToTable(playerEquipment),VectorToTable(playerEquipmentLevel),
			VectorToTable(vipLevel),VectorToTable(player_title),VectorToTable(qualifyingLevel),VectorToTable(zsleve),VectorToTable(playerStar),VectorToTable(playerFighting),VectorToTable(pet),VectorToTable(extranInfo),VectorToTable(playerHeadColour),VectorToTable(playerBodyColour),
			mentoringStr,coupleStr,chumStr,coupleNum,chumNum,mentoringNum,VectorToTable(matchLevel),VectorToTable(matchscore),VectorToTable(joinTimes),VectorToTable(winTimes),VectorToTable(continuousWinTimes),VectorToTable(serviceId),VectorToTable(assist))
	end
end

--@brief	返回房间列表
function ProtocolProcessorBossMap:parse_BOSSMAPROOM_GetRoomListOk(roomCount, roomId, battleStatus, playerCountNum, passWord, playerNum, roomStaus, mapId, roomName)
	-- roomCount : 房间数量
	-- roomId : 房间Id数组
	...
	-- mapId : 房间地图id
    -- roomName : 房间名称
	WZLog("ProtocolProcessorBossMap:parse_BOSSMAPROOM_GetRoomListOk")
