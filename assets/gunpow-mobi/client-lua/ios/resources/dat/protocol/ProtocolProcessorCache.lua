...
function ProtocolProcessorCache:regAll()
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_CACHE, Protocol.CACHE_PlayerInfo, "ProtocolProcessorCache:parse_CACHE_PlayerInfo", "isisssiiiiiiissiiisiiisssiiiitiinsisiisiissviiivstiviviviiiiiiitiivsisviviisiisssiiis")
	
	--@brief	物品信息（CACHE_PlayerItemCache = 2）
	self:regProtocolCallbackFunction( Protocol.MAIN_CACHE, Protocol.CACHE_PlayerItemCache, "ProtocolProcessorCache:parse_CACHE_PlayerItemCache", "vivsvivbvsvivivi")
...
-------------------------------------服务器到客户端协议回调方法模块--------------------------------------
--@brief	物品信息（CACHE_PlayerItemCache = 2）
function ProtocolProcessorCache:parse_CACHE_PlayerItemCache(itemId, lastNum, lastTime, isUse, data, playerItemId, disappearTime, color)
	-- itemId : 物品ID
	...
	WZLog("ProtocolProcessorCache:parse_CACHE_PlayerItemCache",itemId:size())
	for i=0,itemId:size()-1 do
		if GDatatab_item["id_"..itemId:get(i)] ~= nil then
			WZLog("parse_CACHE_PlayerItemCache",GDatatab_item["id_"..itemId:get(i)].name,isUse:get(i),color:get(i))
		end
	end
	CacheCenter:setPlayerItems(itemId, lastNum, lastTime, isUse, data, playerItemId, disappearTime, color)
end
...
function ProtocolProcessorCache:parse_CACHE_UpdateItemCache(playerItemId, key, value)
    ...
    if isEndTeach ~= true and step > 0 then
        TeachGroup1:startGroup({8,6,WndBagMain.m_root})
    end
end

--@brief	角色信息（CACHE_PlayerInfo = 1）
function ProtocolProcessorCache:parse_CACHE_PlayerInfo(id, name, sex, title, guildName, position, level, exp, maxExp, vipLevel, winNum, playNum, fighting, mateName, signature, vigor, maxVigor, guildId, property, strongSuitId, starSuitId, mosaicSuitId, petMessage, mountsMessage, fashionProperty, fashionFighting, tournamentLevel, tournamentIntegral, itemSuitId, itemSuitNum, segmentLevel, totemLevel, lovelLevel, loveSkill, moralityLevel, masterName, vipExp, segmentExp, rankMatchMessage, guildLevel, buyTimesPS, headScul, snsValue, starsoulId, spaceSex, giftNum, allMountsMessage, marryFlag, teamId, prayId, xlId, xlExp, shapeId, shapeLevel, showShape, awakeSoulLevel, awakeStep, itemSuitId2, itemSuitNum2, homeLevel, sheerLuxury, footMark, shapeSkillId, awakeSkillId, runeItemId, runeItemNum, obtainNum, cardMessage, bgId, showMes, coupleMes, childMes, careBuffProp, careToday, headSculStatus, thumbUpNum, badgeInfo)
	...
	-- thumbUpNum : 排位被点赞的次数
	-- badgeInfo : 成就徽章数据
	WZLog("ProtocolProcessorCache:parse_CACHE_PlayerInfo", homeLevel, sheerLuxury)
	CacheCenter:setPlayerInfo(id, name, sex, title, guildName, position, level, exp, maxExp, vipLevel, winNum, playNum, fighting, mateName, signature, vigor, maxVigor, guildId, property, strongSuitId, starSuitId, mosaicSuitId, petMessage, mountsMessage, fashionProperty, fashionFighting, tournamentLevel, tournamentIntegral, itemSuitId, itemSuitNum, segmentLevel, totemLevel, lovelLevel, loveSkill, moralityLevel, masterName, vipExp, segmentExp, rankMatchMessage, guildLevel, buyTimesPS, headScul, snsValue, starsoulId, spaceSex, giftNum, allMountsMessage, marryFlag, teamId, prayId, xlId, xlExp, shapeId, shapeLevel, showShape, awakeSoulLevel, awakeStep, itemSuitId2, itemSuitNum2, homeLevel, sheerLuxury, footMark, shapeSkillId, awakeSkillId, runeItemId, runeItemNum, obtainNum, cardMessage, bgId, showMes, coupleMes, childMes, careBuffProp, careToday, headSculStatus, thumbUpNum, badgeInfo)
end
