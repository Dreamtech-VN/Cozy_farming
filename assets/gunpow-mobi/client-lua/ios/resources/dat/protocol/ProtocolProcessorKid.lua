...
function ProtocolProcessorKid:regAll()
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WEDDING_GetHouseInfoOK, "ProtocolProcessorKid:parse_WEDDING_GetHouseInfoOK", "isiiiivivivivivivivsvivivivivivivivivsviviviviviviviviviviiivivsvi")
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WWEDDING_GetHouseRankInfoOk, "ProtocolProcessorKid:parse_WWEDDING_GetHouseRankInfoOk", "vivivivivsvivivivt")
    --@brief    获取孩子状态（WEDDING_GetChildStatusOk = 86）
    self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WEDDING_GetChildStatusOk, "ProtocolProcessorKid:parse_WEDDING_GetChildStatusOk", "isiiiiiiiiiiisi")
...
function ProtocolProcessorKid:send_WEDDING_GetHouseInfo(playerId )
	WZLog("send_WEDDING_GetHouseInfo")
...
function ProtocolProcessorKid:parse_WEDDING_GetHouseInfoOK(playerId, houseName, sheerLuxury, bearLastTime, nannyStatus, nannyLastTime, configId, x, y, flipStatus, decoration, userId, userName, sex, headId, faceId, bodyId, wingId, headColor, bodyColor, childId, childName, childStatus, childSex, childLevel, childHeadId, childFaceId, childBodyId, growthValue, careValue, playCar, nextCheckTime, careBuffToday, bearChildId, childFight, childProp, touch)
	...
	-- nextCheckTime : 下次请求时间
	-- careBuffToday : 今天是否获取过关爱buff 0否 1是
    -- bearChildId : 生育、领养中的孩子ID
    -- childFight : 小孩战力
	-- childProp : 小孩属性
    -- touch : 是否抚摸过. 0否 1是
	WZLog("ProtocolProcessorKid:parse_WEDDING_GetHouseInfoOK", sheerLuxury, bearLastTime, nannyStatus, nannyLastTime)

	SceneKidHome:setData(playerId, houseName, sheerLuxury, bearLastTime, nannyLastTime, nannyStatus, VectorToTable(configId), VectorToTable(flipStatus), VectorToTable(x), VectorToTable(y), VectorToTable(decoration), VectorToTable(userId), VectorToTable(userName), VectorToTable(sex), VectorToTable(headId), VectorToTable(faceId), VectorToTable(bodyId), VectorToTable(wingId), VectorToTable(headColor), VectorToTable(bodyColor), VectorToTable(childId), VectorToTable(childName), VectorToTable(childStatus), VectorToTable(childSex), VectorToTable(childLevel), VectorToTable(childHeadId), VectorToTable(childFaceId), VectorToTable(childBodyId), VectorToTable(growthValue), VectorToTable(careValue), VectorToTable(playCar), VectorToTable(nextCheckTime), careBuffToday, bearChildId, VectorToTable(childFight), VectorToTable(childProp), VectorToTable(touch))
end
...
function ProtocolProcessorKid:parse_WEDDING_AppeaseChildOk(childId, actionType, childStatus, growthValue)
	-- childId : 孩子ID
	-- actionType : 1.哭闹 2.饥饿 3.尿裤子
	-- childStatus : 孩子事件状态. 与 （1、2、4）分别进行按位与操作，如果不为0，则为对应的(哭闹、饥饿、尿裤子状态)
	-- growthValue : 孩子成长值
    WZLog("ProtocolProcessorKid:parse_WEDDING_AppeaseChildOk")
...
--@brief    获取排行榜（WWEDDING_GetHouseRankInfoOk = 84）
function ProtocolProcessorKid:parse_WWEDDING_GetHouseRankInfoOk(rank, playerId, serverId, childId, childName, level, headId, faceId, sex)
    -- rank : 排名
    -- playerId : 玩家id
    -- serverId : 区服id
    -- childId :  小孩Id
    ...
    if WndKidOperate.m_root then
        WndKidOperate:setData(VectorToTable(rank), VectorToTable(playerId), VectorToTable(serverId), VectorToTable(childId), VectorToTable(childName), VectorToTable(level), VectorToTable(headId), VectorToTable(faceId), VectorToTable(sex))
    end
end

--@brief    获取孩子状态（WEDDING_GetChildStatusOk = 86）
function ProtocolProcessorKid:parse_WEDDING_GetChildStatusOk(childId, childName, childStatus, childSex, childLevel, childHeadId, childFaceId, childBodyId, growthValue, careValue, playCar, nextCheckTime, childFight, childProp, touch)
    -- xTarget : x坐标
    -- yTarget : y坐标
    WZLog("ProtocolProcessorKid:parse_WEDDING_GetChildStatusOk")

    SceneKidHome:updateKidData(childId, childName, childStatus, childSex, childLevel, childHeadId, childFaceId, childBodyId, growthValue, careValue, playCar, nextCheckTime, childFight, childProp, touch)
end
