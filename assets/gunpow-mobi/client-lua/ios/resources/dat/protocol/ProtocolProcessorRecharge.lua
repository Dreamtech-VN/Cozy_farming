...
function ProtocolProcessorRecharge:regAll()
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_PURCHASE, Protocol.PURCHASE_GetSummerGiftIdListOk, "ProtocolProcessorRecharge:parse_PURCHASE_GetSummerGiftIdListOk", "vivsvivivsvtvsvsvsvivnvsvivtvi")
...
function ProtocolProcessorRecharge:parse_PURCHASE_BuySuccess(count,isUp,vipLevel,sRemk,itemId)
    ...
    if WndVipGift.m_root then
        if WndVipGift.m_nType == 2 then --新手定推礼包时候
...
--@brief    获取暑期礼包信息（PURCHASE_GetSummerGiftIdListOk = 13）
function ProtocolProcessorRecharge:parse_PURCHASE_GetSummerGiftIdListOk(id, icon, count, giftDiamondCount, price, showFlag, name, describe, showPrice, itemId, sort, payCodeId, leftTimes, limitType, needVipLv)
    -- id : 产品id
    -- icon : 图标
    -- count : 获得数目
    -- giftDiamondCount : 赠送钻石的数目
    ...
    -- limitType : 限购类型,0:不限购,1:每日限购,2:终身限购
    -- needVipLv : 需要的vip等级
    WZLog("ProtocolProcessorRecharge:parse_PURCHASE_GetSummerGiftIdListOk  ",Serialize(VectorToTable(leftTimes)))
