...
self:regProtocolCallbackFunction( Protocol.MAIN_TRANSACTION, Protocol.TRANSACTION_BuyCommodityStatus, "ProtocolProcessorTransaction:parse_TRANSACTION_BuyCommodityStatus", "tii")
--@brief	获取我的出售商品列表（TRANSACTION_GetSaleListOk = 6）
self:regProtocolCallbackFunction( Protocol.MAIN_TRANSACTION, Protocol.TRANSACTION_GetSaleListOk, "ProtocolProcessorTransaction:parse_TRANSACTION_GetSaleListOk", "vivivivivi")
...
self:regProtocolCallbackFunction( Protocol.MAIN_TRANSACTION, Protocol.TRANSACTION_GetTransactionLogListOk, "ProtocolProcessorTransaction:parse_TRANSACTION_GetTransactionLogListOk", "vtvivivivivs")
...
function ProtocolProcessorTransaction:send_TRANSACTION_GetCommodityList(commodityType, commodityQuality, commodityIds )
	WZLog("send_TRANSACTION_GetCommodityList")
...
function ProtocolProcessorTransaction:parse_TRANSACTION_GetCommodityListOk(commodityIds, itemIds, quantitys, prices)
	...
	-- quantitys : 物品数量
	-- prices : 商品单价
	WZLog("ProtocolProcessorTransaction:parse_TRANSACTION_GetCommodityListOk")
	WndTransaction:setData1(VectorToTable(commodityIds), VectorToTable(itemIds), VectorToTable(quantitys), VectorToTable(prices))
end

--@brief	购买商品（TRANSACTION_BuyCommodityStatus = 4）
function ProtocolProcessorTransaction:parse_TRANSACTION_BuyCommodityStatus(status, itemId, quantity)
	-- status : 购买返回状态（0购买成功，1背包已满，2晶石不足，3商品已出售）
	-- itemId : 获得物品ID
	-- quantity : 获得物品数量
	WZLog("ProtocolProcessorTransaction:parse_TRANSACTION_BuyCommodityStatus", Serialize(itemId), Serialize(quantity))
	if status == 0 then
...
--@brief	获取我的出售商品列表（TRANSACTION_GetSaleListOk = 6）
function ProtocolProcessorTransaction:parse_TRANSACTION_GetSaleListOk(commodityIds, itemIds, quantitys, saleNums, saleTime)
	-- commodityIds : 商品ID
	-- itemIds : 物品ID
	-- quantitys : 商品数量
	-- saleNums : 已出售数量
	...
	WndTransaction:setData2(VectorToTable(commodityIds), VectorToTable(itemIds), VectorToTable(quantitys), VectorToTable(saleNums), VectorToTable(saleTime))
end
...
--@brief	获取交易日志列表（TRANSACTION_GetTransactionLogListOk = 12）
function ProtocolProcessorTransaction:parse_TRANSACTION_GetTransactionLogListOk(logType, itemIds, itemNums, prices, addSpar, commodityTime)
	...
	WndTransaction:setData4(VectorToTable(logType), VectorToTable(itemIds), VectorToTable(itemNums), VectorToTable(prices), VectorToTable(addSpar), VectorToTable(commodityTime))
end
-------------------------------------服务器到客户端协议回调方法模块End--------------------------------------
