...
function ProtocolProcessorMatchmaking:regAll()
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WEDDING_GetDatingServiceInfoListOk, "ProtocolProcessorMatchmaking:parse_WEDDING_GetDatingServiceInfoListOk", "vivsvtvivivivivivsvivsvsvb")
...
--@brief	交友列表（WEDDING_GetDatingServiceInfoListOk = 103）
function ProtocolProcessorMatchmaking:parse_WEDDING_GetDatingServiceInfoListOk(playerId, playerName, sex, level, vipLevel, headId, faceId, headColor, photoUrl, fight, guildName, msg, recommend)
	-- photoUrl : 照片url
	-- msg : 交友宣言
	-- recommend : 是否推荐
	
	WZLog("ProtocolProcessorMatchmaking:parse_WEDDING_GetDatingServiceInfoListOk")
	WndMatchmaking:setPlayerListData(VectorToTable(playerId), VectorToTable(sex), VectorToTable(level), VectorToTable(playerName), VectorToTable(vipLevel), VectorToTable(headId), VectorToTable(faceId), VectorToTable(headColor), VectorToTable(guildName), VectorToTable(fight), VectorToTable(msg), VectorToTable(photoUrl), VectorToTable(recommend))
end
