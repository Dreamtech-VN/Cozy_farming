...
function ProtocolProcessorWakeup:regAll()
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_RUNE, Protocol.RUNE_GetRuneInfoOk, "ProtocolProcessorWakeup:parse_RUNE_GetRuneInfoOk", "viviviivivi")
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_TRATE, Protocol.PRAY_GetPrayMessOk, "ProtocolProcessorWakeup:parse_PRAY_GetPrayMessOk", "iiiviviviviviviviviviviivivi")
...
--@brief    获取符文信息（RUNE_GetRuneInfoOk = 2）
function ProtocolProcessorWakeup:parse_RUNE_GetRuneInfoOk(placeIds, placeItemId, rpIds, runeLevel, itemIds, itemNums)
    -- placeIds : 槽位id
...
--@brief    获取祈福信息成功（PRAY_GetPrayMessOk = 2）
function ProtocolProcessorWakeup:parse_PRAY_GetPrayMessOk(debris, prayerId, summonNum, bagIds, bagExps, bagPrayIds, roomIds, roomExps, roomPrayIds, prayNum, equipPrayId, equipId, equipExp, fightNum, num, openlevel)
    -- debris : 碎片数量
    -- prayerId : 祈福师Id
    -- summonNum : 今天召唤次数
    ...
    if WndExtraction.m_root then
        WndExtraction:setBlessData(bagIds, bagExps, bagPrayIds)
    end
end
