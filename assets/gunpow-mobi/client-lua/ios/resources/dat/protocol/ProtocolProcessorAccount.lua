...
function ProtocolProcessorAccount:regAll()
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_ACCOUNT, Protocol.ACCOUNT_RoleActorLoginOk, "ProtocolProcessorAccount:parse_ACCOUNT_RoleActorLoginOk", "isiiiisiisvsiiibiiiiiiiis")
	--返回随机名称(S->C)
	...
	--@brief	更新玩家位置坐标信息（NEARBY_UpdatePlayerLocationInfo = 17）错误处理(S->C)
	self:regProtocolCallbackFunction( Protocol.MAIN_NEARBY, Protocol.NEARBY_UpdatePlayerLocationInfo, "ProtocolProcessorAccount:send_NEARBY_UpdatePlayerLocationInfo_ErrorProcess", "is" )
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_SYSTEM, Protocol.SYSTEM_SynButtonInfo, "ProtocolProcessorAccount:parse_SYSTEM_SynButtonInfo", "vivivi")
...
--@brief	更新玩家位置坐标信息（NEARBY_UpdatePlayerLocationInfo = 17）
function ProtocolProcessorAccount:send_NEARBY_UpdatePlayerLocationInfo(longitude, latitude )
	WZLog("send_NEARBY_UpdatePlayerLocationInfo")
	local sender = Protocol:getSender( Protocol.MAIN_NEARBY, Protocol.NEARBY_UpdatePlayerLocationInfo )
	if sender==nil then WZLog("sender == nil") return end

	sender:writeInt( longitude )	-- 玩家的经度（1000000倍）
	sender:writeInt( latitude )	-- 玩家的纬度（1000000倍）
	SendProtocol(sender,false) --true:showLoading
end
...
--@brief	角色登录成功（ACCOUNT_RoleActorLoginOk = 23）
function ProtocolProcessorAccount:parse_ACCOUNT_RoleActorLoginOk(playerId, playerName, maxLevel, playerSex, level, exp, guildName, upgradeexp, vipLevel, player_title, wbUserId, zsleve, qualifyingLevel, honor, doubleCard, fighting, guildId, steps, vipMark, vipLastDay, guideLevel, blastLevel, guideLevel2, property)
	-- playerId : 角色id
	-- playerName : 角色名称
	-- maxLevel : 最高等级
	-- playerSex : 性别
	...
	WZLog("ProtocolProcessorAccount:parse_ACCOUNT_RoleActorLoginOk", playerId,playerName,level) 
	--ProtocolProcessorAccount:send_ACCOUNT_SynAccountState(SceneLogin:isRegistered())
    ...
    --发送预获取的协议数据
    PrefetchCache:sendPrefetchProtocal()
...
function ProtocolProcessorAccount:parse_SYSTEM_GetIslandStateOk(islandState, openNewTeach, popNotice, openTipLevel, showItemsRemainimgDays, downloadRewardSwitch)
    ...
    if PassportSdkManager.b_createRole then
    	...
    	if PassportSdkManager.postGameInfoVn then
    		PassportSdkManager:postGameInfoVn("register_vn")
    	end
    ...
    if teach then
        --TeachFollowingFiveLevel:getServerData(Teach.DATA.saveStep[1].ids, 1, -1, Teach.DATA.saveStep[1].step)
	...
	elseif (GlobalGame.g_nCurrentUIChannelId == -1 or (TeachGroup1.GROUP ~= -1 and TeachGroup1.STEP ~= -1) or TeachGroup1.LOST_NET == true or isTeach) and WBattleGlobal:getCurrent():getBattleMode() == nil and (GlobalGame.m_bIsLostNetSettlement == nil) then
		...
		if GlobalGame.g_nCurrentUIChannelId ~= Chat_Channel_Island then
	    	...
	    	if SceneWorldTeamBoss.m_root or SceneWorldTeamBossRoom.m_root then
...
--@bridf 同步按钮状态协议
function ProtocolProcessorAccount:parse_SYSTEM_SynButtonInfo(id,showLevel,openLevel)
	...
	for i=0,id:size()-1 do
		...
		if GDatatab_button_info["id_"..idKey] ~= nil then
			WZLog("ProtocolProcessorAccount:parse_SYSTEM_SynButtonInfo2", idKey, showLevel:get(i), openLevel:get(i))
...
--@brief	更新玩家位置坐标信息（NEARBY_UpdatePlayerLocationInfo = 17）错误处理函数(S->C)
--@param	nFlag:标志位
--@param	sMessage:错误信息
--@note	在此对协议错误进行相应处理
function ProtocolProcessorAccount:send_NEARBY_UpdatePlayerLocationInfo_ErrorProcess(nFlag, sMessage)
	WZLog("ProtocolProcessorAccount:send_NEARBY_UpdatePlayerLocationInfo_ErrorProcess")
	ProtocolErrorProcessor:errorProcess(Protocol.MAIN_NEARBY, Protocol.NEARBY_UpdatePlayerLocationInfo, nflag, sMessage)
end
