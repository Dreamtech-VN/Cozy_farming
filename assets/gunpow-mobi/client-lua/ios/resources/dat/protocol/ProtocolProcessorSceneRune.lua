...
function ProtocolProcessorSceneRune:regAll()
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_RUNE, Protocol.RUNE_GetRuneInfoOk, "ProtocolProcessorSceneRune:parse_RUNE_GetRuneInfoOk", "viviviivivi")
...
-------------------------------------------------receive ok-------------------------------------------------------------
--@brief	获取符文信息（RUNE_GetRuneInfoOk = 2）
function ProtocolProcessorSceneRune:parse_RUNE_GetRuneInfoOk(placeIds, placeItemId, rpIds, runeLevel, itemIds, itemNums)
	-- placeIds : 槽位id
	...
	-- itemIds : 拥有的符文ID
	-- itemNums : 拥有的符文数量
	WZLog("ProtocolProcessorSceneRune:parse_RUNE_GetRuneInfoOk")
	placeIds = VectorToTable(placeIds)
	placeItemId = VectorToTable(placeItemId)
	SceneRune:setRuneInfo(placeIds,placeItemId,VectorToTable(rpIds),runeLevel,VectorToTable(itemIds),VectorToTable(itemNums))
