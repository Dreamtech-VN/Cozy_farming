...
function ProtocolProcessorPrefetchCache:regAll()
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_VIP, Protocol.VIP_GetVipInfoOk, "ProtocolProcessorPrefetchCache:parse_VIP_GetVipInfoOk", "iiib")
	--@brief	获取奖励列表成功（VIP_GetVipRewardsInfoOk = 6）
	self:regProtocolCallbackFunction( Protocol.MAIN_VIP, Protocol.VIP_GetVipRewardsInfoOk, "ProtocolProcessorPrefetchCache:parse_VIP_GetVipRewardsInfoOk", "viviviviivi")

	--@brief    获取公告列表等信息成功	--add by wuweidong
    self:regProtocolCallbackFunction( Protocol.MAIN_ANNOUNCEMENT, Protocol.CIRCULAR_ANNOUNCEMENT_GETDETAILOK, "ProtocolProcessorPrefetchCache:parse_CIRCULAR_GetBulletinListOk", "vsvs")
	--@brief	获取任务列表成功
	...
	--获取VIP礼包领取情况
	self:regProtocolCallbackFunction( Protocol.MAIN_VIP, Protocol.VIP_VipReceiveInfo ,"ProtocolProcessorPrefetchCache:parse_VIP_ReceiveInfo","vii")
...
--@brief    获取公告列表等信息成功
function ProtocolProcessorPrefetchCache:parse_CIRCULAR_GetBulletinListOk(theme, content)
    -- theme : 公告主题
    -- content : 公告内容
    WZLog("ProtocolProcessorPrefetchCache:parse_CIRCULAR_GetBulletinListOk")
    PrefetchCache:setBulletinList(VectorToTable(theme),VectorToTable(content))
end
...
--@brief	获取vip信息成功（VIP_GetVipInfoOk = 4）
function ProtocolProcessorPrefetchCache:parse_VIP_GetVipInfoOk(vipExp, vipLv, nextLvExp, isReceiveDayPack)
	-- vipExp	vip经验
	-- vipLv	vip等级
	-- nextLvExp	升级到下个等级需要经验
	-- isReceiveDayPack	是否领取了vip每日礼包
	-- isReceiveLvPack	是否领取了vip等级礼包信息 1 为以领取(一共10级)
	WZLog("ProtocolProcessorPrefetchCache:parse_VIP_GetVipInfoOk2")
	PrefetchCache:setVipList( vipExp, vipLv, nextLvExp, isReceiveDayPack)
    if WndVip.m_root and WndMonthCards.m_root then
        WndVip:setVipList( vipExp, vipLv, nextLvExp, isReceiveDayPack)
    end
end

--@brief	获取奖励列表成功（VIP_GetVipRewardsInfoOk = 6）
function ProtocolProcessorPrefetchCache:parse_VIP_GetVipRewardsInfoOk(vipLevel, itemId, count, days, typeReturn, isReceiveLvPack)
	-- vipLevel	会员等级
	-- itemId	物品id
	-- count	物品数量
	-- days	int[]	有效天数
	WZLog("ProtocolProcessorPrefetchCache:parse_VIP_GetVipRewardsInfoOk")
	PrefetchCache:getVipRewardsOK(VectorToTable(vipLevel),VectorToTable(itemId),
		VectorToTable(count),VectorToTable(days),typeReturn, VectorToTable(isReceiveLvPack))
end
...
--@brief   vip礼包领取情况
function ProtocolProcessorPrefetchCache:parse_VIP_ReceiveInfo(isReceiveLvPack,isEveryDayPack)
    local tabGiftNum = {}
    GlobalGame.g_nVipGiftBagNum = 0
    tabGiftNum = VectorToTable(isReceiveLvPack)
    for i,v in pairs(tabGiftNum) do
        WZLog("礼包数量",i,tabGiftNum[i])
        if tabGiftNum[i] == 1 then
            GlobalGame.g_nVipGiftBagNum = GlobalGame.g_nVipGiftBagNum + 1
        end
    end
    if isEveryDayPack == 1 then
    	WndVip.isCanClickGetGiftBag = true
		GlobalGame.g_nVipPrivilege = 1
        GlobalGame.g_nVipGiftBagNum = GlobalGame.g_nVipGiftBagNum + 1
    else
		GlobalGame.g_nVipPrivilege = 0
    	WndVip.isCanClickGetGiftBag = false
    end
    
    WZLog("VIP礼包数量",GlobalGame.g_nVipGiftBagNum)
end
