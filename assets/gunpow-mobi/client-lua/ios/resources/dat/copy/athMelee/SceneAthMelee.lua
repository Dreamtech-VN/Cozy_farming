...
function SceneAthMelee:_addTop()
    ...
    local img = "ui/common/bag_icon_daluandou.png"
    if self.m_nType == 2 then
        img = "ui/Hall/common_icon_jdmx.png"
...
function SceneAthMelee:onCheckAthLv( element )
    ...
    WndTips:show(element,tipCon,4,CacheCenter:getPlayerInfo(),GlobalMethod:ccp(300,-160))
end
...
function SceneAthMelee:_updatePlayerInfo()
    ...
    -- 等级图标以及经验
    local hallInfo = GDatatab_integral["id_"..tInfo.tournamentLevel]
    local txtExp = GetElement(self.m_root,"txtExp_SceneAthMelee", WZUILabelTTF)
    local curExp,AllExp = tInfo.tournamentIntegral,hallInfo.upgrade_integral
    ...
    local LvNum = (tInfo.tournamentLevel-1)%10+1
