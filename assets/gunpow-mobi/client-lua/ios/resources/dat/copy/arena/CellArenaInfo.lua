...
--@note		在这里做场景进入前的准备工作
function CellArenaInfo:onEnter(element)
	self.m_root = element
end
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function CellArenaInfo:onExit(element)
	self:_unInit()
...
-- 更新
function CellArenaInfo:_update()
    local curData = self.m_tData
    -- 底
    local imgDi = GetElement(self.m_root, "imgDi_CellArenaInfo",WZUI9Image)
    ...
    local imgLv = GetElement(self.m_root, "imgAthLv_CellArenaInfo",WZUIImage)
    imgLv:setFile("ui/common/"..hallInfo.iocn..".png")

    local LvNum = (curData.tournamentLevel-1)%10+1
    local lafLv = GetElement(self.m_root, "lafAthLv_CellArenaInfo",WZUILabelAtlasFont)
    lafLv:setText(LvNum)

    -- 等级
    local txtLevel = GetElement(self.m_root, "txtLevel_CellArenaInfo", WZUILabelTTF)
    ...
    local ftbName = GetElement(self.m_root, "ftbName_CellArenaInfo", WZUIFreeTextBox)
    local txtName = GetElement(self.m_root, "txtName_CellArenaInfo", WZUILabelTTF)
    ...
    -- 击杀
    local txtKill = GetElement(self.m_root, "txtKill_CellArenaInfo", WZUILabelTTF)
    txtKill:setText(curData.killCount)
    -- 命中
    local txtHitRate = GetElement(self.m_root, "txtHitRate_CellArenaInfo", WZUILabelTTF)
    txtHitRate:setText(curData.shootRate.."%")
    -- 分数
    local txtScore = GetElement(self.m_root, "txtScore_CellArenaInfo", WZUILabelTTF)
    ...
    local imgKill = GetElement(self.m_root, "imgFirstBlood_CellArenaInfo",WZUIImage)
