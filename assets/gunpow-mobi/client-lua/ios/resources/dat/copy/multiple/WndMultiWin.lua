...
function WndMultiWin:onClickCard(tCard)
    ...
    if nType == 2 and nVipLevel < 5 then --vip免费翻牌 vip等级不足
        MsgBoxManager:showTipBox(LocalStrings.TURNCARD_VIP_TIPS)
    elseif nType == 3 and not DiamondIsEnoughNum(math.ceil(20 * self.m_tData.flopRebate/100)) then --钻石翻牌 钻石不足
...
function WndMultiWin:scheduleTurnCardCountdown(element, delta)
    ...
    if self.m_nCountdown <= 0 then
        ...
        if self.m_bCard2Flag == false and CacheCenter:getPlayerInfo().vipLevel >= 5 then
