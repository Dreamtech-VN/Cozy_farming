...
function SceneWorldTeamBossRoom:updateMiddleInfo()
    ...
    local conTop = GetElement(self.m_root,"conTop_SceneWorldTeamBossRoom",WZUIContainer)

    local roomName = WZUILabelTTF:luaTo(conTop:getChildElement("txtRoomName_SceneWorldTeamBossRoom"))
    roomName:setText(self.m_tData.roomId .. " " .. self.m_tData.roomName)

    local roomPass = WZUILabelTTF:luaTo(conTop:getChildElement("txtRoomPass_SceneWorldTeamBossRoom"))
...
function SceneWorldTeamBossRoom:_createASeat(index,bgType,isused)
    ...
    cellObj:setKickOutPt(GlobalMethod:ccp(0.92,1.011))
    local friendInfo = self:getFriendRV(self.m_tData.playerId[index])
...
function SceneWorldTeamBossRoom:onClickInvPlayer(element)
    ...
    WndFriendList:showInterface(14, self, self.inviteFriend)
end
...
function SceneWorldTeamBossRoom:inviteFriend(tFriend)
    ...
    --音效
    ProtocolProcessorWorldTeamBossRoom:send_TEAMWORLDBOSS_Invite(self.m_tData.roomId, tFriend.id)
end
...
function SceneWorldTeamBossRoom:_adaptLanguage_vn( )
    GetElement(self.m_root, "txtRoomPass_SceneWorldTeamBossRoom", WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.38,0.5))
end
...
function SceneWorldTeamBossRoom:_adaptLanguage_es( )
    GetElement(self.m_root, "txtRoomPass_SceneWorldTeamBossRoom", WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.55,0.5))
