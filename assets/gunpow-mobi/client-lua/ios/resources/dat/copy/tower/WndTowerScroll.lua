...
-------------------------------------公有方法模块Begin--------------------------------------
local MAPSIZE = {w=960,h=640} --地图块大小
...
--    GlobalGame.g_tRedPointList.tower = nil
    ...
    local width = math.max(self.m_nScreenSize.width/self.m_nScreenSize.height*640,960)
    local x = width/2
...
function WndTowerScroll:onBtnRaids(element)
    ...
    if self.UserData.nowFloor >= self.UserData.topFloor then
...
function WndTowerScroll:onCallBackAddSpeed( nId , nType)
    ...
    local needMoney = (self.UserData.topFloor-self.UserData.nowFloor) * 2
...
function WndTowerScroll:onClickSwitch(element)
    ...
    self.m_nTowerType = math.mod(self.m_nTowerType + 1, 2)
...
local VISIBLEWIDTH = 960 --可见宽度，从玩家当前位置起，左右各可见的宽度
...
function WndTowerScroll:loadMap(bFirstMapScaleX)
...
function WndTowerScroll:_updateSweepState()
    if self.SweepState == 1 and self.m_nSweepTime > 0 then
        ...
        local costDiamond = self.UserData.topFloor - self.UserData.nowFloor
        local txtSweapFast = GetElement(self.m_root,"txtSweapFast_WndTowerScroll",WZUILabelTTF)
        txtSweapFast:setText((costDiamond*2))
...
--@brief   是否需要X轴旋转
function WndTowerScroll:_isNeedFlipX(node)
    if node then
...
--@brief   X轴旋转
function WndTowerScroll:_flipX(parNode,animNode)
    if self:_isNeedFlipX(parNode) then
...
function WndTowerScroll:_showContentByType()
    ...
    if self.m_nTowerType == 0 then --怪物塔
        ...
        if SceneCopy.m_tCellTopHandle then
            SceneCopy.m_tCellTopHandle:setTitleFile("ui/common/common_icon_ptfb.png")
        end
    elseif self.m_nTowerType == 1 then --英雄塔
        ...
        if SceneCopy.m_tCellTopHandle then 
            SceneCopy.m_tCellTopHandle:setTitleFile("ui/common/common_icon_herotower.png")
        end
...
function WndTowerScroll:initMapHero()
    ...
    self.m_nInitMapCount =  4 --最多加载三页(一页为640的高度)
    self.m_nLoadCount = 0
    self.m_bFirstLoad = true
    self.m_tPlayerStartP = nil
    self.m_oBeforeFloor = nil
...
function WndTowerScroll:_adaptLanguage_pt()
    local txtTop = GetElement(self.m_root,"txtTop_WndTowerScroll",WZUILabelTTF)
    txtTop:setRelativePosition(GlobalMethod:ccp(0.600161,0.674821))
    txtTop:setFontSize(20)
    
    local txtTopFloor = GetElement(self.m_root,"txtTopFloor_WndTowerScroll",WZUILabelTTF)
    txtTopFloor:setFontSize(20)
    txtTopFloor:setRelativePosition(GlobalMethod:ccp(0.617038,0.674821))
...
function WndTowerScroll:_adaptLanguage_tr() 
    GetElement(self.m_root,"txtTop_WndTowerScroll",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.614447,0.674821))
    GetElement(self.m_root,"txtFight_WndTowerScroll",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.640162,0.325178))
    GetElement(self.m_root,"txtTopFloor_WndTowerScroll",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.622752,0.674821))
    GetElement(self.m_root,"txtFightNum_WndTowerScroll",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.648467,0.325178))
...
function WndTowerScroll:_adaptLanguage_es(  )
    GetElement(self.m_root,"txtTop_WndTowerScroll",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.625876,0.674821))
    GetElement(self.m_root,"txtTopFloor_WndTowerScroll",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.639895,0.674821))
