...
-- 游戏顶部
function SceneCopy:_addTop(imgPath,chatFlag)
    local cell,tcell = CellTopHandle:createElement()
    local con = GetElement(SceneCopy.m_root,"conMain_SceneCopy",WZUIContainer)
    con:addChild(cell)
    cell:setZOrder(1)
    tcell:setTopData(imgPath,SceneCopy,SceneCopy.onReturn,true,true,chatFlag,"SceneCopy")
...
function SceneCopy:_initUI()
    local imgPath = {
        "ui/common/common_icon_drfb.png",
        "ui/common/common_scale9_zuduifb.png",
        "ui/common/common_icon_rcfb.png",
        "ui/common/common_icon_ptfb.png",
    }
    
    if self.copyType == 2 then
        self:_addTop(imgPath[self.copyType],true)
    else
        self:_addTop(imgPath[self.copyType],false)
    end
    --获取近期进入的单人副本章节
