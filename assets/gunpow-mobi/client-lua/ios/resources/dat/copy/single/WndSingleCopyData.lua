...
function WndSingleCopy:getSectionRewardOK(sectionId, rewardNum, rewardId, rewardCount)
    ...
    self.m_bGetRewardItems = false
    self:_updateStarReward(self.m_nCurPageIndex+1)
end
...
function WndSingleCopy:receiveStartChallengeOk(mapId, mapType, itemId)
    ...
    local weaponSkill = ""
    -- for i , v in pairs (CacheCenter:getEquipList()) do
    --     WZLog("WndSingleCopy:receiveStartChallengeOk three-1", Serialize(v))
    --     if v.isUse == true and v.maintype == 4 and (v.subtype == 0 or v.subtype == 1) then
    --         WZLog("WndSingleCopy:receiveStartChallengeOk three-2")
    --         weaponSkill = v.extraInfo.weaponskill
    --         break
    --     end
    -- end
    ...
    if CacheCenter:getPlayerInfo().shapeId > 0 then
        ...
        local info = GDatatab_shape_skins["id_" .. CacheCenter:getPlayerInfo().shapeId]
        WZLog("WndSingleCopy:receiveStartChallengeOk_two22", CacheCenter:getPlayerInfo().shapeId, tostring(info))
        local shapeSkill = CacheCenter:getPlayerInfo().shapeSkillId or info.passive_skill[1][1]
        WZLog("WndSingleCopy:receiveStartChallengeOk_two222", tostring(shapeSkill), tostring(CacheCenter:getPlayerInfo().shapeSkillId))
        weaponSkill = weaponSkill .. "|" .. shapeSkill
        WZLog("WndSingleCopy:receiveStartChallengeOk_two2222", tostring(weaponSkill))
    end
    ...
    WBattleGlobal:getCurrent().m_tMakePairOk = {
        ...
        maxSP={[1]=0},
        attack={[1]=atk},
        critRate={[1]=CacheCenter.m_tPlayerInfo.critRate},
        defence={[1]=CacheCenter.m_tPlayerInfo.defend},
        injuryFree={[1]=CacheCenter.m_tPlayerInfo.injuryFree},
        wreckDefense={[1]=CacheCenter.m_tPlayerInfo.wreckDefense},
        reduceCrit={[1]=CacheCenter.m_tPlayerInfo.reduceCrit},
        reduceBury={[1]=0},
        power={[1]=power},
        armor={[1]=CacheCenter.m_tPlayerInfo.armor},
        constitution={[1]=CacheCenter.m_tPlayerInfo.physique},
        agility={[1]=CacheCenter.m_tPlayerInfo.agility},
        lucky={[1]=CacheCenter.m_tPlayerInfo.luck},
        ...
        petId={[1]=petAnim},
        petSkill={[1]=weaponSkill .. "|" .. (CacheCenter.m_tPlayerInfo.petInfo and CacheCenter.m_tPlayerInfo.petInfo.skill or "") .. "|" .. (CacheCenter.m_tPlayerInfo.awakeSkillId or "")},
        petSkillId={[1]=0},
    ...
    WZLog("WndSingleCopy:receiveStartChallengeOk two", Serialize(CacheCenter:getEquipList()))
    ...
    for i,v in ipairs(tSingleCopyData) do
        ...
        if tLevelData then
            if self.m_nCopyType == tLevelData.map_type then
                ...
                if tLevelData.section*100 + nLevelIndex > self.m_nCurCopyIndex*100 + self.m_nCurLevelIndex then
...
--@brief   显示单人副本类型按钮
function WndSingleCopy:showCopyType(curPage)
    WZLog(" WndSingleCopy:showCopyType = ",self.m_nCopyType,tostring(curPage))
    local cbCommon = GetElement(curPage,"cbCommon_WndSingleCopy",WZUICheckBox)
    local cbElite = GetElement(curPage,"cbElite_WndSingleCopy",WZUICheckBox)
    local cbDevil = GetElement(curPage,"cbDevil_WndSingleCopy",WZUICheckBox)
    if self.m_nCopyType == 1 then
        cbCommon:setCheckIndex(1)
        cbElite:setCheckIndex(0)
        cbDevil:setCheckIndex(0)
    elseif self.m_nCopyType == 2 then
        cbCommon:setCheckIndex(0)
        cbElite:setCheckIndex(1)
        cbDevil:setCheckIndex(0)
    elseif self.m_nCopyType == 3 then
        cbCommon:setCheckIndex(0)
        cbElite:setCheckIndex(0)
        cbDevil:setCheckIndex(1)
    end
...
function WndSingleCopy:getCuPageObject()
    WZLog("WndSingleCopy:getCuPageObject = ",self.m_nCurPageIndex)
    local pgconCopy = GetElement(self.m_root, "pgconCopy_WndSingleCopy", WZUIPageContainer)
    local cell = pgconCopy:getPageElement(self.m_nCurPageIndex)
    if cell~= nil then
...
function WndSingleCopy:_getSectionRewardStateByIndex(nIndex, pageIndex, copyType)
    ...
    if nState == 0 then
        ...
        if nStarNum >= tRewardLocalData["condition"..nIndex] then
            nState = 2
...
--@brief    获取小岛数据成功
function WndSingleCopy:getIslandDataOk(seizeMapId, nextSeizeMapId)
    -- body
    self.m_tIslandHostId = seizeMapId  --已占岛Id
    self.m_tNextHostId = nextSeizeMapId    --候选Id
...
function WndSingleCopy:setIslandBtnVisible(element)
    -- body
    if #self.m_tIslandHostId > 0 or #self.m_tNextHostId > 0 then
        GetElement(element, "btnIsland_WndSingleCopy", WZUIButton):setVisible(true)
    else
        GetElement(element, "btnIsland_WndSingleCopy", WZUIButton):setVisible(false)
    end
end
...
function WndSingleCopy:setSectionListData()
    ...
    for i, value in pairs(GDatatab_section) do
        ...
        if not bExist then 
            ...
            tItem.resources = value.resources
