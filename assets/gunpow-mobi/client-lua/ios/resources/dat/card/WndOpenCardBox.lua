...
function WndOpenCardBox:sureUseDiamondInstead()
    ...
    ProtocolProcessorCard:send_CARD_OpenCardSet(self.m_tData.item_id)
end
...
function WndOpenCardBox:openCardBoxOK(code, itemId, itemNum, cdTime, openNum)
    ...
    if code == 0 then --打开卡套成功
        ...
        self:_refreshTime()
        --刷新数据
        WndCard:refreshCardDataAndInfo(CopyTable(itemId), CopyTable(itemNum), cdTime, openNum, self.m_tData)
...
function WndOpenCardBox:_update()
    ...
    local txtTitle = GetElement(self.m_root, "txtTitle_WndOpenCardBox", WZUILabelTTF)
    ...
    if self.m_tData.basicInfo.sub_type == 1 then  --卡包
        txtBtnText:setText(LocalStrings.CARD_TEXT27)
        txtTitle:setText(LocalStrings.CARD_TEXT27)
        --图标
        imgCardBoxIcon:setFile(self.m_tData.icon)
        imgSection:setVisible(false)
    else
        txtBtnText:setText(LocalStrings.CARD_TEXT10)
        txtTitle:setText(LocalStrings.CARD_TEXT10)
        --图标
    ...
    if self.m_tData.at_least_2 > 0 then
        local sContent3 = string.format(LocalStrings.CARD_TEXT12, "ui/common/common_icon_zk.png", self.m_tData.at_least_2)
        txtFreeText3:setShowText(sContent3)
    end
    local txtFreeText4 = GetElement(self.m_root, "txtFreeText4_WndOpenCardBox", WZUIFreeTextBox)
    if self.m_tData.at_least_3 > 0 then
        local sContent4 = string.format(LocalStrings.CARD_TEXT13, "ui/common/common_icon_zk.png", self.m_tData.at_least_3)
        txtFreeText4:setShowText(sContent4)
    end

    if self.m_tData.at_least_2 == 0 and self.m_tData.at_least_3 == 0 then
        txtCardBoxName:setRelativePosition(GlobalMethod:ccp(0,0.75))
        txtFreeText1:setRelativePosition(GlobalMethod:ccp(0,0.6))
        txtFreeText2:setRelativePosition(GlobalMethod:ccp(0,0.45))
        txtFreeText3:setVisible(false)
        txtFreeText4:setVisible(false)
        txtFreeText5:setRelativePosition(GlobalMethod:ccp(0,0.3))
    elseif self.m_tData.at_least_2 ~= 0 and self.m_tData.at_least_3 == 0 then
        txtCardBoxName:setRelativePosition(GlobalMethod:ccp(0,0.75))
        txtFreeText1:setRelativePosition(GlobalMethod:ccp(0,0.6))
        txtFreeText2:setRelativePosition(GlobalMethod:ccp(0,0.45))
        txtFreeText3:setRelativePosition(GlobalMethod:ccp(0,0.3))
        txtFreeText5:setRelativePosition(GlobalMethod:ccp(0,0.15))
    elseif self.m_tData.at_least_2 == 0 and self.m_tData.at_least_3 ~= 0 then
        txtCardBoxName:setRelativePosition(GlobalMethod:ccp(0,0.75))
        txtFreeText1:setRelativePosition(GlobalMethod:ccp(0,0.6))
        txtFreeText2:setRelativePosition(GlobalMethod:ccp(0,0.45))
        txtFreeText4:setRelativePosition(GlobalMethod:ccp(0,0.3))
        txtFreeText5:setRelativePosition(GlobalMethod:ccp(0,0.15))
    end
    --有几率获得的卡牌列表
...
function WndOpenCardBox:_refreshTime()
    ...
    if self.m_nTime == 0 then
        ...
        GetElement(self.m_root, "conCDTime_WndOpenCardBox", WZUIContainer):setVisible(true)
    end
...
function WndOpenCardBox:_createCardList()
    ...
    if self.m_nMySimpleCopyId == nil then
        ...
        if ProjConfig.LANGUAGE == "en" or ProjConfig.LANGUAGE == "pt"
            ...
            ShowPanelNullTip(conCardList,LocalStrings.CARD_TEXT29,nil,nil,nil,nil,1)
        end
    ...
    for i = 1, #tCardList do
        ...
        if celElement and tNewObj then
            tNewObj:setDataOther(tCardList[i], GlobalMethod:ccp(0.5,0.46))
            celElement:setTag(i - 1)
            celElement = WZUIContainer:luaTo(celElement)
            celElement:setAbsContentSize(GlobalMethod:CCSize(114,170))
            celElement:setRelativeSize(GlobalMethod:CCSize(114/520, 1))
