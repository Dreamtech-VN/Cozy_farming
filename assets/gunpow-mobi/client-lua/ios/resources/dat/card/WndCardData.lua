...
function WndCard:_init()
    ...
    self.m_nUnCollectCardNum = nil        --未收集的卡牌数量
    self.m_nListTag = 0
    ...
    self.m_nOperateType = 0             --操作状态值99：凌晨商店刷新；0：进入界面；1：升级；2：点击卡套标签；3：点击商店标签；4：购买卡牌
...
function WndCard:_unInit()
    ...
    self.m_nTime = nil 
    self.m_nLeftListIndex = nil         --卡牌选项卡索引
    self.m_nCollectCardNum = nil        --已收集的卡牌数量
    self.m_nUnCollectCardNum = nil      --未收集的卡牌数量
    self.m_nListTag = nil 
    self.m_tActiveCardList = nil        --已经激活的卡
    self.m_tUnActiveCardList = nil      --未激活的卡
...
--@brief    外部接口
--@param    nTabIndex: 顶部标签索引：1->卡牌；2->卡套；3->商店
--@param    卡牌系统的外部接口
...
--@brief    设置数据
function WndCard:setData(itemId, level, num, lookItemId)
    -- body
    --已激活的卡牌
...
function WndCard:setCardBoxData(cardSetId, count, cdTime, openNum)
    WZLog("WndCard:setCardBoxData", self.m_nTime)
    -- body
    self.m_nTime = cdTime
...
function WndCard:refreshCardDataAndInfo(itemId, itemNum, cdTime, openNum, operateData)
    -- body
    self.m_nTime = cdTime
    self.m_nLeftOpenTimes = self.m_nTotalOpenTimes - openNum
    --刷新时间
...
function WndCard:_getNextData(itemId, curLevel)
    -- body
    for i, value in pairs(GDatatab_card_property) do
        if value.item_id == itemId and value.level == curLevel + 1 then
            return value
        end
    end
