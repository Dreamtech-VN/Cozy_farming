...
function TeachStepGroup3:_getTeachElementById( id, teachType, groupId, uiId )
    ...
    elseif id == self.Step_11_6 then
        if WndMultipleMap.m_root ~= nil then
        if WndChallengeLevel.m_root ~= nil or Teach:getTaskState(-11) == 2 then
        return
        end
    ...
    elseif id == self.Step_11_7 then
        ...
        if WndChallengeLevel.m_root ~= nil then
            element = GetElementWithoutAssert(WndChallengeLevel.m_root, "checkBoxSimple_WndChallengeLevel", WZUICheckBox)
            element:setZOrder(500)
        end
    elseif id == self.Step_11_8 then
        if WndChallengeLevel.m_root ~= nil then
            element = GetElementWithoutAssert(WndChallengeLevel.m_root, "btnSure_WndChallengeLevel", WZUIButton)
            if element ~= nil and element:getTouchEnable() ~= true then
            element = nil
            end
        end
