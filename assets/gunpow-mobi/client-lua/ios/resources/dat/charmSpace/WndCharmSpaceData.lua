...
--@brief	接收鲜花榜的总排行榜数据
function WndCharmSpace:setData3(ranking,playerId,name,sex,level,param1,param2,param3,param4,param5,param6)
	self.rank = ranking
...
--@brief	接收点赞榜的总排行榜数据
function WndCharmSpace:setDataFashion(ranking, playerId, name, faceId, headId, sex, level, param1, param2, param3, param4, param5, param6, param7, vipLevel, param8, headColor, param9)
	self.rank = ranking
	self.flowerNum = param1
	self.partner = param2
	self.community = param3
...
--@brief 	获取玩家报名信息数据成功
function WndCharmSpace:getPlayerFashionInfoOK(playerId, playerName, level, headId, faceId, bodyId, wingId, colour, bodyColor, sex, like, isApply, time, isRecomm)
	-- body
	...
	WZLog("WndCharmSpace:getPlayerFashionInfoOK", isApply, time, isRecomm)
...
--@brief 	报名、推荐成功
function WndCharmSpace:applyOrRecommendSuccess(result, time, operationType)
	-- body
	WZLog("WndCharmSpace:applyOrRecommendSuccess", result, time, operationType)
...
--@brief 	获取推荐数据成功
function WndCharmSpace:getFashionRecommendDataOK(playerId, name, level, headId, faceId, bodyId, wingId, headColor, bodyColor, cross, sex, like, isRecomm, likeNum)
	-- body
...
--@brief 	搜索玩家成功
function WndCharmSpace:searchFashionPlayerOK(playerId, name, level, headId, faceId, bodyId, wingId, headColor, bodyColor, cross, sex, like, isRecomm)
	-- body
	WZLog("WndCharmSpace:getFashionRecommendDataOK")
	self.m_tFashionRecommendData = {}
...
--@brief 	获取历届数据成功
function WndCharmSpace:getFashionPeriodDataOK(playerId, name, level, headId, faceId, bodyId, wingId, headColor, bodyColor, cross, sex, periodNum)
	-- body
...
function WndCharmSpace:giveGoodOK(result, playerId)
	--body
