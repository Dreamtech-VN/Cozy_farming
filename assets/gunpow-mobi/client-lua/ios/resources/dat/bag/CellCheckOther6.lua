...
--@note		在这里做场景进入前的准备工作
function CellCheckOther6:onEnter(element)
	self.m_root = element
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function CellCheckOther6:onExit(element)
	self:_unInit()
	self.m_nType = nil
	self.m_tDataList = nil
	self.m_nBtnTag = nil
...
-------------------------------------私有方法模块Begin--------------------------------------
--@brief	点击图标
function CellCheckOther6:onClick(element)
	WZLog("CellCheckOther6:onClick",element:getTag())
...
--@brief	设置高亮
function CellCheckOther6:setHighLight(bool)
...
--@brief	更新标题
function CellCheckOther6:update(i)
	...
	GetElement(self.m_root,"txtTitle_CellCheckOther6",WZUILabelTTF):setText(title[i])
	...
	--		GetElement(self.m_root,"btnSave_CellCheckOther6",WZUIButton):setVisible(true)
	--	else
	--		GetElement(self.m_root,"btnSave_CellCheckOther6",WZUIButton):setVisible(false)
	--	end
	--end
end

--@brief	设置标题
function CellCheckOther6:setTitle(title)
	GetElement(self.m_root,"txtTitle_CellCheckOther6",WZUILabelTTF):setText(title)
end

--@brief	设置签名
function CellCheckOther6:setSignature(signature)
	GetElement(self.m_root,"editSign_CellCheckOther6",WZUIEditBox):setText(signature)
	if signature == LocalStrings.NONE or signature == "" then
		GetElement(self.m_root,"editSign_CellCheckOther6",WZUIEditBox):setText(LocalStrings.BAGTIP1)
	end 
	GetElement(self.m_root,"conIcon_CellCheckOther6",WZUIContainer):setVisible(false)
end

--@brief	显示坐骑
function CellCheckOther6:showMounts(tData)
	--WZLog("CellCheckOther6:showMounts",Serialize(tData))
	self.m_nType = 1
...
--@brief	显示足迹
function CellCheckOther6:showFootMark(tData)
	WZLog("CellCheckOther6:showFootMark",Serialize(tData))
	self.m_nType = 5
	local qualityPic = {"ui/common/common_scale9_lv.png",
					"ui/common/common_scale9_lan.png",
					"ui/common/common_scale9_zi.png",
...
function CellCheckOther6:showStarSoul(tData)
	self.m_nType = 2
	local num = 0
	self.m_tDataList = {}
	for i=1,#tData do
...
function CellCheckOther6:showPrays(tData)
	self.m_nType = 3
	local num = 0
	self.m_tDataList = {}
	for i=1,6 do
...
--@brief	显示修炼
function CellCheckOther6:showPractice(tData, tData1)
	self.m_nType = 4
