--CellDesignationTwoData.lua
--@brief	CellDesignationTwo的数据模块
--@date		2015/03/26
--@author	clc
...
function CellDesignationTwo:_init()
...
function CellDesignationTwo:_unInit()
	self.m_root = nil
...
--@note		请仅用此方法创建场景
function CellDesignationTwo:createElement()
	local tNewObj = self:_new()
	assert(tNewObj, "CellDesignationTwo table create failed!")
	tNewObj:_init()

	local element = WZUIContainer:create()
    element:setUseAbsSize(true)
    element:setName("__CellDesignationTwo")
    element:setAbsContentSize(GlobalMethod:CCSize(590,103))
    element:setLuaObjectIndex(tNewObj)
    return element,tNewObj
end

--@brief 	设置特殊称号数据
function CellDesignationTwo:setDesiData(tData)
	-- body
...
--@brief  设置子成就Cell的UI
function  CellDesignationTwo:setCellUI(title ,desc, status, reward, nComplete, nTarget)
	-- body
    WZLog("CellDesignationTwo:setCellUI",title,status)
    if self.m_tData == nil then
        self.m_tData = {}
    end
...
--@return	新建的表实例对象
function CellDesignationTwo:_new( )
	local tNewObj = {}
