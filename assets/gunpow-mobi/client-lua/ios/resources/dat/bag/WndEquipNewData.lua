...
--@brief	全部列表
function WndEquipNew:setAllData(tArms)
...
--@brief	武器列表
function WndEquipNew:setArmsData(tArms)
	if self.m_root == nil or tArms == nil then
		return
	end
	--self.m_tItem = CopyTable(tArms)
...
--@brief	装扮列表
function WndEquipNew:setDressData(tDress)
...
--@brief	宝石列表
function WndEquipNew:setGemData(tData)
...
--@brief	其它列表
function WndEquipNew:setOtherData(tOther)
...
--@brief	皮肤列表
function WndEquipNew:setSkinData(tOther)
	if self.m_root == nil or tOther == nil then
		return
	end
	--self.m_tItem = CopyTable(tOther)
...
--@brief	更新玩家物品信息(数据更新)
function WndEquipNew:updatePlayerItemData()
	if self.m_root == nil then return end
	self:updateItemList()
end

function WndEquipNew:updateItemList()
	if self.m_root == nil then
		return
	end
	if self.m_nBagIndex == 1 then
		self:setArmsData(CacheCenter:getEquipList())--武器列表
	elseif self.m_nBagIndex == 2 then
		self:setDressData(CacheCenter:getPropList())--道具列表
	elseif self.m_nBagIndex == 3 then
		self:setOtherData(CacheCenter:getBagMaterialList())--材料列表
	elseif self.m_nBagIndex == 4 then
		self:setOtherData(CacheCenter:getGemList())--宝石列表
	elseif self.m_nBagIndex == 5 then
		self:setSkinData(CacheCenter:getSkinAndFootList())--皮肤列表
	else
		self:setArmsData(CacheCenter:getAllList())--全部列表
	end
	self:_updateItem()--更新物品列表
end

--@brief	更新玩家身上装备
function WndEquipNew:updateEquipItem(tData)
...
--@brief	按钮回调
function WndEquipNew:setBtnBackFun(lua,onStrongClick,onSaleClick)
...
--@note		请仅用此方法创建场景
function WndEquipNew:createElement()
	local element = WZUISystem:getInstance():createElement("WndEquipNew")
	assert(element, "WndEquipNew create element failed!")
	self:_init()
	return element
end

function WndEquipNew:setItemBackFun(tCell,backFun,onItem)
...
--@brief	设置文本内容属性
function WndEquipNew:_setTxtDesc(tCell,desc)
...
--显示tip信息
function WndEquipNew:_addTip(tItem,element,pCell)
...
--@brief  获取两个值的最大
function WndEquipNew:_getMaxSort(x)
	if x == true then
		return 1 
	else
		return 0
...
function WndEquipNew:_getMaxRecommended(x,y)
	if x == true then
		return 1 
	else
		return 0
...
--是否过期
function WndEquipNew:_getMaxExpired(x)
	if x == true  then
		return 0 
	else
		return 1
...
--装备类型
function WndEquipNew:_getMaxType(tData)
...
function sortAll(a,b)
	local x = WndEquipNew:_getMaxSort(a.isUse)--穿上
	local y = WndEquipNew:_getMaxSort(b.isUse)--穿上
	local r = WndEquipNew:_getMaxRecommended(a.recommended,a.expired)--推荐
	local t = WndEquipNew:_getMaxRecommended(b.recommended,b.expired)--推荐
	local n = WndEquipNew:_getMaxExpired(a.expired)
	local m = WndEquipNew:_getMaxExpired(b.expired)
	local e = WndEquipNew:_getMaxType(a)
	local f = WndEquipNew:_getMaxType(b)
	if x ~= y then--是否已装备
		return x >= y 
...
function sortOther(a,b)
	local n = WndEquipNew:_getDay(a)
	local m = WndEquipNew:_getDay(b)
	if n ~= m then
		return n >= m 
	elseif a.basicInfo.quality ~= b.basicInfo.quality then
		return a.basicInfo.quality > b.basicInfo.quality 
...
function WndEquipNew:_getDay(ab)
...
--@brief  获取两个值的最大
function WndEquipNew:_getMaxdays(d)
	if d == -1 then
		return 1 
	else
		return 0
...
--@brief  获取当前穿上的装扮的tag
function WndEquipNew:_getCurItemUseTag(tItem)
	for i,data in pairs(self.m_tItem) do 
...
--这个函数是针对旧的戒指做判断
function WndEquipNew:_checkRank(data,tItem,i)
...
--@brief  排序
function WndEquipNew:_sortTable(tag)
	if tag == 1 then
		table.sort(self.m_tItem , sortArms)
	elseif tag == 2 then
		table.sort(self.m_tItem , sortDress)
...
--@brief   显示文本属性
function WndEquipNew:_setTxtDesc(element,desc)
...
function WndEquipNew:_adaptLanguage_vn()
	...
	WZUILabelTTF:luaTo(self.m_root:getChildElement("txtArms_WndEquipNew")):setFontSize(20)
	WZUILabelTTF:luaTo(self.m_root:getChildElement("txtDress_WndEquipNew")):setFontSize(20)
	WZUILabelTTF:luaTo(self.m_root:getChildElement("txtOther_WndEquipNew")):setFontSize(20)
	WZUILabelTTF:luaTo(self.m_root:getChildElement("txtOther_WndEquipNew")):setFontSize(20)
	WZUILabelTTF:luaTo(self.m_root:getChildElement("txtOther_WndEquipNew")):setFontSize(20)
end
