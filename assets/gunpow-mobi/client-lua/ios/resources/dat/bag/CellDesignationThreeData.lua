--CellDesignationThreeData.lua
--@brief	CellDesignationThree的数据模块
--@date		2015/03/27
--@author	clc
--@note		成就系统-称号面板-称号cell

CellDesignationThree = {
	-- 请在这里定义和初始化全局成员变量
}

--@brief	定义并初始化表的实例成员变量
--@note		表的实例变量必须在这里定义和初始化
function CellDesignationThree:_init()
	self.m_root         = nil   --Cell的根节点
	self.nDesignationId = -1    --称号id
	self.nType          = -1    --称号类型
	self.nPos           = -1    --Cell的位置

	self.nStatus        = 0     --称号的状态  状态，0不可用，1，可用，2，使用中
	self.m_bIsLoad 		= false
	self.m_tData 		= nil 
	self.m_bGouVisible  = nil
end

--@brief	反初始化表的成员变量
--@note		在退出场景时回调的onExit函数里面必须调用本函数
function CellDesignationThree:_unInit()
	self.m_root         = nil
	self.nDesignationId = nil    --称号id
	self.nType          = nil    --称号类型
	self.nPos           = nil    --Cell的位置

	self.nStatus        = nil     --称号的状态  状态，0不可用，1，可用，2，使用中
	self.m_bIsLoad 		= nil 
	self.m_tData 		= nil
	self.m_bGouVisible  = nil
end


-------------------------------------公有方法模块Begin--------------------------------------

--@brief	创建控件
--@return	#1，控件element的引用
--@return	#2, 表的引用，也可以用element:getLuaObjectIndex()
--@note		请仅用此方法创建场景
function CellDesignationThree:createElement()
	local tNewObj = self:_new()
	assert(tNewObj, "CellDesignationThree table create failed!")
	tNewObj:_init()

	local element = WZUIContainer:create()
    element:setUseAbsSize(true)
    element:setName("__CellDesignationThree")
    element:setAbsContentSize(GlobalMethod:CCSize(773,75))
    element:setLuaObjectIndex(tNewObj)
    return element,tNewObj
end

-------------------------------------公有方法模块End----------------------------------------


-------------------------------------私有方法模块Begin--------------------------------------

--@brief	以本表为模版创建一个新的表实例对象
--@return	新建的表实例对象
function CellDesignationThree:_new( )
	local tNewObj = {}
	setmetatable(tNewObj, self)
	self.__index = self
	return tNewObj
end

-------------------------------------私有方法模块End----------------------------------------
