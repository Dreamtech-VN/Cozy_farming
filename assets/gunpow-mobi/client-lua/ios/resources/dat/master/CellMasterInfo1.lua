...
function CellMasterInfo1:onAccept(element)
	WZLog("CellMasterInfo1:onAccept")
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)  --点击音效
	g_nOperatePlayerId = tonumber(self.m_tData.playerId)
	ProtocolProcessorWndMaster:send_MENTORING_Processing(tonumber(self.m_tData.playerId), 1 )
end

--@brief	拒绝请求
function CellMasterInfo1:onRefuse(element)
	WZLog("CellMasterInfo1:onRefuse")
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)  --点击音效
	g_nOperatePlayerId = tonumber(self.m_tData.playerId)
	ProtocolProcessorWndMaster:send_MENTORING_Processing(tonumber(self.m_tData.playerId), 0 )
end
-------------------------------------公有方法模块End----------------------------------------
...
function CellMasterInfo1:setMasterInfo1(tData)
	...
	if textTable.name ~= nil then
		GetElement(self.m_root,"ttfName_CellMasterInfo1",WZUILabelTTF):setText(textTable.name)
	end
