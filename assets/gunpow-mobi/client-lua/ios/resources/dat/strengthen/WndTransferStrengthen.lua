...
function WndTransferStrengthen:onTransfer(element)
	...
	--保存宝石id
	if self.m_weapon1LuaObj.m_tItem.extraInfo ~= nil and self.m_weapon1LuaObj.m_tItem.extraInfo.hpStone ~= nil then
		self.stone1 = tonumber(self.m_weapon1LuaObj.m_tItem.extraInfo.hpStone)
	else
		self.stone1 = nil
	end
	if self.m_weapon1LuaObj.m_tItem.extraInfo ~= nil and self.m_weapon1LuaObj.m_tItem.extraInfo.attackStone ~= nil then
		self.stone2 = tonumber(self.m_weapon1LuaObj.m_tItem.extraInfo.attackStone)
	else
		self.stone2 = nil
	end
	if self.m_weapon1LuaObj.m_tItem.extraInfo ~= nil and self.m_weapon1LuaObj.m_tItem.extraInfo.defendStone ~= nil then
		self.stone3 = tonumber(self.m_weapon1LuaObj.m_tItem.extraInfo.defendStone)
	else
		self.stone3 = nil
	end
	if self.m_weapon1LuaObj.m_tItem.extraInfo ~= nil and self.m_weapon1LuaObj.m_tItem.extraInfo.gongmingStone ~= nil then
		self.stone4 = tonumber(self.m_weapon1LuaObj.m_tItem.extraInfo.gongmingStone)
	else
		self.stone4 = nil
	end
	--是否橙装转移给紫装
...
function WndTransferStrengthen:onTransferSuccess()
	...
	--刷新显示效果
	self.m_weapon2LuaObj:setCellGoodItem(self.m_weapon2LuaObj.m_tItem,1)
...
function WndTransferStrengthen:addEquipToCell(tEquip,tag)
    ...
    if tEquip == nil then
        ...
        self.m_weapon1LuaObj:setCellGoodItem(tEquip,1) --添加到cell
        if tag ~= 1 then
            self.m_weapon2LuaObj:removeAllChild()
            WndStrengthen:updateEquipList()
        end
...
function WndTransferStrengthen:addEquipToCell2(tEquip)
    ...
    self.m_weapon2LuaObj:setCellGoodItem(tEquip,1) --添加到cell
    self:_updateEquipInfo2()
...
function WndTransferStrengthen:_updateEquipInfo1()
    ...
    if self.m_weapon1LuaObj.m_tItem ~= nil then
        ...
        local eStarAttr1 = 0
        if eStrLv1 > 0 then
            eStrAttr1 = WndIntensifyStrengthen:getStrengthenTableInfo(eQuality,eStrLv1,property[1][1]).attrAdd
        end
        if eStarLv1 > 0 then
...
function WndTransferStrengthen:_updateEquipInfo2()
    ...
    if self.m_weapon2LuaObj.m_tItem ~= nil then
        ...
        local e2StarAttr = 0
        if eStrLv2 > 0 then
            eStrAttr2 = WndIntensifyStrengthen:getStrengthenTableInfo(eQuality,eStrLv2,attrType).attrAdd
        end
        if eStarLv2 > 0 then
            eStarAttr2 = property[1][2] * (GDatatab_stars_up["id_"..eStarLv2].property_rate/10000)
        end
        if e2AfterStrLv > 0 then
            e2StrAttr = WndIntensifyStrengthen:getStrengthenTableInfo(eQuality,e2AfterStrLv,attrType).attrAdd
        end
...
function WndTransferStrengthen:_updateWeaponTwoSkill(tWeapon1, tWeapon2)
    ...
    if #NewSkillList > #skillIdList then
