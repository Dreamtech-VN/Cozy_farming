...
--@brief    外部调用创建此窗口
--@param    tag:1宝石窗口，2装备窗口,3选择继承前装备
--@param    type:宝石或装备类别
function WndSelectTipsStrengthen:showSelectTips(tag,tData)
	WZLog("WndSelectTipsStrengthen:showSelectTips",tag)
	if self.m_root ~= nil then return end
    local selectTipsWin = WndSelectTipsStrengthen:createElement()
    --WindowManager:addWindow(selectTipsWin , WndSelectTipsStrengthen,nil,nil,nil,true)
