...
--@note		请仅用此方法创建场景
function SceneRoom:createElement()
	WZLog("SceneRoom:createElement")
	local element = WZUISystem:getInstance():createElement("SceneRoom")
	assert(element, "SceneRoom create element failed!")
	self:_init()
...
--@brief	设置房间信息   
function SceneRoom:setData(roomId,roomStatus, battleMode, roomChannel, playerNumMode,sechedule, mapId, wnersId, startMode, playerNum, seatUsed, playerId, playerName, playerLevel, playerReady, playerSex, playerEquipment, playerWeaponLevel, vipLevel, playerTitle,roomName,roomPassword,fighting,pet,tournamentLevel,winNum,playNum,extranInfo,serviceId,tournamentExp,headColors,bodyColors,mentoringStr,coupleStr,chumStr,coupleNum,chumNum,mentoringNum,qualifyingLevel,matchscore,joinTimes,winTimes,continuousWinTimes)
    ...
    WZLog("SceneRoom:setData = ",Serialize(self.m_tData))
...
function SceneRoom:receiveEnterRoomOk(roomId, roomStatus,battleMode, roomChannel, playerNumMode,sechedule,mapId, wnersId, startMode, playerNum, seatUsed, playerId, playerName, playerLevel, playerReady, playerSex, playerEquipment, playerWeaponLevel, vipLevel, playerTitle,roomName,roomPassword,fighting,pet,tournamentLevel,winNum,playNum,extranInfo,serviceId,tournamentExp,headColors,bodyColors,mentoringStr,coupleStr,chumStr,coupleNum,chumNum,mentoringNum,qualifyingLevel,matchscore,joinTimes,winTimes,continuousWinTimes)
	...
	WZLog("SceneRoom:receiveEnterRoomOk =",chumStr)
	if self.m_root and self.m_bIsVoice and self.m_bIsVoiceState and GetElement(self.m_root,"conFigureVoice1_SceneRoom",WZUIContainer) then
...
function SceneRoom:closeLoading()
...
--@brief	正在匹配中
--@param	入参与服务端发送给客户端的协议回调方法参数相同
function SceneRoom:receiveMakePairring(roomId)
	if self.m_root == nil then
		return
	end
	WZLog("SceneRoom:receiveMakePairring",roomId)
	if roomId == self.m_tData.roomId then
		...
		if self:getIsRoomOwner() and self.m_tData.startMode == 1 then
			self:changeStartGameBtn("ui/common/common_icon_qxpp.png")
		end
	end
end

--@brief  改开始游戏为取消匹配
function SceneRoom:changeStartGameBtn(iconPath)
	WZLog("SceneRoom:changeStartGameBtn")
...
--@brief	正在匹配中
--@param	入参与服务端发送给客户端的协议回调方法参数相同
function SceneRoom:receiveMakePairError(nFlag, sMessage)
	if self.m_root == nil then
		return
	end
	WZLog("SceneRoom:receiveMakePairError",nFlag, sMessage)
...
--@brief	匹配失败
--@param	入参与服务端发送给客户端的协议回调方法参数相同
function SceneRoom:receiveMakePairFail()
	if self.m_root == nil then
		return
	end
	WZLog("SceneRoom:receiveMakePairFail")
	self.m_bStartGame = false
	self:closeLoading()
	if self.m_tData.startMode == 1 then
		MsgBoxManager:showConfirmBox(LocalStrings.MATCHES_TIMEOUT,self,self.startTeach20_5,nil,nil,true)
	else
		MsgBoxManager:showConfirmBox(LocalStrings.MATCH_FAILED,self,self.startTeach20_5,nil,nil,true)
	end
	self:setAllBtnStats(true)
	if self:getIsRoomOwner() then
		self:changeStartGameBtn("ui/common/common_icon_ksyx.png")
...
--@brief  教学
function SceneRoom:startTeach20_5()
	TeachGroup1:startGroup({20,5,SceneRoom.m_root})
end

--@brief  是否已点击开始游戏按钮
function SceneRoom:isStartGame()
	WZLog("SceneRoom:isStartGame")
	local imgStartGame = GetElement(self.m_root,"imgStartGame_SceneRoom",WZUIImage)
	if self:getIsRoomOwner() then
		if imgStartGame:getFile() == "ui/common/common_icon_ksyx.png" then
		    return false
	    end
	end
...
--@brief	匹配完成
--@param	入参与服务端发送给客户端的协议回调方法参数相同
function SceneRoom:receiveMakePairOk(battleId, battleMode, battleChannle, schedule, mapId, 
	...
	playerLevel, playerSex, maxHP, maxPF, maxSP, attack, critRate, defence, injuryFree, wreckDefense,
	 ...
	 weaponSkill,tournamentLevel,teamId,teamName,url,petLevel, colour, bodyColour, isCaptain,footmark, monsterId)
	if self.m_root == nil then
		return
	end
...
teamId =teamId,teamName=teamName,url=url, colour=colour, bodyColour=bodyColour, isCaptain=isCaptain,footmark = footmark, monsterId = monsterId}
	...
	--等待一秒执行加载战斗场景界面
    WZLog("SceneRoom:receiveMakePairOk",tostring(GlobalGame.g_tPlayerInfo.nLevel),tostring(GlobalGame.g_tSysConfig.openTipLevel),tostring(GlobalGame.g_tPlayerInfo.nLevel),tostring(self.m_nPairRemainTime), Serialize(buffId))
...
--@brief	退出房间
--@param	入参与服务端发送给客户端的协议回调方法参数相同
function SceneRoom:receiveQuitRoomOk(mark)
	WZLog("SceneRoom:receiveQuitRoomOk = ",mark)
	if self.m_root == nil then
		return
	end
...
function SceneRoom:removeBottomBar()
...
function SceneRoom:exitClose()
	WZLog("SceneRoom:exitClose")
	if self.m_root == nil then
		return
	end
    ...
    elseif self.m_tData.battleMode == GlobalGame.g_tBattleMode.BATTLE_MODE_JH and self.m_tData.roomChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_YL then --均衡赛
		ScenePvpAmuse:showScene()
	elseif self.m_tData.battleMode == GlobalGame.g_tBattleMode.BATTLE_MODE_JJ and self.m_tData.roomChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_LX then
		replaceScene(ScenePvp:createElement())
    else -- 对战赛
		replaceScene(SceneHall:createElement())
    end
end

function SceneRoom:setPlayerTeam(playerId,team)
	WZLog("SceneRoom:setPlayerTeam")
	GlobalGame.g_nPlayerInTeam = team
end

--显示当前房间的玩家聊天信息
function SceneRoom:showChat(txtMsg,playerId,bubbleId)
	WZLog("SceneRoom:showChat ",txtMsg,playerId)
	...
	if seatIndex > 0 then
		...
		local conSeat = GetElement(conCenter,"conSeat" .. seatIndex .. "_SceneRoom",WZUIContainer)
		local conPlayer = GetElement(conSeat,"conPlayer_SceneRoom")
...
function SceneRoom:checkCellChatBubble(seatIndex)
	WZLog("SceneRoom:checkCellChatBubble ")
	local cellChatBubble = self.m_root:getChildByTag(seatIndex+1110)
...
--保存各种关系的数值
function SceneRoom:initRelationShip()
	WZLog("SceneRoom:initRelationShip")
	self.m_MasterList = nil   --师徒关系列表
    self.m_FriendList = nil   --朋友关系列表
    self.m_SpouseList = nil   --夫妻关系列表
...
--解析后的好友度信息使用按规则保存,返回
--@param  玩家ID
function SceneRoom:getFriendRV(playerId)
    WZLog("SceneRoom:getFriendRV")
    local temp3 = {}
...
--解析师徒信息按规则保存,返回
--@param  玩家ID
function SceneRoom:getMasterRV(playerId,playerLevel)
    WZLog("SceneRoom:getMasterRV = ",Serialize(self.m_MasterList))
...
--解析夫妻信息按规则保存,返回
--@param  玩家ID
function SceneRoom:getSpouseRV(playerId,playerSex,playerName)
	WZLog("SceneRoom:getSpouseRV ")
...
--@brief 	获取是否可以踢出房间
function SceneRoom:getClickSeat()
	-- body
	if SceneRoom.m_root == nil then return nil end

	return self.m_bCanClickSeat
end

--@brief    更新多套时装数据
function SceneRoom:updateDressSuitData(nType)
    -- body
...
--@param	物品id表
--@return   物品路径表
function SceneRoom:idToPath(idTable)
...
--@brief  添加顶部导航栏
function SceneRoom:addTop()
	WZLog("SceneRoom:addTop")
	local cell,tcell = CellTopHandle:createElement()
    self.m_root:addChild(cell)
    self.m_tTopHangle = tcell
    self.m_oTopObject = cell
    tcell:setTopData("ui/common/common_icon_jingjifangji.png",SceneRoom,SceneRoom.onCloseClick,true,true,true,"SceneRoom")
end
