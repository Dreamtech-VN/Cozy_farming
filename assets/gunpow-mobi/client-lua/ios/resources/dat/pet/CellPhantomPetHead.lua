--CellPhantomPetHead.lua
--@brief	CellPhantomPetHead的UI模块
--@date		2017/07/26
--@author	Tianxiang_Xu
--@note		家园建筑节点
...
--@note		在这里做场景进入前的准备工作
function CellPhantomPetHead:onEnter(element)
	self.m_root = element
end
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function CellPhantomPetHead:onExit(element)
	self:_unInit()
end

--@brief    加载
function CellPhantomPetHead:onLoadData(element)
    -- body
    self:_update()
end

--@brief    点击头像回调
function CellPhantomPetHead:onClickHead(element)
    -- body
...
-------------------------------------私有方法模块Begin--------------------------------------
--@brief    刷新
function CellPhantomPetHead:_update()
    -- body
...
--@brief    创建建筑物容器节点
function CellPhantomPetHead:_createBtnBuilding()
    -- body
    btnBuilding = WZUIButton:create()
    btnBuilding:setName("btnHead_CellPhantomPetHead")
...
function CellPhantomPetHead:setItemStateWord(text)
    -- body
    if text then
...
function CellPhantomPetHead:setItemSelState(bVisible)
    -- body
    if bVisible then
