...
function CellPetFetterItem:_update()
	...
	if txtDesc then 
		txtDesc:setText(string.format(LocalStrings.PET_FETTER3, self.m_tData.fetters[1][3], self.m_tData.fetters[1][2]))
	end
	--状态
	...
	for i = 1, #self.m_tData.attribute do
		local txtProperty = GetElement(self.m_root, "txtProperty" .. i .. "_CellPetFetterItem", WZUILabelTTF)
		if txtProperty then 
			if self.m_tData.attribute[i][1] == 99 then 
				txtProperty:setText(LocalStrings.INTELLIGENCE .. "+" .. self.m_tData.attribute[i][2])
			else
				txtProperty:setText(ATTR_TITLE[self.m_tData.attribute[i][1]] .. "+" .. self.m_tData.attribute[i][2])
			end
		end
	end
	--宠物头像
