...
function WndPetRaffle:getTime(type,time)
    ...
    local isVisible = WZUIContainer:luaTo(self.m_root:getChildElement("conPetRaffle3_WndPetRaffle")):isVisible()
...
function WndPetRaffle:onItemClick(element,tag,tData)
  ...
  -- 	WndPets:setAptitudePost(self.m_root, "conPetAptitude4_WndPetRaffle",quality)
    ...
    local sLevel = string.format([[<I>%s</I><T C=%s S="24" P="1" SE="1" SS="4" SC="79,60,48">%s</T>]],s0, color, name)
...
function WndPetRaffle:_setLocalText()
	...
	for i =1, 3 do
		GetElement(self.m_root,"txtDesc"..i.."_WndPetRaffle",WZUILabelTTF):setText(desc[i])		
	end
    ...
    imgExchangeGoods3:setScale(0.6)
...
function WndPetRaffle:_adaptLanguage_vn(  )
    ...
    local txtDrawCouponExplain2 = GetElement(self.m_root,"txtDrawCouponExplain2_WndPetRaffle",WZUILabelTTF)
    txtDrawCouponExplain2:setFontSize(16)
    txtDrawCouponExplain2:setRelativePosition(GlobalMethod:ccp(0.3,0.36))
    txtDrawCouponExplain2:setDimensions(GlobalMethod:CCSize(180))
