...
--@param	element:按钮的UI节点引用
--@note		在这里做相应的按钮相应事件
function WndActivityMenu:onClickActivity(element)
	WZLog("WndActivityMenu:onClickActivity")
    
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
    CheckLuaLoad(LUAFILES_BLOCK_COMMON)
    CheckLuaLoad(Chat_Channel_Active)
    local wndActive = WndActive:createElement()
    if wndActive then
        WindowManager:addWindow( wndActive , WndActive )
    end
end
