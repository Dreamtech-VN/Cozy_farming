...
function WndFootMark:changeFootMark(originFootMarkId, curFootMarkId)
	...
	--改变按钮的状态
	if originFootMarkId ~= 0 then
		self:_updateSingleCell(originFootMarkId,false)
		if curFootMarkId ~= 0 then
			self:_updateSingleCell(curFootMarkId,false)
		end
	else 
		if curFootMarkId ~= 0 then
			self:_updateSingleCell(curFootMarkId,false)
...
function WndFootMark:_getFootPriority(footMarkData)
    ...
    if footMarkData.remainTime <= nCurTime and footMarkData.remainTime ~= -1 then
    	...
    	local curCnt = CacheCenter:getPlayerFootItemCountById(costId)
...
function WndFootMark:setCellData(tag, element, tcell)
    ...
    self.m_tFootCellList[tag].element = element
...
function WndFootMark:_getMountMaxLevel(mount)
	local maxLv = CacheCenter:getPlayerInfo().level
