...
-- 初始化静态文本
function WndFootMarkUpgrade:_initStaticText()
    local txtFight = GetElement(self.m_root,"txtFight_WndFootMarkUpgrade",WZUILabelTTF)
...
function WndFootMarkUpgrade:_initMountInfo(isCreateMount,isLv,isStar)
    ...
    if isLv then
        ...
        local ftbMountName = GetElement(self.m_root,"ftbMountName_WndFootMarkUpgrade",WZUIFreeTextBox)
    ...
    if isStar then
        ...
        for i =1, 20 do
            ...
            if i >= 11 and i <= 20 and index == 2 then
                ...
                local star = GetElement(self.m_root,"imgStar"..starIdx.."_WndFootMarkUpgrade",WZUIImage)
...
function WndFootMarkUpgrade:_initLvPro(state)
    ...
    local starsSpecialAttr = CacheCenter:getFootMapProps()
    local _, sortDataRate, tAttrData2 = self:_getPropertyRate(starsSpecialAttr)
    for i = 1, 5 do
        -- 升级前属性
        ...
        if nextUpPro then
            local EndPro = math.ceil( math.ceil((basePro[i] + nextUpPro[i]) * (1 + starRate/10000)) * (1 + sortDataRate[i]/10000) )
            add:setText(EndPro)
        end
    end
    ...
    if state == 1 then
        ...
        -- 成功率
        local strRate = ""
        local tempRate = self:_getAddUpdateRate(starsSpecialAttr)
        if tempRate > 0 then
            strRate = " (+"..math.floor(tempRate * 100/10000).."%)"
        end
        local txtSuccess = GetElement(self.m_root,"txtLvSuccess_WndFootMarkUpgrade",WZUILabelTTF)
        txtSuccess:setText(math.floor(curUpData.probability * 100/10000).."%"..strRate)
...
function WndFootMarkUpgrade:_initStarPro(state)
    ...
    for i = 1, 5 do
        ...
        if nextStarData then
            local EndPro = math.ceil( math.ceil((basePro[i] + upPro[i]) * (1 + nextRate/10000)) * (1 + sortDataRate[i]/10000) )
            add:setText(EndPro)
        end
    end
...
function WndFootMarkUpgrade:_updateLuckyValue()
    WZLog("WndFootMarkUpgrade:_updateLuckyValue")
    if self.m_root == nil then return end

    local prgLucky = GetElement(self.m_root, "prgLucky_WndFootMarkUpgrade", WZUIProgress)
    
    -- 升下一阶的数据
    local nextAdvencedData = GDatatab_footmark_advanced["id_" .. (self.m_tData.advancedLevel + 1)]
    if nextAdvencedData == nil then
        nextAdvencedData = GDatatab_footmark_advanced["id_" .. self.m_tData.advancedLevel]
    end

    if prgLucky then
        prgLucky:setPercentage(self.m_tData.blessingValue/nextAdvencedData.blessing*100)
    end

    local txtLuckyValue = GetElement(self.m_root, "txtLuckyValue_WndFootMarkUpgrade", WZUILabelTTF)
    if txtLuckyValue then
        txtLuckyValue:setText(LocalStrings.LUCKVALUE .. ":" .. math.floor(self.m_tData.blessingValue/nextAdvencedData.blessing*1000)/10 .. "%")
    end
end
...
function WndFootMarkUpgrade:_updateUpLevelLuckyValue()
    ...
    -- 升下一级的数据
    local nextLevelData = GDatatab_footmark_upgrade["id_" .. (self.m_tData.upgradeLevel + 1)]
    if nextLevelData == nil then
        nextLevelData = GDatatab_footmark_upgrade["id_" .. self.m_tData.upgradeLevel]
    end

    if prgLucky then
        prgLucky:setPercentage(self.m_tData.upgradeBless/nextLevelData.blessing*100)
    end

    local txtLuckyValue = GetElement(self.m_root, "txtUpLuckyValue_WndFootMarkUpgrade", WZUILabelTTF)
    if txtLuckyValue then
        txtLuckyValue:setText(LocalStrings.LUCKVALUE .. ":" .. math.floor(self.m_tData.upgradeBless/nextLevelData.blessing*1000)/10 .. "%")
    end
end

--@brief 点击放大镜按钮显示属性
function WndFootMarkUpgrade:onShowAttribute(element)
...
function WndFootMarkUpgrade:_adaptLanguage_pt(  )
    ...
    txtName:setDimensions(GlobalMethod:CCSize(400))
    local txtLv = GetElement(self.m_root,"txtMountLv_WndFootMarkUpgrade",WZUILabelTTF)
    txtLv:setScale(0.8)
    txtLv:setDimensions(GlobalMethod:CCSize(400))
end
...
function WndFootMarkUpgrade:_adaptLanguage_vn()
    ...
    local txtLv2 = GetElement(self.m_root,"txtLv2_WndFootMarkUpgrade",WZUILabelTTF)
    txtLv2:setRelativePosition(GlobalMethod:ccp(0.72,0.5))
    ...
    -- txtPro4:setRelativePosition(GlobalMethod:ccp(-0.0248139,0.288585))
    local txtPro5 = GetElement(self.m_root,"txtPro5_WndFootMarkUpgrade",WZUILabelTTF)
    txtPro5:setRelativePosition(GlobalMethod:ccp(0.2,0.144))

    -- local txtNextPro1 = GetElement(self.m_root,"txtNextPro1_WndFootMarkUpgrade",WZUILabelTTF)
    -- txtNextPro1:setRelativePosition(GlobalMethod:ccp(0.2,0.711))
    -- local txtNextPro2 = GetElement(self.m_root,"txtNextPro2_WndFootMarkUpgrade",WZUILabelTTF)
    ...
    local txtNextPro5 = GetElement(self.m_root,"txtNextPro5_WndFootMarkUpgrade",WZUILabelTTF)
    txtNextPro5:setRelativePosition(GlobalMethod:ccp(0.2,0.144))

    GetElement(self.m_root,"txtStar1_WndFootMarkUpgrade",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.277032,0.5))
    GetElement(self.m_root,"imgStar11_WndFootMarkUpgrade",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.412412,0.5))
    GetElement(self.m_root,"txtStar2_WndFootMarkUpgrade",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.825,0.5))
    GetElement(self.m_root,"imgStar12_WndFootMarkUpgrade",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.919381,0.5))
...
function WndFootMarkUpgrade:_adaptLanguage_ug(  )
    for i = 1, 5 do
        local txtProT = GetElement(self.m_root,"txtProT"..i.."_WndFootMarkUpgrade",WZUILabelTTF)
        txtProT:setAnchorPoint(GlobalMethod:ccp(1,0.5))
        txtProT:setRelativePosition(GlobalMethod:ccp(2,1.044-i*0.19)) 
        local txtPro = GetElement(self.m_root,"txtPro"..i.."_WndFootMarkUpgrade",WZUILabelTTF)
        txtPro:setAnchorPoint(GlobalMethod:ccp(1,0.5))
        txtPro:setRelativePosition(GlobalMethod:ccp(-0.16,1.044-i*0.19))
        local txtNextProT = GetElement(self.m_root,"txtNextProT"..i.."_WndFootMarkUpgrade",WZUILabelTTF)
        txtNextProT:setAnchorPoint(GlobalMethod:ccp(1,0.5))
        txtNextProT:setRelativePosition(GlobalMethod:ccp(1.86,1.044-i*0.19))
        local txtNextPro = GetElement(self.m_root,"txtNextPro"..i.."_WndFootMarkUpgrade",WZUILabelTTF)
        txtNextPro:setAnchorPoint(GlobalMethod:ccp(1,0.5))
        txtNextPro:setRelativePosition(GlobalMethod:ccp(-0.023,1.044-i*0.19))
    end
    local txtLvT1 = GetElement(self.m_root,"txtLvT1_WndFootMarkUpgrade",WZUILabelTTF)
    txtLvT1:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    txtLvT1:setRelativePosition(GlobalMethod:ccp(0.48,0.5))
    local txtLv1 = GetElement(self.m_root,"txtLv1_WndFootMarkUpgrade",WZUILabelTTF)
    txtLv1:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    txtLv1:setRelativePosition(GlobalMethod:ccp(0.22,0.5))
    local txtLvT2 = GetElement(self.m_root,"txtLvT2_WndFootMarkUpgrade",WZUILabelTTF)
    ...
    txtLv2:setRelativePosition(GlobalMethod:ccp(0.7,0.5))
    local txtStarT1 = GetElement(self.m_root,"txtStarT1_WndFootMarkUpgrade",WZUILabelTTF)
    txtStarT1:setScale(0.7)
    txtStarT1:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    txtStarT1:setRelativePosition(GlobalMethod:ccp(0.48,0.5))
    local txtStar1 = GetElement(self.m_root,"txtStar1_WndFootMarkUpgrade",WZUILabelTTF)
    txtStar1:setScale(0.7)
    txtStar1:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    txtStar1:setRelativePosition(GlobalMethod:ccp(0.185,0.5))
    local imgStar11 = GetElement(self.m_root,"imgStar11_WndFootMarkUpgrade",WZUIImage)
    imgStar11:setScale(0.7)
    imgStar11:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    imgStar11:setRelativePosition(GlobalMethod:ccp(0.14,0.5))
    local txtStarT2 = GetElement(self.m_root,"txtStarT2_WndFootMarkUpgrade",WZUILabelTTF)
    ...
    local txtStar2 = GetElement(self.m_root,"txtStar2_WndFootMarkUpgrade",WZUILabelTTF)
    txtStar2:setScale(0.7)
    txtStar2:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    txtStar2:setRelativePosition(GlobalMethod:ccp(0.665,0.5))
    local imgStar12 = GetElement(self.m_root,"imgStar12_WndFootMarkUpgrade",WZUIImage)
    imgStar12:setScale(0.7)
    imgStar12:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    imgStar12:setRelativePosition(GlobalMethod:ccp(0.62,0.5))
    
    GetElement(self.m_root,"txtAthShopCost1_WndFootMarkUpgrade",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.73,0.72))
    GetElement(self.m_root,"imgCostIcon_WndFootMarkUpgrade",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.52,0.72))
    GetElement(self.m_root,"txtUpCost_WndFootMarkUpgrade",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.46,0.72))
    GetElement(self.m_root,"txtUpCost_WndFootMarkUpgrade",WZUILabelTTF):setAnchorPoint(GlobalMethod:ccp(1,0.5))
    GetElement(self.m_root,"txtUpCnt_WndFootMarkUpgrade",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(-0.015,0.72))
    GetElement(self.m_root,"txtAthShopCost_WndFootMarkUpgrade",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.73,0.72))
    GetElement(self.m_root,"imgStarUpCost_WndFootMarkUpgrade",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.52,0.72))
    GetElement(self.m_root,"txtStarCost_WndFootMarkUpgrade",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.46,0.72))
    GetElement(self.m_root,"txtStarCost_WndFootMarkUpgrade",WZUILabelTTF):setAnchorPoint(GlobalMethod:ccp(1,0.5))
    GetElement(self.m_root,"txtPillCnt_WndFootMarkUpgrade",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(-0.015,0.72))

    local txtLvSuccessT = GetElement(self.m_root,"txtUpSuccess_WndFootMarkUpgrade",WZUILabelTTF)
    txtLvSuccessT:setScale(0.9)
    txtLvSuccessT:setRelativePosition(GlobalMethod:ccp(0.58,0.28))
    local txtLvSuccess = GetElement(self.m_root,"txtLvSuccess_WndFootMarkUpgrade",WZUILabelTTF)
    ...
    local txtStarSuccessT = GetElement(self.m_root,"txtStarSuccessT_WndFootMarkUpgrade",WZUILabelTTF)
    txtStarSuccessT:setScale(0.9)
    txtStarSuccessT:setRelativePosition(GlobalMethod:ccp(0.58,0.28))
    local txtStarSuccess = GetElement(self.m_root,"txtStarSuccess_WndFootMarkUpgrade",WZUILabelTTF)
    ...
    local txtStrengtenTip1 = GetElement(self.m_root,"txtStrengtenTip1_WndFootMarkUpgrade",WZUILabelTTF)
    txtStrengtenTip1:setScale(0.6)
    txtStrengtenTip1:setDimensions(GlobalMethod:CCSize(600))
    local txtStrengtenTip = GetElement(self.m_root,"txtStrengtenTip_WndFootMarkUpgrade",WZUILabelTTF)
    txtStrengtenTip:setScale(0.6)
    txtStrengtenTip:setDimensions(GlobalMethod:CCSize(600))

    GetElement(self.m_root,"txtUpgradeLog_WndFootMarkUpgrade",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.5,0.878504))
    for i=1,6 do
       local tfbLog = GetElement(self.m_root,"tfbLog"..i.."_WndFootMarkUpgrade",WZUIFreeTextBox)
