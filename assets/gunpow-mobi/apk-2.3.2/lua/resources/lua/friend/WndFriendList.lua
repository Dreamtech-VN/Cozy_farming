...
function WndFriendList:onFriendClick(tCell,tag,tData)
    ...
    local function onClickFriendCallback()
        ...
        if self.m_nInterface == 4 or self.m_nInterface == 7 then
            self:onBackClick()
        elseif self.m_nInterface == 3 or self.m_nInterface == 6 or self.m_nInterface==31 or self.m_nInterface == 8 or self.m_nInterface == 9 or self.m_nInterface == 11 or self.m_nInterface == 12 or self.m_nInterface == 13 or self.m_nInterface == 14 or self.m_nInterface == 15 or self.m_nInterface == 16 or self.m_nInterface == 18 or self.m_nInterface == 21 then
    ...
    if self.m_nInterface == 18 then --岛主挑战
        local landlordConfig = json.decode(CacheCenter:getGameParam().landlordConfig)
        if self:getListData()[tag+1].assistSize >= landlordConfig.assistNum then
            MsgBoxManager:showConfirmBox(LocalStrings.ISLAND_OWNER_TEXT27, nil, onClickFriendCallback)
        else
            onClickFriendCallback()
        end
    else
        onClickFriendCallback()
    end
...
function WndFriendList:onFriend(element)
	...
	conImgTheme_WndFriendList:setRelativePosition(GlobalMethod:ccp(1.08,pt.y))
    self:createLoading()
...
function WndFriendList:onHall(element)
    ...
    if self.m_nInterface ~= 3 and self.m_nInterface ~= 6 and self.m_nInterface ~= 8 and self.m_nInterface ~= 11 and self.m_nInterface ~= 12 and self.m_nInterface ~= 14 and self.m_nInterface ~= 16 and self.m_nInterface ~= 18 then
        return
    end
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN2)
...
--师门
function WndFriendList:onMaster(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN2)
    ...
    local pt = element:getRelativePosition()
    GetElement(self.m_root,"conImgTheme_WndFriendList",WZUIContainer):setRelativePosition(GlobalMethod:ccp(1.08,pt.y))
...
function WndFriendList:_update()
    ...
    if t == nil or #t == 0 then
        ...
        elseif self.m_nSelect == 3 then
            desc = LocalStrings.NO_PLAYER_IN_HALL
        elseif self.m_nSelect == 6 then
...
function WndFriendList:_showMultiLanguage()
    ...
    if GlobalMethod:crossServiceOpen() == 0 then
        GetElement(self.m_root,"btnTheme5_WndFriendList",WZUIButton):setRelativePosition(GlobalMethod:ccp(1.08,0.41))
        GetElement(self.m_root,"conCheck5_WndFriendList",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.5,0.116))
    end
end
    ...
    for i = 1, 6 do
        ...
        if txtCheck then 
            if i == self.m_nSelect then
                ...
                txtCheck:setColor(GlobalMethod:ccc3(127,70,26))
                txtCheck:setEnableStroke(false)
            end
        end
    end
...
function WndFriendList:_adaptLanguage_ug()
    for i = 1, 5 do 
        local txtCheckName = GetElement(self.m_root,"txtCheck"..i.."_WndFriendList",WZUILabelTTF)
            if txtCheckName then
            txtCheckName:setScale(0.56)
            txtCheckName:setDimensions(GlobalMethod:CCSize(150))
        end
    end

    local txtAssetInTip = GetElement(self.m_root,"txtAssetInTip_WndFriendsList",WZUILabelTTF)
    txtAssetInTip:setScale(0.7)
    txtAssetInTip:setDimensions(GlobalMethod:CCSize(1000))
end
-------------------------------------语言适配End--------------------------------------------
