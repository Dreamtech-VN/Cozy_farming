...
function WndFriendInviteCode:_showInviteFriendInfo()
    ...
    if self.m_sMyInviteCode then
        ...
        local sCodeFormat = [[<T C="127,70,26" S="22" P="1">%s</T><T C="229,105,22" S="22" P="1">%s</T>]]
    ...
    local txtLevel = GetElement(self.m_root, "txtLevel_WndFriendInviteCode", WZUIFreeTextBox)
    local levelContent = string.format(sFormat, LocalStrings.MOUNT_LEVEL1, self.m_tFriend.level)
    txtLevel:setShowText(levelContent)
    --服务器
    if self.m_tFriend.serverName then
        ...
        sFormat = [[<T C="127,70,26" S="22" P="1">%s</T><T C="229,105,22" S="22" P="1">%s</T>]]
