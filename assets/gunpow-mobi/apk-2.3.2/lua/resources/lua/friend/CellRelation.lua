...
function CellRelation:onClickIcon(element)
    ...
    if self.m_nType == 1 then
        ...
        local tTempInfo = self:getRelationName(self.m_nType, nValue, self.m_tData.sex, self.m_tPlayerInfo.sex)
...
function CellRelation:getRelationName(nType, nValue, dataSex, playerSex)
    -- body
    local tTempList = {}
    for i, value in pairs(GDatatab_relationship) do
        if value.type == nType then
            ...
            if nType == 2 then
                ...
                if dataSex ~= playerSex then
                    tItem.title = tTitle[2]
                elseif dataSex == playerSex and dataSex == 0 then
                    tItem.title = tTitle[1]
                elseif dataSex == playerSex and dataSex == 1 then
