...
--@brief	显示头像
function CellDynamic:_showPhone()
	--设置默认显示
	local conHead = WZUIContainer:luaTo(self.m_root:getChildElement("conHead_CellDynamic"))
	WZLog("好友数据",Serialize(self.m_tFriend))
...
function CellDynamic:_showGiftButton(status, sendType, typeList)
	...
	local txtOnlineState = GetElement(self.m_root, "txtOnlineState_CellDynamic", WZUILabelTTF)
	for i = 1,2 do
		GetElement(self.m_root,"imgVigor" ..i.. "_CellDynamic",WZUIImage):setFile("ui/common/common_icon_hyzshltb.png")
		GetElement(self.m_root,"imgRecv" ..i.. "_CellDynamic",WZUIImage):setFile("ui/common/common_icon_hylqhltb.png")
	end
	local isBestFriend = CacheCenter:judgeWhetherBestFriend(self.m_tFriend.id)
	if isBestFriend then
		for i = 1,2 do
			WZLog("设置按钮图片")
			GetElement(self.m_root,"imgVigor" ..i.. "_CellDynamic",WZUIImage):setFile("ui/common/common_icon_hymy.png")
			GetElement(self.m_root,"imgRecv" ..i.. "_CellDynamic",WZUIImage):setFile("ui/common/common_icon_hymy_1.png")
		end	
	end
    WZLog("status====",Serialize(self.m_tFriend), status, sendType, typeList)
...
function CellDynamic:_showTTF(desc,type)
	...
	elseif type == 4 then
		if self.m_tFriend.level then 
        	sName = string.format(LocalStrings.TO_YOU, LocalStrings.LV .. tostring(self.m_tFriend.level) .. " " .. self.m_tFriend.name)
		else
        	sName = string.format(LocalStrings.TO_YOU, self.m_tFriend.name)
        end
        if self.m_tFriend.serverId ~= nil and self.m_tFriend.serverId ~= CacheCenter:getPlayerInfo().serverId then
	...
	elseif type == 5 then
		...
		if self.m_tFriend.serverId ~= nil and self.m_tFriend.serverId ~= CacheCenter:getPlayerInfo().serverId then
            sName = string.format(LocalStrings.YOU_TO_KUAFU, self.m_tFriend.name)
        end
		sContent = string.format(LocalStrings.GIFT_ADD_FRIENDLINESS, self.m_tFriend.friendliness)
	elseif type == 6 then
		sName = string.format(LocalStrings.YOU_TO, self.m_tFriend.name)
...
-------------------------------------语言适配Begin----------------------------------------
function CellDynamic:_adaptLanguage_ug(  )
	GetElement(self.m_root, "txtAddDes_CellDynamic", WZUIFreeTextBox):setScale(0.5)
end

function CellDynamic:_adaptLanguage_vn(  )
	local txtDesName = GetElement(self.m_root, "txtToName_CellDynamic", WZUIFreeTextBox)
	txtDesName:setScale(0.7)
	local txtAddDes = GetElement(self.m_root, "txtAddDes_CellDynamic", WZUIFreeTextBox)
	txtAddDes:setMaxWidth(280)
	txtAddDes:setScale(0.7)
end
