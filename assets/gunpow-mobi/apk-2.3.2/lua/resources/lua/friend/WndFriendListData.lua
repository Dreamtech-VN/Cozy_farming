...
function WndFriendList:setFriendData(tFriend)
	...
	if self.m_nSelect == 1 then 
        ...
        if self.m_nInterface == 3 or self.m_nInterface == 6 or self.m_nInterface == 7 or self.m_nInterface == 11 or self.m_nInterface == 12 or self.m_nInterface == 14 or self.m_nInterface == 15 or self.m_nInterface == 16 or self.m_nInterface == 18 or self.m_nInterface == 21 then
...
--1、发请柬,2、婚礼邀请,3、战斗邀请,4、邮件邀请,5、异性单身 ,31、公会战好友(Add By Shengqiang), 8、英雄联赛邀请, 9.公会战房间邀请（Add by Tianxiang）,11、排位赛，12、娱乐赛，13、公会邀请, 14.组队世界boss邀请, 15.邀请双修 16.双人爬塔邀请 18.岛主挑战 20.战略赛 21联盟邀请
--tCell回调的类self a
...
function WndFriendList:onInitInterface(index)
    ...
    local checkTheme6 = GetElement(self.m_root,"btnTheme6_WndFriendList",WZUIButton) --师门
    ...
    local conCheck6 = GetElement(self.m_root,"conCheck6_WndFriendList",WZUIContainer) --师门
    ...
    if index == 4 or index == 7 then
        ...
        conTheme3:setRelativePosition(GlobalMethod:ccp(0.5,0.595))
        conTheme1:setVisible(false)

        checkTheme2:setAnchorPoint(GlobalMethod:ccp(0.5,0.5))
        checkTheme2:setRelativePosition(GlobalMethod:ccp(1.08,0.815))
        conImgTheme_WndFriendList:setRelativePosition(GlobalMethod:ccp(1.08,0.815))
        checkTheme3:setAnchorPoint(GlobalMethod:ccp(0.5,0.5))
        checkTheme3:setRelativePosition(GlobalMethod:ccp(1.08,0.68))
    ...
    elseif index == 8 then
        ...
        checkTheme1:setRelativePosition(GlobalMethod:ccp(1.08,0.815))
    
        checkTheme2:setAnchorPoint(GlobalMethod:ccp(0.5,0.5))
        checkTheme2:setRelativePosition(GlobalMethod:ccp(1.08,0.68))
        checkTheme2:setVisible(true)

        checkTheme3:setAnchorPoint(GlobalMethod:ccp(0.5,0.5))
        checkTheme3:setRelativePosition(GlobalMethod:ccp(1.08,0.545))
        
        conImgTheme_WndFriendList:setRelativePosition(GlobalMethod:ccp(1.08,0.68))
        
        conTheme1:setAnchorPoint(GlobalMethod:ccp(0.5,0.5))
        conTheme1:setRelativePosition(GlobalMethod:ccp(0.5,0.835))
        
        conTheme2:setAnchorPoint(GlobalMethod:ccp(0.5,0.5))
        conTheme2:setRelativePosition(GlobalMethod:ccp(0.5,0.595))
        conTheme2:setVisible(true)
        
        conTheme3:setAnchorPoint(GlobalMethod:ccp(0.5,0.5))
        conTheme3:setRelativePosition(GlobalMethod:ccp(0.5,0.355))
        self:_setSignWordStrokeColor()
    elseif index == 13 or index == 21 then
        txtWndTitleName_WndFriendList:setText(LocalStrings.INVITE)
        checkTheme1:setVisible(false)
        checkTheme3:setVisible(false)
        conTheme1:setVisible(false)
    ...
    elseif index == 15 then 
        ...
        checkTheme1:setVisible(false)
        conTheme1:setVisible(false)
    elseif index == 16 then --师门
        self.m_nSelect=3
        checkTheme6:setVisible(true)
        conCheck6:setVisible(true)
        checkTheme6:setRelativePosition(GlobalMethod:ccp(1.08,0.41))
        conCheck6:setRelativePosition(GlobalMethod:ccp(0.5,0.116))
        txtWndTitleName_WndFriendList:setText(LocalStrings.INVITE)
    else
        txtWndTitleName_WndFriendList:setText(LocalStrings.INVITE)
        self.m_nSelect=3
    ...
    elseif index == 16 then  --邀请助战双人爬塔
        ProtocolProcessorWndFriends:send_FRIEND_GetFriend(index,3)
    elseif index == 18 then  --邀请岛主挑战
        ProtocolProcessorWndFriends:send_FRIEND_GetFriend(index,3)
    elseif index == 21 then  --邀请加入联盟
        ProtocolProcessorWndFriends:send_FRIEND_GetFriend(index,1)
    end
end
