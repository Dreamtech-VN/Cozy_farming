...
function CellFriendInvite:_showLevel()
    WZLog("显示等级")
    local txtLevel= WZUIFreeTextBox:luaTo(self.m_root:getChildElement("txtLevel_CellFriendInvite"))
    local sFormat = [[<T C="127,70,26" S="22" P="1">%s </T><T C="229,105,22" S="22" P="1">%d</T>]]
    local txtContent = string.format(sFormat, LocalStrings.MOUNT_LEVEL1, self.m_tFriend.level)
    txtLevel:setShowText(txtContent)
end
...
function CellFriendInvite:_showServer()
    ...
    local sFormat = [[<T C="127,70,26" S="22" P="1">%s </T><T C="229,105,22" S="22" P="1">%s</T>]]
...
function CellFriendInvite:_adaptLanguage_hk(  )
    ...
    local txtLevel = GetElement(self.m_root, "txtLevel_CellFriendInvite", WZUIFreeTextBox)
