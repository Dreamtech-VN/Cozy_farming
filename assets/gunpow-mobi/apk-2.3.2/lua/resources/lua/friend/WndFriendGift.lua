...
--@note		在这里做场景进入前的准备工作
function WndFriendGift:onEnter(element)
	self.m_root = element
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function WndFriendGift:onExit(element)
	self:_unInit()
	CacheCenter:unregisterUpatePlayerItemObserver(self)
end

--@brief    加载动画完后调用
function WndFriendGift:onEnterTransitionDidFinish(element)
    --body
    self:setData()
end

--@brief    触摸开始函数
function WndFriendGift:onTouchBegin(element, pt)
    WZLog("WndFriendGift:onTouchBegin",pt.x,pt.y)
...
--@brief    关闭界面
function WndFriendGift:onCloseClick(element)
    -- body
...
--@brief    关闭按钮回调事件
function WndFriendGift:onCloseActionCallback(element,data)
    WindowManager:removeWindow(self.m_root, self, true)
end

--@brief    其它Item点击回调
function WndFriendGift:onOthersClick(luaTable,tag,tData)
    if tData == nil then
       return
    end
...
--@brief    点击赠送按钮回调
function WndFriendGift:onGiveGiftClick(element)
    -- body
    ...
    WZLog("WndFriendGift:onGiveGiftClick", Serialize(tData))
    if nLastNum <= 0 then
...
--@brief    收到缓存推送后更新装备信息
function WndFriendGift:updatePlayerItemData()
    if WndFriendGift.m_root == nil then return end
    WZLog("WndFriendGift:updatePlayerItemData")
    local tableGiftList = GetElement(self.m_root, "tableconGiftList_WndFriendGift", WZUITableContainer)
    if self.m_nClickItemTag ~= nil then
        ...
        local conForGift = GetElement(element, "conForGift_WndFriendGift", WZUIContainer) 
...
function WndFriendGift:_update()
    ...
    for i = 1, #tGiftList do
        ...
        if ProjConfig.LANGUAGE == "ug" then
            GetElement(element, "txtGiveGift_conCellFriendGift", WZUIButton):setScale(0.7)
        end
    end
end

--@brief    更新礼物子项的显示信息
function WndFriendGift:_updateCellInfo(element, tData)
    -- body
    --增加友好度提示
    local txtFriendliness = GetElement(element, "txtFriendliness_WndFriendGift", WZUIFreeTextBox)
    local sDesText = string.format(LocalStrings.ADD_FRIENDLINESS, tData.value)
    if ProjConfig.LANGUAGE == "vn" then 
        sDesText = string.gsub(sDesText, "24", "14")
        txtFriendliness:setMaxWidth(120)
    end
...
--@brief    礼物不足,跳到购买界面
function WndFriendGift:_buyGift(element, btnTag)
    -- body
    local tData = self.m_tGiftList[self.m_nClickItemTag + 1]
    WZLog("WndFriendGift:_buyGift 000", btnTag, MSGBOXTYPE_CONFIRM)
    if btnTag == MSGBOXTYPE_CONFIRM then
        WZLog("WndFriendGift:_buyGift", tData.id)
...
--@brief    显示好友度
function WndFriendGift:_updateFriendliness()
    -- body
    if self.m_nFriendliness then
        local txtFriendLiness = GetElement(self.m_root, "txtFriendLiness_WndFriendGift", WZUILabelTTF)
