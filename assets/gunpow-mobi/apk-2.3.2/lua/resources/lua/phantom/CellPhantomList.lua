...
--@note		在这里做场景进入前的准备工作
function CellPhantomList:onEnter(element)
	self.m_root = element
	AdaptLanguage(self)
end
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function CellPhantomList:onExit(element)
	self:_unInit()
end

function CellPhantomList:onEnterTransitionDidFinish(element)

end

function CellPhantomList:setData(tData)
	WZLog("CellPhantomList:setData")
	self.m_tData = tData
	self:update1()
end
-------------------------------------公有方法模块End----------------------------------------
...
-------------------------------------私有方法模块Begin--------------------------------------
function CellPhantomList:update1()
	WZLog("CellPhantomList:update1")
	...
	for i=1,#tFetter.attribute do
		...
		GetElement(self.m_root,"txtAttrWord"..i .. "_CellPhantomList",WZUILabelTTF):setText(ATTR_TITLE[tt[1]])
		GetElement(self.m_root,"txtAttr"..i,WZUILabelTTF):setText("+"..tt[2])
	end

	local attrNum = #tFetter.attribute
	if attrNum == 1 then
		GetElement(self.m_root,"txtName",WZUILabelTTF):setRelativePosition(ccp(0.12,0.67))
		GetElement(self.m_root,"txtStatus",WZUILabelTTF):setRelativePosition(ccp(0.115,0.3))
	elseif attrNum == 2 then
		GetElement(self.m_root,"txtName",WZUILabelTTF):setRelativePosition(ccp(0.12,0.67))
		GetElement(self.m_root,"txtStatus",WZUILabelTTF):setRelativePosition(ccp(0.115,0.17))
	elseif attrNum == 3 then
		GetElement(self.m_root,"txtName",WZUILabelTTF):setRelativePosition(ccp(0.12,0.8))
		GetElement(self.m_root,"txtStatus",WZUILabelTTF):setRelativePosition(ccp(0.115,0.17))
	end
		...
		for k,v in pairs(GDatatab_shape_skins) do
			if v.channel == self.m_tData.content[i] then
				...
				if v.own then
					...
					local tempValue = WndPhantom:whetherOwnPhantom(v)
					if tempValue and tempValue.own then 
						local cellElement,tCell
						cellElement,tCell = CellPhantom:createElement()
						tCell:setData(tempValue)
						cellElement:setScale(1)
						con:removeAllChildrenWithCleanup(true)
						con:addChild(cellElement)
						if tempValue.remainTime ~= -1 then
							active = false
						end
						find = true
						break
	...
	WZLog("CellPhantomList:update2", tFetter.name, active)

	if active then
		GetElement(self.m_root,"txtStatus",WZUILabelTTF):setText("("..LocalStrings.STAR_SOUL_HAVED_ACTIVE..")")
		GetElement(self.m_root,"txtStatus",WZUILabelTTF):setColor(ccc3(5,180,0))
	else
		GetElement(self.m_root,"txtStatus",WZUILabelTTF):setText("("..LocalStrings.STAR_SOUL_NOT_ACTIVE..")")
		GetElement(self.m_root,"txtStatus",WZUILabelTTF):setColor(ccc3(138,122,106))
	end
end
...
function CellPhantomList:_adaptLanguage_vn(  )
...
function CellPhantomList:_adaptLanguage_en(  )
...
function CellPhantomList:_adaptLanguage_th(  )
	GetElement(self.m_root,"txtName",WZUILabelTTF):setScale(0.5)
	GetElement(self.m_root,"txtName",WZUILabelTTF):setDimensions(GlobalMethod:CCSize(160))
	GetElement(self.m_root,"txtAttr1",WZUILabelTTF):setScale(0.5)
	GetElement(self.m_root,"txtAttr2",WZUILabelTTF):setScale(0.5)
...
function CellPhantomList:_adaptLanguage_pt(  )
...
function CellPhantomList:_adaptLanguage_es(  )
	local txtName = GetElement(self.m_root,"txtName",WZUILabelTTF)
	txtName:setScale(0.55)
	txtName:setDimensions(GlobalMethod:CCSize(150))
	GetElement(self.m_root,"txtAttr1",WZUILabelTTF):setScale(0.55)
...
function CellPhantomList:_adaptLanguage_ug(  )
	GetElement(self.m_root,"txtName",WZUILabelTTF):setScale(0.5)
	GetElement(self.m_root,"txtName",WZUILabelTTF):setDimensions(GlobalMethod:CCSize(160))
	GetElement(self.m_root,"txtAttr1",WZUILabelTTF):setScale(0.5)
	GetElement(self.m_root,"txtAttr2",WZUILabelTTF):setScale(0.5)
