--CellPhantomItemData.lua
--@brief	CellPhantomItem的数据模块
--@date		2020/07/22
--@author	XTX
--@note		皮肤列表项
...
function CellPhantomItem:_init()
...
function CellPhantomItem:_unInit()
	self.m_root = nil
...
--@note		请仅用此方法创建场景
function CellPhantomItem:createElement()
	local tNewObj = self:_new()
	assert(tNewObj, "CellPhantomItem table create failed!")
	tNewObj:_init()

	local element = WZUIContainer:create()
	element:setName("__CellPhantomItem")
...
--@brief 	设置数据
function CellPhantomItem:setData(tData)
	-- body
	self.m_tData = tData 
...
--@brief 	刷新数据
function CellPhantomItem:resetData(tData)
	-- body
	self.m_tData = tData
