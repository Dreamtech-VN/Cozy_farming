--CellPhantomList3.lua
--@brief	CellPhantomList3的UI模块
--@date		2017/12/22
--@author	zsq
--@note		皮肤列表
...
--@note		在这里做场景进入前的准备工作
function CellPhantomList3:onEnter(element)
	self.m_root = element
	AdaptLanguage(self)
end
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function CellPhantomList3:onExit(element)
	self:_unInit()
end

function CellPhantomList3:onEnterTransitionDidFinish(element)

end

function CellPhantomList3:setData(tData)
	WZLog("CellPhantomList3:setData")
	self.m_tData = tData
	self:update1()
end
-------------------------------------公有方法模块End----------------------------------------
...
-------------------------------------私有方法模块Begin--------------------------------------
function CellPhantomList3:update1()
	...
	for i=1,#tFetter.attribute do
		...
		GetElement(self.m_root,"txtAttrWord"..i .. "_CellPhantomList3",WZUILabelTTF):setText(ATTR_TITLE[tt[1]])
...
function CellPhantomList3:_adaptLanguage_vn(  )
...
function CellPhantomList3:_adaptLanguage_en(  )
...
function CellPhantomList3:_adaptLanguage_th(  )
	GetElement(self.m_root,"txtName",WZUILabelTTF):setScale(0.5)
	GetElement(self.m_root,"txtName",WZUILabelTTF):setDimensions(GlobalMethod:CCSize(160))
	GetElement(self.m_root,"txtAttr1",WZUILabelTTF):setScale(0.5)
	GetElement(self.m_root,"txtAttr2",WZUILabelTTF):setScale(0.5)
...
function CellPhantomList3:_adaptLanguage_pt(  )
...
function CellPhantomList3:_adaptLanguage_es(  )
	local txtName = GetElement(self.m_root,"txtName",WZUILabelTTF)
	txtName:setScale(0.55)
	txtName:setDimensions(GlobalMethod:CCSize(150))
	GetElement(self.m_root,"txtAttr1",WZUILabelTTF):setScale(0.55)
...
function CellPhantomList3:_adaptLanguage_ug(  )
	GetElement(self.m_root,"txtName",WZUILabelTTF):setScale(0.5)
	GetElement(self.m_root,"txtName",WZUILabelTTF):setDimensions(GlobalMethod:CCSize(160))
	GetElement(self.m_root,"txtAttr1",WZUILabelTTF):setScale(0.5)
	GetElement(self.m_root,"txtAttr2",WZUILabelTTF):setScale(0.5)
