...
function WndExtraction:_createCellGoodItem(nTag)
    ...
    tItem:setBackImgFile("ui/common/common_scale9_beibaodi.png", true)
    return eItem, tItem
end
...
function WndExtraction:onQuick(element)
    ...
    if CacheCenter:getPlayerInfo().level < tBtnsInfo.open_level and CacheCenter:getPlayerInfo().vipLevel < 3 and not whetherHaveWelfareCard() then
...
function WndExtraction:_moreLanguage()
    ...
    if CheckButtonOpen(181, false) then 
        GetElement(self.m_root,"txtVIPRapid_WndExtraction",WZUILabelTTF):setText("")
    else 
        GetElement(self.m_root,"txtVIPRapid_WndExtraction",WZUILabelTTF):setText(LocalStrings.WELFARECARD_VIP_TEXT1)
    end
...
function WndExtraction:_adaptLanguage_ug(  )
    local txtTab1 = GetElement(self.m_root,"txtTab1_WndExtraction",WZUILabelTTF)
    txtTab1:setScale(0.6)
    txtTab1:setDimensions(GlobalMethod:CCSize(140))
    local txtTab1Sel = GetElement(self.m_root,"txtTab1Sel_WndExtraction",WZUILabelTTF)
    ...
    local txtCostWord = GetElement(self.m_root, "txtCostWord_WndExtraction", WZUILabelTTF)
    txtCostWord:setScale(0.8)
    txtCostWord:setDimensions(GlobalMethod:CCSize(160,0))
    txtCostWord:setRelativePosition(GlobalMethod:ccp(1,0.5))
    GetElement(self.m_root, "conCost1_WndExtraction", WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.5,0.5))
    GetElement(self.m_root, "conCost2_WndExtraction", WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.15,0.5))

    local txtBtn1 = GetElement(self.m_root,"txtBtn1_WndExtraction",WZUILabelTTF)
    txtBtn1:setScale(0.7)
    txtBtn1:setDimensions(GlobalMethod:CCSize(150))
    local txtBtn2 = GetElement(self.m_root,"txtBtn2_WndExtraction",WZUILabelTTF)
    txtBtn2:setScale(0.7)
    txtBtn2:setDimensions(GlobalMethod:CCSize(150))

    local txtPet1 = GetElement(self.m_root,"txtPet1_WndExtraction",WZUILabelTTF)
    txtPet1:setScale(0.6)
    txtPet1:setDimensions(GlobalMethod:CCSize(120))
    local txtPet2 = GetElement(self.m_root,"txtPet2_WndExtraction",WZUILabelTTF)
    txtPet2:setScale(0.6)
    txtPet2:setDimensions(GlobalMethod:CCSize(120))
    local txtPet1 = GetElement(self.m_root,"txtBless1_WndExtraction",WZUILabelTTF)
    txtPet1:setScale(0.6)
    txtPet1:setDimensions(GlobalMethod:CCSize(120))
    local txtPet2 = GetElement(self.m_root,"txtBless2_WndExtraction",WZUILabelTTF)
    txtPet2:setScale(0.6)
    txtPet2:setDimensions(GlobalMethod:CCSize(120))

    local cost = GetElement(self.m_root,"cost_WndExtraction",WZUILabelTTF)
    cost:setDimensions(GlobalMethod:CCSize(120))
    cost:setRelativePosition(GlobalMethod:ccp(0.84,0.5))
    GetElement(self.m_root,"imgCost_WndExtraction",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.44,0.5))
    GetElement(self.m_root,"txtValue_WndExtraction",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.5,0.5))

    GetElement(self.m_root,"txtVIPRapid_WndExtraction",WZUILabelTTF):setScale(0.7)

    GetElement(self.m_root,"conRapid_WndExtraction",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.35,0.3))
end
-------------------------------------语言适配End--------------------------------------------