...
function CellWakeupDetail:_showInbornValue()
    ...
    if ftxtInbornValue then
        ...
        local sContentFormat = [[<T C="93,222,254" S="20" P="1" SC="79,60,48" SS="4" SE="1">%s:</T><T C="255,236,193" S="20" P="1">%d</T><T C="255,236,193" S="20" P="1">(%s)</T>]]
...
function CellWakeupDetail:_setDynamicPrg(nTempExp, nLevel)
    ...
    if ftxtSoulExp then
        local sContentFormat = [[<T C="99,255,95" S="20" P="1" SC="79,60,48" SS="4" SE="1">Exp:</T><T C="255,236,193" S="20" P="1">%d/%d</T>]]
...
function CellWakeupDetail:_showEvolveInfo()
    ...
    for i, value in pairs(tEvolveList) do
        ...
        if element then 
            ...
            local imgEvolveState = GetElement(element, "imgEvolveState_CellEvolveItem", WZUIImage)
            ...
            if nEvolveLevel >= value.id then 
                ...
                imgEvolveState:setVisible(true)
...
--@brief    设置开箱特效
function CellWakeupDetail:setSpineAni()
    local spineReceive = GetElement(self.m_root, "spineReceive_CellWakeupDetail", WZUISpine)
    local spinePath = "ui/otherUI/ui_juexingzhihun_lingqu"
    local bIsExist = CheckEffectFile(spinePath)

    if bIsExist then 
        spineReceive:setFileJson(spinePath .. ".json")
        spineReceive:setFileAtlas(spinePath .. ".atlas")
    end
end
-------------------------------------私有方法模块End----------------------------------------
