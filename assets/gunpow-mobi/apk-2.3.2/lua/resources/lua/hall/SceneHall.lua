...
function SceneHall:_addTop()
    ...
    tcell:setTopData("ui/common/common_icon_jjdt.png",SceneHall,SceneHall.onReturn,false,true,true,"SceneHall")
    tcell:setTopType()
...
function SceneHall:register()
    GlobalGame:getGameEventDispathcer():Add(bottomMeneEvent.WndBottomMeneEvent_HonorPointCountDown,self._onSceneHallInfoData,self)
end
function SceneHall:unregister()
    GlobalGame:getGameEventDispathcer():Remove(bottomMeneEvent.WndBottomMeneEvent_HonorPointCountDown,self._onSceneHallInfoData,self)
end
--@brief	退出场景时被调用的函数
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
...
function SceneHall:_onSceneHallInfoData(honourPoint, restoreTime, serverTime)
    local _, score = GlobalMethod:HonorPointStatus(2)
    if tonumber(honourPoint) >= score then
        if self.m_nTouchButtonType then
            if self.m_nTouchButtonType == 1 then --查找
            ...
            elseif self.m_nTouchButtonType == 2 then--快速游戏
                self:_onQuickGame()
            elseif self.m_nTouchButtonType == 3 then --快速加入
                ProtocolProcessorSceneHall:send_ROOM_SelectRoom(0,self.matchType,0,"-1")  --房间id为0表示快速加入
            elseif self.m_nTouchButtonType == 4 then
        ...
        local status, score = GlobalMethod:HonorPointStatus(2)
...
function SceneHall:searchRoom(roomId,password)
    ...
    if id == nil then
    	...
    	if password == nil  then 
			...
			ProtocolProcessorSceneHall:send_ROOM_SelectRoom(id,self.matchType,0,"-1")
		else
			WZLog("SceneHall:searchRoom password ~= nil ")
			if password == "" then 
				ProtocolProcessorSceneHall:send_ROOM_SelectRoom(id,self.matchType,0,"-1")
			else 
				ProtocolProcessorSceneHall:send_ROOM_SelectRoom(id,self.matchType,0,password) 
			end 
		end
        self:createLoadingBox()
        return true
...
function SceneHall:onFastGameButtonClick(element)
    ...
    if CacheCenter:getGameParam()["arenaOpenTime"] then --11/27修改忽略
    ...
    ProtocolProcessorWndTask:send_PLAYER_GetHonourInfo( )
    self.m_nTouchButtonType = 2
end
function SceneHall:_onQuickGame()
...
-- 快速加入
function SceneHall:onQuickJoin(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)

    ProtocolProcessorWndTask:send_PLAYER_GetHonourInfo( )
    self.m_nTouchButtonType = 3
end
...
function SceneHall:closeTimeView()
    if not self.m_root then return end
    self:closeLoadingBox()
    local con = GetElement(self.m_root,"conMark_SceneHall",WZUIContainer)
    con:setVisible(false)
...
function SceneHall:_initPlayerDress()
    ...
    conPlayer:getAnimNode():setScale(1.2)
...
--@brief 显示足迹
function SceneHall:showRoleFootEffect(conP, conPlayer)
    local footId = CacheCenter:getUsingFootMarkId()
    if footId == nil then return end
    if not self.m_sRoleFootSpine then
        self.m_sRoleFootSpine = FootEffectManager:addEffect1(conP, footId, conPlayer:getPosition(),true,20,nil,nil,-150)
    end
end
...
--    WZLog("-----------------------5555-------------------------",self.m_root,Serialize(tInfo))
...
function SceneHall:onCreateRoomOK(element)
    ...
    if tag == 1 and CacheCenter:getPlayerInfo().level == 8 then 
        PostPlayerEvent:postEvent(PostPlayerEvent.event_eightLvChooseNum)
    end
    ProtocolProcessorWndTask:send_PLAYER_GetHonourInfo( )
    self.m_nTouchButtonType = 4
end
...
function SceneHall:onReward( element)
    ...
    WndActivityIntegrate:showInterface(1, g_tGameActivityTypes.ACTIVITY_TARGETREWARD_2, tMsg)
end
...
function SceneHall:_adaptLanguage_ug(  )
    local txtQuickBtn = GetElement(self.m_root,"txtQuickBtnDesc_SceneHall",WZUILabelTTF)
    txtQuickBtn:setDimensions(GlobalMethod:CCSize(180,0))
    txtQuickBtn:setScale(0.7)

    for i=1,2 do
        local txtFindRoom = GetElement(self.m_root,"txtFindRoom"..i.."_SceneHall",WZUILabelTTF)
        txtFindRoom:setDimensions(GlobalMethod:CCSize(180,0))
        txtFindRoom:setScale(0.7)
        local txtCreateRoom = GetElement(self.m_root,"txtCreateRoom"..i.."_SceneHall",WZUILabelTTF)
        txtCreateRoom:setDimensions(GlobalMethod:CCSize(180,0))
