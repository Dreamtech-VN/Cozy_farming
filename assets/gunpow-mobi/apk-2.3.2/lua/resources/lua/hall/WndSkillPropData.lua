...
function WndSkillProp:_init()
	...
	self.m_nCurrentSkillID = nil --当前技能的ID，主要用于技能预览的时候
	self.m_tPreViewSkill = {} --预览技能
	self.m_tProfesPreViewSkill = {} --预览职业技能
	self.m_tSkillGroup = {} --已经存在的技能
	self.m_tTempSaveAllSkill = {}
end
...
function WndSkillProp:_unInit()
	...
	self.m_nCurrentSkillID = nil
	self.m_tPreViewSkill = nil
	self.m_tProfesPreViewSkill = nil
	self.m_tSkillGroup = nil
	self.m_tTempSaveAllSkill = nil
end
...
function WndSkillProp:showSkillInfo()
    ...
    for i,v in ipairs(self.m_tPlayerSkillInfo.skillId) do
    	if v ~= -1 and v > 0 then
		    ...
		    local lafActionValue = GetElement(conPlayerSkill,"lafActionValue_WndSkillProp",WZUILabelTTF)
		    btnSkillCell:setTag(i)
    		local skillInfo = GDatatab_skill["id_"..v]
    		if skillInfo then
    			if skillInfo.skill_group ~= -1 then
	    			self.m_tSkillGroup[skillInfo.skill_group] = skillInfo.skill_group
	    		end
    			...
    			local imgSkillCost = GetElement(conPlayerSkill, "imgSkillCost_WndSkillProp", WZUIImage)
    			if skillInfo.upgrade and type(skillInfo.upgrade) == "table" and skillInfo.upgrade[1][1] == knowledge_id then 
    				imgSkillCost:setVisible(true)
    				imgSkillCost:setFile(GDatatab_item["id_" .. skillInfo.upgrade[1][1]].icon)
    			else
    				imgSkillCost:setVisible(false)
    			end
    		end
	...
	for i,v in ipairs(self.m_tPlayerSkillInfo.skillId) do
		if v == -1 then
        	...
        	if ProjConfig.LANGUAGE == "en" or ProjConfig.LANGUAGE == "es" or ProjConfig.LANGUAGE == "tr" or ProjConfig.LANGUAGE == "pt" then
...
function WndSkillProp:showSkillProps(itemId,expv)
	...
	--道具红点
	WndSkillContainer:setPropsRed(CacheCenter:getPropsRed()) 
end
--@brief  显示所有可以拥有的技能列表
function WndSkillProp:showAllSkill()
	...
	if self.m_bClickUpgrade then
    	...
    	local data = {}
        data.path = "ui/otherUI/skill_daojushenji"
        data.play = "effect"
        data.ccp = GlobalMethod:ccp(0.492268,0.659259)
        data.tag = 1102
        
    	local skillUpgrade = createEffectSpine(con, data)
    	if skillUpgrade then 
	...
	local temp_skill = {}
	temp_skill = CopyTable(self.m_tTempSaveAllSkill.itemId)
	local openSkillCount = #temp_skill
	-- if self.m_nWinType == 1 and CacheCenter.m_tSkill.mentorSkill ~= 0 then
	-- 	table.insert(temp_skill, CacheCenter.m_tSkill.mentorSkill)
	-- end
	WZLog("WndSkillProp:showAllSkill one", tostring(CacheCenter.m_tSkill.mentorSkill))
	for i,v in ipairs(temp_skill) do
		local skillInfo = GDatatab_skill["id_" .. v]
--		WZLog("显示技能", v)
		local id_group  = skillInfo.id_group
		local skillItem = {id = v,status = 2,exp = self.m_tTempSaveAllSkill.expv[i],group = id_group,equip=4,hdtj=skillInfo.hdtj,hdtjcs=skillInfo.hdtjcs,level=skillInfo.specialAttackParam,sort=skillInfo.sort, sub_type = skillInfo.sub_type, isMasterSkill = false}
	...
	for i,v in ipairs(self.m_tSkillList) do
		...
		-- if self.m_nWinType == 1 and CacheCenter.m_tSkill.mentorSkill and v == CacheCenter.m_tSkill.mentorSkill then
		-- 	isExit = false
		-- 	local temp_id = nil
		-- 	for _, m in pairs(GDatatab_skill) do
		-- 		if skillInfo.id_group == m.id_group and m.specialAttackParam == 1 and skillInfo.id_group == m.skill_group then
		-- 			temp_id = m.id
		-- 			break
		-- 		end
		-- 	end
		-- 	if temp_id then
		-- 		local skillInfoOne = GDatatab_skill["id_"..temp_id]
		-- 		local tData = CacheCenter:getSkill()
		-- 		for _, _v in pairs(tData.unlockSkill) do --自已解锁的id
		-- 			local temp_info = GDatatab_skill["id_".._v]
		-- 			if temp_info and skillInfoOne and temp_info.id_group == skillInfoOne.id_group then --如果与师傅的id相同的时候
		-- 				isExit = true
		-- 				break
		-- 			end
		-- 		end
		-- 	end
		-- end
		if not isExit and skillInfo then
			local id_group = skillInfo.id_group
			if skillInfo.specialAttackParam == 1 and not (id_group >= 1001 and id_group <= 1009) then
				local skillItem = {id = skillInfo.id, status = 1 ,exp = 0,group = id_group,equip=4,hdtj=skillInfo.hdtj,hdtjcs=skillInfo.hdtjcs,level=skillInfo.specialAttackParam,sort=skillInfo.sort, sub_type = skillInfo.sub_type, isMasterSkill = false}
		        table.insert(skills, skillItem)
			end
		end
	end
	...
	--添加师傅技能
	if self.m_nWinType == 1 and CacheCenter.m_tSkill.mentorSkill ~= 0 then
		local skillInfo = GDatatab_skill["id_" .. CacheCenter.m_tSkill.mentorSkill]
		local id_group  = skillInfo.id_group
		local skillItem = {id = CacheCenter.m_tSkill.mentorSkill,status = 2,exp = 0,group = id_group,equip=4,hdtj=skillInfo.hdtj,hdtjcs=skillInfo.hdtjcs,level=skillInfo.specialAttackParam,sort=skillInfo.sort, sub_type = skillInfo.sub_type, isMasterSkill = true}
		local bIsAdd = true 
		for j,k in ipairs(skills) do
			local tempSkillInfo = GDatatab_skill["id_" .. k.id]
			if k.status ~= 1 and (k.id == CacheCenter.m_tSkill.mentorSkill or (skillInfo.skill_group == tempSkillInfo.skill_group and skillInfo.specialAttackParam <= k.level)) then
				bIsAdd = false
				break 
			end
		end
	...
	self.m_tAllSkillProps = self:sortSkill(equipsSkillCount, openSkillCount, skills)
	...
	for i,v in ipairs(self.m_tAllSkillProps) do
		...
		m_tSkillItem[i] = conSkillInfo
		GetElement(conSkillInfo,"txtMaster",WZUILabelTTF):setVisible(false)
		if self.m_nWinType == 1 and (skillStats == LocalStrings.EQUIPPED or skillStats == LocalStrings.UNEQUIPPED) then
			if m_tMasterSkill[skillTable.skill_group] == nil then
				m_tMasterSkill[skillTable.skill_group] = {}
			end
			local tab = {}
			tab.index = i
			tab.level = skillTable.specialAttackParam
			tab.skill_id = v.id
			table.insert(m_tMasterSkill[skillTable.skill_group], tab)
		end
	    ...
	    --显示学识角标
	    local imgSkillCost = GetElement(conSkillInfo, "imgSkillCost_WndSkillProp", WZUIImage)
	    if imgSkillCost then 
	    	if skillTable.upgrade and type(skillTable.upgrade) == "table" and skillTable.upgrade[1][1] == knowledge_id then 
	    		imgSkillCost:setFile(GDatatab_item["id_" .. skillTable.upgrade[1][1]].icon)
	    		imgSkillCost:setVisible(true)
	    		GetElement(self.m_root, "btnResetSp_WndSkillProp", WZUIButton):setVisible(false)
	    	else
	    		imgSkillCost:setVisible(false)
	    		GetElement(self.m_root, "btnResetSp_WndSkillProp", WZUIButton):setVisible(true)
	    	end
	    end
	end
	if self.m_nWinType == 1 then
		local m_nSkillIndex = nil
		for _,v in pairs(m_tMasterSkill) do
			for i=1,#v do
				if CacheCenter.m_tSkill.mentorSkill ~= 0 and CacheCenter.m_tSkill.mentorSkill == v[i].skill_id then
					m_nSkillIndex = v[i].index
					break
				end				
			end
		end
		if m_nSkillIndex and m_tSkillItem[m_nSkillIndex] then
			GetElement(m_tSkillItem[m_nSkillIndex],"txtMaster",WZUILabelTTF):setVisible(true)
		end
	end
    ...
    elseif isHaveProp <= 0 and self.m_bIsActionEnd == true then
        ...
        TeachGroup1:startGroup({5,8,WndSkillContainer.m_tTopElement})
    end
...
function WndSkillProp:showSkillDetailInfo(skillId,bEquipped,bActive)
	...
	self.m_nCurrentSkillID = skillId
	if skillId then
		...
		if skillInfo then
			...
			if skillInfo.image ~= -1 then
				local bExist = WZFileUtil:isFileExist("ui/skill/"..skillInfo.image)
				if bExist then
			...
			self:_showOwnCostGood(skillId)

			local txtSkillName = GetElement(self.m_root,"txtSkillName_WndSkillProp",WZUILabelTTF)
			local lafActionValue3 = GetElement(self.m_root,"lafActionValue3_WndSkillProp",WZUILabelTTF)
			local lafActionValue6 = GetElement(self.m_root,"lafActionValue6_WndSkillProp",WZUILabelTTF)
			local txtCTV1 = GetElement(self.m_root,"txtCTV1_WndSkillProp",WZUILabelTTF)
			local txtCTV2 = GetElement(self.m_root,"txtCTV2_WndSkillProp",WZUILabelTTF)
		    ...
		    local txtCTV5 = GetElement(self.m_root,"txtCTV5_WndSkillProp",WZUILabelTTF)
			local txtCTV6 = GetElement(self.m_root,"txtCTV6_WndSkillProp",WZUILabelTTF)
			local txtStartTimeInitial = GetElement(self.m_root,"txtStartTimeInitial",WZUILabelTTF)
		    ...
		    if lvIcon ~= nil and type(lvIcon) == "string" then
				...
				--技能等级
				self.m_nCurrentSkillLev = tonumber(skillInfo.specialAttackParam) --tonumber(string.sub(lvIcon,-5,-5))
				GetElement(self.m_root,"value1_WndSkillProp",WZUILabelTTF):setText(self.m_nCurrentSkillLev)--string.sub(lvIcon,-5,-5))
				next_lv = string.sub(lvIcon,-5,-5)+1
				GetElement(self.m_root,"value4_WndSkillProp",WZUILabelTTF):setText(self.m_nCurrentSkillLev+1)--next_lv)
			else
				imgSkillL:setFile("")
		    end
		    ...
		    --初始项
		    local start_time = skillInfo.start_time
			GetElement(self.m_root,"conCd5",WZUIContainer):setVisible(start_time >= 1000)
			GetElement(self.m_root,"conCd6",WZUIContainer):setVisible(start_time >= 1000)
			GetElement(self.m_root,"arrow_4",WZUIImage):setVisible(start_time >= 1000)
			GetElement(self.m_root,"txtStartTimeInitial",WZUILabelTTF):setVisible(start_time >= 1000)
		    if start_time >= 1000 then
		    	txtCTV5:setText(start_time / 1000)
		    else
		    	txtCTV5:setText("")
		    end
		    --下一等级初始项
	    	local nextSkillId = skillInfo.upgrade_id
	    	local skillNextInfo = GDatatab_skill["id_"..nextSkillId]
	    	if skillNextInfo then
	    		if skillNextInfo.upgrade == -1 then
	    			local next_time = skillNextInfo.start_time
				    if next_time >= 1000 then
				    	txtCTV6:setText(next_time / 1000)
				    else
				    	txtCTV6:setText("")
				    end
	    		else
		    ...
		    if bEquipped or bActive then  
		    	...
		    	for i,v in ipairs(self.m_tAllSkillProps) do
		    		if v.exp and v.id == skillId then
...
--新界面的预览按钮
function WndSkillProp:onClickBtnPreview()
	if not self.m_nCurrentSkillID then return end
	if self.m_nCurrentSkillID == 61 then --飞行道具特殊处理

	end

	local skill_info = GDatatab_skill["id_"..self.m_nCurrentSkillID]
	if skill_info then
		local skill, profes = self:_setSkillSubType(skill_info.sub_type, skill_info.id_group)
		local preview = WndSkillPreView:createElement(skill, profes, self.m_nCurrentSkillLev, self.m_nCurrentSkillID)
		WindowManager:addWindow(preview, WndSkillPreView, false)
	end
end
...
function WndSkillProp:skillCanUpdate(skillId, skillProps)
	if skillId then
		if self.m_nWinType == 1 then
			...
			for i,v in ipairs(skillProps) do
    			if v.id == skillId then
					if v.status == 2 and skillInfo.upgrade_id > 0 and type(needNum) == "table" and (needNum[1][1] == 63 and skillNum >= needNum[1][2] or needNum[1][1] ~= 63 and CacheCenter:getPlayerItemCountById(needNum[1][1]) >= needNum[1][2]) then
	    				return true
	    			else
	    				return false
    				end
		...
		for i,v in ipairs(skillProps) do
    		if v.id == skillId then
    			if skillInfo.upgrade_id > 0 and type(upgrade) == "table" then
	    			if v.exp and v.exp >= upgrade[1][2] then
	    				return true
	    			end
	    		else
	    			return false
...
function WndSkillProp:setDisplay() 
	...
	if self.m_nWinType == 1 then
		...
		tbCon:setAbsContentSize(GlobalMethod:CCSize(520,260))
...
function WndSkillProp:setSkillState(state, tip) 
	...
	--判断是否师徒技能
	for i, v in pairs(self.m_tAllSkillProps) do
		if v.id == self.m_nCurShowSkillId and v.isMasterSkill then 
			GetElement(self.m_root,"btnUp",WZUIButton):setVisible(false)
			break 
		end
	end
	local status = not (state == 3 or state == 1)
	GetElement(self.m_root,"conCd2",WZUIContainer):setVisible(status)
	GetElement(self.m_root,"conCd4",WZUIContainer):setVisible(status)
	GetElement(self.m_root,"value4_WndSkillProp",WZUILabelTTF):setVisible(status)
	GetElement(self.m_root,"arrow_1",WZUIImage):setVisible(status)
	GetElement(self.m_root,"arrow_2",WZUIImage):setVisible(status)
	GetElement(self.m_root,"arrow_3",WZUIImage):setVisible(status)

	local isVisible = GetElement(self.m_root,"conCd5",WZUIContainer):isVisible()
	if isVisible == true then
		GetElement(self.m_root,"arrow_4",WZUIImage):setVisible(status)
		GetElement(self.m_root,"conCd6",WZUIContainer):setVisible(status)
	end
end

--@brief    缓存推送更新物品时调用的函数
function WndSkillProp:updatePlayerItemData()
    WZLog("WndSkillProp:updatePlayerItemData")

    if self.m_root ~= nil and self.m_nWinType == 1 then
        self:_showOwnCostGood(self.m_nCurShowSkillId)
    end
end
-------------------------------------公有方法模块End----------------------------------------
...
--注意的是可能存在沒有類型的時候
function WndSkillProp:_setSkillSubType(sub_type, id_group)
	local temp_type = sub_type .. id_group
	if self.m_tPreViewSkill[temp_type] and next(self.m_tPreViewSkill[temp_type]) ~= nil then
		local pro = {}
		if profes_type[sub_type] then
			if self.m_tProfesPreViewSkill[profes_type[sub_type]] then
				pro = self.m_tProfesPreViewSkill[profes_type[sub_type]]
			end
		end
		return self.m_tPreViewSkill[temp_type], pro
	end
	-- 预览技能
	self.m_tPreViewSkill[temp_type] = {}
	if profes_type[sub_type] then
		self.m_tProfesPreViewSkill[profes_type[sub_type]] = {}
	end
	for i,v in pairs(GDatatab_skill) do
		if self.m_nWinType == 1 then --技能的时候
			if v.sub_type == sub_type and v.skill_type == 0 and v.isNotSync == 0 then
				local tab = self:_getExtractData(v)
				table_insert(self.m_tPreViewSkill[temp_type],tab)
			end
		elseif self.m_nWinType == 2 then --道具的时候
			if v.id_group == id_group and v.sub_type == sub_type and v.specialAttackParam ~= -1 then
				local tab = self:_getExtractData(v)
				table_insert(self.m_tPreViewSkill[temp_type],tab)
			end
		end
		--附帶的職業技能
		if v.sub_type == profes_type[sub_type] then
			local tab = self:_getExtractData(v)
			table_insert(self.m_tProfesPreViewSkill[profes_type[sub_type]],tab)
		end
	end
	table_sort(self.m_tPreViewSkill[temp_type], function(a,b) return a.specialAttackParam < b.specialAttackParam end)
	if profes_type[sub_type] then
		table_sort(self.m_tProfesPreViewSkill[profes_type[sub_type]], function(a,b) return a.specialAttackParam < b.specialAttackParam end)
	end
	local pre_skill = {}
	if profes_type[sub_type] then
		pre_skill = self.m_tProfesPreViewSkill[profes_type[sub_type]]
	end
	return self.m_tPreViewSkill[temp_type], pre_skill
end
function WndSkillProp:_getExtractData(data)
	if not data then return {} end

	local tab = {}
	tab.specialAttackParam = data.specialAttackParam --等级
	--初始项
	local start_num = data.start_time
	if start_num >= 1000 then
		tab.start_num = start_num / 1000
	else
		tab.start_num = 0
	end
	--冷却
	local coolingTime = data.cooling_time
    if coolingTime >= 1000 then
    	tab.coolingTime = coolingTime / 1000
    else
    	tab.coolingTime = LocalStrings.NONE
    end
	tab.describe = data.describe2 or 0 --伤害
	tab.tool_desc = data.tool_desc or "" --说明
	tab.consume = math.ceil(data.consume/1000)--升级消耗
