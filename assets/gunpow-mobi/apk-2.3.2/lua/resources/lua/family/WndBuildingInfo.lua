...
function WndBuildingInfo:_update()
    ...
    if conForBuilding then
        ...
        if celElement and tNewObj then
            ...
            elseif tData.basicData.type == 1 and tData.basicData.sub_type == 8 then
                celElement:setScale(0.66)
            end
    ...
    --名字
    local sFormat = [[<T C="255,255,255" S="16" P="1" SC="132,66,29" SE="1" SS="4">%s</T><T C="255,255,255" S="16" P="1" SC="132,66,29" SE="1" SS="4">%s</T>]]
    --
    ...
    if ftxtSpeed then
        ...
        elseif tData.basicData.type == 9 then
            sFormat = [[<T C="127,70,26" S="20" P="1">%s</T><T C="229,105,22" S="20" P="0">%s</T>]]
            ftxtSpeed:setMaxWidth(240)
            ftxtSpeed:setRelativePosition(GlobalMethod:ccp(-0.3,1))
            ftxtSpeed:setShowText(string.format(sFormat, LocalStrings.KID_TEXT131..":", SceneKidSchoolHome:getEffectId()))
        end
    end
    ...
    if ftxtMaxNum then
        ...
        elseif tData.basicData.type == 9 then
            local skillEffect = 0
            for k,v in pairs(GDatatab_scstudy) do
                if v.addition == SceneKidSchoolHome:getEffectId() then
                    skillEffect = v.skill
                end
            end
            local strDesc = string.format(LocalStrings.KID_TEXT215,skillEffect)
            ftxtMaxNum:setMaxWidth(240)
            ftxtMaxNum:setRelativePosition(GlobalMethod:ccp(-0.3,1))
            ftxtMaxNum:setShowText(string.format(LocalStrings.KID_TEXT215, skillEffect))
        end
    end
    --描述
...
--@brief    刷新学校建筑
function WndBuildingInfo:_update3()
    -- body
    local tData = self.m_tBuildingData 
    local txtTitle = GetElement(self.m_root, "txtTitle_WndBuildingInfo", WZUILabelTTF)
    if txtTitle then 
    ...
    if conForBuilding then
        ...
        if celElement and tNewObj then
            tNewObj:setBuildingData(tData)
            tNewObj:setBuildingBG()
            tNewObj:setBuildingTouch(false)
            if tData.basicData.type == 8 then
                celElement:setScale(0.3)
                celElement:setRelativePosition(GlobalMethod:ccp(0.5, 0.6))
            elseif tData.basicData.type == 9 then
                celElement:setScale(0.5)
                celElement:setRelativePosition(GlobalMethod:ccp(0.5, 0.4))
            elseif tData.basicData.type == 7 or tData.basicData.type == 10 then
                celElement:setScale(0.5)
            elseif tData.basicData.type == 11 then
                celElement:setScale(0.7)
            elseif tData.basicData.type == 13 then
                celElement:setRelativePosition(GlobalMethod:ccp(0.5, 0.35))
            end
            conForBuilding:addChild(celElement)
        end
    ...
    GetElement(self.m_root, "conForInfo_WndBuildingInfo", WZUIContainer):setVisible(false)
    local ftxtDesc1 = GetElement(self.m_root, "ftxtDesc1_WndBuildingInfo", WZUIFreeTextBox)
    if ftxtDesc1 then
        if tData.basicData.type == 9 then
            sFormat = [[<T C="127,70,26" S="18" P="1">%s</T><T C="229,105,22" S="20" P="0">%s</T>]]
            ftxtDesc1:setShowText(string.format(sFormat, LocalStrings.KID_TEXT131..":", SceneKidSchoolHome:getEffectId()))
        end
    end
    local ftxtDesc2 = GetElement(self.m_root, "ftxtDesc2_WndBuildingInfo", WZUIFreeTextBox)
    if ftxtDesc2 then
        if tData.basicData.type == 9 then
            local skillEffect = 0
            for k,v in pairs(GDatatab_scstudy) do
                if v.addition == SceneKidSchoolHome:getEffectId() then
                    skillEffect = math.ceil(v.skill/60)
                end
            end
            local strDesc = string.format(LocalStrings.KID_TEXT215,skillEffect)
            ftxtDesc2:setShowText(string.format(LocalStrings.KID_TEXT215, skillEffect))
        end
    end

    --描述
    local txtDesc2 = GetElement(self.m_root, "txtDesc2_WndBuildingInfo", WZUILabelTTF)
    if txtDesc2 then
        txtDesc2:setText(tData.basicInfo.desc)
    end
end
...
function WndBuildingInfo:_adaptLanguage_ug(  )
    GetElement(self.m_root, "txtTitle_WndBuildingInfo", WZUILabelTTF):setScale(0.7)
