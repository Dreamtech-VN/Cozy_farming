...
function SceneFamily:onClickBuildingCallBack(element, tCell, tData)
    ...
    if self.m_nPlayerId ~= CacheCenter:getPlayerInfo().id then
        if (tData.basicData.id >= 40900 and tData.basicData.id <= 40919) then
            if CacheCenter:getPlayerInfo().pastureId <= 0 then
                MsgBoxManager:showTipBox(LocalStrings.PASTURE_TEXT54)
                return
            end
        else
            return
...
function SceneFamily:_createOneBuilding(indexX, indexY, lawnState,con)
    ...
    if self.m_tGridList[indexX][indexY].configId > 0 then
        ...
        if celElement and tNewObj then
            ...
            if self.m_tGridList[indexX][indexY].basicData.type == 1 and self.m_tGridList[indexX][indexY].basicData.sub_type == 8 then
