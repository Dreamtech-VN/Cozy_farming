...
function CellCharmRank:_update(  )
	...
	local txtCommunity = GetElement(self.m_root,"txtCommunity_CellCharmRank",WZUILabelTTF)
	...
	txtCommunity:setText(self.tData.community == "" and LocalStrings.COMMUNITY_COMPETE_TEXT44 or self.tData.community)

	if self.tData.playerId == CacheCenter:getPlayerInfo().id then
		imgBackground:setFile("ui/common/frame_lieb_01.png")
	else
		imgBackground:setFile("ui/common/frame_lieb.png")
	end
	if self.m_nRankType == 48 or self.m_nRankType == 49 or self.m_nRankType == 57 or self.m_nRankType == 58 then 
		GetElement(self.m_root, "imgTypeIcon_CellCharmRank", WZUIImage):setFile("ui/charmSpace/charmspace_good_icon.png")
	elseif self.m_nRankType == 50 or self.m_nRankType == 51 then 
		GetElement(self.m_root, "imgTypeIcon_CellCharmRank", WZUIImage):setFile("ui/space/common_icon_caiyc.png")
	else
		GetElement(self.m_root, "imgTypeIcon_CellCharmRank", WZUIImage):setFile("ui/space/common_icon_songh.png")
	end
	-- WZLog("选中item",Serialize(self.tData),Serialize(self.m_tOtherInfo))
