...
function CellDressGoodSeat:onClickGood(element)
	...
	local currentView = WndCharmSpace:getCurrentView()
	if nTimes <= 0 then 
		...
		if currentView == 1 then 
			ProtocolProcessorWndSpace:send_SPACE_GiveLike(1,self.m_tData.id)
		elseif currentView == 2 then
			ProtocolProcessorWndSpace:send_SPACE_GiveLike(2,self.m_tData.id)
		end
	end
end
...
function CellDressGoodSeat:updateGoodNum()
	...
	local sFormat1 = [[<T C="255,227,116" S="22" P="1" SC="132,66,29" SE="1" SS="4">%s</T>]]
...
--@brief 	设置点赞数隐藏可见
function CellDressGoodSeat:setGoodNumVisible(bVisible)
	GetElement(self.m_root, "ftxtGoodNum_CellDressGoodSeat", WZUIFreeTextBox):setVisible(bVisible)
end
-------------------------------------公有方法模块End----------------------------------------
