...
function CellCommunityAppoint:setData(tData, nWinType)
    self.m_tData = tData
...
--function CellCommunityAppoint:update()
	...
	GetElement(self.m_root, "txtFightWord_CellCommunityAppoint", WZUILabelTTF):setText(LocalStrings.COMBAT..":")
	--名字
	...
	elseif self.m_nWinType == 1 then 
		txtJob:setText(UNION_POSITION[position + 1])
	end
	--战斗力
	GetElement(self.m_root, "lafFighting_CellCommunityAppoint", WZUILabelAtlasFont):setText(self.m_tData.fight)
	if self.m_nWinType == 0 then 
		--本周贡献
