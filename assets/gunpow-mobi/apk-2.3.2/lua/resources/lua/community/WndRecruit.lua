...
function WndRecruit:onEnterTransitionDidFinish(element)
	...
	elseif self.m_nWinType == 1 then 
		ProtocolProcessorUnion:send_LEAGUE_GetApplyerList()
		local guildInfo = CacheCenter:getUnionInfo()
		if guildInfo == nil then return end
		GetElement(self.m_root,"ttfNumber",WZUILabelTTF):setText(WndUnionHall.m_nMembers.."/"..GDatatab_league_level["id_"..guildInfo.guildLevel].total)
	end
	AdaptLanguage(self)
end
...
function WndRecruit:onAgreeBtn(element)
	...
	elseif self.m_nWinType == 1 then 
		local guildLevel = WndUnionHall.m_nGuildLevel
		WZLog("当前人数",WndUnionHall.m_nMembers,"总人数",GDatatab_league_level["id_"..guildLevel].total)
		if tonumber(WndUnionHall.m_nMembers+#self.m_SelPlayerIdList) > tonumber(GDatatab_league_level["id_"..guildLevel].total) then
			MsgBoxManager:showTipBox(LocalStrings.UNION_TEXT1[30])
			return
		end
	end
	...
	WZLog("点击同意",Serialize(self.m_SelPlayerIdList))
	if self.m_SelPlayerIdList and next(self.m_SelPlayerIdList) then 
		...
		elseif self.m_nWinType == 1 then 
			ProtocolProcessorUnion:send_LEAGUE_Approval(vans, 1 )
...
function WndRecruit:onRefuseBtn(element)
	...
	--把表格转换成整型数组
	local vans = WZLuaVector_int_:create()
	WZLog("点击拒绝",Serialize(self.m_SelPlayerIdList))
	if self.m_SelPlayerIdList and next(self.m_SelPlayerIdList) then 
		for var = 1 ,#self.m_SelPlayerIdList do
			vans:push(tonumber(self.m_SelPlayerIdList[var]))
		end 
		...
		elseif self.m_nWinType == 1 then 
			ProtocolProcessorUnion:send_LEAGUE_Approval(vans, 0)
		end
	else 
		MsgBoxManager:showTipBox(LocalStrings.COMMUNITYINFO240)
	end 	
...
function WndRecruit:ScheduleCreateCell(element, delta)
	...
	if celElement ~= nil and tCell ~= nil then 
		...
		tCell:setRecruitListContent(self.m_tPendProList[self.m_nCurCelIndex].playerLevel,
									...
									self.m_tPendProList[self.m_nCurCelIndex].headEffectId)
		--设置ID
...
function WndRecruit:_update()
	...
	local conTextMemberList = GetElement(self.m_root, "conTextMemberList_WndRecruit", WZUIContainer)
	if #self.m_tPendProList == 0 then 
		ShowPanelNullTip(conTextMemberList)
		return 
	end
	removeShowPanelNullTip(conTextMemberList)
...
function WndRecruit:_adaptLanguage_ug()
	GetElement(self.m_root,"txtSelectAll_WndRecruit",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.64,0.843))
    local ttfNumber = GetElement(self.m_root,"ttfNumber",WZUILabelTTF)
    ttfNumber:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    ttfNumber:setRelativePosition(GlobalMethod:ccp(0.16,0.843))
    GetElement(self.m_root,"ttfRecruit_WndRecruit",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.28,0.843))
end
-------------------------------------私有方法模块End----------------------------------------
