...
function SceneMemberList:onNormal() 
	...
	SceneCommunity:resetTopTitle("ui/community/common_icon_ghdt.png", SceneCommunity, SceneCommunity.onClose)
	GetElement(self.m_root,"conMain1",WZUIContainer):setVisible(true)
	GetElement(self.m_root,"conMain3",WZUIContainer):setVisible(false)
...
function SceneMemberList:onBtm3() 
	...
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	SceneCommunity:resetTopTitle("ui/community/common_icon_ghgl.png", SceneMemberList, SceneMemberList.onNormal)
...
function SceneMemberList:onDonate(element)
	...
	WndCommunityCheckDonate:showInterface(0)
end
...
function SceneMemberList:onAppoint()
	...
	WndCommunityAppoint:showInterface(0)
end
...
function SceneMemberList:_update()
	...
	local info2 = [[<T C="255,236,193" S="18" P="0">%s </T><BR>6</BR><T C="255,236,193" S="18" P="0">%s</T>]]
	...
	local txtDeclarationStatus = GetElement(self.m_root,"txtDeclarationStatus_SceneMemberList",WZUILabelTTF)
	if tonumber(CacheCenter:getPlayerInfo().position) == COMMUNITY_PRESIDENT then --我是会长
		txtDeclarationStatus:setVisible(true)
		if self.m_nNoticeStatus == 0 then
			txtDeclarationStatus:setColor(ccc3(255,227,116))
			txtDeclarationStatus:setText("("..LocalStrings.PENDING_REVIEW..")")
		elseif self.m_nNoticeStatus == 1 then
			txtDeclarationStatus:setColor(ccc3(255,89,74))
			txtDeclarationStatus:setText("("..LocalStrings.FAILED_TO_PASS..")")
		elseif self.m_nNoticeStatus == 2 then
			txtDeclarationStatus:setColor(ccc3(99,255,95))
			txtDeclarationStatus:setText("("..LocalStrings.TRAINCAMP_DEC3..")")
		end
	else
		txtDeclarationStatus:setVisible(false)
	end
	...
	local position = tonumber(CacheCenter:getPlayerInfo().position)	
	local btnSave = GetElement(self.m_root, "btnSave_SceneMemberList", WZUIButton)
	...
	local otherInfo = LocalStrings.COMMUNITYINFO50..":" .. guildInfo.donate .. "  " .. LocalStrings.TDONATE .. self.m_nLimitDonate.."/"..gameParam.guildMaxDonate
	...
	local strNameAndLv = guildInfo.guildName .. "(" .. LocalStrings.LV .. guildInfo.guildLevel .. ")" .. "  " .. LocalStrings.PEOPLE_NUM .. self.m_nMembers.."/"..GDatatab_guild_level["id_"..guildInfo.guildLevel].total
...
--@brief 	添加捐献模块
function SceneMemberList:addDonate()
	-- body
	if self.m_root == nil then return end 
	
	local conDonate = GetElement(self.m_root, "conDonate_SceneMemberList", WZUIContainer)
	conDonate:removeAllChildrenWithCleanup(true)
	
	local wndCommunityDonate = WndCommunityDonate:createElement()
	if wndCommunityDonate then 
		conDonate:addChild(wndCommunityDonate)
	end
end
...
function SceneMemberList:_adaptLanguage_ug(  )
	local communityPrestige1 = GetElement(self.m_root,"CommunityPrestige1",WZUILabelTTF)
	communityPrestige1:setRelativePosition(GlobalMethod:ccp(0.44,0.3))
	...
	for i=1,2 do
		local txtMain3Check = GetElement(self.m_root,"txtMain3Check"..i.."_SceneMemberList",WZUILabelTTF)
		txtMain3Check:setScale(0.6)
		txtMain3Check:setDimensions(GlobalMethod:CCSize(210,0))
		txtMain3Check:setRelativePosition(GlobalMethod:ccp(0.8,0.5))
	end

	local txtManage1 = GetElement(self.m_root,"txtManage1_SceneMemberList",WZUILabelTTF)
	txtManage1:setDimensions(GlobalMethod:CCSize(180,0))
	txtManage1:setScale(0.7)
	local txtManage2 = GetElement(self.m_root,"txtManage2_SceneMemberList",WZUILabelTTF)
	txtManage2:setDimensions(GlobalMethod:CCSize(180,0))
	txtManage2:setScale(0.7)
	local txtManage3 = GetElement(self.m_root,"txtManage3_SceneMemberList",WZUILabelTTF)
	txtManage3:setDimensions(GlobalMethod:CCSize(180,0))
	txtManage3:setScale(0.7)
	local txtManage4 = GetElement(self.m_root,"txtManage4_SceneMemberList",WZUILabelTTF)
	txtManage4:setDimensions(GlobalMethod:CCSize(180,0))
	txtManage4:setScale(0.7)
	local txtManage5 = GetElement(self.m_root,"txtManage5_SceneMemberList",WZUILabelTTF)
	txtManage5:setDimensions(GlobalMethod:CCSize(180,0))
	txtManage5:setScale(0.7)
	local txtManage6 = GetElement(self.m_root,"txtManage6_SceneMemberList",WZUILabelTTF)
	txtManage6:setDimensions(GlobalMethod:CCSize(220,0))
	txtManage6:setScale(0.55)
	...
	local txtMain3Btn = GetElement(self.m_root,"txtMain3Btn_SceneMemberList",WZUILabelTTF)
	txtMain3Btn:setScale(0.7)
	txtMain3Btn:setDimensions(GlobalMethod:CCSize(140,0))
	txtMain3Btn:setRelativePosition(GlobalMethod:ccp(0.01,0.5))
	txtMain3Btn:setAlignment(kCCTextAlignmentCenter)

	local txtJob = GetElement(self.m_root,"txtJob_SceneMemberList",WZUILabelTTF)
	txtJob:setScale(0.7)
	txtJob:setDimensions(GlobalMethod:CCSize(160))
	local txtCon = GetElement(self.m_root,"txtCon_SceneMemberList",WZUILabelTTF)
	txtCon:setScale(0.7)
	txtCon:setDimensions(GlobalMethod:CCSize(160))
	local txtState = GetElement(self.m_root,"txtState_SceneMemberList",WZUILabelTTF)
	txtState:setScale(0.7)
	txtState:setDimensions(GlobalMethod:CCSize(160))

	local CommunityNameT = GetElement(self.m_root,"CommunityNameT",WZUILabelTTF)
	CommunityNameT:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityNameT:setRelativePosition(GlobalMethod:ccp(0.88,0.81))
	local CommunityName = GetElement(self.m_root,"CommunityName",WZUILabelTTF)
	CommunityName:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityName:setRelativePosition(GlobalMethod:ccp(0.48,0.81))
	local CommunityIdT = GetElement(self.m_root,"CommunityIdT",WZUILabelTTF)
	CommunityIdT:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityIdT:setRelativePosition(GlobalMethod:ccp(0.88,0.64))
	local CommunityId = GetElement(self.m_root,"CommunityId",WZUILabelTTF)
	CommunityId:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityId:setRelativePosition(GlobalMethod:ccp(0.48,0.64))
	local CommunityLvT = GetElement(self.m_root,"CommunityLvT",WZUILabelTTF)
	CommunityLvT:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityLvT:setRelativePosition(GlobalMethod:ccp(0.88,0.47))
	local CommunityLv = GetElement(self.m_root,"CommunityLv",WZUILabelTTF)
	CommunityLv:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityLv:setRelativePosition(GlobalMethod:ccp(0.48,0.47))
	local CommunityPrestige = GetElement(self.m_root,"CommunityPrestige",WZUILabelTTF)
	CommunityPrestige:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityPrestige:setRelativePosition(GlobalMethod:ccp(0.88,0.3))
	local CommunityPrestige1 = GetElement(self.m_root,"CommunityPrestige1",WZUILabelTTF)
	CommunityPrestige1:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityPrestige1:setRelativePosition(GlobalMethod:ccp(0.48,0.3))
	local CommunityMember = GetElement(self.m_root,"CommunityMember",WZUILabelTTF)
	CommunityMember:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityMember:setRelativePosition(GlobalMethod:ccp(0.88,0.13))
	local CommunityMember1 = GetElement(self.m_root,"CommunityMember1",WZUILabelTTF)
	CommunityMember1:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityMember1:setRelativePosition(GlobalMethod:ccp(0.48,0.13))

	local Contribution = GetElement(self.m_root,"Contribution",WZUILabelTTF)
	Contribution:setScale(0.7)
	Contribution:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	Contribution:setRelativePosition(GlobalMethod:ccp(0.92,0.75))
	local Contribution1 = GetElement(self.m_root,"Contribution1",WZUILabelTTF)
	Contribution1:setScale(0.7)
	Contribution1:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	Contribution1:setRelativePosition(GlobalMethod:ccp(0.34,0.75))
	local Contribution5 = GetElement(self.m_root,"Contribution5",WZUILabelTTF)
	Contribution5:setScale(0.7)
	Contribution5:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	Contribution5:setRelativePosition(GlobalMethod:ccp(0.92,0.2))
	local Contribution6 = GetElement(self.m_root,"Contribution6",WZUILabelTTF)
	Contribution6:setScale(0.7)
	Contribution6:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	Contribution6:setRelativePosition(GlobalMethod:ccp(0.34,0.2))

	local txtBtnLog = GetElement(self.m_root,"txtBtnLog_SceneMemberList",WZUILabelTTF)
	txtBtnLog:setScale(0.7)
	txtBtnLog:setDimensions(GlobalMethod:CCSize(170,0))
	local txtManage = GetElement(self.m_root,"txtManage_SceneMemberList",WZUILabelTTF)
	txtManage:setScale(0.7)
	txtManage:setDimensions(GlobalMethod:CCSize(170,0))
	local txtContribution = GetElement(self.m_root,"txtContribution_SceneMemberList",WZUILabelTTF)
	txtContribution:setScale(0.7)
	txtContribution:setDimensions(GlobalMethod:CCSize(170,0))
	local txtbtnContribution1 = GetElement(self.m_root,"txtbtnContribution1_SceneMemberList",WZUILabelTTF)
	txtbtnContribution1:setScale(0.7)
	txtbtnContribution1:setDimensions(GlobalMethod:CCSize(170,0))

	GetElement(self.m_root,"txtMain2Title1_SceneMemberList",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.5,0.5))
	GetElement(self.m_root,"txtMain2Title2_SceneMemberList",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.5,0.5))
	GetElement(self.m_root,"txtMain2Title3_SceneMemberList",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.5,0.5))

	GetElement(self.m_root,"txtSave_SceneMemberList",WZUILabelTTF):setScale(0.7)
	GetElement(self.m_root,"txtConfirm2_SceneMemberList",WZUILabelTTF):setScale(0.6)

	local CommunityMember2 = GetElement(self.m_root,"CommunityMember2",WZUILabelTTF)
	CommunityMember2:setScale(0.8)
	CommunityMember2:setRelativePosition(GlobalMethod:ccp(0.1,0.500371))
	local CommunityMember3 = GetElement(self.m_root,"CommunityMember3",WZUILabelTTF)
	CommunityMember3:setScale(0.8)
	CommunityMember3:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	CommunityMember3:setRelativePosition(GlobalMethod:ccp(0.09,0.500371))
end
