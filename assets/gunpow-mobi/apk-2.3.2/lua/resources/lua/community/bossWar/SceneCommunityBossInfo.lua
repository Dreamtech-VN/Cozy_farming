...
function SceneCommunityBossInfo:receiveMakePairOk(battleId, mapId, playerCount, 
    ...
    guaiAtk,weaponSkill,petLevel, colour, bodyColour, footmark, professionId, 
    professionSkill, mountId, childId, childName, childSex, childImage, assistSkillIds, 
    defaultShapeBigSkill, blastEffect, extPropertyKey, extPropertyValue, extPropertyCount)
    self:closeLoading()

    WZLog("SceneCommunityBossInfo:receiveMakePairOk", Serialize(professionId), Serialize(professionSkill))
    ...
    WBattleGlobal:getCurrent().m_tMakePairOk ={
        ...
        injuryFree=injuryFree,wreckDefense=wreckDefense,reduceCrit=reduceCrit,power=power,armor=armor,constitution=constitution,
        agility=agility, lucky=lucky,inspire=inspire,headId=headId,faceId=faceId,bodyId=bodyId,weaponId=weaponId,wingId=wingId,
        item_id=item_id,playerBuffCount, buffId=buffId,petId=petId,petSkill = petSkill,petLevel=petLevel,petSkillId=petId,petParam=petParam,
        guaiBattleId=guaiBattleId,guaiId=guaiId,guaiMaxHP=tempGuaiMaxHp,guaiNowHP=guaiNowHP,guaiAtk=guaiAtk,weaponSkill=weaponSkill, colour=colour, 
        bodyColour=bodyColour, footmark = footmark, professionId = professionId, professionSkill = professionSkill, mountId = mountId, 
        childId = childId, childName = childName, childSex = childSex, childImage = childImage, assistSkillIds = assistSkillIds, 
        defaultShapeBigSkill = defaultShapeBigSkill, blastEffect = blastEffect, extPropertyKey = extPropertyKey, extPropertyValue = extPropertyValue, extPropertyCount = extPropertyCount
    }
...
function SceneCommunityBossInfo:_updateView(data)
    ...
    WZLog("公会boss信息",data.bossHp,data.bossMaxHp)
    prg:setPercentage(math.min(data.bossHp*100/data.bossMaxHp, 100))

    local txtExp = GetElement(self.m_root, "labProHp_SceneCommunityBossInfo", WZUILabelTTF)
    txtExp:setText(math.floor(data.bossHp).."/"..data.bossMaxHp)
    --挑战人数
