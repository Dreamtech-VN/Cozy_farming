...
function CellCommunityRemove:onLoadData(element)
	...
	if self.m_nWinType == 1 then 
		...
		dt = math.floor(self.m_nDonateTime / 1000)
	end
	...
	if self.m_nLevelNum ~= nil then 
		GetElement(self.m_root,"txtLevel_CellCommunityRemove",WZUILabelTTF):setText(self.m_nLevelNum)
	end 
	if self.m_nWinType == 0 then 
		--文本
		GetElement(self.m_root, "txtWeekDonate_CellCommunityRemove", WZUILabelTTF):setText(LocalStrings.COMMUNITYINFO106 .. ": ")
		GetElement(self.m_root, "txtDayDonate_CellCommunityRemove", WZUILabelTTF):setText(LocalStrings.COMMUNITYINFO50 .. ": ")
		GetElement(self.m_root, "txtAllDonate_CellCommunityRemove", WZUILabelTTF):setText(LocalStrings.COMMUNITYINFO105 .. ": ")
		--贡献值
		GetElement(self.m_root, "txtWeekValue_CellCommunityRemove", WZUILabelTTF):setText(self.m_nWeekDonate)
		GetElement(self.m_root, "txtDayValue_CellCommunityRemove", WZUILabelTTF):setText(self.m_nTodayContribution)
		GetElement(self.m_root, "txtAllValue_CellCommunityRemove", WZUILabelTTF):setText(self.m_nAllDonate)
	end
	--玩家姓名 
	local txtPlayerName = self.m_root:getChildElement("txtPlayerName_CellCommunityRemove")
	if txtPlayerName ~= nil then 
	...
	--战斗力
	GetElement(self.m_root,"txtFightWord_CellCommunityRemove",WZUILabelTTF):setText(LocalStrings.COMBAT..": ")
	GetElement(self.m_root,"txtFight_CellCommunityRemove", WZUILabelAtlasFont):setText(self.fight)
	...
	if self.m_sState == 1 then
		txtState = WZUILabelTTF:luaTo(txtState)
		txtState:setText(LocalStrings.REWARD_BTN_LOGIN..":" .. LocalStrings.REWARD_BTN_ONLINE)
		return
	end
	if txtState ~= nil then 
		txtState = WZUILabelTTF:luaTo(txtState)
