...
function WndCommunityCheckDonate:update()
	...
	elseif self.m_nWinType == 1 then 
		if WndUnionHall == nil or WndUnionHall.m_tMemberList == nil then return end
...
function WndCommunityCheckDonate:_adaptLanguage_ug(  )
	for i=1,3 do
		local txtTab = GetElement(self.m_root,"txtTab"..i,WZUILabelTTF)
		local txtTabSel = GetElement(self.m_root,"txtTab"..i.."Sel",WZUILabelTTF)
		txtTab:setDimensions(GlobalMethod:CCSize(160,0))
		txtTab:setScale(0.5)
		txtTabSel:setDimensions(GlobalMethod:CCSize(150,0))
		txtTabSel:setScale(0.5)
	end
	for i=1,5 do
		local txtLabel = GetElement(self.m_root,"txtLabel"..i,WZUILabelTTF)
		txtLabel:setDimensions(GlobalMethod:CCSize(160,0))
