...
function WndCommunityTask:showBeforeRelease()
	...
	if self.m_tDataList1 == nil or #self.m_tDataList1 == 0 then 
		ShowPanelNullTip(conMsg,nil,GlobalMethod:ccc3(195,171,148))
...
function WndCommunityTask:_adaptLanguage_ug()
	local txtMsg = GetElement(self.m_root,"txtMsg_WndCommunityTask",WZUILabelTTF)
	txtMsg:setScale(0.7)
	txtMsg:setDimensions(GlobalMethod:CCSize(190))

	local txtRewardDes = GetElement(self.m_root,"txtRewardDes_WndCommunityTask",WZUILabelTTF)
	txtRewardDes:setScale(0.6)
	txtRewardDes:setDimensions(GlobalMethod:CCSize(700))
	txtRewardDes:setRelativePosition(GlobalMethod:ccp(0.7,0.056))
end
