...
function SceneCommunityMain:_addTop()
    ...
    tcell:setTopData("ui/common/common_icon_gh.png",SceneCommunityMain,SceneCommunityMain.onBuildClickBack,true,true,true,"SceneCommunityMain")
    tcell:setTopType()
end
...
function SceneCommunityMain:onEnterTransitionDidFinish(element)
    ...
    --延时显示成就特效
    ShowDelayAchie()
    if self.m_sSelMark then 
    	SceneCommunity:showInterface(self.m_sSelMark)
    end
end
...
function SceneCommunityMain:getDataOk()
    ...
    local name = LocalStrings.LV..CacheCenter:getGuildInfo().storeLevel..LocalStrings.ATH_SHOP
    self:createBuildNameText(imgShop, name)
    --大厅等级
    ...
    local name = LocalStrings.LV..CacheCenter:getGuildInfo().guildLevel..LocalStrings.HALL
    self:createBuildNameText(imgHall, name)
    --图腾等级
    ...
    local name = LocalStrings.LV..CacheCenter:getGuildInfo().totemLevel..LocalStrings.COMMUNITY_TEXT1
    self:createBuildNameText(imgTotem, name)
    ...
    local name = LocalStrings.LV..CacheCenter:getGuildInfo().schoolLevel..LocalStrings.COMMUNITY_TEXT2
    self:createBuildNameText(imgSkill, name)
	...
	elseif self.jumpTo == "hall" then
		self.jumpTo = nil
		SceneCommunity:showInterface("hall")
	elseif self.jumpTo == "copy" then
		self.jumpTo = nil
...
--@brief 	点击公会副本boss形象回调
function SceneCommunityMain:onClickCopyBoss(element)
	self:onCopy(element)
end
-------------------------------------公有方法模块End----------------------------------------
...
function SceneCommunityMain:onBuildClickTotem(element)
	...
	SceneCommunity:showInterface("totem")
end
...
function SceneCommunityMain:onBuildClickSkill(element)
	...
	if guildLevel == nil or guildLevel < 3 then
		...
		SceneCommunity:showInterface("skill")
	end
end
...
function SceneCommunityMain:onTask(element)
	...
	SceneCommunity:showInterface("task")
end
...
--@brief	公会副本
function SceneCommunityMain:onCopy(element)
...
function SceneCommunityMain:setBtn6Position()
	...
	local positionX = 0.016
	--判断是否显示公会战
    if CheckButtonShow(56) and GlobalMethod:crossServiceOpen() == 1 then
		positionX = positionX + 0.07
	end

	GetElement(self.m_root,"btnList6_SceneCommunityMain",WZUIButton):setRelativePosition(GlobalMethod:ccp(positionX,-1.1))
end
...
function SceneCommunityMain:setImpeachBtn()
	...
	GetElement(self.m_root,"btnList4_SceneCommunityMain",WZUIButton):setRelativePosition(GlobalMethod:ccp(positionX,-1.1))
...
function SceneCommunityMain:AdaptResolution()
	...
	--iphone4适配
	if directorSize.width == 1136 and directorSize.height == 640 then
		--GetElement(self.m_root,"btnBossMap_SceneCityScene",WZUIButton):setPosition(GlobalMethod:ccp(130 + (1136 - 1136)/2,370))
		--GetElement(self.m_root,"btnResearch_SceneCityScene",WZUIButton):setPosition(GlobalMethod:ccp(1036 - (1136 - 1136)/2,388))
		--GetElement(self.m_root,"img1_SceneCommunity",WZUIImage):setPosition(GlobalMethod:ccp(193 + (1136 - 1136)/2,217))
		--GetElement(self.m_root,"img4_SceneCommunity",WZUIImage):setPosition(GlobalMethod:ccp(931 - (1136 - 1136)/2,215))
	end
	--iphone5适配
...
--@brief    创建建筑名字文本
function SceneCommunityMain:createBuildNameText(element, name)
...
--@brief 	创建公会副本boss形象
function SceneCommunityMain:_createCopyBoss()
	local btnCopyBoss = GetElement(self.m_root, "btnCopyBoss_SceneCommunityScene", WZUIButton)
	if self.m_tBossData ~= nil then
		btnCopyBoss:setVisible(true)
	else
		btnCopyBoss:setVisible(false)
		return
	end

	btnCopyBoss:removeAllChildrenWithCleanup(true)
	...
	imgDizuo:setRelativePosition(GlobalMethod:ccp(0.5,0))
	imgDizuo:setUseOriginSize(true)
	imgDizuo:setFile("ui/community/building/community_pic_presidentdown01.png")
	imgDizuo:setTouchEnable(false)
	btnCopyBoss:addChild(imgDizuo)

	--boss形象
    local boss = BattleAnimation:createAnimation(self.m_tBossData.monsterInfo.AniFileId, false, "battle/monster")
    boss:play("wait",true)
    boss:setScale(self.m_tBossData.scale/100)
    btnCopyBoss:addChild(boss:getAnimNode())

    --名字
    local txtName = WZUILabelTTF:create()
    txtName:setRelativePosition(GlobalMethod:ccp(0.5, 1))
    txtName:setText(self.m_tBossData.monsterInfo.name)
    txtName:setFontSize(20)
    txtName:setEnableStroke(true)
    txtName:setStrokeSize(4)
    txtName:setStrokeColor(GlobalMethod:ccc3(105,65,46))
    txtName:setColor(GlobalMethod:ccc3(255,236,193))
    btnCopyBoss:addChild(txtName)

    --血条
    local conProBg = WZUIContainer:create()
    conProBg:setRelativePosition(GlobalMethod:ccp(0.5,0.9))
	conProBg:setAbsContentSize(GlobalMethod:CCSize(92,16))
	conProBg:setUseAbsSize(true)
    conProBg:setTouchEnable(false)
	btnCopyBoss:addChild(conProBg)
    local imgPro = WZUIImage:create()
    imgPro:setFile("ui/common/progress_gh_01.png")
    imgPro:setTouchEnable(false)
    conProBg:addChild(imgPro)
    local conProHp = WZUIContainer:create()
	conProHp:setAbsContentSize(GlobalMethod:CCSize(88,12))
	conProHp:setUseAbsSize(true)
    conProHp:setTouchEnable(false)
	conProBg:addChild(conProHp)
	WZLog("公会场景",self.m_tBossData.bossHp,self.m_tBossData.bossMaxHp)
    local progress = WZUIProgress:create()
    progress:setBgPicture("ui/common/progress_gh_02.png")
    progress:setTouchEnable(false)
    progress:setPercentage(math.ceil(self.m_tBossData.bossHp/self.m_tBossData.bossMaxHp*100))
    conProHp:addChild(progress)
    local txtHp = WZUILabelTTF:create()
    txtHp:setText(math.ceil(self.m_tBossData.bossHp/self.m_tBossData.bossMaxHp*100).."%")
    txtHp:setFontSize(18)
    txtHp:setScale(0.9)
    txtHp:setColor(GlobalMethod:ccc3(255,255,255))
    conProBg:addChild(txtHp)
end
