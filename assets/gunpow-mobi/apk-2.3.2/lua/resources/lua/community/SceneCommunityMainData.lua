...
--@brief    外部接口
function SceneCommunityMain:showInterface(mark)
    -- body
    local tempScene = SceneCommunityMain:createElement()
    if tempScene then
        self.m_sSelMark = mark 
...
--@brief   更新boss数据
function SceneCommunityMain:updateGuildBossInfo(data)
    if not self.m_root  then
        return
    end

    WZLog("SceneCommunityMain:updateGuildBossInfo",Serialize(data))
    self.m_tBossData = nil
    for k,v in pairs(GDatatab_guild_boss_map) do
        if v.id == data.bossId then
            self.m_tBossData = {}
            self.m_tBossData.scale = v.scale
            self.m_tBossData.bossHp = data.bossHp
            self.m_tBossData.monsterInfo = GDatatab_monster["id_"..v.monster[1][1]]
            self.m_tBossData.bossMaxHp = data.bossMaxHp
        end
    end

    self:_createCopyBoss()
end
-------------------------------------公有方法模块End----------------------------------------
