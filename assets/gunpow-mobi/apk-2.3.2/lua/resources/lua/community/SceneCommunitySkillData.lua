...
function SceneCommunitySkill:_init()
	...
	self.learnNum = 1 					--学习技能次数
	self.n_leranNum = 1					--保存服务端学习技能次数
end
...
function SceneCommunitySkill:setSkillLevel(id, level)
	...
	local consumeDonate = 0
	for i = 1,self.n_leranNum do
		for k,v in pairs(GDatatab_guild_property) do
			if v.type == self.m_tSkillLevels[self.m_nSkillTag].id and v.level == (self.m_tSkillLevels[self.m_nSkillTag].level + i) then
				consumeDonate = consumeDonate + v.cost[1][2]
			end
		end
	end

		local guildInfo = CacheCenter:getGuildInfo()
		guildInfo.totalDonate = guildInfo.totalDonate - consumeDonate
	...
	local sortTable = {3,4,1,5,7,19,20,9,12,11,10,13}
