...
function WndDismissCommunity:onSureBtn(element)
	...
	elseif self.m_nFlagWindow == 6 then 
		ProtocolProcessorUnion:send_LEAGUE_Resignations()
	end
...
function WndDismissCommunity:onPresidentYesBtn(element)
	...
	if self.m_nFlagWindow and self.m_nFlagWindow == 6 then --转让盟主
		ProtocolProcessorUnion:send_LEAGUE_Abdicate(self.m_nCelPlayerId)
	else
...
function WndDismissCommunity:setPresidentContainerVisable(flagWindow)
	if self.m_root == nil then 
		WZLog("WndDismissCommunity:destroyPresidentContainer self.m_root is nil ")
		return 
	end 
	self.m_nFlagWindow = flagWindow
	if flagWindow and flagWindow == 6 then 
		GetElement(self.m_root, "txtTitle_WndDismiss", WZUILabelTTF):setText(LocalStrings.UNION_TEXT1[27])
		--设置是否让位文本
		self:_setTextContent(LocalStrings.UNION_TEXT2[1])
	else
...
function WndDismissCommunity:setExitCommunityWindow(nFlagWindow)
	if self.m_root == nil then 
	...
	elseif self.m_nFlagWindow == 6 then 
		GetElement(self.m_root, "txtTitle_WndDismiss", WZUILabelTTF):setText(LocalStrings.UNION_TEXT1[5])
		self:setTxtMidContent(LocalStrings.UNION_TEXT1[46])
	end
end 
...
function WndDismissCommunity:_adaptLanguage_ug()
	local txtSure = GetElement(self.m_root, "txtSure_WndDismissCommunity", WZUILabelTTF)
	txtSure:setScale(0.7)
	txtSure:setDimensions(GlobalMethod:CCSize(160))
    local txtCancel = GetElement(self.m_root, "txtCancel_WndDismissCommunity", WZUILabelTTF)
	...
	txtOK:setScale(0.7)
	txtOK:setDimensions(GlobalMethod:CCSize(160))
end 
------------------------------------语言适配模块End----------------------------------------