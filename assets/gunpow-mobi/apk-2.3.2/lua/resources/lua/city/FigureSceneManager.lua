        ...
        if isStatue or CacheCenter:getPlayerInfo().id ~= info.playerId then
            ...
            if isStatue ~= true then
                for index, figure in pairs (self.m_tFigureList) do
                    if figure.m_tPlayerInfo ~= nil and info.playerId == figure.m_tPlayerInfo.id and figure.m_nFigureType ~= FigureType.Statue and figure.m_nFigureType ~= FigureType.Score and figure.m_nFigureType ~= FigureType.Fighting and figure.m_nFigureType ~= FigureType.Strategic3V3 then
                        figureIndex = index
            ...
            if figureIndex ~= -1 then
                ...
                playerInfo.professionId = info.professionId         --玩家职业
                playerInfo.openStatus = info.openStatus
                playerInfo.qqHallData = info.qqHallData
                if isStatue then
                    ...
                    if self.m_bIsScore == true and info.statueType == 3 then
                        if self.m_tFigureList then
                            for i , v in pairs (self.m_tFigureList) do
                                if self.m_tFigureList[i] and self.m_tFigureList[i].m_nFigureType == FigureType.Fighting then
                                    self.m_tFigureList[i]:destroy()
                                    self.m_tFigureList[i] = nil
                                    break
                                end
                    ...
                    if self.m_bIsScore == true and info.statueType == 4 then
                        if self.m_tFigureList then
                            for i , v in pairs (self.m_tFigureList) do
                                if self.m_tFigureList[i] and self.m_tFigureList[i].m_nFigureType == FigureType.Strategic3V3 then
                                    self.m_tFigureList[i]:destroy()
                                    self.m_tFigureList[i] = nil
                                    break
                                end
                    ...
                    elseif info.statueType == 3 then
                        self.m_bIsScore = true
                        nTempStatue = FigureType.Fighting
                    elseif info.statueType == 4 then
                        self.m_bIsScore = true
                        nTempStatue = FigureType.Strategic3V3
                    end
                    ...
                    if figure then
                        ...
                        if info.statueType == 3 then
                            figure.m_tUINode:setContentSize(GlobalMethod:CCSize(80, 200))
                            posx, posy = 1250, 295
                        elseif info.statueType == 2 or info.statueType == 4 then
                            figure.m_tUINode:setContentSize(GlobalMethod:CCSize(80, 200))
                            posx, posy = 2120, 270
                        end
                    ...
                    if figure then
                        ...
                        for i, figure in pairs(list) do
                            if true or (figure.m_nFigureType ~= FigureType.Npc and figure.m_nFigureType ~= FigureType.Statue and figure.m_nFigureId ~= playerInfo.id and figure.m_nFigureType ~= FigureType.Score and figure.m_nFigureType ~= FigureType.Fighting and figure.m_nFigureType ~= FigureType.Strategic3V3) then
...
function FigureSceneManager:onClickBg(element,event,x,y)
    if not self.m_tScene then return end
...
function FigureSceneManager:update(dt0)
    ...
    if self.m_bCanUpdate then
        ...
        if screenLeft ~= self.m_nScreenLeft and screenRight ~= self.m_nScreenRight then
            ...
            --self.m_tFigure:setPosition({x=1136, y=220})
        end
...
function FigureSceneManager:onOtherHide()
    ...
    if WndOwnCity.hideOtherHead then
...
--@brief    显示自己和伴侣互动动画
function FigureSceneManager:showCoupleAnimation()
    local nCoupleId = 0
    ...
    local bExist = false
    if nCoupleId > 0 and self.m_tFigureList then
        for i , v in pairs (self.m_tFigureList) do
            if self.m_tFigureList[i] and self.m_tFigureList[i].m_nFigureType == FigureType.Other then
                if self.m_tFigureList[i].m_nFigureId == nCoupleId then
                    bExist = true
                    break
                end
            end
    ...
    if self.m_tFigureList then
        for i , v in pairs (self.m_tFigureList) do
            if self.m_tFigureList[i] then
                if bExist and ((self.m_tFigureList[i].m_nFigureType == FigureType.Other and self.m_tFigureList[i].m_nFigureId == nCoupleId) or (self.m_tFigureList[i].m_nFigureType == FigureType.Myself and self.m_tFigureList[i].m_nFigureId == CacheCenter:getPlayerInfo().id)) then
                    ShowCoupleAni(self.m_tFigureList[i].m_tUINode, true, GlobalMethod:ccp(0.5,1.1), 0.65)
                else
                    ShowCoupleAni(self.m_tFigureList[i].m_tUINode, false, GlobalMethod:ccp(0.5,1.1), 0.65)
                end
            end
        end
    end
...
function FigureSceneManager:_init()
    ...
    local scale = math.floor(g_figureSceneManager.m_tWinSize.width / 1136)
