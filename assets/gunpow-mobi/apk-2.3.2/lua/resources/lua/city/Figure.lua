...
Figure =
{
    ...
    m_nNameOffsetYChangeMonster = 3,
    m_nNameOffsetXChangeMonster = 0.45,
    m_nNameOffsetYMonster = 3,
    ...
    m_nFootId = nil,
    m_tKidAni = nil,             --小孩动画
    m_professionIcon = nil,      --玩家职业图标
    m_qqBluePriIcon = nil,       --玩家蓝钻特权图标
    m_qqBluePriYearIcon = nil,       --玩家蓝钻特权年费图标
    m_conName = nil,             --名字、职业图标、蓝钻特权容器
}
...
function Figure:create(equipList, sex, figureType, name, figureId, isArmature, isHaveAnim,lv, title, equips,footId)
    ...
    figure.m_nSpeed = 0.24
    figure.m_nSpeedMount = 0.156
...
function Figure:createFigureAnim(equipList, sex, isArmature, isReal, isOther)
    ...
    local scale = 0.60
    if self.m_nFigureType == FigureType.Score or self.m_nFigureType == FigureType.Fighting or self.m_nFigureType == FigureType.Strategic3V3 then
        scale = 0.75
    ...
    if true then
        ...
        elseif self.m_nFigureType == FigureType.Strategic3V3 then
            self.m_tUINode:setClickCallback("onStrategic3V3Click")
        elseif self.m_nFigureType == FigureType.Fighting then
            self.m_tUINode:setClickCallback("onFightingClick")
        end
    ...
    if self.m_nFigureType == FigureType.Statue then
        ...
        -- self.m_tUINode:addChild(img,1)
        local data = {}
        data.path = "ui/city/newMainScene/armature/scene_qiqiu2"
        data.play = "animation"
        data.loop = true
        data.ccp = GlobalMethod:ccp(0.22,0.25)
        data.zOrder = 1
        local spineQiQiu = createEffectSpine(self.m_tUINode, data)
    ...
    elseif self.m_nFigureType == FigureType.Score or self.m_nFigureType == FigureType.Fighting or self.m_nFigureType == FigureType.Strategic3V3 then
        --积分雕像底座
        ...
        img:setAnchorPoint(GlobalMethod:ccp(0.5,0.5))
        self.m_tShadow = img
        if self.m_nFigureType == FigureType.Fighting then 
            self.m_tShadow:setPosition(GlobalMethod:ccp(30,5))

            --称号底框
            local imgTitle = WZUIImage:create()
            imgTitle:setFile("ui/common/common_zlzw_di_01.png")
            imgTitle:setUseOriginSize(true)
            imgTitle:setScale(1.4)
            imgTitle:setRelativePosition(GlobalMethod:ccp(0.1,1.8))
            self.m_tUINode:addChild(imgTitle,1)

            local txtTitle1 = createLabel(LocalStrings.OTHER_TEXT1[8], GlobalMethod:ccp(0.1,1.83), GlobalMethod:ccp(0.5,0.5), 26, GlobalMethod:ccc3(255,250,236))
            txtTitle1:setEnableStroke(true)
            txtTitle1:setStrokeColor(GlobalMethod:ccc3(163,74,20))
            txtTitle1:setStrokeSize(4)
            if ProjConfig.LANGUAGE == "cn" then
                txtTitle1:setVisible(false)
            end
            self.m_tUINode:addChild(txtTitle1,2)
        elseif self.m_nFigureType == FigureType.Strategic3V3 then 
            self.m_tShadow:setPosition(GlobalMethod:ccp(30,5))

            --称号底框
            local imgTitle = WZUIImage:create()
            imgTitle:setFile("ui/common/common_zlzw_lsxx_di.png")
            imgTitle:setUseOriginSize(true)
            imgTitle:setScale(1.4)
            imgTitle:setRelativePosition(GlobalMethod:ccp(0.1,1.8))
            self.m_tUINode:addChild(imgTitle,1)

            local txtTitle1 = createLabel(LocalStrings.FIGURE_STATUE_TEXT1, GlobalMethod:ccp(0.1,1.83), GlobalMethod:ccp(0.5,0.5), 26, GlobalMethod:ccc3(255,250,236))
            txtTitle1:setEnableStroke(true)
            txtTitle1:setStrokeColor(GlobalMethod:ccc3(163,74,20))
            txtTitle1:setStrokeSize(4)
            self.m_tUINode:addChild(txtTitle1,2)
        else
            self.m_tShadow:setPosition(GlobalMethod:ccp(30,8))
        end
...
--@brief    点击战略赛3v3雕像
function Figure:onStrategic3V3Click()
    WZLog("Figure:onStrategic3V3Click")
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
    WndPvpStrategicRank:showWin4UI(1,3)
end

--@brief    点击战力之王雕像回调
function Figure:onFightingClick()
...
function Figure:onNpcClick()
    ...
    elseif self.m_nAnimId == 1 then
        ...
        WndStore:showStoreByType(10, nil, nil, 1)
    end
...
function Figure:createName()
    ...
    if self.m_tNameLabel ~= nil then
        ...
        --更新职业
        local professionId = self.m_tPlayerInfo.professionId 
--        WZLog("创建名称1",self.m_tPlayerInfo.name,self.m_tPlayerInfo.professionId,self.m_tPlayerInfo.openStatus)
        if self.m_professionIcon then 
            if professionId and professionId > 0 then 
                if self.m_tPlayerInfo.openStatus == 1 then
                    self.m_professionIcon:setFile(g_professionIcon[professionId])
                elseif self.m_tPlayerInfo.openStatus == 2 then 
                    self.m_professionIcon:setFile(g_professionIcon2[professionId])
                end
                namePosX = namePosX + 40
            else
                self.m_professionIcon:setFile("")
            end
        end
        if self.m_tPlayerInfo.qqHallData and bShowQQInfo then 
            if (self.m_tPlayerInfo.qqHallData.is_blue_vip or self.m_tPlayerInfo.qqHallData.is_super_blue_vip) and self.m_qqBluePriIcon then 
                if self.m_tPlayerInfo.qqHallData.is_super_blue_vip then 
                    self.m_qqBluePriIcon:setFile("ui/qqHall/hh_" .. self.m_tPlayerInfo.qqHallData.blue_vip_level .. ".png")
                else
                    self.m_qqBluePriIcon:setFile("ui/qqHall/pz_" .. self.m_tPlayerInfo.qqHallData.blue_vip_level .. ".png")
                end
                self.m_qqBluePriIcon:setAbsPosition(GlobalMethod:ccp(namePosX, 20))
                namePosX = namePosX + 30
                if self.m_tPlayerInfo.qqHallData.is_blue_year_vip and self.m_qqBluePriYearIcon then 
                    self.m_qqBluePriYearIcon:setFile("ui/qqHall/nian.png")
                    namePosX = namePosX + 30
                elseif self.m_qqBluePriYearIcon then 
                    self.m_qqBluePriYearIcon:setFile("")
                end
            else
                self.m_qqBluePriIcon:setFile("")
            end
        end
        --Modify By Tianxiang_Xu
        self.m_tNameLabel:setAbsPosition(GlobalMethod:ccp(namePosX, 20))
        local nameSize = self.m_tNameLabel:getContentSize()
        namePosX = namePosX + nameSize.width
        if self.m_conName then 
            self.m_conName:setAbsContentSize(GlobalMethod:CCSize(namePosX, 40))
            self.m_conName:updateRelativeSize()
            self.m_conName:setRelativePositionLuaTo(0.5, -0.5)
        end
--        WZLog("Figure:createName **********", titleY, self:getAnimation():isFlipX(), self:isFlipX())
        CreateDesiSpine(self.m_tNameContainer, self.m_tLvLabel, title, GlobalMethod:ccp(0.5,titleY), false)
        if self:getAnimation():isFlipX() then 
            if self.m_tNameContainer and self.m_tNameContainer:getChildByTag(444) then 
                local spineDes = WZUISpine:luaTo(self.m_tNameContainer:getChildByTag(444))
                spineDes:setFlipX(self:getAnimation():isFlipX())
            end
        end
    ...
    conName:setRelativePositionLuaTo(0.5, -0.5)
    imgContainer:addChild(conName)
    self.m_conName = conName
    --添加职业图标
    if self.m_tPlayerInfo ~= nil then
        local professionId = self.m_tPlayerInfo.professionId
        local imgProfessionIcon = WZUIImage:create()
        imgProfessionIcon:setUseOriginSize(true)
        imgProfessionIcon:setScale(0.35)
        imgProfessionIcon:setAnchorPoint(GlobalMethod:ccp(0, 0.5))
        imgProfessionIcon:setUseAbsCoordinate(true)
        imgProfessionIcon:setAbsPosition(GlobalMethod:ccp(namePosX, 20))

        self.m_professionIcon = imgProfessionIcon 
--        WZLog("创建名称2",self.m_tPlayerInfo.name,self.m_tPlayerInfo.professionId,self.m_tPlayerInfo.openStatus)
        if professionId and professionId > 0 then
            if self.m_nFigureType == FigureType.Myself then
                if self.m_tPlayerInfo.professionAttr2 == "{}" then
                     imgProfessionIcon:setFile(g_professionIcon[professionId])
                else 
                    imgProfessionIcon:setFile(g_professionIcon2[professionId])
                end                   
            else
                if self.m_tPlayerInfo.openStatus == 1 then
                    imgProfessionIcon:setFile(g_professionIcon[professionId])
            ...
            imgProfessionIcon:setFile("")
        end
        conName:addChild(imgProfessionIcon)
        ...
        self.m_qqBluePriIcon = imgQQBlue
        ...
        self.m_qqBluePriYearIcon = imgQQYear
        conName:addChild(imgQQYear)
        if self.m_tPlayerInfo.qqHallData and bShowQQInfo then 
            if self.m_tPlayerInfo.qqHallData.is_blue_vip or self.m_tPlayerInfo.qqHallData.is_super_blue_vip then 
                if self.m_tPlayerInfo.qqHallData.is_super_blue_vip then 
                    self.m_qqBluePriIcon:setFile("ui/qqHall/hh_" .. self.m_tPlayerInfo.qqHallData.blue_vip_level .. ".png")
                else
                    self.m_qqBluePriIcon:setFile("ui/qqHall/pz_" .. self.m_tPlayerInfo.qqHallData.blue_vip_level .. ".png")
                ...
                namePosX = namePosX + 30
                self.m_qqBluePriYearIcon:setAbsPosition(GlobalMethod:ccp(namePosX, 20))
                if self.m_tPlayerInfo.qqHallData.is_blue_year_vip then 
                    namePosX = namePosX + 30
                    self.m_qqBluePriYearIcon:setFile("ui/qqHall/nian.png")
    ...
    ttf:setText(name)
    ttf:setAnchorPoint(GlobalMethod:ccp(0,0.5))
    --Modify By Tianxiang_Xu
    ttf:setEnableStroke(true)
    ttf:setStrokeColor(GlobalMethod:ccc3(62,34,8))
    ttf:setStrokeSize(4)
    ttf:setFontSize(18)
    ttf:setUseAbsCoordinate(true)
    ttf:setAbsPosition(GlobalMethod:ccp(namePosX, 20))
    if self.m_nFigureType == FigureType.Fighting or self.m_nFigureType == FigureType.Strategic3V3 then
        ttf:setFontSize(26)
        ttf:setUseAbsCoordinate(false)
        ttf:setAnchorPoint(GlobalMethod:ccp(0.5,0.5))
        ttf:setAbsPosition(GlobalMethod:ccp(0,0))
        ttf:setRelativePosition(GlobalMethod:ccp(0.2,1.18))
        self.m_tUINode:addChild(ttf,2)
    else
        conName:addChild(ttf)
    end
    local tempttfPosX = namePosX
    local nameSize = ttf:getContentSize()
    namePosX = namePosX + nameSize.width
    ...
    elseif self.m_nFigureType == FigureType.Fighting or self.m_nFigureType == FigureType.Strategic3V3 then
        ttf:setColor(GlobalMethod:ccc3(127,70,26))
        ttf:setEnableStroke(false)
    end
    ...
    --Modify By Tianxiang_Xu
    ttfLv:setRelativePositionLuaTo(0.5, 0.28)
    ...
    if self.m_nFigureType == FigureType.Npc then
        if self.m_nAnimId == 1 then
            imgContainer:setRelativePositionLuaTo(0.5, 1.25)
        else
            imgContainer:setRelativePositionLuaTo(0.5, 1.3)
        end
    elseif self.m_bIsChangeToMonster then
        if self.m_nFigureType == FigureType.Other then
            imgContainer:setRelativePositionLuaTo(self.m_nNameOffsetXChangeMonster, self.m_nNameOffsetYChangeMonster-1.78)
        elseif self.m_nFigureType == FigureType.Score or self.m_nFigureType == FigureType.Fighting or self.m_nFigureType == FigureType.Strategic3V3 then
            imgContainer:setRelativePositionLuaTo(self.m_nNameOffsetXChangeMonster, self.m_nNameOffsetYChangeMonster-0.8)
        elseif self.m_nFigureType == FigureType.Statue then
            imgContainer:setRelativePosition(GlobalMethod:ccp(0.3, self.m_nNameOffsetYMonster-0.7))
        else
    ...
    elseif self.m_bIsMonster then
        if self.m_nFigureType == FigureType.Other then
            imgContainer:setRelativePositionLuaTo(self.m_nNameOffsetXMonster, self.m_nNameOffsetYMonster-0.5)
        elseif self.m_nFigureType == FigureType.Score or self.m_nFigureType == FigureType.Fighting or self.m_nFigureType == FigureType.Strategic3V3 then
            imgContainer:setRelativePositionLuaTo(self.m_nNameOffsetXMonster, self.m_nNameOffsetYMonster-0.8)
        elseif self.m_nFigureType == FigureType.Statue then
            imgContainer:setRelativePosition(GlobalMethod:ccp(0.3, self.m_nNameOffsetYMonster-0.7))
        else
...
function Figure:changeMount(isMount)
    ...
    if self.m_tPlayerInfo and self.m_tPlayerInfo.mountsId ~= nil and self.m_tPlayerInfo.mountsId ~= 0 and isMount ~= false then
        ...
        if self.m_tNameContainer ~= nil then
            if self.m_nFigureType == FigureType.Npc then
                if self.m_nAnimId == 1 then
                    self.m_tNameContainer:setRelativePositionLuaTo(0.5, 1.25)
                else
                    self.m_tNameContainer:setRelativePositionLuaTo(0.5, 1.3)
                end
            elseif self.m_bIsChangeToMonster then
                if self.m_nFigureType == FigureType.Other then
                    self.m_tNameContainer:setRelativePositionLuaTo(self.m_nNameOffsetXChangeMonster, self.m_nNameOffsetYChangeMonster-1.78)
                elseif self.m_nFigureType == FigureType.Score or self.m_nFigureType == FigureType.Fighting or self.m_nFigureType == FigureType.Strategic3V3 then
                    self.m_tNameContainer:setRelativePositionLuaTo(self.m_nNameOffsetXChangeMonster, self.m_nNameOffsetYChangeMonster-0.8)
                elseif self.m_nFigureType == FigureType.Statue then
                    self.m_tNameContainer:setRelativePositionLuaTo(0.3, self.m_nNameOffsetYChangeMonster-0.7)
                else
                    self.m_tNameContainer:setRelativePositionLuaTo(self.m_nNameOffsetXChangeMonster, self.m_nNameOffsetYChangeMonster)
                end
            elseif self.m_bIsMonster then
                if self.m_nFigureType == FigureType.Other then
                    self.m_tNameContainer:setRelativePositionLuaTo(self.m_nNameOffsetXMonster, self.m_nNameOffsetYMonster-0.5)
                elseif self.m_nFigureType == FigureType.Score or self.m_nFigureType == FigureType.Fighting or self.m_nFigureType == FigureType.Strategic3V3 then
                    self.m_tNameContainer:setRelativePositionLuaTo(self.m_nNameOffsetXMonster, self.m_nNameOffsetYMonster-0.8)
                elseif self.m_nFigureType == FigureType.Statue then
                    self.m_tNameContainer:setRelativePosition(GlobalMethod:ccp(0.3, self.m_nNameOffsetYMonster-0.7))
                else
...
function Figure:destroy()
    ...
    if self.m_conName ~= nil then
        self.m_conName:removeFromParentAndCleanup(true)
        self.m_conName = nil 
    end
