...
function WndOwnCity:_init()
    ...
    -- self.m_nAdditionalMoveDis = 0.02--1.6.4:右上角活动栏整行下移大概20像素
    self.m_sRes = nil
    self.m_tBtnList = nil 
end
...
function WndOwnCity:_unInit()
    ...
    self.m_tArmatureCardWelfare = nil
    self.m_tArmatureCardWelfare2 = nil
    self.m_bIsNeedMoveVerticalBar = nil 
    self.m_bIsMoveVerticalBar = nil 
    self.m_nMoveDirection = nil 
    -- self.m_nAdditionalMoveDis = 0       --1.6.4:右上角活动栏整行下移大概20像素
    self.m_sRes = nil
    self.m_tBtnList = nil 
end
...
function WndOwnCity:updatePlayerInfoData()
    ...
    self:updateSingleCopyGuild()
end

-- 监听时装改变，改变玩家装扮
function WndOwnCity:updateDecorationData()
...
function WndOwnCity:updatePlayerItemData()
    WZLog("WndOwnCity:updatePlayerItemData")
    if self.m_root and self.m_tHeadAnim then
        local headEffectId = CacheCenter:getPlayerHeadEffectItemId()
        self.m_tHeadAnim:resetHeadEffect(headEffectId)
    end
end
...
function WndOwnCity:_setDefaultBtnsInfo()
    ...
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == ISLAND_UP_MAGIC_STONE and (g_cityExtenInfo == nil or g_cityExtenInfo.magicStoneStatus == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == ISLAND_UP_INVESTREBATE and (g_cityExtenInfo == nil or g_cityExtenInfo.IRStatus == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == ISLAND_UP_HAPPYSHAKE and (g_cityExtenInfo == nil or g_cityExtenInfo.activityPokerStatus == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == ISLAND_UP_CRAZY_DOUBLING and (g_cityExtenInfo == nil or g_cityExtenInfo.CDStatus == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == ISLAND_UP_AUCTION_HOUSE and (g_cityExtenInfo == nil or g_cityExtenInfo.auction == nil or g_cityExtenInfo.auction == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == DOUBLE_SEVEN_CONFREE and (g_cityExtenInfo == nil or g_cityExtenInfo.qiXiActivity == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == NATIONAL_FESTIVAL and (g_cityExtenInfo == nil or g_cityExtenInfo.rechargeSignInActivity == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == NATIONAL_ANSWER and (g_cityExtenInfo == nil or g_cityExtenInfo.funAnswerActivity == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == PEOPLESHOP and (g_cityExtenInfo == nil or g_cityExtenInfo.shoppingActivity == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == TREASURESEARCH and (g_cityExtenInfo == nil or g_cityExtenInfo.treasureActivity == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == EVERYDAYBUY and (g_cityExtenInfo == nil or g_cityExtenInfo.dailyBuyActivity == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == NEWYEARDAY and (g_cityExtenInfo == nil or g_cityExtenInfo.Activity6120 == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == FEBRUARYNEWYEAR and (g_cityExtenInfo == nil or g_cityExtenInfo.activityLabel10 == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == DOLLMACHINE and (g_cityExtenInfo == nil or g_cityExtenInfo.activity7010 == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == ACTIVITYLIMITLOGIN and (g_cityExtenInfo == nil or g_cityExtenInfo.activity7013 == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == ACTIVITYNEWRECHARGE and (g_cityExtenInfo == nil or g_cityExtenInfo.activity7012 == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == REBACKACTIVITY and (g_cityExtenInfo == nil or g_cityExtenInfo.activity7011 == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == FIGHT_ACTIVITY and (g_cityExtenInfo == nil or g_cityExtenInfo.activity7018 == nil or g_cityExtenInfo.activity7018 == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    for i = 1, #self.m_tBtnsInfo do
        if self.m_tBtnsInfo[i].buttonId == ISLAND_UP_FLOWER_RANK and (g_cityExtenInfo == nil or g_cityExtenInfo.Activity5019 == 0) then
            table.remove(self.m_tBtnsInfo, i)
            break 
        end
    end
    ...
    local tempIndex = 1 
    while tempIndex <= #self.m_tBtnsInfo do
        for i = tempIndex, #self.m_tBtnsInfo do
            local hide_lv = GDatatab_button_info["id_" .. self.m_tBtnsInfo[i].buttonId].hide_lv
