...
function WndCityBottomBar:onEnter(element)
    ...
    Protocol:reg( Protocol.MAIN_MINING, Protocol.MINING_GetRelicList, "ProtocolProcessorDigGem:send_MINING_GetRelicList_ErrorProcess", "is" )
    ...
    --小游戏按钮 华为包不显示小游戏
    local btnSmallGame = GetElement(self.m_root,"btnSmallGame_WndBottomBar",WZUIButton)
    btnSmallGame:setVisible(false)
    --华为包:"com.wyd.hero.dandandao.huawei" 苹果渠道:1116 这些不显示小游戏
    if WGameCmUtil:GetBundleIdentifier() ~= "com.wyd.hero.dandandao.huawei" and ProjConfig.CHANNEL_ID ~= 1116 then
        btnSmallGame:setVisible(true)
    end

    GlobalGame:getGameEventDispathcer():Add(DownloadEvent.DownloadFinish,self._downloadFinish,self)
    self:_setStaticText()
    --岛主按钮
    self:updateIslandOwnerBtn()
...
function WndCityBottomBar:onEnterTransitionDidFinish(element)
    WZLog("WndCityBottomBar:onEnterTransitionDidFinish", self.m_nCountdownTime, CacheCenter:getGameParam()["roleCancleDelayTime"])
end

--@brief    计算倒计时时间:角色删除限时 - (服务器当前时间 - 角色注销时间)
function WndCityBottomBar:caculateCountdownTime()
    ...
    --WZLog("sun---WndCityBottomBar:caculateCountdownTime", result)
    return result
end

--@brief    判断是否显示删除角色倒计时按钮
function WndCityBottomBar:showBtnDelRole()
    WZLog("WndCityBottomBar:showBtnDelRole", self.m_nCountdownTime)
    --判断玩家角色是否处在注销状态，是则显示删除角色图标并创建倒计时，否则什么都不干，隐藏图标
    local btnDelRole = GetElement(self.m_root, "btnDelRole_WndBottomBar", WZUIButton)
    --local txtCountdown = GetElement(self.m_root, "txtCountdown_WndBottomBar", WZUILabelTTF)
    self.m_nCountdownTime = 0--7 * 24 * 3600
    if GlobalGame.g_tPlayerInfo.nUnregisterTime and GlobalGame.g_tPlayerInfo.nUnregisterTime > 0 then
        self.m_nCountdownTime = self:caculateCountdownTime()
    end
    local isCancel = false--是否注销
    if GlobalGame.g_tPlayerInfo.nPlayerStatus and GlobalGame.g_tPlayerInfo.nPlayerStatus == 2 and self.m_nCountdownTime > 0 then
        isCancel = true
    end
    if btnDelRole then
        btnDelRole:disableSchedule()
        if isCancel then
            btnDelRole:setVisible(true)
            btnDelRole:enableSchedule("scheduleDelRole", 1)
        else
            btnDelRole:setVisible(false)
        end
    end
    if isCancel and GlobalGame.g_bIsShowDelRoleTipsInMainCity == false then
        GlobalGame.g_bIsShowDelRoleTipsInMainCity = true
        local bIsOnlyOneButton = false
        MsgBoxManager:showWndConfirmBoxWithOtherWidget(LocalStrings.DELETEROLE_TEXT6, MSGBOXLEVEL_HIGH, nil,bIsOnlyOneButton, nil, self.m_nCountdownTime)
    end
end

--@brief    删除角色倒计时计时器
function WndCityBottomBar:scheduleDelRole(element)
    --self.m_nCountdownTime = self.m_nCountdownTime - 1
    self.m_nCountdownTime = self:caculateCountdownTime()
    if self.m_nCountdownTime > 0 then
        local txtCountdown = GetElement(self.m_root, "txtCountdown_WndBottomBar", WZUILabelTTF)
        if txtCountdown then
            txtCountdown:setText(returnToTimeFormat(self.m_nCountdownTime))
        end
    else
        local btnDelRole = GetElement(self.m_root, "btnDelRole_WndBottomBar", WZUIButton)
        if btnDelRole then
...
--@brief	删除多余的资源
function WndCityBottomBar:finishCallFunc(element)
    WZLog("WndCityBottomBar:finishCallFunc")
    self:_update() 
...
function WndCityBottomBar:_update()
    ...
    for i,v in ipairs(self.m_tBtnsInfo) do
		...
		if btnIndex[v.buttonId] ~= nil then
            ...
            if btnIndex[v.buttonId] == "Pet" then
                if SceneCity:checkIconButtonOpen(v) ~= true and checkbuttonChannel(v.buttonChannel) then
                    isOpen = true
                end
            end
            ...
            if btnIndex[v.buttonId] == "Pet" then
                ...
                if level < 11 then
                    local conAll = GetElement(self.m_root,"conAll_WndBottomBar",WZUIContainer)
                    local layerLocal = conBtn:getParent()
                    local posx, posy = conBtn:getPositionX(), conBtn:getPositionY()
                    local pos = GlobalMethod:ccp(posx, posy)
                    local pos1 = layerLocal:convertToWorldSpace(pos)
                    local pos2 = conAll:convertToNodeSpace(pos1)
                    local conTips = WZUIContainer:create()
                    conTips:setShowAll(true)
                    conTips:setTouchEnable(false)
                    conTips:setAbsContentSize(GlobalMethod:CCSize(200,60))
                    conTips:setUseAbsSize(true)
                    conTips:setAnchorPoint(GlobalMethod:ccp(0.5,0))
                    conTips:setAbsPosition(GlobalMethod:ccp(pos2.x,pos2.y+conBtn:getContentSize().height+10))
                    conTips:setUseAbsCoordinate(true)
                    conAll:addChild(conTips)
                    local imgbg = WZUI9Image:create()
                    imgbg:setFile("ui/teach/dialog_box_4.png")
                    conTips:addChild(imgbg)
                    local txt1 = WZUILabelTTF:create()
                    txt1:setRelativePosition(GlobalMethod:ccp(0.5,0.7))
                    txt1:setColor(ccc3(127,70,26))
                    txt1:setFontSize(20)
                    txt1:setText(LocalStrings.GUIDE_TEXT2)
                    conTips:addChild(txt1)
                    local txt2 = WZUILabelTTF:create()
                    txt2:setRelativePosition(GlobalMethod:ccp(0.5,0.3))
                    txt2:setColor(ccc3(127,70,26))
                    txt2:setFontSize(20)
                    txt2:setText(LocalStrings.GUIDE_TEXT3)
                    conTips:addChild(txt2)
...
--    self:_updateExtend()
    ...
    for i,v in ipairs(self.m_tLeftBtnsInfo) do
        ...
        elseif conBtn and isOpen then
            if ISLAND_NPC_TEACHER == v.buttonId then
                conBtn:setVisible(false)
            else
                conBtn:setVisible(true)
            end
            ...
            -- 1.6.4:邮件-好友-设置放到右上角
            if v.buttonId == ISLAND_LEFT_MAIL or v.buttonId == ISLAND_LEFT_FRIEND or v.buttonId == ISLAND_LEFT_SETTING then
            else                
                id =id +1
                local offsetY = 0.027 + offset * (id-1)
            --    local offsetY = 0.963 - offset * (id - 1)
...
function WndCityBottomBar:onClickMore(element)
    ...
    if isFinish42 ~= true and finishStep42 >= 0 and CacheCenter:getPlayerInfo().level == 24 then
...
--@brief    点击小游戏按钮后的响应方法
function WndCityBottomBar:onClickSmallGame(element)
    WndOwnCity:onClickSmallGame(element)
end
...
--@brief    点击师徒按钮后的响应方法
function WndCityBottomBar:onClickMaster(element)
    WZLog("WndCityBottomBar:onClickMaster")
    SoundManager:playEffectSound(SoundDefine.E_S_BUILDING_BTN)

    JumpByUIId(93)
end

--@brief    点击删除角色按钮后的响应方法
--@param    element:按钮的UI节点引用
--@note 在这里做相应的按钮相应事件
function WndCityBottomBar:onClickDelRole(element)
    WZLog("WndCityBottomBar:onClickDelRole")
    SoundManager:playEffectSound(SoundDefine.E_S_BUILDING_BTN)
    --弹出删除角色信息框
    local bIsOnlyOneButton = false
    local countdownTime = 0--7 * 24 * 3600
    if GlobalGame.g_tPlayerInfo.nPlayerStatus and GlobalGame.g_tPlayerInfo.nPlayerStatus == 2 and GlobalGame.g_tPlayerInfo.nUnregisterTime and GlobalGame.g_tPlayerInfo.nUnregisterTime > 0 then
...
function WndCityBottomBar:endMoveVerticalBar(sender, levelUp, isTrailerAnim)
    ...
    elseif isFinish19 ~= true and CheckButtonOpen(28,false) then
        ...
        tryTeach = true
    elseif isFinish43 ~= true and finishStep43 >= 0 and (CacheCenter:getPlayerInfo().level == 23 or TeachGroup1.ISTEACHMODE) then
...
function WndCityBottomBar:onClickTask(element)
    ...
    TeachGroup1:endTeachStep({3,2},{7,4},{8,6},{18,2},{20,7},{26,10},{41,8})
    SoundManager:playEffectSound(SoundDefine.E_S_BUILDING_BTN)
...
--@brief    点击任务跳转或领取任务奖励
function WndCityBottomBar:onClickTaskJump(element)
    ...
    WZLog("WndCityBottomBar:onClickTaskJump", nTaskId)
    ...
    if self.m_curTaskData.nTaskStatus == TASKSTATUS_DOING then
        ...
        if SceneCity:weatherInGetReward() then return end 
        SceneCity:setGetRewardLimit(true)
        local tData = {}
        ...
        WZLog("CellTaskListItem:onCommitEvent GetReward", nTaskId)
        SceneCity.m_nLoadingBoxId = MsgBoxManager:showLoadingBox()
...
function WndCityBottomBar:showRemainsBtn(boolShow)
    WZLog("WndCityBottomBar:showRemainsBtn")
    GetElement(self.m_root,"btnRemains_WndBottomBar",WZUIButton):setVisible(boolShow)
end

--@brief    点击遗迹按钮后的响应方法
--@param    element:按钮的UI节点引用
--@note 在这里做相应的按钮相应事件
function WndCityBottomBar:onClickRemains(element)
    WZLog("WndCityBottomBar:onClickRemains")
    SoundManager:playEffectSound(SoundDefine.E_S_BUILDING_BTN)

    local wndRemainsChallenge = WndRemainsChallenge:createElement()
    WindowManager:addWindow(wndRemainsChallenge,WndRemainsChallenge,nil,nil,nil,true)
end

--@brief    点击岛主复仇按钮回调
function WndCityBottomBar:updateIslandOwnerBtn()
    local btnIslandOwner = GetElement(self.m_root,"btnIslandOwner_WndBottomBar",WZUIButton)
    self:_scheduleIslandOwner(btnIslandOwner)
    btnIslandOwner:enableSchedule("_scheduleIslandOwner",1)
end

--@brief    岛主按钮计时器
function WndCityBottomBar:_scheduleIslandOwner(element)
    if self.m_root == nil then
        return
    end
    local btnIslandOwner = GetElement(self.m_root,"btnIslandOwner_WndBottomBar",WZUIButton)

    local tIslandOwnerData = CacheCenter:getIslandOwnerData()
    if tIslandOwnerData == nil then
        btnIslandOwner:disableSchedule()
        btnIslandOwner:setVisible(false)
    else
        --把过期的复仇去掉
        for i=#tIslandOwnerData,1,-1 do
            if tIslandOwnerData[i].leaveTime <= SystemTime:getServerTime() then
                table.remove(tIslandOwnerData,i)
            end
        end

        if #tIslandOwnerData == 0 then
            btnIslandOwner:disableSchedule()
            btnIslandOwner:setVisible(false)
        else
            btnIslandOwner:setVisible(true)
        end
    end
end

--@brief    点击岛主复仇按钮后的响应方法
function WndCityBottomBar:onClickIslandOwner(element)
    WZLog("WndCityBottomBar:onClickIslandOwner")
    SoundManager:playEffectSound(SoundDefine.E_S_BUILDING_BTN)

    local tIslandOwnerData = CacheCenter:getIslandOwnerData()
    if tIslandOwnerData == nil or tIslandOwnerData == {} then
        return
    end

    local wndIslandOwnerRevenge = WndIslandOwnerRevenge:createElement()
    WindowManager:addWindow(wndIslandOwnerRevenge,WndIslandOwnerRevenge,nil,nil,nil,true)
end

--@brief    点击职业入口按钮回调
function WndCityBottomBar:onClickProfession(element)
    -- body
    SoundManager:playEffectSound(SoundDefine.E_S_BUILDING_BTN)
...
--@brief    点击注魂入口按钮回调
function WndCityBottomBar:onClickCastSoul(element)
    -- body
    SoundManager:playEffectSound(SoundDefine.E_S_BUILDING_BTN)

    if CheckButtonOpen(ISLAND_UP_DRESS_CASTSOUL) then
        WndDressCastSoul:showInterface()
    end
end

--@brief    点击夫妻入口按钮回调
function WndCityBottomBar:onClickCouple(element)
    SoundManager:playEffectSound(SoundDefine.E_S_BUILDING_BTN)
...
--@brief    点击联盟入口按钮回调
function WndCityBottomBar:onClickUnion(element)
    SoundManager:playEffectSound(SoundDefine.E_S_BUILDING_BTN)

    if CheckButtonOpen(ISLAND_RIGHT_UNION) then
        WndUnionList:onJumpToCommunity()
    end
end

--@brief    展示当前的任务
function WndCityBottomBar:showCurTask()
    -- body
    if self.m_root == nil then return end 

    local ftxtTaskDesc = GetElement(self.m_root, "ftxtTaskDesc_WndBottomBar", WZUIFreeTextBox)
    local formatString = [[<T C="255,236,193" S="18" P="1">%s</T><T C="255,236,193" S="18" P="1">(%d/%d)</T>]]
    ...
    if taskList and taskList[1] then 
        ...
        if taskList[1].nTaskStatus == TASKSTATUS_TOSUBMIT then 
            formatString = [[<T C="255,236,193" S="18" P="1">%s</T><T C="99,255,95" S="18" P="1">(%s)</T>]]
        ...
        formatString = [[<T C="255,236,193" S="18" P="1">%s</T>]]
        ftxtTaskDesc:setShowText(string.format(formatString, LocalStrings.TASK_NEWTEXT1))
    end
end

--@brief    设置快捷任务栏的可见与否
function WndCityBottomBar:setQuickTaskVisible(nValue)
    -- body
    if self.m_root == nil then return end 

    if nValue == 1 then 
        GetElement(self.m_root, "btnTask2_WndBottomBar", WZUIButton):setVisible(false)
    else
        GetElement(self.m_root, "btnTask2_WndBottomBar", WZUIButton):setVisible(true)
    end
end

--@brief    点击红包回调
function WndCityBottomBar:onClickRedPack(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)

    ProtocolProcessorGlobal:send_CHAT_GrabRedEnvelope(-1,g_RedPackChannel)
    --点击过，隐藏快捷领取按钮
...
--@brief    设置红包按钮是否可见
function WndCityBottomBar:setRedPackBtnVisible()
    if self.m_root == nil then return end 

    local btnRedPack = GetElement(self.m_root, "btnRedPack_WndBottomBar", WZUIButton)
    btnRedPack:setVisible(g_QuickRedPackState or g_QuickRedPackState2)
end
-------------------------------------公有方法模块End----------------------------------------
-------------------------------------私有方法模块Begin--------------------------------------
...
--@brief    点击查看后台下载详情按钮后的响应方法
function WndCityBottomBar:onClickDownload(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)

    WndBossHall:showInterface()
end

--@brief    设置边玩边下进度显示状态
function WndCityBottomBar:setDownloadState()
    if self.m_root == nil then return end 

    local platForm =  WZUISystem:getInstance():getPlatformInfo()
    local bIsNeedShow = false 
    if isChannelPC() then
    else
        if platForm ~= 3 then 
        ...
        if ProjConfig.LANGUAGE == "vn" then 
            if platForm == 1 then --ios
                if ProjConfig.INSTALLVERSION <= "3.8.7" then 
                    bIsNeedShow = false 
                end
            elseif platForm == 2 then --android
                if ProjConfig.INSTALLVERSION < "2.2.8" then 
                    bIsNeedShow = false 
                end
            end
        end
    ...
    if bIsNeedShow then 
        GetElement(self.m_root, "btnDownload_WndBottomBar", WZUIButton):setVisible(true)
        self:_setDownloadPro()
    end
end

--@brief    点击下载资源回调
function WndCityBottomBar:clickDownloadCallback()
    if self.m_conDownloadDia then  
        self.m_conDownloadDia:removeFromParentAndCleanup(true)
    end
end
...
function WndCityBottomBar:_setNetSignal()
    ...
    conNetSignal:setRelativePosition(GlobalMethod:ccp(0.96,0.5))
    if IsIphoneX() then
        conNetSignal:setRelativePosition(GlobalMethod:ccp(0.9,0.5))
    end
    CellNetSignal:showInterface(conNetSignal, nil, 1)
end

--@brief    发送点击展开导航栏事件
function WndCityBottomBar:_postClickFunctionEvent()
    -- body
    local level = CacheCenter:getPlayerInfo().level
    if level == 3 or level == 4 or level == 8 then 
        local eventKey = PostPlayerEvent["event_ClickFunction" .. level]
        if eventKey then 
...
--@brief    设置静态文本
function WndCityBottomBar:_setStaticText()
    GetElement(self.m_root, "txtUnion_WndCityBottomBar", WZUILabelTTF):setText(LocalStrings.UNION_TEXT1[1])
    GetElement(self.m_root, "txtUnionSel_WndCityBottomBar", WZUILabelTTF):setText(LocalStrings.UNION_TEXT1[1])
    GetElement(self.m_root, "txtDownBtn_WndCityBottomBar", WZUILabelTTF):setText(LocalStrings.OTHER_UI_TEXT[2])
    self.m_nodeBtnDown = GetElement(self.m_root, "btnDownload_WndBottomBar", WZUIButton)
end

--@brief    设置游戏内下载进度
function WndCityBottomBar:_setDownloadPro()
    local btnDownload = GetElement(self.m_root, "btnDownload_WndBottomBar", WZUIButton)
    if not btnDownload:isVisible() then return end 
    
    local prgDownload = GetElement(self.m_root, "prgDownload_WndCityBottomBar", WZUIProgress)
    local totalSize, downloadSize = DownloadManager:getTotalResSize()
    local percent = math.floor(downloadSize/totalSize * 100)
    if prgDownload then 
        prgDownload:setPercentage(percent)
    end
    local ftxtPro = GetElement(self.m_root, "ftxtPro_WndCityBottomBar", WZUIFreeTextBox)
    if ftxtPro then 
        local strFormat = [[<A IMG="ui/common_num/bt_text_xzjd_0-9.png" Z="0.8" W="8" H="12" CHAR="0">%d</A><I Z="0.6" P="1">ui/city/beta/bt_text_xzjd_bfb.png</I>]]
        local strContent = string.format(strFormat, percent)
        ftxtPro:setShowText(strContent)
    end
...
--@brief    显示需要下载提醒
function WndCityBottomBar:_showNeedDownloadAtt(dt)
    if self.m_tScene == SceneHVHotSpring then return end 
    if self.m_nodeBtnDown == nil or not self.m_nodeBtnDown:isVisible() then return end 

    local platForm =  WZUISystem:getInstance():getPlatformInfo()
    local bIsShowAtt = true 
    if platForm == 1 or platForm == 2 then 
        if platForm == 1 then --ios
            if (ProjConfig.INSTALLVERSION <= "2.5.3" and (ProjConfig.CHANNEL_ID == 1112 or ProjConfig.CHANNEL_ID == 51)) or (ProjConfig.INSTALLVERSION <= "4.1.3" and ProjConfig.CHANNEL_ID == 1116) then 
                bIsShowAtt = false 
            end
        end           
        if bIsShowAtt and GlobalGame.g_bIsDownloadingResCheckAll == false and ProjConfig.DressAniDownloadConfig_All and ProjConfig.DressAniDownloadConfig_All ~= "" then 

            if self.m_conDownloadDia == nil then 
                local conLeftUp = GetElement(self.m_root, "conLeftUp_WndBottomBar", WZUIContainer)
                self.m_conDownloadDia = self:_createAttDia()
                conLeftUp:addChild(self.m_conDownloadDia)
                self.m_conDownloadDia:setRelativePosition(GlobalMethod:ccp(0.1,0.58))
            end

            if self.m_conDownloadDia then 
                self.m_nAttDiaShowTime = self.m_nAttDiaShowTime + dt
                if self.m_nAttDiaShowTime >= 4 then
                    self.m_nAttDiaShowTime = 0
                    self.m_conDownloadDia:setVisible(true)
                elseif self.m_nAttDiaShowTime >= 2 then 
                    self.m_conDownloadDia:setVisible(false)
                end
            end
        end
    end
...
--@brief    创建提示气泡
function WndCityBottomBar:_createAttDia()
    local con = WZUIContainer:create()
    con:setAbsContentSize(GlobalMethod:CCSize(116, 87))
    con:setAnchorPoint(GlobalMethod:ccp(0.5, 0))
    con:setUseAbsSize(true)

    local img = createImage("ui/common/commom_qipao_2.png", nil, nil, true, GlobalMethod:ccp(0.5, 0.5))
    img:setOpacity(200)
    img:setTouchEnable(false)
    con:addChild(img)

    local txtAtt = createLabel(LocalStrings.OTHER_UI_TEXT[9], GlobalMethod:ccp(0.5, 0.77), nil, 18, GlobalMethod:ccc3(127,70,26))
    txtAtt:setDimensions(GlobalMethod:CCSize(100,0))
...
--@brief    iphoneX适配
function WndCityBottomBar:_adaptIphoneX()
    if IsIphoneX() then
        self.m_nodeBtnDown:setRelativePosition(GlobalMethod:ccp(0.05,0.524544))
    end
end
-------------------------------------私有方法模块End----------------------------------------
...
function WndCityBottomBar:_adaptLanguage_vn(  )
    ...
    local ftxtTaskDesc = GetElement(self.m_root, "ftxtTaskDesc_WndBottomBar", WZUIFreeTextBox)
    ftxtTaskDesc:setScale(0.7)
    ftxtTaskDesc:setMaxWidth(280)
end
...
function WndCityBottomBar:_adaptLanguage_ug(  )
    local txtTask = GetElement(self.m_root,"txtTask_WndBottomBar",WZUILabelTTF)
    txtTask:setScale(0.6)
    txtTask:setDimensions(GlobalMethod:CCSize(233))
    txtTask:setRelativePosition(GlobalMethod:ccp(0.5,0.5))
