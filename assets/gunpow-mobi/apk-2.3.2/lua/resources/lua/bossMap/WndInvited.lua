...
function WndInvited:onRefusalClick()
	...
	if self.m_nInterface and self.m_nPlayerId then
		ProtocolProcessorGlobal:send_PLAYER2_ActToOther(1, self.m_nPlayerId)
	end
	--停止定时器
...
function WndInvited:onSureClick()
	...
	if self.m_sPassword == "" or self.m_sPassword == nil then
		if self.m_nRoomChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_WTB or self.m_nRoomChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_FQZB then
	...
	if self.m_nRoomChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_ZLS then
		if IsShowZlsPunishTime(true) then
			WindowManager:removeWindow( self.m_root , WndInvited , true )
			return
		end
	end
	...
	if self.m_nRoomChannel and self.m_nRoomChannel > 0  then
		if self.m_nNumMode then
			self.m_tCallbackFun( self.m_tCell , self.m_nId , self.m_nRoomChannel, self.m_nNumMode, self.m_sPassword, self.mapId, self.m_sDesc , self.m_nBattleId,self.m_nAssistFight )
		else
...
function WndInvited:bContinue()
	...
	if SceneRoom.m_root == nil and SceneBossRoom.m_root == nil and SceneBattle.m_root == nil and SceneBattleLoading.m_root == nil and 
	   GlobalGame.g_bIfInBattle == false and WndTeachOpenModule.m_root == nil and WndTeachTalk.m_root == nil and SceneKingMain.m_root == nil and 
	   not WndChat.m_bRecording and not SceneHall:getMatchState() and not WndStrengthen.m_root and not SceneWeddingChurch.m_root and 
	   not SceneLeagueRoom.m_root and not SceneAthMelee.m_root and SceneMarryCopy.m_root == nil and SceneTabooBattle.m_root == nil and 
	   SceneGuildWarRoom.m_root == nil and SceneWorldTeamBossRoom.m_root == nil and SceneCoupleHegemonyRoom.m_root == nil and WndDoubleTowerRoom.m_root == nil and WndAnswerSurvey.m_root == nil and
	   WndSingleCopyInfo.m_root == nil then
       return true
    end
    return false
end
...
function WndInvited:showInterface(element, callbackfun, nId, password ,mapId, desc, playerName, nBattleId, Schedule, roomChannel, assistFight, interfaceId, playerId, numMode)
	if self:bContinue() then
        ...
        if G_Corps_INVITE == 1 and element == WndLeagueTeamDetail then
        	return
        end
        ...
        --战略赛正在匹配的时候，不弹被邀请框
        if ScenePvpStrategic.m_root and ScenePvpStrategic:getMatchState() then
        	return 
        end
		...
		if roomChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_ZLS then
			IsShowZlsPunishTime(false)
		end
		--改变文本
		if interfaceId == 1 or interfaceId == 2 or interfaceId == 3 or interfaceId == 4 or interfaceId == 5 or interfaceId == 6 then
			self.m_nInterface = interfaceId
			self.m_nPlayerId = playerId
			local info = CacheCenter:getPlayerInfo()
			local str_1 = ""
			if interfaceId == 6 then
				str_1 = LocalStrings.ROLEINFO_MATE
			else
				if info and info.masterId == playerId then
					str_1 = LocalStrings.MASTER
				else
					local bIsPupil = CacheCenter:judgeIsPupilById(playerId)
					local bIsFriend = CacheCenter:judgeIsContainsById( playerId )
					if bIsPupil then 
						str_1 = LocalStrings.APPRENTICE
					elseif bIsFriend then 
						str_1 = LocalStrings.FRIEND
					else
						str_1 = LocalStrings.PLAYER
					end
				end
			end
			local str_title = {LocalStrings.INN6, LocalStrings.PVP_HALL_9, LocalStrings.DOUBLETOWER_TEXT1, LocalStrings.OPTIMIZE_TEXT89, LocalStrings.DOUBLETOWER_TEXT1, LocalStrings.COUPLE_HEGEMONY_TEXT1}
			local temp_desc = string.format(LocalStrings.OPTIMIZE_TEXT88, str_1, playerName, str_title[interfaceId])
			self:_showInviteDesc( temp_desc )
		else
			self:_showInviteDesc( desc )
			self.m_nAssistFight = assistFight
			if roomChannel and roomChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_DTF then
				self:_showAssistFightTip(false)
			elseif roomChannel and roomChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_DZTZ then
				self:_showAssistFightTip(false)
			else
		...
		--@brief	设置回调函数
		self:_setCallBackFun(element, callbackfun, nId, password, mapId, nBattleID, roomChannel, numMode)
...
function WndInvited:_showAssistFightTip(bAssistFight)
	...
	local imgHelpIcon = GetElement(self.m_root, "imgHelpIcon_WndInvited", WZUIImage)
	imgHelpIcon:setVisible(false)
	if bAssistFight == 1 then
...
function WndInvited:_setCallBackFun(element, callbackfun, nId, password, mapId, nBattleID, roomChannel, numMode)
	if self.m_root == nil then
		return
	end
