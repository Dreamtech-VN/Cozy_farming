...
function WndBuyActivity:getReturnData()
    ...
    --获取配置的最大等级的数据
    local maxVipLevel = 0
    local maxVipConfig = nil 
    for i, value in pairs(GDatatab_consume_return_diamond) do
        if value.vip_level >= maxVipLevel then
            maxVipLevel = value.vip_level
            maxVipConfig = value
        end
    end

    if vipLevel > maxVipLevel and maxVipConfig ~= nil then 
        if self.m_tBaseData.nType == 0 then
            return maxVipConfig.plutus_cat_return
        elseif self.m_tBaseData.nType == 1 then
            return maxVipConfig.buy_vigor_return
        end
    end
