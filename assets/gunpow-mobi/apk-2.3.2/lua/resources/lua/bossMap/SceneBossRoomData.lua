...
function SceneBossRoom:_init()
    ...
    self.click = false
    self.m_nSelVipLevel = 0
    self.m_nSelPlayerSeatIndex = 0
...
function SceneBossRoom:_unInit()
    ...
    self.m_tCellDressSuit = nil         --多套时装的cell
    self.click = false
    self.m_nSelVipLevel = nil
    self.m_nSelPlayerSeatIndex = nil
    self.m_sRoleFootSpine = nil
end
...
function SceneBossRoom:setData(roomId, passWord, roomName, playerNumMode, mapId, wnersId, playerNum, seatUsed, playerId, serverId,
    ...
    self.m_tData = {roomId = roomId,battleMode = GlobalGame.g_tBattleMode.BATTLE_MODE_FB,playerStar = playerStar ,fighting = playerFighting,
                    serviceId=serviceId, roomChannel=GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_ZF, playerNumMode=playerNumMode, mapId=mapId, 
                    wnersId=wnersId, playerNum=playerNum, seatUsed=seatUsed, playerId=playerId, playerName=playerName, playerLevel=playerLevel, 
                    playerReady=playerReady, playerSex=playerSex, playerEquipment=playerEquipment, playerWeaponLevel=playerWeaponLevel, vipLevel=vipLevel, 
                    playerTitle=player_title,roomName=roomName,passWord=passWord,pet=pet,extranInfo=extranInfo,serviceId=serviceId,headColors=playerHeadColour,
                    bodyColors=playerBodyColour,mentoringStr=mentoringStr,coupleStr=coupleStr,chumStr= chumStr,coupleNum=coupleNum,chumNum=chumNum,
                    mentoringNum=mentoringNum,qualifyingLevel=matchLevel,matchscore = matchscore,joinTimes=joinTimes,winTimes=winTimes,continuousWinTimes=continuousWinTimes,
                    assist=assist, assistTimesState = assistTimesState, useMountsMes = useMountsMes, professionId = professionId, topMapId = topMapId, roomAssist = roomAssist, 
                    openStatus = openStatus}
    WZLog("SceneBossRoom:setData", Serialize(useMountsMes), Serialize(topMapId), self.m_tData.roomName)
...
function SceneBossRoom:receiveEnterRoomOk(roomId, passWord, roomName, playerNumMode, mapId, wnersId, playerNum, seatUsed, playerId,serverId, playerName, playerLevel, playerReady, playerSex, playerEquipment, playerEquipmentLevel, vipLevel, player_title, qualifyingLevel, zsleve, playerStar, playerFighting, pet, extranInfo, playerHeadColour, playerBodyColour, mentoringStr, coupleStr, chumStr, coupleNum, chumNum, mentoringNum, matchLevel, matchscore, joinTimes, winTimes, continuousWinTimes, serviceId, assist, assistTimesState, useMountsMes, professionId, topMapId, roomAssist, openStatus)
    ...
    self:setData(roomId, passWord, roomName, playerNumMode, mapId, wnersId, playerNum, seatUsed, playerId,serverId,
            ...
            mentoringStr,coupleStr,chumStr,coupleNum,chumNum,mentoringNum,matchLevel,matchscore,joinTimes,winTimes,continuousWinTimes,serviceId,assist, assistTimesState, useMountsMes, professionId, topMapId, roomAssist, openStatus)
...
--@brief    玩家被邀请进入副本房间
function SceneBossRoom:beInvited(roomId , playerName, mapId, password, roomChannel, assist, interfaceId, playerId)
    ...
    local diffName = {LocalStrings.COMMON,LocalStrings.DIFFICULTY,LocalStrings.HELL, LocalStrings.MULCOPY_TEXT2}
    local desc = string.format(LocalStrings.ROOM_BEINVITED_4,playerName,data.map_name,diffName[diff])
    if assist == 0 then --不是助战玩家
...
function SceneBossRoom:showChat(txtMsg,playerId,bubbleId)
    ...
    if seatIndex > 0 then
        ...
        if conPlayer ~= nil then
            ...
            WZLog("ps = ",ps.x,ps.y)
            tPS = {x = 150, y = 360}
            local cellChatBubble = conSeat:getChildByTag(seatIndex+1110)
            if not cellChatBubble then
                local cellChatBubblenode,luaObject  = CellChatBubble:showChatBubble(conSeat, tPS, false)
...
--@brief    更新玩家的副本数据
function SceneBossRoom:updateData()
    if self.m_root == nil then return end 
...
--@brief    更新技能方案名字
function SceneBossRoom:updateSkillSuitData()
    -- body
    if self.m_root == nil then return end 
...
--@brief    获取是否可以踢出房间
function SceneBossRoom:getClickSeat()
    -- body
    if SceneBossRoom.m_root == nil then return nil end

    return self.m_bCanClickSeat
end

--@brief    根据结果，重新设置选中的难度
function SceneBossRoom:resetSelDiffercenty()
    if self.m_root == nil then return end 
...
function SceneBossRoom:addTop()
    ...
    tcell:setTopData("ui/common/common_icon_zdfb.png",SceneBossRoom,SceneBossRoom.onClose,true,true,true,"SceneBossRoom",{scale = 0.75},nil,false)

    tcell:setWifiSignalVisible(false)
    tcell.goldCellInfo.cell:setVisible(false)
    tcell:setTopType()
    tcell:setImgVnVisible(false)
end
...
--@brief    获取房主等级
function SceneBossRoom:getOwnerLevel()
    -- body
    local nLevel = 1
    for i = 1, self.m_tData.playerNum do
        if self.m_tData.playerId[i] == self.m_nHomeowner then 
            nLevel = self.m_tData.playerLevel[i]
            break 
        end
    end
...
--@brief    获取我的助战状态
function SceneBossRoom:getMyAssist()
    -- body
    local assist = 0 

    for i = 1, self.m_tData.playerNum do
        if self.m_tData.playerId[i] == CacheCenter:getPlayerInfo().id then 
            assist = self.m_tData.assist[i]
            break 
        end
    end
...
--@brief    获取我的赏金状态
function SceneBossRoom:getMyAssistTimesState()
    -- body
    local assistTimesState = 0 

    for i = 1, self.m_tData.playerNum do
        if self.m_tData.playerId[i] == CacheCenter:getPlayerInfo().id then 
            assistTimesState = self.m_tData.assistTimesState[i]
            break 
        end
    end

    return assistTimesState
end

--@brief    获取角色动作,坐骑id
--@param    nType : 0等待；1跑动
function SceneBossRoom:getPlayerAction(nType, index)
    -- body
    local name = "wait0"
    local mountId = 0
    if nType == 0 then 
        local tMountInfo = json.decode(self.m_tData.useMountsMes[index])
        if tMountInfo ~= nil and tMountInfo.mountsId ~= nil then 
            mountId = tMountInfo.mountsId
            name = "wait"
        end
    else
        local tMountInfo = json.decode(self.m_tData.useMountsMes[index])
        if tMountInfo ~= nil and tMountInfo.mountsId ~= nil then 
            ...
            if basicInfo and basicInfo.sub_type == 1 then
                name = "wait"
            elseif basicInfo and basicInfo.sub_type == 2 then
                name = "walk2"
            elseif basicInfo and basicInfo.sub_type == 3 then
                name = "walk3"
            elseif basicInfo and basicInfo.sub_type == 4 then
                name = "walk4"
            else
                name = "walk"
            end
...
--@brief    获取房间等级
function SceneBossRoom:getRoomLevel(playerCount, playerId, playerLevel, playerCamp)
    -- body
    local myCamp = 0
    ...
    local nTotalNum = 0
    for i = 1, playerCount do
        if playerCamp[i] == myCamp then 
            if playerLevel[i] > maxLevel then 
                maxLevel = playerLevel[i]
            end

            nTotalNum = nTotalNum + 1
            totalLevel = totalLevel + playerLevel[i]
        end
    end

    return math.floor(maxLevel/2 + totalLevel/nTotalNum/2)
end
-------------------------------------私有方法模块End----------------------------------------
