...
function WndSingleMapDesc:showInterface(desc,btnInfo, bAddBuyBtn, nType)
...
--@param    #1 desc:规则说明内容
--@param 	bAddBuyBtn : 增加购买纪念币购买按钮
function WndSingleMapDesc:showInterface1(desc, bAddBuyBtn, nType)
	...
	if self.m_nType == 1 then 
		local img9Bg = GetElement(self.m_root, "img9Bg_WndSingleMapDesc", WZUI9Image)
		local img9TitleBg = GetElement(self.m_root, "img9TitleBg_WndSingleMapDesc", WZUI9Image)
		img9Bg:setUseOriginSize(true)
		img9Bg:setFile("ui/specialBg/frame_tc_yyb_xd.png")
		img9TitleBg:setUseOriginSize(true)
		img9TitleBg:setFile("ui/newActivity/title_frame_29.png")
	end
    GetElement(self.m_root, "txtDesc", WZUILabelTTF):setVisible(false)
    GetElement(self.m_root, "txtDesc1", WZUIFreeTextBox):setVisible(true)
    GetElement(self.m_root, "txtDesc1", WZUIFreeTextBox):setShowText(desc)
...
function WndSingleMapDesc:_resetScrollContainerSize()
	...
	if self.m_bIsAddBuyBtn then 
		...
		if conRuleDetail then 
			conRuleDetail:setRelativeSize(GlobalMethod:CCSize(0.9, 0.57))
...
function WndSingleMapDesc:_adaptLanguage_ug( )
    for i = 1, 3 do
    	GetElement(self.m_root, "txtConfirm"..i.."_WndSingleMapDesc", WZUILabelTTF):setScale(0.6)
    end
end

function WndSingleMapDesc:_adaptLanguage_vn( )
    GetElement(self.m_root,"txtDesc1",WZUIFreeTextBox):setMaxWidth(440)
end
