...
function WndLeagueTeamDetail:onEnterTransitionDidFinish(element)
	...
	WZLog("WndLeagueTeamDetail:onEnterTransitionDidFinish", directorSize.width/1136)
	local width = directorSize.width
	local height = directorSize.height
	local tempWidth = width/(height/640)
	if width/height > 1.5 then
		GetElement(self.m_root,"conSpeake",WZUIContainer):setScaleX(tempWidth/1136)
		GetElement(self.m_root,"btnSpeaker_WndLeagueTeamDetail",WZUIButton):setScaleX((1136/tempWidth)*0.8)
		GetElement(self.m_root,"btnMic_WndLeagueTeamDetail",WZUIButton):setScaleX((1136/tempWidth)*0.8)
	end
...
function WndLeagueTeamDetail:onStart(element)
	...
	if self.m_bGameStart == false then
		if self.m_bFighting == true then 
			if not self:_judgeCanFight() then 
				return 
			end
	...
	if not self:_judgeCanFight() then return end 
	--队伍人数不足
...
function WndLeagueTeamDetail:showRole(position, dataIndex)
	...
	if isMonster == true then
		conPlayer:getAnimNode():setRelativePosition(GlobalMethod:ccp(0.52,0))
		conPlayer:getAnimNode():setRelativePosition(ccp(0.5,0.4))
	end
...
function WndLeagueTeamDetail:update()
	...
	GetElement(self.m_root,"ttfTeamID",WZUILabelTTF):setText(tostring(self.m_tData.teamId))
	--排名
...
function WndLeagueTeamDetail:updateTime()
	...
	if gameState == 1 then
		--海选赛
		local strFail = LocalStrings.LEAGUE113
		if tonumber(SceneLeagueMain.m_tTime.failScore) >= 0 then 
			strFail = strFail .. "+" .. SceneLeagueMain.m_tTime.failScore
		else
			strFail = strFail .. SceneLeagueMain.m_tTime.failScore
		end

		showGameTime = LocalStrings.LEAGUE18..":"..SceneLeagueMain.m_tTime.startTime1.."-"..SceneLeagueMain.m_tTime.endTime1.."\n"..LocalStrings.LEAGUE111..":"..SceneLeagueMain.m_tTime.startTime.."-"..SceneLeagueMain.m_tTime.endTime.."\n"..LocalStrings.LEAGUE112.."+"..SceneLeagueMain.m_tTime.winScore..","..strFail
