...
function WndLeagueTeamDetail:setData(teamId, teamName, photoURL, declaration, playerId, faceId, headId, bodyId, wingId, captain, score, fightNum, winNum, readyPlayerId, watchPlayerId, rank, sex, pet, status, timeMes, openTime, level, name, lastFight, itemId, extraInfo, kictNum, outNum, picStatue, canFight, readyed, fight, viceCaptain, headColor, bodyColor, mentoringStr, coupleStr, chumStr, coupleNum, chumNum, mentoringNum, matchLevel, matchscore, joinTimes, winTimes, continuousWinTimes)
	...
	for i=1,#playerId do
		...
		tempTable.headColor = (headColor and headColor[i]) and headColor[i] or 0 --headColor[i]
		tempTable.bodyColor = (bodyColor and bodyColor[i]) and bodyColor[i] or 0 --bodyColor[i]
		tempTable.matchLevel = (matchLevel and matchLevel[i]) and matchLevel[i] or 0 --matchLevel[i]
		tempTable.matchscore = (matchscore and matchscore[i]) and matchscore[i] or 0 --matchscore[i]
		tempTable.joinTimes = (joinTimes and joinTimes[i]) and joinTimes[i] or 0 --joinTimes[i]
		tempTable.winTimes = (winTimes and winTimes[i]) and winTimes[i] or 0 --winTimes[i]
		tempTable.continuousWinTimes = (continuousWinTimes and continuousWinTimes[i]) and continuousWinTimes[i] or 0--continuousWinTimes[i]
		if outNum ~= nil then
...
--@brief 	比赛资格判断
function WndLeagueTeamDetail:_judgeCanFight()
