...
function WndMarryLetter:_getFileNameAndLetter()
    local fileName = nil
    local txt = nil
    WZLog("WndMarryLetter:_getFileNameAndLetter", self.m_nMarryType)
...
function WndMarryLetter:_showTypeWindow()
    ...
    if self.m_nWindowType == WndMarryLetter.wndType.SEND_LETTER then
        ...
        GetElement(self.m_root,"btnChange_WndMarryLetter",WZUIButton):setVisible(true)
         --显示玩家名字
        ...
        GetElement(self.m_root,"btnChange_WndMarryLetter",WZUIButton):setVisible(false)
        -- 显示玩家名字
...
--@brief  点击更换按钮回调
function WndMarryLetter:onClickChange(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
    WndMarry:showInterface()
end
...
function WndMarryLetter:_adaptLanguage_ug(  )
    local txtContent = GetElement(self.m_root,"txtContent_WndMarryLetter",WZUILabelTTF)
    txtContent:setScale(0.8)
    txtContent:setDimensions(GlobalMethod:CCSize(500))

    local txtNameDesc = GetElement(self.m_root,"txtNameDesc_WndMarryLetter",WZUILabelTTF)
    txtNameDesc:setScale(0.7)
    txtNameDesc:setRelativePosition(GlobalMethod:ccp(0.45,0.42))
    local txtCoupleName = GetElement(self.m_root,"txtCoupleName_WndMarryLetter",WZUILabelTTF)
    txtCoupleName:setScale(0.7)
    txtCoupleName:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    txtCoupleName:setRelativePosition(GlobalMethod:ccp(0.25,0.42))

    local txtProp = GetElement(self.m_root,"txtProp_WndMarryLetter",WZUILabelTTF)
    txtProp:setScale(0.7)
    txtProp:setDimensions(GlobalMethod:CCSize(400))
    txtProp:setRelativePosition(GlobalMethod:ccp(0.5,0.87))
    local txtProposeTips = GetElement(self.m_root,"txtProposeTips_WndMarryLetter",WZUILabelTTF)
    txtProposeTips:setScale(0.7)
    txtProposeTips:setDimensions(GlobalMethod:CCSize(500))

    local txtYourName = GetElement(self.m_root,"txtYourName_WndMarryLetter",WZUILabelTTF)
    txtYourName:setRelativePosition(GlobalMethod:ccp(-0.22,0.416671))
    txtYourName:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    local txtYourNameT = GetElement(self.m_root,"txtYourNameT_WndMarryLetter",WZUILabelTTF)
    txtYourNameT:setRelativePosition(GlobalMethod:ccp(0.65,0.423876))
