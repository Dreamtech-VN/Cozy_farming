...
function WndKidDress:register()
	GlobalGame:getGameEventDispathcer():Add(KidEvent.KidEvent_ChangeSexResult,self._onChildChangeSexResult,self)
end
function WndKidDress:unregister()
	GlobalGame:getGameEventDispathcer():Remove(KidEvent.KidEvent_ChangeSexResult,self._onChildChangeSexResult,self)
end
...
--@brief	点击头像框标签
function WndKidDress:onTab4()
	WZLog("WndKidDress:onTab4")
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--@brief 点击星钻图标回调
function WndKidDress:onClickStarDia(element)
	-- body
...
function WndKidDress:showFighting()
	-- body
    local txtFight = GetElement(self.m_root,"txtFight_WndKidDress",WZUILabelTTF)
	...
	if ftxtFighting and SceneKidHome.m_tKidData then
		local tCurKidData = SceneKidHome.m_tKidData[self.m_nKidIndex]
		ftxtFighting:setShowText(string.format(LocalStrings.FIGHT_POWER1, tCurKidData.fighting))
	end
end
...
function WndKidDress:setTabText()
	...
	local txtTab4 = GetElement(self.m_root, "txtTab4_WndKidDress", WZUILabelTTF)
	local txtTab1Sel = GetElement(self.m_root, "txtTab1Sel_WndKidDress", WZUILabelTTF)
	local txtTab2Sel = GetElement(self.m_root, "txtTab2Sel_WndKidDress", WZUILabelTTF)
	local txtTab3Sel = GetElement(self.m_root, "txtTab3Sel_WndKidDress", WZUILabelTTF)
	local txtTab4Sel = GetElement(self.m_root, "txtTab4Sel_WndKidDress", WZUILabelTTF)

	txtTab1:setText(LocalStrings.HEAD)
	txtTab2:setText(LocalStrings.FACE)
	txtTab3:setText(LocalStrings.BODY)
	txtTab4:setText(LocalStrings.HEAD_EFFECT)
	txtTab1Sel:setText(LocalStrings.HEAD)
	txtTab2Sel:setText(LocalStrings.FACE)
	txtTab3Sel:setText(LocalStrings.BODY)
	txtTab4Sel:setText(LocalStrings.HEAD_EFFECT)
end
...
function WndKidDress:updateDress()
	--body
	--当前部位数量
	local s = {LocalStrings.BAGTIP31,LocalStrings.BAGTIP32,LocalStrings.BAGTIP33, LocalStrings.GOPHERBALL_TEXT1[20]}
	GetElement(self.m_root,"ttf1_WndKidDress",WZUILabelTTF):setText(s[self.m_nCurrentIndex])
	local num = 0
	if not SceneKidHome.m_tKidData then return end

	local tCurKidData = SceneKidHome.m_tKidData[self.m_nKidIndex]
	--获取当前性别的孩子拥有的时装数据
	if self.m_nCurrentIndex == 4 then 
		local tempList = CacheCenter:getPlayerOwnKidHeadEffect()
		num = #tempList
	else
		...
		--星钻
		local ttf4 = GetElement(self.m_root, "ttf4_WndKidDress", WZUILabelTTF)
		if ttf4 then 
			local nStarDiaNum = CacheCenter:getPlayerItemCountById(35)
			ttf4:setText(nStarDiaNum)
		end
		local imgStarDia = GetElement(self.m_root, "imgStarDia_WndKidDress", WZUIImage)
		if imgStarDia and GDatatab_item["id_35"] then 
			imgStarDia:setScale(0.5)
			imgStarDia:setFile(GDatatab_item["id_35"].icon)
		end
	end
	...
	local tableHeadEffectCon = GetElement(self.m_root,"tableHeadEffectCon_WndKidDress",WZUITableContainer)
	if self.m_nCurrentIndex == 4 then 
		tableCon:setVisible(false)
		tableHeadEffectCon:setVisible(true)
		tableHeadEffectCon:setLoadCountPerFrame(4)
		...
		--时装列表及背景图大小
		tableCon:setVisible(true)
		tableHeadEffectCon:setVisible(false)
...
--改名
function WndKidDress:onBtnClickEditName()
	local element = WndEditBox:createElement()
	local function onApplyRename(txt,lua,tData)
		local tCurKidData = SceneKidHome.m_tKidData[self.m_nKidIndex]
		if tCurKidData and tCurKidData.name == lua then
			MsgBoxManager:showTipBox(LocalStrings.KID_TEXT124)
			return
		end
		if tCurKidData then
			ProtocolProcessorKid:send_WWEDDING_ChangeChildName(tCurKidData.id, lua)
		end
	end
	local childChangeConfig = json.decode(CacheCenter:getGameParam().childChangeConfig)
	local _string = string.sub(childChangeConfig.changeNameCost,2,-2) 
	local id = SplitStringWithSeparator(_string,",")[1]
	local num = SplitStringWithSeparator(_string,",")[2]

	-- local itemCount = CacheCenter:getPlayerItemCountById(id)
	local itemCount = CacheCenter:getPlayerHomeItemCountById(id)
	if tonumber(num) > itemCount then
		-- MsgBoxManager:showTipBox(LocalStrings.OPTIMIZE_TEXT44)
		WndPurchase:showBuyInterface(3,tonumber(id),nil,nil,nil,nil,nil,3,nil,nil)
		return
	end

	WndEditBox:setOkCallBack(onApplyRename, self)
...
--变性
function WndKidDress:onBtnClickChangeSex()
	if not SceneKidHome.m_tKidData then return end

	local childChangeConfig = json.decode(CacheCenter:getGameParam().childChangeConfig)
	local _string = string.sub(childChangeConfig.transsexCost,2,-2) 
	local id = SplitStringWithSeparator(_string,",")[1]
	local num = SplitStringWithSeparator(_string,",")[2]
	...
	local tCurKidData1 = SceneKidHome.m_tKidData[self.m_nKidIndex]
	local tEquipmentList = CacheCenter:getKidEquipmentDressList(tCurKidData1.sex, tCurKidData1.id)
	...
	if (tEquipmentList and #tEquipmentList > 0) or hasAttachment == true then
		local data = {}
		data.childEquip = #tEquipmentList
		data.hasMail = hasAttachment
		WndChangeSex:show(true, data)
		return 
	else
		MsgBoxManager:showConfirmBox(LocalStrings.CHANGESEX14, self, function() 
			local tCurKidData = SceneKidHome.m_tKidData[self.m_nKidIndex]
			if tCurKidData then
				ProtocolProcessorKid:send_WWEDDING_ChangeChildSex(tCurKidData.id, tCurKidData.sex)
			end
		end)
	end
end
...
function WndKidDress:_onChildChangeNameResult(childId, name, status)
	if status == 1 then
		MsgBoxManager:showTipBox(LocalStrings.SUCCESS)
		local tCurKidData = SceneKidHome.m_tKidData[self.m_nKidIndex]
		if tCurKidData then
			if tCurKidData.id == childId then
				SceneKidHome.m_tKidData[self.m_nKidIndex].name = name
			end
			SceneKidHome:createKidAni()
		end
	else
		local str = {LocalStrings.NAME_HAVED_EXIST,LocalStrings.ILLEGAL_CHARACTER,LocalStrings.KID_TEXT125,LocalStrings.KID_TEXT126,LocalStrings.NAME_CANT_BE_NUMBER}
		MsgBoxManager:showTipBox(str[status-1])
	end
end
function WndKidDress:_onChildChangeSexResult(childId, sex, status)
	if status == 0 then
		MsgBoxManager:showTipBox(LocalStrings.SUCCESS)
		local tCurKidData = SceneKidHome.m_tKidData[self.m_nKidIndex]
		if tCurKidData then
			if tCurKidData.id == childId then
				SceneKidHome.m_tKidData[self.m_nKidIndex].sex = sex
			end
			self:updateDress()
			SceneKidHome:createKidAni()
		end
	else
		local str = {LocalStrings.KID_TEXT127,LocalStrings.KID_TEXT128}
		MsgBoxManager:showTipBox(str[status])
	end
end

--@brief 	显示头像框
function WndKidDress:showFrame()
	local tableHeadEffectCon = GetElement(self.m_root, "tableHeadEffectCon_WndKidDress", WZUITableContainer)
	tableHeadEffectCon:cleanTable()
	local conDressR = GetElement(self.m_root, "conDressR_WndKidDress", WZUIContainer)

	local tCurKidData = SceneKidHome.m_tKidData[self.m_nKidIndex]
	local tListData = CacheCenter:getPlayerOwnKidHeadEffect(tCurKidData.id)

	if tListData == nil or #tListData == 0 then 
		ShowPanelNullTip( conDressR, LocalStrings.CHARM_RESULT)
		return 
	end
	removeShowPanelNullTip(conDressR)
	...
	for i = 1, #tListData do
		...
		if celElement and tCell then
			...
			tableHeadEffectCon:setCellElement(celElement)
		end
	end
end

--@brief 	点击物品回调
function WndKidDress:onItemClick(tCell,tag,tData)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)

	WndItemInfo:showInfo(tCell.m_root,self.m_root,1,tData,nil,nil,false)	
end
-------------------------------------私有方法模块End----------------------------------------
...
function WndKidDress:_adaptLanguage_vn( )
	...
	txtTips:setFontSize(16)
	txtTips:setRelativePosition(GlobalMethod:ccp(0.54,0.04))

	local ftxtFighting = GetElement(self.m_root, "ftxtFighting_WndKidDress", WZUIFreeTextBox)
	ftxtFighting:setScale(0.8)

	for i=1,4 do
		local txtTab = GetElement(self.m_root, "txtTab"..i.."_WndKidDress", WZUILabelTTF)
		txtTab:setScale(0.8)
		local txtTabSel = GetElement(self.m_root, "txtTab"..i.."Sel_WndKidDress", WZUILabelTTF)
...
function WndKidDress:_adaptLanguage_tr( )
	GetElement(self.m_root,"ttf1_WndKidDress",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.38,0.950909))
	GetElement(self.m_root,"ttf2_WndKidDress",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.385,0.948517))
...
function WndKidDress:_adaptLanguage_ug( )
	GetElement(self.m_root,"ttf1_WndKidDress",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.9,0.950909))
	GetElement(self.m_root,"ttf2_WndKidDress",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.05,0.948517))

	local txtTips = GetElement(self.m_root,"txtTips_WndKidDress",WZUILabelTTF)
	txtTips:setScale(0.7)
	txtTips:setRelativePosition(GlobalMethod:ccp(0.54,0.04))
	txtTips:setDimensions(GlobalMethod:CCSize(340))
