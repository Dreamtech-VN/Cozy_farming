...
function WndKidFeed:onClick(element)
	...
	elseif self.m_nType == 3 then --小屋扩建
		ProtocolProcessorKid:send_WEDDING_AppeaseChild(tCurKidData.id, 4, tData.basicInfo.id, self.m_nNum)
		WindowManager:removeWindow(self.m_root, self, true) --关闭
	elseif self.m_nType == 4 then --4孩子变性卡
		self:onBtnClickChangeSex()
		WindowManager:removeWindow(self.m_root, self, true)
	elseif self.m_nType == 5 then --5孩子改名笔
		self:onBtnClickEditName()
		WindowManager:removeWindow(self.m_root, self, true)
	end
end
...
function WndKidFeed:setStaticText()
	...
	elseif self.m_nType == 3 then
		GetElement(self.m_root,"txtTitle_WndKidFeed",WZUILabelTTF):setText(LocalStrings.KID_TEXT247)
		GetElement(self.m_root, "txtFeed_WndKidFeed", WZUILabelTTF):setText(LocalStrings.USE)
	elseif self.m_nType == 4 then
		GetElement(self.m_root,"txtTitle_WndKidFeed",WZUILabelTTF):setText(LocalStrings.KID_TEXT249)
		GetElement(self.m_root, "txtFeed_WndKidFeed", WZUILabelTTF):setText(LocalStrings.USE)
	elseif self.m_nType == 5 then
		GetElement(self.m_root,"txtTitle_WndKidFeed",WZUILabelTTF):setText(LocalStrings.KID_TEXT250)
		GetElement(self.m_root, "txtFeed_WndKidFeed", WZUILabelTTF):setText(LocalStrings.USE)
	end
	if self.m_nType == 1 then
		GetElement(self.m_root,"txtExplanation_WndKidFeed",WZUILabelTTF):setText(LocalStrings.OPENCHEST1)
	else
		GetElement(self.m_root,"txtExplanation_WndKidFeed",WZUILabelTTF):setText(LocalStrings.ISLAND_OWNER_TEXT19)
	end
	GetElement(self.m_root,"useNum_WndKidFeed",WZUILabelTTF):setText(self.m_nNum)
end
	...
	if txtFoodDesc then
		if self.m_nNum and self.m_nNum > 0 then
			...
			elseif self.m_nType == 2 then
				...
				txtFoodDesc:setText(string.format(LocalStrings.ISLAND_OWNER_TEXT18, self.m_tData.basicInfo.name, nValue))
			end
		else
			txtFoodDesc:setText("")
		end
...
--@brief 	孩子改名笔
function WndKidFeed:onBtnClickEditName()
...
--@brief 	孩子变性卡
function WndKidFeed:onBtnClickChangeSex()
...
function WndKidFeed:_adaptLanguage_tr( )
	local txtFeed = GetElement(self.m_root, "txtFeed_WndKidFeed", WZUILabelTTF)
	txtFeed:setScale(0.7)
...
function WndKidFeed:_adaptLanguage_ug( )
	GetElement(self.m_root,"txtTitle_WndKidFeed",WZUILabelTTF):setScale(0.7)

	GetElement(self.m_root,"txtExplanation_WndKidFeed",WZUILabelTTF):setDimensions(GlobalMethod:CCSize(360))
	local txtFoodDesc = GetElement(self.m_root, "txtFoodDesc_WndKidFeed", WZUILabelTTF)
	txtFoodDesc:setScale(0.6)
	txtFoodDesc:setDimensions(GlobalMethod:CCSize(500))
