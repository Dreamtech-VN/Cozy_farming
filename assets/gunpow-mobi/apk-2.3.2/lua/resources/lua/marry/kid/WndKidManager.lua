...
function WndKidManager:onCheck(element)
	...
	if nTag == 6 then
		if SceneKidHome.m_tKidData and #SceneKidHome.m_tKidData == 0 then
...
function WndKidManager:_adaptLanguage_ug(  )
	for i = 1, 7 do
		local txtTab = GetElement(self.m_root, "txtTab" .. i .. "_WndKidManager", WZUILabelTTF)
		txtTab:setScale(0.55)
		txtTab:setDimensions(GlobalMethod:CCSize(140))
		local txtTabSel = GetElement(self.m_root, "txtTab" .. i .. "Sel_WndKidManager", WZUILabelTTF)
		txtTabSel:setScale(0.55)
		txtTabSel:setDimensions(GlobalMethod:CCSize(140))
	end
end
-------------------------------------语言适配End----------------------------------------