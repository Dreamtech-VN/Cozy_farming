...
function SceneWeddingDaily:onEnter(element)
    ...
    if isEndTeach ~= true and TeachGroup1:isTeach() and CacheCenter:getPlayerInfo().level == 21  then
        ...
        if WndMarryManager.m_tMarryStatus.marryStatus ~= 0 then
...
--@brief 	求婚按钮
function SceneWeddingDaily:onPleaseBtn(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
function SceneWeddingDaily:onCellBtn(nCellIndex)
	...
	--更新右边文字
	self:_updateRightUiText(self.m_tData[nCellIndex].manName,self.m_tData[nCellIndex].womanName,self.m_tData[nCellIndex].startTime,self.m_tData[nCellIndex].endTime, self.m_tData[nCellIndex].manServerId, self.m_tData[nCellIndex].womanServerId)
...
function SceneWeddingDaily:_update()
    ...
    WZLog("结婚场次信息",Serialize(self.m_tData))
    SceneWeddingDaily:updateProposeBtn()
    GetElement(self.m_root,"txtNotWedding_SceneWeddingDaily",WZUILabelTTF):setVisible(false)
	local tbconWeddingList = WZUITableContainer:luaTo(GetElement(self.m_root,"tbconWeddingList_SceneWeddingDaily"))
	if self.m_tData and next(self.m_tData) then
		...
		--右边文字
		self:_updateRightUiText(self.m_tData[self.m_nCurrentCellIndex].manName,self.m_tData[self.m_nCurrentCellIndex].womanName,self.m_tData[self.m_nCurrentCellIndex].startTime,self.m_tData[self.m_nCurrentCellIndex].endTime, self.m_tData[self.m_nCurrentCellIndex].manServerId, self.m_tData[self.m_nCurrentCellIndex].womanServerId)
		...
		GetElement(self.m_root,"txtNotWedding_SceneWeddingDaily",WZUILabelTTF):setVisible(true)
	end
end
...
function SceneWeddingDaily:createTenListInfo()
    ...
    for i=1+self.m_nLoadListIndex*10,10+self.m_nLoadListIndex*10 do
		...
		tCell:setWeddingListUi(self.m_tData[self.m_nCurrendLoadCell+1].wedId,
											...
											self.m_tData[self.m_nCurrendLoadCell+1].manServerId,
											self.m_tData[self.m_nCurrendLoadCell+1].womanServerId,
											self.m_tData[self.m_nCurrendLoadCell+1].progress)
...
function SceneWeddingDaily:_updateRightUiText(sBriggeName,sBrigeName,startTime,endTime, manServerId, womanServerId)
    ...
    if manServerId ~= CacheCenter:getPlayerInfo().serverId then 
    	GetElement(self.m_root, "imgKuafuIconR_SceneWeddingDaily", WZUIImage):setVisible(true)
    else
    	GetElement(self.m_root, "imgKuafuIconR_SceneWeddingDaily", WZUIImage):setVisible(false)
    end
    --新娘
    WZUILabelTTF:luaTo(GetElement(self.m_root,"txtBridgeName_SceneWeddingDaily")):setText(sBrigeName)
    if womanServerId ~= CacheCenter:getPlayerInfo().serverId then 
    	GetElement(self.m_root, "imgKuafuIconL_SceneWeddingDaily", WZUIImage):setVisible(true)
    else
    	GetElement(self.m_root, "imgKuafuIconL_SceneWeddingDaily", WZUIImage):setVisible(false)
    end
...
--更新求婚按钮
function SceneWeddingDaily:updateProposeBtn()
	if self.m_root == nil then
		return
	end
    if WndMarryManager.m_tMarryStatus.coupleName ~= "" then
    	local btnPlease = GetElement(self.m_root,"btnPlease_SceneWeddingDaily",WZUIButton)
    	btnPlease:setVisible(false)
    	local btnJoin = GetElement(self.m_root,"btnJoin_SceneWeddingDaily",WZUIButton)
    	btnJoin:setRelativePosition(GlobalMethod:ccp(0.335,0.5))
    end 
end
...
function SceneWeddingDaily:_adaptLanguage_vn(  )
    ...
    GetElement(self.m_root,"txtJoin1_SceneWeddingDaily",WZUILabelTTF):setScale(0.8)
    GetElement(self.m_root,"txtJoin2_SceneWeddingDaily",WZUILabelTTF):setScale(0.8)
    GetElement(self.m_root,"txtJoin3_SceneWeddingDaily",WZUILabelTTF):setScale(0.8)
end
...
function SceneWeddingDaily:_adaptLanguage_ug()
	local txtTitle = GetElement(self.m_root,"txtTitle_SceneWeddingDaily",WZUILabelTTF)
    txtTitle:setRelativePosition(GlobalMethod:ccp(0.27,0.5))
    local txtWeddingStartTime = GetElement(self.m_root,"txtWeddingStartTime_SceneWeddingDaily",WZUILabelTTF)
    txtWeddingStartTime:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    txtWeddingStartTime:setRelativePosition(GlobalMethod:ccp(0.26,0.5))
end
