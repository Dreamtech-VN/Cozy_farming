...
function CellMatchMaking:onChat(element)
    -- body
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
    if whetherCanPrivateChat(self.m_tData.id) then 
    	WndChat:showChatWindowForPrivateWithIdAndName(self.m_tData.id,self.m_tData.name, self.m_tData.sex, self.m_tData.level, self.m_tData.vipLevel, self.m_tData.headId, self.m_tData.faceId, self.m_tData.headColor, self.m_tData.headEffectId)
    end
end
...
function CellMatchMaking:_update()
	--名字等级
	local strformat = [[<T C="127,70,26" S="18" P="1">Lv</T><T C="229,105,22" S="18" P="1">%d </T><T C="127,70,26" S="18" P="1">%s</T>]]
	if self.m_tData.serverId ~= CacheCenter:getPlayerInfo().serverId then
		strformat = [[<T C="127,70,26" S="18" P="1">Lv</T><T C="229,105,22" S="18" P="1">%d </T><T C="127,70,26" S="18" P="1">%s</T><I Z="1">ui/common/common_icon_kuafu.png</I>]]
	end
	GetElement(self.m_root, "ftbName_CellMatchMaking", WZUIFreeTextBox):setShowText(string.format(strformat,self.m_tData.level,self.m_tData.name))
	--公会名字
	...
	--战力
	GetElement(self.m_root, "txtFightWord_CellMatchMaking", WZUILabelTTF):setText(LocalStrings.COMBAT..":")
