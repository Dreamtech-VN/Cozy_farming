...
function CellGuestList:setArmGuestInfo()
	...
	if self.m_nServerId ~= CacheCenter:getPlayerInfo().serverId then 
		GetElement(self.m_root, "imgKuafuIcon_CellGuestList", WZUIImage):setVisible(true)
	end
	...
	local absX = self.m_moveWidth * randomPos[pIndex][1]
	local absY = self.m_moveHeigth * randomPos[pIndex][2]
...
function CellGuestList:schedulePlayerMoving(element)
	...
	if self.m_nSchedulCount >= self.m_nStopSecond then
		...
		--播放人物移动
		local screenSize = CCEGLView:sharedOpenGLView():getFrameSize()
		local absX = self.m_moveWidth * randomPos[self.m_nRandomIndex][1]
		local absY = self.m_moveHeigth * randomPos[self.m_nRandomIndex][2]
		if absX == self.m_tMoveDest.x and absY == self.m_tMoveDest.y then
			local ind = math.random(20)
			absX = self.m_moveWidth * randomPos[ind][1]
		    absY = self.m_moveHeigth * randomPos[ind][2]
		else
...
function CellGuestList:updateFootEffect(nX, nY)
    ...
    if self.m_nFootId and self.m_nFootId > 0 then
        ...
        if  BattleCommon:pointDis(self.m_tOldFootPos,pos) > distance then
	        self.m_tOldFootPos = pos
	        FootEffectManager:getInstance():addEffect(self.m_nFootId,pos,-25,self.m_tPlayerAni:getAnimNode():getScaleX(),self.m_tPlayerAni:getAnimNode():getScaleY())
	    end
    end
end
-------------------------------------公有方法模块End----------------------------------------
