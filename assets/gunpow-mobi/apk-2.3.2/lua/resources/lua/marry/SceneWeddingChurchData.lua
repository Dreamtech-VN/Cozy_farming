...
function SceneWeddingChurch:_init()
	...
	self.m_tGuestElement2 = {}			--宾客动画节点列表2
	self.m_tGuestLuaObj = {}			--宾客动画lua表对象列表
	self.m_tGuestLuaObj2 = {}			--宾客动画lua表对象列表2
	...
	self.m_tCoupleElement = {}
	self.m_tCoupleLuaObj = {}
	self.m_nProgress = 1					--婚礼进度 1等待双方入场 | 2等待举办婚礼 | 3正在举办婚礼 | 4正在婚宴
	self.m_nStartWeddingCD = 0			--开始婚礼cd
	self.m_nBackSceneCD = 0				--返回场景cd
	self.m_nNoticeCD = 30				--确认婚礼倒计时
	self.m_nPriestTalkIndex = 1			--神父说话内容索引
end
...
function SceneWeddingChurch:_unInit()
	...
	self.m_tGuestElement2 = nil			--宾客动画节点列表2
	self.m_tGuestLuaObj = nil			--宾客动画lua表对象列表
	self.m_tGuestLuaObj2 = nil			--宾客动画lua表对象列表2
	self.m_nLoadingCircleId = nil       --转菊花ID
	self.m_bSendExitFlag = false        --退出婚礼现象标志
	self.m_tData = nil 
	self.m_nFirworkTime = nil           --播放烟花时间次数
	...
	self.m_tCoupleElement = nil
	self.m_tCoupleLuaObj = nil
	self.m_nProgress = nil
	self.m_nStartWeddingCD = nil
	self.m_nBackSceneCD = nil
	self.m_nNoticeCD = nil
	self.m_nPriestTalkIndex = nil
end
...
function SceneWeddingChurch:JoinWeddingOk(manName, womanName, playerId, playerName, playerHeadId, playerFaceId, playerBodyId, playerWingId, sex, level, marryType, weddingHallId,manFaceId,womanFaceId,hallPass,vipLevel,guestHeadColors,guestBodyColors,manHeadColour,womanHeadColour,manBodyColour,womanBodyColour,manFashionId,womanFashionId,manHeadId,womanHeadId,footmark, manServerId, womanServerId, guestServerId, manId, womanId)
	WZLog("SceneWeddingChurch:JoinWeddingOk =",manBodyColour)
	if SceneRoom ~= nil and (SceneRoom.m_root or SceneBossRoom.m_root  or WndLeagueTeamDetail.m_root  or SceneBattle.m_root or WndTowerScroll.m_root or SceneWeddingChurch.m_root  or SceneLeagueRoom.m_root or SceneBattleLoading.m_root or WndDoubleTowerRoom.m_root) then
	   return
	end
	...
	self.manServerId = manServerId
	self.womanServerId = womanServerId
	self.manId = manId
    ...
    for i=1,#playerId do
    	...
    	guestInfo.serverId = guestServerId[i]
...
function SceneWeddingChurch:WeddingOver()
	...
	MsgBoxManager:showConfirmBox(LocalStrings.MARRY_DESC_31, self, function() end, MSGBOXLEVEL_NORMAL, nil,true,nil,nil,nil,30)
end 
...
function SceneWeddingChurch:RefreshWedding(playerId, playerName, playerHeadId, playerFaceId, playerBodyId, playerWingId, sex, level,headColor,bodyColor,footmark, serverId)
	...
	if  self:_ifBrigeGroomOrBrigeEnter(playerName) then 
		...
		if not bExist then
			...
			guestInfo.serverId = serverId
			table.insert(self.m_tGuest,guestInfo)
			if #self.m_tGuest >=20 then
			    return
		    end
			--设置人物走动
		    self:createGuest(playerId,playerName,sex,playerFaceId,playerHeadId,playerBodyId,headColor,bodyColor,footmark, serverId)
		end
	end
end 
...
function SceneWeddingChurch:SchedulePriestWord(element,delta)
	...
	WZUILabelTTF:luaTo(GetElement(self.m_root,"txtPopPriest_SceneWeddingChurch")):setText(self.m_tData.priestSay[self.m_nCurSayIndex])
	GetElement(self.m_root,"conPopPriest_SceneWeddingChurch"):setVisible(true)
	WZUILabelTTF:luaTo(GetElement(self.m_root,"txtPopPriest_SceneWeddingChurch")):enableSchedule("ScheduletxtShowTime",3.8)
...
--@brief 走地毯时显示神父说话内容
function SceneWeddingChurch:showPriestTalk1()
	local spPriest2 = GetElement(self.m_root,"spPriest2_SceneWeddingChurch",WZUISpine)
	local conPopPriest2 = GetElement(self.m_root,"conPopPriest2_SceneWeddingChurch",WZUIContainer)
	local txtPopPriest2 = GetElement(self.m_root,"txtPopPriest2_SceneWeddingChurch",WZUILabelTTF)
	spPriest2:disableSchedule()
	spPriest2:enableSchedule("_schedulePriestTalk1",3)
	txtPopPriest2:setText(LocalStrings.MARRY_DESC_37)
	conPopPriest2:setVisible(true)
end

--@brief 走地毯时更新神父说话框
function SceneWeddingChurch:_schedulePriestTalk1(element,delta)
	element:disableSchedule()
	local conPopPriest2 = GetElement(self.m_root,"conPopPriest2_SceneWeddingChurch",WZUIContainer)
	conPopPriest2:setVisible(false)
end

--@brief 停留时显示神父说话内容
function SceneWeddingChurch:showPriestTalk2()
	if LocalStrings.MARRY_DESC_38 and self.m_nPriestTalkIndex <= #LocalStrings.MARRY_DESC_38 then
		local spPriest2 = GetElement(self.m_root,"spPriest2_SceneWeddingChurch",WZUISpine)
		local conPopPriest2 = GetElement(self.m_root,"conPopPriest2_SceneWeddingChurch",WZUIContainer)
		local txtPopPriest2 = GetElement(self.m_root,"txtPopPriest2_SceneWeddingChurch",WZUILabelTTF)
		spPriest2:disableSchedule()
		spPriest2:enableSchedule("_schedulePriestTalk2",3)
		txtPopPriest2:setText(LocalStrings.MARRY_DESC_38[self.m_nPriestTalkIndex])
		conPopPriest2:setVisible(true)
		self.m_nPriestTalkIndex = self.m_nPriestTalkIndex + 1
	end
end

--@brief 停留时更新神父说话框
function SceneWeddingChurch:_schedulePriestTalk2(element,delta)
	local txtPopPriest2 = GetElement(self.m_root,"txtPopPriest2_SceneWeddingChurch",WZUILabelTTF)
	local conPopPriest2 = GetElement(self.m_root,"conPopPriest2_SceneWeddingChurch",WZUIContainer)
	if self.m_nPriestTalkIndex <= #LocalStrings.MARRY_DESC_38 then
		...
		conPopPriest2:setVisible(false)
		--显示按钮
		SceneWeddingChurch:endWeddingAni()
	end
end
...
--@brief	开始婚宴
--@param	result : 1开始婚礼动画 2对方不在婚礼房间
function SceneWeddingChurch:startWeddingOk(result)
	if result == 1 then
		SceneWeddingChurch:setWeddingChurchType(2)
...
--@brief	开始婚宴
--@param	progress  : 婚礼进度 1等待双方入场 | 2等待举办婚礼 | 3正在举办婚礼 | 4正在婚宴
function SceneWeddingChurch:updateWedding(progress)
...
--@brief	吃婚宴结果
--@param    result : 1成功 | 2已经吃过了 | 3没有吃的权限
function SceneWeddingChurch:getWeddingEatOk(result)
	if result == 1 then
		MsgBoxManager:showTipBox(LocalStrings.MARRY_DESC_26)
	elseif result == 2 then
		MsgBoxManager:showTipBox(LocalStrings.MARRY_DESC_27)
	elseif result == 3 then
		MsgBoxManager:showTipBox(LocalStrings.MARRY_DESC_28)
	end
