...
function WndGradeStrengthen:updateMNum()
	--WZLog("WndGradeStrengthen:updateMNum", debug.traceback())
		local equiGradeInfo = self:getEquiGradeInfo()
		--显示调品箱图片
		GetElement(self.m_root,"imgAdd4",WZUIImage):setFile(GDatatab_item["id_"..equiGradeInfo.cost[1][1]].icon)
		--拥有调品箱数量
		local num1 = CacheCenter:getPlayerItemCountById(equiGradeInfo.cost[1][1])
		--设置调品箱数量
		GetElement(self.m_root,"txtHolyStoneNum",WZUILabelTTF):setText(num1.."/"..equiGradeInfo.cost[1][2])
		if num1 < equiGradeInfo.cost[1][2] then
		...
		if tEquip.extraInfo.orangeEquiGrade == nil or tEquip.extraInfo.orangeEquiGrade == "" then
			...
			if tonumber(grade[1]) >= self:getMaxGrade() then
		...
		GetElement(self.m_root,"txtGrade",WZUILabelTTF):setText(equiGradeInfo.name)
		...
		--显示消耗材料
		GetElement(self.m_root,"cost",WZUILabelTTF):setText(equiGradeInfo.cost_num)
...
function WndGradeStrengthen:onTip1()
	...
	local tData = GDatatab_item["id_"..equiGradeInfo.cost[1][1]]
...
function WndGradeStrengthen:onSure()
	...
	--调品箱是否足够
	local equiGradeInfo = self:getEquiGradeInfo()
	local costItem = GDatatab_item["id_"..equiGradeInfo.cost[1][1]]
	local num1 = CacheCenter:getPlayerItemCountById(equiGradeInfo.cost[1][1])
	if num1 < equiGradeInfo.cost[1][2] then
        MsgBoxManager:showConfirmBox(string.format(LocalStrings.ASCENDING_FUSE15,costItem.name), self, self.buy, nil, nil)
		return
	end
...
function WndGradeStrengthen:buy(btnTag)
    ...
    --if btnTag == MSGBOXTYPE_CONFIRM then
		local equiGradeInfo = self:getEquiGradeInfo()
        checkIsOnSale(equiGradeInfo.cost[1][1])
    --end
end
...
function WndGradeStrengthen:_adaptLanguage_ug(  )
	GetElement(self.m_root,"has",WZUILabelTTF):setText("("..LocalStrings.OWN..":"..CacheCenter:getPlayerItemCountById(blueprintID)..")")
	for i=1,3 do
		GetElement(self.m_root,"txtSure"..i.."_WndGradeStrengthen",WZUILabelTTF):setScale(0.7)
	end

	GetElement(self.m_root,"txtCost",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.64,0.78))
	GetElement(self.m_root,"imgCost",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.58,0.78))
	local cost = GetElement(self.m_root,"cost",WZUILabelTTF)
	cost:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	cost:setRelativePosition(GlobalMethod:ccp(0.52,0.78))
	local has = GetElement(self.m_root,"has",WZUILabelTTF)
	has:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	has:setRelativePosition(GlobalMethod:ccp(0.38,0.78))

	local txtGrade = GetElement(self.m_root,"txtGrade",WZUILabelTTF)
	txtGrade:setScale(0.6)
	txtGrade:setDimensions(GlobalMethod:CCSize(130))
end
-------------------------------------语言适配End--------------------------------------------