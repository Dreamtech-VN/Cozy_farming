...
function WndAscendingTip:_adaptLanguage_ug(  )
	GetElement(self.m_root, "imgCostIcon_WndAscenddingTip", WZUIImage):setRelativePosition(GlobalMethod:ccp(0.176795,0.5))
	local cost = GetElement(self.m_root,"cost",WZUILabelTTF)
	cost:setRelativePosition(GlobalMethod:ccp(0.126667,0.5))
	cost:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	local txtCancel = GetElement(self.m_root,"txtCancel_WndConfirmBox",WZUILabelTTF)
	txtCancel:setScale(0.7)
	txtCancel:setDimensions(GlobalMethod:CCSize(150))
	local txtConfirm = GetElement(self.m_root,"txtConfirm_WndConfirmBox",WZUILabelTTF)
	txtConfirm:setScale(0.55)
	txtConfirm:setDimensions(GlobalMethod:CCSize(180))
end
------------------------------------语言适配End----------------------------------------