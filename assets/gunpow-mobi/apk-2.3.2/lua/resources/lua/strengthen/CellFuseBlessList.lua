...
function CellFuseBlessList:_update()
    ...
    --名字
    local imgQualityRect = GetElement(self.m_root, "imgQualityRect_CellFuseBlessList", WZUI9Image)
    local imgNameBg = GetElement(self.m_root, "imgNameBg_CellFuseBlessList", WZUI9Image)
    imgQualityRect:setFile(g_tQualityBG[tData.basicInfo.quality])
    imgNameBg:setFile(g_tQualityNameBG[tData.basicInfo.quality])
    ...
    self:_showState()
    --高亮
    if self.bHighLight ~= nil then
        self:setHightLightVisible(self.bHighLight)
    end
end
    ...
    if txtState then
        ...
        if self.m_nLabelIndex == 1 then
            ...
            if nHavedNum < tData.nNeededNum then
                txtState:setColor(GlobalMethod:ccc3(195,171,148))
...
function CellFuseBlessList:_adaptLanguage_ug(  )
    local txtState = GetElement(self.m_root,"txtState_CellFuseBlessList",WZUILabelTTF)
    txtState:setScale(0.7)
    txtState:setDimensions(GlobalMethod:CCSize(260))
end
--------------------------------------语言适配End----------------------------------------------