...
function WndUpgradeGem:onTouchBegan()
	for i=1,3 do
		if self["m_tCell"..i] ~= nil then
   			self["m_tCell"..i]:removeGouIcon()
		end
	end
	if self.m_nSelected ~= nil then
   		self["m_tCell"..self.m_nSelected]:showSelectedIcon(5)
	end
end
...
function WndUpgradeGem:onSure(element)
    ...
    for i = 1, #self.m_tTotalCost do
    	if not JudgeMoneyIsEnough(self.m_tTotalCost[i][1], self.m_tTotalCost[i][2]) then 
    		return 
    	end
    end
	...
	if self.m_nSelected ~= nil then
		...
		if self.m_tData.basicInfo.main_type == 43 then
			ProtocolProcessorMerge:send_MERGE_MergeItemPetEquipStone(self.m_tData.playerItemId, self.m_tDataList[1].sub_type,self.m_tDataList[self.m_nSelected].id)
		else
...
function WndUpgradeGem:setCost()
	...
	self.m_tTotalCost = {{costId, cost1+cost2+cost3}}
	WndGemMountingStrengthen:setCost(self.m_tTotalCost)
end
...
function WndUpgradeGem:_adaptLanguage_ug(  )
	local txtTip1 = GetElement(self.m_root,"txtTip1_WndUpgradeGem",WZUILabelTTF)
	txtTip1:setScale(0.85)
	txtTip1:setRelativePosition(GlobalMethod:ccp(0.5,0.765))
	local txtTip = GetElement(self.m_root,"txtTip_WndUpgradeGem",WZUILabelTTF)
	txtTip:setRelativePosition(GlobalMethod:ccp(0.64,0.33))
	local imgGold = GetElement(self.m_root,"imgGold_WndUpgradeGem",WZUIImage)
	imgGold:setRelativePosition(GlobalMethod:ccp(0.26,0.33))
	local txtCost = GetElement(self.m_root,"txtCost",WZUILabelTTF)
	txtCost:setRelativePosition(GlobalMethod:ccp(0.2,0.33))
	txtCost:setAnchorPoint(GlobalMethod:ccp(1,0.5))

	local txtUpgrade = GetElement(self.m_root,"txtUpgrade_WndUpgradeGem",WZUILabelTTF)
	txtUpgrade:setScale(0.7)
	txtUpgrade:setDimensions(GlobalMethod:CCSize(160))
	local txtCancel = GetElement(self.m_root,"txtCancel_WndUpgradeGem",WZUILabelTTF)
