...
WBattleGlobal = {
    ...
    m_tMyProfessionSkill_Beginning = {
        count= nil, id = nil, name = nil, icon = nil, lv = nil, 
        skill_type = nil, node = nil, attribute = nil,
    },
    m_nAutoFightActIndex = 0,   --自动战斗角色动作索引0：无动作；1：射击；2：飞行
    m_bIsPlayerOperateAlready = false, --玩家回合是否已经出过手
    m_bIsCurRoundFirstBullet = true,    --是否是该回合的首颗子弹
    m_tSkinBigSkillId = {},   --皮肤大招id列表
    m_tSkinBigSkillList = nil,  --所拥有的皮肤大招列表
    m_tSkinBigSkillTargetPos = nil,     --皮肤大招靶子坐标 宇航员大招用到
    m_tKMSkillList = {
        itemSubType=nil,            param1=nil,
        name=nil,                   param2=nil,
        icon=nil,                   tireValue=nil,
    ...
    m_tWindLevelLimit = nil, 	--战斗中风力的最小最大值
    m_tCurRoundSoulHeroShootId = nil, --当前回合可以出手的灵魂分身Id
    m_nCurRoundSubHeroShootCount = nil, --当前回合出手的分身计数
}
...
function WBattleGlobal:destroy()
	...
	if self.m_nReference <=0 then
        ...
        for i,kid in pairs(self.m_tKids) do
            if kid then
                kid:destroy()
            end
        end
        ...
        for i, soulHero in pairs(self.m_tSoulHeros) do
            if soulHero then
                soulHero:destroy()
            end
        end
        self.m_tSoulHeros = nil
        ...
        BattleProfessionSkillManager:destroy()
        ...
        BattleMsgPlayerReadyFly.isRun = nil
        BattleMsgPlayerMoveCtrl.isRun = nil
        self.m_tSkinBigSkillId = {} 
        self.m_tSkinBigSkillList = nil
        self.m_tSkinBigSkillTargetPos = nil
	end
    self.m_tWindLevelLimit = nil 
    self.m_tCurRoundSoulHeroShootId = nil 
    self.m_nCurRoundSubHeroShootCount = 0 
end
...
--@brief    清除自身子弹雾
function WBattleGlobal:cleanMyBulletFog(bulletPos, bulletOwner, cleanType, offsetx, offsety, width, height)
    --do return end
    if not WBattleGlobal:getCurrent():isEscapeBattle() then
        return
    end
    local isMyHero = bulletOwner == WBattleGlobal:getCurrent():getMyHero()
    ...
    if isMyHero and (SceneBattle:getBattleLoop():getCount() % 3 == 0 or cleanType == 3) then
        ...
        local x, y = bulletPos.x, bulletPos.y
...
function WBattleGlobal:startNewRound()
    ...
    self.m_tCurRoundSoulHeroShootId = {}
    self.m_nCurRoundSubHeroShootCount = 0
	...
	--更新所有召唤的分身的属性
	for id, soulHero in pairs(self:getSoulHerosList()) do
		if not soulHero:isDead() and (not hero or (hero:getCamp() ~= soulHero:getCamp() and soulHero:getSubType() == CharacterSubType.SUBTYPE_SOUL) or (hero:getBattleId() == soulHero:getOwnerPlayerId() and (soulHero:getSubType() == CharacterSubType.SUBTYPE_BCHESS or soulHero:getSubType() == CharacterSubType.SUBTYPE_WCHESS))) then 
			soulHero:updateByTurn()
			--将召唤那一刻的一些增益属性赋值
			soulHero:copyOwnerAttributeChangeStateList()
    ...
    BattleProfessionSkillManager:updateByTurn()
    ...
    if AutoRunBattleConst.AUTO_RUN_BATTLE or GlobalGame.g_bIsAutoFightOpen then
...
--@brief 检查图腾碰撞
function WBattleGlobal:checkHurtBuffTotem(monsterType)
--    WZLog("WBattleGlobal:checkHurtBuffTotem")
    ...
    for id, character in pairs (self:getCharacterList()) do
        if not character:isDead() then
            local totemId, totemLv, totemUserId = nil, nil, nil
            if monsterType == MonsterType.BUFF_TOTEM then 
                totemId,totemLv,totemUserId = character:getHurtBuffTotemInfo()
            elseif monsterType == MonsterType.FIRE_TOTEM then 
                totemId,totemLv,totemUserId = character:getFireTotemInfo()
            elseif monsterType == MonsterType.GUARDIAN_TOTEM then 
                totemId,totemLv,totemUserId = character:getGuardianTotemInfo()
            elseif monsterType == MonsterType.MARITIME1_TOTEM then
                totemId,totemLv,totemUserId = character:getBuffInfoByType(BuffType.MARITIME1_TOTEM)
            elseif monsterType == MonsterType.MARITIME2_TOTEM then
                totemId,totemLv,totemUserId = character:getBuffInfoById(40029)
            elseif monsterType == MonsterType.MARITIME3_TOTEM then
                totemId,totemLv,totemUserId = character:getBuffInfoByType(BuffType.MARITIME3_TOTEM)
            elseif monsterType == MonsterType.JIANGZIYA_TOTEM then
                totemId,totemLv,totemUserId = character:getBuffInfoByType(BuffType.JIANGZIYA_TOTEM)
            elseif monsterType == MonsterType.UMBRELLA1_TOTEM then
                totemId,totemLv,totemUserId = character:getBuffInfoById(40032)
            elseif monsterType == MonsterType.UMBRELLA2_TOTEM then
                totemId,totemLv,totemUserId = character:getBuffInfoById(40033)
            end
            if totemId and totemId > 0 then
                heroList[character:getBattleId()] ={buffId = totemId,buffLv = totemLv, buffUserId = totemUserId}
            end
        end
    end

    for id, character in pairs (self:getMachinesList()) do
        if not character:isDead() and character.m_nMonsterType == monsterType then
            local list = character:checkBuffTotemCollison()
            local buffId,buffLv,buffUserId,duration,monsterType = character:getBuffInfo()
            for _,hero in ipairs(list) do
                if not buffList[hero:getBattleId()] or buffList[hero:getBattleId()].buffLv < buffLv then
                    buffList[hero:getBattleId()] = {buffId = buffId,buffLv = buffLv, buffUserId = buffUserId, duration = duration, monsterType = monsterType}
                end
            end
        end
    end
...
--    WZLog("WBattleGlobal:checkHurtBuffTotem one", Serialize(addList))
...
function WBattleGlobal:addBuffTotemEffect(infoList)
    ...
    for id,info in pairs(infoList) do
    	if self:getCharacterWithId(id) then
    		if (info.monsterType ~= MonsterType.GUARDIAN_TOTEM and info.monsterType ~= MonsterType.MARITIME1_TOTEM and info.monsterType ~= MonsterType.MARITIME2_TOTEM and info.monsterType ~= MonsterType.MARITIME3_TOTEM and info.monsterType ~= MonsterType.JIANGZIYA_TOTEM 
    				and info.monsterType ~= MonsterType.UMBRELLA1_TOTEM and info.monsterType ~= MonsterType.UMBRELLA2_TOTEM)
                    or (info.monsterType == MonsterType.GUARDIAN_TOTEM or info.monsterType == MonsterType.MARITIME1_TOTEM or info.monsterType == MonsterType.MARITIME2_TOTEM or info.monsterType == MonsterType.MARITIME3_TOTEM or info.monsterType == MonsterType.JIANGZIYA_TOTEM
                    or info.monsterType == MonsterType.UMBRELLA1_TOTEM or info.monsterType == MonsterType.UMBRELLA2_TOTEM) and info.duration and info.duration > 0 then
		        userId = info.buffUserId
		        WZLog("WBattleGlobal:addBuffTotemEffect",id, info.buffId, userId)
		        self:addBuff({self:getCharacterWithId(id)},info.buffId,userId,nil,info.duration)
		        if WBattleGlobal:getCurrent():isHostControl() then
		            ProtocolProcessorSceneBattle:send_BATTLE_BuffChange(WBattleGlobal:getCurrent():getBattleId(), userId, 0, info.buffId, info.duration, {id})
		        end
		    end
	    end
    end
...
function WBattleGlobal:removeHeroBuffById(userId,hero,buffId)
    if hero and not hero:isDead() then
        ...
        if WBattleGlobal:getCurrent():isHostControl() then
            ProtocolProcessorSceneBattle:send_BATTLE_BuffChange(WBattleGlobal:getCurrent():getBattleId(), userId, 1, buffId, 0, {hero:getBattleId()})
        end
    end
end
--=====================================个人副本关键点记录begin========================================================
...
--@param    extraHurt:强制将一些技能回合结束伤害放到buff伤害中
function WBattleGlobal:setHpProRecord(battleId,hurtHp,tShootHero,hitRatio, extraHurt)
    if not WBattleGlobal:getCurrent():isSingleStage() or WBattleGlobal:getCurrent():isReplayGame() then
        return
    end
    ...
    if not hurtRecord.playerExtraHurt then
        hurtRecord.playerExtraHurt = 0
    end
    ...
    if hurtHp < 0 then
        ...
        hurtInfo.explodeRadius = shooter:getRadiusForBulletExplode() or 0
        --计算固定伤害
        local hurtAdd = 0
        ...
        --攻击伤害才记录攻击玩家的属性加成数据(属性增加万分比的属性+1000代表万分比，直接用属性值的+固定值),暴击属性不用处理因为已经跟随hurtRatio了
        hurtInfo.hurtPropertyAdd = -1
        if hurtHp < 0 then 
            local needAddProperty = {"m_nAttack", "m_nDefence"}
            local attriType = {AttributeConfig.Attack, AttributeConfig.Defence}
            for i = 1, #needAddProperty do
                local tAddPercent = shooter.m_tAttributeChangeStateList[needAddProperty[i] .. "AddPercent"]
                local tvalueAddValue = shooter.m_tAttributeChangeStateList[needAddProperty[i] .. "AddValue"]
                local addPer = 0
                local addValue = 0
                if tAddPercent ~= nil then
                    addPer = addPer + tAddPercent.value
                end
                if tvalueAddValue ~= nil then
                    addValue = addValue + tvalueAddValue.value
                end
                local buffPre = BattleBuffMethod:getBuffValue(shooter, EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT, attriType[i])
                local buffVal = BattleBuffMethod:getBuffValue(shooter, EffectTypeConfig.CHANGE_ATTRIBUTE_VALUE, attriType[i])
                addPer = addPer + buffPre
                addValue = addValue + buffVal
                if addPer ~= 0 then 
                    if hurtInfo.hurtPropertyAdd == -1 then 
                        hurtInfo.hurtPropertyAdd = (1000+attriType[i]) .. "_" .. (addPer*10000)
                    else
                        hurtInfo.hurtPropertyAdd = hurtInfo.hurtPropertyAdd .. "_" .. (1000+attriType[i]) .. "_" .. (addPer*10000)
                    end
                end
                if addValue ~= 0 then 
                    if hurtInfo.hurtPropertyAdd == -1 then 
                        hurtInfo.hurtPropertyAdd = attriType[i] .. "_" .. addValue
                    else
                        hurtInfo.hurtPropertyAdd = hurtInfo.hurtPropertyAdd .. "_" .. attriType[i] .. "_" .. addValue
                    end
                end
            end
        end
    ...
    elseif extraHurt and extraHurt < 0 then
        hurtRecord.playerExtraHurt = hurtRecord.playerExtraHurt + extraHurt
...
function WBattleGlobal:setPetHpProRecord(battleId,hurtHp,hurtRatio, isPetContinueAttack)
    ...
    if isPetContinueAttack then 
        if not hurtRecord.petDoubleHurt then
            hurtRecord.petDoubleHurt = 0
            hurtRecord.petDoubleRatio = hurtRatio * 100
        end
        hurtRecord.petDoubleHurt = hurtRecord.petDoubleHurt + hurtHp
    else
    ...
    for i,v in pairs(effectList) do 
        if v ~= -1 then
            ...
            for k,effectInfo in pairs(effectParm) do 
                ...
                if effectType == EffectTypeConfig.RANDOM_SCATTER_SHOOT then
                    atkNum = effectInfo[5]
                    isChange = true
                end
            end
...
--@brief 
--加buff
function WBattleGlobal:addBuff(targetHeroList,buffId,userId,buffAtk,duration)
    ...
    for id, hero in pairs (targetHeroList) do
        if hero:isDead() ~= true then
            local buffNew = BuffBody:new(buffInfo,hero, userId,nil,duration)
            ...
            if not isNext then
                ...
                if not offBuff then
                    ...
                    WZLog("WBattleGlobal:addBuff three-1", hero.m_tBuffAddTimes[buffNew.m_nType])
...
--@brief 是否战略赛
function WBattleGlobal:isArenaZLSStage()
    local battleType = WBattleGlobal:getCurrent().m_nBattleType
    local battleChannel = WBattleGlobal:getCurrent().m_tMakePairOk.battleChannle
    
    if battleType == BattleConstants.g_nBATTLE_TYPE_NORMAL then
        return battleChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_ZLS
    end
    return false
...
--@brief 是否组队世界Boss副本
function WBattleGlobal:isWorldTeamBossStage()
...
--@brief 是否夫妻争霸副本
function WBattleGlobal:isCoupleHegemonyStage()
...
function WBattleGlobal:isGhostStage()
    -- body
    return WBattleGlobal:getCurrent().m_bIsGhost
end
...
--@brief 是否双人爬塔副本
function WBattleGlobal:isDoubleTowerStage()
    if WBattleGlobal:getCurrent().m_nBattleType == BattleConstants.g_nBATTLE_TYPE_BOSS and WBattleGlobal:getCurrent().battleMode == BattleConstants.g_tBossBattleMode.MODE_DOUBLETOWER_STAGE then
        return true
    end
    return false
end
...
--@brief    是否遗迹副本
function WBattleGlobal:isRemainStage()
    -- body
    if WBattleGlobal:getCurrent().m_nBattleType == BattleConstants.g_nBATTLE_TYPE_BOSS and WBattleGlobal:getCurrent().battleMode == BattleConstants.g_tBossBattleMode.MODE_REMAINSBOSS then
        return true
    end
...
--@brief    是否挑战岛主副本
function WBattleGlobal:isHostChallengeStage()
    -- local battleChannle = WBattleGlobal:getCurrent().m_tMakePairOk.battleChannle
    -- if battleChannle == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_DZTZ then
    --     return true
    -- end
    -- return false
    if WBattleGlobal:getCurrent().m_nBattleType == BattleConstants.g_nBATTLE_TYPE_BOSS and WBattleGlobal:getCurrent().battleMode == BattleConstants.g_tBossBattleMode.MODE_ISLANDOWNER_STAGE then
        return true
    end
    return false
end

--@brief    是否可以显示自动战斗按钮的战斗
function WBattleGlobal:isShowAutoFightBtnFight()
    -- body
    if WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_SINGLE) or WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_DAILY) or WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_TOWER) or WBattleGlobal:getCurrent():isGuildBossStage() or WBattleGlobal:getCurrent():isHeroTowerStage() or WBattleGlobal:getCurrent():isRemainStage() then
        return true 
    end
...
--@brief    判断是否竞技场战斗
function WBattleGlobal:isArenaFighting()
    -- body
    if WBattleGlobal:getCurrent():isArenaPWStage() or WBattleGlobal:getCurrent():isArenaStage() then 
        return true
    end
...
--@brief    判断是否挖坑赛战斗
function WBattleGlobal:isDigGappingFighting()
    -- body
    local battleType = WBattleGlobal:getCurrent().m_nBattleType
    local battleChannle = WBattleGlobal:getCurrent().m_tMakePairOk.battleChannle
    local battleMode = WBattleGlobal:getCurrent().m_tMakePairOk.battleMode
    if battleType == BattleConstants.g_nBATTLE_TYPE_NORMAL then
        if battleChannle == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_YL and battleMode == GlobalGame.g_tBattleMode.BATTLE_MODE_WK then 
            return true
        end
    end
    return false
...
--@brief 是否觉醒组队副本
function WBattleGlobal:isAwakeTeamStage()
    if WBattleGlobal:getCurrent().m_nBattleType == BattleConstants.g_nBATTLE_TYPE_BOSS and WBattleGlobal:getCurrent().battleMode == BattleConstants.g_tBossBattleMode.MODE_BOSSMAP_2 then
        local copyData = GDatatab_team_map["id_" .. WBattleGlobal:getCurrent().m_tMakePairOk.mapId]
        if copyData and copyData.difficulty == 4 then 
            return true
        end
    end
    return false
...
--@brief    判断是否娱乐赛战斗
function WBattleGlobal:isAmuseFighting()
    -- body
    local battleType = WBattleGlobal:getCurrent().m_nBattleType
    local battleChannle = WBattleGlobal:getCurrent().m_tMakePairOk.battleChannle
    local battleMode = WBattleGlobal:getCurrent().m_tMakePairOk.battleMode
    WZLog("WBattleGlobal:isAmuseFighting", battleType, battleChannle, battleMode)
    if battleType == BattleConstants.g_nBATTLE_TYPE_NORMAL then
        if battleChannle == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_YL or battleMode == GlobalGame.g_tBattleMode.BATTLE_MODE_LD then 
            return true
        end
    end
    return false
...
function WBattleGlobal:buildHero(reg, nIndex, bMonster)
   ...
   -- end
    ...
    hero.professionId = reg.professionId and reg.professionId[nIndex] or 0
    ...
    elseif WBattleGlobal:getCurrent():isTeamStage() and hero.m_nCamp ~= 0 then 
        hero.m_bOffFrozen = true
...
function WBattleGlobal:buildGuaiWithSuit(reg, nIndex, index)
 ...
 --    hero:getAI():setHero(hero)
    ...
    if monsterData.suitConfig and monsterData.suitConfig ~= -1 then
        ...
        suitConfig.robotKMSkill = {-1}
        suitConfig.robotAttackSkill = {-1}
    else
        suitConfig.sex = 0
    ...
    hero:setAI(BattleAiCtrl:new(reg.guaiBattleId[index]))
    hero:getAI():setHero(hero)
    hero:buildGuaidAiCombination(suitConfig.robotSkill, suitConfig.robotItem, suitConfig.robotKMSkill, suitConfig.robotAttackSkill)
    return hero
end
...
function WBattleGlobal:buildGuai(reg, nIndex, index)
    ...
    --着装的AI脚本控制的小怪
    elseif WBattleGlobal:getCurrent():isSingleStage() or WBattleGlobal:getCurrent():isDoubleTowerStage() or WBattleGlobal:getCurrent():isHostChallengeStage() then
    ...
    elseif reg.guaiBattleId[nIndex] ~= -1 then
        ...
        if monsterData.type == MonsterType.BOSS_CAGE or monsterData.type == MonsterType.BOSS_LASER 
            or monsterData.type == MonsterType.BOSS_MAGMA or monsterData.type == MonsterType.BOSS_POISON or monsterData.type == MonsterType.TREAT_ARRAY then
            ...
            if monsterData.type == MonsterType.TREAT_ARRAY then 
                param.camp = 0
            end
...
--@param	isCloseShoot:皮肤近身攻击
--@return	#1:创建的子弹表
function WBattleGlobal:buildBullet(nPlayerId,nStartX,nStartY,fSpeedX,fSpeedY,isSpatter, bulletIndex, isCloseShoot)
	local hero = nil
    ...
    local isPenetrateMonster = false
    if hero:isDevilGuai() then 
        isPenetrateMap = true
        isPenetrateMonster = true
    elseif hero:getIsKid() or hero:getIsSubHero() then 
    	isPenetrateMap = true
        if hero:getIsSubHero() and hero:getSubType() == CharacterSubType.SUBTYPE_BCHESS and isSpatter then 
            isPenetrateMap = false
        end
    elseif hero:getUseSkinBigSkill() and (hero:getSkinBigSkill() == 3030 or hero:getSkinBigSkill() == 3046 or hero:getSkinBigSkill() == 3054 or hero:getSkinBigSkill() == 3058 or hero:getSkinBigSkill() == 3061) and isSpatter ~= true then --宇航员子弹处理 开始发送时不发生碰撞,直到到达目标点爆炸
        isPenetrateMap = true
        if hero:getSkinBigSkill() ~= 3054 and hero:getSkinBigSkill() ~= 3061 then 
        	isPenetrateMonster = true
        end
    end    
    local bullet = WBullet:buildBullet({x=nStartX,y=nStartY},{x=fSpeedX,y=fSpeedY},accele,hero,isPenetrateMap,isSpatter, bulletIndex, isCloseShoot)
    ...
    if hero:getUseSkinBigSkill() and (hero:getSkinBigSkill() == 3030 or hero:getSkinBigSkill() == 3046) and isSpatter ~= true then --宇航员子弹处理 开始发送时不发生碰撞,直到到达目标点爆炸
        ...
        bullet:addCollisionCharas(WBattleGlobal:getCurrent():getKidSortList())
	...
	--皮肤大招 丘比特子弹追踪
	if hero:getUseSkinBigSkill() and hero:getSkinBigSkill() == 3058 then
		bullet = WSkinFollowEffect:buildEffect(bullet)
	end
...
function WBattleGlobal:buildMachine(machineType,param)
    ...
    elseif machineType == MonsterType.TREAT_ARRAY then 
        machine  = WBattleMachineBuffTreatArray:new(param.battleId,param.templateId,param.camp,param.bronPos)
        self.m_tMachines[machine.m_nBattleId] = machine
    elseif machineType == MonsterType.FIRE_TOTEM then
        machine  = WBattleMachineFireTotem:new(param.battleId,param.templateId,param.camp,param.bronPos, param.userAttack)

        self.m_tMachines[machine.m_nBattleId] = machine

        WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.FIRE_TOTEM)
    elseif machineType == MonsterType.BLACK_HOLE then 
    	machine  = WBattleMachineBlackHole:new(param.battleId,param.templateId,param.camp,param.bronPos)
        self.m_tMachines[machine.m_nBattleId] = machine
    end

    -- 守护光环
    if machineType == MonsterType.GUARDIAN_TOTEM then
        machine  = WBattleMachineGuardianTotem:new(param.battleId,param.templateId,param.camp,param.bronPos, param.userAttack)
        self.m_tMachines[machine.m_nBattleId] = machine
        WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.GUARDIAN_TOTEM)
    end
    -- 海洋领域增益
    if machineType == MonsterType.MARITIME1_TOTEM then
        machine  = WBattleMachineMaritime1Totem:new(param.battleId,param.templateId,param.camp,param.bronPos, param.userAttack)
        self.m_tMachines[machine.m_nBattleId] = machine
        WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.MARITIME1_TOTEM)
    end
    -- 海洋领域减益束缚
    if machineType == MonsterType.MARITIME2_TOTEM then
        machine  = WBattleMachineMaritime2Totem:new(param.battleId,param.templateId,param.camp,param.bronPos, param.userAttack)
        self.m_tMachines[machine.m_nBattleId] = machine
        WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.MARITIME2_TOTEM)
    end
    -- 海洋领域减益扣血
    if machineType == MonsterType.MARITIME3_TOTEM then
        machine  = WBattleMachineMaritime3Totem:new(param.battleId,param.templateId,param.camp,param.bronPos, param.userAttack)
        self.m_tMachines[machine.m_nBattleId] = machine
        WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.MARITIME3_TOTEM)
    end
    -- 姜子牙结界
    if machineType == MonsterType.JIANGZIYA_TOTEM then
        machine  = WBattleMachineJiangZiYaTotem:new(param.battleId,param.templateId,param.camp,param.bronPos, param.userAttack)
        self.m_tMachines[machine.m_nBattleId] = machine
        WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.JIANGZIYA_TOTEM)
    end
    -- 玄奘佛光
    if machineType == MonsterType.XUANZANG_TOTEM then
        machine = WBattleMachineXuanZang:new(param.battleId,param.templateId,param.camp,param.bronPos, param.userAttack)
        self.m_tMachines[machine.m_nBattleId] = machine
    end
    -- 执伞圣女
    if machineType == MonsterType.UMBRELLA1_TOTEM then
        machine = WBattleMachineUmbrella1:new(param.battleId,param.templateId,param.camp,param.bronPos, param.userAttack)
        self.m_tMachines[machine.m_nBattleId] = machine
        WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.UMBRELLA1_TOTEM)
    end
    -- 执伞圣女
    if machineType == MonsterType.UMBRELLA2_TOTEM then
        machine = WBattleMachineUmbrella2:new(param.battleId,param.templateId,param.camp,param.bronPos, param.userAttack)
        self.m_tMachines[machine.m_nBattleId] = machine
...
function WBattleGlobal:singleStageEnd(isWin)
    ...
    local myHero = WBattleGlobal:getCurrent():getMyHero()
    if myHero.m_nMaxRoundHurt < myHero.m_nCurRoundHurtTotal then 
        myHero.m_nMaxRoundHurt = myHero.m_nCurRoundHurtTotal
...
function WBattleGlobal:updateNameInfoIcon(dt)
    ...
    BattleKidName:releaseTransform()
    for i, v in pairs(self:getKidsList()) do
        if v:getPlayerNameIcon() ~= nil then
            v:getPlayerNameIcon():update()
        end
    end
    --灵魂分身
    for i, v in pairs(self:getSoulHerosList()) do
        if v:getPlayerNameIcon() ~= nil then
            v:getPlayerNameIcon():update()
        end
    end
...
function WBattleGlobal:updateTouchCircle()
    ...
    local scaleCircle = (300 + BattleConstants.g_nTouchDistance * 2 * 1)/680 / 3.3 / 0.6 * scalef/scaleMonsterMode
    local distanceLen = (150 + BattleConstants.g_nTouchDistance*scaleCircle/2)
    local distanceX = math.abs(pos.x - width) > pos.x and (pos.x-distanceLen) or (pos.x+distanceLen)
    local distanceY = pos.y - distanceLen
--    WZLog("WBattleGlobal:updateTouchCircle",distanceX,distanceY,pos.x,pos.y, scaleCircle, distanceLen)
    if (distanceX < 0 or distanceX > width or distanceY < 0) and scale == 1 then
        WndBattleHud:onScale(nil,true,nil,2)
    elseif (distanceX >= 0 and distanceX <= width and distanceY >= 0) and scale == 2 then
        WndBattleHud:onScale(nil,nil,true,1)
    end
end
...
function WBattleGlobal:update(dt)
	...
	--更新小孩
    for id,kid in pairs(self:getKidsList()) do
        --WZLog("updateMachine",kid:getPosition().x,kid:getPosition().y)
        kid:update(dt)
    end
    --更新灵魂分身
    for id, soulHero in pairs(self:getSoulHerosList()) do
        --WZLog("updateMachine",soulHero:getPosition().x,soulHero:getPosition().y)
        soulHero:update(dt)
    end
    ...
    --更新皮肤大招阴影中高亮元素位置
    local tBrightnessElement = WndBattleHud:getBrightnessElement()
    if tBrightnessElement and #tBrightnessElement > 0 then
        for i=1,#tBrightnessElement do
            local tag = tBrightnessElement[i]:getTag()
            for id,guai in pairs(WBattleGlobal:getCurrent():getGuaiList()) do
                local element = guai:getAnimation():getAnimNode()
                local nScaleGuai = guai.m_nScale and guai.m_nScale or 1
                if tag == id then
                    ...
                    pos.x = pos.x + (0.5 - anchorPt.x)*elementSize.width * nScaleGuai
                    pos.y = pos.y + (0.5 - anchorPt.y)*elementSize.height * nScaleGuai
                    tBrightnessElement[i]:setPosition(pos)
                end
            end
    ...
    --更新皮肤大招阴影中敌人按钮
    local tBtnEnemyElement = WndBattleHud:getBtnEnemyElement()
    if tBtnEnemyElement and #tBtnEnemyElement > 0 then
        for i=1,#tBtnEnemyElement do
            local tag = tBtnEnemyElement[i]:getTag()
            for id,guai in pairs(WBattleGlobal:getCurrent():getGuaiList()) do
                local element = guai:getAnimation():getAnimNode()
                local nScaleGuai = guai.m_nScale and guai.m_nScale or 1
                if tag == id then
                    ...
                    pos.x = pos.x + (0.5 - anchorPt.x)*elementSize.width * nScaleGuai
                    pos.y = pos.y + (0.5 - anchorPt.y)*elementSize.height * nScaleGuai
                    tNewPos = WndBattleHud:getBigSkillShadedElement():convertToNodeSpace(GlobalMethod:ccp(pos.x,pos.y))
                    tBtnEnemyElement[i]:setPosition(pos)
                end
            end
            for id,guai in pairs(WBattleGlobal:getCurrent():getHeroList()) do
                local element = guai:getAnimation():getAnimNode()
 ...
 --        ProtocolProcessorBattleInterface:send_SYSTEM_BattleShakeHands(self.m_tMakePairOk.battleId)
    ...
    BattleProfessionSkillManager:update()
    ...
    if WBattleGlobal:getCurrent():isGhostStage() then 
        WBattleGlobal:getCurrent():updateRobotPickSkillBox(dt)
        WBattleGlobal:getCurrent():robotUseGhostSkill(dt)
    end
...
function WBattleGlobal:checkMachineLight()
    ...
    if WBattleGlobal:getCurrent().m_nBattleType == BattleConstants.g_nBATTLE_TYPE_BOSS then
        if WBattleGlobal:getCurrent().m_tMakePairOk.mapId == 20501 or WBattleGlobal:getCurrent().m_tMakePairOk.mapId == 20502 or WBattleGlobal:getCurrent().m_tMakePairOk.mapId == 20503 or WBattleGlobal:getCurrent().m_tMakePairOk.mapId == 20504 then
            isCheck = true
        end
    end
    if not isCheck then
    ...
    for i,v in pairs(WBattleGlobal:getCurrent():getHeroList()) do
        if not v:isDead() and not v.m_bLoseNet and v:getCamp() == 0 then
...
function WBattleGlobal:sendHurtProtocol(idWhoSend,charas,values,distance,critType,isPetAttack,hurtParm, superCritMark)
    ...
    local tempChara = senderChara
    if senderChara:getIsKid() then 
        WZLog("WBattleGlobal:sendHurtProtocol zero-0", tempChara:isCanControl(), tempChara.m_bLoseNet)
        tempChara = senderChara:getOwner()
    end
    WZLog("WBattleGlobal:sendHurtProtocol zero-1", idWhoSend, canSendMonster, tempChara:isCanControl(), tempChara.m_bLoseNet)
	if not canSendMonster and not (tempChara:isCanControl() or tempChara.m_bLoseNet) then
        WZLog("WBattleGlobal:sendHurtProtocol zero-2")
		return
	end
	local hurtCount = 0
    ...
    local tSuperCritMark = WZLuaVector_int_:create()
	...
	for id,chara in pairs(charas) do
        ...
        if (isPetAttack ~= nil and isPetAttack == true) or distance == nil  then
            tDistance:push(0)
            tCritType:push(0)
            tSuperCritMark:push(0)
        else
            tDistance:push(distance[id])
            tCritType:push(critType[id])
            if g_nSuperCritHappenMark == 0 then 
                tSuperCritMark:push(superCritMark and superCritMark[id] and superCritMark[id] or 0)
                if superCritMark and superCritMark[id] and superCritMark[id] == 1 then 
                    g_nSuperCritHappenMark = g_nSuperCritHappenMark + 1
                end
            elseif g_nSuperCritHappenMark == -1 then 
                tSuperCritMark:push(superCritMark and superCritMark[id] and superCritMark[id] or 0)
            else
                tSuperCritMark:push(0)
            end
	...
	if true or hurtCount > 0 or hurtGuaiCount > 0 then
        ...
        ProtocolProcessorBattleInterface:send_BATTLE_Hurt(self.m_tMakePairOk.battleId, idWhoSend, hurtIds, hurtValues, tDistance, tSuperCritMark)
	end
end
...
--@param	values:受伤值
--@param	isPetAttack:是否宠物攻击
--@note 	一发子弹一个玩家可以传递多个伤害值
function WBattleGlobal:sendHurtProtocolMul(idWhoSend,charas,values,distance,critType,isPetAttack,hurtParm, superCritMark)
    local senderChara = self:getCharacterWithId(idWhoSend)
    local canSendMonster = false
    canSendMonster = WMonster:canSendHurtByType(senderChara.m_nMonsterType)
    local tempChara = senderChara
    if senderChara:getIsKid() or senderChara:getIsSubHero() then 
        WZLog("WBattleGlobal:sendHurtProtocolMul zero-0", tempChara:isCanControl(), tempChara.m_bLoseNet)
        tempChara = senderChara:getOwner()
    end
    WZLog("WBattleGlobal:sendHurtProtocolMul zero-1", idWhoSend, canSendMonster, tempChara:isCanControl(), tempChara.m_bLoseNet)
	if not canSendMonster and not (tempChara:isCanControl() or tempChara.m_bLoseNet) then
        WZLog("WBattleGlobal:sendHurtProtocolMul zero-2")
		return
	end
	local hurtCount = 0
	local hurtIds = WZLuaVector_int_:create()
    ...
    local hurtType = 0
    WZLog("WBattleGlobal:sendHurtProtocolMul zero", tostring(isPetAttack), tostring(distance))
    if isPetAttack ~= nil and isPetAttack == true then
        hurtType = -2
    elseif distance == nil then
	...
	for id = 1, BattleCommon:tableLen(charas) do
		local chara = charas[id]
        invincibleRound:push(0)
        defend:push(chara:getDefence())
        injuryFree:push(chara:getInjuryFree())
        isHide:push(chara:isHide())
        ...
        local ttDistance = VectorToTable(tDistance)
        local ttCritType = VectorToTable(tCritType)
        --WZLog("WBattleGlobal:sendHurtProtocolMul_zero1", hurtType, hurtCount, tostring(tHurtIds[hurtCount]), tostring(tHurtValues[hurtCount]), tostring(ttDistance[hurtCount]), tostring(ttCritType[hurtCount]))
        WZLog("WBattleGlobal:sendHurtProtocolMul_zero2", Serialize(tHurtIds), Serialize(tHurtValues), Serialize(ttDistance))
        local isHaveSkill = false
        if targetRandom ~= nil then
            local isS1EqualS2 = false
            if chara.m_tWeaponSkillType ~= nil and chara.m_tWeaponSkillType[1] ~= nil and chara.m_tWeaponSkillType[2] ~= nil and chara.m_tWeaponSkillType[1] == chara.m_tWeaponSkillType[2] then
            ...
            for i = 1, 2 do
                    ...
                    if id == v[1] then
                        if isS1EqualS2 == false then
                            if chara.m_tWeaponSkillType ~= nil and chara.m_tWeaponSkillType[i] == v[2] then
                                ttargetRandom:push(v[3] - 1)
                                isHaveSkill = i
                                --WZLog("WBattleGlobal:sendHurtProtocolMul three-1", id, i, j, tostring( chara.m_tWeaponSkillType[i]), tostring(v[3] - 1), #chara.m_tWeaponSkillType)
                            end
                        else
                            if isExist == false and i == 1 and chara.m_tWeaponSkillType ~= nil and chara.m_tWeaponSkillType[i] == v[2] then
                                isExist = true
                                ttargetRandom:push(v[3] - 1)
                                isHaveSkill = i
                                --WZLog("WBattleGlobal:sendHurtProtocolMul three-2", id, i, j, tostring( chara.m_tWeaponSkillType[i]), tostring(v[3] - 1), #chara.m_tWeaponSkillType)
                            elseif isExist == false and i == 2 and chara.m_tWeaponSkillType[i] == v[2] then
                                isExist = true
                            elseif isExist == true and i == 2 and chara.m_tWeaponSkillType[i] == v[2] then
                                isExist = true
                                ttargetRandom:push(v[3] - 1)
                                isHaveSkill = i
                                --WZLog("WBattleGlobal:sendHurtProtocolMul three-3", id, i, j, tostring( chara.m_tWeaponSkillType[i]), tostring(v[3] - 1), #chara.m_tWeaponSkillType)
                            end
                        end
                    end
                end
                if isHaveSkill == false then
                    ttargetRandom:push(-1)
                    --WZLog("WBattleGlobal:sendHurtProtocolMul four", i,  tostring( chara.m_tWeaponSkillType[i]))
                end
            end
        else
            for i = 1, 2 do
        ...
        for i = 1, 2 do
            ...
            for j, v in pairs(attackerRandom) do
                if senderChara.m_tWeaponSkillType[i] == v[2] then
                    tattackerRandom:push(v[3] - 1)
                    isHaveSkill = i
                    --WZLog("WBattleGlobal:sendHurtProtocolMul five", i, j, tostring( senderChara.m_tWeaponSkillType[i]), v[2], tostring(v[3] - 1), #senderChara.m_tWeaponSkillType)
                end
            end
            if isHaveSkill == false then
                tattackerRandom:push(-1)
                --WZLog("WBattleGlobal:sendHurtProtocolMul six", i,tostring( senderChara.m_tWeaponSkillType[i]))
            end
        end
    else
        for i = 1, 2 do
...
function WBattleGlobal:getMachinesSortList()
    ...
    local sortFunc = function(a, b) return a:getBattleId() < b:getBattleId() end
...
function WBattleGlobal:getHeroWithId(nId)
	return self.m_tHeros[nId] or self.m_tExitHeros[nId] or self.m_tSoulHeros[nId]
end
...
--@brief    获取英雄(组队副本觉醒)
--@note     将敌方英雄排除在外
function WBattleGlobal:getRealHero()
    local tHeros = {}
    
    for id, hero in pairs(self.m_tHeros) do
        if self:isMyTeam(hero:getId()) then
            table.insert(tHeros, hero)
        end
    end
...
function WBattleGlobal:isCopyTeach()
    ...
    if TeachGroup1:isTeach() and isCopyTeach == nil then
        local mapId = self.m_tMakePairOk.mapId
        if mapId == 20101 and (not WBattleGlobal:getCurrent():isSingleStage() and not WBattleGlobal:getCurrent():isHostChallengeStage()) then
...
--@brief	是否有角色受伤(包括英雄和怪物)
--@param 	exceptMachines:true排除掉机关怪
--@param 	includeKid:是否包小孩
--@param 	includeSoulHero:是否包含灵魂分身
--@return	#1:true,false
function WBattleGlobal:getCharacterList(exceptMachines, includeKid, includeSoulHero)
    local characterList = {}
    for key,hero in pairs(self:getHeroList()) do
    ...
    if includeKid then 
        for key, kid in pairs(self:getKidsList()) do
            table.insert(characterList, kid)
        end
    end
    if includeSoulHero then 
        for key, souHero in pairs(self:getSoulHerosList()) do
            table.insert(characterList, souHero)
        end
    end
...
function WBattleGlobal:getCharacterWithId(nId)
    ...
    elseif self.m_tKids[nId] ~= nil then 
        return self.m_tKids[nId]
    elseif self.m_tSoulHeros[nId] ~= nil then 
        return self.m_tSoulHeros[nId]
    else
...
--@param    辅助技能ID
--@return   辅助技能属性表
function WBattleGlobal:getKMSkillById(skillId)
    return self.m_tKMSkillList[skillId]
end

--@brief    获取普攻技能属性表
--@param    普攻技能ID
--@return   普攻技能属性表
function WBattleGlobal:getAttackSkillById(skillId)
    return self.m_tAttackSkillList[skillId]
end
...
--@brief    增加引用计数
function WBattleGlobal:release()
    self.m_nReference = self.m_nReference - 1
end
...
--@brief 结束回合
--@param    isEnterBattleTime: 是否进入战斗那一次
function WBattleGlobal:endCurRound(battleId,noteId, isForce, isNotEndCurRound,isOnlyEndCurRound, isEnterBattleTime)
    ...
    if isNotEndCurRound ~= true then
        ...
        msg.m_nbIsFirstMsg = isEnterBattleTime or false 
        MsgManager:pushBlockMsg(msg)
    end
end
...
function WBattleGlobal:addFirstBuff()
    ...
    WZLog("WBattleGlobal:addFirstBuff1", Serialize(WBattleGlobal:getCurrent().m_tMakePairOk.buffId), WBattleGlobal:getCurrent().m_tMakePairOk.buffId[1])
    local nIndex = 1
    for i = 1, WBattleGlobal:getCurrent().m_tMakePairOk.playerCount do
        local nBuffNum = WBattleGlobal:getCurrent().m_tMakePairOk.buffId[nIndex]
        local playerId = WBattleGlobal:getCurrent().m_tMakePairOk.playerId[i]
        local hero = self:getCharacterWithId(playerId)
        if nBuffNum and nBuffNum > 0 then 
            for k = nIndex + 1, nIndex + nBuffNum do
                local v = WBattleGlobal:getCurrent().m_tMakePairOk.buffId[k]
            ...
            nIndex = nIndex + nBuffNum + 1
        else
            nIndex = nIndex + 1
        end
    end

    --岛主挑战,抢岛主时给敌方岛主和助战加上buff,服务端也对应加上了
    if WBattleGlobal:getCurrent():isHostChallengeStage() and SceneBattleLoading:getPlayerCampCount() > 1 then
        local landlordConfig = json.decode(CacheCenter:getGameParam().landlordConfig)
        local islandOwnerSkill = string.sub(landlordConfig.islandOwnerSkill, 2, -2) 
        local tBuffId = SplitStringWithSeparator(islandOwnerSkill, ",")
        for i=1,#tBuffId do
            local playerIds = WBattleGlobal:getCurrent().m_tMakePairOk.playerId
            for j=1,#playerIds do
                if not self:isMyTeam(playerIds[j]) then
                    local hero = self:getCharacterWithId(playerIds[j])
                    if hero then
                        local buff = WBattleGlobal:addBuff({hero},tBuffId[i],playerIds[j],0)
                    end
                end
            end
        end
...
function WBattleGlobal:synchronousBattleInfo(
    ...
    for id, playerId in ipairs (playerIds) do
        ...
        if isExist == false and hp[id] > 0 then
            ...
            elseif monsterType == MonsterType.XUANZANG_TOTEM then
                -- 玄奘佛光
                local machine = WBattleGlobal:getCurrent():buildMachine(MonsterType.XUANZANG_TOTEM,{battleId = battleId,templateId = dataIds[id],camp = campId})
                SceneBattle:getFrontLayer():addChild(machine.m_anim:getAnimNode())
            elseif monsterType == MonsterType.TREAT_TOTEM or monsterType == MonsterType.BUFF_TOTEM or monsterType == MonsterType.FIRE_TOTEM or monsterType == MonsterType.GUARDIAN_TOTEM or monsterType == MonsterType.BLACK_HOLE or monsterType == MonsterType.MARITIME1_TOTEM or monsterType == MonsterType.MARITIME2_TOTEM or monsterType == MonsterType.MARITIME3_TOTEM or monsterType == MonsterType.JIANGZIYA_TOTEM or monsterType == MonsterType.UMBRELLA1_TOTEM or monsterType == MonsterType.UMBRELLA2_TOTEM then
                --治疗图腾 攻击图腾 火焰图腾 守护光环 海洋领域增益,海洋领域减益
    ...
    for id, playerId in ipairs (playerIds) do
        for i, hero in pairs (list) do
            if hero:getBattleId() == playerId then
                ...
                hero:setExtraHp(extraHP[id], true)
            ...
            --    if playerId == WBattleGlobal:getCurrent():getMyBattleId() and hero:isServerDead() == false and isOutOfScene then
                if hero:isServerDead() == false and isOutOfScene then
                    ProtocolProcessorBattleInterface:send_BATTLE_OutOfScene(WBattleGlobal:getCurrent().m_tMakePairOk.battleId, hero:getBattleId() ,hero.m_nPlayerId)   
                end
        ...
        for i, playerId in ipairs (explodePlayerId) do
            ...
            for id=1,explodePosNum[i] do
                ...
                if hero then
                    ...
                    local explodeDir = explodeDirection[startIndex+id]
                    WZLog("WBattleGlobal:synchronousBattleInfo seven", i, id, playerId, explodeSkillId[i], explodePosNum[i], pos.x, pos.y, rect.x, rect.y)
                    BattleMapManager:drawBroke(Vector2:create(pos.x, pos.y), breakCircle, breakCircleMark, rect.x, rect.y, nil, explodeDir)
                end
            end
        end
    end
...
function WBattleGlobal:isMyWeaponHaveSkill(skillType)
    ...
    for i ,skillId in pairs (WBattleGlobal:getCurrent().m_tMySkill_Beginning.id) do
        local skillInfo = GDatatab_skill["id_"..skillId]
        if skillInfo and skillInfo.sub_type == skillType then
...
function WBattleGlobal:petSkillUse()
    ...
    --职业被动技能使用
    BattleProfessionSkillManager:triggerProfessionPassiveSkill(list)
    --玩家身上buff的被动效果, 如：神佑再生buff
    BattleProfessionSkillManager:triggerBuffPassiveSkill(list)
    --本回合出手的灵魂/棋圣分身的职业被动技能使用
    local soulHerolist = {}
    local currentCharacter = WBattleGlobal:getCurrent():getCurrentCharacter()
    for i, hero in pairs(WBattleGlobal:getCurrent():getSoulHerosList()) do
        --灵魂分身
        if not hero:isDead() and hero:getCamp() ~= currentCharacter:getCamp() and hero:getSubType() == CharacterSubType.SUBTYPE_SOUL then
            table.insert(soulHerolist, hero)
            table.insert(WBattleGlobal:getCurrent().m_tCurRoundSoulHeroShootId, hero:getBattleId())
        elseif not hero:isDead() and hero:getOwnerPlayerId() == currentCharacter:getBattleId() and hero:getSubType() == CharacterSubType.SUBTYPE_BCHESS then --棋圣黑分身（只处理需要攻击的黑分身）
            table.insert(soulHerolist, hero)
        end
    end
    BattleProfessionSkillManager:triggerProfessionPassiveSkill(soulHerolist)
end
...
--@brief 第一章第三关引导
function WBattleGlobal:isChapterOne_ThreeTeach()
    local isChapterOneTeach = self.m_bIsChapterOne_ThreeTeach

    if TeachGroup1:isTeach() and isChapterOneTeach == nil then
        local mapId = self.m_tMakePairOk.mapId
        if mapId == 10103 and CacheCenter:getPlayerInfo().level <= 3 and not TeachGroup1:isTaskTeachFinish(10103) then
            isChapterOneTeach = true
            self.m_bIsChapterOne_ThreeTeach = true
        else
            isChapterOneTeach = false
            self.m_bIsChapterOne_ThreeTeach = false
        end
        WZLog("WBattleGlobal:isChapterOneTeach", isChapterOneTeach, mapId, CacheCenter:getPlayerInfo().level)
    end
    return isChapterOneTeach
...
--@brief    获取当前大招的皮肤形象
--@param    skinBigSkill: 皮肤大招
function WBattleGlobal:getSkinBigSkillShape(skinBigSkill)
    -- body
    for i, data in pairs(g_tShapeBigSkillConfig) do
        if data.active_skill[1][1] == skinBigSkill then 
            return data
        end
    end
...
--@brief    获取皮肤大招有效作用的对象
function WBattleGlobal:getSomeSkinBigSkillTarget(hero)
    -- body
    local charaList = {}
    local tempCamp = hero:getCamp()
    local tBoss = WBattleGlobal:getCurrent():getBossList()
    for i, boss in pairs(tBoss) do
        if boss:getCamp() ~= tempCamp and not boss:isDead() then 
            table.insert(charaList, boss)
        end
    end
    local heroList = WBattleGlobal:getCurrent():getHeroList()
    for i, chara in pairs(heroList) do
        if chara:getCamp() ~= tempCamp and not chara:isDead() then 
...
--@param    nIndex:怪物表ID
--@return   #1:创建的怪物表
function WBattleGlobal:buildDevilGuai(playerId, nIndex, battleId)
    WZLog("WBattleGlobal:buildDevilGuai", nIndex, battleId)
    local monsterData = BossData["id_"..nIndex]
    --============================================================
    ...
    local index = getPlayerIndex(playerId)

    local suitConfig = {}
    suitConfig.sex = makePairOk.playerSex[index]
    suitConfig.headId = makePairOk.headId[index]
    suitConfig.faceId = makePairOk.faceId[index]
    suitConfig.bodyId = makePairOk.bodyId[index]
    suitConfig.weaponId = makePairOk.weaponId[index]
    suitConfig.wingId = makePairOk.wingId[index]

    local heroReal = WBattleGlobal:getCurrent():getCharacterWithId(playerId)
    local suit_info = heroReal.suit_info

    if playerId == makePairOk.selfId then 
        suitConfig.robotSkill = CopyTable(WBattleGlobal:getCurrent().m_tMySkill_Beginning.id)
        suitConfig.robotItem = CopyTable(WBattleGlobal:getCurrent().m_tMyProp_Beginning.id)
    else
        suitConfig.robotSkill = CopyTable(heroReal.m_tSkills)
        suitConfig.robotItem = CopyTable(heroReal.m_tItems)
    end
    ...
    WZLog("WBattleGlobal:buildDevilGuai three", tostring(suit_head), tostring(suit_face), tostring(suit_body), tostring(suit_weapon), tostring(suit_wing))
    local tEquipList = {}
    StringIntsertToTable(tEquipList,suit_head)
    StringIntsertToTable(tEquipList,suit_face)
    StringIntsertToTable(tEquipList,suit_body)
    ...
    local weapon = GDatatab_item["id_"..suitConfig.weaponId] or {animation_index_code=1,sub_type=1}
    local hero = WHero:buildHero(heroReal.m_tPlayerBodyInfo, suitConfig.sex, heroReal.m_tSuitInfo)
    --英雄武器类型
    hero.m_nWeaponType = GDatatab_item["id_"..suitConfig.weaponId].sub_type
    hero.suit_info = suit_info
--    hero.m_bIsGuaiWithSuit = true
    hero.m_tProfessionSkills = CopyTable(heroReal.m_tProfessionSkills)
    hero.m_tSkills = CopyTable(suitConfig.robotSkill)
    hero.m_tItems = CopyTable(suitConfig.robotItem)
    hero:setType(CharacterType.TYPE_HERO)
--    hero:setType(CharacterType.TYPE_GUAI)
    hero.m_nAiType = MonsterAiType.AI_ROBOT
    hero:setAI(BattleAiCtrl:new(battleId))
    hero:getAI():setHero(hero)
--    hero.m_bCanControl = true
    hero.m_nAiCtrlId = 1
    hero:buildAiCombination()
...
function WBattleGlobal:_init()
    ...
    g_battleGlobal.m_tSkinBigSkillId = {}
    g_battleGlobal.m_tSkinBigSkillList = nil
    g_battleGlobal.m_tSkinBigSkillTargetPos = nil
    g_battleGlobal.m_tKids = {}
    g_battleGlobal.m_tSoulHeros = {}
...
--@brief    回合开始重新添加霸体盾buff效果
--@note     宠物被动技能列表，每个回合都会清掉（这里特殊处理）
--@note     对所有buff效果有效,不再只针对霸体盾buff [174修改]
function WBattleGlobal:addBatiEffect()
    local heroList = self:getHeroSortList()
    for i, hero in pairs(heroList) do
        for id,buff in pairs (hero.m_tBuffChangeStateList) do
            for i,buffEffect in pairs (buff.m_nEffect) do
                local effect = buffEffect[3] .. "_" ..buffEffect[4]
                if effect == EffectTypeConfig.IMMUNITY_BUFF_ASSIGN then 
                    hero:addImmunityPetSkill(buff.m_nEffectId,{type = EffectTypeConfig.IMMUNITY_BUFF_ASSIGN,param = buffEffect[5]})
                end
                if effect == EffectTypeConfig.IMMUNITY_EFFECT_ASSIGN then 
                    hero:addImmunityPetSkill(buff.m_nEffectId,{type = EffectTypeConfig.IMMUNITY_EFFECT_ASSIGN,param = buffEffect[5] .. "_" .. buffEffect[6]})
                end
            end
        end
    end
...
--@brief    每帧检测机器幽灵拾取幽灵宝箱
function WBattleGlobal:updateRobotPickSkillBox(dt)
    local ghostBoxList = WndBattleHud:getGhostSkillBoxInMap()
    if ghostBoxList == nil or #ghostBoxList == 0 then 
        return 
    end

    for id, chara in pairs(WBattleGlobal:getCurrent():getHeroList()) do
        if chara ~= nil and chara:isRobot() and chara:getIsHero() and chara:isDead() and WBattleGlobal:getCurrent():isGhostStage() and chara:isCanControl() then
            WZLog("WBattleGlobal:updateRobotPickSkillBox two", dt)
            --幽灵移动
            local stepX
            local stepY
            local addYRate 
            local anim = chara:getAnimation()
            local touchPoint 
            chara.m_nGhostMoveGapping = chara.m_nGhostMoveGapping + dt
            local bIsAddMoveMsg = false 
            if chara.m_nGhostMoveGapping >= 0.1 then 
                chara.m_nGhostMoveGapping = chara.m_nGhostMoveGapping - 0.1 
                bIsAddMoveMsg = true
            end
            if bIsAddMoveMsg then 
                if chara.m_ghostSkillSpineBoxData == nil then 
                    local boxNum = #ghostBoxList
                    local randNumIndex = id % 10 + 1
                    local randomIndex = WBattleGlobal:getCurrent().m_tBattleRand[randNumIndex] % boxNum + 1
                    WZLog("WBattleGlobal:updateRobotPickSkillBox three", randomIndex, boxNum)
                    if ghostBoxList[randomIndex] then 
                        chara.m_ghostSkillSpineBoxData = ghostBoxList[randomIndex]
                        touchPoint = {x = chara.m_ghostSkillSpineBoxData.spineBox:getPositionX(), y = chara.m_ghostSkillSpineBoxData.spineBox:getPositionY()}
                    end
                else
                    touchPoint = {x = chara.m_ghostSkillSpineBoxData.spineBox:getPositionX(), y = chara.m_ghostSkillSpineBoxData.spineBox:getPositionY()}
                end
                if touchPoint then 
                    ...
                    if ptDis >= BattleMsgPlayerMoveCtrl.MIN_ENABLEMOVE_DISTANCE then
                        ...
                        WZLog("WBattleGlobal:updateRobotPickSkillBox four", addYRate)

                        local msg = MsgManager:createMsg(BattleMsgGhostMove)
                        msg.m_nBattleId = WBattleGlobal:getCurrent().m_tMakePairOk.battleId
                        msg.m_nPlayerId = chara:getBattleId()
                        msg.m_nCurrentPlayerId = chara:getBattleId()
                        msg.m_nMovecount = 1
                        msg.m_tMovestep = {}
                        msg.m_tMovestepY = {}
...
--@brief    每帧检测机器幽灵使用幽灵技能
function WBattleGlobal:robotUseGhostSkill(dt)
    if WBattleGlobal:getCurrent():isGameOver() == true or TeachGroup1.ISBATTLE then
        return
    end

    for id, chara in pairs(WBattleGlobal:getCurrent():getHeroList()) do
        if chara ~= nil and chara:isRobot() and chara:getIsHero() and chara:isDead() and WBattleGlobal:getCurrent():isGhostStage() and chara:isCanControl() and chara.m_tGhostSkillList then
            WZLog("WBattleGlobal:robotUseGhostSkill two", dt)
            --幽灵移动
            local stepX
            local stepY
            local addYRate 
            local anim = chara:getAnimation()
            local touchPoint 
            chara.m_nRobotUseGhostSkillGapping = chara.m_nRobotUseGhostSkillGapping + dt
            local bIsUseGhostSkill = false 
            if chara.m_nRobotUseGhostSkillRandTime == 0 then 
                chara.m_nRobotUseGhostSkillRandTime = math.random(4, 30)
            end
            if chara.m_nRobotUseGhostSkillGapping >= chara.m_nRobotUseGhostSkillRandTime then 
                chara.m_nRobotUseGhostSkillGapping = 0
                chara.m_nRobotUseGhostSkillRandTime = 0
                bIsUseGhostSkill = true
            end
            if bIsUseGhostSkill then 
                local tIndex = {}
                for i = 1, 3 do
                    if chara.m_tGhostSkillList.id[i] and chara.m_tGhostSkillList.id[i] > 0 then
                        WZLog("WBattleGlobal:robotUseGhostSkill one", chara.m_tGhostSkillList.id[i], i)
                        table.insert(tIndex, i) 
                    end
                end

                if #tIndex > 0 then 
                    local nSkillNum = #tIndex
                    local randNumIndex = chara:getBattleId() % 10 + 1
                    local randNumList = WBattleGlobal:getCurrent().m_tBattleRand
                    local nIndex = randNumList[randNumIndex] % nSkillNum + 1
                    local skillId = chara.m_tGhostSkillList.id[tIndex[nIndex]]
                    local ghostTargetId = WMonster:getGhostSkillTargetPlayer(chara, chara.m_tGhostSkillList.choose[tIndex[nIndex]]) 

                    WZLog("WBattleGlobal:robotUseGhostSkill three", nIndex, tIndex[nIndex], chara.m_tGhostSkillList.skillUniqueId[tIndex[nIndex]], ghostTargetId, skillId)
                    if ghostTargetId and skillId and skillId > 0 and BattleHeroUse:heroUse(chara:getBattleId(), BattleHeroUse.USE_GHOSTSKILL, skillId, nil, nil, nil, {ghostTargetId}, chara.m_tGhostSkillList.skillUniqueId[tIndex[nIndex]]) then
                        chara.m_tGhostSkillList.id[tIndex[nIndex]] = 0
                    end
                end
            end
        end
...
--@brief    获取小孩对象列表 
--@return   #1:小孩对象列表
function WBattleGlobal:getKidsList()
    return self.m_tKids
end

--@brief    获取小孩对象列表 （按id排序）
--@return   #1:角色对象列表
function WBattleGlobal:getKidSortList()
    local list = {}
    for id,hero in pairs(WBattleGlobal:getCurrent():getKidsList()) do
        WZLog("WBattleGlobal:getKidSortList", hero:getBattleId())
        table.insert(list,hero)
    end
    local sortFunc = function(a, b) return a:getBattleId() < b:getBattleId() end
    table.sort(list,sortFunc)
...
--@brief 移除机关
function WBattleGlobal:removeKid(battleId)
    for i,v in pairs(self.m_tKids) do
        local kid = v
        if kid.m_nBattleId == battleId then
            kid:destroy()
            self.m_tKids[battleId] = nil
            break
        end
    end
end
...
--@brief 创建机关
function WBattleGlobal:buildKid(owner, skillId, kidBattleId)
    local kid = WKid:buildKid(owner, skillId)
    local kidId = owner:getKidInfo().id
    kid.kidId = kidId
    kid.m_nBattleId = kidBattleId
    kid.m_nCamp = owner:getCamp()
    WZLog("WBattleGlobal:buildKid", kidBattleId)
    self.m_tKids[kidBattleId] = kid

    WBattleGlobal:getCurrent():setBuildMonsterRecord(kidBattleId, kidBattleId)

    return kid
end

--@brief    小孩射击
function WBattleGlobal:kidTakeShoot()
    -- body
    if self.m_tKids == nil then return end 

    for i, kid in pairs(self.m_tKids) do
        WZLog("WBattleGlobal:kidTakeShoot", i, kid:isDead(), kid:getShootState())
        if not kid:isDead() and kid:getShootState() == 1 then 
            kid:kidTakeShoot()
        end
    end
end
...
--@brief    获取子弹列表
--@return   #1:子弹列表
function WBattleGlobal:getBulletsListByChara(battleId)
    local tBullet = {}
    if self.m_tBullets == nil then return tBullet end 

    for i, bullet in pairs(self.m_tBullets) do
        if bullet.m_ownerChara:getBattleId() == battleId then 
            table.insert(tBullet, bullet)
        end
    end
...
--@brief    通过子弹Id移除子弹
--@param    nIndex:表下标
function WBattleGlobal:removeBulletById(nId)
    for i, bullet in pairs(self.m_tBullets) do
        if bullet.m_nId == nId then 
            table.remove(self.m_tBullets, i)
            break 
        end
    end
end
...
--@brief 	获取战斗风等级的上下限
function WBattleGlobal:getWindLevelLimit()
	return self.m_tWindLevelLimit
end

--@brief    获取灵魂分身对象列表 
--@return   #1:灵魂分身对象列表
function WBattleGlobal:getSoulHerosList()
    return self.m_tSoulHeros
end

--@brief    获取灵魂分身对象列表 （按id排序）
--@return   #1:角色对象列表
function WBattleGlobal:getSoulHerosSortList()
    local list = {}
    for id,hero in pairs(WBattleGlobal:getCurrent():getSoulHerosList()) do
        WZLog("WBattleGlobal:getSoulHerosSortList", hero:getBattleId())
        table.insert(list,hero)
    end
    local sortFunc = function(a, b) return a:getBattleId() < b:getBattleId() end
    table.sort(list,sortFunc)
...
--@brief 移除灵魂分身
function WBattleGlobal:removeSoulHero(battleId)
    for i,v in pairs(self.m_tSoulHeros) do
        local soulHero = v
        if soulHero.m_nBattleId == battleId then
            soulHero:destroy()
            self.m_tSoulHeros[battleId] = nil
            break
        end
    end
end
...
--@param 	shapeId:大招对应的皮肤Id
function WBattleGlobal:buildSoulHero(owner, monsterId, soulHeroBattleId, shapeId)
    local soulHero = WSoulHero:buildSoulHero(owner, monsterId, shapeId)
    soulHero.m_nBattleId = soulHeroBattleId
    soulHero.m_nCamp = owner:getCamp()
    soulHero:setType(CharacterType.TYPE_HERO)
    soulHero:setSubType(CharacterSubType.SUBTYPE_SOUL)
    WZLog("WBattleGlobal:buildSoulHero", soulHeroBattleId)
    self.m_tSoulHeros[soulHeroBattleId] = soulHero

    WBattleGlobal:getCurrent():setBuildMonsterRecord(soulHeroBattleId, soulHeroBattleId)

    return soulHero
end

--@brief    灵魂分身使用技能
function WBattleGlobal:soulHeroEquipSkill()
    -- body
    if self.m_tSoulHeros == nil then return end 
    WZLog("WBattleGlobal:soulHeroEquipSkill")
    local curHero = WBattleGlobal:getCurrent():getCurrentCharacter()
    local nCurCamp = curHero:getCamp()
    for i, soulHero in pairs(self.m_tSoulHeros) do
        local soulCamp = soulHero:getCamp()
        WZLog("WBattleGlobal:soulHeroEquipSkill", i, soulHero:isDead(), nCurCamp, soulCamp)
        if not soulHero:isDead() and nCurCamp ~= soulCamp and soulHero:getSubType() == CharacterSubType.SUBTYPE_SOUL then 
        	soulHero:doKidShoot()
        end
    end
end
...
--@brief    灵魂分身射击
function WBattleGlobal:soulHeroTakeShoot()
    -- body
    local shootHero = {}
    if self.m_tSoulHeros == nil then return shootHero end 

    local curHero = WBattleGlobal:getCurrent():getCurrentCharacter()
    local nCurCamp = curHero:getCamp()
    for i, soulHero in pairs(self.m_tSoulHeros) do
        local soulCamp = soulHero:getCamp()
        WZLog("WBattleGlobal:soulHeroTakeShoot", i, soulHero:isDead(), nCurCamp, soulCamp)
        if not soulHero:isDead() and nCurCamp ~= soulCamp and soulHero:getSubType() == CharacterSubType.SUBTYPE_SOUL then 
            table.insert(shootHero, soulHero)
        end
    end
...
--@param    monsterId:召唤的怪物Id
--@param    shapeId:大招对应的皮肤Id
function WBattleGlobal:buildSubHero(owner, monsterId, subHeroBattleId, shapeId)
    local soulHero = WSubHero:buildHero(owner, monsterId, shapeId)
    soulHero.m_nBattleId = subHeroBattleId
    soulHero.m_nCamp = owner:getCamp()
    soulHero:setType(CharacterType.TYPE_HERO)
    if monsterId == 20092 then 
        soulHero:setSubType(CharacterSubType.SUBTYPE_BCHESS)
    elseif monsterId == 20093 then 
        soulHero:setSubType(CharacterSubType.SUBTYPE_WCHESS)
    end
    WZLog("WBattleGlobal:buildSoulHero", subHeroBattleId)
    self.m_tSoulHeros[subHeroBattleId] = soulHero

    WBattleGlobal:getCurrent():setBuildMonsterRecord(subHeroBattleId, subHeroBattleId)

    return soulHero
end

--@brief    根据玩家id，获取绑定该玩家的棋圣-分身
--@param    playerId:玩家id
--@param    subType:分身类型，传nil黑白分身都获取
function WBattleGlobal:getSubHero(playerId, subType)
    local subHeroList = {}
    if self.m_tSoulHeros == nil then return subHeroList end

    for id, subHero in pairs(self.m_tSoulHeros) do
        if subHero:getOwnerPlayerId() == playerId then 
            if subType then 
                if subHero:getSubType() == subType then 
                    table.insert(subHeroList, subHero)
                end
            else
                if subHero:getSubType() == CharacterSubType.SUBTYPE_BCHESS or subHero:getSubType() == CharacterSubType.SUBTYPE_WCHESS then 
                    table.insert(subHeroList, subHero)
                end
            end
        end
...
--@brief 获取触发的普攻技能id
function WBattleGlobal:getUseWeaponSkillId()
    ...
    if tCurRoundSkillId then
        for i=1, #tCurRoundSkillId do
            ...
            if skillInfo.skill_type == 0 then
                return tCurRoundSkillId[i]
            end
        end
    end
...
--@brief    图腾检测
function WBattleGlobal:allTotemCheck()
    WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.BUFF_TOTEM)
    WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.FIRE_TOTEM)
    WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.GUARDIAN_TOTEM)
    -- WBattleGlobal:getCurrent():checkHurtBuffTotem(MonsterType.MARITIME1_TOTEM)
...
--@brief    免疫限制的效果
function WBattleGlobal:getImmunizeList()
    local immunizeList = {EffectTypeConfig.LIMIT_MOVE, EffectTypeConfig.LIMIT_FLY, EffectTypeConfig.LIMIT_USE_SKILL, EffectTypeConfig.LIMIT_USE_ITEM, EffectTypeConfig.LIMIT_ALL_ACTION, EffectTypeConfig.LIMIT_VISIBLE, EffectTypeConfig.LIMIT_STOP_CTB_BUFF}
