--WBattleMachineBuffTreatArray.lua
--@brief    场景机关
--@date     2015/04/06
--@note     场景机关

WBattleMachineBuffTreatArray = {
...
--@return   新建的表实例对象
function WBattleMachineBuffTreatArray:new(battleId,templateId,camp,bronPos)
    WZLog("WBattleMachineBuffTreatArray:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineBuffTreatArray,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineBuffTreatArray })
    ...
    tNewObj.m_nSkillId = tNewObj.m_tSkillParam ~= -1 and tNewObj.m_tSkillParam[1][3] or 710
    tNewObj.m_nTimeIntervalValue = tNewObj.m_tSkillParam ~= -1 and tNewObj.m_tSkillParam[1][2] or 2000
...
--@brief 读取怪物模板配置
function WBattleMachineBuffTreatArray:parseMonsterData()
    local monsterData =  BossData["id_"..self.m_nPlayerId]
    self.m_tAniFileIndex =  "machine_1011"
...
--@brief 获得配置id
function WBattleMachineBuffTreatArray:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineBuffTreatArray:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
...
--@brief ctb刷新
function WBattleMachineBuffTreatArray:updateBuffByCTB(dt, updateCTB_time)
    if self:isDead() then
        return
    end
    
    WZLog("WBattleMachineBuffTreatArray:updateBuffByCTB", type(dt))
...
function WBattleMachineBuffTreatArray:doEffect()
    WZLog("WBattleMachineBuffTreatArray:doEffect")
    local skillConfig = GDatatab_skill["id_"..self.m_nSkillId]
    -- local hero = WBattleGlobal:getCurrent():getCharacterWithId(self.m_nBattleId)
    ...
    if WBattleGlobal:getCurrent():isHostControl() then
        WZLog("WBattleMachineBuffTreatArray:doEffect send")
        ProtocolProcessorBattleInterface:send_BATTLE_SkillEquip(WBattleGlobal:getCurrent():getBattleId(), self.m_nBattleId, self.m_nSkillId,targetIds)
    end
...
--@brief    初始化基础动画
function WBattleMachineBuffTreatArray:initAnim()
    WZLog("WBattleMachineBuffTreatArray:initAnim")
...
--@brief 设置碰撞
function WBattleMachineBuffTreatArray:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineBuffTreatArray:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineBuffTreatArray:setScale(scale)
...
function WBattleMachineBuffTreatArray:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineBuffTreatArray:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineBuffTreatArray:getAnimation()
    return self.m_anim
end

function WBattleMachineBuffTreatArray:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or 1
end

--@brief    添加碰撞矩形
function WBattleMachineBuffTreatArray:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineBuffTreatArray:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineBuffTreatArray:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineBuffTreatArray:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineBuffTreatArray:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineBuffTreatArray:setPosition(tPos)
...
--@brief 
function WBattleMachineBuffTreatArray:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineBuffTreatArray:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineBuffTreatArray:getConfig()
...
function WBattleMachineBuffTreatArray:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineBuffTreatArray:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineBuffTreatArray:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineBuffTreatArray:getShootPos(target)
...
--@brief
--@return
function WBattleMachineBuffTreatArray:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineBuffTreatArray:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineBuffTreatArray:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineBuffTreatArray:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineBuffTreatArray:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineBuffTreatArray:setDead(bDead, note)
    WZLog("WBattleMachineBuffTreatArray:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineBuffTreatArray:doSuicide()
...
--@brief 获取死亡标记
function WBattleMachineBuffTreatArray:isDead()
    return self.m_bIsDead
end

--@brief 死亡过程
function WBattleMachineBuffTreatArray:updateDeadAct()
    -- local bIsServerDead = self:isServerDead()
    -- local bIsDone = self:getAnimation():isCurrentAnimationDone()
    -- local bHdDone = self:getPlayerNameIcon().m_bIsHpActionDone
    -- WZLog("WBattleMachineBuffTreatArray:updateDeadAct 00000", type(bIsServerDead), type(bIsDone), type(bHdDone), bIsServerDead, bIsDone, bHdDone)
    if self:isServerDead() and self:getAnimation():isCurrentAnimationDone() == true and self:getPlayerNameIcon().m_bIsHpActionDone ~= false then
        WZLog("WBattleMachineBuffTreatArray:updateDeadAct")
        WBattleGlobal:getCurrent():removeMachine(self.m_nBattleId)
    end
end
--@--brief wbattleglobal调用
function WBattleMachineBuffTreatArray:destroy()
    WZLog("WBattleMachineBuffTreatArray:destroy")
    if self:getAnimation():getAnimNode() then
        self:getAnimation():getAnimNode():removeFromParentAndCleanup(false)
        WZLog("WBattleMachineBuffTreatArray:release")
...
function WBattleMachineBuffTreatArray:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineBuffTreatArray:_init()
...
--@brief 解析动画名字
function WBattleMachineBuffTreatArray:_setAnimConfig()
...
function WBattleMachineBuffTreatArray:_addBossName()
...
--@brief 
function WBattleMachineBuffTreatArray:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineBuffTreatArray:getPlayerNameIcon()
    return self.m_tBossName
end

--@brief    清理人物名称信息的显示
function WBattleMachineBuffTreatArray:clearPlayerNameIcon()
   if self.m_tBossName ~= nil then
