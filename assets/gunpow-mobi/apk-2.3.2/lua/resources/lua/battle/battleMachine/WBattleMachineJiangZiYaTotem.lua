--WBattleMachineJiangZiYaTotem.lua
--@brief    场景机关
--@date     2021/12/16
--@note     海洋领域减益扣血

WBattleMachineJiangZiYaTotem = {
...
--@return   新建的表实例对象
function WBattleMachineJiangZiYaTotem:new(battleId,templateId,camp,bronPos, userAttack)
    WZLog("WBattleMachineJiangZiYaTotem:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineJiangZiYaTotem,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineJiangZiYaTotem })
    tNewObj.m_nBattleId = battleId
    tNewObj.m_nPlayerId = templateId
    tNewObj.m_nCamp = camp
    tNewObj.m_bIsInCtb = false--不参与ctb轮转
    
    tNewObj.m_tAniFileIndex =  "machine_1018"
    --伤害几爽基础相关属性
...
--@brief 获取buffId
function WBattleMachineJiangZiYaTotem:getBuffInfo()
    return self.m_nBuffId,self.m_nBuffLv,self.m_nBattleId,self:getLeftCTB(),self.m_nMonsterType
end

--@brief 获得配置id
function WBattleMachineJiangZiYaTotem:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineJiangZiYaTotem:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
...
--@brief ctb刷新
function WBattleMachineJiangZiYaTotem:updateBuffByCTB(dt, updateCTB_time, runCount)
    if self:isDead() then
        return
    end
...
--@brief 剩余ctb
function WBattleMachineJiangZiYaTotem:getLeftCTB()
...
--@brief    初始化基础动画
function WBattleMachineJiangZiYaTotem:initAnim()
    WZLog("WBattleMachineJiangZiYaTotem:initAnim")
...
--@brief 设置碰撞
function WBattleMachineJiangZiYaTotem:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineJiangZiYaTotem:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineJiangZiYaTotem:setScale(scale)
...
function WBattleMachineJiangZiYaTotem:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineJiangZiYaTotem:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineJiangZiYaTotem:getAnimation()
    return self.m_anim
end

function WBattleMachineJiangZiYaTotem:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or self.m_nScale
end

--@brief    添加碰撞矩形
function WBattleMachineJiangZiYaTotem:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineJiangZiYaTotem:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineJiangZiYaTotem:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineJiangZiYaTotem:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineJiangZiYaTotem:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineJiangZiYaTotem:setPosition(tPos)
...
--@brief 
function WBattleMachineJiangZiYaTotem:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineJiangZiYaTotem:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineJiangZiYaTotem:getConfig()
    if not self.m_tConfig then
        WZLog("WBattleMachineJiangZiYaTotem:getConfig",self.m_tAniFileIndex)
...
function WBattleMachineJiangZiYaTotem:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineJiangZiYaTotem:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineJiangZiYaTotem:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineJiangZiYaTotem:getShootPos(target)
...
--@brief
--@return
function WBattleMachineJiangZiYaTotem:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineJiangZiYaTotem:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineJiangZiYaTotem:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineJiangZiYaTotem:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineJiangZiYaTotem:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineJiangZiYaTotem:setDead(bDead, note)
    WZLog("WBattleMachineJiangZiYaTotem:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineJiangZiYaTotem:doSuicide()
...
--@brief 获取死亡标记
function WBattleMachineJiangZiYaTotem:isDead()
    return self.m_bIsDead
end

--@brief 死亡过程
function WBattleMachineJiangZiYaTotem:updateDeadAct()
    if self:isServerDead() then
        WZLog("WBattleMachineJiangZiYaTotem:updateDeadAct")
        WBattleGlobal:getCurrent():removeMachine(self.m_nBattleId)
    end
end
--@--brief wbattleglobal调用
function WBattleMachineJiangZiYaTotem:destroy()
    WZLog("WBattleMachineJiangZiYaTotem:destroy")
    if self:getAnimation():getAnimNode() then
        self:getAnimation():getAnimNode():removeFromParentAndCleanup(false)
        WZLog("WBattleMachineJiangZiYaTotem:release")
...
function WBattleMachineJiangZiYaTotem:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineJiangZiYaTotem:_init()
...
--@brief 解析动画名字
function WBattleMachineJiangZiYaTotem:_setAnimConfig()
...
function WBattleMachineJiangZiYaTotem:checkBuffTotemCollison()
    ...
    --碰撞队列对象
    WZLog("WBattleMachineJiangZiYaTotem:checkBuffTotemCollison one", #tGuaiList)
    ...
    WZLog("WBattleMachineJiangZiYaTotem:checkBuffTotemCollison", #list)
    return list
end

--@brief 
function WBattleMachineJiangZiYaTotem:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineJiangZiYaTotem:getPlayerNameIcon()
