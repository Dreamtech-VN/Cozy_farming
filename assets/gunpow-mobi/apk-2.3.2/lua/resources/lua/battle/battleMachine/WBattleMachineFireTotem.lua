--WBattleMachineFireTotem.lua
--@brief    场景机关
--@date     2015/04/06
--@note     fire图腾

WBattleMachineFireTotem = {
...
--@return   新建的表实例对象
function WBattleMachineFireTotem:new(battleId,templateId,camp,bronPos, userAttack)
    WZLog("WBattleMachineFireTotem:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineFireTotem,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineFireTotem })
    tNewObj.m_nBattleId = battleId
    tNewObj.m_nPlayerId = templateId
    tNewObj.m_nCamp = camp
    tNewObj.m_bIsInCtb = false--不参与ctb轮转
    
    tNewObj.m_tAniFileIndex =  "machine_1012"
    --伤害几爽基础相关属性
...
--@brief 获取buffId
function WBattleMachineFireTotem:getBuffInfo()
    return self.m_nBuffId,self.m_nBuffLv,self.m_nBattleId,self:getLeftCTB()
end

--@brief 获得配置id
function WBattleMachineFireTotem:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineFireTotem:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
...
--@brief ctb刷新
function WBattleMachineFireTotem:updateBuffByCTB(dt, updateCTB_time)
    if self:isDead() then
        return
    end
...
--@brief 剩余ctb
function WBattleMachineFireTotem:getLeftCTB()
...
--@brief    初始化基础动画
function WBattleMachineFireTotem:initAnim()
    WZLog("WBattleMachineFireTotem:initAnim")
...
--@brief 设置碰撞
function WBattleMachineFireTotem:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineFireTotem:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineFireTotem:setScale(scale)
...
function WBattleMachineFireTotem:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineFireTotem:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineFireTotem:getAnimation()
    return self.m_anim
end

function WBattleMachineFireTotem:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or self.m_nScale
end

--@brief    添加碰撞矩形
function WBattleMachineFireTotem:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineFireTotem:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineFireTotem:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineFireTotem:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineFireTotem:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineFireTotem:setPosition(tPos)
...
--@brief 
function WBattleMachineFireTotem:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineFireTotem:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineFireTotem:getConfig()
    if not self.m_tConfig then
        WZLog("WBattleMachineFireTotem:getConfig",self.m_tAniFileIndex)
...
function WBattleMachineFireTotem:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineFireTotem:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineFireTotem:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineFireTotem:getShootPos(target)
...
--@brief
--@return
function WBattleMachineFireTotem:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineFireTotem:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineFireTotem:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineFireTotem:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineFireTotem:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineFireTotem:setDead(bDead, note)
    WZLog("WBattleMachineFireTotem:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineFireTotem:doSuicide()
...
--@brief 获取死亡标记
function WBattleMachineFireTotem:isDead()
    return self.m_bIsDead
end

--@brief 死亡过程
function WBattleMachineFireTotem:updateDeadAct()
    if self:isServerDead() then
        WZLog("WBattleMachineFireTotem:updateDeadAct")
        WBattleGlobal:getCurrent():removeMachine(self.m_nBattleId)
    end
end
--@--brief wbattleglobal调用
function WBattleMachineFireTotem:destroy()
    WZLog("WBattleMachineFireTotem:destroy")
    if self:getAnimation():getAnimNode() then
        self:getAnimation():getAnimNode():removeFromParentAndCleanup(false)
        WZLog("WBattleMachineFireTotem:release")
...
function WBattleMachineFireTotem:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineFireTotem:_init()
...
--@brief 解析动画名字
function WBattleMachineFireTotem:_setAnimConfig()
...
function WBattleMachineFireTotem:checkBuffTotemCollison()
    ...
    --碰撞队列对象
    WZLog("WBattleMachineFireTotem:checkBuffTotemCollison one", #tGuaiList)
    ...
    WZLog("WBattleMachineFireTotem:checkBuffTotemCollison", #list)
    return list
end

--@brief 
function WBattleMachineFireTotem:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineFireTotem:getPlayerNameIcon()
