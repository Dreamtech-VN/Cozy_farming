...
--@return   新建的表实例对象
function WBattleMachineCage:new(battleId,templateId,camp,bronPos)
    WZLog("WBattleMachineCage:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineCage,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineCage })
    tNewObj.m_nBattleId = battleId
    tNewObj.m_nPlayerId = templateId
    tNewObj.m_nCamp = camp
    tNewObj.m_nMonsterType = MonsterType.BOSS_CAGE
    tNewObj.m_bIsInCtb = false--不参与ctb轮转
    tNewObj.m_bOffHurt = true --不参与伤害计算
    tNewObj.m_bOffCollision = true --不参与子弹碰撞计算
    tNewObj.m_tAniFileIndex =  "machine_7001"
...
--@brief 读取怪物模板配置
-- function WBattleMachineCage:parseMonsterData()
...
--@brief 获得配置id
function WBattleMachineCage:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineCage:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
    ...
    -- WZLog("WBattleMachineCage:update",tostring(self:getAnimation():isCurrentAnimationDone()),self:getNormalAnimationName())
    if self:getAnimation():isCurrentAnimationDone() == true and self.m_bAutoStandAction then
        -- WZLog("WBattleMachineCage:update-2")
...
--@return   #2:碰撞的人物列表
function WBattleMachineCage:checkCollision()
    WZLog("WBattleMachineCage:checkCollision")
    ...
    if #collisionList > 0 then
        WZLog("WBattleMachineCage:checkCollision-two",#collisionList)
        BattleMethod:waitForSkillHurt(self,collisionList)
    end
end

--@brief    初始化基础动画
function WBattleMachineCage:initAnim()
    WZLog("WBattleMachineCage:initAnim",self:getConfig().aniFileId)
...
--出现动画
function WBattleMachineCage:playAppearAction()
    self:getAnimation():play(self:getAppearAnimationName(), false)
end

--@brief 设置碰撞
function WBattleMachineCage:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineCage:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineCage:setScale(scale)
...
function WBattleMachineCage:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineCage:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineCage:getAnimation()
    return self.m_anim
end

function WBattleMachineCage:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or 1
end

--@brief    添加碰撞矩形
function WBattleMachineCage:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineCage:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineCage:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineCage:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineCage:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineCage:setPosition(tPos)
...
--@brief 
function WBattleMachineCage:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineCage:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineCage:getConfig()
...
function WBattleMachineCage:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineCage:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineCage:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineCage:getShootPos(target)
...
--@brief
--@return
function WBattleMachineCage:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineCage:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineCage:getAppearAnimationName()
    return self:getAnimationName("skill")
end

--@brief    获取待机动画
function WBattleMachineCage:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineCage:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineCage:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineCage:setDead(bDead, note)
    WZLog("WBattleMachineCage:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineCage:doSuicide()
...
function WBattleMachineCage:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineCage:_init()
...
--@brief 解析动画名字
function WBattleMachineCage:_setAnimConfig()
...
function WBattleMachineCage:_addBossName()
...
--@brief 
function WBattleMachineCage:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineCage:getPlayerNameIcon()
    return self.m_tBossName
end

--@brief    清理人物名称信息的显示
function WBattleMachineCage:clearPlayerNameIcon()
   if self.m_tBossName ~= nil then
...
function WBattleMachineCage:addCollisionCharas(tCharas)
