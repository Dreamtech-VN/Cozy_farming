--WBattleMachineTornado.lua
--@brief    场景机关
--@date     2015/04/06
--@note     龙卷风

WBattleMachineTornado = {
...
--@return   新建的表实例对象
function WBattleMachineTornado:new(battleId,templateId,camp,bronPos)
    WZLog("WBattleMachineTornado:new",battleId,templateId,camp,skillId,interval,duration)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineTornado,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineTornado })
    ...
    firstPos.y = firstPos.y - 100 --策划需求龙卷风的位置要偏下一下
...
--@brief 读取怪物模板配置
function WBattleMachineTornado:parseMonsterData()
    local monsterData =  BossData["id_"..self.m_nPlayerId]
    if self.m_nCamp ~= WBattleGlobal:getCurrent():getMyHero():getCamp() then
         self.m_tAniFileIndex =  "machine_1005"
    else
        self.m_tAniFileIndex =  "machine_1004"
    end
...
--@brief 获得配置id
function WBattleMachineTornado:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineTornado:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
...
--@return   #2:碰撞的人物列表
function WBattleMachineTornado:checkCollision()
    --检测与子弹的碰撞
...
function WBattleMachineTornado:checkCharaCollision()
    local list = WBattleGlobal:getCurrent():getCharacterList(true)
    for i,hero in pairs(list) do
--        WZLog("WBattleMachineTornado:checkCharaCollision",hero:getBattleId(),tostring(hero.m_bOffFrozen),tostring(hero.m_bTornadoReflect))
...
--@brief ctb刷新
function WBattleMachineTornado:updateBuffByCTB(dt, updateCTB_time)
    if self:isDead() then
        return
    end
...
function WBattleMachineTornado:doEffect(bullet)
    --@brief    事件处理
    local wind = WBattleGlobal:getCurrent():getWindLevel().x

    WZLog("WBattleMachineTornado:doEffect three", tostring(bullet), tostring(bullet:getIsExist()))
    if bullet:getIsExist() == true then --and bullet.m_bIsProcessMapEvent == false and bullet.m_bIsCollisionMapEvent == true then
        ...
        WZLog("WBattleMachineTornado:doEffect two", wind, moverPos.x, moverPos.y, startSpeed.x, startSpeed.y, moverSpeed.x, moverSpeed.y,changeSpeed.x, changeSpeed.y)
    end
end

function WBattleMachineTornado:doCharaEffect(hero)
    --@brief    事件处理
    if SceneBattle:getBattleLoop():getBattleStatus() ==  BattleLoop.S_PLAYER_FLY then
       ...
       WZLog("WBattleMachineTornado:doCharaEffect-one")
    else
        local animPos = self.m_anim:getPosition()
        local heroPos = hero:getPosition()
        if animPos.x > heroPos.x then
            hero:setStopMoveByTornado(2)
            local targetPos = GlobalMethod:ccp(animPos.x - 60,heroPos.y + 5)
            if not BattleCommon:checkPosCollision(targetPos,BattleMapManager.m_pixelByte) then
                ...
                if SceneBattle:getFrontLayer() then
                    ...
                    --纵向超出屏幕
                    if animPos.x - 60 < 5 or animPos.x - 60 > sceneSize.width - 5 then
                        WZLog("WBattleMachineTornado:doCharaEffect one", animPos.x - 60)
                        outOfScene = true
                    end
                end
                if not outOfScene then 
                    hero:setPosition(GlobalMethod:ccp(animPos.x - 60,heroPos.y))
                end
            end
        else
            ...
            if not BattleCommon:checkPosCollision(targetPos,BattleMapManager.m_pixelByte) then
                ...
                if SceneBattle:getFrontLayer() then
                    ...
                    --纵向超出屏幕
                    if animPos.x + 60 < 5 or animPos.x + 60 > sceneSize.width - 5 then
                        WZLog("WBattleMachineTornado:doCharaEffect two", animPos.x + 60)
                        outOfScene = true
                    end
                end
                if not outOfScene then 
        ...
        WZLog("WBattleMachineTornado:doCharaEffect-two")
    end
end

--@brief    初始化基础动画
function WBattleMachineTornado:initAnim()
    WZLog("WBattleMachineTornado:initAnim")
...
--@brief 设置碰撞
function WBattleMachineTornado:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineTornado:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineTornado:setScale(scale)
...
function WBattleMachineTornado:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineTornado:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineTornado:getAnimation()
    return self.m_anim
end

function WBattleMachineTornado:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or 1
end

--@brief    添加碰撞矩形
function WBattleMachineTornado:changeRectCollision()
...
function WBattleMachineTornado:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineTornado:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineTornado:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineTornado:setPosition(tPos)
...
--@brief 
function WBattleMachineTornado:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineTornado:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineTornado:getConfig()
...
function WBattleMachineTornado:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineTornado:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineTornado:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineTornado:getShootPos(target)
...
--@brief
--@return
function WBattleMachineTornado:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineTornado:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineTornado:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineTornado:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineTornado:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineTornado:setDead(bDead, note)
    WZLog("WBattleMachineTornado:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineTornado:doSuicide()
...
--@brief 获取死亡标记
function WBattleMachineTornado:isDead()
    return self.m_bIsDead
end

--@brief 死亡过程
function WBattleMachineTornado:updateDeadAct()
    local bIsDead = self:isServerDead()

--    WZLog("WBattleMachineTornado:updateDeadAct 00000", type(bIsDead), bIsDead)
    if self:isServerDead() then
        WZLog("WBattleMachineTornado:updateDeadAct")
        WBattleGlobal:getCurrent():removeMachine(self.m_nBattleId)
    end
end
--@--brief wbattleglobal调用
function WBattleMachineTornado:destroy()
    WZLog("WBattleMachineTornado:destroy")
    if self:getAnimation():getAnimNode() then
        self:getAnimation():getAnimNode():removeFromParentAndCleanup(false)
        WZLog("WBattleMachineTornado:release")
...
function WBattleMachineTornado:getPetAttackPos()
...
--@brief    获取怪物类型
function WBattleMachineTornado:getMonsterType()
...
--@brief    初始化对象
function WBattleMachineTornado:_init()
...
--@brief 解析动画名字
function WBattleMachineTornado:_setAnimConfig()
...
function WBattleMachineTornado:_addBossName()
...
--@brief 
function WBattleMachineTornado:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineTornado:getPlayerNameIcon()
    return self.m_tBossName
end

--@brief    清理人物名称信息的显示
function WBattleMachineTornado:clearPlayerNameIcon()
   if self.m_tBossName ~= nil then
