--WBattleMachineMaritime1Totem.lua
--@brief    场景机关
--@date     2021/12/16
--@note     守护光环

WBattleMachineMaritime1Totem = {
...
--@return   新建的表实例对象
function WBattleMachineMaritime1Totem:new(battleId,templateId,camp,bronPos, userAttack)
    WZLog("WBattleMachineMaritime1Totem:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineMaritime1Totem,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineMaritime1Totem })
    ...
    if tNewObj.m_nCamp == WBattleGlobal:getCurrent():getMyHero():getCamp() then
        tNewObj.m_tAniFileIndex =  "machine_1015"
    else
        tNewObj.m_tAniFileIndex =  "machine_1016"
    end
...
--@brief 获取buffId
function WBattleMachineMaritime1Totem:getBuffInfo()
    return self.m_nBuffId,self.m_nBuffLv,self.m_nBattleId,self:getLeftCTB(),self.m_nMonsterType
end

--@brief 获得配置id
function WBattleMachineMaritime1Totem:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineMaritime1Totem:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
...
--@brief ctb刷新
function WBattleMachineMaritime1Totem:updateBuffByCTB(dt, updateCTB_time, runCount)
    if self:isDead() then
        return
    end
...
--@brief 剩余ctb
function WBattleMachineMaritime1Totem:getLeftCTB()
...
--@brief    初始化基础动画
function WBattleMachineMaritime1Totem:initAnim()
    WZLog("WBattleMachineMaritime1Totem:initAnim")
...
--@brief 设置碰撞
function WBattleMachineMaritime1Totem:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineMaritime1Totem:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineMaritime1Totem:setScale(scale)
...
function WBattleMachineMaritime1Totem:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineMaritime1Totem:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineMaritime1Totem:getAnimation()
    return self.m_anim
end

function WBattleMachineMaritime1Totem:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or self.m_nScale
end

--@brief    添加碰撞矩形
function WBattleMachineMaritime1Totem:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineMaritime1Totem:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineMaritime1Totem:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineMaritime1Totem:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineMaritime1Totem:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineMaritime1Totem:setPosition(tPos)
...
--@brief 
function WBattleMachineMaritime1Totem:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineMaritime1Totem:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineMaritime1Totem:getConfig()
    if not self.m_tConfig then
        WZLog("WBattleMachineMaritime1Totem:getConfig",self.m_tAniFileIndex)
...
function WBattleMachineMaritime1Totem:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineMaritime1Totem:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineMaritime1Totem:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineMaritime1Totem:getShootPos(target)
...
--@brief
--@return
function WBattleMachineMaritime1Totem:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineMaritime1Totem:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineMaritime1Totem:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineMaritime1Totem:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineMaritime1Totem:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineMaritime1Totem:setDead(bDead, note)
    WZLog("WBattleMachineMaritime1Totem:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineMaritime1Totem:doSuicide()
...
--@brief 获取死亡标记
function WBattleMachineMaritime1Totem:isDead()
    return self.m_bIsDead
end

--@brief 死亡过程
function WBattleMachineMaritime1Totem:updateDeadAct()
    if self:isServerDead() then
        WZLog("WBattleMachineMaritime1Totem:updateDeadAct")
        WBattleGlobal:getCurrent():removeMachine(self.m_nBattleId)
    end
end
--@--brief wbattleglobal调用
function WBattleMachineMaritime1Totem:destroy()
    WZLog("WBattleMachineMaritime1Totem:destroy")
    if self:getAnimation():getAnimNode() then
        self:getAnimation():getAnimNode():removeFromParentAndCleanup(false)
        WZLog("WBattleMachineMaritime1Totem:release")
...
function WBattleMachineMaritime1Totem:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineMaritime1Totem:_init()
...
--@brief 解析动画名字
function WBattleMachineMaritime1Totem:_setAnimConfig()
...
function WBattleMachineMaritime1Totem:checkBuffTotemCollison()
    ...
    --碰撞队列对象
    WZLog("WBattleMachineMaritime1Totem:checkBuffTotemCollison one", #tGuaiList)
    ...
    WZLog("WBattleMachineMaritime1Totem:checkBuffTotemCollison", #list)
    return list
end

--@brief 
function WBattleMachineMaritime1Totem:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineMaritime1Totem:getPlayerNameIcon()
