--WBattleMachineGuardianTotem.lua
--@brief    场景机关
--@date     2021/12/16
--@note     守护光环

WBattleMachineGuardianTotem = {
...
--@return   新建的表实例对象
function WBattleMachineGuardianTotem:new(battleId,templateId,camp,bronPos, userAttack)
    WZLog("WBattleMachineGuardianTotem:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineGuardianTotem,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineGuardianTotem })
    tNewObj.m_nBattleId = battleId
    tNewObj.m_nPlayerId = templateId
    tNewObj.m_nCamp = camp
    tNewObj.m_bIsInCtb = false--不参与ctb轮转
    
    tNewObj.m_tAniFileIndex =  "machine_1013"
    --伤害几爽基础相关属性
...
--@brief 获取buffId
function WBattleMachineGuardianTotem:getBuffInfo()
    return self.m_nBuffId,self.m_nBuffLv,self.m_nBattleId,self:getLeftCTB()
end

--@brief 获得配置id
function WBattleMachineGuardianTotem:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineGuardianTotem:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
...
--@brief ctb刷新
function WBattleMachineGuardianTotem:updateBuffByCTB(dt, updateCTB_time)
    if self:isDead() then
        return
    end
...
--@brief 剩余ctb
function WBattleMachineGuardianTotem:getLeftCTB()
    local duration = self.m_nTimeDurationValue - self.m_nTimePassValue
    if duration < 0 then
        duration = 0
    end
    return duration
...
--@brief    初始化基础动画
function WBattleMachineGuardianTotem:initAnim()
    WZLog("WBattleMachineGuardianTotem:initAnim")
...
--@brief 设置碰撞
function WBattleMachineGuardianTotem:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineGuardianTotem:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineGuardianTotem:setScale(scale)
...
function WBattleMachineGuardianTotem:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineGuardianTotem:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineGuardianTotem:getAnimation()
    return self.m_anim
end

function WBattleMachineGuardianTotem:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or self.m_nScale
end

--@brief    添加碰撞矩形
function WBattleMachineGuardianTotem:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineGuardianTotem:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineGuardianTotem:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineGuardianTotem:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineGuardianTotem:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineGuardianTotem:setPosition(tPos)
...
--@brief 
function WBattleMachineGuardianTotem:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineGuardianTotem:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineGuardianTotem:getConfig()
    if not self.m_tConfig then
        WZLog("WBattleMachineGuardianTotem:getConfig",self.m_tAniFileIndex)
...
function WBattleMachineGuardianTotem:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineGuardianTotem:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineGuardianTotem:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineGuardianTotem:getShootPos(target)
...
--@brief
--@return
function WBattleMachineGuardianTotem:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineGuardianTotem:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineGuardianTotem:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineGuardianTotem:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineGuardianTotem:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineGuardianTotem:setDead(bDead, note)
    WZLog("WBattleMachineGuardianTotem:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineGuardianTotem:doSuicide()
...
--@brief 获取死亡标记
function WBattleMachineGuardianTotem:isDead()
    return self.m_bIsDead
end

--@brief 死亡过程
function WBattleMachineGuardianTotem:updateDeadAct()
    if self:isServerDead() then
        WZLog("WBattleMachineGuardianTotem:updateDeadAct")
        WBattleGlobal:getCurrent():removeMachine(self.m_nBattleId)
    end
end
--@--brief wbattleglobal调用
function WBattleMachineGuardianTotem:destroy()
    WZLog("WBattleMachineGuardianTotem:destroy")
    if self:getAnimation():getAnimNode() then
        self:getAnimation():getAnimNode():removeFromParentAndCleanup(false)
        WZLog("WBattleMachineGuardianTotem:release")
...
function WBattleMachineGuardianTotem:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineGuardianTotem:_init()
...
--@brief 解析动画名字
function WBattleMachineGuardianTotem:_setAnimConfig()
...
function WBattleMachineGuardianTotem:checkBuffTotemCollison()
    ...
    --碰撞队列对象
    WZLog("WBattleMachineGuardianTotem:checkBuffTotemCollison one", #tGuaiList)
    ...
    WZLog("WBattleMachineGuardianTotem:checkBuffTotemCollison", #list)
    return list
end

--@brief 
function WBattleMachineGuardianTotem:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineGuardianTotem:getPlayerNameIcon()
