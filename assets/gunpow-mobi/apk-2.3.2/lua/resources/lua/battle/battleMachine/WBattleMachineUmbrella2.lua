--WBattleMachineUmbrella2.lua
--@brief    场景机关
--@date     2024/1/22
--@note     执伞圣女 反击

WBattleMachineUmbrella2 = {
...
--@return   新建的表实例对象
function WBattleMachineUmbrella2:new(battleId,templateId,camp,bronPos, userAttack)
    WZLog("WBattleMachineUmbrella2:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineUmbrella2,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineUmbrella2 })
    tNewObj.m_nBattleId = battleId
    tNewObj.m_nPlayerId = templateId
    tNewObj.m_nCamp = camp
    tNewObj.m_bIsInCtb = false--不参与ctb轮转
    
    tNewObj.m_tAniFileIndex =  "machine_1021"
    --伤害几爽基础相关属性
...
--@brief 获取buffId
function WBattleMachineUmbrella2:getBuffInfo()
    return self.m_nBuffId,self.m_nBuffLv,self.m_nBattleId,self:getLeftCTB(),self.m_nMonsterType
end

--@brief 获得配置id
function WBattleMachineUmbrella2:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineUmbrella2:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
...
--@brief ctb刷新
function WBattleMachineUmbrella2:updateBuffByCTB(dt, updateCTB_time, runCount)
    if self:isDead() then
        return
    end
...
--@brief 剩余ctb
function WBattleMachineUmbrella2:getLeftCTB()
...
--@brief    初始化基础动画
function WBattleMachineUmbrella2:initAnim()
    WZLog("WBattleMachineUmbrella2:initAnim")
...
--@brief 设置碰撞
function WBattleMachineUmbrella2:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineUmbrella2:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineUmbrella2:setScale(scale)
...
function WBattleMachineUmbrella2:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineUmbrella2:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineUmbrella2:getAnimation()
    return self.m_anim
end

function WBattleMachineUmbrella2:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or self.m_nScale
end

--@brief    添加碰撞矩形
function WBattleMachineUmbrella2:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineUmbrella2:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineUmbrella2:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineUmbrella2:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineUmbrella2:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineUmbrella2:setPosition(tPos)
...
--@brief 
function WBattleMachineUmbrella2:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineUmbrella2:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineUmbrella2:getConfig()
    if not self.m_tConfig then
        WZLog("WBattleMachineUmbrella2:getConfig",self.m_tAniFileIndex)
...
function WBattleMachineUmbrella2:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineUmbrella2:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineUmbrella2:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineUmbrella2:getShootPos(target)
...
--@brief
--@return
function WBattleMachineUmbrella2:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineUmbrella2:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineUmbrella2:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineUmbrella2:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineUmbrella2:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineUmbrella2:setDead(bDead, note)
    WZLog("WBattleMachineUmbrella2:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineUmbrella2:doSuicide()
...
--@brief 获取死亡标记
function WBattleMachineUmbrella2:isDead()
    return self.m_bIsDead
end

--@brief 死亡过程
function WBattleMachineUmbrella2:updateDeadAct()
    if self:isServerDead() then
        WZLog("WBattleMachineUmbrella2:updateDeadAct")
        WBattleGlobal:getCurrent():removeMachine(self.m_nBattleId)
    end
end
--@--brief wbattleglobal调用
function WBattleMachineUmbrella2:destroy()
    WZLog("WBattleMachineUmbrella2:destroy")
    if self:getAnimation():getAnimNode() then
        self:getAnimation():getAnimNode():removeFromParentAndCleanup(false)
        WZLog("WBattleMachineUmbrella2:release")
...
function WBattleMachineUmbrella2:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineUmbrella2:_init()
...
--@brief 解析动画名字
function WBattleMachineUmbrella2:_setAnimConfig()
...
function WBattleMachineUmbrella2:checkBuffTotemCollison()
    ...
    --碰撞队列对象
    WZLog("WBattleMachineUmbrella2:checkBuffTotemCollison one", #tGuaiList)
    ...
    WZLog("WBattleMachineUmbrella2:checkBuffTotemCollison", #list)
    return list
end

--@brief 
function WBattleMachineUmbrella2:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineUmbrella2:getPlayerNameIcon()
