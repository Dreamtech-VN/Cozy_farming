--WBattleMachineFire.lua
--@brief    场景机关
--@date     2016/08/20
--@note     boss6火焰(版本比较久 不能通用，通用类型使用WBattleMachineRoundStartCollisionHurt)
...
--@return   新建的表实例对象
function WBattleMachineFire:new(battleId,templateId,camp,bronPos)
    WZLog("WBattleMachineFire:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineFire,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineFire })
    tNewObj.m_nBattleId = battleId
    tNewObj.m_nPlayerId = templateId
    tNewObj.m_nCamp = camp
    tNewObj.m_nMonsterType = MonsterType.BOSS_FIRE
     tNewObj.m_bIsInCtb = false--不参与ctb轮转
    tNewObj.m_bOffHurt = true --不参与伤害计算
    tNewObj.m_bOffCollision = true --不参与子弹碰撞计算
    tNewObj.m_tAniFileIndex =  "machine_1008"
...
--@brief 读取怪物模板配置
function WBattleMachineFire:parseMonsterData()
    local monsterData =  BossData["id_"..self.m_nPlayerId]
    self.m_tAniFileIndex =  "machine_1008"
...
--@brief 获得配置id
function WBattleMachineFire:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineFire:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
    ...
    -- WZLog("WBattleMachineFire:update",tostring(self:getAnimation():isCurrentAnimationDone()),self:getNormalAnimationName())
    if self:getAnimation():isCurrentAnimationDone() == true and self.m_bAutoStandAction then
        -- WZLog("WBattleMachineFire:update-2")
...
--@return   #2:碰撞的人物列表
function WBattleMachineFire:checkCollision()
    WZLog("WBattleMachineFire:checkCollision")
    ...
    if #collisionList > 0 then
        WZLog("WBattleMachineFire:checkCollision-two",#collisionList)
        local boss = nil
...
--@brief    初始化基础动画
function WBattleMachineFire:initAnim()
    WZLog("WBattleMachineFire:initAnim",self:getConfig().aniFileId)
...
--出现动画
function WBattleMachineFire:playAppearAction()
    self:getAnimation():play(self:getAppearAnimationName(), false)
end

--@brief 设置碰撞
function WBattleMachineFire:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineFire:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineFire:setScale(scale)
...
function WBattleMachineFire:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineFire:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineFire:getAnimation()
    return self.m_anim
end

function WBattleMachineFire:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or 1
end

--@brief    添加碰撞矩形
function WBattleMachineFire:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineFire:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineFire:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineFire:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineFire:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineFire:setPosition(tPos)
...
--@brief 
function WBattleMachineFire:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineFire:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineFire:getConfig()
...
function WBattleMachineFire:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineFire:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineFire:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineFire:getShootPos(target)
...
--@brief
--@return
function WBattleMachineFire:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineFire:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineFire:getAppearAnimationName()
    return self:getAnimationName("skill")
end

--@brief    获取待机动画
function WBattleMachineFire:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineFire:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineFire:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineFire:setDead(bDead, note)
    WZLog("WBattleMachineFire:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineFire:doSuicide()
...
function WBattleMachineFire:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineFire:_init()
...
--@brief 解析动画名字
function WBattleMachineFire:_setAnimConfig()
...
function WBattleMachineFire:_addBossName()
...
--@brief 
function WBattleMachineFire:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineFire:getPlayerNameIcon()
    return self.m_tBossName
end

--@brief    清理人物名称信息的显示
function WBattleMachineFire:clearPlayerNameIcon()
   if self.m_tBossName ~= nil then
...
function WBattleMachineFire:addCollisionCharas(tCharas)
