--WBattleMachineMaritime2Totem.lua
--@brief    场景机关
--@date     2021/12/16
--@note     海洋领域减益束缚

WBattleMachineMaritime2Totem = {
...
--@return   新建的表实例对象
function WBattleMachineMaritime2Totem:new(battleId,templateId,camp,bronPos, userAttack)
    WZLog("WBattleMachineMaritime2Totem:new",battleId,templateId,camp)
    -- WBattleGlobal:getCurrent():setBuildMonsterRecord(battleId,templateId)
    setmetatable(WBattleMachineMaritime2Totem,{__index = WCharacter})
    local tNewObj = {}
    setmetatable(tNewObj, { __index = WBattleMachineMaritime2Totem })
...
--@brief 获取buffId
function WBattleMachineMaritime2Totem:getBuffInfo()
    return self.m_nBuffId,self.m_nBuffLv,self.m_nBattleId,self:getLeftCTB(),self.m_nMonsterType
end

--@brief 获得配置id
function WBattleMachineMaritime2Totem:getAniFileIndex()
    return self.m_tAniFileIndex
end

--@brief 道具刷新
function WBattleMachineMaritime2Totem:update(dt)
    if self:isDead() then
        self:updateDeadAct()
        return
    end
...
--@brief ctb刷新
function WBattleMachineMaritime2Totem:updateBuffByCTB(dt, updateCTB_time, runCount)
    if self:isDead() then
        return
    end
...
--@brief 剩余ctb
function WBattleMachineMaritime2Totem:getLeftCTB()
...
--@brief    初始化基础动画
function WBattleMachineMaritime2Totem:initAnim()
    WZLog("WBattleMachineMaritime2Totem:initAnim")
...
--@brief 设置碰撞
function WBattleMachineMaritime2Totem:setMover()
    --小怪Mover
...
--@brief    获取缩放系数
function WBattleMachineMaritime2Totem:getScale()
    return self.m_anim:getScale()
end

--@brief    设置缩放系数
function WBattleMachineMaritime2Totem:setScale(scale)
...
function WBattleMachineMaritime2Totem:getMover()
    return self.m_mover
end

--@brief 获取形象名字
function WBattleMachineMaritime2Totem:getAnimationName(index)
    return self.animNormal[index] or self.animNormal["standby"]
end

--@brief 获取形象
function WBattleMachineMaritime2Totem:getAnimation()
    return self.m_anim
end

function WBattleMachineMaritime2Totem:getAnimScale()
    return self:getConfig().scale and self:getConfig().scale or self.m_nScale
end

--@brief    添加碰撞矩形
function WBattleMachineMaritime2Totem:changeRectCollision()
    ...
    if rectCollisionConfig then 
        for i, info in pairs (rectCollisionConfig)do
            ...
            WZLog("WBattleMachineMaritime2Totem:changeRectCollision two", info.width, info.height,info.x, info.y, scale)
        end
    end
end
...
function WBattleMachineMaritime2Totem:addRectCollision(width,height,xOffset,yOffset)
...
--@brief 获取中心点
function WBattleMachineMaritime2Totem:getCenterPos()
...
--@brief    获得当前的位置
--@return   #1, 返回当前的位置
function WBattleMachineMaritime2Totem:getPosition()
...
--@brief    设置当前的位置
--@param    tPos 当前位置
function WBattleMachineMaritime2Totem:setPosition(tPos)
...
--@brief 
function WBattleMachineMaritime2Totem:setRotation(rota)
    if self.m_anim then
...
--@brief 
function WBattleMachineMaritime2Totem:getRotation(rota)
    if self.m_anim then
...
--@brief 获取配置
function WBattleMachineMaritime2Totem:getConfig()
    if not self.m_tConfig then
        WZLog("WBattleMachineMaritime2Totem:getConfig",self.m_tAniFileIndex)
...
function WBattleMachineMaritime2Totem:getType()
    return 100
end

--@brief    获取子弹碰撞半径
--@return   #1:子弹碰撞半径
function WBattleMachineMaritime2Totem:getRadiusForBulletCollision()
    return 0
end

--@brief    获得碰撞范围
--@return   #1:碰撞范围
function WBattleMachineMaritime2Totem:getCollisionRang()
    return self.m_tCollisionRang
end

--@brief 射击点
function WBattleMachineMaritime2Totem:getShootPos(target)
...
--@brief
--@return
function WBattleMachineMaritime2Totem:getAutoStandAction()
    return self.m_bAutoStandAction
end

--@brief 设置自动切换待机动画
--@return
function WBattleMachineMaritime2Totem:setAutoStandAction(val)
    self.m_bAutoStandAction = val
end

--@brief    获取待机动画
function WBattleMachineMaritime2Totem:getNormalAnimationName()
    return self:getAnimationName("standby")
end

--@brief    获取受伤动画
function WBattleMachineMaritime2Totem:getHurtAnimationName()
    return self:getAnimationName("hurt")
end

--@brief    获取死亡动画
function WBattleMachineMaritime2Totem:getDeadAnimationName()
    return self:getAnimationName("dead")
end

--@brief 设置死亡
function WBattleMachineMaritime2Totem:setDead(bDead, note)
    WZLog("WBattleMachineMaritime2Totem:setDead one", tostring(note))
...
--@brief 自杀
function WBattleMachineMaritime2Totem:doSuicide()
...
--@brief 获取死亡标记
function WBattleMachineMaritime2Totem:isDead()
    return self.m_bIsDead
end

--@brief 死亡过程
function WBattleMachineMaritime2Totem:updateDeadAct()
    if self:isServerDead() then
        WZLog("WBattleMachineMaritime2Totem:updateDeadAct")
        WBattleGlobal:getCurrent():removeMachine(self.m_nBattleId)
    end
end
--@--brief wbattleglobal调用
function WBattleMachineMaritime2Totem:destroy()
    WZLog("WBattleMachineMaritime2Totem:destroy")
    if self:getAnimation():getAnimNode() then
        self:getAnimation():getAnimNode():removeFromParentAndCleanup(false)
        WZLog("WBattleMachineMaritime2Totem:release")
...
function WBattleMachineMaritime2Totem:getPetAttackPos()
...
--@brief    初始化对象
function WBattleMachineMaritime2Totem:_init()
...
--@brief 解析动画名字
function WBattleMachineMaritime2Totem:_setAnimConfig()
...
function WBattleMachineMaritime2Totem:checkBuffTotemCollison()
    ...
    --碰撞队列对象
    WZLog("WBattleMachineMaritime2Totem:checkBuffTotemCollison one", #tGuaiList)
    for i ,v in ipairs(tGuaiList) do
        if not v:isDead() and v:getHp() > 0 and self:getCamp() ~= v:getCamp() and not v.m_bOffFrozen then
            local collision = false
    ...
    WZLog("WBattleMachineMaritime2Totem:checkBuffTotemCollison", #list)
    return list
end

--@brief 
function WBattleMachineMaritime2Totem:getNameLayerOffset()
    return {x = 0, y = -10}
end

--@brief    获得人物名称信息的显示
--@return   #1, 人物名称信息的显示
function WBattleMachineMaritime2Totem:getPlayerNameIcon()
