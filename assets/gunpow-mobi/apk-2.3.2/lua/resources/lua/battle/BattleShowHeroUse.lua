...
BattleShowHeroUse = {
	...
	m_tUseAnim = nil,
	m_nUseId = nil, 
	m_tHoldAnim = nil, --使用技能道具后，保持的特效（非buff）
}
...
function BattleShowHeroUse:addHeroUse(playerId,useType,useId,bNotShowCell)
	...
	if useType == BattleHeroUse.USE_BIGSKILL then
		local bIsUseSkinBigSkill = hero:getUseSkinBigSkill()
        local info = GDatatab_skill["id_"..hero.m_nBigSkillType].name
		usePng = "battleitems/battle_icon_dazhao.png"
        if bIsUseSkinBigSkill then 
        	info = GDatatab_skill["id_"..hero.m_nBigSkinSkillType].name
			usePng = GDatatab_skill["id_"..hero.m_nBigSkinSkillType].icon
        end
	...
	elseif useType == BattleHeroUse.USE_TREASURE then
		...
		if useType == BattleHeroUse.USE_KMSKILL or useType == BattleHeroUse.USE_SKILL_OR_ITEM then
			local skill = WBattleGlobal:getCurrent():getKMSkillById(useId)
			if skill then
				usePng = skill.icon
				useName = skill.name
				if skill.skillType == 6 then 
					cellBg = "ui/common/common_zd_dk_ww.png"
				elseif skill.skillType == 7 then 
					cellBg = "ui/common/common_zd_dk_zq.png"
				end
				useType = BattleHeroUse.USE_SKILL
		...
		if useType == BattleHeroUse.USE_ATTACK_SKILL then
			local skill = WBattleGlobal:getCurrent():getAttackSkillById(useId)
			if skill then
				usePng = skill.icon
				useName = skill.name
				useType = BattleHeroUse.USE_ATTACK_SKILL
	...
	if bNotShowCell == nil or bNotShowCell == false then
		self:addHeroUseCell(heroPos,useType,useId,usePng,lv, cellBg)
	end
...
--@param	hero:英雄table
--@param	animName:动画名字
--@param	tOtherConfig:其他数据配置
--@param	bIsHoldEffect:特效是否需要保留
function BattleShowHeroUse:runUseAnim(hero,animName, tOtherConfig, bIsHoldEffect)
	local heroPos = hero:getAnimation():getPosition()
	...
	elseif animName == "devilCoin" then
		anim = BattleAnimation:createAnimation("skill_jineng_mingyun", false, "battle/skill")
		anim:setScale(1)
		anim:getAnimNode():setUseAbsCoordinate(true)
		SceneBattle:getFrontLayer():addChild(anim:getAnimNode(), 10)
		anim:getAnimNode():setAbsPosition(GlobalMethod:ccp(hero:getPosition().x,hero:getPosition().y + 50))
		anim:play("skill_jineng_myemo")
	elseif animName == "angelCoin" then
		anim = BattleAnimation:createAnimation("skill_jineng_mingyun", false, "battle/skill")
		anim:setScale(1)
		anim:getAnimNode():setUseAbsCoordinate(true)
		SceneBattle:getFrontLayer():addChild(anim:getAnimNode(), 10)
		anim:getAnimNode():setAbsPosition(GlobalMethod:ccp(hero:getPosition().x,hero:getPosition().y + 50))
		anim:play("skill_jineng_tianshi")
	elseif animName == "blood_contract" then --血之契约
		anim = BattleAnimation:createAnimation("skill_xuezhiqiyue_buff", false, "battle/atkEffect")
		anim:setScale(1)
		anim:getAnimNode():setUseAbsCoordinate(true)
		hero:getAnimation():getAnimNode():addChild(anim:getAnimNode())
        anim:getAnimNode():setAbsPosition(GlobalMethod:ccp(83,88))
		anim:play("wait", true)
	else
	...
	local tSkillData = nil 
	if self.m_nUseId then 
		tSkillData = GDatatab_skill["id_" .. self.m_nUseId]
	end
	if tSkillData and tSkillData.skill_type == 9 then 
		WZLog("BattleShowHeroUse UUUUUUUUUUUUUUUUUUUUUU")
	else
		if animName ~= "devilCoin" and animName ~= "angelCoin" then 
	...
	if bIsHoldEffect then 
		if self.m_tHoldAnim ==nil then
			self.m_tHoldAnim = {}
		end
		table.insert(self.m_tHoldAnim, anim)
	else
...
function BattleShowHeroUse:setHeroUsePosition(heroPos)
	local defaultValue = 145
	local hero =  WBattleGlobal:getCurrent():getCharacterWithId(self.m_nPlayerId)
	if hero and hero:getUseSkinBigSkill() then 
		defaultValue = 200
	end
    local point = SceneBattle:getFrontLayer():convertToWorldSpaceAuto(CCAutoPoint:create(heroPos.x,heroPos.y + defaultValue))
...
--@param	cellBg:底框
--@note
function BattleShowHeroUse:addHeroUseCell(heroPos,useType,useId,usePng,lv, cellBg)
	...
	table.insert(self.m_tInfo,{useType = useType , useId = useId , usePng = usePng, cellBg = cellBg})

    local pic = self:_createCell(usePng, lv, useType, cellBg)
...
--@brief 	移除保留的特效
function BattleShowHeroUse:clearHoldEffect()
	if self.m_tHoldAnim then
		for i=#self.m_tHoldAnim,1,-1 do
			self.m_tHoldAnim[i]:getAnimNode():removeFromParentAndCleanup(true)
			table.remove(self.m_tHoldAnim,i)
		end
	end
end
-------------------------------------公有方法模块End----------------------------------------
...
--@note		增加Cell时候使用
function BattleShowHeroUse:_createCell(usePng,lvIcon, useType, cellBg)
