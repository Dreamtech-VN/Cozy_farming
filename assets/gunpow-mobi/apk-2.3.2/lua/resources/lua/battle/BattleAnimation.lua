...
function BattleAnimation:init(sAninName,bUseDragonBone,folder,eventLuaObject)
        ...
        for index,config in ipairs (ArmaturesFolderConfig) do
            for name,folderName in pairs (config) do
                if string.sub(sAninName, 1, string.len(name)) == name then
                    WZLog("BattleAnimation:createAnimation two", name, sAninName,  string.sub(sAninName, 1, string.len(name)), folderName)
                    folder = folderName    
                end
            end
            if folder ~= nil then
    ...
    if bUseDragonBone == true then
        ...
        if folder then
            ...
            local existSpine = CheckEffectFile(folder.."/"..self.m_sAninName, true)
            if existSpine then 
        ...
        local existSpine = false 
        if folder == "pet" then 
            existSpine = CheckEffectFile(file)
            if not existSpine then 
                self.m_sAninName = GetPetAnimation(11004, 4)

                file = folder .. "/" .. self.m_sAninName
            end
        elseif folder == "battle/monster" then 
            existSpine = CheckEffectFile(file)
            -- if not existSpine then 
            --     file = folder .. "/" .. "monster_0003"
            -- end
        end
        existSpine = CheckEffectFile(file)
        if existSpine then 
    ...
    if folder == "pet" then
        ...
        local existSpine = CheckEffectFile("pet_particle/"..particleAniName)
        if existSpine then
...
--@brief    下载宠物粒子特效回调
function BattleAnimation:downloadPetEffectCallback(taskId,extraData,failed)
    WZLog("BattleAnimation:downloadPetEffectCallback", taskId,extraData,failed)
end
...
--@brief    播放下一个动作
function BattleAnimation:playNext(actionName)
    self.m_strNextAction = actionName or "attack_loop"
    -- body
    self.m_node:enableSchedule("_playLoopStorageAni")
end

--@brief    播放蓄力循环动画
function BattleAnimation:_playLoopStorageAni(element)
    -- body
    element = WZUISpine:luaTo(element)
    WZLog("BattleAnimation:_playLoopStorageAni", type(element))
    if element and element:isCurrentAnimationDone() then 
        element:disableSchedule()
        element:play((self.m_strNextAction or "attack_loop"), true)
        self.m_strNextAction = nil
    end
end