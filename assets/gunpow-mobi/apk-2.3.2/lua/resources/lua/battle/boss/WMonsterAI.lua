...
function WMonsterAI:castSkill(id,                          --ID
    ...
    owner,
    target,
    superCritHurt,                                          --触发超暴击时候的伤害值（用于计算吸血）
    param1                                                  --其他参数 子弹下标 (哪吒大招第二阶段时也用到了这个字段传了溅射子弹爆炸位置列表的索引)
    )
    ...
    local msg = MsgManager:createMsg(BattleMsgBossMapSkill)
    msg.m_tTargetChara = target
    msg.m_nId = id
    msg.m_tConditionList = conditionList

    msg.m_nTalkId = talkId

    msg.m_tOwner = self.m_tBoss or owner or WBattleGlobal:getCurrent():getCurrentCharacter()
    ...
    msg.m_nSuperCritHurt = superCritHurt
    msg.m_param1 = param1
