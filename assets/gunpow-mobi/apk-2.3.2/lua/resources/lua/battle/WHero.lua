...
WHero = {
    ...
    m_nUseSkillId = nil, 	--当前回合玩家使用的技能

    m_tGhostSkillList = nil, 	--玩家的幽灵技能
    m_ghostSkillSpineBoxData = nil, --机器玩家想要拾取的宝箱的数据
    m_nGhostMoveGapping = 0,
    m_nRobotUseGhostSkillGapping = 0, --机器人使用幽灵技能时间统计
    m_nRobotUseGhostSkillRandTime = 0, --机器人使用幽灵技能间隔
    m_tKMSkills = nil,				  --辅助技能列表
    m_tAttackSkills = nil, 				--普攻技能列表
    m_tPetEquipAttr = nil,				--宠物装备属性
    m_nMountId = nil, 		--出战的坐骑Id
    m_nKidId = 0,           --出战的小孩Id
    m_tKidInfo = {}, 		--出战的小孩数据
    m_tKillEnemyKidSkill = {},  --杀死对手的小孩技能Id
    m_nExtraHP = 0,       --额外生命
    m_nExtraHP_Encrypt = nil,
    m_tSoulHeroConfig = nil, 	--灵魂分身配置
    m_tDoneProfessionPassiveSkill = {}, --回合生效的职业被动技能
...
function WHero:buildHero(tStrList, nBoyOrGirl, suit_info)
	...
	hero.m_nHurtRoundNum = 0    
    hero.m_nMaxRoundHurt = 0    
    hero.m_nUseItemTimes = 0   
    hero.m_nCurRoundHurtTotal = 0
    hero.m_bIsHitEnemy = false 
    hero.m_tUsedSkillAndItemList = {}
    hero.m_tKillEnemySkill = {}
    hero.m_tKillEnemyKidSkill = {}
	...
	if hero.m_bIsMonster then
		...
		local info = GDatatab_shape_skins["id_" .. hero.m_nMonsterId]
		if info then
			hero.m_nWeaponType = info.attack_type
			hero.m_sWeaponName = info.bullet
		end
	end
...
function WHero:updateFootEffect()
    ...
    if self.m_nFootId and self.m_nFootId > 0 then
    	if not self.m_tOldFootPos then
        	self.m_tOldFootPos = 15--self:getPosition()
    	end
        local pos = self:getPosition()
        local distance = 15--GDatatab_footmark["id_" .. self.m_nFootId] and GDatatab_footmark["id_" .. self.m_nFootId].distance or 40
    --    if BattleCommon:pointDis(self.m_tOldFootPos,pos) > distance then
        if self.m_tOldFootPos > distance then
            self.m_tOldFootPos = 0
            local bValue = self:getAnimation():isFlipX()
            local tempPos = pos
            if bValue then 
            	tempPos.x = tempPos.x + 50
            else
            	tempPos.x = tempPos.x - 50
            end
            FootEffectManager:getInstance():addEffect(self.m_nFootId,tempPos,20,self.m_anim:getAnimNode():getScaleX(),self.m_anim:getAnimNode():getScaleY())
        else
        	self.m_tOldFootPos = self.m_tOldFootPos + 1
        end
    end
end
    ...
    if not self:isDead() then 
	    ...
	    if isCollision then 
	        for i, kid in pairs(tempCharas) do
	            WZLog("WHero:update", kid:getBattleId())
	            kid:setDead(true, 19)
...
function WHero:destroy()
    ...
    if self.m_skinBigSkillAnim then 
    	self.m_skinBigSkillAnim:getAnimNode():release()
    	self.m_skinBigSkillAnim = nil 
    end
...
function WHero:markHurt(nHurtValue,tShootHero,isSkillHurt,isPetHurt,isBuffHurt,hurtRatio, isTransHurt, skinSectionTotalHurt, superCritMark, beatBackPetOwnerId, isPetContinueAttack)
    local hurt = 0
	...
	WCharacter.markHurt(self,nHurtValue,tShootHero,isSkillHurt,isPetHurt,isBuffHurt,hurtRatio, isTransHurt, skinSectionTotalHurt, superCritMark, beatBackPetOwnerId, isPetContinueAttack)
end
...
--@brief 获得对话框位置
function WHero:getAnimDialogPos()
    if not self.m_tDialogPos then
        local scale = self:getAnimation():getAnimNode():getScaleY()
        local size = self:getAnimation():getAnimNode():getContentSize()
...
--@brief 生成AI控制组合
function WHero:buildGuaidAiCombination(skills,items, kmSkills, attackSkill)
    WZLog("WHero:buildGuaidAiCombination")
    self.m_tSkills = {}
    self.m_tItems = {}
    self.m_tKMSkills = {}
    self.m_tAttackSkills = {}
    ...
    for k,iv in pairs(kmSkills) do
        if iv ~= -1 then
            table.insert(self.m_tKMSkills,iv)
        end
    end
    for k,iv in pairs(attackSkill) do
        if iv ~= -1 then
            table.insert(self.m_tAttackSkills,iv)
        end
    end
end
...
function WHero:addAngerAnimation()
    ...
    if self.m_bIsGuaiWithSuit ~= true and self.m_angerAnim == nil and self:isDead() ~= true then
    	if self.m_bUseSkinBigSkill then 
			self.m_angerAnim = BattleAnimation:createAnimation("skill_nh_sd_02",false)
    	else
...
function WHero:showAttackFace(isVaild)
	-- do return end
	if not self:isRobot() or not WBattleGlobal:getCurrent():isArenaPWStage() or not WBattleGlobal:getCurrent():isArenaZLSStage() then
		return
	end
...
--@brief	获取皮肤大招对象
--@return	#1:Animation动画控制对象
function WHero:getSkinBigSkillAnimation()
	return self.m_skinBigSkillAnim
end
...
function WHero:setPosition(tPos)
	...
	if self.m_skinBigSkillAnim then 
		local tPos1 = self:getPosition()
		self.m_skinBigSkillAnim:getAnimNode():setAbsPosition(GlobalMethod:ccp(tPos1.x,tPos1.y + 0))
	end
	--更新棋圣-分身位置
	local tSubHeroList = WBattleGlobal:getCurrent():getSoulHerosList()
	if tSubHeroList then 
		for id, subHero in pairs(tSubHeroList) do
			if subHero:getOwnerPlayerId() == self:getBattleId() and (subHero:getSubType() == CharacterSubType.SUBTYPE_BCHESS or subHero:getSubType() == CharacterSubType.SUBTYPE_WCHESS) then 
				if subHero:getSubType() == CharacterSubType.SUBTYPE_BCHESS then 
					subHero:getAnimation():setPosition(Vector2:create(tPos.x - 50,tPos.y + 100))
    ...
    if self:getExtraHPBuff() then 
        ...
        if remainExtraHP > 0 then 
            for i = 1, #self.m_tHurtValue do
                local value = self.m_tHurtValue[i]
...
function WHero:setUseSkinBigSkill(bUseSkinBigSkill)
	self.m_bUseSkinBigSkill = bUseSkinBigSkill
end

--@brief 	判断英雄是否用皮肤大招
--@return 	英雄是否使用皮肤大招
function WHero:getUseSkinBigSkill()
	return self.m_bUseSkinBigSkill
end

--@brief    获取英雄皮肤大招
function WHero:getSkinBigSkill()
    -- body
...
function WHero:updateByTurn()
	...
	self.m_nUseKMSkillTime = 0
	self.m_bHurt = false
	self.m_nUseSkillId = nil 
	self.m_tSoulHeroConfig = nil 
	self.m_tDoneProfessionPassiveSkill = {} 
	--[[
	--金币副本
...
function WHero:playReadyShootAnim()
	...
	if self:getUseBigSkill() then
		if self.m_bUseSkinBigSkill then 
			if self:getSkinBigSkillAnimation() then 
				local skinBigSkill = self:getSkinBigSkill()
				if skinBigSkill == 3005 or skinBigSkill == 3006 or skinBigSkill == 3047 then
					self:getSkinBigSkillAnimation():play("attack", false)
				elseif skinBigSkill == 3016 or skinBigSkill == 3017 or skinBigSkill == 3040 then 
					self:getSkinBigSkillAnimation():play("shoot_1", false)
				elseif skinBigSkill == 3033 or skinBigSkill == 3056 or skinBigSkill == 3057 then 
					self:getSkinBigSkillAnimation():play("attack_1", false)
					self:getSkinBigSkillAnimation():playNext("attack_2")
				elseif skinBigSkill == 3034 or skinBigSkill == 3042 then 
					self:getSkinBigSkillAnimation():play("attack_2", false)
				elseif skinBigSkill == 3046 then
					self:getSkinBigSkillAnimation():play("attack1", false)
				elseif skinBigSkill == 3048 then
					self:getSkinBigSkillAnimation():play("attack", false)
					self:getSkinBigSkillAnimation():playNext()
				elseif skinBigSkill == 3049 or skinBigSkill == 3051 then
					self:getSkinBigSkillAnimation():play("wait", true)
				else
					self:getSkinBigSkillAnimation():play("attack_1", false)
					if skinBigSkill == 3011 or skinBigSkill == 3014 or skinBigSkill == 3015 or skinBigSkill == 3023 or skinBigSkill == 3024 or skinBigSkill == 3026 or skinBigSkill == 3028 or skinBigSkill == 3029 or skinBigSkill == 3030 or skinBigSkill == 3032 or skinBigSkill == 3036 or skinBigSkill == 3037 or skinBigSkill == 3041 or skinBigSkill == 3054 or skinBigSkill == 3058 or skinBigSkill == 3059 then 
						self:getSkinBigSkillAnimation():playNext()
					end
				end
			end
...
function WHero:playRepeatShootAnim(RepeatTimes)
	...
	if self:getUseBigSkill() then
		if self.m_bUseSkinBigSkill then
			if self:getSkinBigSkill() == 3032 or self:getSkinBigSkill() == 3037 or self:getSkinBigSkill() == 3045 or self:getSkinBigSkill() == 3058 then 
				self:getSkinBigSkillAnimation():play("attack_2", false)
			elseif self:getSkinBigSkill() == 3033 or self:getSkinBigSkill() == 3056 or self:getSkinBigSkill() == 3057 then
				self:getSkinBigSkillAnimation():play("attack_3", false)
			elseif self:getSkinBigSkill() == 3049 then
				self:getSkinBigSkillAnimation():play("attack_1", false)
			end
		else
			if self.m_nWeaponType == 0 --[[self:getBigSkillType() == 0 or self:getBigSkillType() == 2]] then
...
function WHero:playEndShootAnim()
	...
	if self:getUseBigSkill() then
		if self.m_bUseSkinBigSkill then
			if self:getSkinBigSkill() == 3046 or self:getSkinBigSkill() == 3051 then
				if not (self:getSkinBigSkillAnimation():isCurrentAnimationDone() ~= true and self:getSkinBigSkillAnimation():getAnimNode():getAnimationName() == "attack3") then
					self:getSkinBigSkillAnimation():play("attack3", false)
				end
			elseif self:getSkinBigSkill() == 3047 then
				if not (self:getSkinBigSkillAnimation():isCurrentAnimationDone() ~= true and self:getSkinBigSkillAnimation():getAnimNode():getAnimationName() == "attack") then
					self:getSkinBigSkillAnimation():play("wait", false)
				end
			else
				self:getSkinBigSkillAnimation():play("wait", true)
			end
...
function WHero:addUseKMSkillTime(nTime)
	self.m_nUseKMSkillTime = self.m_nUseKMSkillTime + nTime
	WndBattleHud:updateSkillItem(self:getBattleId())
end

function WHero:getUseKMSkillTime()
	if WBattleGlobal:getCurrent():isReplayGame() then
		return 0
	end
    WZLog("WHero:getUseSkillTime", tostring(TeachGroup1.ISBATTLE))
	return self.m_nUseKMSkillTime
end
...
--@brief    皮肤大招攻击时候，隐藏英雄
function WHero:_showSkinHero()
    -- body
    WZLog("WHero:_showSkinHero")
    if self.m_skinBigSkillAnim then 
    	self:getAnimation():getAnimNode():setOpacity(0)
        
        local bValue = self:getAnimation():isFlipX()
        self.m_skinBigSkillAnim:getAnimNode():setFlipX(bValue)
        self.m_skinBigSkillAnim:play("wait", true)
        
        self.m_skinBigSkillAnim:getAnimNode():setVisible(true)
        if self.m_nHideOpecity then 
            self.m_skinBigSkillAnim:getAnimNode():setOpacity(self.m_nHideOpecity)
        else
            self.m_skinBigSkillAnim:getAnimNode():setOpacity(255)
        end
    end
end

--@brief    皮肤大招攻击结束后，显示英雄
function WHero:_showAttackHero()
    -- body
    WZLog("WHero:_showAttackHero", self.m_skinBigSkillAnim, self.m_skinBigSkillAnim:isCurrentAnimationDone())
    if self.m_skinBigSkillAnim then 
    	if self:getSkinBigSkill() == 3011 and self:getRunStatus() == RunStatus.DEF_ST_FLY then 
    		return false 
    	else
	        if self.m_skinBigSkillAnim:getAnimNode():getAnimationName() ~= "wait" then 
	        	self.m_skinBigSkillAnim:getAnimNode():setVisible(true)
	        	local tPos1 = self:getPosition()
				self.m_skinBigSkillAnim:getAnimNode():setAbsPosition(GlobalMethod:ccp(tPos1.x,tPos1.y + 0))
	            self.m_skinBigSkillAnim:play("wait", false)
	            return false 
	        else 
                self.m_skinBigSkillAnim:play("wait", true)
                self.m_skinBigSkillAnim:getAnimNode():setVisible(false)
...
--@brief    回合结束，隐藏皮肤，显示正常英雄
function WHero:_showNormalHero()
    -- body
    WZLog("WHero:_showNormalHero")
    if self.m_skinBigSkillAnim then 
        if self.m_skinBigSkillAnim:getAnimNode():isVisible() == true then 
            self.m_skinBigSkillAnim:play("wait", true)
            self.m_skinBigSkillAnim:getAnimNode():setVisible(false)
...
--@brief    获取最大伤害
function WHero:getMaxHurt()
    -- body
    return self.m_nMaxRoundHurt
end

--@brief    获取命中率
function WHero:getHitRate()
    -- body
    return math.floor(self.m_nHurtRoundNum/self:getAttackRound() * 100)
end

--@brief    获取使用道具次数
function WHero:getUseItemTimes()
    -- body
    return self.m_nUseItemTimes
end

--@brief    获取使用过的技能道具列表
function WHero:getUseSkillAndItemList()
    -- body
    return self.m_tUsedSkillAndItemList
end

--@brief    获取杀死敌人的技能Id列表
function WHero:getKillEnemySkills()
    -- body
    return self.m_tKillEnemySkill
end

function WHero:getSceneAnchorPoint()
    return GlobalMethod:ccp(0.5,0.15)
end

--@brief 	执行击杀生效
function WHero:doKillEffect()
	-- body
...
--@brief 	玩家是否使用皮肤近身攻击
function WHero:isUseSkinAttack()
	-- body
	WZLog("WHero:isUseSkinAttack zero", tostring(self.m_bIsMonster), tostring(self.m_nUseSkillId))
	if self.m_bIsMonster and self.m_nUseSkillId then 
		local action = GDatatab_shape_skins["id_" .. self.m_nMonsterId].action
		local param4 = GDatatab_skill["id_" .. self.m_nUseSkillId].param4
		WZLog("WHero:isUseSkinAttack", action, param4)
		if action == 1 and param4 == 1 then 
			return true
		end
	end
...
--@brief 	设置本回合使用的技能
function WHero:setUseSkillId(skillId)
	-- body
	local skillData = GDatatab_skill["id_" .. skillId]
	WZLog("WHero:setUseSkillId", skillData.param4, tostring(self.m_nUseSkillId))
	if skillData and skillData.param4 == 1 and self.m_nUseSkillId == nil then 
		self.m_nUseSkillId = skillId
	end
end

--@brief    皮肤近身攻击后，显示形象
function WHero:_showHero()
    -- body
    WZLog("WHero:_showHero")

    if self.m_nHideOpecity then 
        self:getAnimation():getAnimNode():setOpacity(self.m_nHideOpecity)
    else
...
--@param	dt:距离上一次调用的时间（秒）
--@note		由定时器调用
function WHero:pickGhostSkillUpdate(dt)
    --WZLog("WHero:pickGhostSkillUpdate zero")
    if self:isDead() and self:isCanControl() then
        local isCollision, uniqueId = self:checkCollisionGhostBox()
        if isCollision == true and uniqueId > 0 then
            ProtocolProcessorSceneBattle:send_BATTLE_GetGhostSkill(WBattleGlobal:getCurrent():getBattleId(), self:getBattleId(), uniqueId)
        end
    end
end
...
--@return	#2:英雄
function WHero:checkCollisionGhostBox()
    --WZLog("WHero:checkCollisionGhostBox")
	local isCollision = false
	local uniqueId = 0
	if WndBattleHud.m_tTotalGhostSkill and #WndBattleHud.m_tTotalGhostSkill > 0 then 
		for i = 1, #WndBattleHud.m_tTotalGhostSkill do
			if WndBattleHud.m_tTotalGhostSkill[i] and not WndBattleHud.m_tTotalGhostSkill[i].pickMark then 
			    local x,y = WndBattleHud.m_tTotalGhostSkill[i].spineBox:getPosition()

			    local posV2 = Vector2:create(x,y)
			    local nScale = WndBattleHud.m_tTotalGhostSkill[i].spineBox:getScale()
			    local raduis = WndBattleHud.m_tTotalGhostSkill[i].spineBox:getContentSize().width * nScale * 2 / 5
		--	    WZLog("WHero:checkCollisionGhostBox", raduis, WndBattleHud.m_tTotalGhostSkill[i].spineBox:getContentSize().width, nScale)
				...
				if isCollision then 
					uniqueId = WndBattleHud.m_tTotalGhostSkill[i].uniqueId
					WndBattleHud.m_tTotalGhostSkill[i].pickMark = true --防止一个宝箱多次请求拾取
					break 
				end
			end
		end
...
--@return	#1:true:撞了,false:没撞
--@return	#2:碰撞的人物列表
function WHero:checkCollisionWithCharacterList(pos,raduis)
	--WZLog("WHero:checkCollisionWithCharacterList")
	local isCollision = false
    local chara = self
    --检测与人物的碰撞
...
--@brief 	获取玩家小孩数据
function WHero:getKidInfo()
	-- body
	return self.m_tKidInfo
end
...
function WHero:receiveBuildKid(childCombatId, skillId, positionX, positionY)
	--body
    self.kidBattleId = childCombatId
    self.kidPositionX = positionX
    self.kidPositionY = positionY
  
    WZLog("WHero:receiveBuildKid one", childCombatId, skillId, positionX, positionY)
end

--@brief    检测小孩碰撞
--@return   #1:true:撞了,false:没撞
--@return   #2:碰撞的人物列表
function WHero:checkKidCollision()
    --WZLog("WHero:checkHeroCollision")
...
--        WZLog("WHero:checkCollision three")
        ...
        if isCollision then
            ...
            -- WZLog("WHero:checkCollision collosion",curPos.x)
...
--@return   #1:true:撞了,false:没撞
--@return   #2:碰撞的人物列表
function WHero:checkCollisionWithKidList(pos)
    local tmpCharas = {}
    local isCollision = false
    local charaRaidus = self:getRadiusForBulletCollision()
    for id, kid in ipairs(WBattleGlobal:getCurrent():getKidSortList()) do
--        WZLog("WHero:checkCollisionWithKidList one", id, tostring(kid:isDead()), kid:getBattleId())
 ...
 --           WZLog("WHero:checkCollisionWithKidList two", id, kid:getBattleId(), tostring(kid:isDead()), charaPos.x, charaPos.y)
            local _isCollision = self:checkCollisionWithRang(pos, charaRaidus, charaPos, collisionRang)

            if _isCollision then
--                WZLog("WHero:checkCollisionWithKidList four")
...
--@param    rang:区域
--@return   #1:true:撞了,false:没撞
function WHero:checkCollisionWithRang(pos, charaRaidus, charaPos, collisionRang)
    local dis = nil
    if collisionRang ~= nil then
        for i,rang in pairs(collisionRang) do
            if rang.m_nType == 1 then
                ...
                local circle = {x = pos.x, y=pos.y, r = charaRaidus}
                local _isCollision = BattleCommon:rectCircleOverLap(rect, circle)
                WZLog("WHero:checkCollisionWithRang two", i)
                if _isCollision then
                   return true
                end
            end
    ...
    WZLog("WHero:checkCollisionWithRang four")
    return false
end

--@brief    获取杀死敌人的小孩技能Id列表
function WHero:getKillEnemyKidSkills()
    -- body
    return self.m_tKillEnemyKidSkill
end

--@brief 	设置额外血量
--@param 	nHp 当前额外血量
function WHero:setExtraHp(nHp, bSet)
    nHp = tonumber(nHp)
    if self.m_nExtraHP == nHp then
        return
    end
    WZLog("WHero:setExtraHp one",self:getId(), self.m_nExtraHP, nHp)
...
--@brief 	获得人物当前额外血量
--@return 	#1,人物当前额外血量
function WHero:getExtraHp()
	return self.m_nExtraHP
end

--@brief 	设置灵魂分身数据配置
function WHero:setSoulHeroConfig(effectParm)
	self.m_tSoulHeroConfig = {}
	self.m_tSoulHeroConfig.callNum = effectParm[5]
	self.m_tSoulHeroConfig.monsterIds = {}
	local nIndex = 1
	for i = 6, 5 + effectParm[5] do 
		self.m_tSoulHeroConfig.monsterIds[nIndex] = effectParm[i]
...
--@brief 	获取灵魂分身数据配置
function WHero:getSoulHeroConfig()
	return self.m_tSoulHeroConfig
end

--@brief    获得人物当前累加的最大额外血量
--@return   #1,人物当前最大额外血量（叠加的）
function WHero:getMaxExtraHp()
...
--@return   #1,人物当前最大额外血量（叠加的）
function WHero:changeMaxExtraHp(param, bIsAdd)
	...
	if bIsAdd then 
    	...
    	self.m_nExtraHP = self.m_nExtraHP + addValue
    else
    	self.m_nMaxExtraHP = self.m_nMaxExtraHP - addValue
    end
