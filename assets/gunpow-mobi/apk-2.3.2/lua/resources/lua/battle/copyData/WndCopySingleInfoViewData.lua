--WndCopySingleInfoViewData.lua
--@brief	WndCopySingleInfoView的数据模块
--@date		2015/11/09
--@author	mbq
--@note		单人副本战斗UI

WndCopySingleInfoView = {
	--请不要在这里定义变量
}

--@brief    定义并初始化表的成员变量
--@note     变量的定义和初始化必须在这里完成,外部无需调用该方法
function WndCopySingleInfoView:_init()
...
function WndCopySingleInfoView:_unInit()
    self.m_root = nil
end
...
--@note     请仅用此方法创建场景
function WndCopySingleInfoView:createElement()
    local element = WZUISystem:getInstance():createElement("WndCopySingleInfoView")
    assert(element, "WndCopySingleInfoView create element failed!")
    self:_init()
    return element
end
...
-------------------------------------公有方法模块End----------------------------------------
--@brief 监听事件
function WndCopySingleInfoView:_initEvent()
    WZLog("WndCopySingleInfoView:_initEvent")
...
--@brief 移除事件
function WndCopySingleInfoView:_removeEvent()
    WZLog("WndCopySingleInfoView:_removeEvent")
...
--@brief 伤害回调
function WndCopySingleInfoView:_characterHurtHandler(hurter)
    local hero  = WBattleGlobal:getCurrent():getMyHero()
    WZLog("WndCopySingleInfoView:_characterHurtHandler",hero:getHp(),hero:getBattleId(),hurter:getBattleId())
...
--@brief 攻击回合回调
function WndCopySingleInfoView:_playerAttRoundUpdateHandler()

    local hero  = WBattleGlobal:getCurrent():getMyHero()
    local attRound = hero:getAttackRound()
    WZLog("WndCopySingleInfoView:_playerAttRoundUpdateHandler", tostring(TeachGroup1.attackRound), attRound)
