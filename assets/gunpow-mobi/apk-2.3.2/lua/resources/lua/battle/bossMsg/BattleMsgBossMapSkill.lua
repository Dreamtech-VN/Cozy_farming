...
BattleMsgBossMapSkill = {
    ...
    m_tPetEffectBornPos = nil,       --技能召唤怪出生点，针对宠物装备技能黑洞加的
}
...
function BattleMsgBossMapSkill:init()
    ...
    if TeachGroup1.ISBATTLE_MYTURN ~= true and SceneBattle:getBattleLoop():getBattleStatus() ~= BattleLoop.S_NORMAL and (self.m_tSkillTypeList[1] ~= SkillTypeConfig.EFFECT and self.m_tSkillTypeList[1] ~= SkillTypeConfig.HIT_DO_EFFECT and self.m_tSkillTypeList[1] ~= SkillTypeConfig.BEHIT_DO_EFFECT) then
        return "done"
    end

    if self.m_tSkillTypeList[1] and self.m_tSkillTypeList[1] ~= SkillTypeConfig.EFFECT and self.m_tSkillTypeList[1] ~= SkillTypeConfig.HIT_DO_EFFECT and self.m_tSkillTypeList[1] ~= SkillTypeConfig.BEHIT_DO_EFFECT then
...
--    if hero == nil or hero:isDead() then
    if hero == nil then --宇航員皮肤大招首次攻击触发宠物反击致出手玩家死亡卡战斗的问题修改
    ...
    --协议发送
    if self.m_tSkillTypeList[1] and self.m_tOwner:isCanControl() and (self.m_tSkillTypeList[1] ~= SkillTypeConfig.EFFECT and self.m_tSkillTypeList[1] ~= SkillTypeConfig.HIT_DO_EFFECT and self.m_tSkillTypeList[1] ~= SkillTypeConfig.BEHIT_DO_EFFECT ) then
...
--    if not self.m_tOwner or self.m_tOwner:isDead() then
    if not self.m_tOwner then --宇航員皮肤大招首次攻击触发宠物反击致出手玩家死亡卡战斗的问题修改
        return true
    end
    
    if self.m_tSkillTypeList[1] and SceneBattle:getBattleLoop():getBattleStatus() ~= BattleLoop.S_PLAYER_SHOOT and self.m_tSkillTypeList[1] ~= SkillTypeConfig.EFFECT and self.m_tSkillTypeList[1] ~= SkillTypeConfig.HIT_DO_EFFECT and self.m_tSkillTypeList[1] ~= SkillTypeConfig.BEHIT_DO_EFFECT then
        return true
    end
    ...
    if #self.m_tStepFunction > 0 then
        local res = self.m_tStepFunction[1][1](self,self.m_tStepFunction[1][2],self.m_tStepFunction[1][3],self.m_tStepFunction[1][4], self.m_tStepFunction[1][5], self.m_tStepFunction[1][6], self.m_tStepFunction[1][7])
...
function BattleMsgBossMapSkill:done()
    ...
    WZLog("BattleMsgBossMapSkill:done zero-11",tostring(MsgManager.m_tNonBlockMsgList ~= nil and MsgManager.m_tNonBlockMsgList[1] ~= nil and MsgManager.m_tNonBlockMsgList[1].m_nSkillStatusCount), tostring(MsgManager.m_tNonBlockMsgList ~= nil and MsgManager.m_tNonBlockMsgList[1] ~= nil and MsgManager.m_tNonBlockMsgList[1].m_sName))
    ...
    if self.m_nTakeEffectType ~= 2 and (MsgManager.m_tBlockMsgList ~= nil and MsgManager.m_tBlockMsgList[1] ~= nil and MsgManager.m_tBlockMsgList[1].m_nSkillStatusCount > 0) then
        ...
        return
    elseif self.m_nTakeEffectType ~= 2 and MsgManager.m_tNonBlockMsgList ~= nil and MsgManager.m_tNonBlockMsgList[1] ~= nil then
        for i, msg in pairs(MsgManager.m_tNonBlockMsgList) do
            if msg.m_nSkillStatusCount > 0 and msg.m_sName == "BattleMsgKidShoot" then
                if self.m_tHurtBullet and self.m_bIsDoSkill == nil then
                    self.m_bIsDoSkill = true
                    if self.m_tOwner:getType() == 0 then
                        local charas, values, distance, critType,hurtRatios = self.m_tHurtBullet:checkHurt()
                        self:_charaAddHurtValue(charas,values,hurtRatios)
                        WZLog("WBullet:checkHurt three-2",Serialize(values), Serialize(distance))
                        self:_sendHurtProtocol(charas,nil,values,distance,critType)
                    ...
                    self.m_tHurtBullet:destroy()
                    self.m_tHurtBullet = nil
                    WBattleGlobal:getCurrent():setDoEffectAfterAttack(nil,"bossMapSkill-5")
                end

                WZLog("BattleMsgBossMapSkill:done two", tostring(self:waitForHurtNum()), msg.m_nSkillStatusCount)
                if self:waitForHurtNum() ~= true then
                    return false
                end
                WBattleGlobal:getCurrent():setDoEffectAfterAttack(nil,"bossMapSkill-4")

                msg.m_nSkillStatusCount = msg.m_nSkillStatusCount - 1
            elseif msg.m_nSkillStatusCount >= 0 and msg.m_sName == "BattleMsgPlayerShoot" then --棋圣分身出手后，m_nSkillStatusCount大于0，卡战斗
                msg.m_nSkillStatusCount = msg.m_nSkillStatusCount - 1
            end
        end
    end
...
--@brief    添加步骤
--@param    targetHeroHurts:子弹击中目标对应的伤害值，与param参数中的目标对象一一对应
--@note     添加处理步骤
function BattleMsgBossMapSkill:addStep(skillType, param, isBuff, param2, takeType, targetHeroHurts)
    if TeachGroup1.ISBATTLE_MYTURN == true then
        param = {WBattleGlobal:getCurrent():getMyHero()}
    end
    WZLog("BattleMsgBossMapSkill:addStep zero", skillType, tostring(takeType), tostring(targetHeroHurts))
    ...
    elseif skillType == EffectTypeConfig.CHANGE_HURT_VALUE or skillType == EffectTypeConfig.CHANGE_HURT_PERCENT or skillType == EffectTypeConfig.CHANGE_HURT_ADD_VALUE or skillType == EffectTypeConfig.CHANGE_HURT_MUL_PERCENT or skillType == EffectTypeConfig.CHANGE_WIND_ADD_HURT_PERCENT then
        table.insert(self.m_tStepFunction,{self._ChangeHurt,param,skillType,isBuff,param2})
    ...
    elseif skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_VALUE or skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT or skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT_HURT or skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT_HURT2 or skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT_HURT3 then
        table.insert(self.m_tStepFunction,{self._changeAttribute,param,skillType,isBuff,param2,takeType, targetHeroHurts})
    elseif skillType == EffectTypeConfig.CHANGE_CTB_VALUE then
        WZLog("BattleMsgBossMapSkill:addStep CTB", self._changeCtbValue,param,skillType,tostring(isBuff),self.m_nChangeValue)
        table.insert(self.m_tStepFunction,{self._changeCtbValue,param,skillType,isBuff,self.m_nChangeValue})
    ...
    elseif skillType == EffectTypeConfig.XUANZANG_TOTEM then
        self.m_bIsSummonMsg = true
        table.insert(self.m_tStepFunction,{self._waitMonsterId})
        table.insert(self.m_tStepFunction,{self._buildXuanZangTornado})
    ...
    elseif skillType == EffectTypeConfig.IMMUNITY_BUFF_ASSIGN then
        table.insert(self.m_tStepFunction,{self._immunityBuff,param, self.m_nImmunityBuffType})
    ...
    elseif skillType == EffectTypeConfig.PET_CONTINUE_SHOOT then
        self.m_tOwner:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.PET_CONTINUE_SHOOT})
        table.insert(self.m_tStepFunction,{self._changePetAtkTimes})
    elseif skillType == EffectTypeConfig.PROFESSION_PET_BEATBACK then
        self.m_tOwner:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.PROFESSION_PET_BEATBACK, maxTimes = 1})
    elseif skillType == EffectTypeConfig.THORNS then
        table.insert(self.m_tStepFunction,{self._thornsStart,param,isBuff})
    elseif skillType == SkillTypeConfig.BEHIT_DO_EFFECT then
        table.insert(self.m_tStepFunction,{self._getEffectConfigWithId})
        table.insert(self.m_tStepFunction,{self._doEffect})
    ...
    elseif skillType == EffectTypeConfig.RANDOM_SCATTER_SHOOT then
        table.insert(self.m_tStepFunction,{self._changeScatter,param})
    elseif skillType == EffectTypeConfig.ABSORB_ATTRIBUTE_BECOMES_HURT then
        table.insert(self.m_tStepFunction,{self._AbsorbAttributeBecomesHurt,param,skillType,isBuff,param2})
   ...
   --     table.insert(self.m_tStepFunction,{self._requestMonsterId})
    ...
    elseif skillType == EffectTypeConfig.ADD_RANDOMBUFF then
        WZLog("BattleMsgBossMapSkill:addStep one_addRandomBuff", self.m_nAddBuffId)
        table.insert(self.m_tStepFunction,{self._addRandomBuff})
    elseif skillType == EffectTypeConfig.ADD_RANDOM_BUFF_TWO then
        WZLog("BattleMsgBossMapSkill:addStep ADD_RANDOM_BUFF_TWO", self.m_nAddBuffId)
        table.insert(self.m_tStepFunction,{self._addRandomBuffTwo,param})
    ...
    elseif self.m_tSkillTypeList and self.m_tSkillTypeList[1] == SkillTypeConfig.BEHIT_DO_EFFECT and skillType == EffectTypeConfig.THORNS_HURT_PERCENT then
        table.insert(self.m_tStepFunction,{self._percentThornsStart,param})
    elseif skillType == EffectTypeConfig.REDUCE_BUFFCTB_ALL then
        table.insert(self.m_tStepFunction,{self._reduceBuffCTB,param, self.m_tReduceBuffConfig})
    elseif skillType == EffectTypeConfig.MARITIME1_TOTEM then
        self.m_bIsSummonMsg = true
        table.insert(self.m_tStepFunction,{self._waitMonsterId})
        table.insert(self.m_tStepFunction,{self._buildMaritime1Totem})
    elseif skillType == EffectTypeConfig.MARITIME2_TOTEM then
        self.m_bIsSummonMsg = true
        table.insert(self.m_tStepFunction,{self._waitMonsterId})
        table.insert(self.m_tStepFunction,{self._buildMaritime2Totem})
    elseif skillType == EffectTypeConfig.MARITIME3_TOTEM then
        self.m_bIsSummonMsg = true
        table.insert(self.m_tStepFunction,{self._waitMonsterId})
        table.insert(self.m_tStepFunction,{self._buildMaritime3Totem})
    elseif skillType == EffectTypeConfig.JIANGZIYA_TOTEM then
        self.m_bIsSummonMsg = true
        table.insert(self.m_tStepFunction,{self._waitMonsterId})
        table.insert(self.m_tStepFunction,{self._buildFireTotem, MonsterType.JIANGZIYA_TOTEM})
    elseif skillType == EffectTypeConfig.UMBRELLA1_TOTEM then
        self.m_bIsSummonMsg = true
        table.insert(self.m_tStepFunction,{self._waitMonsterId})
        table.insert(self.m_tStepFunction,{self._buildUmbrella1Totem})
    elseif skillType == EffectTypeConfig.UMBRELLA2_TOTEM then
        self.m_bIsSummonMsg = true
        table.insert(self.m_tStepFunction,{self._waitMonsterId})
        table.insert(self.m_tStepFunction,{self._buildUmbrella2Totem})
    elseif skillType == EffectTypeConfig.DISPERSE_MONSTER_BY_TYPE then
        WZLog("DISPERSE_MONSTER_BY_TYPE")
...
--@brief    获取效果配置
function BattleMsgBossMapSkill:_getEffectConfigWithId()
    WZLog("BattleMsgBossMapSkill:_getEffectConfigWithId", self.m_nEffcetId)
    local config = self:_getEffectData(self.m_nEffcetId)
    self.m_tEffcetConfig = config.effect

    WZLog("BattleMsgBossMapSkill:_getEffectConfigWithId two", self.m_nEffcetId, Serialize(self.m_tEffcetConfig), Serialize(self.m_tOwner.m_fRectForBulletExplodeBombChange or {}))
end
-------------------------------------私有方法模块--------------------------------------
...
function BattleMsgBossMapSkill:_getEffectConfig()
    ...
    if skillInfo.skill_type == 0 or skillInfo.skill_type == 2 or skillInfo.skill_type == 6 then
    ...
    if type(skillInfo.boom_scope) == "table" then
        self.m_tOwner.m_fRectForBulletExplodeBombChange = {x=skillInfo.boom_scope[1][1],y=skillInfo.boom_scope[1][2],digHoleDir=skillInfo.direction}
        WZLog("BattleMsgBossMapSkill:_getEffectConfig one", skillInfo.effect_id[1][1], Serialize(skillInfo.boom_scope))
    end
    if self.m_nEffcetId == 62 and WBattleGlobal:getCurrent():isDigGappingFighting() then 
        self.m_tOwner.m_tSkillCdList[self.m_nEffcetId] = skillInfo.cooling_time + g_nAdditionCoolTime
    else
        self.m_tOwner.m_tSkillCdList[self.m_nEffcetId] = skillInfo.cooling_time
    end

    if skillInfo.skill_type ~= 1 and skillInfo.skill_type ~= 4 and skillInfo.skill_type ~= 5 and skillInfo.skill_type ~= 9 and skillInfo.skill_type ~= 10 and skillInfo.skill_type ~= 7 and skillInfo.skill_type ~= 11 then
        self.m_tOwner.m_bIsUseSkill = true
    end
    ...
    elseif skillInfo.sub_type == SkillTableTypeConfig.TWOSHOOT then
        self.m_tOwner.m_bIsTwoShootBomb = true
    elseif skillInfo.sub_type == SkillTableTypeConfig.EASYHURT then
        self.m_tOwner.m_tIsEasyHurtBomb = true
    elseif skillInfo.sub_type == SkillTableTypeConfig.CALM then
        self.m_tOwner.m_bIsCalmBomb = true
    elseif skillInfo.sub_type == SkillTableTypeConfig.SPEED then
        self.m_tOwner.m_bIsSpeedBomb = true
    end
...
function BattleMsgBossMapSkill:_doEffect()
    local skillInfo = self:_getSkillData(self.m_nEffcetId)
    WZLog("BattleMsgBossMapSkill:_doEffect", self.m_nEffcetId, Serialize(self.m_tEffcetConfig), skillInfo and Serialize(skillInfo))
    local effect = self.m_tEffcetConfig
    if skillInfo == nil and self.m_nEffcetId and (self.m_nEffcetId == 1402 or self.m_nEffcetId == 1403 or (self.m_nEffcetId >= 1405 and self.m_nEffcetId <= 1414)) then 
        local tempConfig = self:_getEffectData(self.m_nEffcetId)
        effect = tempConfig.effect
    end
    local boss = self.m_tOwner
    WZLog("BattleMsgBossMapSkill:_doEffect 000", boss:getBattleId())
    for i, skillParm in pairs (effect) do
        ...
        if boss.m_tSkillTakeEffectInfo == self.m_nEffcetId and boss.m_tSkillTakeEffectIndex and #boss.m_tSkillTakeEffectIndex > 0 and (self.m_nTakeEffectType == nil or (self.m_nTakeEffectType and self.m_nTakeEffectType ~= TakeEffectType.COLLISION)) then
        ...
        elseif self.m_tSkillTypeList[1] == SkillTypeConfig.BEHIT_DO_EFFECT then
            isCanTakeEffect = true
        else
            isCanTakeEffect = nil
        end
        WZLog("BattleMsgBossMapSkill:_doEffect haha", tostring(isCanTakeEffect), tostring(effectParm.isTakeEffect), turnTime, tostring(self.m_nTakeEffectType))
        if ((isCanTakeEffect == nil and (takeEffectParm == TakeEffectType.USE or takeEffectParm == TakeEffectType.BEFORE_DOEFFECT or (boss.m_bActiveAttack == true and (takeEffectParm == TakeEffectType.HIT or takeEffectParm == TakeEffectType.HIT_TWO or takeEffectParm == TakeEffectType.HIT_ENEMY or takeEffectParm == TakeEffectType.BULLET_HIT or takeEffectParm == TakeEffectType.AFTERHURT_DOEFFECT or takeEffectParm == TakeEffectType.AFTER_BLEED) and (skillInfo and skillInfo.skill_type ~= 4 or ((skillInfo and skillInfo.sub_type == 1) or (skillInfo and skillInfo.sub_type == 0 and boss.m_bPetActiveAttack)) or (self.m_nEffcetId and (self.m_nEffcetId == 1402 or self.m_nEffcetId == 1403 or (self.m_nEffcetId >= 1405 and self.m_nEffcetId <= 1414))))) or 
            (boss.m_bActiveAttack == true and (takeEffectParm == TakeEffectType.COLLISION or (boss.m_bActiveAttack2 == true and takeEffectParm == TakeEffectType.COLLISION_TWO))))) or isCanTakeEffect == true) and (effectParm.isTakeEffect == nil or 
            turnTime > effectParm.isTakeEffect) and takeEffectParm ~= TakeEffectType.BETWEEN_ATTACK_HURT then

            local targetParm = effectParm[2]
            effectParm.isTakeEffect = turnTime
            local targetHeroList, targetHeroHurts = nil, nil 
            ...
            elseif self.m_bIsWChess then 
                targetHeroList = self:_getHeroById(self.m_tTargetPlayerId)
            else
                targetHeroList, targetHeroHurts = self:_chooseEffectTarget(takeEffectParm,targetParm)
            end
        
            if takeEffectParm == TakeEffectType.HIT or takeEffectParm == TakeEffectType.HIT_TWO or takeEffectParm == TakeEffectType.HIT_ENEMY or takeEffectParm == TakeEffectType.BULLET_HIT or takeEffectParm == TakeEffectType.AFTERHURT_DOEFFECT then
            ...
            local bIsDoContinue = true 
            --爆破生效时候，执行效果，屏蔽掉立即生效的配置，只执行爆破生效的效果
            if boss.m_bActiveAttack == true and self.m_nTakeEffectType and (self.m_nTakeEffectType == TakeEffectType.COLLISION or (boss.m_bActiveAttack2 and self.m_nTakeEffectType == TakeEffectType.COLLISION_TWO)) and boss:getUseSkinBigSkill() and (boss:getSkinBigSkill() == 3030 or boss:getSkinBigSkill() == 3038) then 
                bIsDoContinue = false
                if (takeEffectParm == TakeEffectType.COLLISION or takeEffectParm == TakeEffectType.COLLISION_TWO) and boss.m_tSkillTakeEffectCollionIndex and #boss.m_tSkillTakeEffectCollionIndex > 0 then 
                    for index, u in pairs(boss.m_tSkillTakeEffectCollionIndex) do
                        if i == u then
                            bIsDoContinue = true 
                            break 
                        end
            ...
            elseif boss.m_bActiveAttack == true and self.m_nTakeEffectType and (self.m_nTakeEffectType == TakeEffectType.BULLET_HIT or self.m_nTakeEffectType == TakeEffectType.HIT or self.m_nTakeEffectType == TakeEffectType.AFTER_BLEED) and takeEffectParm ~= self.m_nTakeEffectType then
                bIsDoContinue = false
            elseif boss.m_bActiveAttack == true and self.m_nTakeEffectType and (self.m_nTakeEffectType == TakeEffectType.HIT_ENEMY or takeEffectParm == TakeEffectType.HIT_ENEMY) and takeEffectParm ~= self.m_nTakeEffectType then 
                bIsDoContinue = false
            elseif boss.m_bActiveAttack == true and self.m_nTakeEffectType and self.m_nTakeEffectType == TakeEffectType.COLLISION and takeEffectParm ~= self.m_nTakeEffectType then --针对武器大招天命棍法加的限制，不然伤害加成会加两次
                if not boss:getUseSkinBigSkill() then --修复最强一击/审批大招/玄奘皮肤大招  击中生效效果没有生效的问题
                    bIsDoContinue = false 
                end
            elseif boss.m_bActiveAttack == true and self.m_nTakeEffectType and (self.m_nTakeEffectType == TakeEffectType.KILL or takeEffectParm == TakeEffectType.KILL) and takeEffectParm ~= self.m_nTakeEffectType then
                bIsDoContinue = false
            end
            if bIsDoContinue then 
                ...
                elseif effect == EffectTypeConfig.CHANGE_HURT_PERCENT or effect == EffectTypeConfig.CHANGE_WIND_ADD_HURT_PERCENT then
                    param2 = self.m_nHurtAddPercent
                end
                WZLog("BattleMsgBossMapSkill:_doEffect two_00", takeEffectParm)
                self:addStep(effect, targetHeroList, nil, param2, takeEffectParm, targetHeroHurts)
            end
        elseif skillInfo and skillInfo.skill_type == 4 and skillInfo.sub_type == 0 and takeEffectParm == TakeEffectType.HIT then
            boss.m_tPetSkillTakeEffectInfo = boss.m_tPetSkillTakeEffectInfo or {}
            --防止宠物虚弱技能会添加两次
            if not utilsValueInTable(self.m_nEffcetId, boss.m_tPetSkillTakeEffectInfo) then 
                table.insert(boss.m_tPetSkillTakeEffectInfo, self.m_nEffcetId)
            end
            WZLog("pet hit use effect",self.m_nEffcetId)
        elseif boss.m_bActiveAttack ~= true and (takeEffectParm == TakeEffectType.HIT or takeEffectParm == TakeEffectType.HIT_ENEMY or takeEffectParm == TakeEffectType.BULLET_HIT or takeEffectParm == TakeEffectType.BETWEEN_ATTACK_HURT or takeEffectParm == TakeEffectType.AFTERHURT_DOEFFECT) then
            ...
            elseif effect == EffectTypeConfig.TWOSHOOT then
                self.m_tOwner.m_nIsSecondShoot = true
            elseif effect == EffectTypeConfig.SPATTER_TWO then
                self.m_tOwner.m_nIsSpatterTwo = true
            end
        ...
        elseif boss.m_bActiveAttack == true and takeEffectParm == TakeEffectType.COLLISION_TWO then
            local effect = effectParm[3] .. "_" ..effectParm[4]
            WZLog("碰撞后生效2 COLLISION_TWO",self.m_nEffcetId, effect)
            -- boss.m_tSkillTakeEffectCollionList = {}
            boss.m_tSkillTakeEffectCollionInfo2 = self.m_nEffcetId
            if boss.m_tSkillTakeEffectCollionIndex == nil then
                boss.m_tSkillTakeEffectCollionIndex = {}
            end
            table.insert(boss.m_tSkillTakeEffectCollionIndex, i)
        ...
        elseif boss.m_bActiveAttack == true and takeEffectParm == TakeEffectType.COLLISION and self.m_nTakeEffectType == takeEffectParm and (self.m_nEffcetId == 3029 or self.m_nEffcetId == 3054 or self.m_nEffcetId == 3056) then 
            WZLog("big skin skill 3029|3054|3056")
            local targetParm = effectParm[2]
            effectParm.isTakeEffect = turnTime
            local targetHeroList = self:_chooseEffectTarget(takeEffectParm,targetParm)
        
            WZLog("BattleMsgBossMapSkill:_doEffect three", #targetHeroList,tostring(takeEffectParm), tostring(targetParm), tostring(effectParm[5]), tostring(effectParm[6]))
            self:_doEffectType(effectParm,targetHeroList,nil)
            local effect = effectParm[3] .. "_" ..effectParm[4]
            self:addStep(effect, targetHeroList, nil)
        elseif boss.m_bActiveAttack == true and takeEffectParm == TakeEffectType.COLLISION and self.m_nEffcetId == 3011 then 
            WZLog("big skin skill")
            local targetParm = effectParm[2]
            effectParm.isTakeEffect = turnTime
            local targetHeroList = self:_chooseEffectTarget(takeEffectParm,targetParm)
            ...
            local effect = effectParm[3] .. "_" ..effectParm[4]
            self:addStep(effect, targetHeroList, nil)
        elseif boss.m_bActiveAttack ~= true and takeEffectParm == TakeEffectType.KILL then
            boss.m_tSkillTakeEffectKillList = {}
            boss.m_tSkillTakeEffectKillInfo = self.m_nEffcetId
            if boss.m_tSkillTakeEffectKillIndex == nil then
                boss.m_tSkillTakeEffectKillIndex = {}
            end
            table.insert(boss.m_tSkillTakeEffectKillIndex, i)
            local effect = effectParm[3] .. "_" ..effectParm[4]

            WZLog("kill use effect",self.m_nEffcetId, effect)
        elseif takeEffectParm == TakeEffectType.KILL and self.m_nTakeEffectType == TakeEffectType.KILL then 
            --击杀触发
            local targetParm = effectParm[2]
            effectParm.isTakeEffect = turnTime
            local targetHeroList = self:_chooseEffectTarget(takeEffectParm,targetParm)
        
            WZLog("BattleMsgBossMapSkill:_doEffect ten", #targetHeroList,tostring(takeEffectParm), tostring(targetParm), tostring(effectParm[5]), tostring(effectParm[6]))
            self:_doEffectType(effectParm,targetHeroList,nil)
            local effect = effectParm[3] .. "_" ..effectParm[4]
            self:addStep(effect, targetHeroList, nil)
        elseif self.m_nTakeEffectType == TakeEffectType.END_CUR_ROUND then 
            local targetParm = effectParm[2]
            effectParm.isTakeEffect = turnTime
            local targetHeroList = self:_chooseEffectTarget(takeEffectParm,targetParm)
            ...
            local effect = effectParm[3] .. "_" ..effectParm[4]
            self:addStep(effect, targetHeroList, nil)
        elseif self.m_nTakeEffectType == TakeEffectType.INJURED then 
            local targetParm = effectParm[2]
            effectParm.isTakeEffect = turnTime
            local targetHeroList = self:_chooseEffectTarget(takeEffectParm,targetParm)
        
            WZLog("BattleMsgBossMapSkill:_doEffect elevent", #targetHeroList,tostring(takeEffectParm), tostring(targetParm), tostring(effectParm[5]), tostring(effectParm[6]))
            self:_doEffectType(effectParm,targetHeroList,nil)
            local effect = effectParm[3] .. "_" ..effectParm[4]
            self:addStep(effect, targetHeroList, nil)
        elseif self.m_nTakeEffectType == TakeEffectType.SUPERCRIT then 
            WZLog("SUPERCRIT ............. SUPERCRIT")
            local targetParm = effectParm[2]
            effectParm.isTakeEffect = turnTime
            local targetHeroList = self:_chooseEffectTarget(takeEffectParm,targetParm)
            self:_doEffectType(effectParm,targetHeroList,nil)
            local effect = effectParm[3] .. "_" ..effectParm[4]
            self:addStep(effect, targetHeroList, nil)
        elseif self.m_nTakeEffectType == TakeEffectType.ABSORB_ATTRIBUTE_BECOMES_HURT then 
            WZLog("SUPERCRIT ............. ABSORB_ATTRIBUTE_BECOMES_HURT")
            local targetParm = effectParm[2]
            effectParm.isTakeEffect = turnTime
            local targetHeroList = self:_chooseEffectTarget(takeEffectParm,targetParm)
            self:_doEffectType(effectParm,targetHeroList,nil)
            local effect = effectParm[3] .. "_" ..effectParm[4]
            self:addStep(effect, targetHeroList, nil)
        elseif boss.m_bActiveAttack ~= true and (takeEffectParm == TakeEffectType.AFTER_BLEED or takeEffectParm == TakeEffectType.HIT_TWO) then
            WZLog("_doEffect TakeEffectType.AFTER_BLEED")
            boss.m_tSkillTakeEffectAfterBleedInfo = self.m_nEffcetId
        elseif self.m_nTakeEffectType and self.m_nTakeEffectType == TakeEffectType.USE and takeEffectParm == TakeEffectType.END_CUR_ROUND then
            WZLog("_doEffect TakeEffectType END_CUR_ROUND", self.m_nSkillId)
            if boss.m_tSkillTakeEffectEndRoundList == nil then
                boss.m_tSkillTakeEffectEndRoundList = {}
            end
            if not utilsValueInTable(self.m_nSkillId, boss.m_tSkillTakeEffectEndRoundList) then 
                table.insert(boss.m_tSkillTakeEffectEndRoundList, self.m_nSkillId)
            end
        end
    end
end
...
function BattleMsgBossMapSkill:_doEffectType(effectParm,targetHeroList,isBuff)
    ...
    if effect == EffectTypeConfig.PET_CRIT then
        WZLog("BattleMsgBossMapSkill:_doEffect seven-31")
    ...
    elseif effect == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT_HURT or effect == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT_HURT2 or effect == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT_HURT3 then
        WZLog("BattleMsgBossMapSkill:_doEffect seven-32", self.m_nSuperCritHurt)
    ...
    elseif effect == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT then
        WZLog("BattleMsgBossMapSkill:_doEffect seven-2")
    ...
    elseif effect == EffectTypeConfig.SCATTER_SHOOT then
        WZLog("BattleMsgBossMapSkill:_doEffect seven-3")
        self.m_nAttScatterNum = effectParm[5]
    elseif effect == EffectTypeConfig.TIMES_SHOOT then
        WZLog("BattleMsgBossMapSkill:_doEffect seven-4")
    ...
    elseif effect == EffectTypeConfig.ADD_BUFF then
        WZLog("BattleMsgBossMapSkill:_doEffect seven-14")
    ...
    elseif effect == EffectTypeConfig.CANCEL_BUFF_ALL then
        WZLog("BattleMsgBossMapSkill:_doEffect seven-14-1")
    elseif effect == EffectTypeConfig.CANCEL_BUFF_TYPE then
        WZLog("BattleMsgBossMapSkill:_doEffect seven-14-2")
        self.m_nCancelBuffType = effectParm[5]
    elseif effect == EffectTypeConfig.CANCEL_BUFF_ID then
        WZLog("BattleMsgBossMapSkill:_doEffect seven-14-3")
    ...
    elseif effect == EffectTypeConfig.TRANS then
        WZLog("BattleMsgBossMapSkill:_doEffect three")
    ...
    elseif effect == EffectTypeConfig.HURT then
        WZLog("BattleMsgBossMapSkill:_doEffect four-1")
    ...
    elseif effect == EffectTypeConfig.INVINCIBLE then
        WZLog("BattleMsgBossMapSkill:_doEffect six")
    elseif effect == EffectTypeConfig.SUMMON then
        WZLog("BattleMsgBossMapSkill:_doEffect eight")
    ...
    elseif effect == EffectTypeConfig.TORNADO then
        ...
        elseif self.m_nSkillId == 3029 and WBattleGlobal:getCurrent().m_tExplodeInfoCurRound and #WBattleGlobal:getCurrent().m_tExplodeInfoCurRound > 0 then
            local tExplodeInfoCurRound = WBattleGlobal:getCurrent().m_tExplodeInfoCurRound
            local nRandomExplode = math.floor(WBattleGlobal:getCurrent().m_tBattleRand[1]) % #tExplodeInfoCurRound + 1 --这里为了随机龙卷风位置
            self.m_tTornadoInfo = {battleId = self.m_tOwner:getBattleId() + 1000,templateId = effectParm[6], camp=targetHeroList[1]:getCamp(), bronPos={x=tExplodeInfoCurRound[nRandomExplode].x, y=tExplodeInfoCurRound[nRandomExplode].y}}
            self.m_tSummonMonsterId = {}
            self.m_tSummonMonsterBattleId = {}
            ...
            WZLog("BattleMsgBossMapSkill:_doEffect nine-2_0", targetHeroList[1]:getBattleId(), self.m_tOwner.m_bPetActiveAttack)
            if self.m_tOwner.m_bPetActiveAttack then 
                self.m_tTornadoInfo = {battleId = self.m_tOwner:getBattleId() + 1000,templateId = effectParm[6], camp=self.m_tOwner:getCamp(), bronPos=CopyTable(self.m_tOwner.m_tActiveAttackPos[1])}
            else
                WZLog("BattleMsgBossMapSkill:_doEffect nine-2", tostring(targetHeroList[1]:getBattleId()), tostring(targetHeroList[1]:getCamp()), tostring(targetHeroList[1].m_tActiveAttackPos[1].x),tostring(targetHeroList[1].m_tActiveAttackPos[1].y))
    ...
    elseif effect == EffectTypeConfig.XUANZANG_TOTEM then
        if self:_isGhostSkill(self.m_nSkillId) and self.m_tOwnPlayerId then 
            local hero = WBattleGlobal:getCurrent():getCharacterWithId(self.m_tOwnPlayerId)
            self.m_tTornadoInfo = {battleId = hero:getBattleId() + 1000,templateId = effectParm[6], camp=hero:getCamp(), bronPos={x=self.m_tOwner:getPosition().x, y=self.m_tOwner:getPosition().y}}
            self.m_tSummonMonsterId = {}
    ...
    elseif effect == EffectTypeConfig.TREAT_TOTEM or effect == EffectTypeConfig.BUFF_TOTEM or effect == EffectTypeConfig.FIRE_TOTEM or effect == EffectTypeConfig.GUARDIAN_TOTEM or effect == EffectTypeConfig.BLACK_HOLE or effect == EffectTypeConfig.JIANGZIYA_TOTEM then
        ...
        local userAttack = 0
        if effect == EffectTypeConfig.FIRE_TOTEM then 
            if self.m_tOwner.m_tSpatterPosList and self.m_param1 and self.m_tOwner.m_tSpatterPosList[self.m_param1] then --这里是哪吒大招溅射出来的子弹
                tmpBronPos = CopyTable(self.m_tOwner.m_tSpatterPosList[self.m_param1])
            else
                if self.m_nSkillId == 3050 then
                    tmpBronPos = CopyTable(self.m_tOwner.m_tActiveAttackPos[self.m_param1])
                else
                    tmpBronPos = CopyTable(self.m_tOwner.m_tActiveAttackPos[1])
                end
            end
            userAttack = self.m_tOwner:getAttack(true)--WBattleGlobal:getCurrent().m_tCharacterAttributeList[self.m_tOwner:getBattleId()] and WBattleGlobal:getCurrent().m_tCharacterAttributeList[self.m_tOwner:getBattleId()].atk
        elseif effect == EffectTypeConfig.GUARDIAN_TOTEM or effect == EffectTypeConfig.BLACK_HOLE or effect == EffectTypeConfig.JIANGZIYA_TOTEM then
            if self.m_tPetEffectBornPos then 
                tmpBronPos = CopyTable(self.m_tPetEffectBornPos)
            else
                tmpBronPos = CopyTable(self.m_tOwner.m_tActiveAttackPos[1])
            end
            userAttack = self.m_tOwner:getAttack(true)
        end
        if effect == EffectTypeConfig.GUARDIAN_TOTEM or effect == EffectTypeConfig.JIANGZIYA_TOTEM then
            self.m_tTreatTotemInfo = {battleId = self.m_tOwner:getBattleId() + 1000,templateId = effectParm[6],bronPos = tmpBronPos,charaId=self.m_tOwner:getBattleId(), camp=self.m_tOwner:getCamp(), userAttack = userAttack}
        else
    ...
    elseif effect == EffectTypeConfig.MARITIME1_TOTEM then
        WZLog("BattleMsgBossMapSkill:_doEffect EffectTypeConfig.MARITIME1_TOTEM",effectParm[5],effectParm[6],effectParm[7])
        local tmpBronPos = CopyTable(self.m_tOwner.m_tActiveAttackPos[1])
        local userAttack = self.m_tOwner:getAttack(true)
        self.m_tMaritime1TotemInfo = {battleId = self.m_tOwner:getBattleId() + 1000,templateId = effectParm[6],bronPos = tmpBronPos,charaId=self.m_tOwner:getBattleId(), camp=self.m_tOwner:getCamp(), userAttack = userAttack}

        self.m_tSummonMonsterId1 = {}
        self.m_tSummonMonsterBattleId = {}
        self.m_tSummonMonsterPositionX = {}
        self.m_tSummonMonsterPositionY = {}
        self.m_tOwner.m_tCursummonList = {}
        ...
        for i = 1,count,1 do
            ...
            local posY = 500
            WZLog("BattleMsgSkillEffect:_doEffect summon",index,posIndex)
            table.insert(self.m_tSummonMonsterId1,templateId)
            table.insert(self.m_tSummonMonsterPositionX, tmpBronPos.x)
            table.insert(self.m_tSummonMonsterPositionY, tmpBronPos.y)
        end
    elseif effect == EffectTypeConfig.MARITIME2_TOTEM then
        WZLog("BattleMsgBossMapSkill:_doEffect EffectTypeConfig.MARITIME2_TOTEM",effectParm[5],effectParm[6],effectParm[7])
        local tmpBronPos = CopyTable(self.m_tOwner.m_tActiveAttackPos[1])
        local userAttack = self.m_tOwner:getAttack(true)
        self.m_tMaritime2TotemInfo = {battleId = self.m_tOwner:getBattleId() + 1000,templateId = effectParm[6],bronPos = tmpBronPos,charaId=self.m_tOwner:getBattleId(), camp=self.m_tOwner:getCamp(), userAttack = userAttack}

        self.m_tSummonMonsterId2 = {}
        self.m_tSummonMonsterBattleId = {}
        self.m_tSummonMonsterPositionX = {}
        self.m_tSummonMonsterPositionY = {}
        self.m_tOwner.m_tCursummonList = {}
        ...
        for i = 1,count,1 do
            ...
            local posY = 500
            WZLog("BattleMsgSkillEffect:_doEffect summon",index,posIndex)
            table.insert(self.m_tSummonMonsterId2,templateId)
            table.insert(self.m_tSummonMonsterPositionX, tmpBronPos.x)
            table.insert(self.m_tSummonMonsterPositionY, tmpBronPos.y)
        end
    elseif effect == EffectTypeConfig.MARITIME3_TOTEM then
        WZLog("BattleMsgBossMapSkill:_doEffect EffectTypeConfig.MARITIME3_TOTEM",effectParm[5],effectParm[6],effectParm[7])
        local tmpBronPos = CopyTable(self.m_tOwner.m_tActiveAttackPos[1])
        local userAttack = self.m_tOwner:getAttack(true)
        self.m_tMaritime3TotemInfo = {battleId = self.m_tOwner:getBattleId() + 1000,templateId = effectParm[6],bronPos = tmpBronPos,charaId=self.m_tOwner:getBattleId(), camp=self.m_tOwner:getCamp(), userAttack = userAttack}

        self.m_tSummonMonsterId3 = {}
        self.m_tSummonMonsterBattleId = {}
        self.m_tSummonMonsterPositionX = {}
        self.m_tSummonMonsterPositionY = {}
        self.m_tOwner.m_tCursummonList = {}
        ...
        for i = 1,count,1 do
            ...
            local posY = 500
            WZLog("BattleMsgSkillEffect:_doEffect summon",index,posIndex)
            table.insert(self.m_tSummonMonsterId3,templateId)
            table.insert(self.m_tSummonMonsterPositionX, tmpBronPos.x)
            table.insert(self.m_tSummonMonsterPositionY, tmpBronPos.y)
        end
    elseif effect == EffectTypeConfig.UMBRELLA1_TOTEM then
        WZLog("BattleMsgBossMapSkill:_doEffect EffectTypeConfig.UMBRELLA1_TOTEM",effectParm[5],effectParm[6],effectParm[7])
        local tmpBronPos = CopyTable(self.m_tOwner.m_tActiveAttackPos[1])
        local userAttack = self.m_tOwner:getAttack(true)
        self.m_tMaritime1TotemInfo = {battleId = self.m_tOwner:getBattleId() + 1000,templateId = effectParm[6],bronPos = tmpBronPos,charaId=self.m_tOwner:getBattleId(), camp=self.m_tOwner:getCamp(), userAttack = userAttack}
    ...
    elseif effect == EffectTypeConfig.UMBRELLA2_TOTEM then
        WZLog("BattleMsgBossMapSkill:_doEffect EffectTypeConfig.UMBRELLA2_TOTEM",effectParm[5],effectParm[6],effectParm[7])
        local tmpBronPos = CopyTable(self.m_tOwner.m_tActiveAttackPos[1])
        local userAttack = self.m_tOwner:getAttack(true)
        self.m_tMaritime2TotemInfo = {battleId = self.m_tOwner:getBattleId() + 1000,templateId = effectParm[6],bronPos = tmpBronPos,charaId=self.m_tOwner:getBattleId(), camp=self.m_tOwner:getCamp(), userAttack = userAttack}
    ...
    elseif effect == EffectTypeConfig.THORNS then
        self.m_tOwner:addImmunityPetSkill(self.m_nSkillId, {type = EffectTypeConfig.THORNS})
        self.m_tThornsValue = {type = effectParm[5], value = effectParm[6]}
    elseif effect == EffectTypeConfig.PROFESSION_SAVELIFE or effect == EffectTypeConfig.PROFESSION_SAVELIFE_PERCENT then
        WZLog("BattleMsgBossMapSkill:_doEffect PROFESSION_SAVELIFE_PERCENT")
        self.m_tLeftHPValue = {type = effect, hp = effectParm[5], maxTimes = effectParm[6]}
        self.m_tOwner:addImmunityPetSkill(self.m_nSkillId, self.m_tLeftHPValue)
    ...
    elseif effect == EffectTypeConfig.RANDOM_SCATTER_SHOOT then
        WZLog("BattleMsgBossMapSkill:_doEffect RANDOM_SCATTER_SHOOT")
        self.m_nAttScatterNum = effectParm[5]
    elseif effect == EffectTypeConfig.ABSORB_ATTRIBUTE_BECOMES_HURT then
        WZLog("BattleMsgBossMapSkill:_doEffect ABSORB_ATTRIBUTE_BECOMES_HURT",effectParm[5],effectParm[6],effectParm[7])
        self.m_nChangeValueIndex = effectParm[5]
        self.m_nChangeValue = effectParm[6]
        self.m_nHurtConversion = effectParm[7]
    elseif effect == EffectTypeConfig.ADD_RANDOMBUFF then
        WZLog("BattleMsgBossMapSkill:_doEffect seven-144")
        self.m_nAddBuffId = self.m_nAddBuffId or {}
        local buffId = BattleMethod:getRandomBuff(effectParm, self.m_tOwner:getBattleId())
        table.insert(self.m_nAddBuffId, buffId)
    elseif effect == EffectTypeConfig.ADD_RANDOM_BUFF_TWO then
        WZLog("BattleMsgBossMapSkill:_doEffect ADD_RANDOM_BUFF_TWO")
        self.m_tRandomBuffParam = effectParm
    elseif effect == EffectTypeConfig.THORNS_HURT_PERCENT then
        self.m_nThornsValue = effectParm[5]
    elseif effect == EffectTypeConfig.REDUCE_BUFFCTB_ALL then
        WZLog("BattleMsgBossMapSkill:_doEffect REDUCE_BUFFCTB_ALL")
        self.m_tReduceBuffConfig = {effectParm[5], effectParm[6]}
    ...
    elseif effect == EffectTypeConfig.SUMMON_SOULHERO or effect == EffectTypeConfig.SUMMON_SUBCHESSHERO then
        self.m_tOwner:setSoulHeroConfig(effectParm)
    end
...
function BattleMsgBossMapSkill:_chooseEffectTarget(takeType,effectParm)
    ...
    local targetHeroHurts = nil --目标玩家当前子弹对其的伤害值
    ...
    local tHeroHurtsList = nil 
    if takeType == TakeEffectType.HIT or takeType == TakeEffectType.AFTERHURT_DOEFFECT then
        if self.m_tOwner.m_bPetActiveAttack then
            tHeroList = {self.m_tOwner.m_tPetAttackHero}
        else
            if takeType == TakeEffectType.HIT then --如果传了目标过来，用传过来的
                tHeroList = self.m_tTargetChara or self.m_tOwner.m_tHitTargets or {}
            else
                tHeroList = self.m_tOwner.m_tHitTargets or {}
            end
        end
    elseif takeType == TakeEffectType.BULLET_HIT then
        tHeroList = self.m_tOwner.m_tBulletHitTargets[self.m_param1]
        tHeroHurtsList = self.m_tOwner.m_tBulletHitTargetsHurts[self.m_param1]
    elseif takeType == TakeEffectType.AFTER_BLEED and chooseTargetType == EffectTargetType.HIT_ROLE then 
        tHeroList = self.m_tTargetChara or self.m_tOwner.m_tHitTargets or {}
    end
    ...
    for i, chara in pairs(tHeroList) do
        ...
        if isMacth then
            ...
            if tHeroHurtsList then 
                if targetHeroHurts == nil then targetHeroHurts = {} end 
                table.insert(targetHeroHurts, tHeroHurtsList[i])
            end
        end
    end

    --对13,1生效类型处理 击中1个敌人,给自己生效1次
    if takeType == TakeEffectType.HIT_ENEMY and chooseTargetType == EffectTargetType.MYSELF then
        self.m_tTargetHeroList = {}
        tHeroList = self.m_tOwner.m_tBulletHitTargets[self.m_param1]
        if tHeroList then
            for i, chara in pairs(tHeroList) do
    ...
    return self.m_tTargetHeroList, targetHeroHurts
end
...
function BattleMsgBossMapSkill:_spatter()
    ...
    if not self.m_bSpatterInit then
        ...
        -- add 这里把溅射弹的爆炸范围改成技能表自己的id对应的范围,不和主弹范围一样了
        local skillInfo = self:_getSkillData(self.m_tSpatterInfo.hurtSkillId)
        if skillInfo.skill_type == 0 or skillInfo.skill_type == 2 or skillInfo.skill_type == 6 then
            self.m_tOwner.m_fRadiusForBulletExplodeChange = (skillInfo.scope > 0 and skillInfo.scope) or nil
        end
        if type(skillInfo.boom_scope) == "table" then
            self.m_tOwner.m_fRectForBulletExplodeBombChange = {x=skillInfo.boom_scope[1][1],y=skillInfo.boom_scope[1][2],digHoleDir=skillInfo.direction}
            WZLog("BattleMsgBossMapSkill:_spatter one", skillInfo.effect_id[1][1], Serialize(skillInfo.boom_scope))
        end
        -- end
...
--    startAngle = -1 * BattleConstants.g_fWB_SCATTER_ANGLE * (math.floor(nScatterNum / 2) - (nScatterNum+1)%2/2)
    ...
    WZLog("BattleMsgBossMapSkill:_createBulletSpatter three", type(self.m_tSpatterAngle))
...
function BattleMsgBossMapSkill:_showOtherSpatter(element, dt)
    ...
    if self.m_nSpatterIndex > self.m_tSpatterInfo.count then 
        ...
        if self.m_tOwner:getIsSubHero() then 
            for i, msg in pairs(MsgManager.m_tNonBlockMsgList) do
                if msg.m_nBuildBulletsSkillStatusCount and msg.m_nBuildBulletsSkillStatusCount > 0 and msg.m_sName == "BattleMsgPlayerShoot" then
                    msg.m_nBuildBulletsSkillStatusCount = msg.m_nBuildBulletsSkillStatusCount - 1
                end
            end
        end
    ...
    local spatterAngle = self.m_tSpatterAngle and self.m_tSpatterAngle[self.m_nSpatterIndex] and self.m_tSpatterAngle[self.m_nSpatterIndex] or 90
    local speedVec = BattleCommon:vectorWithAngle(self.m_tSpeedPt, spatterAngle)
...
--生成玄奘龙卷风
function BattleMsgBossMapSkill:_buildXuanZangTornado()
    WZLog("BattleMsgBossMapSkill:_buildXuanZangTornado", self.m_tTornadoInfo.camp,self.m_tTornadoInfo.templateId)
    if not WBattleGlobal:getCurrent():isSingleStage() then
        --只创建一个 直接赋值
        for i=1,#self.m_tSummonMonsterId do
            for j=1, #self.m_tOwner.guaiId do
    ...
    local tornado = WBattleGlobal:getCurrent():buildMachine(MonsterType.XUANZANG_TOTEM,self.m_tTornadoInfo)
    SceneBattle:getFrontLayer():addChild(tornado.m_anim:getAnimNode())
    -- if WBattleGlobal:getCurrent().m_tMapEvents == nil then
    --     WBattleGlobal:getCurrent().m_tMapEvents = {}
    -- end
...
function BattleMsgBossMapSkill:_buildGhostTornado()
    ...
    if not WBattleGlobal:getCurrent():isSingleStage() then
        --只创建一个 直接赋值
        for i=1,#self.m_tSummonMonsterId do
            for j=1, #hero.guaiId do
                if self.m_tSummonMonsterId[i] == hero.guaiId[j] then
    ...
    for id, hero in pairs (targetHeroList) do
        if hero:isDead() ~= true then
            for index, buff in pairs (hero.m_tBuffChangeStateList) do 
                ...
                if self.m_nCancelBuffType and self.m_nCancelBuffType == buff.m_nEffectType and buff.m_nCanRemove == 0 then --9_2需要判断buff表disperse字段
                    isClear = true
                end
                if self.m_nCancelBuffAssign and self.m_nCancelBuffAssign == buff.m_nType then
                    isClear = true
                end
                if not self.m_nCancelBuffId and not self.m_nCancelBuffType and not self.m_nCancelBuffAssign and buff.m_nCanRemove == 0 then --9_1需要判断buff表disperse字段
                    isClear = true
                end
                if isClear then
...
function BattleMsgBossMapSkill:_addBuff(targetHeroList)
    WZLog("BattleMsgBossMapSkill:_addBuff one", Serialize(self.m_nAddBuffId),self.m_tOwner:getBattleId(), self.m_nSkillId)
    ...
    for i, buffId in pairs (self.m_nAddBuffId) do
        ...
        for id, hero in pairs (targetHeroList) do
            if hero:isDead() ~= true then
                local ownId = self.m_tOwner:getBattleId()
                if self.m_nSkillId and self.m_tOwnPlayerId then 
                    local skillConfigData = GDatatab_skill["id_" .. self.m_nSkillId]
                    if skillConfigData and skillConfigData.skill_type == 9 then 
                        ownId = self.m_tOwnPlayerId
                    end
                end
                local buffNew = BuffBody:new(buffInfo, hero, ownId, self.m_nSkillId)
                ...
                if not offBuff then
                    ...
                    WZLog("BattleMsgBossMapSkill:_addBuff three-1", hero.m_tBuffAddTimes[buffNew.m_nType], round)
                    if buffExistIndex and buffNew.m_nMaxAddNum > 1 then 
                        if hero.m_tBuffAddTimes[buffNew.m_nType] < buffNew.m_nMaxAddNum then
                            ...
                            if hero.m_nHideOpecity == nil or hero.m_nHideOpecity ~= 0 then
                                BattleProfessionSkillManager:showUseName(BattleCommon:getPointTable(pos.x,pos.y + 85), buffInfo.buff_name, 2)
                            end
    ...
    for i, hero in pairs (targetHeroList) do
        ...
        if self.m_tOwner:getUseSkinBigSkill() and self.m_tOwner:getSkinBigSkill() == 3011 then 
            ...
            if offSkillId then
                ...
                WZLog("BattleMsgBossMapSkill:_repelFly four", self.m_tOwner.m_tActiveAttackPos and Serialize(self.m_tOwner.m_tActiveAttackPos) or tostring(self.m_tOwner.m_tActiveAttackPos), heroPos.x)
...
--改变ctb
function BattleMsgBossMapSkill:_changeCtbValue(targetHeroList,skillType,isBuff,nChangeValue)
    for i,hero in pairs(targetHeroList) do
        if not hero:isDead() then
            WZLog("BattleMsgBossMapSkill:_changeCtbValue", hero:getBattleId(), self.m_nSkillId, WBattleGlobal.getCurrent().m_nAwakeSkillId, tostring(nChangeValue))
            if not (self.m_nSkillId == WBattleGlobal.getCurrent().m_nAwakeSkillId) then
                WZLog("BattleMsgBossMapSkill:_changeCtbValue two")
                BattleCtbManager:addCtb(hero:getBattleId(), nChangeValue or 0)
            end
             --设置ctb过程记录
            WBattleGlobal:getCurrent():setCtbProRecord(hero:getBattleId(), nChangeValue )
        end
    end
end

--改变属性
function BattleMsgBossMapSkill:_changeAttribute(targetHeroList, skillType, isBuff, param2, takeType, targetHeroHurts)
    ...
    WZLog("BattleMsgBossMapSkill:_changeAttribute", targetHeroList, skillType, tostring(tempChangeValueIndex), tostring(tempChangeValueIndex2), tostring(tempChangeValue), tostring(tempChangeValue2))
    local Attribute = {}
        ...
        if isBuff == nil and not hero:isDead() and hero:getHp() > 0 then
            if skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_VALUE then
                if tempChangeValueIndex == AttributeConfig.HP then
                    local change = tempChangeValue or 0
                    ...
                    change = hero:hurtEffectHandle(change * -1, nil, self.m_tOwner)
                    ...
                    if change < 0 and (hero:isHide() == true and WBattleGlobal:getCurrent():isMyTeam(hero:getBattleId()) or hero:isHide() ~= true) and not hero:getIsSubHero() then
                ...
                elseif tempChangeValueIndex == AttributeConfig.SP then
                    WZLog("BattleMsgBossMapSkill:_changeAttribute 3",hero:getId(),tempChangeValue)
                    ...
                    if self.m_tSkillConfig and self.m_tSkillConfig.skill_type == 4 then
                        hero:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.CHANGE_ATTRIBUTE_VALUE, subType = tempChangeValueIndex,param = tempChangeValue})
                    end
                end
            ...
            elseif skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT then
                if tempChangeValueIndex2 == AttributeConfig.HP then
                    local change = hero:getMaxHp(true) * (tempChangeValue2 or 0)/10000
                    if change > 0 then
                        change = math.ceil(change)
                    else
                        change = math.floor(change)
                    ...
                    if change < 0 then
                        ...
                        WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT 11", change, recoveryAddPercent)
                    end

                    if change * -1 + hero.m_nHP > hero.m_nMaxHP then
                        change = (hero.m_nMaxHP - hero.m_nHP) * -1
                    end
                    WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT 4", change)
                    local curHp = hero.m_nHP - change
                    hero:setHp(curHp)
                    --设置过程伤害记录
                    if math.abs(change) > 0 then
                        WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT 66", tostring(takeType))
                        if takeType and takeType == TakeEffectType.END_CUR_ROUND then --共生技能-铸世神火/血之契约 回合结束扣血后端校验不通过（将这个伤害放到附件伤害中）
                            WBattleGlobal:getCurrent():setHpProRecord(hero:getBattleId(), 0, nil, nil, -change)
                        else
                            WBattleGlobal:getCurrent():setHpProRecord(hero:getBattleId(),-change)
                        end
                    end

                    if (hero:isHide() == true and WBattleGlobal:getCurrent():isMyTeam(hero:getBattleId()) or hero:isHide() ~= true) and not hero:getIsSubHero() then --202版本新增道具血之契约回合结束扣血
                        local pos = BattleCommon:getPointTable(hero:getPosition().x + 100,hero:getPosition().y + 20)
                        local numType = 0 
                        if change < 0 then 
                            numType = 3
                        end
                        local element = WZUISystem:getInstance():createElement(string.format("conHurtType%d_HurtNumber", numType))
                        element:setLuaObjectIndex(self)
                        if element ~= nil then
                            GetElement(element,"txtHurtValue_HurtNumber",WZUILabelAtlasFont):setText(change * -1)
                            local conHurt = WZUIContainer:luaTo(element)
                ...
                elseif tempChangeValueIndex2 == AttributeConfig.PF then
                    WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT 5")
                    hero.m_nPF = hero.m_nPF + (hero:getMaxPF(true) * (tempChangeValue2 or 0)/10000)
                    hero:setPF(hero.m_nPF)
                elseif tempChangeValueIndex2 == AttributeConfig.SP then
                    WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT 6")
                    hero.m_nSP = hero.m_nSP + (100 * (tempChangeValue2 or 0)/10000)
                    hero:setSp(hero.m_nSP)
                else
                    if tempChangeValueIndex2 == AttributeConfig.Attack then
                        tempChangeValue2 = tempChangeValue2/100
                    end
                    hero.m_tAttributeChangeStateList["m_n"..Attribute[tempChangeValueIndex2].."AddPercent"] = {timeType=self.m_nChangeValueTimeType,timeValue=self.m_nChangeValueTimeValue,value=(tempChangeValue2 or 0)/100}
                    if self.m_tSkillConfig and self.m_tSkillConfig.skill_type == 4 then
                        hero:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT, subType = tempChangeValueIndex2,param = (tempChangeValue2 or 0)/100})
                    end
                    WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT 44", "m_n"..Attribute[tempChangeValueIndex2].."AddPercent", tempChangeValue2)
            ...
            elseif skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT_HURT then
                if self.m_nChangeHurtPercentIndex == AttributeConfig.HP then
                    ...
                    if takeType and takeType == TakeEffectType.BULLET_HIT and targetHeroHurts and targetHeroHurts[i] then --子弹击中回血
                        value = targetHeroHurts[i]
                    end
                    WZLog("BattleMsgBossMapSkill:_changeAttribute 99", self.m_nTakeEffectType, TakeEffectType.SUPERCRIT, tostring(self.m_nSuperCritHurt), value, tostring(targetHeroHurts))
                    if self.m_nTakeEffectType == TakeEffectType.SUPERCRIT then 
                        value = self.m_nSuperCritHurt or 0
                    end
            ...
            elseif skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT_HURT2 then
                if self.m_nChangeHurtPercentIndex == AttributeConfig.HP then
                    local value = hero.m_nCurRoundHurt2 or 0
                    WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT_HURT2-1", self.m_nTakeEffectType, TakeEffectType.SUPERCRIT, tostring(self.m_nSuperCritHurt))
                    if self.m_nTakeEffectType == TakeEffectType.SUPERCRIT then 
                        value = self.m_nSuperCritHurt or 0
                    end
                    if self.m_bIsPetSkillEffect then
                    ...
                    if change < 0 then
                        ...
                        WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT_HURT2-2", change, recoveryAddPercent)
                    end

                    if change * -1 + hero.m_nHP > hero.m_nMaxHP then
                        change = (hero.m_nMaxHP - hero.m_nHP) * -1
                    end
                    WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT_HURT2-3", value, change)
                    local curHp = hero.m_nHP - change
                    hero:setHp(curHp)
                    --设置过程伤害记录
                    if math.abs(change) > 0 then
            ...
            elseif skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_PERCENT_HURT3 then
                if self.m_nChangeHurtPercentIndex == AttributeConfig.HP then
                    local value = hero.m_nCurRoundHurt3 or 0
                    WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT_HURT3-1", self.m_nTakeEffectType, TakeEffectType.SUPERCRIT, tostring(self.m_nSuperCritHurt))
                    if self.m_nTakeEffectType == TakeEffectType.SUPERCRIT then 
                        value = self.m_nSuperCritHurt or 0
                    end
                    if self.m_bIsPetSkillEffect then
                    ...
                    if change < 0 then
                        ...
                        WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT_HURT3-2", change, recoveryAddPercent)
                    end

                    if change * -1 + hero.m_nHP > hero.m_nMaxHP then
                        change = (hero.m_nMaxHP - hero.m_nHP) * -1
                    end
                    WZLog("BattleMsgBossMapSkill:_changeAttribute CHANGE_ATTRIBUTE_PERCENT_HURT3-3", value, change)
                    local curHp = hero.m_nHP - change
                    hero:setHp(curHp)
                    --设置过程伤害记录
                    if math.abs(change) > 0 then
...
--@brief    吸收属性变成伤害
function BattleMsgBossMapSkill:_AbsorbAttributeBecomesHurt(targetHeroList, skillType, isBuff, param2)
    local tempChangeValueIndex = self.m_nChangeValueIndex
    local tempChangeValue = self.m_nChangeValue
    if skillType == EffectTypeConfig.CHANGE_ATTRIBUTE_VALUE and param2 and param2[1] and param2[2] then
        tempChangeValueIndex = param2[1]
    ...
    WZLog("BattleMsgBossMapSkill:_changeAttribute", targetHeroList, skillType, tostring(tempChangeValueIndex), tostring(self.m_nChangePercentIndex), tostring(tempChangeValue), tostring(self.m_nChangePercent))
    local Attribute = {}
    for i, v in pairs (AttributeConfig)do
        Attribute[v] = i
    end
    ...
    for i, hero in pairs (targetHeroList) do
        if isBuff == nil and not hero:isDead() and hero:getHp() > 0 and hero:getId() ~= self.m_tOwner:getId() and hero:getIsKid() ~= true and not hero:getIsSubHero() then
            if skillType == EffectTypeConfig.ABSORB_ATTRIBUTE_BECOMES_HURT then
                if tempChangeValueIndex == AttributeConfig.SP then

                    local key = "m_n"..Attribute[tempChangeValueIndex].."Value"
                    local value = tempChangeValue
                    if hero.m_nSP + tempChangeValue < 0 then
                        value = (0 - math.max(hero.m_nSP,0))
                    end
                    self.m_tOwner:changeAbsorbAttributeList(key, value)
    ...
    local key = "m_n"..Attribute[tempChangeValueIndex].."Value"
    local nAbsorbSPValue = self.m_tOwner:getAbsorbAttributeValue(key)

    --修改伤害万分比
    local nHurtAddPercent =  (0 - nAbsorbSPValue) * self.m_nHurtConversion
    local nSkillType = EffectTypeConfig.CHANGE_HURT_PERCENT
    self:_ChangeHurt(targetHeroList, nSkillType, isBuff, nHurtAddPercent)
end
...
--@brief    改变伤害
function BattleMsgBossMapSkill:_ChangeHurt(targetHeroList, skillType, isBuff, param1)
    local hurtValue
    if param1 then
        hurtValue = param1
    else
        if skillType == EffectTypeConfig.CHANGE_HURT_VALUE then
            hurtValue = self.m_nHurtChangeValue
        elseif skillType == EffectTypeConfig.CHANGE_HURT_PERCENT then
            hurtValue = self.m_nHurtAddPercent
        elseif skillType == EffectTypeConfig.CHANGE_HURT_ADD_VALUE then
            hurtValue = self.m_nHurtAddValue
        elseif skillType == EffectTypeConfig.CHANGE_HURT_MUL_PERCENT then
            hurtValue = self.m_nHurtMulPercent
        elseif skillType == EffectTypeConfig.CHANGE_WIND_ADD_HURT_PERCENT then
            hurtValue = self.m_nHurtAddPercent
        end
    end
    ...
    if isBuff == nil then
        for i, hero in pairs (targetHeroList) do
            ...
            if skillType == EffectTypeConfig.CHANGE_HURT_VALUE then
                hero:changeAttrListValue("m_nHurtChangeValue", hurtValue)
                --hero.m_tAttributeChangeStateList.m_nHurtChangeValue = {timeType=self.m_nHurtChangeValueTimeType,timeValue=self.m_nHurtChangeValueTimeValue,value=hurtValue}
            elseif skillType == EffectTypeConfig.CHANGE_HURT_PERCENT then
                hero:changeAttrListValue("m_nHurtAddPercent", hurtValue)
                --hero.m_tAttributeChangeStateList.m_nHurtAddPercent = {timeType=self.m_nHurtAddPercentTimeType,timeValue=self.m_nHurtAddPercentTimeValue,value=hurtValue}
                --幻化伤害减免
                if self.m_tSkillConfig and self.m_tSkillConfig.skill_type == 4 then
                    hero:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.CHANGE_HURT_PERCENT,param = hurtValue})
                end
            elseif skillType == EffectTypeConfig.CHANGE_HURT_ADD_VALUE then
                hero:changeAttrListValue("m_nHurtAddValue", hurtValue)
                --hero.m_tAttributeChangeStateList.m_nHurtAddValue = {timeType=self.m_nHurtAddValueTimeType,timeValue=self.m_nHurtAddValueTimeValue,value=hurtValue}
                --幻化伤害减免
                if self.m_tSkillConfig and self.m_tSkillConfig.skill_type == 4 then
                    hero:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.CHANGE_HURT_ADD_VALUE,param = hurtValue})
                end
            elseif skillType == EffectTypeConfig.CHANGE_HURT_MUL_PERCENT then
                hero:changeAttrListValue("m_nHurtMulPercent", hurtValue)
                --hero.m_tAttributeChangeStateList.m_nHurtMulPercent = {timeType=self.m_nHurtAddPercentTimeType,timeValue=self.m_nHurtAddPercentTimeValue,value=hurtValue}
                --幻化伤害减免
                if self.m_tSkillConfig and self.m_tSkillConfig.skill_type == 4 then
                    hero:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.CHANGE_HURT_MUL_PERCENT,param = hurtValue})
                end
            elseif skillType == EffectTypeConfig.CHANGE_WIND_ADD_HURT_PERCENT then
                hero:changeAttrListValue("m_nHurtAddPercent", hurtValue)
                --hero.m_tAttributeChangeStateList.m_nHurtAddPercent = {timeType=self.m_nHurtAddPercentTimeType,timeValue=self.m_nHurtAddPercentTimeValue,value=hurtValue}
                --幻化伤害减免
                if self.m_tSkillConfig and self.m_tSkillConfig.skill_type == 4 then
                    hero:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.CHANGE_WIND_ADD_HURT_PERCENT,param = hurtValue})
                end
            end
        end
    end
    ...
    if isBuff == nil then
        for i, hero in pairs (targetHeroList) do
            ...
            elseif skillType == EffectTypeConfig.CHANGE_BEHURT_PERCENT then
                ...
                elseif self.m_nEffcetId ~= nil and self.m_nEffcetId == 1404 then 
                    hero:addImmunityPetSkill(self.m_nEffectId,{type = EffectTypeConfig.CHANGE_BEHURT_PERCENT,param = self.m_nBeHurtAddPercent, subType = 1, propertyKey = PetEquipRandomAttr.IMMUNEBUFF_REDUCEHURT_RATE}) --subType = 1表明是宠物装备被动减伤
                end
...
function BattleMsgBossMapSkill:_doSkillEffectParser(skillParm,i,isMoment)
    ...
    while skillParm do
        ...
        if ((isCanTakeEffect == nil and (takeEffectParm == TakeEffectType.USE or takeEffectParm == TakeEffectType.BEFORE_DOEFFECT or (boss.m_bActiveAttack == true and (takeEffectParm == TakeEffectType.HIT or takeEffectParm == TakeEffectType.HIT_TWO or takeEffectParm == TakeEffectType.AFTERHURT_DOEFFECT)) or (boss.m_bActiveAttack == true and (takeEffectParm == TakeEffectType.COLLISION or (boss.m_bActiveAttack2 == true and takeEffectParm == TakeEffectType.COLLISION_TWO))))) or isCanTakeEffect == true) and (effectParm.isTakeEffect == nil or turnTime > effectParm.isTakeEffect) then
            local targetParm = effectParm[2]
            effectParm.isTakeEffect = turnTime
            if self.m_tTargetPlayerId and GDatatab_skill["id_" .. self.m_nSkillId].skill_type == 9 then 
                targetHeroList = self:_getHeroById(self.m_tTargetPlayerId)
            ...
            if takeEffectParm == TakeEffectType.HIT or takeEffectParm == TakeEffectType.HIT_TWO or takeEffectParm == TakeEffectType.AFTERHURT_DOEFFECT then
                for i, v in pairs(targetHeroList) do
                    local isExist = false
                    if boss.m_tSkillTakeEffectList ~= nil then
                        for j, u in pairs(boss.m_tSkillTakeEffectList) do
        ...
        elseif boss.m_bActiveAttack ~= true and takeEffectParm == TakeEffectType.COLLISION then
            ...
            table.insert(boss.m_tSkillTakeEffectCollionIndex, i)
        elseif boss.m_bActiveAttack == true and takeEffectParm == TakeEffectType.COLLISION_TWO then
            WZLog("碰撞后生效1 COLLISION_TWO", self.m_nSkillId)
            -- boss.m_tSkillTakeEffectCollionList = {}
            boss.m_tSkillTakeEffectCollionInfo2 = self.m_nSkillId
            if boss.m_tSkillTakeEffectCollionIndex == nil then
                boss.m_tSkillTakeEffectCollionIndex = {}
            end
            table.insert(boss.m_tSkillTakeEffectCollionIndex, i)
        elseif takeEffectParm == TakeEffectType.INJURED then
            WZLog("受伤后生效", self.m_nSkillId)
            self.m_tOwner:addImmunityPetSkill(self.m_nSkillId,{type = effect})
        elseif takeEffectParm == TakeEffectType.SUPERCRIT then
            WZLog("超暴击生效1", self.m_nSkillId)
            if boss.m_tSkillTakeEffectSuperCritList == nil then
                boss.m_tSkillTakeEffectSuperCritList = {}
            end
            if not utilsValueInTable(self.m_nSkillId, boss.m_tSkillTakeEffectSuperCritList) then 
                table.insert(boss.m_tSkillTakeEffectSuperCritList, self.m_nSkillId)
            end
        elseif takeEffectParm == TakeEffectType.BEHIT_TODEAD then
            WZLog("受到致命伤害生效", self.m_nSkillId)
            local config = self:_getSkillData(self.m_nSkillId)
            tSkillEffectParm = self:_getEffectData(config.effect_id[1][1]).effect
            self.m_tOwner:addImmunityPetSkill(self.m_nSkillId,{type = effect, hp = tSkillEffectParm[1][5], maxTimes = tSkillEffectParm[1][6]})
        elseif takeEffectParm == TakeEffectType.USEITEMS then
            WZLog("使用道具生效...")
            local skillInfo = self:_getSkillData(self.m_nSkillId)
            local effectInfo = self:_getEffectData(skillInfo.effect_id[1][1])
            local tempData = {type = EffectTypeConfig.PROFESSION_RANDOM_ITEMPROP, maxTimes = effectParm[7], name = skillInfo.name, proRatioFirst = effectParm[5], proRatioSecond = effectParm[6]}
            self.m_tOwner:addImmunityPetSkill(self.m_nSkillId, tempData)
        elseif takeEffectParm == TakeEffectType.END_CUR_ROUND then
            WZLog("EndRound ............. EndRound", self.m_nSkillId)
            if boss.m_tSkillTakeEffectEndRoundList == nil then
                boss.m_tSkillTakeEffectEndRoundList = {}
            end
            if not utilsValueInTable(self.m_nSkillId, boss.m_tSkillTakeEffectEndRoundList) then 
...
function BattleMsgBossMapSkill:_getSkillConfig()
    ...
    local config = self:_getSkillData(self.m_nSkillId)
    if config.skill_type ~= 1 and config.skill_type ~= 4 and config.skill_type ~= 5 and config.skill_type ~= 9 and config.skill_type ~= 10 and config.skill_type ~= 7 then
        self.m_tOwner.m_bIsUseSkill = true
    end
...
--@brief    添加被动免疫buff效果
function BattleMsgBossMapSkill:_immunityBuff(targetHeroList, buffType)
    WZLog("BattleMsgBossMapSkill:_immunityBuff", tostring(buffType or self.m_nImmunityBuffType))
    if isBuff == nil then
        for i, hero in pairs (targetHeroList) do
            hero:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.IMMUNITY_BUFF_ASSIGN,param = buffType or self.m_nImmunityBuffType})
        end
    end
end
...
function BattleMsgBossMapSkill:_gatherTogether(targetHeroList, skillType)
    WZLog("BattleMsgBossMapSkill:_gatherTogether one", tostring(self.m_tOwner.m_bActiveAttack))
        ...
        if not WBattleGlobal:getCurrent():isSameTeam(hero:getId(), self.m_tOwner:getId()) and WBattleGlobal:getCurrent().m_bMapCanDigHole and (hero:getType() == 0 or hero:getType() == 1) and not hero:isDead() then 
            ...
            if self.m_tOwner.m_tActiveAttackPos ~= nil and #self.m_tOwner.m_tActiveAttackPos > 0 then 
                ...
                if attackDis <= self.m_nGatherDis then 
                    ...
                    local _, normalizeVec = BattleCommon:vectorNormalize(v)
...
--@brief    普攻后碰撞触发的散射
function BattleMsgBossMapSkill:_secondShoot()
    WZLog("BattleMsgBossMapSkill:_secondShoot", self.m_tTwoShootInfo.count, self.m_tTwoShootInfo.hurtSkillId)
...
--@brief    创建散射射子弹
--@param    nScatterNum:溅射数量
function BattleMsgBossMapSkill:_createBulletTwoShoot(nScatterNum)
    WZLog("BattleMsgBossMapSkill:_createBulletTwoShoot")
...
--@brief    普攻后碰撞触发的散射
function BattleMsgBossMapSkill:_spatterTwo()
    WZLog("BattleMsgBossMapSkill:_spatterTwo", self.m_tSpatterTwoInfo.count, self.m_tSpatterTwoInfo.hurtSkillId)
...
--@brief    创建散射射子弹
--@param    nScatterNum:溅射数量
function BattleMsgBossMapSkill:_createBulletSpatterTwo(nScatterNum)
    WZLog("BattleMsgBossMapSkill:_createBulletSpatterTwo")
    local msg = MsgManager:createMsg(BattleMsgSkillShow)
    msg.m_tOwner = self.m_tOwner
    msg.m_nSkillId = self.m_tSpatterTwoInfo.hurtSkillId
    msg.m_nTalkId = 0
...
--@brief    改变宠物的攻击次数
function BattleMsgBossMapSkill:_changePetAtkTimes()
    -- body
    local hero = self.m_tOwner
...
--@brief    反伤
function BattleMsgBossMapSkill:_thornsStart(targetHeroList)
    local hero = self.m_tTargetChara or WBattleGlobal:getCurrent():getCurrentCharacter()
    local value = 0
    if self.m_tThornsValue.type == 4 then
        value = self.m_tOwner:getDefence(true)
    end
    local skillId = self.m_tOwner:getIsImmunityByPetSkill(1, EffectTypeConfig.THORNS)

    local change = value * (self.m_tThornsValue and self.m_tThornsValue.value or 0)/100 * -1
    if change > 0 then
        change = math.ceil(change)
    else
        change = math.floor(change)
    ...
    WZLog("BattleMsgBossMapSkill:_thornsStart 1", self.m_tThornsValue and self.m_tThornsValue.value, change)
    change = hero:hurtEffectHandle(change * -1, true)
    ...
    if change < 0 then
        ...
        WZLog("BattleMsgBossMapSkill:_thornsStart 3", change, recoveryAddPercent)
    end

    if change * -1 + hero.m_nHP > hero.m_nMaxHP then
        change = (hero.m_nMaxHP - hero.m_nHP) * -1
    end
    WZLog("BattleMsgBossMapSkill:_thornsStart 4", change, hero.m_nHP)
    local curHp = hero.m_nHP - change
    self.m_tOwner.m_nShootHurtTotal = self.m_tOwner.m_nShootHurtTotal + change
    hero:setHp(curHp)
    hero.m_bIsThorns = true
    if hero:getHp() <= 0 and hero:isDead() ~= true then
        hero:setDead(true,66)
    end
    if WBattleGlobal:getCurrent():isSingleStage() then
        if hero:getHp() <= 0 then
            local pos = hero:getPosition()
            WBattleGlobal:getCurrent():killMonster(hero:getId(),hero:getBattleId(),GlobalMethod:ccp(pos.x,pos.y))
            --self:setDead(true)
        end
    end
    GlobalGame:getBattleEventDispatcher():Dispatch(BATTLE_EVENT_TYPE.CHARACTER_HURT,self,change)

    
    hero:getAnimation():play(hero:getHurtAnimationName(),false)
    
    --设置过程伤害记录
    if math.abs(change) > 0 then
        WBattleGlobal:getCurrent():setBuffHurt(hero:getBattleId(), skillId, -change)
    end
    WZLog("BattleMsgBossMapSkill:_thornsStart 5", hero.m_nHP)
    ...
    elseif change >= 0 and (hero:isHide() == true and WBattleGlobal:getCurrent():isMyTeam(hero:getBattleId()) or hero:isHide() ~= true) then
        local pos = BattleCommon:getPointTable(hero:getPosition().x + 100,hero:getPosition().y + 20)
        ...
        if nSettlementHurt > 0 then
            if chara:isInBuffByType(BuffType.CHAIN_BUFF) then
                ...
                for i, character in pairs(tCharacterList) do
...
--@brief 创建火焰图腾
function BattleMsgBossMapSkill:_buildFireTotem(monsterType)
    ...
    WZLog("BattleMsgBossMapSkill:_buildFireTotem", monsterType, Serialize(self.m_tTreatTotemInfo))
    local buffTotem = WBattleGlobal:getCurrent():buildMachine(monsterType, self.m_tTreatTotemInfo)
    SceneBattle:getFrontLayer():addChild(buffTotem.m_anim:getAnimNode())
end

--@brief 创建海洋领域增益图腾
function BattleMsgBossMapSkill:_buildMaritime1Totem()
    if not WBattleGlobal:getCurrent():isSingleStage() then
        --只创建一个 直接赋值

        for i=1,#self.m_tSummonMonsterId1 do
            for j=1, #self.m_tOwner.guaiId do
                if self.m_tSummonMonsterId1[i] == self.m_tOwner.guaiId[j] then
                    self.m_tMaritime1TotemInfo.battleId = self.m_tOwner.guaiBattleId[j]
                    table.remove(self.m_tOwner.guaiId, j)
                    table.remove(self.m_tOwner.guaiBattleId, j)
                    table.remove(self.m_tOwner.guaiPositionX, j)
                    table.remove(self.m_tOwner.guaiPositionY, j)
        ...
        --单人副本创建小怪 为小怪添加battleId
        self.m_tMaritime1TotemInfo.battleId = WBattleGlobal:getCurrent():getBuildGuaiBattleId(true)
    end
    WZLog("BattleMsgBossMapSkill:_buildMaritime1Totem", Serialize(self.m_tMaritime1TotemInfo))
    local buffTotem = WBattleGlobal:getCurrent():buildMachine(MonsterType.MARITIME1_TOTEM,self.m_tMaritime1TotemInfo)
    SceneBattle:getFrontLayer():addChild(buffTotem.m_anim:getAnimNode())
end

--@brief 创建海洋领域减益图腾
function BattleMsgBossMapSkill:_buildMaritime2Totem()
    if not WBattleGlobal:getCurrent():isSingleStage() then
        --只创建一个 直接赋值
        for i=1,#self.m_tSummonMonsterId2 do
            for j=1, #self.m_tOwner.guaiId do
                if self.m_tSummonMonsterId2[i] == self.m_tOwner.guaiId[j] then
                    self.m_tMaritime2TotemInfo.battleId = self.m_tOwner.guaiBattleId[j]
                    table.remove(self.m_tOwner.guaiId, j)
                    table.remove(self.m_tOwner.guaiBattleId, j)
                    table.remove(self.m_tOwner.guaiPositionX, j)
                    table.remove(self.m_tOwner.guaiPositionY, j)
        ...
        --单人副本创建小怪 为小怪添加battleId
        self.m_tMaritime2TotemInfo.battleId = WBattleGlobal:getCurrent():getBuildGuaiBattleId(true)
    end
    WZLog("BattleMsgBossMapSkill:_buildMaritime2Totem", Serialize(self.m_tMaritime2TotemInfo))
    local buffTotem = WBattleGlobal:getCurrent():buildMachine(MonsterType.MARITIME2_TOTEM,self.m_tMaritime2TotemInfo)
    SceneBattle:getFrontLayer():addChild(buffTotem.m_anim:getAnimNode())
end

--@brief 创建海洋领域减益图腾
function BattleMsgBossMapSkill:_buildMaritime3Totem()
    if not WBattleGlobal:getCurrent():isSingleStage() then
        --只创建一个 直接赋值
        for i=1,#self.m_tSummonMonsterId3 do
            for j=1, #self.m_tOwner.guaiId do
                if self.m_tSummonMonsterId3[i] == self.m_tOwner.guaiId[j] then
                    self.m_tMaritime3TotemInfo.battleId = self.m_tOwner.guaiBattleId[j]
                    table.remove(self.m_tOwner.guaiId, j)
                    table.remove(self.m_tOwner.guaiBattleId, j)
                    table.remove(self.m_tOwner.guaiPositionX, j)
                    table.remove(self.m_tOwner.guaiPositionY, j)
        ...
        --单人副本创建小怪 为小怪添加battleId
        self.m_tMaritime3TotemInfo.battleId = WBattleGlobal:getCurrent():getBuildGuaiBattleId(true)
    end
    WZLog("BattleMsgBossMapSkill:_buildMaritime3Totem", Serialize(self.m_tMaritime3TotemInfo))
    local buffTotem = WBattleGlobal:getCurrent():buildMachine(MonsterType.MARITIME3_TOTEM,self.m_tMaritime3TotemInfo)
    SceneBattle:getFrontLayer():addChild(buffTotem.m_anim:getAnimNode())
end

--@brief 创建执伞圣女图腾
function BattleMsgBossMapSkill:_buildUmbrella1Totem()
    if not WBattleGlobal:getCurrent():isSingleStage() then
        --只创建一个 直接赋值
        for i=1,#self.m_tSummonMonsterId1 do
            for j=1, #self.m_tOwner.guaiId do
    ...
    WZLog("BattleMsgBossMapSkill:_buildUmbrella1Totem", Serialize(self.m_tMaritime1TotemInfo))
    local buffTotem = WBattleGlobal:getCurrent():buildMachine(MonsterType.UMBRELLA1_TOTEM,self.m_tMaritime1TotemInfo)
    SceneBattle:getFrontLayer():addChild(buffTotem.m_anim:getAnimNode())
end

--@brief 创建执伞圣女图腾
function BattleMsgBossMapSkill:_buildUmbrella2Totem()
    if not WBattleGlobal:getCurrent():isSingleStage() then
        --只创建一个 直接赋值
        for i=1,#self.m_tSummonMonsterId2 do
            for j=1, #self.m_tOwner.guaiId do
    ...
    WZLog("BattleMsgBossMapSkill:_buildUmbrella2Totem", Serialize(self.m_tMaritime2TotemInfo))
    local buffTotem = WBattleGlobal:getCurrent():buildMachine(MonsterType.UMBRELLA2_TOTEM,self.m_tMaritime2TotemInfo)
    SceneBattle:getFrontLayer():addChild(buffTotem.m_anim:getAnimNode())
end

--@brief 创建守护光环
function BattleMsgBossMapSkill:_buildGuardianTotem()
    if not WBattleGlobal:getCurrent():isSingleStage() then
        --只创建一个 直接赋值
        for i=1,#self.m_tSummonMonsterId do
            for j=1, #self.m_tOwner.guaiId do
    ...
    WZLog("BattleMsgBossMapSkill:_buildGuardianTotem", Serialize(self.m_tTreatTotemInfo))
    local buffTotem = WBattleGlobal:getCurrent():buildMachine(MonsterType.GUARDIAN_TOTEM,self.m_tTreatTotemInfo)
    SceneBattle:getFrontLayer():addChild(buffTotem.m_anim:getAnimNode())
end

--@brief    检查是否有触发宠物反击
function BattleMsgBossMapSkill:checkIsHavePetBeatBack()
    -- body
    if self.m_tOwner == nil then return end 

    if self.m_tOwner.m_tPetBeatBackMsgList then 
        for i = 1, #self.m_tOwner.m_tPetBeatBackMsgList do
            local tempMsgData = self.m_tOwner.m_tPetBeatBackMsgList[i]
            if not tempMsgData.shootHero:isDead() and (not tempMsgData.shootHero:isPetSeal() or tempMsgData.shootHero:isInIngoreBuff(BuffType.PETSEAL)) and tempMsgData.shootHero.m_nRunPetBeatBackTimes <= 0 then 
...
--命运硬币加随机buff
function BattleMsgBossMapSkill:_addRandomBuff()
    WZLog("BattleMsgBossMapSkill:_addRandomBuff one", Serialize(self.m_nAddBuffId),self.m_tOwner:getBattleId(), self.m_nSkillId)
    ...
    for i, buffId in pairs (self.m_nAddBuffId) do
        ...
        for id, hero in pairs (targetHeroList) do
            if hero:isDead() ~= true then
                    ...
                    for id, effectParm in pairs (buffNew.m_nEffect) do
                        ...
                        for id, effectType in pairs (immunizeList) do
                            if effect == effectType then
                                -- buffExist = buffNew
                                offBuff = true
                                WZLog("BattleMsgBossMapSkill:_addRandomBuff offBuff", effectType)
                            end
                        end
                    end
                end
                ...
                if buffNew.m_nType == BuffType.SHAPE_NO_HOLE then
                    hero:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.NO_HOLE})
                    WZLog("BattleMsgBossMapSkill:_addRandomBuff four", self.m_nSkillId)
                elseif offSkillId then
                    BattlePetSkillManager:triggerPassiveSkillView(hero,offSkillId)
                    offBuff = true
                end

                WZLog("BattleMsgBossMapSkill:_addRandomBuff two", hero:getBattleId(), tostring(offBuff))
                if not offBuff then
                    WZLog("BattleMsgBossMapSkill:_addRandomBuff three-0", tostring(buffExistIndex), buffNew.m_nEffect)
                    ...
                    if hero.m_tBuffAddTimes[buffNew.m_nType] == nil then hero.m_tBuffAddTimes[buffNew.m_nType] = 0 end
                    WZLog("BattleMsgBossMapSkill:_addRandomBuff three-1", hero.m_tBuffAddTimes[buffNew.m_nType], round)
                    if buffExistIndex and buffNew.m_nMaxAddNum > 1 then 
                        if hero.m_tBuffAddTimes[buffNew.m_nType] < buffNew.m_nMaxAddNum then
                            local round = WBattleGlobal:getCurrent().m_nTurnTimes
                            WBattleGlobal:getCurrent().m_tBuffAddSkillPlayerList = WBattleGlobal:getCurrent().m_tBuffAddSkillPlayerList or {}
...
--选择敌人加随机buff
function BattleMsgBossMapSkill:_addRandomBuffTwo(targetHeroList)
    WZLog("BattleMsgBossMapSkill:_addRandomBuffTwo one",self.m_tOwner:getBattleId(), self.m_nSkillId)

    for id, hero in pairs (targetHeroList) do
        local buffId = BattleMethod:getRandomBuffTwo(self.m_tRandomBuffParam, hero:getBattleId())
        WZLog("BattleMsgBossMapSkill:_addRandomBuffTwo one-2",hero:getBattleId(),buffId)
        -- for i, buffId in pairs (self.m_nAddBuffId) do
            local buffInfo = GDatatab_buff["id_"..buffId]
            if hero:isDead() ~= true then
                -- if buffInfo.onset_type == 0 then 
                    ...
                    for id, effectParm in pairs (buffNew.m_nEffect) do
                        ...
                        for id, effectType in pairs (immunizeList) do
                            if effect == effectType then
                                -- buffExist = buffNew
                                offBuff = true
                                WZLog("BattleMsgBossMapSkill:_addRandomBuffTwo offBuff", effectType)
                            end
                        end
                    end
                end
                ...
                if buffNew.m_nType == BuffType.SHAPE_NO_HOLE then
                    hero:addImmunityPetSkill(self.m_nSkillId,{type = EffectTypeConfig.NO_HOLE})
                    WZLog("BattleMsgBossMapSkill:_addRandomBuffTwo four", self.m_nSkillId)
                elseif offSkillId then
                    BattlePetSkillManager:triggerPassiveSkillView(hero,offSkillId)
                    offBuff = true
                end
...
--@brief    反弹受到的伤害的百分比伤害
function BattleMsgBossMapSkill:_percentThornsStart()
    local hero = self.m_tTargetChara
    if hero == nil or hero:isDead() then 
        return true
    end
    local value = self.m_param1
    
    local skillId = self.m_nEffcetId

    local change = value * (self.m_nThornsValue and self.m_nThornsValue or 0)/100 * -1
    if change > 0 then
        change = math.ceil(change)
    else
        change = math.floor(change)
    ...
    WZLog("BattleMsgBossMapSkill:_percentThornsStart 1", self.m_nThornsValue, change, hero:getBattleId())
    change = hero:hurtEffectHandle(change * -1, true)
    ...
    if change < 0 then
        ...
        WZLog("BattleMsgBossMapSkill:_percentThornsStart 3", change, recoveryAddPercent)
    end

    if change * -1 + hero.m_nHP > hero.m_nMaxHP then
        change = (hero.m_nMaxHP - hero.m_nHP) * -1
    end
    WZLog("BattleMsgBossMapSkill:_percentThornsStart 4", change, hero.m_nHP)
    local curHp = hero.m_nHP - change
    self.m_tOwner.m_nShootHurtTotal = self.m_tOwner.m_nShootHurtTotal + change
    hero:setHp(curHp)
    hero.m_bIsThorns = true
    ...
    WZLog("BattleMsgBossMapSkill:_percentThornsStart 5", hero.m_nHP)
...
--@brief 扣减所有buffCTB
function BattleMsgBossMapSkill:_reduceBuffCTB(targetHeroList, tReduceBuffConfig)
   WZLog("BattleMsgBossMapSkill:_reduceBuffCTB id", Serialize(tReduceBuffConfig))

    for id, hero in pairs (targetHeroList) do
        WZLog("BattleMsgBossMapSkill:_reduceBuffCTB id0", hero:getBattleId())
        if hero:isDead() ~= true then
            for index, buff in pairs (hero.m_tBuffChangeStateList) do 
                WZLog("BattleMsgBossMapSkill:_reduceBuffCTB id1", buff.m_nID, buff.m_nCanRemove, buff.m_nEffectType)

                local nReduceBuffType = tReduceBuffConfig[1]
                local nReduceCTBValue = tReduceBuffConfig[2]
                WZLog("BattleMsgBossMapSkill:_reduceBuffCTB id2", nReduceBuffType, nReduceCTBValue)
                if buff.m_nCanRemove == 0 and (nReduceBuffType == 2 or (nReduceBuffType ~= 2 and nReduceBuffType == buff.m_nEffectType)) then 
                    local isClear = false
                    if buff.m_nTimeDurationValue ~= -1 and buff.m_nTimeDurationValue ~= 9999999 and nReduceCTBValue then
                        buff.m_nTimePassValue = buff.m_nTimePassValue - nReduceCTBValue
                        buff.m_nTimePassValueReal = buff.m_nTimePassValueReal - nReduceCTBValue
                        ...
                        if buff.m_nTimePassValue >= buff.m_nTimeDurationValue then 
                            isClear = true
                        end
                    end
                    if isClear then
                        if hero:getBattleId() == WBattleGlobal:getCurrent():getMyBattleId() then 
                            WndBattleHud:removeBuffIcon(buff)
                        end
                        hero:removeBuffSpecialInfluence(buff)
                        buff:removeAnime()
                        hero.m_tBuffChangeStateList[index] = nil
...
--@brief    驱散指定类型的怪
function BattleMsgBossMapSkill:_disperseMonsterByType()
    ...
    WZLog("_disperseMonsterByType 00", #machineList, Serialize(self.m_tDisperseType))
    if machineList == nil or #machineList == 0 then return end 

    for i = 1, #machineList do
        WZLog("_disperseMonsterByType 00_1", machineList[i]:isDead(), machineList[i].getMonsterType)
        if machineList[i]:isDead() ~= true and machineList[i].getMonsterType then 
            local monsterType = machineList[i]:getMonsterType()
            WZLog("_disperseMonsterByType 00_2", monsterType)
...
--@brief    设置额外生命
function BattleMsgBossMapSkill:_setExtraHP(targetHeroList)
