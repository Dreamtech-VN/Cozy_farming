...
function BattleAiCtrlHelper:getTargetPlayer()
    ...
    if WBattleGlobal:getCurrent().m_nBattleType == BattleConstants.g_nBATTLE_TYPE_BOSS and hero:getIsGuai() ~= true then
        if WBattleGlobal:getCurrent():isMyTeam(hero:getId()) then 
            self.m_tTargetPlayer = WMonster:getRandomGuai()
        else
            self.m_tTargetPlayer = WMonster:getRandomPlayer(hero)
        end
    else
        self.m_tTargetPlayer = WMonster:getRandomPlayer(hero)
    end
    WZLog("BattleAiCtrlHelper:getTargetPlayer", type(self.m_tTargetPlayer))
    if self.m_tTargetPlayer == nil then
        return
    end

    if WBattleGlobal:getCurrent().m_tMakePairOk.fighting == nil or WBattleGlobal:getCurrent().m_tMakePairOk.fighting[1] == nil or WBattleGlobal:getCurrent():isChapterOne_ThreeTeach() then
...
function BattleAiCtrlHelper:isNeedMove()
    ...
    --如果是组队副本觉醒冰雪、小丑、机械，则不动
    if WBattleGlobal:getCurrent().m_tMakePairOk.mapId == 20404 or WBattleGlobal:getCurrent().m_tMakePairOk.mapId == 20504 or WBattleGlobal:getCurrent().m_tMakePairOk.mapId == 20804 then 
        isNeedMove = 0
    end
...
function BattleAiCtrlHelper:isCanUseSkill(id)
    ...
    if not id or id == -1 or id == 0 then
        return false
    end
...
function BattleAiCtrlHelper:isCanUseItem(id)
    ...
    local itemData = GDatatab_skill["id_" .. id]
    if itemData and hero.m_nUsePoint + itemData.consume > 10000 then 
        return false 
    end
...
function BattleAiCtrlHelper:getUseItemAction()
    ...
    -- end
    for i=1,#hero.m_tItems do
        if hero.m_tItems[i] then
            local tItemData = GDatatab_skill["id_" .. hero.m_tItems[i]]
            if tItemData then
