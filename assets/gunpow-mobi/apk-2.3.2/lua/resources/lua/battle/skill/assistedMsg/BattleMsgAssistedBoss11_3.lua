-- BattleMsgAssistedBoss11_3.lua
--@brief    高射炮
--@date     2016/10/18
--@note

--@brief    消息数据表
BattleMsgAssistedBoss11_3 = {
    m_sName = "BattleMsgAssistedBoss11_3.lua",
...
function BattleMsgAssistedBoss11_3:init()
    WZLog("BattleMsgAssistedBoss11_3:init")
...
function BattleMsgAssistedBoss11_3:initStep()
...
function BattleMsgAssistedBoss11_3:bossMove()
...
--@brief 初始化运动曲线参数
function BattleMsgAssistedBoss11_3:getParaData(startPos,targetPos,dis)
    WZLog(" BattleMsgAssistedBoss11_3:getParaData",dis)
    local pos = startPos
...
function BattleMsgAssistedBoss11_3:shootEnd()
    self:playOwnerAnim("flak_3",false)
    self.m_bShootEnd = true
end

function BattleMsgAssistedBoss11_3:waitForAction()
...
function BattleMsgAssistedBoss11_3:flakReadyShoot()
    self:playOwnerAnim("flak_1",false)
end

function BattleMsgAssistedBoss11_3:flakShoot()
...
function BattleMsgAssistedBoss11_3:buildBullet()
    WZLog("BattleMsgAssistedBoss11_3:buildBullet")
...
function BattleMsgAssistedBoss11_3:moveBullet(bullet,curPos)
    ...
    -- WZLog("BattleMsgAssistedBoss11_3:moveBullet",tostring(bullet.backFire),tolua.type(bullet.backFire),tostring(curPos))
...
function BattleMsgAssistedBoss11_3:process(dt)
...
function BattleMsgAssistedBoss11_3:updateBullet()
...
--@brief 获得位移过程 y坐标
function BattleMsgAssistedBoss11_3:getPosY(posX,paraData)
    local posY = paraData[1] * posX * posX + paraData[2] * posX + paraData[3];
    return posY
end

function BattleMsgAssistedBoss11_3:checkCollision(pos)
...
function BattleMsgAssistedBoss11_3:removeBullet(index)
...
function BattleMsgAssistedBoss11_3:makeHurt(pos)
    local targetList = {}
...
--@brief    更新屏幕(主要是屏幕震动)
function BattleMsgAssistedBoss11_3:updateScene()
...
--@brief    设置屏幕震动
--@param    tPos:震动时的位置
function BattleMsgAssistedBoss11_3:setSceneSpring(tPos)
...
function BattleMsgAssistedBoss11_3:cameraMove()
...
function BattleMsgAssistedBoss11_3:playOwnerAnim(animName,isLoop)
    local monster = self:getOwner()
    monster:play(monster:getAnimationName(animName), isLoop)
end

--@brief 镜头控制
function BattleMsgAssistedBoss11_3:isCanCtrlCamera()
    return self.m_tSkillShowMsg:isCanCtrlCamera()
end

--@brief 获得技能所有者
function BattleMsgAssistedBoss11_3:getOwner()
    return self.m_tSkillShowMsg.m_tOwner
end

--@brief 获得owner坐标
function BattleMsgAssistedBoss11_3:getOwnerPos()
    return self.m_tSkillShowMsg.m_tOwner:getPosition()
end

--@brief 获得目标列表
function BattleMsgAssistedBoss11_3:getMsgTargetList()
    return self.m_tSkillShowMsg.m_tTargetList
end

--@brief 添加表演
function BattleMsgAssistedBoss11_3:msgDoAction(config)
    self.m_tSkillShowMsg:doAction(config,config.isWait)
end

--@brief    消息处理完成函数
--@note     消息系统最后一次调用process函数后调用
function BattleMsgAssistedBoss11_3:done()
    WZLog("BattleMsgAssistedBoss11_3:done")
    for i = #self.m_tBulletList,1,-1 do
        self:removeBullet(i)
    end
