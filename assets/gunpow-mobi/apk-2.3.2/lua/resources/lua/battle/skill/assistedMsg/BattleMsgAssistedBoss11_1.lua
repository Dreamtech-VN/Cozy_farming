-- BattleMsgAssistedBoss11_1.lua
--@brief    召唤心魔
--@date     2020/11/23
--@note

--@brief    消息数据表
BattleMsgAssistedBoss11_1 = {
    m_sName = "BattleMsgAssistedBoss11_1",
...
function BattleMsgAssistedBoss11_1:init()
    WZLog("BattleMsgAssistedBoss11_1:init")
    
    local bornPosList = {GlobalMethod:ccp(700,800),GlobalMethod:ccp(1500,800)}
...
function BattleMsgAssistedBoss11_1:process(dt)
...
function BattleMsgAssistedBoss11_1:cameraMove()
...
function BattleMsgAssistedBoss11_1:build()
    local list = {}
    ...
    WZLog("BattleMsgAssistedBoss11_1:build", #list)
    ...
    --    monster:setPosition(self.m_tTargetPos)
        ...
        if monster.setAppearAttribute then 
        ...
        if WBattleGlobal:getCurrent():isHostControl() then
            ProtocolProcessorBattleInterface:send_BATTLE_SkillEquip(WBattleGlobal:getCurrent():getBattleId(), monster:getBattleId(), 13077, {})
        end
    end
...
--@brief 镜头控制
function BattleMsgAssistedBoss11_1:isCanCtrlCamera()
    return self.m_tSkillShowMsg:isCanCtrlCamera()
end

--@brief 获得技能所有者
function BattleMsgAssistedBoss11_1:getOwner()
    return self.m_tSkillShowMsg.m_tOwner
end

--@brief 获得owner坐标
function BattleMsgAssistedBoss11_1:getOwnerPos()
    return self.m_tSkillShowMsg.m_tOwner:getPosition()
end

--@brief 获得目标列表
function BattleMsgAssistedBoss11_1:getMsgTargetList()
    return self.m_tSkillShowMsg.m_tTargetList
end

--@brief 添加表演
function BattleMsgAssistedBoss11_1:msgDoAction(config)
    self.m_tSkillShowMsg:doAction(config,config.isWait)
end

--@brief    消息处理完成函数
--@note     消息系统最后一次调用process函数后调用
function BattleMsgAssistedBoss11_1:done()
    WZLog("BattleMsgAssistedBoss11_1:done")
