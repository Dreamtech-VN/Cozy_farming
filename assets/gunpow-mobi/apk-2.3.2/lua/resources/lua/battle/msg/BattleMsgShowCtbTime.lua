...
function BattleMsgShowCtbTime:syncBattleInfo()
    ...
    if self.m_tPlayerId then
        ...
        local tNowExtraHP = self.m_tExtraHP
            ...
            for id, playerId in pairs (tPlayerId) do
                if combat:getBattleId() == playerId then
                    ...
                    if tNowHP[id] ~= combat:getHp() then
                        ...
                        if ProjConfig.DEBUG == 1 and math.abs(tNowHP[id] - combat:getHp()) >= 100 then 
                            MsgBoxManager:showTipBox("玩家(" .. combat:getPlayerName() .. ")伤害不一致")
                        end
                        combat:setHp(tNowHP[id])
                    end
                    if tNowExtraHP[id] ~= combat:getExtraHp() then
                        WZLog("ProtocolProcessorSceneBattle:parse_BATTLE_CanStartCurRound synchronize three", combat:getBattleId(), tostring(combat:isDead()), combat:getExtraHp(), tNowExtraHP[id])
                        combat:setExtraHp(tNowExtraHP[id], true)
                    end
...
function BattleMsgShowCtbTime:process(dt)
	if self.m_nCommonCtb then
        ...
        self.m_nRunCount = self.m_nRunCount + 1
		WZLog("BattleMsgShowCtbTime:process",self.m_nCommonCtb, BattleCtbManager.m_nUpdateCTB_time , self.m_nCount)

		if BattleCtbManager.m_nUpdateCTB_time > 0 then
            for id, chara in pairs (WBattleGlobal:getCurrent():getCharacterList(nil, true, true)) do
                chara:updateBuffByCTB(dt, self.m_nCommonCtb, self.m_nRunCount)
			end
