...
--@brief	消息数据表
BattleMsgMaritimeMove = {
    m_sName = "BattleMsgMaritimeMove",
	m_nBattleId = 0, --战斗id
...
function BattleMsgMaritimeMove:init()
	...
	BattleMsgMaritimeMove.m_nProcess = BattleMsgMaritimeMove.m_nProcess + 1
	
	local hero = WBattleGlobal:getCurrent():getCharacterWithId(self.m_nCurrentPlayerId)
	if hero == nil then
		WZLog("BattleMsgMaritimeMove:init", "can't find player:", self.m_nCurrentPlayerId)
		return
	end
    ...
    WZLog("BattleMsgMaritimeMove:init", self.m_nCurPositionX, self.m_nCurPositionY, localPlayerPos.x, localPlayerPos.y, BattleMsgMaritimeMove.m_nProcess)
	if self.m_nCurrentPlayerId == WBattleGlobal:getCurrent():getMyBattleId() and (not WBattleGlobal:getCurrent():isAudience() or not WBattleGlobal:getCurrent():isReplayGame()) then
...
function BattleMsgMaritimeMove:process()
	WZLog("BattleMsgMaritimeMove:process zero", self.m_nMovecount, self.m_tMovestep[1], BattleMsgMaritimeMove.m_nProcess)
	if BattleMsgMaritimeMove.m_nProcess ~= 1 then
		return false
	end
	...
	if hero == nil then
		WZLog("BattleMsgMaritimeMove:process", "can't find player:", self.m_nCurrentPlayerId)
		return
	elseif hero.m_bIsMoved == true then
		WZLog("BattleMsgMaritimeMove:process", "player has been moved:", self.m_nCurrentPlayerId)
		return
	end
	...
	if self.m_nMovecount > 0 then
        ...
        if hero == WBattleGlobal:getCurrent():getMyHero() and hero.m_bIsGuaiWithSuit ~= true then
            ...
            --WZLog("BattleMsgMaritimeMove:process", hero:getPF())
		...
		if (hero.m_nStopByTornado == 1 and self.m_tMovestep[1] == 1) or (hero.m_nStopByTornado == 2 and self.m_tMovestep[1] == 0) then 
			WZLog("BattleMsgMaritimeMove:process 4444444", hero.m_nStopByTornado, hero:getAnimation():isFlipX(), self.m_tMovestep[1], self.m_nMovecount)
...
function BattleMsgMaritimeMove:done()
    local hero = WBattleGlobal:getCurrent():getCharacterWithId(self.m_nCurrentPlayerId)

	BattleMsgMaritimeMove.m_nProcess = BattleMsgMaritimeMove.m_nProcess - 1

    WZLog("BattleMsgMaritimeMove:done", BattleMsgMaritimeMove.m_nProcess)
    ...
    if WBattleGlobal:getCurrent():isFlyCopy() then
        ...
        if dis < 80 then
            ...
            WZLog("BattleMsgMaritimeMove:done zero")
        end
    end
