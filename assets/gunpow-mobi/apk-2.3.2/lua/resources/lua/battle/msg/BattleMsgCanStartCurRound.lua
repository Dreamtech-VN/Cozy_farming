...
function BattleMsgCanStartCurRound:init()
	...
	WBattleGlobal:getCurrent().m_nAutoFightActIndex = 0
    WBattleGlobal:getCurrent().m_bIsPlayerOperateAlready = false
	WBattleGlobal:getCurrent():startNewRound()
    WBattleGlobal:getCurrent():addBatiEffect()
    ...
    if not AutoRunBattleConst.AUTO_RUN_BATTLE and not WBattleGlobal:getCurrent():isReplayGame() and not GlobalGame.g_bIsAutoFightOpen then
...
--                    WZLog("BattleMsgCanStartCurRound:init one",i,v.triggerWay[1][1],v.triggerWay[1][2],v.triggerWay[1][3],WBattleGlobal:getCurrent().m_tMakePairOk.mapId, tostring(WndTeachTalk:isStoryFinish(v.storyId)), v.count == -1)
    ...
    if WBattleGlobal:getCurrent().m_nBattleType == BattleConstants.g_nBATTLE_TYPE_BOSS and WBattleGlobal:getCurrent().battleMode == BattleConstants.g_tBossBattleMode.MODE_COUPLE_HEGEMONY then
        if WBattleGlobal:getCurrent().m_tMakePairOk.guaiId[1] == currentCharacter.m_nPlayerId then
            WndCoupleHegemonyInfoView:setRoundNum(currentCharacter.m_nMyTurnCount)
        end
    end
    if WBattleGlobal:getCurrent().m_nBattleType == BattleConstants.g_nBATTLE_TYPE_BOSS and WBattleGlobal:getCurrent().battleMode == BattleConstants.g_tBossBattleMode.MODE_REMAINSBOSS then
        if WBattleGlobal:getCurrent().m_tMakePairOk.guaiId[1] == currentCharacter.m_nPlayerId then
            WndCopyRelicView:setRoundNum(currentCharacter.m_nMyTurnCount)
        end
    end

    self:showMonsterDialog2001()
end
            ...
            elseif mapId == 9999 then
                ...
                if self.m_nCurrentPlayerId == WBattleGlobal:getCurrent():getMyHero():getBattleId() then
                    ...
                    if myTurnCount == 0 then
                        PostPlayerEvent:postEvent(PostPlayerEvent.event_teachBossAttack)
                        PostPlayerEvent:postTeach("0-2")
                    elseif myTurnCount == 1 then
                        PostPlayerEvent:postEvent(PostPlayerEvent.event_teachBossScatter)
                        msg.m_nStep = 14
                    elseif myTurnCount == 2 then
                        PostPlayerEvent:postEvent(PostPlayerEvent.event_teachBossContinueShoot)
                        msg.m_nStep = 15
                    elseif myTurnCount == 3 then
...
--@brief    显示怪物对白2001-回合数
function BattleMsgCanStartCurRound:showMonsterDialog2001()
    for _,guai in pairs(WBattleGlobal:getCurrent():getGuaiList()) do
        if guai:getType() == CharacterType.TYPE_GUAI or guai:getType() == CharacterType.TYPE_HERO and guai.m_bIsGuaiWithSuit == true then
            local tMonsterInfo = GDatatab_monster["id_"..guai:getId()]
                ...
                if tempTalk and tempTalk[1] and tempTalk[1][1] and tempTalk[1][1] ~= -1 then
                    for i=1, #tempTalk[1] do
                        ...
                        if tStoryTalk and tempTalk[2] and tempTalk[2][i] and tempTalk[3] and tempTalk[3][i] then
                            if tempTalk[2][i] == 2001 then
                                local nTurnTimes = guai:getPlayerTurnNumber()
                                if WBattleGlobal:getCurrent():getCurrentCharacter().m_nPlayerId == guai:getId() and nTurnTimes == tempTalk[3][i] then
                                    guai:talk(tStoryTalk.talk)
                                    return
                                end
                            end
                        end
