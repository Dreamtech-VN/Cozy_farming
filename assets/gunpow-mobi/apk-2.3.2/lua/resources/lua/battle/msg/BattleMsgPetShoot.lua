...
g_nPetShootAcctionFlag = -1			--CCAcction状态标记(-1:空状态,0:有状态,>0:id)

-------------------------------------公有方法模块--------------------------------------

--@brief	消息初始化函数
--@note		消息系统第一次调用process函数前调用
function BattleMsgPetShoot:init()
	WZLog("BattleMsgPetShoot:init")
...
function BattleMsgPetShoot:done()
	WZLog("BattleMsgPetShoot:done")
    ...
    if tPet ~= nil and self.m_nOldOpacity then
    	local newOpacity = self.m_shootHero:getAnimation():getAnimNode():getOpacity()--防止坐骑隐身时候宠物没有隐身
    	if newOpacity ~= self.m_nOldOpacity then 
    		self.m_nOldOpacity = newOpacity
    	end
...
--@brief	宠物技能回调
function BattleMsgPetShoot:usePetSkillOkCallback()
	WZLog("BattleMsgPetShoot:usePetSkillOkCallback")
	self.m_bIsUsePetSkillOk = true
end
-------------------------------------私有方法模块--------------------------------------
--@brief    检查全部人是否着地或掉坑或死亡
function BattleMsgPetShoot:_checkAllCollision()
...
--@brief	宠物技能
function BattleMsgPetShoot:_petSkill()
	if not self.m_bIsUsePetSkill then
		WZLog("BattleMsgPetShoot:_petSkill")
		local tPet = self.m_shootHero:getPet()
...
--@brief	消失完成回调
function EndDisppear(sender)
	g_nPetShootAcctionFlag = -1
end

--@brief	出现完成回调
function EndAppear(sender)
	g_nPetShootAcctionFlag = -1
end

--@brief	投弹回调
function DropBomb(sender)
	WZLog("DropBomb")
	local tTable = g_nPetShootAcctionFlag
...
--@brief	投弹完成回调
function EndDropBomb(sender)
	g_nPetShootAcctionFlag = -1
end
--[[--@brief	延迟播放受伤动画
function DelayStartHurt()
	local chara = WBattleGlobal:getCurrent():getCharacterWithId(g_nPetShootAcctionFlag)
...
--@brief	准备投弹
function BattleMsgPetShoot:_readyDropDomb()
...
--@brief	投弹
function BattleMsgPetShoot:_dropADomb()
	WZLog("_dropADomb")
	...
	array:addObject(CCCallFuncN:create(DropBomb))
	array:addObject(CCDelayTime:create(0.1))
	array:addObject(CCFadeOut:create(0.08))
	array:addObject(CCCallFuncN:create(EndDropBomb))
	g_nPetShootAcctionFlag = self
	self.m_shootHero:getPet():getAnimation():getAnimNode():runAction(CCSequence:create(array))
	return true
end

--@brief	完成投弹
function BattleMsgPetShoot:_endDropDomb()
WZLog("_endDropDomb")
	return g_nPetShootAcctionFlag == -1
end

--@brief	播放缓冲动画
function BattleMsgPetShoot:_playBufferAnim()
	WZLog("_playBufferAnim")
    self.m_shootHero:getPet():getAnimation():play(self.m_shootHero:getPet():getPetAtkAnimName(),false)
	return true
end
--@brief	播放准备消失动画
function BattleMsgPetShoot:_playReadyDisappearAnim()
    if self.m_beShootedChara.m_nRepulseDis == nil or self.m_beShootedChara.m_nRepulseDis == 0 then
        if g_nPetShootAcctionFlag == -1 then
                g_nPetShootAcctionFlag = 0
                local array = CCArray:create()
                array:addObject(CCFadeOut:create(0.07))
                array:addObject(CCCallFuncN:create(EndDisppear))
                self.m_shootHero:getPet():getAnimation():getAnimNode():runAction(CCSequence:create(array))
                return true
        end
    else
        WZLog("BattleMsgPetShoot:_playReadyDisappearAnim", self.m_beShootedChara:getBattleId(), tostring(self.m_beShootedChara:isDead()), self.m_beShootedChara.m_nRepulseDis)
    end
	return false
end
--@brief	消失
function BattleMsgPetShoot:_disappear()
	if g_nPetShootAcctionFlag == -1 then
...
--@brief	屏幕移到目标上
function BattleMsgPetShoot:_runToTarget()

	if g_nPetShootAcctionFlag == -1 then
		local zoom = BattleScreen:followHero(self.m_beShootedChara:getAnimation():getPosition())
...
--[[--@brief	播放射击动画
function BattleMsgPetShoot:_targetMarkHurt()
	g_nPetShootAcctionFlag = self.m_beShootedChara:getBattleId()
	local array = CCArray:create()
...
function BattleMsgPetShoot:_endAttackAnim()
    ...
    if self.m_shootHero:getPet().m_bIsAtk then
        ...
        if hurtValue ~= nil and hurtValue > 0 then
	        chara:markHurt(hurtValue,self.m_shootHero,nil,true,nil,petRatio, nil, nil, nil, nil, self.m_nPetShootIndex and self.m_nPetShootIndex > 1 and true or false)
...
--@brief	出现
function BattleMsgPetShoot:_appear()
	WZLog("_appear")
	g_nPetShootAcctionFlag = 0
	local array = CCArray:create()
	array:addObject(CCFadeIn:create(0.1))
	array:addObject(CCCallFuncN:create(EndAppear))
	self.m_shootHero:getPet():getAnimation():getAnimNode():runAction(CCSequence:create(array))
	return true
end

--@brief	等待出现完毕
function BattleMsgPetShoot:_waitEndAppear()
	WZLog("_waitEndAppear")
	return g_nPetShootAcctionFlag == -1
end

--@brief	更新子弹状态
function BattleMsgPetShoot:_updateBullet()
...
--@brief	更新子弹状态
--@return	#1:返回伤害数值
function BattleMsgPetShoot:_getHurtValue()
	local chara = self.m_beShootedChara
	local hurtValue,petRatio = self:calculateHurt(chara,self.m_shootHero:getPet().m_tPetInfo.petParam1 / 10000)
    WZLog("BattleMsgPetShoot:_getHurtValue", tostring(self.m_shootHero:getPet().m_tPetInfo.petParam1), hurtValue, self.m_shootHero.m_nAttackPet)

	return hurtValue,petRatio
end

--@brief	屏幕跟踪子弹
function BattleMsgPetShoot:_followBullet()
...
--@brief	播放射击动画
function BattleMsgPetShoot:_playAttackAnim()
	if g_nPetShootAcctionFlag == -1 then
		local tPet = self.m_shootHero:getPet()
        --SoundManager:playEffectSound(tPet.m_sAtkSound or SoundDefine.E_S_SHOOT_PET)
		tPet:getAnimation():play(tPet:getPetAtkAnimName(),false)
		local effectAnim = tPet:getAnimation():getEffectAnim()
		WZLog("BattleMsgPetShoot:_playAttackAnim", tostring(effectAnim))
		if effectAnim then
...
--@brief	创建子弹
function BattleMsgPetShoot:_createBullet()
	WZLog("_createBullet")
	if self.m_shootHero:getPet():getAnimation():isCurrentAnimationDone() then
...
--@brief	等待子弹飞行和英雄受伤
function BattleMsgPetShoot:_waitForBulletAndHurt()
...
--@brief	屏幕初始化
function BattleMsgPetShoot:_resetZoomToHero()
...
--@brief	屏幕移向英雄
function BattleMsgPetShoot:_zoomToHero()
    WZLog("BattleMsgPetShoot:_zoomToHero")
...
--@param	charas:英雄列表
--@param	values:伤害列表
function BattleMsgPetShoot:_sendHurtProtocol(charas,values)
    ...
    --         for id,chara in pairs(charas) do
    --             if v:getId() == chara:getId() then
    --                 record.guaiHurtWithPet[index] = values[id]
    --             end
    --         end
	...
	if hurtCount > 0 then
		ProtocolProcessorBattleInterface:send_BATTLE_PetAttack(WBattleGlobal:getCurrent().m_tMakePairOk.battleId, self.m_shootHero:getBattleId(),hurtIds,hurtValues)
	end
end

function BattleMsgPetShoot:getHurtType(shootHero,targetHero)
    return WBullet.getHurtType(self,shootHero,targetHero)
end

--@brief	根据距离、射击玩家、被射玩家计算出基础伤害值
--@return	#1：伤害
function BattleMsgPetShoot:calculateHurt(targetHero,petCoef)
	WZLog("BattleMsgPetShoot:calculateHurt", petCoef,self.m_shootHero:getPetCrit(), self.m_shootHero.m_tPetSkillTakeEffectInfo and Serialize(self.m_shootHero.m_tPetSkillTakeEffectInfo))
	...
	if hero.m_tPetSkillTakeEffectInfo ~= nil then
		WZLog("BattleMsgPetShoot:calculateHurt two", Serialize(hero.m_tPetSkillTakeEffectInfo))
   	...
   	if hero.m_tPetEquipEffectTakeEffectInfo ~= nil then
		WZLog("BattleMsgPetShoot:calculateHurt two_11", Serialize(hero.m_tPetEquipEffectTakeEffectInfo))
		for i,v in ipairs(hero.m_tPetEquipEffectTakeEffectInfo) do
			WMonsterAI:castSkill(nil,
    		nil,
            nil,
            {[1]=SkillTypeConfig.EFFECT},
