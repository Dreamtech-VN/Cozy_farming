...
function BattleMsgAddBuff:init()
    ...
    WZLog("BattleMsgAddBuff:init one",self.m_tData.playerId , self.m_tData.buffId)
    ...
    if self.m_tData.useType == 0 then
        ...
        if WBattleGlobal:getCurrent().m_tBuffAddPlayerList then
            ...
            for i, buffPlayer in pairs (WBattleGlobal:getCurrent().m_tBuffAddPlayerList) do
                if (buffPlayer.round == round or self.m_tData.buffId == 7019 or self.m_tData.buffId == 7020) and buffPlayer.buffId == self.m_tData.buffId and buffPlayer.playerId == self.m_tData.playerId then
                    index = i
                    break
                end
            end
