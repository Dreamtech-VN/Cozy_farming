...
function ScenePvpAmuse:_init()
	...
	self.m_nCurDropNum = 0 				--当前已掉落的数量
	self.m_nMatchTypeIndex = 1
end
...
function ScenePvpAmuse:_unInit()
	self.m_root = nil
	self.m_tItemList = nil 
	self.m_nCurDropNum = nil 				--当前已掉落的数量
	self.m_nMatchTypeIndex = 1
end
...
function ScenePvpAmuse:setOpenState(matchType, activityStatus, dayOfWeek, dayBegin, coinDropNum)
	WZLog("ScenePvpAmuse:setOpenState =",Serialize(matchType),Serialize(activityStatus),Serialize(dayOfWeek), Serialize(dayBegin), coinDropNum)
	if self.m_root == nil then
		return
	end

	self.m_nCurDropNum = coinDropNum
	self.m_tItemList = {}
	...
	table.sort(self.m_tItemList, function (a,b)
		...
		elseif a.dayBegin ~= b.dayBegin then 
			return a.dayBegin < b.dayBegin
		else
