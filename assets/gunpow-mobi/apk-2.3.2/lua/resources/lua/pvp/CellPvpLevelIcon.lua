...
function CellPvpLevelIcon:onCheck(element)
    ...
    if self.m_tData.id == 1 then
        ...
        local tData = {icon="ui/common/common_icon_pws1.png",
            ...
            highLightObj = self,
            pvprankMark = 1,
            maxLevel = self.m_nHistoryMaxLevel,
            }
        WndTips:show(element,ScenePvpRank.m_root,1,tData,GlobalMethod:ccp(336,60), true)
    else
        if self.m_tPvpData then
            local data = {winNum = self.m_tPvpData.winTimes,total = self.m_tPvpData.joinTimes,maxWinNum = self.m_tPvpData.continous, exp = self.m_nCurIntegral, maxLevel = self.m_nHistoryMaxLevel}
...
function CellPvpLevelIcon:_update()
    ...
    local spineIcon = GetElement(self.m_root, "spineIcon_CellPvpLevelIcon", WZUISpine)
    if tData.icon == "common_icon_pws1" then
        spinePath = "icon_qingtong_01"
    elseif tData.icon == "common_icon_pws2" then
        spinePath = "icon_baiyin_01"
    elseif tData.icon == "common_icon_pws3" then
        spinePath = "icon_huangjin_01"
    elseif tData.icon == "common_icon_pws4" then
        spinePath = "icon_baijin_01"
    elseif tData.icon == "common_icon_pws5" then
        spinePath = "icon_zuanshi_01"
    elseif tData.icon == "common_icon_pws6" then
