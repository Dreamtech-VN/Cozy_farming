--SceneWorldTeamBossRoomData.lua
--@brief	SceneWorldTeamBossRoom的数据模块
--@date		2018/07/12
--@author	Tianxiang_Xu
--@note		世界组队boss房间
...
function SceneWorldTeamBossRoom:_init()
...
function SceneWorldTeamBossRoom:_unInit()
	self.m_root = nil
	self.m_tMapData = nil
...
--@note		请仅用此方法创建场景
function SceneWorldTeamBossRoom:createElement()
	local element = WZUISystem:getInstance():createElement("SceneWorldTeamBossRoom")
	assert(element, "SceneWorldTeamBossRoom create element failed!")
	self:_init()
	return element
end

--@brief   外部接口
function SceneWorldTeamBossRoom:showEnter()
    ProtocolProcessorWorldTeamBossRoom:regAll() --注册协议

    local name = string.format(LocalStrings.ROOMS, CacheCenter:getPlayerInfo().name)
    ProtocolProcessorWorldTeamBossRoom:send_TEAMWORLDBOSS_CreateRoom(name, "-1")
end

--@brief   外部接口
function SceneWorldTeamBossRoom:showInterface()
    local scene = SceneWorldTeamBossRoom:createElement()
    replaceScene( scene )
end

--@brief    设置房间信息   
function SceneWorldTeamBossRoom:setData(roomId, passWord, roomName, playerNumMode, mapId, wnersId, playerNum, seatUsed, playerId, serverId,
            playerName, playerLevel, playerReady, playerSex, playerEquipment, playerEquipmentLevel, vipLevel, player_title, qualifyingLevel, zsleve, playerStar, playerFighting, pet, extranInfo, playerHeadColour, playerBodyColour, mentoringStr, coupleStr, chumStr, coupleNum, chumNum, mentoringNum, matchLevel, matchscore, joinTimes, winTimes, continuousWinTimes, serviceId, inspire, openStatus)
...
function SceneWorldTeamBossRoom:receiveEnterRoomOk(roomId, passWord, roomName, playerNumMode, mapId, wnersId, playerNum, seatUsed, playerId,serverId, playerName, playerLevel, playerReady, playerSex, playerEquipment, playerEquipmentLevel, vipLevel, player_title, qualifyingLevel,zsleve, playerStar, playerFighting, pet, extranInfo, playerHeadColour, playerBodyColour, mentoringStr, coupleStr, chumStr, coupleNum, chumNum, mentoringNum, matchLevel, matchscore, joinTimes, winTimes, continuousWinTimes, serviceId, inspire, openStatus)
    ...
    self:setData(roomId, passWord, roomName, playerNumMode, mapId, wnersId, playerNum, seatUsed, playerId,serverId,
            ...
            mentoringStr,coupleStr,chumStr,coupleNum,chumNum,mentoringNum,matchLevel,matchscore,joinTimes,winTimes,continuousWinTimes,serviceId,inspire,openStatus)
    
    WZLog("SceneWorldTeamBossRoom:receiveEnterRoomOk =", Serialize(inspire))
    if self.m_root and self.m_bIsVoice and self.m_bIsVoiceState and GetElement(self.m_root,"conFigureVoice1_SceneWorldTeamBossRoom",WZUIContainer) then
...
function SceneWorldTeamBossRoom:closeLoading()
...
--@brief    玩家被邀请进入副本房间
function SceneWorldTeamBossRoom:beInvited(roomId , playerName,mapId, password, roomChannel, interfaceId, playerId)
    WZLog("SceneWorldTeamBossRoom:beInvited")

    local data = GDatatab_team_world_boss_map["id_"..mapId]
    desc = string.format(LocalStrings.TEAMBOSS_TEXT6, playerName)
...
function SceneWorldTeamBossRoom:send_EnterRoom(roomId,roomChannel,password,mapId,des,battleId)
    WZLog("SceneWorldTeamBossRoom:send_EnterRoom ")
...
--@brief    手动更新房间
function SceneWorldTeamBossRoom:updateRoom()
    WZLog("SceneWorldTeamBossRoom:updateRoom")
    if self.m_root ~= nil then
    	ProtocolProcessorWorldTeamBossRoom:send_TEAMWORLDBOSS_UpdateRoom(self.m_tData.roomId, self.m_tData.passWord, self.m_tData.roomName)
    end
end

--@brief    正在匹配中
--@param    入参与服务端发送给客户端的协议回调方法参数相同
function SceneWorldTeamBossRoom:receiveMakePairring(roomId)
    if self.m_root == nil then
        return
    end
    WZLog("SceneWorldTeamBossRoom:receiveMakePairring",roomId)
...
--@brief  改开始游戏为取消匹配
function SceneWorldTeamBossRoom:changeStartGameBtn(iconPath)
    WZLog("SceneWorldTeamBossRoom:changeStartGameBtn")
    local imgStartGame = GetElement(self.m_root,"imgStartGame_SceneWorldTeamBossRoom",WZUIImage)
    imgStartGame:setFile(iconPath)
end

--@brief    玩家技能
--@param    入参与服务端发送给客户端的协议回调方法参数相同
function SceneWorldTeamBossRoom:receiveGetPlayerSkillOk(itemId,skillExplain)
    WZLog("SceneWorldTeamBossRoom:receiveGetPlayerSkillOk")
...
--@brief    正在匹配中
--@param    入参与服务端发送给客户端的协议回调方法参数相同
function SceneWorldTeamBossRoom:receiveMakePairError(nFlag, sMessage)
    if self.m_root == nil then
        return
    end
    WZLog("SceneWorldTeamBossRoom:receiveMakePairError",nFlag, sMessage)
    if nFlag == 0 then
...
--@brief    匹配失败
--@param    入参与服务端发送给客户端的协议回调方法参数相同
function SceneWorldTeamBossRoom:receiveMakePairFail()
    WZLog("SceneWorldTeamBossRoom:receiveMakePairFail")
...
function SceneWorldTeamBossRoom:receiveStartOk(battleId, mapId, playerCount, playerId, serverId, playerName, playerTitle, playerGuild, playerLevel, playerSex, maxHP, maxPF, maxSP, attack, critRate, defence, injuryFree, wreckDefense, reduceCrit, power, armor, constitution, agility, lucky, inspire, headId, faceId, bodyId, weaponId, wingId, item_id, skillId, playerBuffCount, buffId, petAnimation, petGift, guaiBattleId, guaiId, guaiMaxHP, guaiNowHP, guaiAttack, petAdvancedLevel, colour, bodycolour, footmark, professionId, professionSkill, mountId, childId, childName, childSex, childImage, assistSkillIds, defaultShapeBigSkill, blastEffect, extPropertyKey, extPropertyValue, extPropertyCount)
	...
	WBattleGlobal:getCurrent().m_tMakePairOk ={
        battleId=battleId, battleMode=BattleConstants.g_tBossBattleMode.MODE_WORLDTEAMBOSS, battleMull=false, battleChannle=-1,
    ...
    WBattleGlobal:getCurrent().battleMode = BattleConstants.g_tBossBattleMode.MODE_WORLDTEAMBOSS
...
--@brief    退出房间需要发送退出协议
function SceneWorldTeamBossRoom:onBackScene(isTurnToScene)
    if SceneWorldTeamBossRoom.m_root == nil then return end
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
    WZLog("SceneWorldTeamBossRoom:onBackScene", self.m_tData, tostring(isTurnToScene))
...
--显示当前房间的玩家聊天信息
function SceneWorldTeamBossRoom:showChat(txtMsg,playerId,bubbleId)
    WZLog("SceneWorldTeamBossRoom:showChat ",txtMsg,playerId)
    ...
    if seatIndex > 0 then
        ...
        if self.m_tData.roomChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_LX then  --练习赛
            conCenter = GetElement(self.m_root,"conCenter2_SceneWorldTeamBossRoom",WZUIContainer)
        end
        local conSeat = GetElement(conCenter,"conSeat" .. seatIndex .. "_SceneWorldTeamBossRoom",WZUIContainer)
        local conPlayer = GetElement(conSeat,"conPlayer_SceneWorldTeamBossRoom")
...
function SceneWorldTeamBossRoom:checkCellChatBubble(seatIndex)
    WZLog("SceneWorldTeamBossRoom:checkCellChatBubble ")
    local cellChatBubble = self.m_root:getChildByTag(seatIndex+1110)
...
--保存各种关系的数值
function SceneWorldTeamBossRoom:initRelationShip()
    WZLog("SceneWorldTeamBossRoom:initRelationShip")
    self.m_MasterList = nil   --师徒关系列表
    self.m_FriendList = nil   --朋友关系列表
    self.m_SpouseList = nil   --夫妻关系列表
...
--解析后的好友度信息使用按规则保存,返回
--@param  玩家ID
function SceneWorldTeamBossRoom:getFriendRV(playerId)
    WZLog("SceneWorldTeamBossRoom:getFriendRV")
    local temp3 = {}
...
--解析师徒信息按规则保存,返回
--@param  玩家ID
function SceneWorldTeamBossRoom:getMasterRV(playerId,playerLevel)
    WZLog("SceneWorldTeamBossRoom:getMasterRV = ",Serialize(self.m_MasterList))
...
--解析夫妻信息按规则保存,返回
--@param  玩家ID
function SceneWorldTeamBossRoom:getSpouseRV(playerId,playerSex,playerName)
    WZLog("SceneWorldTeamBossRoom:getSpouseRV ")
...
--@brief    更新多套时装数据
function SceneWorldTeamBossRoom:updateDressSuitData(nType)
    -- body
...
--@brief 	获取boss当前血量成功
function SceneWorldTeamBossRoom:getBossBloodOk(nCurBlood, maxHp)
	-- body
    self:closeLoadingBox()
...
--@brief 	退出房间
function SceneWorldTeamBossRoom:exitMulRoom()
...
--@brief    鼓舞返回
function SceneWorldTeamBossRoom:inspireResult(inspire)
    --body
...
--@brief    设置伤害榜数据
function SceneWorldTeamBossRoom:setHurtList(rankPlayerCount, rankPlayerId,rankPlayerName, rankPlayerSex, rankPlayerHeadId, rankPlayerFaceId, rankPlayerHeadColor, rankPlayerVipLevel, rankHurt, level, myRank, myHurt)
    -- body
    self:closeLoadingBox()
    local nIndex = 1
    self.hurtInfo = {}
...
--    WZLog("SceneWorldTeamBossRoom:setHurtList", Serialize(self.hurtInfo), self.bossRoomInfo.myRank, self.bossRoomInfo.hurt)
...
function SceneWorldTeamBossRoom:setEnterRoomData( mapId, bossBloodCurrent, inspire, dimaCDTime, goldCDTime, bossState, openTime, challengeNum, leaveNum, maxHp)
    self:closeLoadingBox()
    ...
    WZLog("SceneWorldTeamBossRoom:setEnterRoomData", self.bossRoomInfo.totalNum, self.bossRoomInfo.leftNum, self.bossRoomInfo.openTime, self.bossRoomInfo.bossState, self.bossRoomInfo.bossBloodCurrent)
    -- 设置鼓舞状态、
...
--@brief    更新挑战次数
function SceneWorldTeamBossRoom:updateChallengeTimes(times)
    -- body
    self:closeLoadingBox()
...
--@brief    设置退出世界boss回调
function SceneWorldTeamBossRoom:setCallBackFun(tCell, func)
    -- body
...
--@brief    更新技能方案名字
function SceneWorldTeamBossRoom:updateSkillSuitData()
    -- body
...
--@param    物品id表
--@return   物品路径表
function SceneWorldTeamBossRoom:idToPath(idTable)
...
--@brief  添加顶部导航栏
function SceneWorldTeamBossRoom:addTop()
    WZLog("SceneWorldTeamBossRoom:addTop")
    local cell,tcell = CellTopHandle:createElement()
    self.m_root:addChild(cell)
    self.m_tTopHangle = tcell
    self.m_oTopObject = cell
    
	tcell:setTopData("ui/common/common_icon_sylz.png",SceneWorldTeamBossRoom,SceneWorldTeamBossRoom.onClose,false,true,true,"SceneWorldTeamBossRoom")
    tcell:setImageBgVisible(true)
end

function SceneWorldTeamBossRoom:_shieldClick()
    WZLog("SceneWorldTeamBossRoom:_shieldClick")
...
-- 初始化排行榜奖励的信息和击杀信息
function SceneWorldTeamBossRoom:initRewardRankInfo()
...
-- endP 结束鼓舞值
-- bFlag 是否处于鼓舞状态
function SceneWorldTeamBossRoom:_initInspireState()
    self.inspireData = {startP = 0, endP = 0, bFlag = false}
end

function SceneWorldTeamBossRoom:_setCurInspireState(inspire)
...
--@brief    挑战状态判断
function SceneWorldTeamBossRoom:_challengeStateJudge()
    -- body
...
--@brief    挑战次数判断
function SceneWorldTeamBossRoom:_challengeTimesJudge()
    -- body
 --   do return true end 
...
--@brief    确定购买次数
function SceneWorldTeamBossRoom:sureToBuyTimes()
    -- body
    local num = self.bossRoomInfo.totalNum - self.m_tSysConfig.freeNum
    local buyData = self:getVipLimitData(num + 1)
...
--@brief    确定使用蓝钻代替粉钻购买
function SceneWorldTeamBossRoom:sureToUseDiaInstead()
    -- body
...
--@brief    确定充值
function SceneWorldTeamBossRoom:sureToRecharge()
    --body
    PassportSdkManager:gotoPaymentPage()
end

--@brief    根据购买次数获取限购数据
function SceneWorldTeamBossRoom:getVipLimitData(times)
    -- body
...
function SceneWorldTeamBossRoom:_getMapBgById()
    WZLog("SceneWorldTeamBossRoom:_getMapBgById")
    local roomData = self.m_tData
    local tCopyData = GDatatab_team_world_boss_map["id_"..roomData.mapId]
