...
function WndSingleCopySettlement:onEnter(element)
    ...
    if self.isWin then
        ...
        -- end
        --越南完美通关虚空幻境11层弹商店评分
        if self.m_nStarNum == 3 and WBattleGlobal:getCurrent():getBattleMode() == BattleConstants.g_tBossBattleMode.MODE_DOUBLETOWER_STAGE then 
            local mapId = WBattleGlobal:getCurrent().m_tMakePairOk.mapId
            local tLevelData = GDatatab_grouptower_map["id_" .. mapId]
            if tLevelData and tLevelData.floor_num == 11 then 
                ShowStoreRating()
            end
        end
    else
...
function WndSingleCopySettlement:goback()
    ...
    elseif WBattleGlobal:getCurrent():getBattleMode() == BattleConstants.g_tBossBattleMode.MODE_DOUBLETOWER_STAGE then
        ...
        self:_postBackToSingleCopyEvent()
...
function WndSingleCopySettlement:_updateStar()
    local nStarNum = 0
    if self.m_nWinType == 1 then 
        local tBits = NumberToBits(self.m_tData.flopRebate, 3)
...
function WndSingleCopySettlement:_updateConditionList()
    ...
    if self.m_nWinType == 1 then 
        ...
        tGoalList = {}
        for i = 1, 3 do
            if type(tLevelData["pass" .. i]) == "table" then 
                local tItem = {}
                local content1 
                local nConIndex = tLevelData["pass" .. i][1][1]
                if nConIndex == 6 or nConIndex == 8 then 
                    local skillName = WndDoubleTowerRoom:getSkillName(tLevelData["pass" .. i][1][2])
                    content1 = string.format(LocalStrings.DOUBLETOWER_TEXT6[nConIndex], skillName)
                else
                    content1 = string.format(LocalStrings.DOUBLETOWER_TEXT6[nConIndex], tLevelData["pass" .. i][1][2])
                end
                tItem[1] = content1
                for k = 1, #self.m_tData.flopId do
                    if self.m_tData.flopId[k] == nConIndex then 
                        tItem[2] = self.m_tData.flopCount[k]
                        break
                    end
                end
                tItem[3] = tBits[i] or 0

                table.insert(tGoalList, tItem)
            end
        end
    else
        local tLevelData = GDatatab_single_map["id_"..self.m_tData.pointId]
...
function WndSingleCopySettlement:_updateReward()
    ...
    if self.m_nWinType == 1 then 
        ...
        for i = 1, #self.m_tData.playerIds do
            if self.m_tData.playerIds[i] > 0 and self.m_tData.playerIds[i] == CacheCenter:getPlayerInfo().id then
                local playerData = self:_getPlayerSettlementData(i)
                tReward = playerData.reward
                break 
            end
        end
        ...
        for i = 1, #tReward do
            if tReward[i].rewardId == 2 then --金币
                nGold = tReward[i].rewardCount
            elseif tReward[i].rewardId == 3 then --经验
                nExp = nExp + tReward[i].rewardCount
            else --其他物品
                if playerLv >= needLv or (playerLv < needLv and tReward[i].rewardId ~= 95) then
                    local reward = {itemId = tReward[i].rewardId, itemCnt = tReward[i].rewardCount}
    ...
    if not pInfo then
        ...
        pInfo.exp = CacheCenter:getPlayerInfo().exp - nExp
    end
...
--@brief    单人副本胜利事件
function WndSingleCopySettlement:_postSingleCopyWinEvent()
    -- body
    if self.m_nWinType == 1 then return end 
    if not WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_SINGLE) then return end 

    local tempCopyLevelInfo = GDatatab_single_map["id_" .. self.m_tData.pointId]
    if tempCopyLevelInfo and tempCopyLevelInfo.map_type == 1 and tempCopyLevelInfo.section >= 1 and tempCopyLevelInfo.section <= 3 then 
        local eventKey = PostPlayerEvent["event_SingleCopyWin" .. tempCopyLevelInfo.section .. "_" .. tempCopyLevelInfo.map_num]
        if eventKey then 
...
--@brief    发送单人副本打完返回副本界面事件
function WndSingleCopySettlement:_postBackToSingleCopyEvent()
    -- body
    if self.m_nWinType == 1 then return end 
    if not WBattleGlobal:getCurrent():isSingleStage(COPYTYPE_SINGLE) then return end 
    ...
    local tempCopyLevelInfo = GDatatab_single_map["id_" .. self.m_tData.pointId]
    if tempCopyLevelInfo and tempCopyLevelInfo.map_type == 1 and tempCopyLevelInfo.section >= 1 and tempCopyLevelInfo.section <= 3 then 
        local eventKey = PostPlayerEvent["event_backtoSingleCopy" .. tempCopyLevelInfo.section .. "_" .. tempCopyLevelInfo.map_num]
        if eventKey then 
            PostPlayerEvent:postEvent(eventKey)    
        end
    end
