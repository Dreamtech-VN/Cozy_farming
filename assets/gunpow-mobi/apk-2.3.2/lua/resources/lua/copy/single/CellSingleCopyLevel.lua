...
function CellSingleCopyLevel:onClickLevel(element)
    ...
    TeachGroup1:endTeachStep({1,4},{3,5},{5,13},{9,1},{9,6},{29,5},{32,3})
...
function CellSingleCopyLevel:_updateForState(nState)
    ...
    if self.m_bShowArm and (self.m_tData.map_type == 2 or self.m_tData.map_type == 3)  then
        ...
        if not conArm:getChildByTag(1145) then
            local existArmature = CheckEffectFile("ui/otherUI/ui_danrenfuben_jingyingnandu", true)
            if existArmature then 
                ...
                arm:setArmatureFile("ui/otherUI/ui_danrenfuben_jingyingnandu.xml")
                conArm:addChild(arm)
            else
                local _sIndex = "ui_danrenfuben_jingyingnandu"
                local downloadInfo = GetDownloadInfo(_sIndex, "uiEffect")
                if downloadInfo then 
                    DownloadManager:addDownloadTask(14204,downloadInfo.url,downloadInfo.md5,_sIndex,"DownloadResourceCallback", _G)
                end
            end
        end
    end
...
--@brief    红点
function CellSingleCopyLevel:setReDot(bShow)
    GetElement(self.m_root,"imgRedDot_CellSingleCopyLevel",WZUIImage):setVisible(bShow)
end
...
function CellSingleCopyLevel:_adaptLanguage_ug( )
    local txtLevelName = GetElement(self.m_root,"txtLevelName_CellSingleCoypLevel",WZUILabelTTF)
    txtLevelName:setScale(0.8)
    txtLevelName:setDimensions(GlobalMethod:CCSize(240))
    txtLevelName:setRelativePosition(GlobalMethod:ccp(0.5,0.6))
    local txtMonsterName = GetElement(self.m_root,"txtMonsterName_CellSingleCoypLevel",WZUILabelTTF)
    txtMonsterName:setScale(0.8)
    txtMonsterName:setDimensions(GlobalMethod:CCSize(240))
end-------------------------------------语言适配end----------------------------------------
