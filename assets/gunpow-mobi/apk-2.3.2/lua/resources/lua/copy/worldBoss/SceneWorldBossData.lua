...
function SceneWorldBoss:setEnterRoomData( mapId, bossBloodMax, bossBloodCurrent, rankPlayerId,rankPlayerName, rankHurt,  headId,faceId,sex,headColor,vipLevel,  hurt, cdTime,
    ...
    self.bossRoomInfo.headId = {}
    self.bossRoomInfo.faceId = {}
    self.bossRoomInfo.sex = {}
    self.bossRoomInfo.headColor = {}
    self.bossRoomInfo.vipLevel = {}

    self.bossRoomInfo.hurt = tonumber(hurt)                         --自己对boss造成的伤害合计值
    ...
    WZLog("SceneWorldBoss:setEnterRoomData", rankPlayerId:size())
	for i = 0 , rankPlayerName:size() - 1 do
        ...
        table.insert( self.bossRoomInfo.headId, headId:get(i))
        table.insert( self.bossRoomInfo.faceId, faceId:get(i))
        table.insert( self.bossRoomInfo.sex, sex:get(i))
        table.insert( self.bossRoomInfo.headColor, headColor:get(i))
        table.insert( self.bossRoomInfo.vipLevel, vipLevel:get(i))
    end
...
--@brief    更新人物缓存信息
function SceneWorldBoss:updatePlayerInfoData()
    WZLog("SceneWorldBoss:updatePlayerInfoData")
    self:_createPlayer()
end

--@brief    缓存推送更新物品时调用的函数
function SceneWorldBoss:updatePlayerItemData()
    WZLog("SceneWorldBoss:updatePlayerItemData")
...
function SceneWorldBoss:_initHurtRankInfo()
    ...
    for i = 1 , #self.bossRoomInfo.rankPlayerName do
        local info = {
            id = self.bossRoomInfo.rankPlayerId[i],
            name = self.bossRoomInfo.rankPlayerName[i], 
            hurt = self.bossRoomInfo.rankHurt[i],

            headId = self.bossRoomInfo.headId[i],
            faceId = self.bossRoomInfo.faceId[i],
            sex = self.bossRoomInfo.sex[i],
            headColor = self.bossRoomInfo.headColor[i],
            vipLevel = self.bossRoomInfo.vipLevel[i],
            }
...
-- 获取伤害排行数据
function SceneWorldBoss:getBossRoomInfo()
    return self.bossRoomInfo
end
