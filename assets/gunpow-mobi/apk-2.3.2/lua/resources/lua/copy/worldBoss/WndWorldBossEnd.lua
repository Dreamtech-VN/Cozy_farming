...
function WndWorldBossEnd:actionCallback()
    ...
    elseif self.m_nType == 3 then 
        --夫妻争霸Boss
        WndCoupleHegemonyEnd:showWindow(self.data)
    else
        if self.isDesc then
...
function WndWorldBossEnd:_update( )
    ...
    elseif self.m_nType == 3 then 
        ...
        elseif data.timeOver then
            sContent = string.format(sFormat1, LocalStrings.COUPLE_HEGEMONY_TEXT5)
        end
...
function WndWorldBossEnd:_adaptLanguage_ug(  )
    local txtDesc = GetElement(self.m_root,"txtDesc_WndWorldBossEnd",WZUILabelTTF)
    txtDesc:setDimensions(GlobalMethod:CCSize(360,0))
    txtDesc:setFontSize(18)
    txtDesc:setRelativePosition(GlobalMethod:ccp(0.5,0.62)) 

    GetElement(self.m_root,"txtConfirm_WndWorldBossEnd",WZUILabelTTF):setScale(0.6)
end
--------------------------------------语言适配End-------------------------------------
