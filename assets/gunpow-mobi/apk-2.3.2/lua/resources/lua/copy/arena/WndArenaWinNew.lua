...
function WndArenaWinNew:onEnter(element)
    ...
    self:_postFirstArenaResultEvent()
...
--@brief 排位赛结算信息
function WndArenaWinNew:onBtnZlsResultClick()
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
function WndArenaWinNew:_initUI()
    ...
    self.m_conZlsRankResult = GetElement(self.m_root,"conZlsRankResult_WndArenaWinNew",WZUIContainer)


    self.m_conZlsRankResult:setVisible(false)
...
function WndArenaWinNew:_initStep()
    ...
    table.insert(self.m_tStepList,{self._waitForContinueClick})
      ...
      --第三阶段
    if battleChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_PW then --排位赛
        ...
        table.insert(self.m_tStepList,{self._waitForContinueClick})
    elseif battleChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_ZLS then --战略赛
        table.insert(self.m_tStepList,{self._showZlsResultInfo})
        table.insert(self.m_tStepList,{self._updateZlsExpInfo})
        table.insert(self.m_tStepList,{self._showZlsResultBtn})
        table.insert(self.m_tStepList,{self._waitForContinueClick})
    end
  --练习赛不加
    if battleChannel ~= GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_LX and battleChannel ~= GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_ZLS then
        --通用结算
...
function WndArenaWinNew:_showRankResultInfo()
    ...
    GetElement(self.m_root,"txtZlsRankResultSkip_WndArenaWinNew",WZUILabelTTF):setVisible(false)
...
--@brief 战略赛结算
function WndArenaWinNew:_showZlsResultInfo()
    self.m_conZlsRankResult:setVisible(true)
    GetElement(self.m_root,"btnZlsRankResultSkip_WndArenaWinNew",WZUIButton):setVisible(false)
    GetElement(self.m_root,"txtZlsRankResultSkip_WndArenaWinNew",WZUILabelTTF):setVisible(false)

    --段位保护开启
    local txtZlsRankResultProtect = GetElement(self.m_root,"txtZlsRankResultProtect_WndArenaWinNew",WZUILabelTTF)
    if self.m_tData.troProtectStatus > 0 then
        txtZlsRankResultProtect:setText(LocalStrings.PVP_STRATEGIC_TEXT1[29])
    else
        txtZlsRankResultProtect:setText("")
    end

    local fightInfo = self.m_tTeammate[1]
    local rankAddlist = {}
    table.insert(rankAddlist,{name = LocalStrings.SETTLMENT_KILL,desc = fightInfo.killCount})
    table.insert(rankAddlist,{name = LocalStrings.PVP_STRATEGIC_TEXT1[20],desc = self.m_tData.continuousWinTimes})
    local myAssistNum = 0
    for i=1,#self.m_tData.playerIds do
        if self.m_tData.playerIds[i] == WBattleGlobal:getCurrent():getMyBattleId() then
            myAssistNum = self.m_tData.assistNum[i]
            break
        end
    end
    table.insert(rankAddlist,{name = LocalStrings.PVP_STRATEGIC_TEXT1[16],desc = myAssistNum})
    local strAddScore = "0"
    if self.m_tData.addRankScore > 0 then
        strAddScore = "+"..self.m_tData.addRankScore
    elseif self.m_tData.addRankScore < 0 then
        strAddScore = self.m_tData.addRankScore
    end
    table.insert(rankAddlist,{name = LocalStrings.PVP_STRATEGIC_TEXT1[9],desc = strAddScore})
    for i = 1,#rankAddlist do
        local txtZlsRankRecord = GetElement(self.m_root,"txtZlsRankRecord"..i.."_WndArenaWinNew",WZUILabelTTF)
        local txtZlsRankValue = GetElement(self.m_root,"txtZlsRankValue"..i.."_WndArenaWinNew",WZUILabelTTF)
        txtZlsRankRecord:setText(rankAddlist[i].name)
        txtZlsRankValue:setText(rankAddlist[i].desc)
    end

    --进度条变量
    self.m_nCurProtectNum = self.m_tData.troProtectStatus
    self.m_nCurRankExp = self.m_tData.preRankScore
    self.m_nCurRankLv = self.m_tData.preRankLevel
    self.m_nAddRankExp = self.m_tData.addRankScore

    self.m_nAddRankExpSpeed = self.m_nAddRankExp/60
    self:updateZlsRankLevel()
...
--@brief 战略赛等级
function WndArenaWinNew:updateZlsRankLevel()
    local curInfo = GetZlsPvpDataByLevel(self.m_nCurRankLv)

    self.m_nRankLvExpMax = curInfo.level_up_score

    local txtZlsRankName = GetElement(self.m_root,"txtZlsRankName_WndArenaWinNew",WZUILabelTTF)
    txtZlsRankName:setText(curInfo.name)
    local spinePath = "ui/otherUI/" .. curInfo.animation
    local existSpine = CheckEffectFile(spinePath)
    if existSpine then 
        local spineZlsRankIcon = GetElement(self.m_root,"spineZlsRankIcon_WndArenaWinNew",WZUISpine)
        spineZlsRankIcon:setFileJson(spinePath .. ".json")
        spineZlsRankIcon:setFileAtlas(spinePath .. ".atlas")
        spineZlsRankIcon:play(curInfo.action,true)
    else
        local imgZlsRankIcon = GetElement(self.m_root,"imgZlsRankIcon_WndArenaWinNew",WZUIImage)
        imgZlsRankIcon:setVisible(true)
        imgZlsRankIcon:setFile("ui/common/"..curInfo.icon..".png")
    end
    local formatStar = [[<I Z="0.6" P="1">ui/common/common_icon_xingxing5.png</I><T C="127,70,26" S="20" P="1">x%d</T>]]
    local ftbZlsRankStar = GetElement(self.m_root,"ftbZlsRankStar_WndArenaWinNew",WZUIFreeTextBox)
    ftbZlsRankStar:setShowText("")
    if curInfo.star then
        ftbZlsRankStar:setShowText(string.format(formatStar,curInfo.star))
    end
end

--@brief 刷新排位赛经验
function WndArenaWinNew:_updateZlsRankExpProgress(cur,max)
    local prg = GetElement(self.m_root, "proZlsRankResult_WndArenaWinNew", WZUIProgress)
    prg:setPercentage(math.min(cur*100/max, 100))
end

function WndArenaWinNew:_updateZlsExpInfo()
    if self.m_nAddRankExp > 0 then --加经验
        if self.m_nCurRankExp >= self.m_nRankLvExpMax then
            self.m_nCurRankLv = self.m_nCurRankLv + 1
            local nextLevelInfo = GetZlsPvpDataByLevel(self.m_nCurRankLv)
            self.m_nCurRankExp = self.m_nCurRankExp - nextLevelInfo.level_up_score
            self:updateZlsRankLevel()
        end
    ...
    elseif self.m_nAddRankExp < 0 then --减经验
        if self.m_nCurRankExp <= 0 then
            if self.m_nCurProtectNum > 0 then
        ...
        if self.m_nCurProtectNum > 0 and self.m_nCurRankExp <= 0 then
            self.m_nCurProtectNum = self.m_nCurProtectNum - 1
            self.m_nAddRankExp = 0
        ...
        self:_updateZlsRankExpProgress(self.m_nCurRankExp,self.m_nRankLvExpMax)
        return true
    end
    self:_updateZlsRankExpProgress(self.m_nCurRankExp,self.m_nRankLvExpMax)
    return false
...
--@brief 显示排位赛按钮
function WndArenaWinNew:_showZlsResultBtn()
    WZLog("WndArenaWinNew:_showZlsResultBtn")
    GetElement(self.m_root,"btnZlsRankResultSkip_WndArenaWinNew",WZUIButton):setVisible(true)
    GetElement(self.m_root,"txtZlsRankResultSkip_WndArenaWinNew",WZUILabelTTF):setVisible(true)
    self.m_nContinueBtnDelay = self.m_nInfoBtnDelayTime
    self.m_sContinueBtnName = "btnZlsRankResultSkip_WndArenaWinNew"
    self.m_sContinueBtnLabName = "txtZlsRankResultSkip_WndArenaWinNew"
    -- self.m_tCurContainer = self.m_conZlsRankResult
...
function WndArenaWinNew:_initRankItemView(curLv,nextLv)
    ...
    eCell3:setScale(0.6)
    if self.m_tData.troProtectStatus == 1 then 
        tCell3:setData(curLv,false,true)
    else
...
function WndArenaWinNew:_updateRankSecDes(curLv,nextLv)
    ...
    if desStr then
        ...
        if not self.m_tData.isWin and self.m_tData.troProtectStatus == 1 then
            if template.fail_level == 0 then
    ...
    --显示掉星
    if self.m_tData.troProtectStatus and self.m_tData.troProtectStatus == 1 then
        local txtProtectStatus = WZUILabelTTF:create()
        txtProtectStatus:setText(LocalStrings.COMMUNITY_TEXT5)
        txtProtectStatus:setColor(GlobalMethod:ccc3(229,105,22))
        txtProtectStatus:setFontSize(16)
        txtProtectStatus:setAnchorPoint(GlobalMethod:ccp(0.5,0))
        txtProtectStatus:setRelativePosition(GlobalMethod:ccp(0.5,1))
        labTips:addChild(txtProtectStatus)
    end
end
...
function WndArenaWinNew:_showArenaResultInfo()
    ...
    if battleChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_PW or battleChannel == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_ZLS then
...
function WndArenaWinNew:_updateArenaLabel(fightInfo)
    ...
    if WBattleGlobal:getCurrent():isAmuseFighting() then 
        GetElement(self.m_root,"labCardTitleArenaResult_WndArenaWinNew",WZUILabelTTF):setTextKey("AMUSERANK_TEXT9")
        local amuseScoreLimit = CacheCenter:getGameParam().sportPointWeekLimitEntertainment or 5000
        GetElement(self.m_root,"labCardAddArenaResult_WndArenaWinNew",WZUILabelTTF):setText(self.m_tData.addRankScore .. "/" .. amuseScoreLimit)
    end
...
function WndArenaWinNew:_updateArenaLv()
    ...
    local txtScoreTitle = GetElement(self.m_root,"txtScoreTitle_WndArenaWinNew", WZUILabelTTF)
    local imgScoreType = GetElement(self.m_root,"imgScoreType_WndArenaWinNew", WZUIImage)

    local template = GDatatab_integral["id_"..self.m_nCurArenaLv]
    if WBattleGlobal:getCurrent():isAmuseFighting() then
        template = GDatatab_entertainment_level["id_"..self.m_nCurArenaLv]
        txtScoreTitle:setTextKey("AMUSERANK_TEXT7")
        imgScoreType:setFile(GDatatab_item["id_91"].icon)
    end
    if template then
...
function WndArenaWinNew:_updateArenaExpInfo()
    ...
    if self.m_nAddArenaExp > 0 then
        ...
        if self.m_nCurArenaExp >= maxExp then
            ...
            if WBattleGlobal:getCurrent():isAmuseFighting() then
                nextTemp = GDatatab_entertainment_level["id_"..nextLv]
            end
            if nextTemp then
                --升级
...
function WndArenaWinNew:_updateArenaResultAction()
    ...
    local battleChannel = WBattleGlobal:getCurrent().m_tMakePairOk.battleChannle
    if battleChannel ~= GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_PW and battleChannel ~= GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_ZLS then
...
function WndArenaWinNew:_initActionView()
    ...
    for i = 1, #leftList do
        ...
        -- --等级
        if nBattleChannle == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_PW or nBattleChannle == GlobalGame.g_tRoomChannel.BATTLE_CHANNEL_ZLS then
...
--@brief    发送首场竞技结果事件
function WndArenaWinNew:_postFirstArenaResultEvent()
    -- body
    if WBattleGlobal:getCurrent():isFirstPvp() then 
        if self.m_tData.isWin then 
            PostPlayerEvent:postEvent(PostPlayerEvent.event_eightLvFirstWin)
            PostPlayerEvent:postEvent(PostPlayerEvent.event_eightLvEnterWinUI)
        else
            PostPlayerEvent:postEvent(PostPlayerEvent.event_eightLvFirstFail)
        end
    end
end  
----------------------------------私有方法模块End----------------------------------------
...
function WndArenaWinNew:_adaptLanguage_pt( )
    ...
    GetElement(self.m_root,"btnRankResultLab_WndArenaWinNew",WZUILabelTTF):setScale(0.7)
    GetElement(self.m_root,"btnArenaResultLab_WndArenaWinNew",WZUILabelTTF):setScale(0.7)
    GetElement(self.m_root,"btnActionResultLab_WndArenaWinNew",WZUILabelTTF):setScale(0.7)
    GetElement(self.m_root,"btnFightResultLab_WndArenaWinNew",WZUILabelTTF):setScale(0.7)

    GetElement(self.m_root, "txtBraveScore_WndArenaWinNew", WZUILabelTTF):setScale(0.8)
    GetElement(self.m_root, "labT2RankResult_WndArenaWinNew", WZUILabelTTF):setScale(0.8)
    GetElement(self.m_root, "labT3RankResult_WndArenaWinNew", WZUILabelTTF):setScale(0.8)
    GetElement(self.m_root, "labT4RankResult_WndArenaWinNew", WZUILabelTTF):setScale(0.8)
    local lab1RankResult = GetElement(self.m_root,"lab1RankResult_WndArenaWinNew",WZUILabelTTF)
    lab1RankResult:setScale(0.8)
    lab1RankResult:setRelativePosition(GlobalMethod:ccp(0.28,0.60964))
    local lab2RankResult = GetElement(self.m_root,"lab2RankResult_WndArenaWinNew",WZUILabelTTF)
    lab2RankResult:setScale(0.8)
    lab2RankResult:setRelativePosition(GlobalMethod:ccp(0.28,0.563923))
    local lab3RankResult = GetElement(self.m_root,"lab3RankResult_WndArenaWinNew",WZUILabelTTF)
    lab3RankResult:setScale(0.8)
    lab3RankResult:setRelativePosition(GlobalMethod:ccp(0.28,0.51861))
    local lab4RankResult = GetElement(self.m_root,"lab4RankResult_WndArenaWinNew",WZUILabelTTF)
    lab4RankResult:setScale(0.8)
    lab4RankResult:setRelativePosition(GlobalMethod:ccp(0.28,0.471735))
...
function WndArenaWinNew:_adaptLanguage_vn(  )
    GetElement(self.m_root,"lab1RankResult_WndArenaWinNew",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.266,0.60964))
    GetElement(self.m_root,"lab2RankResult_WndArenaWinNew",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.266,0.563923))
    GetElement(self.m_root,"lab3RankResult_WndArenaWinNew",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.266,0.51861))
    GetElement(self.m_root,"lab4RankResult_WndArenaWinNew",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.266,0.471735))
end


function WndArenaWinNew:_adaptLanguage_en(  )
    GetElement(self.m_root,"btnRankResultLab_WndArenaWinNew",WZUILabelTTF):setScale(0.8)
    GetElement(self.m_root,"btnArenaResultLab_WndArenaWinNew",WZUILabelTTF):setScale(0.8)
    GetElement(self.m_root,"btnActionResultLab_WndArenaWinNew",WZUILabelTTF):setScale(0.8)
    GetElement(self.m_root,"btnFightResultLab_WndArenaWinNew",WZUILabelTTF):setScale(0.8)

    GetElement(self.m_root,"lab1RankResult_WndArenaWinNew",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.265,0.60964))
    GetElement(self.m_root,"lab2RankResult_WndArenaWinNew",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.265,0.563923))
    GetElement(self.m_root,"lab3RankResult_WndArenaWinNew",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.265,0.51861))
    GetElement(self.m_root,"lab4RankResult_WndArenaWinNew",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.265,0.471735))
end
----------------------------------语言适配Begin------------------------------------------