...
-- 排行
function WndPvpRankList:onRank1()
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--奖励
function WndPvpRankList:onRank2()
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--奖励
function WndPvpRankList:onRank3()
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
function WndPvpRankList:createMatchGoal()
    if not self.matchGoal then
        ...
        local sContent = string.format(LocalStrings.ATH_DESC20, tostring(self.m_nMyRank))
        if self.m_nMyRank == -1 then
            sContent = string.format(LocalStrings.ATH_DESC20, LocalStrings.NOT_IN_RANKLIST)
        end
...
-- 创建段位奖励
function WndPvpRankList:createMatchRank1()
...
-- 创建排名奖励
function WndPvpRankList:createMatchRank2()
...
function WndPvpRankList:setCheckBoxAndCon(tag)
    ...
    GetElement(self.m_root, "cbgCheck_WndPvpRankList", WZUICheckBoxGroup):setCheckIndex(tag - 1)

    local con1 = GetElement(self.m_root,"con1_WndPvpRankList",WZUIContainer)
    local con2 = GetElement(self.m_root,"con2_WndPvpRankList",WZUIContainer)
    if tag == 1 then
    ...
    local str = {LocalStrings.PVP_RANK_12,LocalStrings.RANK_SEGMENT_REWARD,LocalStrings.PVP_RANK_11}
