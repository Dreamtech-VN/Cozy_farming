...
function WndPvpRankKing:actionCallback(element, data)
    ...
    if self.m_nType == 99 then 
        GetElement(self.m_root,"txtTitle_WndPvpRankKing",WZUILabelTTF):setText(LocalStrings.PVP_KING_TEXT1)
    ...
    elseif self.m_nType == 98 then 
        GetElement(self.m_root,"txtTitle_WndPvpRankKing",WZUILabelTTF):setText(LocalStrings.PVP_KING_TEXT2)
...
function WndPvpRankKing:showPlayer(info)
    ...
    local conMain = GetElement(self.m_root, "conMain_WndPvpRankKing", WZUIContainer)
    ...
    if celElement ~= nil and tCell ~= nil then 
        ...
        tCell:setOtherData(self.m_nType, self, conMain)
