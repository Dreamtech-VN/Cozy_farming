...
function WndPvpSegmentReward:onClickRewardItem(luaObject, tag, data)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
    WndItemInfo:showInfo(luaObject.m_root,self.m_root,1,data,false)
end

-- 显示UI
function WndPvpSegmentReward:showWndUI(tReward, other_data)
    local wnd = WndPvpSegmentReward:createElement()
    if wnd then 
        self.m_tReward = tReward
...
function WndPvpSegmentReward:createSegmentReward()
    WZLog("WndPvpSegmentReward:createSegmentReward", type(self.m_tReward))
    local txtRankTitleName = GetElement(self.m_root,"txtRankTitleName",WZUILabelTTF)
    if self.m_tOtherData.titleName then
        txtRankTitleName:setText(self.m_tOtherData.titleName)
    else
        txtRankTitleName:setText(LocalStrings.SHOOTARROW_TEXT20)
    end
    if self.m_tReward then
        local fbSegment = GetElement(self.m_root, "fbSegment_WndPvpRankList", WZUIFreeListContainer)
        fbSegment:removeAll()
        for i = 1, #self.m_tReward do
            local cell,tcell = CellPvpRankSegment:createElement()
            fbSegment:pushBack(WZUIContainer:luaTo(cell))
            tcell:setReward(self.m_tReward[i])
        end
    end
end
-------------------------------------私有方法模块End----------------------------------------