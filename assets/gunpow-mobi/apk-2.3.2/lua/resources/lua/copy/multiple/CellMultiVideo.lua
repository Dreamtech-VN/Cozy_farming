...
function CellMultiVideo:_update()
    -- 难度
    local path = {"ui/copy/copy_icon_jd1.png","ui/copy/copy_icon_kn1.png","ui/copy/copy_icon_dy1.png","ui/copy/copy_icon_jx1.png"}
    local imgDif = GetElement(self.m_root, "imgDif_CellMultiVideo", WZUIImage)
    imgDif:setFile(path[self.data.difficulty])

    local str = {LocalStrings.BEST_VIDEO_DIF1,LocalStrings.BEST_VIDEO_DIF2,LocalStrings.BEST_VIDEO_DIF3, LocalStrings.MULCOPY_TEXT3}
...
function CellMultiVideo:_adaptLanguage_ug(  )
    local txt = GetElement(self.m_root,"txt_CellMultiVideo",WZUILabelTTF)
    txt:setRelativePosition(GlobalMethod:ccp(0.783212,0.5))
    txt:setScale(0.7)
    txt:setDimensions(GlobalMethod:CCSize(100))
    local txtFight = GetElement(self.m_root,"txtFight_CellMultiVideo",WZUILabelTTF)
    txtFight:setScale(0.7)
    txtFight:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    txtFight:setRelativePosition(GlobalMethod:ccp(0.766239,0.5))
    local txtDif = GetElement(self.m_root,"txtDif_CellMultiVideo",WZUILabelTTF)
    txtDif:setScale(0.7)
    txtDif:setDimensions(GlobalMethod:CCSize(200))
end
--------------------------------------语言适配End-------------------------------------------------------------------------