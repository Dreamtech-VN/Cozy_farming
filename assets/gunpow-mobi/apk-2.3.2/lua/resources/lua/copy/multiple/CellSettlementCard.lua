...
function CellSettlementCard:setType(nType)
    ...
    if nType == 2 then
        if g_nMyAssistState == 1 then 
            GetElement(self.m_root, "imgHelpIcon_CellSettlementCard", WZUIImage):setVisible(true)
        else
            local txtVipNeed = GetElement(self.m_root, "txtVipNeed_CellSettlementCard", WZUILabelTTF)
            txtVipNeed:setVisible(true)
        end
    elseif nType == 3 then
        if g_nMyAssistState == 1 then 
            GetElement(self.m_root, "imgHelpIcon_CellSettlementCard", WZUIImage):setVisible(true)
            ...
            WZLog("CellSettlementCard:setType", awakenFlop)
            ...
            local txtOriginCost = GetElement(self.m_root, "txtOriginCost_CellSettlementCard", WZUILabelTTF)
            txtOriginCost:setText(awakenFlop)
            if self.m_nFlopRebate < 100 then 
                ...
                local nMod = math.fmod(self.m_nFlopRebate, 10)
                local nDiscount 
                if nMod == 0 then 
                ...
                if ProjConfig.LANGUAGE=="en" then
                    nDiscount = (100 - self.m_nFlopRebate)
                end
                ...
                txtCurCost:setText(math.ceil(awakenFlop * self.m_nFlopRebate/100))
            end
        end
    else
...
function CellSettlementCard:_update()
    ...
    -- WZTempLog("self.m_nType.... CellSettlementCard:_update 00 .....: ",self.m_nType)
    -- WZTempLog("self.m_nType.... CellSettlementCard:_update 11 .....: ",Serialize(self.m_tData))
    local tItemData = GetItemLocalData(self.m_tData.flop[self.m_nType].flopId)
    WZLog("CellSettlementCard:_update qqq", self.m_nType)
    WZLog("CellSettlementCard:_update www", self.m_tData.flop[self.m_nType].flopId)
	local nCount
	...
	if tItemData and CacheCenter:getPlayerItemCountById(tItemData.id) == -1 then
...
--@brief    设置开箱特效
function CellSettlementCard:setSpineAni()
    local spineMy = GetElement(self.m_root,"spineMy_CellSettlementCard",WZUISpine)
    local spinePath = "ui/otherUI/skill_kptx_sd"
    ...
    if bIsExist then 
        spineMy:setFileJson(spinePath .. ".json")
        spineMy:setFileAtlas(spinePath .. ".atlas")
        spineMy:play("chixu", true)
    end
end
-------------------------------------私有方法模块End----------------------------------------
-------------------------------------语言适配Begin-----------------------------------------
function CellSettlementCard:_adaptLanguage_en(  )
    ...
    local txtCurCostWord = GetElement(self.m_root, "txtCurCostWord_CellSettlementCard", WZUILabelTTF)
    txtCurCostWord:setScale(0.55)
    txtCurCostWord:setDimensions(GlobalMethod:CCSize(130))
end
