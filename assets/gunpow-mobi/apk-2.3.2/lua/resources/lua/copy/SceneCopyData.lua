...
--@brief	显示副本场景
--@param	copyType 1:单人，2:组队，3:日常，4:爬塔，5：英雄塔
--@param    levelId 暂时没用
--@param    showInfo 附带信息
--@param    bTaskJump : 是否任务跳转
--@param    chapter : 章节（例如单人副本有10个章节每个章节有10个关卡。可以指定跳转到那个章节上）
--@note		调用此接口显示副本场景
-- typeTag 单人副本中跳转的类型，（普通、精英、噩梦）
--@param    nJumpIsland : 1跳转岛主挑战并新建房间 2跳转岛主挑战不新建房间(被邀请时)
function SceneCopy:showScene(copyType, levelId, showInfo,bTaskJump,chapter,bSet, typeTag, nJumpIsland)
    WZLog("SceneCopy:showScene", copyType)

    self.nJumpIsland = nJumpIsland
