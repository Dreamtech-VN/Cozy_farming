--WndCoupleHegemonyEndData.lua
--@brief	WndCoupleHegemonyEnd的数据模块
--@date		2018/07/19
--@author	Tianxiang_Xu
--@note		世界组队Boss结算界面

WndCoupleHegemonyEnd = {
	--请不要在这里定义变量
}

--@brief	定义并初始化表的成员变量
--@note		变量的定义和初始化必须在这里完成,外部无需调用该方法
function WndCoupleHegemonyEnd:_init()
...
function WndCoupleHegemonyEnd:_unInit()
	self.m_root = nil
	self.data = nil
end
...
--@note		请仅用此方法创建场景
function WndCoupleHegemonyEnd:createElement()
	if WndCoupleHegemonyEnd.m_root ~= nil then
		WindowManager:removeWindow(WndCoupleHegemonyEnd.m_root, WndCoupleHegemonyEnd, true)
	end
	local element = WZUISystem:getInstance():createElement("WndCoupleHegemonyEnd")
	assert(element, "WndCoupleHegemonyEnd create element failed!")
	self:_init()
	return element
end

function WndCoupleHegemonyEnd:showWindow(data)
    local wnd = WndCoupleHegemonyEnd:createElement()
    WindowManager:addWindow( wnd ,WndCoupleHegemonyEnd,false)

    self.data = data
    WZLog("WndCoupleHegemonyEnd:showWnd", Serialize(self.data))
    self:_update()
end
-------------------------------------公有方法模块End----------------------------------------
...
-------------------------------------私有方法模块Begin--------------------------------------
--@brief    获取玩家结算数据
function WndCoupleHegemonyEnd:_getPlayerSettlementData()
    ...
    for i = 1, #tMakePairOk.playerId do
        ...
        local conPlayer = GetElement(self.m_root, "conPlayer" .. i .. "_WndCoupleHegemonyEnd", WZUIContainer)
