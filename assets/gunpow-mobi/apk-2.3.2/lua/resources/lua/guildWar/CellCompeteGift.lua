...
function CellCompeteGift:_createGoodsIcon(tGoodsData, nGoodsIndex)
    ...
    if WndCompeteGift.m_nType == 1 then -- 公会战奖励
        local sex = CacheCenter:getPlayerInfo().sex
        tCell:setCellGoodLocalId(tGoodsData[sex+1], tGoodsData[3], 17)
    else
...
-------------------------------------语言适配begin--------------------------------------
function CellCompeteGift:_adaptLanguage_ug(  )
    GetElement(self.m_root, "labelRank", WZUILabelTTF):setScale(0.7)
end
