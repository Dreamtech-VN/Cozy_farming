...
function SceneSelectActor:_createRoleInfo(index)
    ...
    ftbNameLv:setShowText(string.format(LocalStrings.SHOP_NAME_AND_LEVEL1,info.level,""))
    local txtPlayerName = GetElement(self.m_root, "txtPlayerName_SceneSelectActor", WZUILabelTTF)
    if txtPlayerName then 
        txtPlayerName:setText(info.name)
    end
