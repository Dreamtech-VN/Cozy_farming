...
function WndMounts:_init()
    ...
    self.firstEntry = nil 
    self.mountCurPos = nil              --当前移动到的位置
    self.m_tMountTypeConfig = {}
end
...
function WndMounts:_unInit()
    ...
    self.m_nRunIndex = nil 
    self.firstEntry = nil 
    self.mountCurPos = nil
    self.m_tMountTypeConfig = nil
end
...
function WndMounts:_judgeLockState(mounts)
    ...
    elseif count == 3 then
        -- 物品够，提示红点，钻石除外
        if mounts.state == true then return false end
...
--@brief    获取坐骑灵石特殊属性加成
function WndMounts:_getMountStoneSpecialProAdd()
    -- body
    local tEquipStone = CacheCenter:getUsingMountStoneList()
    WZLog("WndMounts:_getMountStoneSpecialProAdd", Serialize(tEquipStone))
    local tPropertyAdd = {}
    local tMountType = {[5] = 1, [6] = 2, [7] = 3}
    local tMountTypeConfig = self.m_tMountTypeConfig

    local tMount = CacheCenter:getMountInfo()
    local tMountTypeNum = {0, 0, 0}
    for i, mount in pairs(tMount) do
        if mount.isHave then 
            local nType = GDatatab_mounts["id_"..mount.id].type
            tMountTypeNum[nType] = tMountTypeNum[nType] + 1
        end
    end
     WZLog("WndMounts:_getMountStoneSpecialProAdd two", Serialize(tMountTypeNum), Serialize(tMountTypeConfig))
    --
    for i, value in pairs(tEquipStone) do
        if value.extraInfo and value.extraInfo.spriteStoneEffect and value.extraInfo.spriteStoneEffect > 0 then 
            local stoneEffect = value.extraInfo.spriteStoneEffect
            local info = GDatatab_sprite_stone_effect["id_" .. stoneEffect]
            local level = value.extraInfo.strongLevel
            WZLog("WndMounts:_getMountStoneSpecialProAdd three", level, info.type, Serialize(tMountTypeConfig[info.type]))
            if tMountType[info.type] ~= nil and tMountTypeConfig[info.type] then 
                for j, val in pairs(tMountTypeConfig[info.type]) do
                    if val.id == info.id and level >= val.value[1][1] then 
                        if tMountTypeNum[tMountType[info.type]] > 0 then 
                            local tempItem = {val.value[1][2], val.value[1][3] * tMountTypeNum[tMountType[info.type]]}
                            local bExist = false 
                            for t, pro in pairs(tPropertyAdd) do
                                if tempItem[1] == pro[1] then 
                                    pro[2] = pro[2] + tempItem[2]
                                    bExist = true 
                                    break 
                                end
                            end
                            if not bExist then 
                                table.insert(tPropertyAdd, tempItem)
                            end
                        end
                    end
                end
...
--@brief    获取坐骑灵石特殊特效配置数据
function WndMounts:getMountTypeConfig()
    -- body
    self.m_tMountTypeConfig = {}

    for i, v in pairs(GDatatab_sprite_stone_effect) do
        if v.type >= 5 and v.type <= 7 then 
            if self.m_tMountTypeConfig[v.type] == nil then 
                self.m_tMountTypeConfig[v.type] = {}
            end
            table.insert(self.m_tMountTypeConfig[v.type], v)
        end
    end
end
-------------------------------------私有方法模块End----------------------------------------
