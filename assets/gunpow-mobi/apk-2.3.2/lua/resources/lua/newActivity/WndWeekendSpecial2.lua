--WndWeekendSpecial2.lua
--@brief	WndWeekendSpecial2的UI模块
--@date		2024/08/19
--@author	yrd
--@note		周末特惠 - 7182
...
--@note		在这里做场景进入前的准备工作
function WndWeekendSpecial2:onEnter(element)
	self.m_root = element
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function WndWeekendSpecial2:onExit(element)
...
--@brief    onenter函数已执行
function WndWeekendSpecial2:onEnterTransitionDidFinish(element)
	ProtocolProcessorWndActivityOnLine:send_ACTIVITY_GetActivityInfo(g_cityExtenInfo.activity7182, 7182)
end

--@brief    点击关闭窗口按钮
function WndWeekendSpecial2:showInterface()
	LoadNewActivityRes(true)
	local wnd = WndWeekendSpecial2:createElement()
	WindowManager:addWindow(wnd, WndWeekendSpecial2, nil, nil, nil, true)
end

--@brief    点击关闭窗口按钮
function WndWeekendSpecial2:onClickClose(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLOSE_WIN)
	WindowManager:removeWindow(self.m_root, self, true)
end

--@brief    显示礼包
function WndWeekendSpecial2:showGifts()
	...
	for i=1,nShowCount do
		if self.m_tFirstGiftObj[i] then
			...
			local celElement, tLuaObj = CellWeekendSpecial2:createElement()
...
--@brief    播放动画
function WndWeekendSpecial2:playAnimation()
...
--@brief    播放动画
function WndWeekendSpecial2:_scheduleAnimation1(element)
	local giftObjList = {self.m_tFirstGiftObj, self.m_tSuperGiftObj}
	local tGiftObj = giftObjList[self.m_tReceivedInfo.type]
...
--@brief    播放动画
function WndWeekendSpecial2:_scheduleAnimation2(element)
...
--@brief    显示剩余时间
function WndWeekendSpecial2:showRemainingTime(element)
...
--@brief    显示剩余时间
function WndWeekendSpecial2:_ScheduleTime(element)
	local txtLeftTime = GetElement(self.m_root,"txtLeftTime",WZUILabelTTF)
