--WndHotBasketballData.lua
--@brief	WndHotBasketball的数据模块
--@date		2023/09/01
--@author	XTX
--@note		热血篮球活动主界面
...
function WndHotBasketball:_init()
	...
	self.m_nCoinId = 160503
	self.m_nMaxLotteryCount = 20    --最大抽奖次数
	self.m_nCount = 0 
	self.m_tBallAniName = {{"H_wait", "H_failure_1", "H_failure_2", "H_win_1", "H_win_2"}, {"T_wait", "T_failure_1", "T_failure_2", "T_win_1", "T_win_2"}}
	...
	self.m_nCurExp = 0 
	self.m_nPaintedEggTimesLeft = 0
	self.m_nPaintedEggTime = -1
	self.m_nPaintedTimeLimit = 10 
end


--@brief	反初始化表的成员变量
--@note		在退出场景时回调的onExit函数里面必须调用本函数
function WndHotBasketball:_unInit()
	self.m_root = nil
	self.m_nStartTime = nil 
	self.m_nEndTime = nil 
	self.m_tContent = nil 
	...
	self.m_tLvRewardList = nil 
	self.m_nCurExp = nil 
	self.m_nPaintedEggTimesLeft = nil 
	self.m_nPaintedEggTime = nil 
	self.m_nPaintedTimeLimit = nil 
end
...
--@note		请仅用此方法创建场景
function WndHotBasketball:createElement()
	if WndHotBasketball.m_root ~= nil then
		WindowManager:removeWindow(WndHotBasketball.m_root, WndHotBasketball, true)
	end
	local element = WZUISystem:getInstance():createElement("WndHotBasketball")
	assert(element, "WndHotBasketball create element failed!")
	self:_init()
	return element
end

--@brief 	外部接口
function WndHotBasketball:showInterface()
	LoadNewActivityRes(true)
	local wndWater = WndHotBasketball:createElement()
	if wndWater then 
		WindowManager:addWindow(wndWater, WndHotBasketball, false)
	end
end

--@brief 	获取活动详情成功
function WndHotBasketball:GetActivityInfoOK(activityId,maxCount,count,status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	-- body
	WZLog("WndHotBasketball:GetActivityInfoOK", activityId)
	if g_cityExtenInfo.activity7091 == activityId then 
		...
		self.m_nChooseReward = GetOperateTimes("HOTBASKETBALLACTIVITYID", self.m_nActivityId)
		...
		for i = 1, #self.m_tContent.scoreConfig do
			...
			self.m_tLvRewardList[i].id = i - 1
			self.m_tLvRewardList[i].lv = self.m_tContent.levelConfig[i]
			self.m_tLvRewardList[i].name = LocalStrings.HOTBASKETBALL_TEXT1[17][i + 1]
...
--@brief 	获取其他活动数据
function WndHotBasketball:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then --等级奖励数据
		local tResult = json.decode(jsonData)
		WZLog("WndHotBasketball:_onGetOtherData 111", Serialize(tResult))
		self.m_nCurExp = tResult.score
		self.m_nPaintedEggTime = tResult.doubleEndTime - SystemTime:getServerTime()
		self.m_nPaintedEggTimesLeft = self.m_tContent.doubleConfig[1] - tResult.doubleNum
	...
	elseif doType == 2 then --等级奖励数据
		local tResult = json.decode(jsonData)
		WZLog("WndHotBasketball:_onGetOtherData 222", Serialize(tResult))
		...
		if tResult.pool == 0 then 
			local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.HOTBASKETBALL_TEXT1[12], listBgSize = {474,228}, listBgPos = {0.5,0.431}}
			for i = 1, #array do
		...
		elseif tResult.pool == 1 then 
			local tItem = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.HOTBASKETBALL_TEXT1[13], strAtt = LocalStrings.HOTBASKETBALL_TEXT1[16], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
		...
		elseif tResult.pool == 2 then 
			local tItem = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.HOTBASKETBALL_TEXT1[14], strAtt = LocalStrings.HOTBASKETBALL_TEXT1[16], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
	...
	elseif doType == 3 then --开启结果
		local tResult = json.decode(jsonData)
		WZLog("WndHotBasketball:_onGetOtherData 333", Serialize(tResult))
		self.m_tOpenResult = {}
		...
		if tResult.fItemIds then 
			for i = 1, #tResult.fItemIds do
				...
				tItem.playerItemId = tResult.playerItemIds[itemIdIndex]
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.HOTBASKETBALL_TEXT1[12])
		...
		if tResult.sItemIds then 
			for i = 1, #tResult.sItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.HOTBASKETBALL_TEXT1[13])
				tItem.txtTitlePt = {0.5,0.885}
		...
		if tResult.nItemIds then 
			for i = 1, #tResult.nItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.HOTBASKETBALL_TEXT1[14])
		...
		if tResult.shopNum and tResult.shopNum > 0 then 
			local tItem = {160504, tResult.shopNum}
	...
	elseif doType == 4 then --选择奖励
		local tResult = json.decode(jsonData)
		WZLog("WndHotBasketball:_onGetOtherData 444", Serialize(tResult))
		if result == 0 then 
			local tTempList = nil 
			local nTag = 0
	...
	elseif doType == 5 then --领取等级奖励
		local tResult = json.decode(jsonData)
		WZLog("WndHotBasketball:_onGetOtherData 666", Serialize(tResult))
		if result == 0 then 
			self.m_tLvRewardList[tResult.lv + 1].status = 2
...
function WndHotBasketball:updatePlayerItemData()
	WZLog("WndHotBasketball:updatePlayerItemData")
...
--@brief 	设置射箭的状态
function WndHotBasketball:setOpenState(state)
	-- body
	if self.m_root == nil then return end 
...
-------------------------------------私有方法模块Begin--------------------------------------
--@brief 	解析大奖数据
function WndHotBasketball:_analyzeBigReward()
	-- body
	...
	local tItem1 = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.HOTBASKETBALL_TEXT1[13], strAtt = LocalStrings.HOTBASKETBALL_TEXT1[16], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461} , cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31}
	for i = 1, #array1 do
	...
	local tItem2 = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.HOTBASKETBALL_TEXT1[14], strAtt = LocalStrings.HOTBASKETBALL_TEXT1[16], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31}
	for i = 1, #array2 do
...
--@brief 	关闭抽奖奖励展示界面回调
function WndHotBasketball:_afterCloseReward()
	if self.m_root == nil then return end 
...
--@brief 	获取当前捕鼠等级
function WndHotBasketball:getCurLvInfo()
	local nLevel = 0 
