--WndKickingBirdieData.lua
--@brief	WndKickingBirdie的数据模块
--@date		2023/11/02
--@author	yrd
--@note		踢毽子
...
function WndKickingBirdie:_init()
	self.m_root = nil	 	  			--场景根节点
	self.m_nCoinId = 160535				--货币
	...
	self.m_tClipAniName = {{"renwu_wait1", "pt_1", "pt_2"}, {"renwu_wait2", "gj_1", "gj_2"}}	--机器特效播放动作名
...
function WndKickingBirdie:_unInit()
	self.m_root = nil
	self.m_nCoinId = nil
...
--@note		请仅用此方法创建场景
function WndKickingBirdie:createElement()
	if WndKickingBirdie.m_root ~= nil then
		WindowManager:removeWindow(WndKickingBirdie.m_root, WndKickingBirdie, true)
	end
	local element = WZUISystem:getInstance():createElement("WndKickingBirdie")
	assert(element, "WndKickingBirdie create element failed!")
	self:_init()
	return element
end

--@brief	缓存推送更新物品时调用的函数
function WndKickingBirdie:updatePlayerItemData()
	WZLog("WndKickingBirdie:updatePlayerItemData")
...
--@brief 	获取活动详情成功
function WndKickingBirdie:GetActivityInfoOK(activityId, maxCount, count, status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if g_cityExtenInfo.activity7097 == activityId then
...
--@brief 	获取其他活动数据
function WndKickingBirdie:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then
		local tResult = json.decode(jsonData)
		WZLog("WndKickingBirdie:_onGetOtherData", doType, Serialize(tResult))
	...
	elseif doType == 2 then --获取达人奖、大奖限量数据
		...
		if tResult.pool == 0 then 
			local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.KICKING_BIRDIE_TEXT1[9], listBgSize = {474,228}, listBgPos = {0.5,0.431}}
			for i = 1, #array do
		...
		elseif tResult.pool == 1 then 
			local tItem = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.KICKING_BIRDIE_TEXT1[8], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		elseif tResult.pool == 2 then
			local tItem = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.KICKING_BIRDIE_TEXT1[7], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
	...
	elseif doType == 3 then --开启结果
		...
		if tResult.fItemIds then 
			for i = 1, #tResult.fItemIds do
				...
				tItem.playerItemId = tResult.playerItemIds[itemIdIndex]
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.KICKING_BIRDIE_TEXT1[9])
		...
		if tResult.sItemIds then 
			for i = 1, #tResult.sItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.KICKING_BIRDIE_TEXT1[8])
				tItem.txtTitlePt = {0.5,0.885}
		...
		if tResult.nItemIds then 
			for i = 1, #tResult.nItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.KICKING_BIRDIE_TEXT1[7])
		...
		if result == 1 then 
			...
			self.m_tOpenResult.addExp = tResult.shopNum
...
--@brief 	设置射箭的状态
function WndKickingBirdie:setOpenState(state)
	if self.m_root == nil then return end 
	self.m_bOpenState = state
end

--@brief    添加保存上次选择的瓶子
function WndKickingBirdie:saveToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("KICKINGBIRDIE_MARK", _KeyString)
    local curValue = self.m_nDrawToolType
    data:setStringValue("KICKINGBIRDIE_MARK", _KeyString, curValue)
    data:flush()
end

--@brief    获取上次保存的的瓶子
function WndKickingBirdie:getToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
...
function WndKickingBirdie:saveOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
...
function WndKickingBirdie:getOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "OPERATETIMES" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("KICKINGBIRDIE_MARK", _KeyString)
...
--@brief 	关闭抽奖奖励展示界面回调
function WndKickingBirdie:_afterCloseReward()
	if self.m_root == nil then return end 
