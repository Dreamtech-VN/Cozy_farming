--WndKickingBirdie.lua
--@brief	WndKickingBirdie的UI模块
--@date		2023/11/02
--@author	yrd
--@note		踢毽子
...
--@note		在这里做场景进入前的准备工作
function WndKickingBirdie:onEnter(element)
	self.m_root = element
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function WndKickingBirdie:onExit(element)
...
--@brief    onenter函数已执行
function WndKickingBirdie:onEnterTransitionDidFinish(element)
	ProtocolProcessorWndActivityOnLine:send_ACTIVITY_GetActivityInfo(g_cityExtenInfo.activity7097, 7097)
end

--@brief    点击关闭窗口按钮
function WndKickingBirdie:showInterface()
	LoadNewActivityRes(true)
	local wnd = WndKickingBirdie:createElement()
	WindowManager:addWindow(wnd, WndKickingBirdie, false)
end

--@brief    点击关闭窗口按钮
function WndKickingBirdie:onClickClose(element)
...
--@brief    点击规则按钮
function WndKickingBirdie:onClickRule(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	WndSingleMapDesc:showInterface(LocalStrings.KICKING_BIRDIE_TEXT2)
end

--@brief    初始化静态文本
function WndKickingBirdie:_initStaticText()
	self:getToolType()
	self:_showAnimal()

	GetElement(self.m_root,"txtOperateBtn1_1",WZUILabelTTF):setText(LocalStrings.KICKING_BIRDIE_TEXT1[2])
	GetElement(self.m_root,"txtOperateBtn1_2",WZUILabelTTF):setText(LocalStrings.KICKING_BIRDIE_TEXT1[2])
	GetElement(self.m_root,"txtOperateBtn2_1",WZUILabelTTF):setText(LocalStrings.KICKING_BIRDIE_TEXT1[3])
	GetElement(self.m_root,"txtOperateBtn2_2",WZUILabelTTF):setText(LocalStrings.KICKING_BIRDIE_TEXT1[3])
	GetElement(self.m_root,"txtOperateBtn3_1",WZUILabelTTF):setText(LocalStrings.KICKING_BIRDIE_TEXT1[4])
	GetElement(self.m_root,"txtOperateBtn3_2",WZUILabelTTF):setText(LocalStrings.KICKING_BIRDIE_TEXT1[4])
	GetElement(self.m_root,"txtOperateBtn4",WZUILabelTTF):setText(LocalStrings.KICKING_BIRDIE_TEXT1[5])
	GetElement(self.m_root,"txtOperateBtn5",WZUILabelTTF):setText(LocalStrings.KICKING_BIRDIE_TEXT1[6])
end

--@brief 	更新许愿币的数量
function WndKickingBirdie:_updateCoinNum()
...
--@brief 	初始化活动时间
function WndKickingBirdie:_initActivityTime()
...
--@brief 	红点
function WndKickingBirdie:showRedDot()
	if self.m_root == nil then return end 

	local imgBtnRedDot1 = GetElement(self.m_root, "imgBtnRedDot1", WZUIImage)
	if GlobalGame.g_tRedPointTypeList and (GlobalGame.g_tRedPointTypeList[217097] or GlobalGame.g_tRedPointTypeList[227097]) then 
	...
	if GlobalGame.g_tRedPointTypeList and GlobalGame.g_tRedPointTypeList[237097] then 
...
--@brief 	刷新"全民许愿"礼包的信息
function WndKickingBirdie:showBagGiftInfo()
...
--@brief 	点击选择瓶子按钮回调
function WndKickingBirdie:onClickChooseTool(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--@brief 	更新许愿按钮
function WndKickingBirdie:updateWishingBtn()
	for i=1, 2 do
		...
		if freeCount == 1 then
			if i == 1 then
				txtUseTool:setText(LocalStrings.KICKING_BIRDIE_TEXT1[10])
			elseif i == 2 then
				if nTempTimes == 0 then
					txtUseTool:setText(string.format(LocalStrings.KICKING_BIRDIE_TEXT1[11], self.m_tDrawNumList[i]))
				else
					txtUseTool:setText(string.format(LocalStrings.KICKING_BIRDIE_TEXT1[11], nTimes))
				end
			end
		else
			txtUseTool:setText(string.format(LocalStrings.KICKING_BIRDIE_TEXT1[11], nTimes))
...
--@brief 	设置待机特效
function WndKickingBirdie:_showAnimal()
	local spinePath = "activity/hd_tijianzi"
	...
	if existSpine then
		...
		if spineBG2 then 
			...
			spineBG2:play("beijing_yun", true)
		end
		...
		local _sIndex = "hd_tijianzi"
		local downloadInfo = GetDownloadInfo(_sIndex, "activityEffect")
		if downloadInfo then 
			DownloadManager:addDownloadTask(7097, downloadInfo.url, downloadInfo.md5, _sIndex, "downloadEffectCallback", WndKickingBirdie)
		end
	end
end

function WndKickingBirdie:downloadEffectCallback(taskId,extraData,failed)
    WZLog("WndKickingBirdie:downloadEffectCallback",taskId,extraData,failed)
    self:_showAnimal()
end

--@brief 	设置播放的保龄球的动画
--@param 	aniIndex:1待机；2击球
function WndKickingBirdie:_playAni(aniIndex, bLoop)
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
...
--@brief 	设置播放的保龄球的动画
--@param 	aniIndex:1待机；2击球
function WndKickingBirdie:_playAnotherAni(aniIndex, bLoop)
	local spineCopy = GetElement(self.m_root, "spineCopy", WZUISpine)
	aniIndex = aniIndex or 0
	if aniIndex == 0 then
...
--@brief 	显示开启动画
function WndKickingBirdie:showOpenAction()
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
	local spinePath1 = "activity/hd_tijianzi"
...
function WndKickingBirdie:showShootBefore()
	self:_playAni(0)
	local nSeconds = 2
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
...
--@brief 	显示开启奖励
function WndKickingBirdie:showShootReward()
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
	spineOpen:disableSchedule()
	self:_playAni(1, true)
	self:_playAnotherAni(0)
	...
	if self.m_tOpenResult.addScore and self.m_tOpenResult.addScore > 0 then 
		strContent = strContent .. LocalStrings.KICKING_BIRDIE_TEXT1[17] .. "+" .. self.m_tOpenResult.addScore .. "    "  
	end
	if self.m_tOpenResult.addExp and self.m_tOpenResult.addExp > 0 then 
		strContent = strContent .. LocalStrings.KICKING_BIRDIE_TEXT1[14] .. "+" .. self.m_tOpenResult.addExp .. "    "  
	end
	if strContent ~= "" then 
...
--@brief 	点击"自选奖励"按钮回调
function WndKickingBirdie:onClickChoosePrize(element)
...
--@brief 	点击许愿按钮回调
function WndKickingBirdie:onClickUseTool(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--@brief 	前往小推车购买
function WndKickingBirdie:goToBuy(nId, nResType)
...
--@brief 	点击按钮回调
function WndKickingBirdie:onClickOperateBtn(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	local nTag = element:getTag()
	if nTag == 1 then
		CellNewYearTask:showInterface(44, self.m_nActivityId)
	elseif nTag == 2 then
		WndShopRank:showInterface(63, self.m_nActivityId) 
	elseif nTag == 3 then
		WndShopRank:showInterface(62, self.m_nActivityId)
	elseif nTag == 4 then
		if self.m_nGiftRewardNum >= 1 then
			...
			local tData = {}
			tData.txtTitle = string.format(LocalStrings.KICKING_BIRDIE_TEXT1[15], self.m_tContent.globalConfig[1])
			tData.nType = 2
			WndTips:show(element, self.m_root, 52, tData, GlobalMethod:ccp(70,80), true)
		end
	elseif nTag == 5 then
		WndDollMachineShop:showInterface(14, self.m_nActivityId)
	end
end
...
--@brief 	iphoneX适配
function WndKickingBirdie:_adaptIphoneX()
	if IsIphoneX() then
		GetElement(self.m_root, "conBtns", WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.93,0.379))
...
function WndKickingBirdie:_adaptLanguage_vn(  )
	GetElement(self.m_root,"txtUseTool1",WZUILabelTTF):setScale(0.7)
	GetElement(self.m_root,"txtUseTool2",WZUILabelTTF):setScale(0.7)
	GetElement(self.m_root,"txtOperateBtn1_1",WZUILabelTTF):setScale(0.7)
	GetElement(self.m_root,"txtOperateBtn1_2",WZUILabelTTF):setScale(0.7)
	GetElement(self.m_root,"txtOperateBtn2_1",WZUILabelTTF):setScale(0.7)
	GetElement(self.m_root,"txtOperateBtn2_2",WZUILabelTTF):setScale(0.7)
end
