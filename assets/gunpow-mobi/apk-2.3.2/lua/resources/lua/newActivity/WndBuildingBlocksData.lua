--WndBuildingBlocksData.lua
--@brief	WndBuildingBlocks的数据模块
--@date		2023/12/07
--@author	yrd
--@note		拼装积木
...
function WndBuildingBlocks:_init()
	self.m_root = nil	 	  			--场景根节点
	self.m_nCoinId = 160553				--货币
...
function WndBuildingBlocks:_unInit()
	self.m_root = nil
	self.m_nCoinId = nil
...
--@note		请仅用此方法创建场景
function WndBuildingBlocks:createElement()
	if WndBuildingBlocks.m_root ~= nil then
		WindowManager:removeWindow(WndBuildingBlocks.m_root, WndBuildingBlocks, true)
	end
	local element = WZUISystem:getInstance():createElement("WndBuildingBlocks")
	assert(element, "WndBuildingBlocks create element failed!")
	self:_init()
	return element
end

--@brief	缓存推送更新物品时调用的函数
function WndBuildingBlocks:updatePlayerItemData()
	WZLog("WndBuildingBlocks:updatePlayerItemData")
...
--@brief 	获取活动详情成功
function WndBuildingBlocks:GetActivityInfoOK(activityId, maxCount, count, status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if g_cityExtenInfo.activity7104 == activityId then
		...
		for i = 1, #self.m_tContent.scoreConfig do
			...
			self.m_tLvRewardList[i].name = LocalStrings.BUILDING_BLOCKS_TEXT3[i + 1]
...
--@brief 	获取其他活动数据
function WndBuildingBlocks:_onGetOtherData(activityId, doType, result, jsonData)
	...
	elseif doType == 2 then --获取达人奖、大奖限量数据
		...
		if tResult.pool == 0 then 
			local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.BUILDING_BLOCKS_TEXT1[12], listBgSize = {474,228}, listBgPos = {0.5,0.431}}
			for i = 1, #array do
		...
		elseif tResult.pool == 1 then 
			local tItem = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.BUILDING_BLOCKS_TEXT1[11], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		elseif tResult.pool == 2 then
			local tItem = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.BUILDING_BLOCKS_TEXT1[10], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
	...
	elseif doType == 3 then --开启结果
		...
		if tResult.fItemIds then 
			for i = 1, #tResult.fItemIds do
				...
				tItem.playerItemId = tResult.playerItemIds[itemIdIndex]
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.BUILDING_BLOCKS_TEXT1[12])
		...
		if tResult.sItemIds then 
			for i = 1, #tResult.sItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.BUILDING_BLOCKS_TEXT1[11])
				tItem.txtTitlePt = {0.5,0.885}
		...
		if tResult.nItemIds then 
			for i = 1, #tResult.nItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.BUILDING_BLOCKS_TEXT1[10])
...
--@brief 	设置射箭的状态
function WndBuildingBlocks:setOpenState(state)
	if self.m_root == nil then return end 
	self.m_bOpenState = state
end

--@brief    添加保存上次选择的瓶子
function WndBuildingBlocks:saveToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("BUILDINGBLOCKS_MARK", _KeyString)
    local curValue = self.m_nDrawToolType
    data:setStringValue("BUILDINGBLOCKS_MARK", _KeyString, curValue)
    data:flush()
end

--@brief    获取上次保存的的瓶子
function WndBuildingBlocks:getToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
...
function WndBuildingBlocks:saveOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
...
function WndBuildingBlocks:getOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "OPERATETIMES" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("BUILDINGBLOCKS_MARK", _KeyString)
...
--@brief 	关闭抽奖奖励展示界面回调
function WndBuildingBlocks:_afterCloseReward()
	if self.m_root == nil then return end 
...
--@brief 	获取当前捕鼠等级
function WndBuildingBlocks:getCurLvInfo()
	local nLevel = 0 
