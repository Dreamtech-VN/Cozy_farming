...
function WndFootballShooting:_init()
	self.m_root = nil	 	  			--场景根节点
	self.m_nCoinId = 160702				--货币
	...
	self.m_tClipAniName = {{"", "shemen_1", "shemen_1"}, {"", "shemen_2", "shemen_2"}}	--机器特效播放动作名

	self.m_tScoreConfig = nil 			--积分配置
	self.m_tDsTask = nil 				--定时任务

	self.m_tBaseScore = {5,3,2,2,3,5,3,2,1,1,2,3} --基础分数
	self.m_tScoreIndex = {[1]={9,10},[2]={3,4,8,11},[3]={2,5,7,12},[5]={1,6}} --分数对应的位置
	self.m_tShootIndex = {} --要射门的位置
end


--@brief	反初始化表的成员变量
--@note		在退出场景时回调的onExit函数里面必须调用本函数
function WndFootballShooting:_unInit()
	self.m_root = nil
	self.m_nCoinId = nil
	...
	self.m_tBaseScore = nil
	self.m_tScoreIndex = nil
	self.m_tShootIndex = nil
end
...
--@note		请仅用此方法创建场景
function WndFootballShooting:createElement()
	if WndFootballShooting.m_root ~= nil then
		WindowManager:removeWindow(WndFootballShooting.m_root, WndFootballShooting, true)
	end
	local element = WZUISystem:getInstance():createElement("WndFootballShooting")
	assert(element, "WndFootballShooting create element failed!")
	self:_init()
	return element
end

--@brief	缓存推送更新物品时调用的函数
function WndFootballShooting:updatePlayerItemData()
	WZLog("WndFootballShooting:updatePlayerItemData")
...
--@brief 	获取活动详情成功
function WndFootballShooting:GetActivityInfoOK(activityId, maxCount, count, status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if g_cityExtenInfo.activity7133 == activityId then
		...
		self.m_nChooseReward = GetOperateTimes("OPERATETIMES_FOOTBALLSHOOTING", self.m_nActivityId)
...
--@brief 	获取其他活动数据
function WndFootballShooting:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then --获取达人奖、大奖限量数据
		local tResult = json.decode(jsonData)
		WZLog("WndFootballShooting:_onGetOtherData", doType, Serialize(tResult))
		...
		--每日任务
		if tResult.dsTaskId then
			self.m_tDsTask = {}
			self.m_tDsTask.dsTaskId = tResult.dsTaskId
			self.m_tDsTask.dsTaskProgress = tResult.dsTaskProgress
			self.m_tDsTask.dsTaskTarget = tResult.dsTaskTarget
			self.m_tDsTask.dsTaskStatus = tResult.dsTaskStatus
			self.m_tDsTask.dsTaskStartTime = tResult.dsTaskStartTime
			self.m_tDsTask.dsTaskEndTime = tResult.dsTaskEndTime
		end
		self:showDsTask()
	...
	elseif doType == 2 then --获取达人奖、大奖限量数据
		...
		if tResult.pool == 0 then 
			local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.FOOTBALL_SHOOT_TEXT1[7], listBgSize = {476,346}, listBgPos = {0.5,0.445}, listSize = {460,330}, listPos = {0.5,0.445}, cellElementHeight = 0.27}
			for i = 1, #array do
		...
		elseif tResult.pool == 1 then 
			local tItem = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.FOOTBALL_SHOOT_TEXT1[8], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {476,206}, listSize = {460,186}, listPos = {0.5,0.616}, listBgPos = {0.5,0.616}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		elseif tResult.pool == 2 then
			local tItem = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.FOOTBALL_SHOOT_TEXT1[9], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {476,206}, listSize = {460,186}, listPos = {0.5,0.616}, listBgPos = {0.5,0.616}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		if self.m_bIsOpenReward and self.m_tGetTimes and #self.m_tGetTimes == 3 then 
			...
			otherData.chooseInfo = {strKey="FOOTBALL_SHOOT_TEXT1", wordIndex=7, doType=4}
	...
	elseif doType == 3 then --开启结果
		...
		self.m_tOpenResult.extraRewards = {}
		...
		--额外奖
		strTitleFormat = [[<T C="255,255,255" S="46" P="1" SC="222,78,0" SS="4" SE="1">%s</T>]]
		if tResult.mItemIds and #tResult.mItemIds > 0 then
			self.m_bDoubling = true
			for i = 1, #tResult.mItemIds do
				...
				tItem.titlePt = GlobalMethod:ccp(0.5, 1)
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.FOOTBALL_SHOOT_TEXT1[7])
				table.insert(self.m_tOpenResult.extraRewards, tItem)
				itemIdIndex = itemIdIndex + 1
			end
		end
		--大奖
		...
		if tResult.sItemIds then 
			for i = 1, #tResult.sItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.FOOTBALL_SHOOT_TEXT1[8])
				tItem.txtTitlePt = {0.5,0.885}
		...
		if tResult.nItemIds then 
			for i = 1, #tResult.nItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.FOOTBALL_SHOOT_TEXT1[9])
		...
		if result == 1 then 
			...
			for i=1,#tResult.giftScore do
				local num = tResult.giftScore[i] / ratio
				table.insert(self.m_tGiftScore, num)
			end
	...
	elseif doType == 6 then --领取积分进度礼包
		...
		if result == 0 then
			...
			local imgScoreBox = GetElement(self.m_root, "imgScoreBox" .. (tResult.id + 1), WZUIImage)
			local spineScoreBox = GetElement(self.m_root, "spineScoreBox" .. (tResult.id + 1), WZUISpine)
			local imgScoreYlq = GetElement(self.m_root, "imgScoreYlq" .. (tResult.id + 1), WZUIImage)
			if tResult.status == -1 then
...
--@brief 	设置射箭的状态
function WndFootballShooting:setOpenState(state)
	if self.m_root == nil then return end 
...
--@brief 	关闭抽奖奖励展示界面回调
function WndFootballShooting:_afterCloseReward()
	if self.m_root == nil then return end 
	...
	local tExtraReward = {}
	if self.m_tOpenResult.extraRewards and #self.m_tOpenResult.extraRewards > 0 then 
		table.insert(tExtraReward, self.m_tOpenResult.extraRewards)
	end

	if self.m_tOpenResult.normalRewards and #self.m_tOpenResult.normalRewards > 0 then 
		WndHoraryBigReward:showInterface(8, self.m_tOpenResult.normalRewards, tBigReward, tExtraReward)
	elseif self.m_tOpenResult.extraRewards and #self.m_tOpenResult.extraRewards > 0 then 
		WndHoraryBigReward:showInterface(8, self.m_tOpenResult.extraRewards, tBigReward)
...
--@brief 	领取任务奖励结果
function WndFootballShooting:_onGetTaskResult(activityId,taskId)
	if self.m_tDsTask and self.m_tDsTask.dsTaskId == taskId then
		self.m_tDsTask.dsTaskStatus = 1
		self:showDsTask()
	end
end
-------------------------------------公有方法模块End----------------------------------------
