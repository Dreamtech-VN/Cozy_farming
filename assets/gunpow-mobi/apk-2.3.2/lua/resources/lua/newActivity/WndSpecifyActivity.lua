...
function WndSpecifyActivity:_setTitle()
    -- body
    local txtTitle = GetElement(self.m_root, "txtTitle_WndSpecifyActivity", WZUILabelTTF)
    txtTitle:setTextKey("SPECIFY_ACTIVITY_TEXT")
...
function WndSpecifyActivity:_createLeftMenu()
    -- body
    local tconItem = GetElement(self.m_root, "tconItem_WndSpecifyActivity", WZUITableContainer)
    tconItem:cleanTable()
    self.m_tLeftCell = {}
    ...
    for i = 1, #self.m_tListItem do
        ...
        local element, tCell = CellSevenDayItem:createElement()
        if element and tCell then 
            ...
            tCell:setTitle(self.m_tListItem[i].name, self.m_tListItem[i].item_id)
            tCell:setCallBack(self, self.onClickLeftMenu)
            element = WZUIContainer:luaTo(element)
            element:setTag(i-1)
            tconItem:setCellElement(element)
            table.insert(self.m_tLeftCell, tCell)
        end
    end
    tconItem:getMoveElement():setPositionY(tconItem:getMinPosition().y)
    --
    self:_setLightVisible()
end
...
function WndSpecifyActivity:_setLightVisible()
    ...
    if self.m_tLeftCell then
        for i = 1, #self.m_tLeftCell do
            if self.m_tLeftCell[i]:getId() == self.m_nCurItemId then
                self.m_tLeftCell[i]:isItemHighLighted(true)
            else
                self.m_tLeftCell[i]:isItemHighLighted(false)
            end
        end
    end
end
