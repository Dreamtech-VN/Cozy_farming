--WndPotionsRefining.lua
--@brief	WndPotionsRefining的UI模块
--@date		2023/12/29
--@author	yrd
--@note		魔药炼制活动
...
--@note		在这里做场景进入前的准备工作
function WndPotionsRefining:onEnter(element)
	self.m_root = element
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function WndPotionsRefining:onExit(element)
...
--@brief    onenter函数已执行
function WndPotionsRefining:onEnterTransitionDidFinish(element)
	ProtocolProcessorWndActivityOnLine:send_ACTIVITY_GetActivityInfo(g_cityExtenInfo.activity7110, 7110)
end

--@brief    点击关闭窗口按钮
function WndPotionsRefining:showInterface()
	LoadNewActivityRes(true)
	local wnd = WndPotionsRefining:createElement()
	WindowManager:addWindow(wnd, WndPotionsRefining, false)
end

--@brief    点击关闭窗口按钮
function WndPotionsRefining:onClickClose(element)
...
--@brief    点击规则按钮
function WndPotionsRefining:onClickRule(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	WndSingleMapDesc:showInterface(LocalStrings.POTIONS_REFININ_TEXT2)
end

--@brief    初始化静态文本
function WndPotionsRefining:_initStaticText()
	self:getToolType()
	self:_showAnimal()
	...
	GetElement(self.m_root,"txtOperateBtn1",WZUILabelTTF):setText(LocalStrings.POTIONS_REFININ_TEXT1[4])
	GetElement(self.m_root,"txtOperateBtn2",WZUILabelTTF):setText(LocalStrings.POTIONS_REFININ_TEXT1[5])
	GetElement(self.m_root,"txtOperateBtn3",WZUILabelTTF):setText(LocalStrings.POTIONS_REFININ_TEXT1[6])
end

--@brief 	更新许愿币的数量
function WndPotionsRefining:_updateCoinNum()
...
--@brief 	初始化活动时间
function WndPotionsRefining:_initActivityTime()
...
--@brief 	红点
function WndPotionsRefining:showRedDot()
	if self.m_root == nil then return end 

	local imgBtnRedDot2 = GetElement(self.m_root, "imgBtnRedDot2", WZUIImage)
	if GlobalGame.g_tRedPointTypeList and (GlobalGame.g_tRedPointTypeList[217110] or GlobalGame.g_tRedPointTypeList[227110] or GlobalGame.g_tRedPointTypeList[237110]) then 
...
--@brief 	更新界面
function WndPotionsRefining:updateUI()

end

--@brief 	点击选择瓶子按钮回调
function WndPotionsRefining:onClickChooseTool(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--@brief 	更新许愿按钮
function WndPotionsRefining:updateWishingBtn()
	for i=1, 2 do
		...
		if freeCount == 1 then
			if i == 1 then
				txtUseTool:setText(LocalStrings.POTIONS_REFININ_TEXT1[2])
			elseif i == 2 then
				if nTempTimes == 0 then
					txtUseTool:setText(string.format(LocalStrings.POTIONS_REFININ_TEXT1[3], self.m_tDrawNumList[i]))
				else
					txtUseTool:setText(string.format(LocalStrings.POTIONS_REFININ_TEXT1[3], nTimes))
				end
			end
		else
			txtUseTool:setText(string.format(LocalStrings.POTIONS_REFININ_TEXT1[3], nTimes))
...
--@brief 	设置待机特效
function WndPotionsRefining:_showAnimal()
	local spinePath = "activity/hd_pic_mylz"
	...
	if existSpine then
		...
		local _sIndex = "hd_pic_mylz"
		local downloadInfo = GetDownloadInfo(_sIndex, "activityEffect")
		if downloadInfo then 
			DownloadManager:addDownloadTask(7110, downloadInfo.url, downloadInfo.md5, _sIndex, "downloadEffectCallback", WndPotionsRefining)
		end
	end
...
function WndPotionsRefining:downloadEffectCallback(taskId,extraData,failed)
    WZLog("WndPotionsRefining:downloadEffectCallback",taskId,extraData,failed)
    self:_showAnimal()
end

--@brief 	设置播放的保龄球的动画
--@param 	aniIndex:1待机；2击球
function WndPotionsRefining:_playAni(aniIndex, bLoop)
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
...
--@brief 	设置播放的保龄球的动画
--@param 	aniIndex:1待机；2击球
function WndPotionsRefining:_playAnotherAni(aniIndex, bLoop)
	local spineCopy = GetElement(self.m_root, "spineCopy", WZUISpine)
	aniIndex = aniIndex or 0
	if aniIndex == 0 then
...
--@brief 	显示开启动画
function WndPotionsRefining:showOpenAction()
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
	local spinePath1 = "activity/hd_pic_mylz"
...
function WndPotionsRefining:showShootBefore()
	self:_playAni(0)
	local nSeconds = 2
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
...
--@brief 	显示开启奖励
function WndPotionsRefining:showShootReward()
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
	spineOpen:disableSchedule()
	self:_playAni(1, true)
	self:_playAnotherAni(0)
	...
	if self.m_tOpenResult.fRewardNum and self.m_tOpenResult.fRewardNum > 0 then 
		strContent = strContent .. LocalStrings.POTIONS_REFININ_TEXT1[11] .. "+" .. self.m_tOpenResult.fRewardNum .. "    "
	end
	if self.m_tOpenResult.sRewardNum and self.m_tOpenResult.sRewardNum > 0 then 
		strContent = strContent .. LocalStrings.POTIONS_REFININ_TEXT1[10] .. "+" .. self.m_tOpenResult.sRewardNum .. "    "
	end
...
--@brief 	点击"自选奖励"按钮回调
function WndPotionsRefining:onClickChoosePrize(element)
...
--@brief 	点击许愿按钮回调
function WndPotionsRefining:onClickUseTool(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--@brief 	前往小推车购买
function WndPotionsRefining:goToBuy(nId, nResType)
...
--@brief 	点击按钮回调
function WndPotionsRefining:onClickOperateBtn(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	local nTag = element:getTag()
	if nTag == 1 then
		WndHouseInvite:showInterface(14, self.m_nActivityId, 1)
	elseif nTag == 2 then
		local otherData = {}
		otherData.taskCount = 2 --几个任务标签
		otherData.tTaskTypeName = {LocalStrings.POTIONS_REFININ_TEXT1[12], LocalStrings.POTIONS_REFININ_TEXT1[13]} --任务标签名字
		otherData.taskTitle = LocalStrings.POTIONS_REFININ_TEXT1[5]
		otherData.taskType = 2
		otherData.redPoint = {117110, 127110} --长线；日常；每天
		CellNewYearTask:showInterface(60, self.m_nActivityId, otherData)
	elseif nTag == 3 then
		local otherData = {}
		otherData.type = 1
		otherData.strRankTitleName = LocalStrings.POTIONS_REFININ_TEXT1[6]
		otherData.strChangeTitle = LocalStrings.POTIONS_REFININ_TEXT1[14]
		otherData.strScoreTitle = LocalStrings.POTIONS_REFININ_TEXT1[15] .. ":"
		WndShopRank:showInterface(90, self.m_nActivityId, nil, nil, otherData) 
	end
end

--@brief 	成熟度
function WndPotionsRefining:_showProgress()
...
--@brief 	设置步数积分宝箱数量
function WndPotionsRefining:_showStepScoreNum()
	for i = 1, 6 do
...
--@brief 	点击积分宝箱回调
function WndPotionsRefining:onClickScoreBox(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--@brief 	iphoneX适配
function WndPotionsRefining:_adaptIphoneX()
	if IsIphoneX() then
...
function WndPotionsRefining:_adaptLanguage_vn()
