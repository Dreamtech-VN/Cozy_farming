--WndBoatingLakeData.lua
--@brief	WndBoatingLake的数据模块
--@date		2023/12/29
--@author	yrd
--@note		泛舟游湖活动
...
function WndBoatingLake:_init()
	self.m_root = nil	 	  			--场景根节点
	self.m_nCoinId = 160564				--货币
	...
	self.m_tClipAniName = {{"xc_wait", "xc_erhu2", "xc_erhu1"}, {"dc_wait", "dc_erhu2", "dc_erhu1"}}	--机器特效播放动作名
...
function WndBoatingLake:_unInit()
	self.m_root = nil
	self.m_nCoinId = nil
...
--@note		请仅用此方法创建场景
function WndBoatingLake:createElement()
	if WndBoatingLake.m_root ~= nil then
		WindowManager:removeWindow(WndBoatingLake.m_root, WndBoatingLake, true)
	end
	local element = WZUISystem:getInstance():createElement("WndBoatingLake")
	assert(element, "WndBoatingLake create element failed!")
	self:_init()
	return element
end

--@brief	缓存推送更新物品时调用的函数
function WndBoatingLake:updatePlayerItemData()
	WZLog("WndBoatingLake:updatePlayerItemData")
...
--@brief 	获取活动详情成功
function WndBoatingLake:GetActivityInfoOK(activityId, maxCount, count, status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if g_cityExtenInfo.activity7106 == activityId then
...
--@brief 	获取其他活动数据
function WndBoatingLake:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then --获取达人奖、大奖限量数据
		local tResult = json.decode(jsonData)
		WZLog("WndBoatingLake:_onGetOtherData", doType, Serialize(tResult))
	...
	elseif doType == 2 then --获取达人奖、大奖限量数据
		...
		if tResult.pool == 0 then 
			local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.BOATING_LAKE_TEXT1[9], listBgSize = {474,228}, listBgPos = {0.5,0.431}}
			for i = 1, #array do
		...
		elseif tResult.pool == 1 then 
			local tItem = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.BOATING_LAKE_TEXT1[8], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		elseif tResult.pool == 2 then
			local tItem = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.BOATING_LAKE_TEXT1[7], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
	...
	elseif doType == 3 then --开启结果
		...
		if tResult.fItemIds then 
			for i = 1, #tResult.fItemIds do
				...
				tItem.playerItemId = tResult.playerItemIds[itemIdIndex]
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.BOATING_LAKE_TEXT1[9])
		...
		if tResult.sItemIds then 
			for i = 1, #tResult.sItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.BOATING_LAKE_TEXT1[8])
				tItem.txtTitlePt = {0.5,0.885}
		...
		if tResult.nItemIds then 
			for i = 1, #tResult.nItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.BOATING_LAKE_TEXT1[7])
...
--@brief 	设置射箭的状态
function WndBoatingLake:setOpenState(state)
	if self.m_root == nil then return end 
	self.m_bOpenState = state
end

--@brief    添加保存上次选择的瓶子
function WndBoatingLake:saveToolType()
    local _KeyString = ""
...
--@brief    获取上次保存的的瓶子
function WndBoatingLake:getToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
...
function WndBoatingLake:saveOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
...
function WndBoatingLake:getOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "OPERATETIMES" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("PIANIST_MARK", _KeyString)
...
--@brief 	关闭抽奖奖励展示界面回调
function WndBoatingLake:_afterCloseReward()
	if self.m_root == nil then return end 
