--WndPotionsRefiningData.lua
--@brief	WndPotionsRefining的数据模块
--@date		2023/12/29
--@author	yrd
--@note		魔药炼制活动
...
function WndPotionsRefining:_init()
	self.m_root = nil	 	  			--场景根节点
	self.m_nCoinId = 160581				--货币
	...
	self.m_tClipAniName = {{"wait1_3", "wait3_1", "wait3_2"}, {"wait1_2", "wait2_1", "wait2_2"}}	--机器特效播放动作名
...
function WndPotionsRefining:_unInit()
	self.m_root = nil
	self.m_nCoinId = nil
...
--@note		请仅用此方法创建场景
function WndPotionsRefining:createElement()
	if WndPotionsRefining.m_root ~= nil then
		WindowManager:removeWindow(WndPotionsRefining.m_root, WndPotionsRefining, true)
	end
	local element = WZUISystem:getInstance():createElement("WndPotionsRefining")
	assert(element, "WndPotionsRefining create element failed!")
	self:_init()
	return element
end

--@brief	缓存推送更新物品时调用的函数
function WndPotionsRefining:updatePlayerItemData()
	WZLog("WndPotionsRefining:updatePlayerItemData")
...
--@brief 	获取活动详情成功
function WndPotionsRefining:GetActivityInfoOK(activityId, maxCount, count, status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if g_cityExtenInfo.activity7110 == activityId then
...
--@brief 	获取其他活动数据
function WndPotionsRefining:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then --获取达人奖、大奖限量数据
		local tResult = json.decode(jsonData)
		WZLog("WndPotionsRefining:_onGetOtherData", doType, Serialize(tResult))
	...
	elseif doType == 2 then --获取达人奖、大奖限量数据
		local tResult = json.decode(jsonData)
		WZLog("WndBuildingBlocks:_onGetOtherData", doType, Serialize(tResult))
		if tResult.pool == 0 or tResult.pool == 1 or tResult.pool == 2 then
			table.insert(self.m_tGetTimes, tResult.pool)
		-- elseif tResult.pool == 3 or tResult.pool == 4 then
		...
		if tResult.pool == 0 then 
			local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.POTIONS_REFININ_TEXT1[11], listBgSize = {474,228}, listBgPos = {0.5,0.431}}
			for i = 1, #array do
		...
		elseif tResult.pool == 1 then 
			local tItem = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.POTIONS_REFININ_TEXT1[10], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		elseif tResult.pool == 2 then
			local tItem = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.POTIONS_REFININ_TEXT1[9], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		-- elseif self.m_bIsOpenReward and self.m_tGetTimes2 and #self.m_tGetTimes2 == 2 then 
		-- 	local otherData = {}
		-- 	otherData.activityId = self.m_nActivityId
	...
	elseif doType == 3 then --开启结果
		...
		if tResult.fItemIds then 
			for i = 1, #tResult.fItemIds do
				...
				tItem.playerItemId = tResult.playerItemIds[itemIdIndex]
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.POTIONS_REFININ_TEXT1[11])
		...
		if tResult.sItemIds then 
			for i = 1, #tResult.sItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.POTIONS_REFININ_TEXT1[10])
				tItem.txtTitlePt = {0.5,0.885}
		...
		if tResult.nItemIds then 
			for i = 1, #tResult.nItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.POTIONS_REFININ_TEXT1[9])
		...
		if result == 1 then 
			...
			self.m_tOpenResult.fRewardNum = tResult.fRewardNum
			self.m_tOpenResult.sRewardNum = tResult.sRewardNum
	...
	elseif doType == 4 then --选择奖励
		local tResult = json.decode(jsonData)
		WZLog("WndPotionsRefining:_onGetOtherData", Serialize(tResult))
		if result == 0 then 
			local tTempList = nil 
			local nTag = 0
...
--@brief 	设置射箭的状态
function WndPotionsRefining:setOpenState(state)
	if self.m_root == nil then return end 
	self.m_bOpenState = state
end

--@brief    添加保存上次选择的瓶子
function WndPotionsRefining:saveToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("POTIONSREFINING_MARK", _KeyString)
    local curValue = self.m_nDrawToolType
    data:setStringValue("POTIONSREFINING_MARK", _KeyString, curValue)
    data:flush()
end

--@brief    获取上次保存的的瓶子
function WndPotionsRefining:getToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
...
function WndPotionsRefining:saveOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
...
function WndPotionsRefining:getOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "OPERATETIMES" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("POTIONSREFINING_MARK", _KeyString)
...
--@brief 	关闭抽奖奖励展示界面回调
function WndPotionsRefining:_afterCloseReward()
	if self.m_root == nil then return end 
