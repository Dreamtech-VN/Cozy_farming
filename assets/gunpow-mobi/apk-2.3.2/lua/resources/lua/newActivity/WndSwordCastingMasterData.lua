--WndSwordCastingMasterData.lua
--@brief	WndSwordCastingMaster的数据模块
--@date		2023/12/07
--@author	yrd
--@note		铸剑神匠

WndSwordCastingMaster = {
	--请不要在这里定义变量
}

--@brief	定义并初始化表的成员变量
--@note		变量的定义和初始化必须在这里完成,外部无需调用该方法
function WndSwordCastingMaster:_init()
	self.m_root = nil	 	  			--场景根节点
	self.m_nCoinId = 160562				--货币
	...
	self.m_tClipAniName = {{"wait2_1", "wait2_2", "wait2_3"}, {"wait3_1", "wait3_2", "wait3_3"}}	--机器特效播放动作名

	self.m_nCurExp = 0
	self.m_nDkTaskRemainTime = nil 
...
function WndSwordCastingMaster:_unInit()
	self.m_root = nil
	self.m_nCoinId = nil
...
--@note		请仅用此方法创建场景
function WndSwordCastingMaster:createElement()
	if WndSwordCastingMaster.m_root ~= nil then
		WindowManager:removeWindow(WndSwordCastingMaster.m_root, WndSwordCastingMaster, true)
	end
	local element = WZUISystem:getInstance():createElement("WndSwordCastingMaster")
	assert(element, "WndSwordCastingMaster create element failed!")
	self:_init()
	return element
end

--@brief	缓存推送更新物品时调用的函数
function WndSwordCastingMaster:updatePlayerItemData()
	WZLog("WndSwordCastingMaster:updatePlayerItemData")
...
--@brief 	获取活动详情成功
function WndSwordCastingMaster:GetActivityInfoOK(activityId, maxCount, count, status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if g_cityExtenInfo.activity7105 == activityId then
		...
		-- self.m_nTips = tips
		WZLog("WndSwordCastingMaster:GetActivityInfoOK", Serialize(self.m_tContent))
...
--@brief 	获取其他活动数据
function WndSwordCastingMaster:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then --活动信息
		local tResult = json.decode(jsonData)
		WZLog("WndSwordCastingMaster:_onGetOtherData", doType, Serialize(tResult))

		self.m_nScore = tResult.score
		self:showRewardProgress()

		self.m_nDkTaskId = tResult.dkTaskId
		self.m_nDkTaskStatus = tResult.dkTaskStatus
		self.m_nDkTaskProgress = tResult.dkTaskProgress
		self.m_nDkTaskTarget = tResult.dkTaskTarget
		self.m_nDkTaskRemainTime = tResult.dkTaskRemainTime
	...
	elseif doType == 2 then --获取达人奖、大奖限量数据
		...
		if tResult.pool == 0 then 
			local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.CASTING_MASTER_ACTIVITY_TEXT1[21], listBgSize = {474,228}, listBgPos = {0.5,0.431}}
			for i = 1, #array do
		...
		elseif tResult.pool == 1 then 
			local tItem = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.CASTING_MASTER_ACTIVITY_TEXT1[20], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		elseif tResult.pool == 2 then
			local tItem = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.CASTING_MASTER_ACTIVITY_TEXT1[19], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
	...
	elseif doType == 3 then --开启结果
		...
		if tResult.fItemIds then 
			for i = 1, #tResult.fItemIds do
				...
				tItem.playerItemId = tResult.playerItemIds[itemIdIndex]
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.CASTING_MASTER_ACTIVITY_TEXT1[21])
		...
		if tResult.sItemIds then 
			for i = 1, #tResult.sItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.CASTING_MASTER_ACTIVITY_TEXT1[20])
				tItem.txtTitlePt = {0.5,0.885}
		...
		if tResult.nItemIds then 
			for i = 1, #tResult.nItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.CASTING_MASTER_ACTIVITY_TEXT1[19])
		...
		if tResult.giftItemIds and #tResult.giftItemIds > 0 then 
			for i = 1, #tResult.giftItemIds do
				local tItem = {}
				tItem.itemId = tResult.giftItemIds[i]
				tItem.itemNum = tResult.giftItemNums[i]
				tItem.playerItemId = tResult.playerItemIds[itemIdIndex]
				tItem.type = rewardType
				tItem.imgRewardTitle = "ui/newActivity/text_hd_tqq_di.png"
				tItem.titlePt = GlobalMethod:ccp(0.5, 0.982)
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.CASTING_MASTER_ACTIVITY_TEXT1[28])
		...
		if result == 1 then 
			...
			self.m_tOpenResult.giftNum = tResult.giftNum
...
--@brief 	设置射箭的状态
function WndSwordCastingMaster:setOpenState(state)
	if self.m_root == nil then return end 
	self.m_bOpenState = state
end

--@brief    添加保存上次选择的瓶子
function WndSwordCastingMaster:saveToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("SWORDCASTING_MARK", _KeyString)
    local curValue = self.m_nDrawToolType
    data:setStringValue("SWORDCASTING_MARK", _KeyString, curValue)
    data:flush()
end

--@brief    获取上次保存的的瓶子
function WndSwordCastingMaster:getToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
...
function WndSwordCastingMaster:saveOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
...
function WndSwordCastingMaster:getOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "OPERATETIMES" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("SWORDCASTING_MARK", _KeyString)
...
--@brief 	关闭抽奖奖励展示界面回调
function WndSwordCastingMaster:_afterCloseReward()
	if self.m_root == nil then return end 
...
--@brief 	射箭任务奖励
function WndSwordCastingMaster:_onGetTaskResult(activityId, id)
--	WZLog("WndSwordCastingMaster:_onGetTaskResult", self.m_nActivityId, activityId, id)
	if self.m_nActivityId ~= activityId then
		MsgBoxManager:showTipBox(LocalStrings.RETURNEE_TEXT28)
		return
	end
	
	if id == self.m_nDkTaskId then
		self.m_nDkTaskStatus = 1
		self:showTaskProgress()
	end
end
-------------------------------------公有方法模块End----------------------------------------
