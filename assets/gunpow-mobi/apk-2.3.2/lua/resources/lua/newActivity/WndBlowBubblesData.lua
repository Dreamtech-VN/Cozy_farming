--WndBlowBubblesData.lua
--@brief	WndBlowBubbles的数据模块
--@date		2023/12/29
--@author	yrd
--@note		吹泡泡活动
...
function WndBlowBubbles:_init()
	self.m_root = nil	 	  			--场景根节点
	self.m_nCoinId = 160565				--货币
...
function WndBlowBubbles:_unInit()
	self.m_root = nil
	self.m_nCoinId = nil
...
--@note		请仅用此方法创建场景
function WndBlowBubbles:createElement()
	if WndBlowBubbles.m_root ~= nil then
		WindowManager:removeWindow(WndBlowBubbles.m_root, WndBlowBubbles, true)
	end
	local element = WZUISystem:getInstance():createElement("WndBlowBubbles")
	assert(element, "WndBlowBubbles create element failed!")
	self:_init()
	return element
end

--@brief	缓存推送更新物品时调用的函数
function WndBlowBubbles:updatePlayerItemData()
	WZLog("WndBlowBubbles:updatePlayerItemData")
...
--@brief 	获取活动详情成功
function WndBlowBubbles:GetActivityInfoOK(activityId, maxCount, count, status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if g_cityExtenInfo.activity7107 == activityId then
...
--@brief 	获取其他活动数据
function WndBlowBubbles:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then --获取达人奖、大奖限量数据
		local tResult = json.decode(jsonData)
		WZLog("WndBlowBubbles:_onGetOtherData", doType, Serialize(tResult))

		--每日登录
		self.m_tLoginGiftData = {status = tResult.giftStatus, ids = tResult.giftReward[1], nums = tResult.giftReward[2]}
		self:showRedDot()
	...
	elseif doType == 2 then --获取达人奖、大奖限量数据
		...
		if tResult.pool == 0 then 
			local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.BLOW_BUBBLES_TEXT1[10], listBgSize = {474,228}, listBgPos = {0.5,0.431}}
			for i = 1, #array do
		...
		elseif tResult.pool == 1 then 
			local tItem = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.BLOW_BUBBLES_TEXT1[9], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		elseif tResult.pool == 2 then
			local tItem = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.BLOW_BUBBLES_TEXT1[8], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
	...
	elseif doType == 4 then --选择奖励
		local tResult = json.decode(jsonData)
		WZLog("WndBlowBubbles:_onGetOtherData 444", Serialize(tResult))
		if result == 0 then 
			local tTempList = nil 
			local nTag = 0
	...
	elseif doType == 8 then --每日登录奖励
		local tResult = json.decode(jsonData)
		WZLog("WndBlowBubbles:_onGetOtherData", doType, Serialize(tResult))
		if result == 0 then 
			WndRewardShow:showById(tResult.itemIds, tResult.itemNums)
			self.m_tLoginGiftData = {status = 1, ids = tResult.itemIds, nums = tResult.itemNums}
			self:showRedDot()
		end
	end
end
...
--@brief 	设置射箭的状态
function WndBlowBubbles:setOpenState(state)
	if self.m_root == nil then return end 
	self.m_bOpenState = state
end

--@brief    添加保存上次选择的瓶子
function WndBlowBubbles:saveToolType()
    local _KeyString = ""
...
--@brief    获取上次保存的的瓶子
function WndBlowBubbles:getToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
...
function WndBlowBubbles:saveOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
...
function WndBlowBubbles:getOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "OPERATETIMES" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("PIANIST_MARK", _KeyString)
...
--@brief 	关闭抽奖奖励展示界面回调
function WndBlowBubbles:_afterCloseReward()
	if self.m_root == nil then return end 
