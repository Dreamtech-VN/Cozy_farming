--WndWeekendSpecialData.lua
--@brief	WndWeekendSpecial的数据模块
--@date		2024/08/19
--@author	yrd
--@note		周末特惠
...
function WndWeekendSpecial:_init()
...
function WndWeekendSpecial:_unInit()
	self.m_root = nil
...
--@note		请仅用此方法创建场景
function WndWeekendSpecial:createElement()
	if WndWeekendSpecial.m_root ~= nil then
		WindowManager:removeWindow(WndWeekendSpecial.m_root, WndWeekendSpecial, true)
	end
	local element = WZUISystem:getInstance():createElement("WndWeekendSpecial")
	assert(element, "WndWeekendSpecial create element failed!")
	self:_init()
	return element
end

--@brief 	获取活动详情成功
function WndWeekendSpecial:GetActivityInfoOK(activityId, maxCount, count, status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if g_cityExtenInfo.activity7138 == activityId then
...
--@brief 	获取其他活动数据
function WndWeekendSpecial:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then
		local tResult = json.decode(jsonData)
		WZLog("WndWeekendSpecial:_onGetOtherData", doType, Serialize(tResult))
...
--@brief 	下订单
function WndWeekendSpecial:gotoBuy(rechargeId)
	local sdkData = {}
...
--@brief 	解锁成功回调
function WndWeekendSpecial:_onRechargeSuccessResult()
	--刷新领取状态
...
CellWeekendSpecial = {}
function CellWeekendSpecial:_init()
	self.m_root = nil	 	  			--场景根节点
...
--@brief	反初始化表的成员变量
--@note		在退出场景时回调的onExit函数里面必须调用本函数
function CellWeekendSpecial:_unInit()
	self.m_root = nil
...
--@brief	创建控件
function CellWeekendSpecial:createElement()
...
function CellWeekendSpecial:setData(data)
	self.m_tData = data
	self:updateUI()
end

--@brief 	获取数据
function CellWeekendSpecial:getData()
	return self.m_tData
end

--@brief 	开始加载
function CellWeekendSpecial:onLoadData(element)
	local celElement = WZUISystem:getInstance():createElement("CellWeekendSpecial")
...
--@brief    获得充值数据
function CellWeekendSpecial:getRechargeData()
...
--@brief    更新界面
function CellWeekendSpecial:updateUI()
...
--@brief	显示
function CellWeekendSpecial:showUI(bShow)
	self.m_root:setVisible(bShow)
end

--@brief	设置箭头指向 1朝右,2朝下,3朝左 其他不显示箭头
function CellWeekendSpecial:setArrawType(nType)
...
--@brief	点击物品弹出对应的tips
function CellWeekendSpecial:onItemClick(tCell,tag,tData)
	if tData == nil then
	   return
	end
	WndItemInfo:onCloseClick()
	WndItemInfo:showInfo(tCell.m_root,WndWeekendSpecial.m_root,1,tData,false)
end

--@brief    点击领取奖励
function CellWeekendSpecial:onClickBuy()
	...
	if self.m_tData.startIdx == self.m_tData.index then --第一列
		if self.m_tData.showType == 0 then
			if self.m_tData.status == 0 then
				--用作动画
				WndWeekendSpecial.m_tReceivedInfo = {type=self.m_tData.type,index=self.m_tData.index}

				local sjson = {}
				sjson.giftId = self.m_tData.type - 1
				sjson.id = self.m_tData.index - 1
				sjson = json.encode(sjson)
				ProtocolProcessorFestivalActivity:send_ACTIVITY2_ActivityDo(g_cityExtenInfo.activity7138, 2, sjson)
			elseif self.m_tData.status == 2 then
				...
				sjson = json.encode(sjson)
				ProtocolProcessorFestivalActivity:send_ACTIVITY2_ActivityDo(g_cityExtenInfo.activity7138, 3, sjson)
			end
		elseif self.m_tData.showType == 1 then
			if self.m_tData.config[1] ~= -1 and self.m_tData.config[2] ~= -1 then
...
--@brief 	确定购买
function CellWeekendSpecial:onClickBuyCall()
	WndWeekendSpecial.m_tReceivedInfo = {type=self.m_tData.type,index=self.m_tData.index}

	local sjson = {}
	sjson.id = self.m_tData.index - 1
	sjson = json.encode(sjson)
	ProtocolProcessorFestivalActivity:send_ACTIVITY2_ActivityDo(g_cityExtenInfo.activity7138, 4, sjson)
end

--@return	新建的表实例对象
function CellWeekendSpecial:_new( )
	local tNewObj = {}
