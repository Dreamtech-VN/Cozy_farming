--WndInvestRebateNor.lua
--@brief	WndInvestRebateNor的UI模块
--@date		2020/12/03
--@author	XTX
--@note		投资返利-常规活动模板
...
--@note		在这里做场景进入前的准备工作
function WndInvestRebateNor:onEnter(element)
	self.m_root = element
end
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function WndInvestRebateNor:onExit(element)
	if self.m_root then 
...
--@brief 	界面加载完成回调
function WndInvestRebateNor:onEnterTransitionDidFinish(element)
	-- body
    self.m_root:enableSchedule("_caculateLeftTime", 1)
end

--@brief 	显示
function WndInvestRebateNor:showWindow()
	-- body
	self:_initUIText()
	self:_updateCountAndProgress()
	self:showReward()
	WZLog("WndInvestRebateNor:showWindow33")
end

--@brief 	点击宝箱回调
function WndInvestRebateNor:onClickBox(element)
	-- body
...
--@brief    点击规则按钮回调
function WndInvestRebateNor:onClickRule(element)
    -- body
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
-------------------------------------私有方法模块Begin--------------------------------------
--@brief 	初始化UI文本
function WndInvestRebateNor:_initUIText()
	-- body
	local txtTimeWords = GetElement(self.m_root, "txtTimeWords_WndInvestRebateNor", WZUILabelTTF)
    ...
    local txtTimeValue = GetElement(self.m_root, "txtTimeValue_WndInvestRebateNor", WZUILabelTTF)
...
--@brief 	更新累计购买次数和进度
function WndInvestRebateNor:_updateCountAndProgress()
	-- body
	local txtBuyTimes = GetElement(self.m_root, "txtBuyTimes_WndInvestRebateNor", WZUILabelTTF)
	if txtBuyTimes then 
		txtBuyTimes:setText(self.m_nCount .. LocalStrings.SHOP_CISHU)
	end

	local nCurNum = self.m_nCount
	local prgTotalReward = GetElement(self.m_root, "prgTotalReward_WndInvestRebateNor", WZUIProgress)
...
--@brief 	设置宝箱的状态
function WndInvestRebateNor:setBoxState()
	...
	for i = 1, 5 do
        local imgBox = GetElement(self.m_root, "imgBtn" .. i .. "_WndInvestRebateNor", WZUIImage)
        local txtTimes = GetElement(self.m_root, "txtTimes" .. i .. "_WndInvestRebateNor", WZUILabelTTF)
        local armBox = GetElement(self.m_root, "armBox" .. i .. "_WndInvestRebateNor", WZArmature)
...
--@brief 	展示奖励
function WndInvestRebateNor:showReward()
    if not self.m_tRewardData then return end
    
	-- body
	WZLog("WndInvestRebateNor:showReward", #self.m_tRewardData)
    self.m_tAllCell = {}

    local nServerTime = SystemTime:getServerTime()
    local nTimeLeft = self.m_nEndTime - nServerTime
    local tableItemList = GetElement(self.m_root, "tableItemList_WndInvestRebateNor", WZUITableContainer)
...
--@brief    计算距离活动结束还剩下多久
--@note     活动结束时间必须是凌晨
function WndInvestRebateNor:_caculateLeftTime(element, delta)
    -- body
    local nServerTime = SystemTime:getServerTime()
    local nTimeLeft = self.m_nEndTime - nServerTime
