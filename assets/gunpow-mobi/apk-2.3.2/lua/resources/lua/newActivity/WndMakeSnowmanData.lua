--WndMakeSnowmanData.lua
--@brief	WndMakeSnowman的数据模块
--@date		2023/11/07
--@author	yrd
--@note		堆雪人活动
...
function WndMakeSnowman:_init()
	self.m_root = nil	 	  			--场景根节点
	self.m_nCoinId = 160537				--货币
	self.m_nCoinId2 = 160538			--货币2
	...
	self.m_tClipAniName = {{"wait1_1", "wait1_2", "wait1_3"}, {"wait2_1", "wait2_2", "wait2_3"}}	--机器特效播放动作名
...
function WndMakeSnowman:_unInit()
	self.m_root = nil
	self.m_nCoinId = nil
	self.m_nCoinId2 = nil
...
--@note		请仅用此方法创建场景
function WndMakeSnowman:createElement()
	if WndMakeSnowman.m_root ~= nil then
		WindowManager:removeWindow(WndMakeSnowman.m_root, WndMakeSnowman, true)
	end
	local element = WZUISystem:getInstance():createElement("WndMakeSnowman")
	assert(element, "WndMakeSnowman create element failed!")
	self:_init()
	return element
end

--@brief	缓存推送更新物品时调用的函数
function WndMakeSnowman:updatePlayerItemData()
	WZLog("WndMakeSnowman:updatePlayerItemData")
...
--@brief 	获取活动详情成功
function WndMakeSnowman:GetActivityInfoOK(activityId, maxCount, count, status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if g_cityExtenInfo.activity7099 == activityId then
...
--@brief 	获取其他活动数据
function WndMakeSnowman:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then --活动信息
		local tResult = json.decode(jsonData)
		WZLog("WndMakeSnowman:_onGetOtherData", doType, Serialize(tResult))

		self.m_nGiftRewardNum = tResult.globalNum
		self:showBagGiftInfo()

		self.m_nGiftStatus = tResult.giftStatus
		self:showJigsaw()
	...
	elseif doType == 2 then --获取达人奖、大奖限量数据
		...
		if tResult.pool == 0 then 
			local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.MAKE_SHOWMAN_TEXT1[10], listBgSize = {474,228}, listBgPos = {0.5,0.431}}
			for i = 1, #array do
		...
		elseif tResult.pool == 1 then 
			local tItem = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.MAKE_SHOWMAN_TEXT1[9], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		elseif tResult.pool == 2 or tResult.pool == 3 then
			local tItem = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.MAKE_SHOWMAN_TEXT1[8], strAtt = LocalStrings.PLANETSEARCH_TEXT1[4], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			if tResult.pool == 3 then 
	...
	elseif doType == 3 then --开启结果
		...
		if tResult.fItemIds then 
			for i = 1, #tResult.fItemIds do
				...
				tItem.playerItemId = tResult.playerItemIds[itemIdIndex]
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.MAKE_SHOWMAN_TEXT1[10])
		...
		if tResult.sItemIds then 
			for i = 1, #tResult.sItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.MAKE_SHOWMAN_TEXT1[9])
				tItem.txtTitlePt = {0.5,0.885}
		...
		if tResult.nItemIds then 
			for i = 1, #tResult.nItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.MAKE_SHOWMAN_TEXT1[8])
		...
		if result == 1 then 
			...
			self.m_tOpenResult.addExp = tResult.gift
...
--@brief 	设置棋谱残页的状态
function WndMakeSnowman:setOpenPageState(state)
	if self.m_root == nil then return end 
	self.m_bOpenPageState = state
end

--@brief 	设置射箭的状态
function WndMakeSnowman:setOpenState(state)
	if self.m_root == nil then return end 
	self.m_bOpenState = state
end

--@brief    添加保存上次选择的瓶子
function WndMakeSnowman:saveToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("MAKESNOWMAN_MARK", _KeyString)
    local curValue = self.m_nDrawToolType
    data:setStringValue("MAKESNOWMAN_MARK", _KeyString, curValue)
    data:flush()
end

--@brief    获取上次保存的的瓶子
function WndMakeSnowman:getToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
...
function WndMakeSnowman:saveOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
...
function WndMakeSnowman:getOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "OPERATETIMES" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("MAKESNOWMAN_MARK", _KeyString)
...
--@brief 	关闭抽奖奖励展示界面回调
function WndMakeSnowman:_afterCloseReward()
	if self.m_root == nil then return end 
...
--@brief 	关闭抽奖奖励展示界面回调
function WndMakeSnowman:_afterClosePageReward()
	if self.m_root == nil then return end 
