--WndColdDrinkData.lua
--@brief	WndColdDrink的数据模块
--@date		2024/4/25
--@author	yrd
--@note		清凉冰饮活动主界面
...
function WndColdDrink:_init()
	...
	self.m_nCoinId = 160660
	self.m_nCoinId2 = 160661
	self.m_nMaxLotteryCount = 20    --最大抽奖次数
	self.m_nCount = 0 
	self.m_tBallAniName = {{"", "guocha", "guocha2"}, {"", "naicha1", "naicha2"}}
	self.m_tWaitAniName = {"daiji1", "daiji2"}
	...
	self.m_nDoubleEndTime = nil
	self.m_nDoubleNum = nil
end


--@brief	反初始化表的成员变量
--@note		在退出场景时回调的onExit函数里面必须调用本函数
function WndColdDrink:_unInit()
	self.m_root = nil
	self.m_nStartTime = nil 
	self.m_nEndTime = nil 
	self.m_tContent = nil 
...
--@note		请仅用此方法创建场景
function WndColdDrink:createElement()
	if WndColdDrink.m_root ~= nil then
		WindowManager:removeWindow(WndColdDrink.m_root, WndColdDrink, true)
	end
	local element = WZUISystem:getInstance():createElement("WndColdDrink")
	assert(element, "WndColdDrink create element failed!")
	self:_init()
	return element
end

--@brief 	外部接口
function WndColdDrink:showInterface()
	LoadNewActivityRes(true)
	local wndWater = WndColdDrink:createElement()
	if wndWater then 
		WindowManager:addWindow(wndWater, WndColdDrink, false)
	end
end

--@brief 	获取活动详情成功
function WndColdDrink:GetActivityInfoOK(activityId,maxCount,count,status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	-- body
	WZLog("WndColdDrink:GetActivityInfoOK", activityId)
	if g_cityExtenInfo.activity7124 == activityId then 
		...
		self.m_nChooseReward = GetOperateTimes("OPERATETIMES_COLDDRINK", self.m_nActivityId)
		...
		for i = 1, #self.m_tContent.scoreConfig do
			...
			self.m_tLvRewardList[i].name = LocalStrings.COLDDRINK_TEXT1[17][i + 1]
...
--@brief 	获取其他活动数据
function WndColdDrink:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then --等级奖励数据
		local tResult = json.decode(jsonData)
		WZLog("WndColdDrink:_onGetOtherData 111", Serialize(tResult))
		self.m_nCurExp = tResult.score
		self.m_nDoubleEndTime = tResult.doubleEndTime
		self.m_nDoubleNum = tResult.doubleNum
	...
	elseif doType == 2 then --等级奖励数据
		local tResult = json.decode(jsonData)
		WZLog("WndColdDrink:_onGetOtherData 222", Serialize(tResult))
		...
		if tResult.pool == 0 then 
			local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.COLDDRINK_TEXT1[12], listBgSize = {474,228}, listBgPos = {0.5,0.431}}
			for i = 1, #array do
		...
		elseif tResult.pool == 1 then 
			local tItem = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.COLDDRINK_TEXT1[13], strAtt = LocalStrings.COLDDRINK_TEXT1[16], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
		...
		elseif tResult.pool == 2 then 
			local tItem = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.COLDDRINK_TEXT1[14], strAtt = LocalStrings.COLDDRINK_TEXT1[16], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31, pool = tResult.pool}
			for i = 1, #array do
				local string = string.sub(array[i], 2, -2) 
				local id = tonumber(SplitStringWithSeparator(string,",")[nSex + 1])
				local num = tonumber(SplitStringWithSeparator(string,",")[3])
		...
		if self.m_bIsOpenReward and self.m_tGetTimes and #self.m_tGetTimes == 3 then 
			...
			otherData.chooseInfo = {strKey="COLDDRINK_TEXT1", wordIndex=12, doType=4}
	...
	elseif doType == 3 then --开启结果
		local tResult = json.decode(jsonData)
		WZLog("WndColdDrink:_onGetOtherData 333", Serialize(tResult))
		self.m_tOpenResult = {}
		...
		if tResult.fItemIds then 
			for i = 1, #tResult.fItemIds do
				...
				tItem.playerItemId = tResult.playerItemIds[itemIdIndex]
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.COLDDRINK_TEXT1[12])
		...
		if tResult.sItemIds then 
			for i = 1, #tResult.sItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.COLDDRINK_TEXT1[13])
				tItem.txtTitlePt = {0.5,0.885}
		...
		if tResult.nItemIds then 
			for i = 1, #tResult.nItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.COLDDRINK_TEXT1[14])
	...
	elseif doType == 4 then --选择奖励
		local tResult = json.decode(jsonData)
		WZLog("WndColdDrink:_onGetOtherData 444", Serialize(tResult))
		if result == 0 then 
			local tTempList = nil 
			local nTag = 0
	...
	elseif doType == 5 then --全服礼包
		local tResult = json.decode(jsonData)
		WZLog("WndCrabber:_onGetOtherData", doType, Serialize(tResult))
		if result == 1 then 
			local normalRewards = {}
			for i = 1, #tResult.itemIds do
				local tItem = {}
	...
	elseif doType == 6 then --领取等级奖励
		local tResult = json.decode(jsonData)
		WZLog("WndColdDrink:_onGetOtherData 666", Serialize(tResult))
		if result == 0 then 
			self.m_tLvRewardList[tResult.id + 1].status = tResult.status + 1
...
function WndColdDrink:updatePlayerItemData()
	WZLog("WndColdDrink:updatePlayerItemData")
...
--@brief 	设置射箭的状态
function WndColdDrink:setOpenState(state)
	-- body
	if self.m_root == nil then return end 
...
--@brief 	关闭抽奖奖励展示界面回调
function WndColdDrink:_afterCloseReward()
	if self.m_root == nil then return end 
...
--@brief 	获取当前捕鼠等级
function WndColdDrink:getCurLvInfo()
	local nLevel = 0 
