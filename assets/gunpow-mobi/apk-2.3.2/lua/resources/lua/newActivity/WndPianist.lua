...
--@note		在这里做场景进入前的准备工作
function WndPianist:onEnter(element)
	self.m_root = element
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function WndPianist:onExit(element)
...
--@brief    onenter函数已执行
function WndPianist:onEnterTransitionDidFinish(element)
	ProtocolProcessorWndActivityOnLine:send_ACTIVITY_GetActivityInfo(g_cityExtenInfo.activity7100, 7100)
end

--@brief    点击关闭窗口按钮
function WndPianist:showInterface()
	LoadNewActivityRes(true)
	local wnd = WndPianist:createElement()
	WindowManager:addWindow(wnd, WndPianist, false)
end

--@brief    点击关闭窗口按钮
function WndPianist:onClickClose(element)
...
--@brief    点击规则按钮
function WndPianist:onClickRule(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	WndSingleMapDesc:showInterface(LocalStrings.PIANIST_TEXT2)
end

--@brief    初始化静态文本
function WndPianist:_initStaticText()
	self:getToolType()
	self:_showAnimal()

	GetElement(self.m_root,"txtChoosePrize",WZUILabelTTF):setText(LocalStrings.WATERMELON_TEXT1[22])

	GetElement(self.m_root,"txtOperateBtn1",WZUILabelTTF):setText(LocalStrings.PIANIST_TEXT1[4])
	GetElement(self.m_root,"txtOperateBtn2",WZUILabelTTF):setText(LocalStrings.PIANIST_TEXT1[5])
	GetElement(self.m_root,"txtOperateBtn3",WZUILabelTTF):setText(LocalStrings.PIANIST_TEXT1[6])
	GetElement(self.m_root,"txtOperateBtn4",WZUILabelTTF):setText(LocalStrings.PIANIST_TEXT1[7])
end

--@brief 	更新许愿币的数量
function WndPianist:_updateCoinNum()
...
--@brief 	初始化活动时间
function WndPianist:_initActivityTime()
...
--@brief 	红点
function WndPianist:showRedDot()
	if self.m_root == nil then return end 

	local imgBtnRedDot2 = GetElement(self.m_root, "imgBtnRedDot2", WZUIImage)
	if GlobalGame.g_tRedPointTypeList and (GlobalGame.g_tRedPointTypeList[217100] or GlobalGame.g_tRedPointTypeList[227100] or GlobalGame.g_tRedPointTypeList[237100]) then 
...
--@brief 	刷新全民礼包的红点
function WndPianist:showBagGiftInfo()
...
--@brief 	更新界面
function WndPianist:updateUI()

end

--@brief 	点击选择瓶子按钮回调
function WndPianist:onClickChooseTool(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--@brief 	更新许愿按钮
function WndPianist:updateWishingBtn()
	for i=1, 2 do
		...
		if freeCount == 1 then
			if i == 1 then
				txtUseTool:setText(LocalStrings.PIANIST_TEXT1[2])
			elseif i == 2 then
				if nTempTimes == 0 then
					txtUseTool:setText(string.format(LocalStrings.PIANIST_TEXT1[3], self.m_tDrawNumList[i]))
				else
					txtUseTool:setText(string.format(LocalStrings.PIANIST_TEXT1[3], nTimes))
				end
			end
		else
			txtUseTool:setText(string.format(LocalStrings.PIANIST_TEXT1[3], nTimes))
...
--@brief 	设置待机特效
function WndPianist:_showAnimal()
	local spinePath = "activity/hd_ren_gangqingych"
	...
	if existSpine then
		...
		local _sIndex = "hd_ren_gangqingych"
		local downloadInfo = GetDownloadInfo(_sIndex, "activityEffect")
		if downloadInfo then 
			DownloadManager:addDownloadTask(7100, downloadInfo.url, downloadInfo.md5, _sIndex, "downloadEffectCallback", WndPianist)
		end
	end
...
function WndPianist:downloadEffectCallback(taskId,extraData,failed)
    WZLog("WndPianist:downloadEffectCallback",taskId,extraData,failed)
    self:_showAnimal()
end

--@brief 	设置播放的保龄球的动画
--@param 	aniIndex:1待机；2击球
function WndPianist:_playAni(aniIndex, bLoop)
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
...
--@brief 	设置播放的保龄球的动画
--@param 	aniIndex:1待机；2击球
function WndPianist:_playAnotherAni(aniIndex, bLoop)
	local spineCopy = GetElement(self.m_root, "spineCopy", WZUISpine)
	aniIndex = aniIndex or 0
	if aniIndex == 0 then
...
--@brief 	显示开启动画
function WndPianist:showOpenAction()
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
	local spinePath1 = "activity/hd_ren_gangqingych"
...
function WndPianist:showShootBefore()
	self:_playAni(0)
	local nSeconds = 2
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
...
--@brief 	显示开启奖励
function WndPianist:showShootReward()
	local spineOpen = GetElement(self.m_root, "spineOpen", WZUISpine)
	spineOpen:disableSchedule()
	self:_playAni(1, true)
	self:_playAnotherAni(0)
	...
	if self.m_tOpenResult.addExp and self.m_tOpenResult.addExp > 0 then 
		strContent = strContent .. LocalStrings.PIANIST_TEXT1[18] .. "+" .. self.m_tOpenResult.addExp .. "    "
	end
	if self.m_tOpenResult.addCoin and self.m_tOpenResult.addCoin > 0 then 
		strContent = strContent .. LocalStrings.PIANIST_TEXT1[11] .. "+" .. self.m_tOpenResult.addCoin .. "    "
	end
	if strContent ~= "" then 
...
--@brief 	点击"自选奖励"按钮回调
function WndPianist:onClickChoosePrize(element)
...
--@brief 	点击许愿按钮回调
function WndPianist:onClickUseTool(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
--@brief 	前往小推车购买
function WndPianist:goToBuy(nId, nResType)
...
--@brief 	点击按钮回调
function WndPianist:onClickOperateBtn(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	local nTag = element:getTag()
	if nTag == 1 then
		WndDollMachineShop:showInterface(15, self.m_nActivityId)
	elseif nTag == 2 then
		CellNewYearTask:showInterface(47, self.m_nActivityId)
	elseif nTag == 3 then
		WndShopRank:showInterface(66, self.m_nActivityId)
	elseif nTag == 4 then
		if self.m_nGiftRewardNum >= 1 then
			...
			local tData = {}
			tData.txtTitle = string.format(LocalStrings.PIANIST_TEXT1[17], self.m_tContent.globalConfig[1])
			tData.nType = 2
...
--@brief 	成熟度
function WndPianist:_showProgress()
    ...
    for i = 1, 6 do
	    if self.m_tScoreConfig[i].lastStatus == nil or self.m_tScoreConfig[i].lastStatus ~= self.m_tScoreConfig[i].status then
			...
			elseif self.m_tScoreConfig[i].status == 1 then 
				...
				GetElement(self.m_root, "imgScoreYlq" .. i, WZUIImage):setVisible(false)
			elseif self.m_tScoreConfig[i].status == 2 then
				GetElement(self.m_root, "spineScoreBox" .. i, WZUISpine):play("wait" .. i, true)
...
--@brief 	设置步数积分宝箱数量
function WndPianist:_showStepScoreNum()
	for i = 1, 6 do
...
--@brief 	点击积分宝箱回调
function WndPianist:onClickScoreBox(element)
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)

	local nTag = element:getTag()
	if self.m_tScoreConfig[nTag].status == 1 then 
		--背包已满提示
	    if CacheCenter:getRemainAmount() <= 0 then
	        MsgBoxManager:showTipBox(LocalStrings.MAIL_FULLBAG2)
	        return
		...
		ProtocolProcessorFestivalActivity:send_ACTIVITY2_ActivityDo(self.m_nActivityId, 7, strData)
	else
...
--@brief 	iphoneX适配
function WndPianist:_adaptIphoneX()
	if IsIphoneX() then
...
function WndPianist:_adaptLanguage_vn(  )
