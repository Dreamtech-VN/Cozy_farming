...
function WndPianist:_init()
	self.m_root = nil	 	  			--场景根节点
	self.m_nCoinId = 160539				--货币
	...
	self.m_tClipAniName = {{"wait1", "wait1_1", "wait2_1"}, {"wait2", "wait1_2", "wait2_2"}}	--机器特效播放动作名
...
--@note		请仅用此方法创建场景
function WndPianist:createElement()
	if WndPianist.m_root ~= nil then
		WindowManager:removeWindow(WndPianist.m_root, WndPianist, true)
	end
	local element = WZUISystem:getInstance():createElement("WndPianist")
	assert(element, "WndPianist create element failed!")
	self:_init()
	return element
end

--@brief	缓存推送更新物品时调用的函数
function WndPianist:updatePlayerItemData()
	WZLog("WndPianist:updatePlayerItemData")
...
--@brief 	获取活动详情成功
function WndPianist:GetActivityInfoOK(activityId, maxCount, count, status, rewardCounts, rewardItems, rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if g_cityExtenInfo.activity7100 == activityId then
		...
		--积分进度
		local tempScoreReward = json.decode(self.m_tContent.giftRewards)
		local tempScoreTarget = json.decode(self.m_tContent.giftConfig)
		self.m_tScoreConfig = {}
...
--@brief 	解析大奖数据
function WndPianist:_analyzeBigReward()
	local sBigReward = self.m_tContent.firstRewards
	local array = SplitStringWithSeparator(sBigReward, "&")
	local nSex = CacheCenter:getPlayerInfo().sex
	local tItem = {reward_ids = {}, reward_nums = {}, name = LocalStrings.PIANIST_TEXT1[10], listBgSize = {474,228}, listBgPos = {0.5,0.431}}
	...
	local tItem1 = {reward_ids2 = {}, reward_nums2 = {}, name = LocalStrings.PIANIST_TEXT1[9], strAtt = LocalStrings.TRAMPOLINE_TEXT1[16], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461} , cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31}
	for i = 1, #array1 do
	...
	local tItem2 = {reward_ids1 = {}, reward_nums1 = {}, name = LocalStrings.PIANIST_TEXT1[8], strAtt = LocalStrings.TRAMPOLINE_TEXT1[16], listBgSize = {474,208}, listSize = {474,190}, listPos = {0.5,0.46}, listBgPos = {0.5,0.461}, cellElementHeight = 0.5, chooseState = {}, leftConfig = {}, type = 31}
	for i = 1, #array2 do
	...
	WZLog("WndPianist:_analyzeBigReward", Serialize(self.m_tBigRewardList))
end

--@brief 	获取其他活动数据
function WndPianist:_onGetOtherData(activityId, doType, result, jsonData)
	if self.m_root == nil then return end 

	if doType == 1 then --获取达人奖、大奖限量数据
		local tResult = json.decode(jsonData)
		WZLog("WndPianist:_onGetOtherData", doType, Serialize(tResult))
		...
		--积分进度
		self.m_nCurScore = tResult.giftProgress
		for i = 1, #tResult.giftRewardStatus do
			self.m_tScoreConfig[i].status = tResult.giftRewardStatus[i]
		end
		self:_showProgress()
	...
	elseif doType == 3 then --开启结果
		...
		if tResult.fItemIds then 
			for i = 1, #tResult.fItemIds do
				...
				tItem.playerItemId = tResult.playerItemIds[itemIdIndex]
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.PIANIST_TEXT1[10])
		...
		if tResult.sItemIds then 
			for i = 1, #tResult.sItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.PIANIST_TEXT1[9])
				tItem.txtTitlePt = {0.5,0.885}
		...
		if tResult.nItemIds then 
			for i = 1, #tResult.nItemIds do
				...
				tItem.strTitle = string.format(strTitleFormat, LocalStrings.PIANIST_TEXT1[8])
		...
		if result == 1 then 
			...
			self.m_tOpenResult.addCoin = tResult.extItemNums
...
--@brief 	设置射箭的状态
function WndPianist:setOpenState(state)
	if self.m_root == nil then return end 
	self.m_bOpenState = state
end

--@brief    添加保存上次选择的瓶子
function WndPianist:saveToolType()
    local _KeyString = ""
...
--@brief    获取上次保存的的瓶子
function WndPianist:getToolType()
    local _KeyString = ""
    local curDate = os.date("*t", SystemTime:getServerTime())
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "TOOLTYPE" .. tostring(CacheCenter:getPlayerInfo().id)
...
function WndPianist:saveOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
...
function WndPianist:getOperateTimes()
    local _KeyString = ""
    local data = WZDataFile:getInstance():getUserData()
    _KeyString = "OPERATETIMES" .. tostring(CacheCenter:getPlayerInfo().id)
    local strValue =  data:getStringValue("PIANIST_MARK", _KeyString)
...
--@brief 	关闭抽奖奖励展示界面回调
function WndPianist:_afterCloseReward()
	if self.m_root == nil then return end 
