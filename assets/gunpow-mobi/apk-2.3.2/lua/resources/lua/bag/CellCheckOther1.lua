...
function CellCheckOther1:onAttr5(element)
	...
	local masterId = WndCheckOther.m_tPlayerInfo.masterId
	local icon
	local quality = -1
	local title
	local title1 = ""
	...
	local tData
	if roleLevel >= MASTERLEVEL and tonumber(masterId) == 0 then
		WZLog("师德等级"..level)
	...
	WndTips:show(element,WndCheckOther.m_root,23,tData,ccp(-45,0), true)
end
...
function CellCheckOther1:onAttr7(element)
	...
	if awakeStep <= 0 then
		...
		if tData.awakeStep >= 4 then
			WndTips:show(element,WndCheckOther.m_root,40,tData,ccp(-60,100), true)
		else
