...
function WndDressAttr:updateDressGrid()
	...
	local RuleDescFreeText = GetElement(self.m_root,"RuleDescFreeText",WZUIFreeTextBox)
	local msg = LocalStrings["DRESS_RULE"]
	RuleDescFreeText:setShowText(msg)


	--获取规则说明内容文本的大小
	local txtExplanation = GetElement(self.m_root, "RuleDescFreeText", WZUIFreeTextBox)
	local txtSize = txtExplanation:getContentSize()	
	txtExplanation:setAnchorPoint(ccp(0,1))
	txtExplanation:setPositionY(txtSize.height - 3)
	WZLog("富文本框尺寸是",txtSize.width,txtSize.height)
--
	
	local rollconExplanation = self.m_root:getChildElement("rollconExplanation_WndDressAttr")
...
function WndDressAttr:_adaptLanguage_ug(  )
	GetElement(self.m_root,"txtMaxFight_WndDressAttr",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.55,0.65))
	local maxFight = GetElement(self.m_root,"maxFight",WZUILabelTTF)
	maxFight:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	maxFight:setRelativePosition(GlobalMethod:ccp(0.28,0.65))
	GetElement(self.m_root,"txtFight_WndDressAttr",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.58,0.42))
	local fight = GetElement(self.m_root,"fight_WndDressAttr",WZUILabelTTF)
	fight:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	fight:setRelativePosition(GlobalMethod:ccp(0.21,0.42))

	for i=1,6 do
		local btmTitle = GetElement(self.m_root,"btmTitle"..i,WZUILabelTTF)
		btmTitle:setScale(0.8)
		btmTitle:setAnchorPoint(GlobalMethod:ccp(1,0.5))
		btmTitle:setRelativePosition(GlobalMethod:ccp(0.95,0.5))
		local btmTitle = GetElement(self.m_root,"ttfAttr"..i.."_WndDressAttr",WZUILabelTTF)
		btmTitle:setScale(0.8)
		btmTitle:setAnchorPoint(GlobalMethod:ccp(0,0.5))
		btmTitle:setRelativePosition(GlobalMethod:ccp(0.05,0.5))
	end

	for i=1,4 do
		local topTitle = GetElement(self.m_root,"topTitle"..i,WZUILabelTTF)
		topTitle:setRelativePosition(GlobalMethod:ccp(0.84,0.5))
		topTitle:setScale(0.5)
		topTitle:setDimensions(GlobalMethod:CCSize(200))
		local ttfNum = GetElement(self.m_root,"ttfNum"..i.."_WndDressAttr",WZUILabelTTF)
		ttfNum:setAnchorPoint(GlobalMethod:ccp(1,0.5))
		ttfNum:setRelativePosition(GlobalMethod:ccp(0.7,0.5))
		local positionTitle = GetElement(self.m_root,"positionTitle"..i,WZUILabelTTF)
		positionTitle:setRelativePosition(GlobalMethod:ccp(0.34,0.5))
		positionTitle:setScale(0.5)
		positionTitle:setDimensions(GlobalMethod:CCSize(200))
		local positionAttr = GetElement(self.m_root,"positionAttr"..i.."_WndDressAttr",WZUILabelTTF)
		positionAttr:setAnchorPoint(GlobalMethod:ccp(1,0.5))
		positionAttr:setRelativePosition(GlobalMethod:ccp(0.2,0.5))
	end
end
-------------------------语言适配End----------------------------------
