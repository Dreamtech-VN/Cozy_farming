...
function WndBag:onItemApply(tData)
	...
	elseif tData.main_type == 23 then 
		...
		return
	elseif (tData.main_type == 2 and tData.sub_type == 22) then 
		local nPlayerItemId = self:returnPlayerItemId(tData.id)
		ProtocolProcessorRecycling:send_PLAYERITEM_UseItem(nPlayerItemId, 1, "" )
		return --蜡烛之心
	elseif (tData.main_type == 2 and tData.sub_type == 25) then 
		--觉醒副本挑战次数重置卡
		local tMultiCopyData = CacheCenter:getMultiCopyData()
		if tMultiCopyData == nil then return end 
		if tMultiCopyData.awakeTimes <= 0 then 
			MsgBoxManager:showTipBox(LocalStrings.BATTLE_HELP_TEXT15)
		else
			local nPlayerItemId = self:returnPlayerItemId(tData.id)
			ProtocolProcessorRecycling:send_PLAYERITEM_UseItem(nPlayerItemId, 1, "" )
		end
		return 
	elseif (tData.main_type == 2 and tData.sub_type == 51) then 
		local wndGoodsGet = WndGameGoodsGet:createElement(tData.playerItemId)
        if wndGoodsGet ~= nil then
            WindowManager:addWindow(wndGoodsGet,WndGameGoodsGet,nil,false)
        end
		return
	end
...
function WndBag:_adaptLanguage_ug(  )	
	local txtEquipAll = GetElement(self.m_root,"txtEquipAll_WndBag",WZUILabelTTF)
	txtEquipAll:setScale(0.35)
	txtEquipAll:setDimensions(GlobalMethod:CCSize(180))
end
-------------------------------------语言适配模块End----------------------------------------
