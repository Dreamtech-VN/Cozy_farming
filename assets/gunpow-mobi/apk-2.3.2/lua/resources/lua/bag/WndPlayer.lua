...
function WndPlayer:onEnterFinish(element,t)
	...
	if GlobalGame.g_tRedPointTypeList then
		self:setLibraryRedPoint(GlobalGame.g_tRedPointTypeList[290] or (GlobalGame.g_tRedPointTypeList[291] or false))
	end
end
...
function WndPlayer:onEquipAttr(element) 
	...
	if WndBag.m_root then
		parent = WZUIContainer:luaTo(WndBag.m_root:getChildElement("conLeftA_WndBag"))
	elseif WndBagRole.m_root then
		parent = WZUIContainer:luaTo(WndBagRole.m_root:getChildElement("conLeftA_WndBag"))
	end
	if parent == nil then
		return
	end
...
function WndPlayer:onDressClicked(tLuaObj,tag,tData)
	...
	if tData ~= nil and tData.basicInfo ~= nil then
		...
		local rightCon = nil
		if WndBag.m_root then
			rightCon = WZUIContainer:luaTo(WndBag.m_root:getChildElement("conRight_WndBag"))
		elseif WndBagRole.m_root then
			rightCon = WZUIContainer:luaTo(WndBagRole.m_root:getChildElement("conRight_WndBag"))
		end
		if rightCon == nil then
			return
		end
...
function WndPlayer:onEquipBackFun(luaTable,tag,tData)
	...
	--更新背包显示物品类型
	if WndEquipNew.m_root ~= nil then
		local tagToSubtype = {1,3,2,4,5,6,7,8}
		local tagToId = {1,3,5,2,4,6,7,8}
		local sub_type = tagToSubtype[tag]
		WndEquipNew.allSub = sub_type
		if CacheCenter:getEquipAllList(sub_type) == nil or #CacheCenter:getEquipAllList(sub_type) == 0 then
			WndFastGetItems:show(7799+tagToId[tag])
		else
			WndEquipNew:onLoadEquip()
		end
	end
...
--@brief	图鉴
function WndPlayer:onGotoLibrary(element)
	-- body
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	WndLibrary:show()
end
--图鉴红点
function WndPlayer:setLibraryRedPoint(visible)
...
function WndPlayer:btnGoodsFullShow(istrue)
	-- body
	local btn = GetElement(self.m_root,"btnGotoGoodsFull_WndPlayer",WZUIButton)
...
function WndPlayer:_updateFire(tFire)
	...
	local imgVipIcon = GetElement(self.m_root, "imgVipIcon_WndPlayer", WZUIImage)
	setVipIconByVipLevel(imgVipIcon, tonumber(self.m_tRole.vipLevel))
	...
	--设置等级名字经验进度条
	local nameTemplate = [[<T C="132,66,29" S="20" P="0">%s</T><T C="5,180,0" S="20" P="0">(ID%s)</T>]]
	...
	if tFire.level ~= nil and GDatatab_player_upgrade["id_"..tFire.level] ~= nil then
		maxExp = GDatatab_player_upgrade["id_"..tFire.level].exp
	end
	...
	local _, maxExpFormat = GetMaxExpByLevel(tFire.level)
...
function WndPlayer:_showRoleItem()
	...
	for i=1 , 8 do 
		if self.m_tPlayer[i] ~= nil then
    		...
    		GetElement(self.equipGridList[i].m_root, "btnImg_CellGoodItem", WZUI9Image):setFile("ui/common/common_js_zb_di.png")
...
function WndPlayer:_setPlayer(element, t)
	...
	if self.m_tPlayerAni == nil then
		...
		if playerInfo.shapeId ~= nil and playerInfo.shapeId > 0 and WndPhantom.show == 1 then
			local skins = GDatatab_shape_skins["id_" .. playerInfo.shapeId]
...
function WndPlayer:onTip1(element)
	...
	local conRoot = nil
	if WndBag.m_root then
		conRoot = WndBag.m_root
	elseif WndBagRole.m_root then
		conRoot = WndBagRole.m_root
	end
	if conRoot == nil then
		return
	end

	WndTips:show(element,conRoot,9,tData,GlobalMethod:ccp(250,0))
...
function WndPlayer:onTip4(element)
	...
	if self.m_bCheckOther == true then
		...
		WndTips:show(element,conRoot,11,tData,GlobalMethod:ccp(260,0))
		WndTips.m_root:setShowAll(true)
	end
end
-------------------------------------私有方法模块End----------------------------------------
