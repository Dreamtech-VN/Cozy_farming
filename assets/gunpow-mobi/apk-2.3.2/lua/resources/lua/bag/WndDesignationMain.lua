...
function WndDesignationMain:onCloseClick()
    ...
    --维护称号本地数据表和缓存，清除红点状态
    if self.m_nclickedMainClassicId == 14 then
        self:_updateDesiRed()
    end
end
...
function WndDesignationMain:_initAchiePanel()
    ...
    self:_initTopInfo()
    --下载称号资源
    --DownloadManager:downloadResCheckForType("all_title_frame", 5000, true)
    -- if isChannelQQHall() then
    --     DownloadManager:downloadResCheckForTypeAll("all_title_frame_resources", 3000, true)
    --     DownloadManager:downloadResCheckForTypeAll("all_title_frame_resources_cn", 3200, true)
    -- end
end
...
function WndDesignationMain:_initFreeListAchiePanel()
    ...
    for i = 1, #self.m_tAchieMentList do
    --    if CacheCenter:getGameParam().gameStatus ~= "1" or CacheCenter:getGameParam().gameStatus == "1" and (self.m_tAchieMentList[i].id ~= 1 and self.m_tAchieMentList[i].id ~= 7 and self.m_tAchieMentList[i].id ~= 14) then --审核服隐藏"特殊称号","恩爱夫妻"和"其他称号"标签
            local element,tNewObj = CellDesignationOne:createElement()
            element = WZUIContainer:luaTo(element)
            tNewObj:setCellPos(i)
            tNewObj:setJobId(self.m_tAchieMentList[i].id)
            WZLog("i=====",i,self.m_tAchieMentList[i].id,self.m_tAchieMentList[i].name,self.m_tAchieMentList[i].complete,self.m_tAchieMentList[i].target)
            ...
            if self.m_tAchieMentList[i].statusNum > 0 or (self.m_tAchieMentList[i].id == 14 and self.m_bHaveNewDesi) then
    ...
    AchieFreeList:getMoveElement():setPositionX(AchieFreeList:getMaxPosition().x)
    ...
    for i = 1, #self.m_tAchieMentList do
        ...
        --     if (self.m_tAchieMentList[i].id ~= 1 and self.m_tAchieMentList[i].id ~= 7 and self.m_tAchieMentList[i].id ~= 14) then
        --         if self.m_tAchieMentList[i].statusNum > 0 or (self.m_tAchieMentList[i].id == 14 and self.m_bHaveNewDesi) then
        --             bNewAchie = true
    ...
    if nMainAchieHavedRedPointIndex > 5 then
        local nCurPositionY = AchieFreeList:getMaxPosition().y + (nMainAchieHavedRedPointIndex - 5) * cellHeight
        if nCurPositionY > AchieFreeList:getMinPosition().y then 
            nCurPositionY = AchieFreeList:getMinPosition().y
        end
        AchieFreeList:getMoveElement():setPositionX(nCurPositionY)
    end
    ...
    if not bNewAchie then
        -- if CacheCenter:getGameParam().gameStatus == "1" then
        --     nJobId = self.m_tAchieMentList[2].id
        --     nPos = 2
        -- else
...
function WndDesignationMain:onClickMainCellForAchie( nJobId , nPos, tNewObj)
    ...
    for i=1,#self.m_tAchieMentList do
        if self.m_tAchieMentList[i].id == nJobId then
            local sMainTitle =  "【" .. self.m_tAchieMentList[i].name .. "】"
            if nJobId == 14 then
                sMainTitle = "【" .. self.m_tAchieMentList[i].name .. "】"
                GetElement(self.m_root, "conForSubAchie_WndDesignationMain", WZUIContainer):setVisible(false)
            else
                if nJobId == 1 then 
                    sMainTitle =  "【" .. LocalStrings.HAVE .. self.m_tAchieMentList[i].name .. "】"
                end
            ...
            if nJobId ~= 14 then
                ...
                if nJobId == 1 then 
                    sGetAchiePoints = string.format("%d/%d", self.m_tAchieMentList[i].completeCount, self.m_tAchieMentList[i].totalCount)
                end
    ...
    local tbVipDesi = GetElement(self.m_root, "tbVipDesi_WndDesignationMain", WZUITableContainer)
	...
	if self.m_nclickedMainClassicId == nJobId then 
        ...
        subFreeList:setVisible(false)
        tbVipDesi:setVisible(false)
        local tCurLoadList = nil 
        if nJobId == 1 then 
            tCurLoadList = self.m_tAchieMentList[nPos].childList
            tbVipDesi:setVisible(true)
            self:onShowDesi(tCurLoadList)
            return 
        elseif nJobId == 14 then
            tCurLoadList = self.m_tDesignationList
        else
            tCurLoadList = self.m_tAchieMentList[nPos].childList
        end
        subFreeList:setVisible(true)
        self:onShowAchie(subFreeList, tCurLoadList)
...
--@brief    动态加载显示子成就列表
function WndDesignationMain:onShowAchie(element, tCurLoadList)
    -- body
    if tCurLoadList == nil or #tCurLoadList == 0 then
        return
    end

    self.m_nclickedDesignatin = -1
    element = WZUIFreeListContainer:luaTo(element)
    for i = 1, #tCurLoadList do
        local subTable = tCurLoadList[i]
        local cellElement,tNewObj = CellDesignationTwo:createElement()
        cellElement = WZUIContainer:luaTo(cellElement)

        if self.m_nclickedMainClassicId == 14 then
            tNewObj:setDesiData(tCurLoadList[i])
            --称号红点
...
function WndDesignationMain:acceptPrizeSucess(reward, nLeftAchievePoint)
    ...
    for i = 1, #self.m_tAchieMentList do
        if self.m_tAchieMentList[i].id == self.m_nclickedMainClassicId then
            nPos = i
            local sGetAchiePoints = string.format("%d/%d", self.m_tAchieMentList[i].achiePointsGet, self.m_tAchieMentList[i].achiePoints)
            if self.m_nclickedMainClassicId == 1 then 
                sGetAchiePoints = string.format("%d/%d", self.m_tAchieMentList[i].completeCount, self.m_tAchieMentList[i].totalCount)
            end
            GetElement(self.m_root, "txtAchiePointNum_WndDesignationMain", WZUILabelTTF):setText(sGetAchiePoints)
            local proMainProgress = GetElement(self.m_root, "proAchiePoint_WndDesignationMain", WZUIProgress)
    ...
    if self.m_nclickedMainClassicId == 1 then 
        local  tbVipDesi =  GetElement(self.m_root, "tbVipDesi_WndDesignationMain", WZUITableContainer)
        if tbVipDesi == nil then 
            return
        end

        local nTag = 0 
        local element = tbVipDesi:getCellElement(nTag)
        while element do
            element = WZUIContainer:luaTo(element)
            local cellItem = element:getChildElement("__CellDesignationThree")
            ...
            nTag = nTag + 1
            element = tbVipDesi:getCellElement(nTag)
        end
    else
        local  subFreeList =  GetElement(self.m_root, "subFreeList_WndDesignationMain", WZUIFreeListContainer)
        ...
        for i = 1, subFreeList:size() do
            local element = subFreeList:getAt(i - 1)    
            if element == nil then
                return
            end
            element = WZUIContainer:luaTo(element)
...
--            WZLog("*********** WndDesignationMain:acceptPrizeSucess **************", self.m_tAcSubTable.id, tNewObj:getJobId())
        ...
        for j = 1, #self.m_tAchieMentList[nPos].childList do         --npos对应cell位置和table组的下标
            ...
            element:setTag(j - 1)
            element:setContentSize(GlobalMethod:CCSize(900,110))
            element:setRelativeSize(GlobalMethod:CCSize(1,0.32))
...
function WndDesignationMain:_caculateAchiePoints()
    ...
    for i = 1, #self.m_tAchieMentList do
        ...
        local totalCount = #self.m_tAchieMentList[i].childList
        local completeCount = 0 
        for j=1,#self.m_tAchieMentList[i].childList do
            local subTable = self.m_tAchieMentList[i].childList[j]  
            if mainAchie.id == subTable.p_id then
                if subTable.reward ~= -1 and subTable.reward ~= nil and type(subTable.reward) == "table" then
                    for k = 1, #subTable.reward do
                        if subTable.reward[k][1] == 19 then
                            nAchiePoint = nAchiePoint + subTable.reward[k][2]
                            if subTable.status ~= nil then
                                if subTable.status <= 0 then
                                    nNotFinishAchiePoints = nNotFinishAchiePoints + subTable.reward[k][2]
                                elseif subTable.status == 3 then
                                    nHaveGetPoints = nHaveGetPoints + subTable.reward[k][2]
        ...
        self.m_tAchieMentList[i].totalCount = totalCount
        self.m_tAchieMentList[i].completeCount = completeCount
...
function WndDesignationMain:updateDesiShow(id)
    ...
    if self.m_nclickedMainClassicId == 1 then 
        local tbVipDesi =  GetElement(self.m_root, "tbVipDesi_WndDesignationMain", WZUITableContainer)
        local element = tbVipDesi:getCellElement(self.m_nclickedDesignatin)
        element = WZUIContainer:luaTo(element)
        local cellItem = element:getChildElement("__CellDesignationThree")
        element_obj = WZUIContainer:luaTo(cellItem):getLuaObjectIndex()
        cellId = element_obj:getJobId( )
    else
        local subFreeList =  GetElement(self.m_root, "subFreeList_WndDesignationMain", WZUIFreeListContainer)
        local element =  subFreeList:getAt(self.m_nclickedDesignatin)
        element = WZUIContainer:luaTo(element)
        element_obj = element:getLuaObjectIndex()
        cellId = element_obj:getJobId( )
    end
...
--brief     分贞加载称号列表
function WndDesignationMain:onShowDesi(tCurLoadList)
    -- body
    local element = GetElement(self.m_root, "tbVipDesi_WndDesignationMain", WZUITableContainer)
    element:cleanTable()
    if tCurLoadList == nil or #tCurLoadList == 0 then
        return
    end
...
function WndDesignationMain:updateDesilist( )
    ...
    if self.m_nclickedMainClassicId ~= 14 then return end 
...
function WndDesignationMain:_updateDesiRed()
    ...
    if bIsVisible == true and cellId == 14 then
        local  subFreeList =  GetElement(self.m_root, "subFreeList_WndDesignationMain", WZUIFreeListContainer)
...
function WndDesignationMain:onBadgeUpgradeOk(achievementPort, genre, level, maxLevel, attribute, itemId, itemNum)
    ...
    for i= 1, badgeFreeList:size() do
        ...
        if nTag == genre - 1 then
            ...
            local spinePath = "ui/otherUI/ui_chengjiu_huizhang"
...
function WndDesignationMain:updateBadgeList()
    ...
    for i= 1, #self.m_tBadgeList do
        ...
        elseif ProjConfig.LANGUAGE == "pt" or ProjConfig.LANGUAGE == "tr" or ProjConfig.LANGUAGE == "en" then
            local txtFullLevel = GetElement(element,"txtFullLevel_CellBadgePanelItem",WZUILabelTTF)
            txtFullLevel:setRelativePosition(GlobalMethod:ccp(0.5,0.138))
            txtFullLevel:setDimensions(GlobalMethod:CCSize(240))
        end
    end
...
function WndDesignationMain:_initTopInfo()
    ...
    --当前剩余成就点
    local sCurrentFormat = [[<T C="127,70,26" S="20" P="1">%s</T><I Z="0.5" P="1">%s</I><T C="229,105,22" S="20" P="1">%s</T>]]
    ...
    --总进度
    local sCurrentFormat2 = [[<T C="127,70,26" S="20" P="1">%s</T><T C="229,105,22" S="20" P="1">%d/%d</T>]]
    ...
    if ProjConfig.LANGUAGE == "ug" then
        --当前剩余成就点
        local sCurrentFormat = [[<T C="128,54,13" S="20" P="1">%s</T><I Z="0.5" P="1">%s</I><T C="105,65,46" S="20" P="1">%s</T>]]
        local ftxtLeftAchieNum = GetElement(self.m_root, "ftxtLeftAchieNum_WndDesignationMain", WZUIFreeTextBox)
        ftxtLeftAchieNum:setShowText(string.format(sCurrentFormat, self.m_nLeftAchiePoints, GDatatab_item["id_19"].icon, LocalStrings.LEFT_ACHIE_POINTS))
        --总进度
        local sCurrentFormat2 = [[<T C="128,54,13" S="20" P="1">%d/%d</T><T C="105,65,46" S="20" P="1">%s</T>]]
        local ftxtTotalProgress = GetElement(self.m_root, "ftxtTotalProgress_WndDesignationMain", WZUIFreeTextBox)
        local sTotalPro = string.format(sCurrentFormat2, self.m_nTotalAchiePoints, self.m_nFinishAchiePoints, LocalStrings.TOTAL_PROGRESS)
        ftxtTotalProgress:setShowText(sTotalPro)
    end
end
...
function WndDesignationMain:_setBadgeCell(element, nType, nLevel)
    ...
    local txtLabel_02 = GetElement(element,"txtLabel_02",WZUILabelTTF)
    txtLabel_02:setText(sProperty .. ":")
    GetElement(element, "txtLevelNum_CellBadgePanelItem",WZUILabelTTF):setText("+" .. tostring(tCurLevelData.add_attribute))
    local sTitle = sProperty .. LocalStrings.DOWNLOADREWARD_BADGE
    local txtTitle = string.format([[<T S = "22" C = "255,255,255" SC="132,66,29" SE="1" SS="4">Lv%d </T><T S = "22" C = "255,236,193" SC="132,66,29" SE="1" SS="4">%s</T>]], nLevel, sTitle)
    ...
    local txtLabel_01 = GetElement(element,"txtLabel_01",WZUILabelTTF)
    txtLabel_01:setText(LocalStrings.LEVEL..":")
    GetElement(element, "txtLevelWord_CellBadgePanelItem", WZUILabelTTF):setText("Lv" .. nLevel)
...
--@brief    设置开箱特效
function WndDesignationMain:setSpineAni(element)
    local spineUpgrade = GetElement(element,"spineUpgrade_CellBadgePanelItem",WZUISpine)
    local spinePath = "ui/otherUI/ui_chengjiu_huizhang"
    local bIsExist = CheckEffectFile(spinePath)

    if bIsExist then 
        spineUpgrade:setFileJson(spinePath .. ".json")
        spineUpgrade:setFileAtlas(spinePath .. ".atlas")
    end
end
...
function WndDesignationMain:_adaptLanguage_vn(  )
    ...
    --     -- txtMainTitle:setRelativePosition(GlobalMethod:ccp(-0.05,0.5))
    -- end

    -- local txt = GetElement(self.m_root,"txtAchiePointNum_WndDesignationMain",WZUILabelTTF)
    -- if txt then
    --     txt:setDimensions(GlobalMethod:CCSize(390,0))
    --     txt:setFontSize(18)
    -- end
end
...
function WndDesignationMain:_adaptLanguage_ug()
    local txtMainTitle = GetElement(self.m_root, "txtMainTitle_WndDesignationMain", WZUILabelTTF)
    if txtMainTitle then
        txtMainTitle:setScale(0.7)
        txtMainTitle:setRelativePosition(GlobalMethod:ccp(-0.008,0.5))
        txtMainTitle:setDimensions(GlobalMethod:CCSize(280))
    end

    local ftxtTotalProgress = GetElement(self.m_root,"ftxtTotalProgress_WndDesignationMain",WZUIFreeTextBox)
    ftxtTotalProgress:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    ftxtTotalProgress:setRelativePosition(GlobalMethod:ccp(0.97,0.680555))
    local ftxtLeftAchieNum = GetElement(self.m_root,"ftxtLeftAchieNum_WndDesignationMain",WZUIFreeTextBox)
    ftxtLeftAchieNum:setAnchorPoint(GlobalMethod:ccp(1,0.5))
    ftxtLeftAchieNum:setRelativePosition(GlobalMethod:ccp(0.97,0.347222))

    GetElement(self.m_root, "txtBtnText_WndDesignationMain", WZUILabelTTF):setScale(0.7)

    local button_nor_Label = GetElement(self.m_root,"button_nor_Label",WZUILabelTTF)
    button_nor_Label:setScale(0.7)
    button_nor_Label:setDimensions(GlobalMethod:CCSize(120))
end
-------------------------------------语言适配模块End----------------------------------------
