...
-------------------------------------私有方法模块Begin--------------------------------------
--使用物品的id进行保存
function WndBagMain:setUseSaveItemId(id)
	self.m_nSaveItemId = id
end
function WndBagMain:getUseSaveItemId()
	return self.m_nSaveItemId
end
--@brief	进入背包装备
...
--@brief	进入背包
function WndBagMain:showBeibao()
	WZLog("WndBagMain:showBeibao")
	...
	if self.m_root == nil then
		local wndBagElement = WndBagMain:createElement()
		WndBagMain.jumpTag = "beibao"
		WindowManager:addWindow(wndBagElement, WndBagMain, nil, nil, true)
	end
end

--@brief	进入突破
function WndBagMain:showBreak()
	WZLog("WndBagMain:showBreak")
