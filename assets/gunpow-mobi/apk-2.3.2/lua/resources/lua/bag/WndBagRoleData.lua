...
--@brief 	设置经验兑换按钮是否显示
function WndBagRole:setExchangeExp()
	-- body
...
--@brief	进入背包合成
--@brief	tabIndex进入标签
--@param 	bReCreate:是否重新创建界面
function WndBagRole:showBagSynthesis(tabIndex, synId, bReCreate)
	WZLog("WndBagRole:showBagSynthesis",tabIndex)
	if self.m_root == nil or bReCreate then
...
--@brief	tabIndex进入标签
--@param 	bReCreate:是否重新创建界面
function WndBagRole:showBagSell(tabIndex, bReCreate)
	WZLog("WndBagRole:showBagSell",tabIndex)
	if self.m_root == nil or bReCreate then
		WndBagMain:showBeibao()
	end
	...
	if tabIndex == nil then tabIndex = 1 end
	self.sellIndex = tabIndex
	leftCon:enableSchedule("showBagSellCall",0.1)
end

--@brief	延时进入回收
function WndBagRole:showBagSellCall()
	WZLog("WndBagRole:showBagSellCall")
	if WndItemInfo.m_root ~= nil then return end
   	local leftCon = GetElement(self.m_root,"conLeft_WndBag",WZUIContainer)
	leftCon:disableSchedule()
	WndEquipNew:onSaleClick()

	WndSell:updateTabIndex(self.sellIndex)
end
