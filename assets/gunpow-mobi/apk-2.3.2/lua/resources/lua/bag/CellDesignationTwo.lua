...
function  CellDesignationTwo:setCellStatus(status)
    ...
    local descFormat = [[<T C="127,70,26" S="22" P="1">%s</T><T C="229,105,22" S="22" P="1">%s</T>]]
...
function CellDesignationTwo:_update(title ,desc, status, reward, nComplete, nTarget)
    ...
    if title ~= nil then
        ...
        local tempPoint = GlobalMethod:ccp(0.63,0.85)
...
function CellDesignationTwo:_updateDesi()
    ...
    --描述
    local descFormat = [[<T C="127,70,26" S="22" P="1">%s</T>]]
    ...
    if tData.status == 0 then
        ...
        if tData.title ~= nil then
            ...
            CreateDesiSpine(conTitle, txtDesName, tData.title, tempPoint,nil,0.85)
        end
    end
...
function CellDesignationTwo:_adaptLanguage_ug(  )
    local txtDesName = GetElement(self.m_root, "txtDesName_CellDesignationTwo", WZUILabelTTF)
    if txtDesName then
    ...
    local ftxtDesc = GetElement(self.m_root,"ftxtDesc_CellDesignationTwo",WZUIFreeTextBox)
    ftxtDesc:setScale(0.8)
    ftxtDesc:setMaxWidth(380)

    GetElement(self.m_root,"txtStateWord_CellDesignation",WZUILabelTTF):setScale(0.7)
