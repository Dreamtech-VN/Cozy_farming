...
--@note		在这里做场景进入前的准备工作
function CellRebateMini:onEnter(element)
	self.m_root = element
end
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function CellRebateMini:onExit(element)
	self:_unInit()
end
-------------------------------------公有方法模块End----------------------------------------
...
-------------------------------------私有方法模块Begin--------------------------------------
-- 更新商品的信息
function CellRebateMini:_update()
	WZLog("CellRebateMini:_update", Serialize(self.tData))
    ...
    tcell:_showItemNum()
    if tcell.m_imgItem ~= nil then
        tcell.m_imgItem:setScale(0.85)
    end
    if tcell.m_txtCount ~= nil then
        tcell.m_txtCount:setRelativePosition(ccp(0.92,0.12))
        tcell.m_txtCount:setScale(0.85)
    end
...
function CellRebateMini:OnBtnBuy(element)
    WZLog("---------CellRebateMini:OnBtnBuy------------")
    ...
    WndItemInfo:showInfo(self.m_root,WndApartmentAct.m_root,1,itemInfo,true,nil,nil,other)
	WndItemInfo:setClickButtonCallback(self,self.onClickbuyBtn)
end

--@brief	点击购买回调
function CellRebateMini:onClickbuyBtn()
	--判断是否够钱
	local cost = math.ceil(self.tData.price*self.tData.discount/10)
	if self.tData.discount == 0 then
		cost = self.tData.price
	end
	WndApartmentAct.buyId10 = self.tData.id
	WZLog("CellRebateMini:onClickbuyBtn",self.tData.costId,cost)
	if not JudgeMoneyIsEnough(self.tData.costId, cost, nil, nil, GlobalGame.g_nCurrentUIChannelId, nil, nil, nil, nil, WndApartmentAct, WndApartmentAct.clickSureMoney) then 
