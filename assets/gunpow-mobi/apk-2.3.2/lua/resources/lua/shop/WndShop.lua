...
function WndShop:onEnterTransitionDidFinish(element)
    ...
    self.m_nodeConLeft5 = GetElement(self.m_root, "conLeft5_WndShop", WZUIContainer)
    self.m_nodeConLeft5:enableSchedule("caculateOutSaleTime", 1)
    -- 播放闪光动画
    self:playRoleAni()
...
function WndShop:onExit(element)
    ...
    if self.m_nodeConLeft5 then 
        self.m_nodeConLeft5:disableSchedule()
    end
...
--@brief    刷新兑换列表
function WndShop:reflashExchangeList()
    --body
...
--@brief    检测兑换商品下架时间
function WndShop:caculateOutSaleTime(element, delta)
    -- body
    if self.propDescData == nil then return end 
    if self.propDescData[5] == nil then return end

    local timeOutId = {}
    local needUpdate = {false, false, false}
    for i, value in pairs(self.propDescData[5]) do
        for j, v in pairs(value) do
            
			if v then
                if timeOutId[v.subType] == nil then timeOutId[v.subType] = {} end
                local discountTime = v.initData.discountTime or 0
                if v.initData and tonumber(discountTime) > 0 and tonumber(discountTime) <= tonumber(SystemTime:getServerTime()) then 
                    table.insert(timeOutId[v.subType], v.initData.id)
                    CacheCenter:updateExchangeGoodsInfo(2, v.initData.id, v.initData.limitLeave, false)
                    needUpdate[v.subType] = true
                end            end
        end
    end

    for i = 1, #timeOutId do
        if timeOutId[i] then 
            for j = 1, #timeOutId[i] do
                for k, value in pairs(self.propDescData[5][i]) do
                    if value.initData.id == timeOutId[i][j] then 
                        table.remove(self.propDescData[5][i], k)
                        break
                    end           
                end
                for n = 1, #self.propData do
                    if self.propData[n].id == timeOutId[i][j] then 
                        table.remove(self.propData, n)
                    end
                end
            end
        end
    ...
    if self.leftIndex == 5 then 
        if needUpdate[self.top5Index] then 
            self:reflashExchangeList()
        end
    end
end
-------------------------------------私有方法模块End----------------------------------------
...
function WndShop:onBtnClickRecommend(element)
...
function WndShop:onTempTab(element)
    ...
    if self.m_sTxtCheckLeft ~= nil then
        self.m_sTxtCheckLeft:setEnableStroke(false)
        self.m_sTxtCheckLeft:setColor(GlobalMethod:ccc3(127,70,26))
    end
    local txtCheck = GetElement(self.m_root,"txtCheck"..tag.."_WndShop",WZUILabelTTF)
    txtCheck:setColor(GlobalMethod:ccc3(255,236,193))
    txtCheck:setEnableStroke(true)
    txtCheck:setStrokeColor(GlobalMethod:ccc3(132,66,29))
    txtCheck:setStrokeSize(4)
    self.m_sTxtCheckLeft = txtCheck

    local imgBg_WndShop = GetElement(self.m_root,"imgBg_WndShop",WZUIImage)
    if tag == 1 or tag == 2 or tag == 4 or tag == 6 then
        imgBg_WndShop:setFile("ui/common_bg/common_pic_sc.png")
    else
        imgBg_WndShop:setFile("ui/common_bg/common_bg.png")
    end
	--切换到标签一重新获取热销商品
...
function WndShop:onTopDressTitle(element)
    ...
    if tag == 4 then 
        PostPlayerEvent:postEvent(PostPlayerEvent.event_tenLvClickBodyTab)
    end
...
function WndShop:_createShopItemList(tabCon,tabData,tabCell)
    ...
    WZLog("-------curRightData",self.leftIndex,self.dressTopIndex)
    ...
    elseif leftIndex == 2 and self.dressTopIndex == 4 and isEndTeach26 ~= true and CacheCenter:getPlayerInfo().level <= 10 and  (TeachGroup1:isTaskTeachFinish(TeachGroup1.TASK_ID_7) or TeachGroup1.ISTEACHMODE) then
        ...
        if isExist then
            ...
            if index == 1 then
                ...
                img:setRelativePositionLuaTo(0.56,0.64)
            elseif index == 2 then
                
                img:setRelativePositionLuaTo(0.76,0.64)
            elseif index == 3 then
                
                img:setRelativePositionLuaTo(0.96,0.64)
            elseif index == 4 then
                
                img:setRelativePositionLuaTo(0.56,0.24)
            elseif index == 5 then
                img:setRelativePositionLuaTo(0.76,0.24)
            elseif index == 6 then
                
                img:setRelativePositionLuaTo(0.96,0.24)
            end
...
function WndShop:_updateLeft(tag) 
	...
	GetElement(self.m_root,"conLeftRoleBg",WZUIContainer):setVisible(false)
	GetElement(self.m_root,"conLeftRole",WZUIContainer):setVisible(showRole[tag])
end

function WndShop:onClickbuyBtn(tData) 
	
	WndShop:showShopInterfaceByTag(tData.basicInfo.id,3,tData.id)
end
...
function WndShop:onClick4Call() 
    ...
    local vprice = WZLuaVector_int_:create()
	count:push(0)
	mallId:push(self.m_tItem4.id)
    vprice:push(self.m_tItem4.price)
	ProtocolProcessorWndShop:send_MALL_BuyItems(count, mallId, 1, 1, nil, nil, vprice)
end
...
function WndShop:buyLeft4(tag, tData) 
	...
	WndShop:showShopInterfaceByTag(tData.basicInfo.id, 3, tData.id, true)
end
...
function WndShop:_countDown4() 
	...
	GetElement(self.m_root,"txtLeftTime4",WZUILabelTTF):setText(h..":"..m..":"..s)
end
-------------------------------------特价限购模块End----------------------------------------
...
function WndShop:_adaptLanguage_ug(  )
    local txtCheck1 = GetElement(self.m_root,"txtCheck1_WndShop",WZUILabelTTF)
    local txtCheck2 = GetElement(self.m_root,"txtCheck2_WndShop",WZUILabelTTF)
    local txtCheck3 = GetElement(self.m_root,"txtCheck3_WndShop",WZUILabelTTF)
    local txtCheck4 = GetElement(self.m_root,"txtCheck4_WndShop",WZUILabelTTF)
    ...
    local txtLeft1 = GetElement(self.m_root,"txtLeft1_WndShop",WZUILabelTTF)
    txtLeft1:setScale(0.6)
    txtLeft1:setDimensions(GlobalMethod:CCSize(160,0))
    txtLeft1:setRelativePosition(GlobalMethod:ccp(0.16859,0.942))
    local txtLeft2 = GetElement(self.m_root,"txtLeft2_WndShop",WZUILabelTTF)
    txtLeft2:setScale(0.6)
    txtLeft2:setDimensions(GlobalMethod:CCSize(160,0))
    txtLeft2:setRelativePosition(GlobalMethod:ccp(0.16859,0.942))
    local txtLeft6 = GetElement(self.m_root,"txtLeft6_WndShop",WZUILabelTTF)
    txtLeft6:setScale(0.6)
    txtLeft6:setDimensions(GlobalMethod:CCSize(160,0))
    txtLeft6:setRelativePosition(GlobalMethod:ccp(0.16859,0.942))

    GetElement(self.m_root,"txtSell_WndSellRune",WZUILabelTTF):setScale(1)
    GetElement(self.m_root,"txtBtnSell2_WndSellRune",WZUILabelTTF):setScale(1)
    GetElement(self.m_root,"txtBtnSell4_WndSellRune",WZUILabelTTF):setScale(1)

    GetElement(self.m_root,"txtConLeft3_WndShop",WZUILabelTTF):setScale(0.8)

    local txtConLeft4 = GetElement(self.m_root,"txtConLeft4_WndShop",WZUILabelTTF)
    txtConLeft4:setScale(0.8)
    txtConLeft4:setDimensions(GlobalMethod:CCSize(360))
    txtConLeft4:setRelativePosition(GlobalMethod:ccp(0.5,0.94825))

    local txtLeftTime4 = GetElement(self.m_root,"txtLeftTime4",WZUILabelTTF)
    txtLeftTime4:setScale(0.62)
    txtLeftTime4:setRelativePosition(GlobalMethod:ccp(0.0404028,0.860675))

    GetElement(self.m_root,"discount4",WZUILabelTTF):setScale(0.6)
    local txtName41 = GetElement(self.m_root,"txtName41_WndShop",WZUILabelTTF)
    txtName41:setScale(0.6)
    txtName41:setDimensions(GlobalMethod:CCSize(160))
    txtName41:setRelativePosition(GlobalMethod:ccp(0.33,0.5))
    local txtName42 = GetElement(self.m_root,"txtName42_WndShop",WZUILabelTTF)
    txtName42:setScale(0.6)
    txtName42:setDimensions(GlobalMethod:CCSize(160))
    txtName42:setRelativePosition(GlobalMethod:ccp(0.33,0.5))
    GetElement(self.m_root,"txtTip4",WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.54,0.361128))

    local txtLeft6Btn1 = GetElement(self.m_root,"txtLeft6Btn1_WndShop",WZUILabelTTF)
    txtLeft6Btn1:setScale(0.8)
    txtLeft6Btn1:setDimensions(GlobalMethod:CCSize(150))
    local txtLeft6Btn2 = GetElement(self.m_root,"txtLeft6Btn2_WndShop",WZUILabelTTF)
    txtLeft6Btn2:setScale(0.8)
    txtLeft6Btn2:setDimensions(GlobalMethod:CCSize(150))
end
-------------------------------------语言适配End----------------------------------------
