...
function WndShopSubCheck:_update() 
	...
	elseif WndShop.leftIndex == 7 then
		GetElement(self.m_root,"tab2_WndShop",WZUICheckBox):setRelativePosition(ccp(0,0.38))
		GetElement(self.m_root,"tab3_WndShop",WZUICheckBox):setRelativePosition(ccp(0.16,0.38))
	elseif WndShop.leftIndex == 8 then
		GetElement(self.m_root,"tab1_WndShop",WZUICheckBox):setRelativePosition(ccp(0.03,0.3))
		GetElement(self.m_root,"tab2_WndShop",WZUICheckBox):setRelativePosition(ccp(0.19,0.3))
		GetElement(self.m_root,"tab3_WndShop",WZUICheckBox):setRelativePosition(ccp(0.35,0.3))
		GetElement(self.m_root,"tab4_WndShop",WZUICheckBox):setRelativePosition(ccp(0.51,0.3))
		GetElement(self.m_root,"tab5_WndShop",WZUICheckBox):setRelativePosition(ccp(0.67,0.3))
		GetElement(self.m_root,"tab6_WndShop",WZUICheckBox):setRelativePosition(ccp(0.83,0.3))
		GetElement(self.m_root,"tab7_WndShop",WZUICheckBox):setRelativePosition(ccp(0.99,0.3))
	elseif WndShop.leftIndex == 5 then
		GetElement(self.m_root,"tab1_WndShop",WZUICheckBox):setRelativePosition(ccp(0.03,0.3))
		GetElement(self.m_root,"tab3_WndShop",WZUICheckBox):setRelativePosition(ccp(0.19,0.3))
		GetElement(self.m_root,"tab4_WndShop",WZUICheckBox):setRelativePosition(ccp(0.35,0.3))
	else
		GetElement(self.m_root,"tab1_WndShop",WZUICheckBox):setRelativePosition(ccp(0.82,0.3))
		GetElement(self.m_root,"tab2_WndShop",WZUICheckBox):setRelativePosition(ccp(0.98,0.3))
		GetElement(self.m_root,"tab3_WndShop",WZUICheckBox):setRelativePosition(ccp(1.14,0.3))
		GetElement(self.m_root,"tab4_WndShop",WZUICheckBox):setRelativePosition(ccp(1.3,0.3))
		GetElement(self.m_root,"tab5_WndShop",WZUICheckBox):setRelativePosition(ccp(1.46,0.3))
		GetElement(self.m_root,"tab6_WndShop",WZUICheckBox):setRelativePosition(ccp(1.62,0.3))
		GetElement(self.m_root,"tab7_WndShop",WZUICheckBox):setRelativePosition(ccp(1.78,0.3))
	end
	...
	if WndShop.leftIndex == 3 then
		...
		local nTotalNum = 1
		local tPositionY = {0.52, 0.68, 0.84, 1.0, 1.16, 1.32, 1.48}
		for i = 1, #tSortFuncIndex do
			local nIndex = tSortFuncIndex[i]
			if CheckButtonOpen(tFuncId[nIndex], false) then
				if CacheCenter:getGameParam().gameStatus ~= "1" or CacheCenter:getGameParam().gameStatus == "1" and nIndex ~= 6 then
					local tab = GetElement(self.m_root,"tab"..nIndex.."_WndShop",WZUICheckBox)
					tab:setVisible(true)
					tab:setRelativePosition(GlobalMethod:ccp(tPositionY[nTotalNum], 0.3))
...
function WndShopSubCheck:_adaptLanguage_ug(  )
	for i = 1, 7 do
		local txt1 = GetElement(self.m_root,"txt" .. i, WZUILabelTTF)
		local txt2 = GetElement(self.m_root,"txt" .. i .."_sel", WZUILabelTTF)
		txt1:setScale(0.6)
		txt1:setDimensions(GlobalMethod:CCSize(120))
		txt2:setScale(0.6)
		txt2:setDimensions(GlobalMethod:CCSize(120))
	end
end
