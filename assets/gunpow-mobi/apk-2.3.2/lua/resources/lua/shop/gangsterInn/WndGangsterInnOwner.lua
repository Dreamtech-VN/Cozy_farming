...
--@brief 	设置待机特效
function WndGangsterInnOwner:_setSpineAni()
	local spinePath = "ui/otherUI/skill_srcx_01"
	local existSpine = CheckEffectFile(spinePath)
	if existSpine then 
		local spineLight = GetElement(self.m_root, "spineLight_WndGangsterInnOwner", WZUISpine)
		if spineLight then 
			spineLight:setFileJson(spinePath .. ".json")
			spineLight:setFileAtlas(spinePath .. ".atlas")
			spineLight:play("effects", true)
		end
	end
end
