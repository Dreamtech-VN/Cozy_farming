...
function CellGangsterInn:setData(tData)
	...
	WZLog("黑市商品折扣1",tData.disCount)
	
	if tData.disCount ~= nil and tData.disCount ~= 100 then
		local discoun = tData.disCount/10
		WZLog("黑市商品折扣2",discoun)
		GetElement(self.m_root,"conDiscount_CellGangsterInn",WZUIContainer):setVisible(true)
		GetElement(self.m_root,"labCnt_CellGangsterInn",WZUILabelAtlasFont):setText(discoun)
		-- GetElement(self.m_root,"labCnt_CellGangsterInn1",WZUILabelTTF):setText(discoun)
	else
		GetElement(self.m_root,"conDiscount_CellGangsterInn",WZUIContainer):setVisible(false)
	end
...
function CellGangsterInn:_adaptLanguage_vn()
    local txtDescript = GetElement(self.m_root,"txtDescript_CellGangsterInn",WZUILabelTTF)
    txtDescript:setDimensions(GlobalMethod:CCSize(150))
    txtDescript:setFontSize(14)
end
