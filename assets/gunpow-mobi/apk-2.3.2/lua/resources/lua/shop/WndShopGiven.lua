...
function WndShopGiven:onGive()
    ...
    if self.types == 1 then
        if self.price2 > 0 then --有越南粉钻
            if JudgeMoneyIsEnough(177,self.price2) == false then
                return
            end
            if self.price > 0 then --蓝钻
    ...
    price:push(self.price)

    WZLog("--------OnGive--------------",self.types, mallId, count, self.selFriend.playerId, str)
    ProtocolProcessorWndShop:send_MALL_MallOperate(self.types, mallId, count, self.selFriend.playerId, str, price)
end
...
function WndShopGiven:_updateGivenText()
    ...
    if self.price2 > 0 then --有越南粉钻
        if #self.data == 1 then --只有越南粉钻
            shopDesc = string.format(LocalStrings.SHOP_DESC2,self.price2)
            shopDesc = string.gsub(shopDesc,"ui/common/common_icon_zuanshi.png","shopitems/lizuan.png")
        else --有越南粉钻和蓝钻
            shopDesc = string.format(LocalStrings.SHOP_DESC2,self.price)

            local strFormat = [[<I Z="0.6">shopitems/lizuan.png</I><T C="255,227,116" S="22" P="1" SC="79,60,48" SE="1" SS="4">%d</T>]]
            local otherPriceInfo = string.format(strFormat,self.price2)
            shopDesc = shopDesc .. otherPriceInfo
        end
    else
        local moneyType = self.data[1]["initData"]["moneyId"]
        shopDesc = string.format(LocalStrings.SHOP_DESC2,self.price)
...
function WndShopGiven:_adaptLanguage_ug(  )
    local txtBtn =  GetElement(self.m_root, "txtBtn_WndShopGiven", WZUILabelTTF)
    txtBtn:setScale(0.7)
    txtBtn:setDimensions(GlobalMethod:CCSize(160))

    GetElement(self.m_root,"ftbBuyDesc_WndShopGiven",WZUIFreeTextBox):setScale(0.65)

    local txtGivenDesc = GetElement(self.m_root,"txtGivenDesc_WndShopGiven",WZUILabelTTF)
    txtGivenDesc:setDimensions(GlobalMethod:CCSize(500))
    txtGivenDesc:setScale(0.7)
    txtGivenDesc:setRelativePosition(GlobalMethod:ccp(0.5,0.57))
end
------------------------------------语言适配End--------------------------------------------