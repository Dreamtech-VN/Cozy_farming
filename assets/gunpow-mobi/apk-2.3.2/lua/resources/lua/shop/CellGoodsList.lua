...
function CellGoodsList:event_SureBuyAgain()
    ...
    if isEndTeach26 ~= true and CacheCenter:getPlayerInfo().level <= 10 and  (TeachGroup1:isTaskTeachFinish(TeachGroup1.TASK_ID_7) or TeachGroup1.ISTEACHMODE) then
        PostPlayerEvent:postEvent(PostPlayerEvent.event_tenLvClickDress)
...
function CellGoodsList:_update()
    ...
    --下架倒计时
    self:_showOutSaleTime(self.m_root)
    self.m_root:enableSchedule("_showOutSaleTime", 1)
end
...
function CellGoodsList:_updateSuit() 
	...
	--套装品质
	GetElement(self.m_root,"RoleBg",WZUIImage):setFile(g_tShopItemQuality[quality+1])
...
--@brief    显示下架剩余时间
function CellGoodsList:_showOutSaleTime(element)
    -- body
    if self.m_root == nil then return end

    local txtLeftSaleTime = GetElement(self.m_root, "txtLeftSaleTime_CellGoodsList", WZUILabelTTF)
    if txtLeftSaleTime then 
        local discountTime = self.cellData.initData.discountTime or 0
        if self.cellData.mainType == 5 and discountTime > 0 then 
            local nSeconds = self.cellData.initData.discountTime - SystemTime:getServerTime()
            if nSeconds > 0 then 
                local timeContent = returnToTimeFormat_Day(nSeconds)
                txtLeftSaleTime:setText(timeContent .. LocalStrings.SHOP_EXCHANGE_TIME)
            else
                element:disableSchedule()
                txtLeftSaleTime:setText("")
            end
        else
            element:disableSchedule()
            txtLeftSaleTime:setText("")
...
function CellGoodsList:_adaptLanguage_ug()
    local labCnt = GetElement(self.m_root,"labCnt_CelllGoodsList",WZUILabelAtlasFont)
    labCnt:setRelativePosition(GlobalMethod:ccp(0.566842,0.185294))
    labCnt:setScale(0.75)
    local imgOff = GetElement(self.m_root, "imgOff_CelllGoodsList", WZUIImage)
