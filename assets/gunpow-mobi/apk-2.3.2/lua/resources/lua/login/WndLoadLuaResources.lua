...
function WndLoadLuaResources:downloadAniConfigCall_All(taskId, path, totalSize, nowSize, finish, failed)
    WZLog("WndLoadLuaResources:downloadAniConfigCall_All", taskId, path, totalSize, nowSize, finish, failed)
    --检测资源下载
    local platForm =  WZUISystem:getInstance():getPlatformInfo()
    if platForm == 3 then 
...
function WndLoadLuaResources:updateTip()
    ...
    local tips = LocalStrings.LOADING_TIPS
    if isChannelPC() then 
        tips = LocalStrings.QQHALL_TEXT1[2]
    end
    ...
    local nIndex = math.random(1, #tips)
    if txtTipCnt:getText() == tips[nIndex] then
        nIndex = nIndex+1
        if nIndex > #tips then nIndex = 1 end
    end
    txtTipCnt:setText(LocalStrings.TIPS..":"..tips[nIndex])
end
...
function WndLoadLuaResources:_initUI()
    self:_initText()
    GDatatab_item = CopyTable(GDatatab_item_0)
    for i, value in pairs(GDatatab_item_1) do
        GDatatab_item[i] = value
    end
    --勋章表
    GDatatab_activity_medal = CopyTable(GDatatab_activity_medal_0)
    for i, value in pairs(GDatatab_activity_medal_1) do
        GDatatab_activity_medal[i] = value
    end
end
