...
function WndDownLoad:onEnter(element)
    ...
    if PassportSdkManager and PassportSdkManager.postGameInfoVn then
        PassportSdkManager:postGameInfoVn("cdn_check_start","")--开始检查资源是否更新
    end
...
function WndDownLoad:checkUpdateCallback(isNeedUpdate,url,count,totalSize)
    ...
    if PassportSdkManager and PassportSdkManager.postGameInfoVn then
        PassportSdkManager:postGameInfoVn("vms_request_succ","")--连接服务器成功
    end
...
function WndDownLoad:updateFinishCallback(hasError,errorCode)
	...
	if hasError then
        ...
        if self.downloadFlag == self.DownloadFlag.DOWNLOADFLAG_NORUPDATE then
            ...
            if PassportSdkManager and PassportSdkManager.postGameInfoVn then
                PassportSdkManager:postGameInfoVn("cdn_finish","")--完成下载热更
            end
            WZUpdateManager:getInstance():setUserData(1)
            KEngineReloadAll()--所有资源下载完毕重新加载游戏
            if PassportSdkManager and PassportSdkManager.postGameInfoVn then
                PassportSdkManager:postGameInfoVn("cdn_complete","")--完成解压热更
            end
...
function WndDownLoad:_startDownloadCallback(nId,nType)
	...
	if nType == MSGBOXRESTYPE_CONFIRM then
        ...
        local strTips = LocalStrings.NEED_DOWNLOAD_TIPS
        if WZUISystem:getInstance():getPlatformInfo() == 3 and LocalStrings.NEED_DOWNLOAD_TIPS_1 then
            strTips = LocalStrings.NEED_DOWNLOAD_TIPS_1
        end
        local txtTips = string.format(strTips,self.downloadSize)
		MsgBoxManager:showConfirmBox(txtTips, self, self._startDownloadCallback, MSGBOXLEVEL_NORMAL, nil,true)
...
function WndDownLoad:_checkIsNeedDownloadObb()
    if self.m_sObbFilePath == nil or self.m_sObbFilePath == "" then
        self.m_sObbFilePath = WZDeviceInfo:getExpansionFileName()
    end
    local isObbExist = WZFileUtil:isFileExist(self.m_sObbFilePath)--(WZDeviceInfo:getExpansionFileName())
    WZLog("WndDownLoad:_checkIsNeedDownloadObb 1", tostring(isObbExist), WZDeviceInfo:getExpansionFileName(), self.m_sObbFilePath)
    ...
    if isObbExist == false then
        ...
        local fileSize = WZFileUtil:getFileSize(self.m_sObbFilePath)--(WZDeviceInfo:getExpansionFileName())
        ...
        if fileSize < ProjConfig.USE_DOWNLOAD_OBB then
            ...
            CCFileUtils:sharedFileUtils():setObbResourcePath(self.m_sObbFilePath)--(WZDeviceInfo:getExpansionFileName())
            self:_checkUpdate()
        end
    end
end
...
function WndDownLoad:_downLoadObb()
    local makeDownloadPath = self:_makeDirectory(self.m_sObbFilePath)--(WZDeviceInfo:getExpansionFileName())
    ...
    local fileName = (self.m_sObbFilePath)--WZDeviceInfo:getExpansionFileName()
...
function WndDownLoad:downLoadObbBackFun(taskId, path, totalSize, nowSize, finish, failed)
    ...
    if failed == true then
        local freeSpace = WZFileUtil:getFreeSpace(self.m_sObbFilePath)--(WZDeviceInfo:getExpansionFileName())
