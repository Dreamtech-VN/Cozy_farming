...
function WndPets:_init()
  ...
  self.m_nSpecifyIndex = 1          --指定的功能索引：1->宠物；2->坐骑；3->足迹；4->幻化      
  self.m_nSubIndex = nil              --二级标签功能索引
  self.petAni = nil                   --宠物动画
  self.conPetRight1 = nil

  self.m_nMountChildIndex = nil
  self.m_tMountChileTitle = nil
  self.m_nPhantomChildIndex = nil
  self.m_tPhantomChileTitle = nil
  self.m_nPetChildIndex = nil
  self.m_tPetChileTitle = nil
  self.m_nFootChildIndex = nil
  self.m_tFootChileTitle = nil
...
function WndPets:_unInit()
  ...
  self.m_nSpecifyIndex = nil          --指定的功能索引：1->宠物；2->坐骑；3->足迹；4->幻化      
  self.m_nSubIndex = nil              --二级标签功能索引
  self.petAni = nil                   --宠物动画
  self.conPetRight1 = nil
...
--外部接口
--@param  nIndex:一级标签索引
--@param  nSubIndex:二级标签索引
function WndPets:showInterface(nIndex,nSubIndex)
    -- body
    local wndPets = WndPets:createElement()
    if wndPets ~= nil then
        self.m_nSpecifyIndex = nIndex 
        self.m_nSubIndex = nSubIndex
        WindowManager:addWindow(wndPets, WndPets, false)
    end
end
-------------------------------------公有方法模块End----------------------------------------
...
function WndPets:GetAllPetListOk(itemId, name, icon,animation, advancedLevel, upgradeLevel, property, giftSkill, commonSkill1, commonSkill2, isInUsed, playerPetId,num,petExp,fighting,birthSkill,skill, petSkinItemId, fetterStatus)
    ...
    if WndAscending.m_root then
      if WndAscending.m_nCurTab == 4 then
