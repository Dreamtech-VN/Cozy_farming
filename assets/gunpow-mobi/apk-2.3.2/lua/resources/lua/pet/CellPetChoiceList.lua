...
--@brief    转移界面的初始化
function CellPetChoiceList:_initTransferChoiceList()
    GetElement(self.m_root,"imgIconBg_CellPetChoiceList",WZUIImage):setFile(self.t_PetInfo.icon)
    local element =  GetElement(self.m_root,"imgIconBgQuality_CellPetChoiceList",WZUIImage)
    local quality = GDatatab_item["id_"..self.t_PetInfo.itemId].quality
    WndPets:setIconQuality(element, quality)
...
--@brief    转移界面选中被转移宠物后回调
function CellPetChoiceList:_doClickTransfer()
    -- body
    WndPetSkillTransfer:clickPetForTransferCallback(self.t_PetInfo)
end
...
function CellPetChoiceList:_adaptLanguage_ug(  )
    GetElement(self.m_root,"txtName_CellPetChoiceList",WZUILabelTTF):setScale(0.6)
end
