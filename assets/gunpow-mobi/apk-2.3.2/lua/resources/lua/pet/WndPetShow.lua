...
function WndPetShow:_initShow()
  ...
  self:_showPurplePet(petId)
  if quality>=3 then
    GetElement(self.m_root,"conPetz1_WndPetShow",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.175,0.71))
    GetElement(self.m_root,"conPetz2_WndPetShow",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.5,0.71))
    GetElement(self.m_root,"conPetz3_WndPetShow",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.825,0.71))
    GetElement(self.m_root,"conPetz4_WndPetShow",WZUIContainer):setVisible(true)
    GetElement(self.m_root,"conPetz5_WndPetShow",WZUIContainer):setVisible(true)

    GetElement(self.m_root,"imgArrow1_WndPetShow",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.32,0.71))
    GetElement(self.m_root,"imgArrow2_WndPetShow",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.68,0.71))
    GetElement(self.m_root,"imgArrow3_WndPetShow",WZUIImage):setVisible(true)
    local petId = GDatatab_item["id_"..self.petId].quality == 4 and self.petId or self.petId+20000
    self:_showOrangePet(petId)
  else
    GetElement(self.m_root,"conPetz1_WndPetShow",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.175,0.5))
    GetElement(self.m_root,"conPetz2_WndPetShow",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.5,0.5))
    GetElement(self.m_root,"conPetz3_WndPetShow",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.825,0.5))
    GetElement(self.m_root,"conPetz4_WndPetShow",WZUIContainer):setVisible(false)
    GetElement(self.m_root,"conPetz5_WndPetShow",WZUIContainer):setVisible(false)

    GetElement(self.m_root,"imgArrow1_WndPetShow",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.32,0.5))
    GetElement(self.m_root,"imgArrow2_WndPetShow",WZUIImage):setRelativePosition(GlobalMethod:ccp(0.68,0.5))
    GetElement(self.m_root,"imgArrow3_WndPetShow",WZUIImage):setVisible(false)
  end
end
...
function WndPetShow:_showPurplePet(petId)
  self.m_State = 0
  local showName = {LocalStrings.PETSHOWNAME1,LocalStrings.PETSHOWNAME2,LocalStrings.PETSHOWNAME3,LocalStrings.PETSHOWNAME2,LocalStrings.PETSHOWNAME3}
  local aniName = {"","",""}
  local aniLevel = {2,4,6}
  ...
  for i =1, 6 do
	   GetElement(self.m_root,"txtDesc"..i.."_WndPetShow",WZUIFreeTextBox)
     :setShowText(showDesc[i])
  end
  -- --紫橙显示相关
...
function WndPetShow:_adaptLanguage_ug()
  for i =1, 6 do
      local txtDesc = GetElement(self.m_root,"txtDesc"..i.."_WndPetShow",WZUIFreeTextBox)
      txtDesc:setMaxWidth(1000)
      txtDesc:setScale(0.7)
  end
  local conButton = GetElement(self.m_root,"conButton_WndPetShow",WZUIContainer)
  conButton:setAbsContentSize(GlobalMethod:CCSize(360,50))
  conButton:updateRelativeSize()
end
-------------------------------------私有方法模块End----------------------------------------