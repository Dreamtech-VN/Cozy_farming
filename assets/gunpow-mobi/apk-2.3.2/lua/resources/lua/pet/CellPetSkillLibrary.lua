...
function CellPetSkillLibrary:_adaptLanguage_vn( )
	...
	txtLvK:setScale(0.8)
	txtLvK:setDimensions(GlobalMethod:CCSize(400,0))
end

function CellPetSkillLibrary:_adaptLanguage_ug( )
	local txtName = GetElement(self.m_root,"txtName1_WndPetsSkill",WZUILabelTTF)
	txtName:setRelativePosition(GlobalMethod:ccp(0.993,0.745754))
	txtName:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	txtName:setScale(0.8)
	local txtLvK = GetElement(self.m_root,"txtLvK1_WndPetsSkill",WZUILabelTTF)
	txtLvK:setRelativePosition(GlobalMethod:ccp(0.993,0.5573))
	txtLvK:setAnchorPoint(GlobalMethod:ccp(1,1))
	txtLvK:setScale(0.8)
	txtLvK:setDimensions(GlobalMethod:CCSize(500,0))
	txtLvK:setAlignment(kCCTextAlignmentRight)
end
-------------------------------------语言适配End------------------------------------------------