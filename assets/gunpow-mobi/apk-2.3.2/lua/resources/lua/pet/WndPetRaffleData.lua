...
function WndPetRaffle:_init()
  ...
  self.itemId = nil
  self.giftSkill = nil
  self.m_bIsTeach = true      --判断首次进界面时继续新手引导钻石抽奖
end
...
function WndPetRaffle:rafflePetsData(itemId,giftSkill)
  ...
  if #itemId == 1 then  --抽一个
    ...
    local conShelter = GetElement(self.m_root,"conShelter_WndPetRaffle",WZUIContainer) --遮罩
    conShelter:setVisible(true)
    local spineEgg = GetElement(self.m_root,"spineEgg_WndPetRaffle",WZUISpine)
    spineEgg:setVisible(false)
    local myAni = GetElement(self.m_root,"armOpenEgg_WndPetRaffle",WZArmature)
    myAni:setVisible(true)
    local myArmature = myAni:getArmature()
    myArmature:getAnimation():playByIndex(4)
    myAni:setAnimationFinishLuaFunction("showOnePet")
  else  --抽十个
    self:controlViewShow(1)
    ...
    local myArmature = myAni:getArmature()
    myArmature:getAnimation():playByIndex(4)
    myAni:setAnimationFinishLuaFunction("showTenPet")
    
  end
end

function WndPetRaffle:showOnePet()
    WZLog("WndPetRaffle:rafflePetsData",Serialize(self.itemId),self.n_type)
    local conShelter = GetElement(self.m_root,"conShelter_WndPetRaffle",WZUIContainer) --遮罩
    ...
    local cell,obj = CellPetRaffle:createElement()
    con:addChild(cell)
    local tab = GDatatab_item["id_"..self.itemId[1]]
    tab.aptitude = self.giftSkill[1]
    WZLog("HHHHHHHHHHH1:",self.giftSkill[1] )
    obj:setData(tab, self.n_type)
    -- obj:downAction()
    obj:showPet()
    if tab.quality >= 3 then --紫宠
...
function WndPetRaffle:showTenPet()
  local conShelter = GetElement(self.m_root,"conShelter_WndPetRaffle",WZUIContainer) --遮罩
  conShelter:setVisible(false)
  
  self:controlViewShow(2)
  self.m_bISAlter = false
  WndPetRaffle:tenPetCall1()
end
...
function WndPetRaffle:tenPetCall1()
	...
	self.m_root:enableSchedule("tenPetCall2")
