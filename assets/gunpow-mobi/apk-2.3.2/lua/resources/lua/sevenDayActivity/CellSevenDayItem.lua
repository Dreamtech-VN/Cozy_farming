...
function CellSevenDayItem:onClickCellItem(element)
    ...
    if self.m_tCallBack then
        self.m_tCallBack[2](self.m_tCallBack[1],self.m_nDayIndex)
    end
end
...
function CellSevenDayItem:isItemHighLighted(bState)
    ...
    if bState then 
        ...
        img_itemSelected:setFile("ui/common/common_btn_32.png")
...
--@brief    获取Id
function CellSevenDayItem:getId()
    -- body
    return self.m_nDayIndex
end
-------------------------------------公有方法模块End----------------------------------------
