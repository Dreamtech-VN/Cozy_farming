...
function WndBless:onEnter(element)
    ...
    if isEndTeach ~= true and teachStep < 4 and CacheCenter:getPlayerInfo().level == 24 then
...
--@brief    设置自动吞噬状态
function WndBless:_setAutoDevourStatus()
    local selCheckBox = GetElement(self.m_root, "selCheckBox_WndBless", WZUICheckBox)
    if CacheCenter:getPlayerInfo().level >= 30 then --等级大于等于30级,默认勾选自动吞噬
        selCheckBox:setCheckIndex(1)
    end
    self.m_nAutoDevourAll = selCheckBox:getCheckIndex()
end

--@brief    设置静态内容
function WndBless:_staticContent()
    local wishdraw = CacheCenter:getGameParam()["wishdraw"]
    local ids, nums = SplitItemString(wishdraw)
    for i=1,4 do
        local imgBtnCost = GetElement(self.m_root,"imgBtnCost"..i.."_WndBless",WZUIImage)
        local txtDrawCountTip = GetElement(self.m_root,"txtDrawCountTip"..i.."_WndBless",WZUILabelTTF)
        local itemInfo = GDatatab_item["id_"..ids[i]]
        imgBtnCost:setFile(itemInfo.icon)
        txtDrawCountTip:setText(nums[i])
    end
end

--@brief    免费抽奖
function WndBless:_showFreeTime()
    local txtCountDown1 = GetElement(self.m_root,"txtCountDown1_WndBless",WZUILabelTTF)
    local txtCountDown2 = GetElement(self.m_root,"txtCountDown2_WndBless",WZUILabelTTF)
    
    --按钮中
    local txtBtn3 = GetElement(self.m_root,"txtBtn3_WndBless",WZUILabelTTF)
    local conFengexian3 = GetElement(self.m_root,"conFengexian3_WndBless",WZUIContainer)
    local imgBtnCost3 = GetElement(self.m_root,"imgBtnCost3_WndBless",WZUIImage)
    local txtDrawCountTip3 = GetElement(self.m_root,"txtDrawCountTip3_WndBless",WZUILabelTTF)
    local imgRed1 = GetElement(self.m_root,"imgRed1_WndBless",WZUIImage)
    local txtFree3 = GetElement(self.m_root,"txtFree3_WndBless",WZUILabelTTF) --免费

    h = math.floor(self.m_nTime / 3600) 
    m = math.floor((self.m_nTime % 3600) / 60)
    s = math.floor((self.m_nTime % 3600) % 60)
    txtCountDown1:setText(string.format("%d:%d:%d",h,m,s))

    if self.m_nTime > 0 then
        txtCountDown1:setVisible(true)
        txtCountDown2:setVisible(true)

        txtBtn3:setVisible(true)
        conFengexian3:setVisible(true)
        imgBtnCost3:setVisible(true)
        txtDrawCountTip3:setVisible(true)
        imgRed1:setVisible(false)
        txtFree3:setVisible(false)

        txtCountDown1:enableSchedule("_freeCountdown",1)

    else
        txtCountDown1:setVisible(false)
        txtCountDown2:setVisible(false)

        txtBtn3:setVisible(false)
        conFengexian3:setVisible(false)
        imgBtnCost3:setVisible(false)
        txtDrawCountTip3:setVisible(false)
        imgRed1:setVisible(true)
        txtFree3:setVisible(true)
...
--@brief    免费抽奖倒计时
function WndBless:_freeCountdown(element)
    local txtCountDown1 = GetElement(self.m_root,"txtCountDown1_WndBless",WZUILabelTTF)
...
--@brief    点击抽奖
function WndBless:onClickDraw(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
    local tag = element:getTag()

    local wishdraw = CacheCenter:getGameParam()["wishdraw"]
    local ids, nums = SplitItemString(wishdraw)
    local nOwnCount = CacheCenter:getPlayerItemCountById(tonumber(ids[tag]))
    local txtDrawCountTip = GetElement(self.m_root,"txtDrawCountTip"..tag.."_WndBless",WZUILabelTTF)
    local costNum = tonumber(txtDrawCountTip:getText())

    if txtDrawCountTip:isVisible() then 
        if tag == 3 then 
            local isEndTeach, step = TeachGroup1:isTeachFinish(42)
            if isEndTeach ~= true then
                if nOwnCount >= 1 and CacheCenter:getMoneyList().blessMedal >= costNum then
        ...
        if not JudgeMoneyIsEnough(tonumber(ids[tag]), costNum, nil, nil, Chat_Channel_Bless) then 
            return
        end
    end
    self.m_nCurDrawType = tag-1
    ProtocolProcessorBless:send_PRAY_Pray(tag-1,self.m_nAutoDevourAll)
end
...
--@brief    自动吞噬按钮回调
function WndBless:onAutoDevourAll(element)
    local selCheckBox = GetElement(self.m_root, "selCheckBox_WndBless", WZUICheckBox)
    if selCheckBox:getCheckIndex() == 0 then
        self.m_nAutoDevourAll = 1
    else
        self.m_nAutoDevourAll = 0
    end
end
...
--@brief    祈福成功返回处理
function WndBless:blessSuccess(id, prayId, exp)
    local spineEffect = GetElement(self.m_root, "spineEffect_wndBless", WZUISpine)
    spineEffect:setVisible(true)
    spineEffect:play("stand",false)

    GetElement(self.m_root, "conShelter_WndBless", WZUIContainer):setVisible(true) --遮罩

    self.m_tDisplayBless = {}
    for i = 1, #id do
        local nId = prayId[i]
        ...
        table.insert(self.m_tDisplayBless, tTemp)
    end

    spineEffect:enableSchedule("_showBlessMsg", 2)

    TeachGroup1:endTeachStep({42,4})
end

--吞噬祈福成功时获得的祈福 成功
function WndBless:onceDevourOk(devourId, exp, prayId, ids, fighting)
    local conSwallowItem = GetElement(self.m_root, "conSwallowItem_WndBless", WZUIContainer)
    conSwallowItem:removeAllChildrenWithCleanup(true)
    
    local data = CopyTable(GDatatab_pray["id_"..prayId])
    data.basicInfo = CopyTable(GDatatab_item["id_"..data.item_id])
    data.curExp = exp
    data.name = data.basicInfo.name
    local cellElement, tNewObj = CellBlessItem:createElement()
    tNewObj:setData(data, 2, self.m_root)
    conSwallowItem:addChild(cellElement)
    local txtSwallowItem = GetElement(self.m_root, "txtSwallowItem_WndBless", WZUILabelTTF)
    txtSwallowItem:setText(string.format(LocalStrings.BLESS_DRAW_3,"Lv"..data.level..data.name))

    local conSwallowShow = GetElement(self.m_root, "conSwallowShow_WndBless", WZUIContainer)
    conSwallowShow:setVisible(true)

end

function WndBless:_showBlessMsg(element)

    ProtocolProcessorBless:send_PRAY_GetPrayMess( )

    local spineEffect = GetElement(self.m_root, "spineEffect_wndBless", WZUISpine)
    spineEffect:setVisible(false)
    spineEffect:disableSchedule()

    if #self.m_tDisplayBless == 1 then --单抽
        local conDrawItem = GetElement(self.m_root, "conDrawItem_WndBless", WZUIContainer)
        conDrawItem:removeAllChildrenWithCleanup(true)

        local cellElement, tNewObj = CellBlessItem:createElement()
        tNewObj:setData(self.m_tDisplayBless[1], 2, self.m_root)
        conDrawItem:addChild(cellElement)

        GetElement(self.m_root, "conDrawTip1_WndBless", WZUIContainer):setVisible(true)
    else --十连抽
        for i=1,#self.m_tDisplayBless do
            local conDrawItem = GetElement(self.m_root, "conDrawItem"..i.."_WndBless", WZUIContainer)
            conDrawItem:removeAllChildrenWithCleanup(true)

            local cellElement, tNewObj = CellBlessItem:createElement()
            tNewObj:setData(self.m_tDisplayBless[i], 2, self.m_root)
            conDrawItem:addChild(cellElement)
        end
        GetElement(self.m_root, "conDrawTip2_WndBless", WZUIContainer):setVisible(true)
    end

    GetElement(self.m_root, "conShelter_WndBless", WZUIContainer):setVisible(false) --遮罩

    local isEndTeach, step = TeachGroup1:isTeachFinish(42)
    if isEndTeach ~= true and step < 5 and CacheCenter:getPlayerInfo().level == 24 then
        TeachGroup1:startGroup({42,5,self.m_root})
    end
end
...
--@brief    单抽祈福完后点"确定"按钮
function WndBless:onClickConfirmOne(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
    GetElement(self.m_root, "conDrawTip1_WndBless", WZUIContainer):setVisible(false)


    local isEndTeach, step = TeachGroup1:isTeachFinish(42)
    if isEndTeach ~= true and step < 6 and CacheCenter:getPlayerInfo().level == 24 then
        TeachGroup1:startGroup({42,6,self.m_root})
    end
end

--@brief    单抽祈福完后点"再召一次"按钮
function WndBless:onClickAgainOne(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
    GetElement(self.m_root, "conDrawTip1_WndBless", WZUIContainer):setVisible(false)

    ProtocolProcessorBless:send_PRAY_Pray(self.m_nCurDrawType,self.m_nAutoDevourAll)
end


--@brief    十连祈福完后点"确定"按钮
function WndBless:onClickConfirmTen(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
    
    GetElement(self.m_root, "conDrawTip2_WndBless", WZUIContainer):setVisible(false)
    GetElement(self.m_root, "conSwallowShow_WndBless", WZUIContainer):setVisible(false)
end

--@brief    十连祈福完后点"再召一次"按钮
function WndBless:onClickAgainTen(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
...
-- --@brief    一键吞噬成功
-- function WndBless:devourAllOK(devourId, exp, prayId, ids)
    ...
    while tempElement do
        ...
        if tDataTemp.blessId == tData.blessId then
            ...
            if (WndBless.m_nTotal == 1 or WndBless.m_nTotal == 2) and (tagIndex+1==#WndBless.m_tBlessHallList) then
                ...
                local quickDevour = GetElement(WndBless.m_root,"quickDevour",WZUICheckBox)
                if quickDevour:getCheckIndex() == 1 then
                    WndBless:showQuickDevour()
                end
...
function WndBless:showQuickDevour( )
    local tDevourItem, bIsCanDevourAll = self:_findTheHeightestBlessItem()
    if tDevourItem.level < self:_getMaxLevel(tDevourItem.item_id) and bIsCanDevourAll then
        local name = "Lv" .. tDevourItem.level .. tDevourItem.basicInfo.name
        local sAtt = string.format(LocalStrings.DEVOUR_ATT, name)
        MsgBoxManager:showConfirmCancelBox(sAtt, self,self.sureToDevourAll, nil, nil, "automatic_phagocytosis")
    end
end
...
--@brief    适配iphoneX 
function WndBless:_adaptIphoneX()
    if IsIphoneX() then
        local conAutoDevour = GetElement(self.m_root, "conAutoDevour_WndBless", WZUIContainer)
        conAutoDevour:setRelativePosition(GlobalMethod:ccp(0.9,0.347))
        local btnBlessBag = GetElement(self.m_root, "btnBlessBag_WndBless", WZUIButton)
        btnBlessBag:setRelativePosition(GlobalMethod:ccp(0.9,0.828))
    end
end
...
function WndBless:_adaptLanguage_vn()
    ...
    local txtSub2 = GetElement(self.m_root,"txtSub2_WndBless",WZUILabelTTF)
    txtSub2:setScale(0.8)
    txtSub2:setRelativePosition(GlobalMethod:ccp(0.7,-0.046))
    local txtSub4 = GetElement(self.m_root,"txtSub4_WndBless",WZUILabelTTF)
    txtSub4:setScale(0.8)
...
function WndBless:_adaptLanguage_ug(  )
    GetElement(self.m_root,"txtSub4_WndBless",WZUILabelTTF):setScale(0.6)
    GetElement(self.m_root,"autoSell",WZUICheckBox):setRelativePosition(GlobalMethod:ccp(0.035,0.11))
    GetElement(self.m_root,"txtSub5_WndBless",WZUILabelTTF):setScale(0.6)
    GetElement(self.m_root,"quickPray",WZUICheckBox):setRelativePosition(GlobalMethod:ccp(0.035,0.11))
end
-------------------------------------------语言适配End----------------------------------------
