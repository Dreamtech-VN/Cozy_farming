...
function CellEquipStore:sureUseDiamondInstead()
    ...
    elseif self.m_nStoreType == 14 or self.m_nStoreType == 15 or self.m_nStoreType == 16 or self.m_nStoreType == 17 then
        ProtocolProcessorStore:send_MALL_BuyShopNewGoods(CellEquipStore.m_current.tData[8], CellEquipStore.m_current.tData[1], 1)
    end
end
-------------------------------------公有方法模块End----------------------------------------
...
function CellEquipStore:_createOnePropUI()
    ...
    --限购数量
    if self.m_nStoreType == 14 or self.m_nStoreType == 15 or self.m_nStoreType == 16 or self.m_nStoreType == 17 then
        local txtLimitNum = GetElement(self.m_root,"txtLimitNum_CellEquipStore",WZUILabelTTF)
        if self.tData[7] == -1 then
            txtLimitNum:setText("")
        else
            txtLimitNum:setText(LocalStrings.LIMIT_TEXT1 .. ":" .. (self.tData[7]-self.tData[6]) .. "/" .. self.tData[7])
        end
    end
end
...
function CellEquipStore:_adaptLanguage_ug(  )
    local txtName = GetElement(self.m_root,"txtPropName_CellEquipStore",WZUILabelTTF)
    txtName:setScale(0.7)
    txtName:setDimensions(GlobalMethod:CCSize(220,0))
    txtName:setRelativePosition(GlobalMethod:ccp(0.5,0.208561))
