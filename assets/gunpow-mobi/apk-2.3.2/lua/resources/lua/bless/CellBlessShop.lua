...
function CellBlessShop:_update()
    ...
    imgQuality:setFile(g_tShopItemQuality[tData.basicInfo.quality + 1])
    --物品
    ...
    -- 是否售罄
    local conS = GetElement(self.m_root,"conSell_CellBlessShop",WZUIContainer)
    conS:setVisible(self.m_tData.status == 1)
...
function CellBlessShop:updateSellStatus()
    WZLog("CellBlessShop:updateSellStatus")
    self.m_tData.status = 1
    local conS = GetElement(self.m_root,"conSell_CellBlessShop",WZUIContainer)
...
function CellPetExchange:_adaptLanguage_ug(  )
    local txtPropName = GetElement(self.m_root,"txtPropName_CellBlessShop",WZUILabelTTF)
