...
function WndChallengeEntrance:onEnter(element)
    ...
    -- if isEndTeach49 ~= true and TeachGroup1:isTeach() and CacheCenter:getPlayerInfo().level == 28 then
    --     WindowManager:removeTeachShelterLayer()
    --     local conMove = GetElement(self.m_root, "conMove_WndChallengeEntrance", WZUIMoveContainer)
    --     conMove:getMoveElement():setPositionX(conMove:getMinPosition().x)
...
--@brief    设置深渊领主boss图
function WndChallengeEntrance:setBossImg( )
    -- body
    local mapId = CacheCenter:getGameParam().teamWorldBossMapId
    if mapId ~= nil and mapId ~= 0 then
        local bosImg = GDatatab_team_world_boss_map["id_"..mapId].head
        if bosImg then
            local bossImg = GetElement(self.m_root,"imgTeam_WndChallengeEntrance",WZUIImage)
            bossImg:setFile(bosImg)
            bossImg:setFlipX(true)
        end
    end
...
function WndChallengeEntrance:onClickEvent(element)
    ...
    elseif nTag == 2 then   --禁忌之地 
        ...
        -- MsgBoxManager:showTipBox(LocalStrings.ASCENDING32)
            ...
            SceneTabooBattle:show()
        end
    elseif nTag == 3 then   --世界组队boss
        -- if GlobalMethod:crossServiceOpen() == 0 then
        --     MsgBoxManager:showTipBox(LocalStrings.CANT_IN)
        --     return 
        -- end
        if CheckButtonOpen(ISLAND_UP_WORLDTEAM_BOSS) then
            SceneWorldTeamBossRoom:showEnter()
            SceneWorldTeamBossRoom:setCallBackFun(WndChallengeEntrance, self.showInterface)
        end
    elseif nTag == 4 then   --噩梦塔
        if CheckButtonOpen(ISLAND_BUILDING_DOUBLE_TOWER) then
            SceneCopy:showScene(4, 2)
            SceneCopy:setCallBackFun(WndChallengeEntrance, self.showInterface)
        end
    elseif nTag == 5 then   --英雄塔
        if CheckButtonOpen(151) then 
            SceneCopy:showScene(5)
            SceneCopy:setCallBackFun(WndChallengeEntrance, self.showInterface)
        end
    elseif nTag == 6 then --秘境宝藏
        if CheckButtonOpen(ISLAND_BUILDING_DAILYMAP) then
            TeachGroup1:endTeachStep({13,2})
            SceneCopy:showScene(3, nil, nil,true)
            WindowManager:removeWindow(self.m_root, self, true)
        end
    elseif nTag == 7 then --夫妻争霸
        if CheckButtonOpen(COUPLE_HEGEMONY) then
            SceneCoupleHegemonyRoom:showEnter()
            SceneCoupleHegemonyRoom:setCallBackFun(WndChallengeEntrance, self.showInterface)
        end
    end
end
...
function WndChallengeEntrance:updateRedPoint()
    ...
    if imgClimbRedDot then
        imgClimbRedDot:setVisible(GlobalGame.g_tRedPointList.tower)
    end
    --英雄塔红点
    local imgHeroClimbRedDot = GetElement(self.m_root, "imgHeroClimbRedDot_WndChallengeEntrance", WZUIImage)
    if imgHeroClimbRedDot then
        imgHeroClimbRedDot:setVisible(GlobalGame.g_tRedPointList.heroTower)
    end
    --禁忌之地红点
    local imgTabooRedDot = GetElement(self.m_root, "imgTabooRedDot_WndChallengeEntrance", WZUIImage)
    if imgTabooRedDot then
        imgTabooRedDot:setVisible(GlobalGame.g_tRedPointList.taboo)
    end
end
...
function WndChallengeEntrance:_initOpenDesc()
    self:closeLoading()
    WindowManager:removeTeachShelterLayer()
    TeachGroup1:startGroup({14,2,self.m_root},{28,2,self.m_root},{13,2,self.m_root},{49,2,self.m_root})
...
function WndChallengeEntrance:_addTop()
    ...
    tNewObj:setTopData("ui/common/bag_icon_tz.png", WndChallengeEntrance, WndChallengeEntrance.onClickClose, false, true, false, "WndChallengeEntrance")
    tNewObj:setTopType()
...
function WndChallengeEntrance:controlBtnShow()
    ...
    local btnList = {3,10,117,148,153,151,6,227}
    ...
    -- if CacheCenter:getGameParam().gameStatus == "1" or string.find(IPDhttpServer:getCurServerName(), "审核") then
    --     WZLog("SceneCity:_update 审核服")
    -- end
    local conCoupleHegemony = GetElement(self.m_root,"conCoupleHegemony_WndChallengeEntrance",WZUIContainer)
    local conTower = GetElement(self.m_root,"conTower_WndChallengeEntrance",WZUIContainer)
    local openLv_227 = GDatatab_button_info["id_"..227].open_level    
    WZLog("WndChallengeEntrance:controlBtnShow id_227 openLv=", openLv_227)
    if playerLevel < openLv_227 and conCoupleHegemony then
        conCoupleHegemony:setVisible(false)
        local posCp = conCoupleHegemony:getRelativePosition()
        --将试练塔平移到夫妻副本的位置
        if conTower then
            conTower:setRelativePosition(posCp)
        end
        --整体左移居中显示
        conList:setRelativePosition(GlobalMethod:ccp(-0.08, 0.5))
    end
end
...
function WndChallengeEntrance:_adaptLanguage_vn(  )
    GetElement(self.m_root,"txtTower_WndChallengeEntrance",WZUILabelTTF):setScale(0.65)
    GetElement(self.m_root,"txtWorld_WndChallengeEntrance",WZUILabelTTF):setScale(0.65)
    GetElement(self.m_root,"txtTeamWorldBoss_WndChallengeEntrance",WZUILabelTTF):setScale(0.65)
    GetElement(self.m_root,"txtTaboo_WndChallengeEntrance",WZUILabelTTF):setScale(0.65)
    GetElement(self.m_root,"txtDoubleTower_WndChallengeEntrance",WZUILabelTTF):setScale(0.65)
    GetElement(self.m_root,"txtHeroTower_WndChallengeEntrance",WZUILabelTTF):setScale(0.65)
end
