...
function CellPvpCompete:_update()
    ...
    local txtWords_CellPvpCompete_1 = GetElement(self.m_root, "txtWords_CellPvpCompete_1", WZUIFreeTextBox)
    if self.m_nCurUIId == 118 then
        ...
        imgBK:setFile("ui/newActivity/activity_pic_bs_pws.png")
        --日期
        if txtTime then
        ...
        -- end
        --
        if txtWords_CellPvpCompete_1 then
            txtWords_CellPvpCompete_1:setAnchorPoint(GlobalMethod:ccp(0.5,1))
            if self.m_tData.start_Time > 0 then
                local str1 = self:_timeChangeStyle(self.m_tData.start_Time)
                local time_str = string.format([[<T C="255,236,193" S="22" P="1" SC="127,70,26" SE="1" SS="4">%s</T><T C="99,255,95" S="22" P="1" SC="0,72,3" SE="1" SS="4">%s</T>]],LocalStrings.WORLD_BOSS_OPEN_TIME_DOWN,str1)
                txtWords_CellPvpCompete_1:setShowText(time_str)
                txtWords_CellPvpCompete_1:enableSchedule("_updateOpenTime",1)
            elseif self.m_tData.start_Time <= 0 and self.m_tData.end_Time > 0 then
                local str2 = self:_timeChangeStyle(self.m_tData.end_Time)
                local time_str = string.format([[<T C="255,236,193" S="22" P="1" SC="127,70,26" SE="1" SS="4">%s: </T><T C="99,255,95" S="22" P="1" SC="0,72,3" SE="1" SS="4">%s</T>]],LocalStrings.ACTIVITY_END_COUNTDOWN,str2)
                txtWords_CellPvpCompete_1:setShowText(time_str)
                txtWords_CellPvpCompete_1:enableSchedule("_updateEndTime",1)
            end
        end
    elseif self.m_nCurUIId == 181 then
        txtSeason:setVisible(false)
        imgBtnText:setFile("ui/welfare/common_icon_dld.png")
        imgBK:setFile("ui/newActivity/activity_pic_bs_pws02.png")
...
function CellPvpCompete:_updateOpenTime( element,time )
    ...
    -- end
    local txtWords = GetElement(self.m_root, "txtWords_CellPvpCompete_1", WZUIFreeTextBox)
    if self.m_tData.start_Time > 0 then
        self.m_tData.start_Time = self.m_tData.start_Time - 1
        local str = self:_timeChangeStyle(self.m_tData.start_Time)
        -- txtWords:setText(LocalStrings.WORLD_BOSS_OPEN_TIME_DOWN..str)
        local time_str = string.format([[<T C="255,236,193" S="22" P="1" SC="127,70,26" SE="1" SS="4">%s</T><T C="99,255,95" S="22" P="1" SC="0,72,3" SE="1" SS="4">%s</T>]],LocalStrings.WORLD_BOSS_OPEN_TIME_DOWN,str)
        txtWords:setShowText(time_str)
    else
        element:disableSchedule()
    end
end
...
function CellPvpCompete:_updateEndTime( element,time )
    ...
    if self.m_tData.end_Time > 0 then
        self.m_tData.end_Time = self.m_tData.end_Time - 1
        local str = self:_timeChangeStyle(self.m_tData.end_Time)
        local time_str = string.format([[<T C="255,236,193" S="22" P="1" SC="127,70,26" SE="1" SS="4">%s: </T><T C="99,255,95" S="22" P="1" SC="0,72,3" SE="1" SS="4">%s</T>]],LocalStrings.ACTIVITY_END_COUNTDOWN,str)
        txtWords:setShowText(time_str)
    else
        element:disableSchedule()
    end
...
function CellPvpCompete:_adaptLanguage_vn(  )
    GetElement(self.m_root,"txtSeason_CellPvpCompete",WZUILabelAtlasFont):setRelativePosition(GlobalMethod:ccp(1.4,0.6))
end
...
function CellPvpCompete:_adaptLanguage_ug(  )
    GetElement(self.m_root,"conTime_CellPvpCompete",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.5,0.29))
end
---------------------------------------------语言适配End------------------------------------------