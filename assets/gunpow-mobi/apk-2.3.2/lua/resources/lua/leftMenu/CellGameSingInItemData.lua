...
function CellGameSingInItem:createElement()
    ...
    element:setAbsContentSize(GlobalMethod:CCSize(630,96))
...
--@brief 	重置日期数据表
function CellGameSingInItem:resetTabDate(index, m_tData,b_sign,b_vipSign ,days)
	self.m_tData = m_tData
	self.index = index
	self.b_sign = b_sign 
	self.b_vipSign = b_vipSign 
...
function CellGameSingInItem:GameSignOk(days)
	...
	WndGameSingIn:_finishedLoading()
	WndGameSingIn:_setSignDays( days )
	WndGameSingIn:_postSignInEvent()
	--Add By Tianxiang_Xu 
	local Multiple = 1
