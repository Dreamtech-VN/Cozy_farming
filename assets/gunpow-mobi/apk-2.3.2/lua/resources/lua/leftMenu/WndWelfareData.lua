...
function WndWelfare:_init()
	...
	self.m_tWelfareList = {{name = LocalStrings.SingInTitle, ui_id = 79, button_id = 18},
                           {name = LocalStrings.NEW_ACTIVITY_TEXT_10, ui_id = 999997, button_id = 21},  --福利卡
                           {name = LocalStrings.FUNDINFO6, ui_id = 115, button_id = 19},
                           {name = LocalStrings.GAME_ACTIVITY_TITLE34, ui_id = 165, button_id = 18},  --在线奖励
                           --{name = LocalStrings.LUCKY_GIFT,ui_id = 190,button_id = 97},
                           {name = LocalStrings.ATH_REWARD_CHECK, ui_id = 999998, button_id = 69}, --奖励
                           {name = LocalStrings.OTHER_TEXT1[9], ui_id = 999996, button_id = 66},  --周年感恩
						}
    --欧洲渠道
    if ProjConfig.CHANNEL_ID == 1061 or ProjConfig.CHANNEL_ID == 1062 or ProjConfig.CHANNEL_ID == 1051 or ProjConfig.CHANNEL_ID == 1048 or ProjConfig.CHANNEL_ID == 1053 or ProjConfig.CHANNEL_ID == 1060 then                    
        self.m_tCompeteList = {{name = LocalStrings.BATTLE_MODEL_RANK, ui_id = 118, button_id = 23},
                            ...
                            {name = LocalStrings.WONDERFUL_TEXT1, ui_id = 222, button_id = 132},
                            }
    else
        self.m_tCompeteList = {{name = LocalStrings.BATTLE_MODEL_RANK, ui_id = 118, button_id = 23},
                            {name = LocalStrings.SETTLMENT_GANGFIGHT, ui_id = 110, button_id = 56},
...
function WndWelfare:_unInit()
    ...
    self.m_tCommonPanle = nil 
    self.m_bIsFreeCardDiscount = nil 
    self.m_tFreeCardActivity = nil 
end
...
function WndWelfare:createElement(_type, ui_id, tMsg)
...
--@brief 	获得列表成功
function WndWelfare:GetWelfareListInfoOK( activityId, title, startTime, endTime, serverTime , types, type2, sortValue)
	--body
    if self.m_root == nil then return end
	...
	for i=1,#activityId do
        ...
        if type2[i] == self.m_nType and (types[i] == g_tGameActivityTypes.ACIVIITY_WEEKCARD_DISCOUNT or types[i] == g_tGameActivityTypes.ACIVIITY_MONTHCARD_DISCOUNT) and serverTime < endTime[i] then 
            self.m_bIsFreeCardDiscount = true
            local tItem = {}
            tItem.activityId = activityId[i]
            tItem.startTime = startTime[i]
            tItem.endTime = endTime[i]
            tItem.type = types[i]

            table.insert(self.m_tFreeCardActivity, tItem)
        end
		if type2[i] == self.m_nType then    --1 的才是福利; 6 是回归活动 20精彩推荐
    		if serverTime < endTime[i] then 
    			if types[i] > 0 and types[i] ~= g_tGameActivityTypes.ACIVIITY_WEEKCARD_DISCOUNT and types[i] ~= g_tGameActivityTypes.ACIVIITY_MONTHCARD_DISCOUNT and types[i] ~= g_tGameActivityTypes.ACTIVITY_MONDAY_PLAN_CARD then 
                    ...
                    if sortValue then 
                        self.m_tTempListItem[index].sortValue = sortValue[i]
                    end
    ...
    table.sort(self.m_tTempListItem, function (a, b)
        -- body
...
function WndWelfare:GetRewardOk(rewardItems,rewardCount,ntype)
	...
	elseif ntype == g_tGameActivityTypes.ACTIVITY_TOTALFIRSTRECHARGE or g_tGameActivityTypes.ACTIVITY_CUMULATIVECOST == ntype or g_tGameActivityTypes.ACTIVITY_STRENGTHEN == ntype or g_tGameActivityTypes.ACTIVITY_NEWSERVER_SINGLECOPY == ntype then 
...
function WndWelfare:removeRedDot(ntype)
    ...
    elseif self.m_nType == 20 then 
        if CacheCenter.m_tWonderfulRedDotList ~= nil then
            for idx=1,#CacheCenter.m_tWonderfulRedDotList do
                if ntype == CacheCenter.m_tWonderfulRedDotList[idx] then 
                    m_bIsContainerActivityType = true  
                end 
            end
        end
...
function WndWelfare:GetMonthFightingListOK_newServer(session, playerId, rank, worshipTimes, totlaWorshipTimes, fighting, name, faceId, headId, sex, level, vipLevel, headColor, bodyId, bodyColor, windId, crossServer, rewardRank, reward)
    self:_closeLoading()
    WZLog("********* WndWelfare:GetMonthFightingListOK_newServer *****",self.m_nCurUIId)
    
    local con_ActivityContext = GetElement(self.m_root,"conContext_WndWelfare",WZUIContainer)

    if g_tGameActivityTypes.ACTIVITY_NEWSERVER_FIGHTINGRANK == self.m_nCurUIId then 
        WZLog("WndWelfare:_updateActivityContext_newServer|| 开服活动-月战力榜",self.m_nCurUIId)
        ...
        self.m_tCommonPanelLuaObj:setMessage_newServer(session, playerId, rank, worshipTimes, totlaWorshipTimes, fighting, name, faceId, headId, sex, level, vipLevel, headColor, bodyId, bodyColor, windId, crossServer, self.m_nCurUIId, rewardRank, reward)
        if self.m_tCommonPanelElement ~= nil then
...
--@brief    获取周卡月卡活动数据
function WndWelfare:getFreecaData()
    -- body
    return self.m_tFreeCardActivity
end

--@brief    根据活动类型获取活动日期
function WndWelfare:getActivityTime(activityType)
    -- body
    for i = 1, #self.m_tTempListItem do 
        if self.m_tTempListItem[i].type == activityType then
            return self.m_tTempListItem[i].startTime, self.m_tTempListItem[i].endTime, self.m_tTempListItem[i].activityId
        end
    end
