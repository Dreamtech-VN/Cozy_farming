...
function CellRankListItem:_update()
    ...
    if itemNameLabel ~= nil then 
        ...
        self:normalTitleLabel()
    end
    --选中状态
    -- self:setSelSignVisible(self.m_bIsHighLight)
end

function CellRankListItem:normalTitleLabel()
    if not self.m_root then return end
    local itemNameLabel = GetElement(self.m_root, "label_item", WZUILabelTTF)
    itemNameLabel:setEnableStroke(true)
    itemNameLabel:setColor(GlobalMethod:ccc3(255,236,193))
    itemNameLabel:setStrokeColor(GlobalMethod:ccc3(127,70,26))
    itemNameLabel:setStrokeSize(4)
end
function CellRankListItem:selectTitleLabel()
    if not self.m_root then return end
    local itemNameLabel = GetElement(self.m_root, "label_item", WZUILabelTTF)
    itemNameLabel:setEnableStroke(false)
    itemNameLabel:setColor(GlobalMethod:ccc3(127,70,26))
end
