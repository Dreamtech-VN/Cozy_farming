...
--设置爱心许愿抽奖数据
function WndLoveLottery:setLotteryInfo(startTime, endTime, content, activityId, activityType)
	WZLog("WndLoveLottery:setLotteryInfo ", startTime, endTime, content)
	local tConfig = json.decode(content)
	WZLog("WndLoveLottery:setLotteryInfo ", Serialize(tConfig))
	...
	if tConfig then
		local tName = {}
		local tIcon = {}
		for i = 1, #tConfig.itemId do
			local basicData = GDatatab_item["id_" .. tConfig.itemId[i]]

			table.insert(tName, basicData.name)
			table.insert(tIcon, basicData.icon)
		end
        self.m_tGiftName = tName
    	self.m_tGiftIcon = tIcon
    	self.m_tGiftNum = tConfig.itemNum
    	self.m_tGiftId = tConfig.itemId

    	local string = string.sub(tConfig.price, 2, -2) 
...
--@param  num 物品数量
--@param 	topReward:是否稀有物品1大奖 
function WndLoveLottery:ReceiveRewardOk(gridId, itemId, num, topReward)
...
function WndLoveLottery:_onGetLoveLotteryResult(activityId, doType, result, msg)
	local tConfig = json.decode(msg)

	WZLog("WndLoveLottery:_onGetLoveLotteryResult", tConfig.playerLuckNum)
	self:ReceiveRewardOk(tConfig.gridId, tConfig.itemId, tConfig.itemNum, tConfig.topReward)
	CellLoveLotteryBox:setData(tConfig.playerLuckNum)
end
-------------------------------------私有方法模块End----------------------------------------
