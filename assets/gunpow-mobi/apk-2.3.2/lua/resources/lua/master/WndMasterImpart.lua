...
function WndMasterImpart:onImpartCall()
	...
	for i = 1,#WndMasterMember.m_tMyPupils do 
		...
		if tData.isOnline == true then 
			WndChat:sendChat(CHANNEL_WHISPER,string.format(LocalStrings.MASTERINFO78,shouyeExp),tData.id,tData.name,tData.sex,tData.level,tData.vipLevel,tData.headId,tData.faceId,tData.headColor, tData.headEffectId)
		end
	end 
end
-------------------------------------公有方法模块End----------------------------------------
