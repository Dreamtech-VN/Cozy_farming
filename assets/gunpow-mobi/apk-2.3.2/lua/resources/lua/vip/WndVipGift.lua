...
function WndVipGift:actionCallback(elem,data)
    if self.m_nType == 2 then 
        ...
        if level == 9 then
            PostPlayerEvent:postEvent(PostPlayerEvent.event_nineLvPopGift)
        end
    elseif self.m_nType == 5 then 
        ...
        local rechargeId = SplitStringWithSeparator(string,",")[2]

        self.m_tData = {}
        self.m_tData.id = tonumber(rechargeId)
        self.m_tData.leftTimes = self.m_Count[1] 
        self.m_tData.giftType = giftType
        self.m_tData.limitType = 1
        self.m_tData.needVipLv = 0
        if giftType == 1 then 
            ...
            self.m_tData.name = GDatatab_item["id_" .. item_id].name 
            self.m_tData.itemId = item_id
            self.m_tData.icon = vipData.icon
        end
    end
...
function WndVipGift:onBuy(element)
    ...
    if self.m_nType == 2 or self.m_nType == 5 then 
        ...
        if self.m_tData.giftType == 1 then 
            ...
            if not JudgeMoneyIsEnough(tShopData.moneyId, curPrice, nil, nil, GlobalGame.g_nCurrentUIChannelId, nil, nil, nil, nil, self, self.sureToUseDiamondInstead) then
                return 
            end
...
function WndVipGift:sureToUseDiamondInstead()
    ...
    local tShopData = CacheCenter:getShopGoodData(self.m_tData.id)
    price:push(tShopData.floorPrice)

    self:_createLoading()
    ProtocolProcessorWndShop:send_MALL_BuyItems(count, mallId, 1, 0, nil, nil, price)
end

--@note     设置UI界面数据
function WndVipGift:_update()
    local curData = self.m_tData
    GetElement(self.m_root, "conVipGift_WndVipGift", WZUIContainer):setVisible(true)
...
function WndVipGift:_showBtnWord()
    -- body
    if self.m_nType == 5 then 
        local txtName = GetElement(self.m_root, "txtBtnBuy_WndVipGift", WZUILabelTTF)
        local ftxtCurPrice = GetElement(self.m_root, "ftxtCurPrice1_WndVipGift", WZUIFreeTextBox)
        local sFormat = [[<I Z="0.5" P="1">%s</I><T C="255,250,236" S="22" P="1" SC="0,108,3" SS="4" SE="1">%d</T><T C="255,250,236" S="22" P="1" SC="0,108,3" SS="4" SE="1">%s</T>]]
        if txtName then
...
function WndVipGift:_showCost()
    ...
    local sFormat = [[<T C="255,236,193" S="18" P="1" SC="132,66,29" SS="4" SE="1">%s:</T><I Z="0.5" P="1">%s</I><T C="255,236,193" S="18" P="1" SC="132,66,29" SS="4" SE="1">%d</T>]]
    if ftxtPrice then
        local string = string.sub(self.m_sOriginPrice[1],2,-2) 
...
function WndVipGift:_showLeftTime()
    ...
    if self.m_nType == 5 then 
        if self.m_nEndTime[1] <= 0 then 
            self.m_root:disableSchedule()
            self:onCloseActionCallback()
            MsgBoxManager:showTipBox(LocalStrings.VIPWEEK_PACKAGE7)
            return 
        end
    else
    ...
    if self.m_nType == 5 then 
        nLeftSeconds = self.m_nEndTime[1]
        ftxtLeftTime = GetElement(self.m_root, "ftxtLeftTime1_WndVipGift", WZUIFreeTextBox)
    end
    ...
    if ftxtLeftTime then 
        ...
        if nLeftSeconds >= 0 then 
            ...
            if self.m_nType == 5 then 
                self.m_nEndTime[1] = self.m_nEndTime[1] - 1
                ftxtLeftTime:setShowText(string.format(LocalStrings.VIPWEEK_PACKAGE6, hours, minutes, seconds))
            else
...
--@note     设置UI界面数据
function WndVipGift:_update2()
    GetElement(self.m_root, "conNewGift_WndVipGift", WZUIContainer):setVisible(true)
    if tonumber(self.m_tData.icon) == 2 then 
        GetElement(self.m_root, "imgTitle1_WndVipGift", WZUIImage):setFile("ui/newActivity/text_lb_01.png")
        GetElement(self.m_root, "imgBK_WndVipGift", WZUIImage):setFile("ui/newActivity/activity_pic_lb_01.png")
    end
    self:_showBtnWord()
    self:_showLeftTime()
    self.m_root:enableSchedule("_showLeftTime", 1)

    local con = GetElement(self.m_root,"conItem1_WndVipGift",WZUIContainer)
    con:removeAllChildrenWithCleanup(true)
    local celElement,tLuaObj = CellGoodItem:createElement()
    if celElement ~= nil then 
        celElement = WZUIContainer:luaTo(celElement)
        tLuaObj:setCellGoodLocalId(self.m_tData.itemId, 1, 16)
        tLuaObj:setItemClickFun(self, self.onClickItem)
        celElement:setScale(1)
        con:addChild(celElement)
    end
...
function WndVipGift:onTempClose()
    ...
    if self.m_nType == 2 then 
        ...
        local level = tonumber(CacheCenter:getPlayerInfo().level)
        if level == 9 then
            PostPlayerEvent:postEvent(PostPlayerEvent.event_nineLvClosePopGift)
        end
    else
...
function WndVipGift:_adaptLanguage_ug(  )
    GetElement(self.m_root, "txtName2_WndVipGift", WZUILabelTTF):setScale(0.7)
    GetElement(self.m_root, "ftxtLeftTime_WndVipGift", WZUIFreeTextBox):setScale(0.7)
    local txtMoney = GetElement(self.m_root, "txtRemain_WndVipGift", WZUIFreeTextBox)
    txtMoney:setMaxWidth(200)
    txtMoney:setScale(0.8)

    GetElement(self.m_root, "txtBtn_WndVipGift", WZUILabelTTF):setScale(0.7)
    GetElement(self.m_root, "ftxtCurPrice_WndVipGift", WZUIFreeTextBox):setScale(0.7)
