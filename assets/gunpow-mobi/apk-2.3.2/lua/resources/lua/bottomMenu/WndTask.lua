...
function WndTask:onEnter(element)
    ...
    self:teachOnRefresh()
    ProtocolProcessorSceneActive:regAll()
    self:register()
...
function WndTask:register()
    GlobalGame:getGameEventDispathcer():Add(bottomMeneEvent.WndBottomMeneEvent_TaskOffLine,self._onTaskOffLineInfoData,self)
    GlobalGame:getGameEventDispathcer():Add(bottomMeneEvent.WndBottomMeneEvent_GetTaskOffLineResult,self._getTaskOffOnlineResult,self)
end
function WndTask:unregister()
    GlobalGame:getGameEventDispathcer():Remove(bottomMeneEvent.WndBottomMeneEvent_TaskOffLine,self._onTaskOffLineInfoData,self)
    GlobalGame:getGameEventDispathcer():Remove(bottomMeneEvent.WndBottomMeneEvent_GetTaskOffLineResult,self._getTaskOffOnlineResult,self)
end
function WndTask:_addTop()
    local cell,tcell = CellTopHandle:createElement()
    self.m_root:addChild(cell)
    tcell:setTopData("ui/common/bag_icon_rw.png",WndTask,WndTask.onCloseClick,true,true,false,false,nil,false,false)
    tcell:setTopType()
end
function WndTask:onCloseClick(  )
...
function WndTask:onEnterTransitionDidFinish(element)
    ...
    local checkBoxAthletics = GetElement(self.m_root,"checkBoxAthletics_WndTask",WZUICheckBox)
    ...
    --判断职业是否可以点击
    local bRet_Profession,msg_Profession = self:_ifTaskOpen(152)
    local professionDoingNum = PrefetchCache:getProfessionTaskDoingNum()
    local checkBoxProfession = GetElement(self.m_root,"checkBoxProfession_WndTask",WZUICheckBox)
    if checkBoxProfession == nil then
        return
    end
    if not bRet_Profession or (professionDoingNum == 0 and GlobalGame.g_nProfessionTaskCount == 0) then
        checkBoxProfession:setVisible(false)
        GetElement(self.m_root,"checkBoxOffLine_WndTask",WZUICheckBox):setRelativePosition(GlobalMethod:ccp(0.5,0.28))
        GetElement(self.m_root,"conCheckBoxOffLineSel_WndTask",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.5,0.28))
    else
        checkBoxProfession:setVisible(true)
    end
...
function WndTask:actionCallback(element, data)
    ...
    if level == 9 then
        PostPlayerEvent:postEvent(PostPlayerEvent.event_nineLvOpenTask)
    end
    self:scheduleLoadUI()
    ProtocolProcessorSceneActive:send_TASK_GetRetrieveList( )
end
...
function WndTask:onExit(element)
    ...
    self:unregister()
    ProtocolProcessorSceneActive:unregAll()
	self:_unInit()
end
...
function WndTask:onMainTaskSelected(element)
    ...
    self:_setCheckBoxSel(true, false, false, false, false, false)
    self.m_nCurIndex = 0
...
function WndTask:onDailyTaskSelected(element)
    ...
    self:_setCheckBoxSel(false, false, true, false, false, false)
...
function WndTask:onBranchTaskSelected( element )
    ...
    self:_setCheckBoxSel(false, true, false, false, false, false)
...
function WndTask:onAthleticsSelected(element)
    -- body
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN2)
    self:_setCheckBoxSel(false, false, false, true, false, false)
...
function WndTask:onClickBox(element)
    ...
    if tData.status == 1 then 
        ...
        data.cur_value = CacheCenter:getPlayerItemCountById(tData.coinId)
        data.totle_value = tData.target
        data.rewardIds = reward_id
        data.rewardNums = reward_num
        WndNewTipsReward:showInterface(self.m_root, element, data, true, GlobalMethod:ccp(0.5, 0.43))
    end
end

--@brief    点击职业标签回调
function WndTask:onProfessionSelected(element)
    -- body
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN2)
    self:_setCheckBoxSel(false, false, false, false, true, false)
...
--@brief    点击离线标签回调
function WndTask:onOffLineSelected(element)
    -- body
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN2)
    self:_setCheckBoxSel(false, false, false, false, false, true)
...
-------------------------------------私有方法模块Begin--------------------------------------
-- 获取到离线标签的信息
function WndTask:_onTaskOffLineInfoData(taskId, title, itemId, num, itemSise)
    if next(taskId) == nil then
        GetElement(self.m_root,"checkBoxOffLine_WndTask",WZUICheckBox):setVisible(false)
        return
    end
    GetElement(self.m_root,"checkBoxOffLine_WndTask",WZUICheckBox):setVisible(true)
    local index = 1
    local table_insert = table.insert
    local table_sort = table.sort
    for i=1, #taskId do
        local tab = {}
        tab.task_id = taskId[i]
        tab.title = title[i]

        local item_id = {}
        local item_num = {}
        for i=1,itemSise[i] do
            table_insert(item_id,itemId[index])
            table_insert(item_num,num[index])
            index = index + 1
        end
        tab.item_id = item_id
        ...
        local gold_info = {}
        local icon = nil
        local retrieve_data = GDatatab_retrieve["id_"..taskId[i]]
        if retrieve_data then
            dianmond_info = retrieve_data.expend_1[1]
            gold_info = retrieve_data.expend[1]
            icon = retrieve_data.icon
        end
        tab.dianmond_info = dianmond_info
        tab.gold_info = gold_info
        tab.icon = icon
        self.m_tOffLineTaskData[i] = tab
    end
    table_sort(self.m_tOffLineTaskData, function(a,b) return a.task_id < b.task_id end )

    self:setOffOnlineListData()
end
--离线找回领取的结果
--[[ taskId  返回-1 -2  此收益无法找回   返回 -3   此收益无法金币找回 返回 -4 物品不足  ]]
function WndTask:_getTaskOffOnlineResult(taskId, rType, itemId, num)
    if taskId == -1 or taskId == -2 then
        MsgBoxManager:showTipBox(LocalStrings.OPTIMIZE_TEXT12)
        return
    elseif taskId == -3 then
        MsgBoxManager:showTipBox(LocalStrings.OPTIMIZE_TEXT13)
        return
    ...
    if self.m_tOffLineTaskData then
        for i = #self.m_tOffLineTaskData,1,-1 do
            if taskId == self.m_tOffLineTaskData[i].task_id then
                table.remove(self.m_tOffLineTaskData, i)
                break
            end
        end
        self:setOffOnlineListData()    
...
function WndTask:setOffOnlineListData()
    local offOnlineFreeList = GetElement(self.m_root,"offOnlineFreeList_WndTask",WZUIFreeListContainer)
    if offOnlineFreeList then
        offOnlineFreeList:removeAll()
        for i = 1, #self.m_tOffLineTaskData do
            local element, tLuaObj = CellTaskOffOnlineItem:createElement()
            offOnlineFreeList:pushBack(WZUIContainer:luaTo(element))
            offOnlineFreeList:getMoveElement():setPositionY(offOnlineFreeList:getMinPosition().y)
            tLuaObj:setTaskOffOnlineMessage(i, self.m_tOffLineTaskData[i])
        end
    end
end
--@brief	设置界面上的静态文本
function WndTask:_setStaticText()
	...
	for i=1,1 do
        ...
        GetElement(self.m_root, "txtProfessionTask_"..i.."_WndTask",WZUILabelTTF):setText(LocalStrings.PROFESSION_TEXT8)
	end
...
function WndTask:teach()
    ...
    --    WZLog("WndTask:teach one",self.m_tTaskList.tMainTask[1].nId, self.m_tTaskList.tMainTask[1].nTaskStatus, tostring(isEndTeach3), teachStep3, tostring(isEndTeach5), teachStep5, tostring(isEndTeach7), teachStep7, tostring(isEndTeach8), teachStep8, tostring(isEndTeach9), teachStep9, tostring(isEndTeach32), teachStep32)
        ...
        if self.m_tTaskList.tMainTask[1].nId == TeachGroup1.TASK_ID_1 and self.m_tTaskList.tMainTask[1].nTaskStatus == 1 then
            ...
            if btn:getTouchEnable() == true then
                isTeach = TeachGroup1:startGroup({3,3,WndTask.m_tListItem.m_root})
                tryTeach = true
            else
                TeachGroup1:setTeachFinish(3, -1)
            end
        ...
        if self.m_tTaskList.tMainTask[1].nId == TeachGroup1.TASK_ID_2 and self.m_tTaskList.tMainTask[1].nTaskStatus == 1 then
            ...
            if btn:getTouchEnable() == true then
                isTeach = TeachGroup1:startGroup({5,9,WndTask.m_tListItem.m_root})
                tryTeach = true
            else
        ...
        if self.m_tTaskList.tMainTask[1].nId == TeachGroup1.TASK_ID_3 and self.m_tTaskList.tMainTask[1].nTaskStatus == 0 then
            ...
            if btn:getTouchEnable() == true then
                isTeach = TeachGroup1:startGroup({5,10,WndTask.m_tListItem.m_root})
                tryTeach = true
            else
                TeachGroup1:setTeachFinish(5, -1)
            end
        ...
        if self.m_tTaskList.tMainTask[1].nId == TeachGroup1.TASK_ID_3 and self.m_tTaskList.tMainTask[1].nTaskStatus == 1 then
            ...
            if btn:getTouchEnable() == true then
                isTeach = TeachGroup1:startGroup({7,3,WndTask.m_tListItem.m_root})
                tryTeach = true
            else
        ...
        --     if btn:getTouchEnable() == true then
        --         isTeach = TeachGroup1:startGroup({8,7,WndTask.m_tListItem.m_root})
        --         tryTeach = true
        --     else
        ...
        --     if btn:getTouchEnable() == true then
        --         isTeach = TeachGroup1:startGroup({9,5,WndTask.m_tListItem.m_root})
        --         tryTeach = true
        --     else
        --         TeachGroup1:setTeachFinish(9, -1)
        --     end
        -- end
        -- if isEndTeach9 ~= true and teachStep9 > 0 and self.m_tTaskList.tMainTask[1].nTaskStatus == 0 and (TeachGroup1.GROUP ~= 9 or (TeachGroup1.GROUP == 9 and TeachGroup1.STEP ~= 6)) then
            
        --     if btn:getTouchEnable() == true then
        --         isTeach = TeachGroup1:startGroup({9,6,WndTask.m_tListItem.m_root})
        --         tryTeach = true
        --     else
        --         TeachGroup1:setTeachFinish(9, -1)
        --     end
...
function WndTask:_loadDailyTaskItem( element ,delate )
    ...
    if not (self.m_tDailyCacheTaskList[self.m_nCurrentCellIndex].nTaskStatus == TASKSTATUS_STALE) then
        for i = 1, #self.m_tDailyCacheTaskList do
            ...
            dailyElement:setContentSize(GlobalMethod:CCSize(888,106))
...
function WndTask:_initTaskPanel()
    ...
    elseif GlobalGame.g_nAthleticsTaskCount > 0 then
        ...
        self:_updateAthleticsTask()
    elseif GlobalGame.g_nProfessionTaskCount > 0 then
        self:_setCheckBoxSel(false, false, false, false, true, false)
...
function WndTask:_setTaskCount()
    local bIsHaveRed = PrefetchCache:whetherHaveBoxActive()
    ...
    --职业红点
    local con_ProfessionTaskIconNum = GetElement(self.m_root,"con_ProfessionTaskIconNum",WZUIContainer)
    if con_ProfessionTaskIconNum == nil then
        return
    end    
    
    if GlobalGame.g_nProfessionTaskCount > 0 then
        con_ProfessionTaskIconNum:setVisible(true)
        bIsHaveRed = true
    else
        con_ProfessionTaskIconNum:setVisible(false)
    end

    if not judgeHavedRecordString("TASK_UINAME", true) and not bIsHaveRed then 
        CacheCenter:setRedState("btnTask",false,7)
    end
end
...
function WndTask:_setCheckBoxSel(bMainVisible, bBranchVisible, bDailyVisible, bAthleticsVisible, bProfessionVisible, bOffLineVisible)
    -- body
    ...
    GetElement(self.m_root, "conCheckBoxProfessionSel_WndTask", WZUIContainer):setVisible(bProfessionVisible)
    GetElement(self.m_root, "conCheckBoxOffLineSel_WndTask", WZUIContainer):setVisible(bOffLineVisible)
    ...
    GetElement(self.m_root, "offOnlineFreeList_WndTask", WZUIFreeListContainer):setVisible(bOffLineVisible)
    GetElement(self.m_root, "offOnlineRewardLabel", WZUILabelTTF):setVisible(bOffLineVisible)
end
...
function WndTask:_showDailyActivity()
    ...
    --当前活跃度
    GetElement(self.m_root, "txtCurActivityNum_WndTask", WZUILabelAtlasFont):setText(nCurNum)
    --宝箱数据
    ...
    for i = 1, 5 do
        ...
        local imgBoxRed = GetElement(self.m_root, "imgBoxRed" .. i .. "_WndTask", WZUIImage)
...
--@brief    更新职业任务
function WndTask:_updateProfessionTask()
    local tProfessionList = {}
    local tToSubmit = self.m_tTaskList.tProfessionTask.tToSubmit
    for i = 1, #tToSubmit do
        table.insert(tProfessionList, tToSubmit[i])
    end
    local tDoing = self.m_tTaskList.tProfessionTask.tDoing
    for i = 1, #tDoing do
        table.insert(tProfessionList, tDoing[i])
    end
    local tCompleted = self.m_tTaskList.tProfessionTask.tCompleted
    for i = 1, #tCompleted do
        table.insert(tProfessionList, tCompleted[i])
    end

    if #tProfessionList <= 0  then 
        local confl_commondListView = GetElement(self.m_root,"flconTaskList_WndTask",WZUITableContainer)
        if confl_commondListView == nil then
            return
        end
    ...
    self:_setTaskList(tProfessionList)
end

--@brief    更新红点
function WndTask:updateRedDot()
    ...
    for i = 1, 5 do
        local nCurNum = CacheCenter:getPlayerItemCountById(80)
        if self.m_tDailyActivityData == nil then
            self:setDailyBoxData2()
        end
        local tData = self.m_tDailyActivityData[i]
        if tData.status == 0 and nCurNum >= tData.target then 
            tData.status = 1 
...
function WndTask:_adaptLanguage_es(  )
    ...
    GetElement(self.m_root,"txtCurActivityName_WndTask",WZUILabelTTF):setScale(0.7)
end
