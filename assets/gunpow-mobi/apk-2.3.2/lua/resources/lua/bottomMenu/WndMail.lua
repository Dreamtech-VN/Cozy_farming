...
function WndMail:onReject()
    ...
    ProtocolProcessorWndMail:MAIL_MallMailOperate(self.m_tCurOpenMail.mailId, 3, price)
end
...
function WndMail:onDoPay()
    ...
    -- if tonumber(id[1]) == 177 then
    --     if JudgeMoneyIsEnough(177, tonumber(num[1]), nil, nil, 50)  then
    --         local ss = string.format(LocalStrings.MAIL_DOPAY2_VN, num[1])
    ...
    local nums = {}
    local strDesc = ""
    local costList = SplitStringWithSeparator(self.m_tCurOpenMail.cost,"&")
    if costList then
        for i=1,#costList do
            local costData = string.sub(costList[i],2,-2)
            local id = SplitStringWithSeparator(costData,",")[1]
            local num = SplitStringWithSeparator(costData,",")[2]
            ...
            if i == 1 then
                if tonumber(id) == 1 then
                    strDesc = string.format(LocalStrings.MAIL_DOPAY2, num)
                elseif tonumber(id) == 177 then
                    strDesc = string.format(LocalStrings.MAIL_DOPAY2_VN, num)
                end
            else
                strDesc = strDesc .. "," .. num .. " " .. itemInfo.name
            end
        end
    end
    for i=1,#ids do
        if JudgeMoneyIsEnough(ids[i], nums[i], nil, nil, 50) then
           MsgBoxManager:showConfirmBox(strDesc, WndMail,self.doPay)
        end
    end
...
function WndMail:doPay() 
    ...
    --价格
    local id,num = SplitItemString(self.m_tCurOpenMail.cost)
    local cost = tonumber(num[1])
    local price = WZLuaVector_int_:create()
    price:push(cost)

    ProtocolProcessorWndMail:MAIL_MallMailOperate(self.m_tCurOpenMail.mailId, 2, price)
end
...
function WndMail:onGetAllClick()
    ...
    for i = 1, #self.m_tMailList do
        if self.m_tMailList[i].mail_type == 0 then --普通邮件
            ids:push(self.m_tMailList[i].mailId)
        end
    end
    ProtocolProcessorWndMail:send_MAIL_GetAllMailReward(ids)
end
...
function WndMail:openMailCallBack(id, state)
    ...
    if not self.m_tCurOpenMail then return end
    ...
    if self.m_nOpenTag == 1 then
        if hasAnnex then
            GetElement(self.m_root, "scrollNotice_WndMail", WZUIMoveContainer):setContentSize(GlobalMethod:CCSize(505,105))
        else
            GetElement(self.m_root, "scrollNotice_WndMail", WZUIMoveContainer):setContentSize(GlobalMethod:CCSize(505,265))
        end
    end
    GetElement(self.m_root,"mailContent_WndMail",WZUILabelTTF):setText(content)
    self:_upMoveContainerLayer()
...
function WndMail:openShopMail()
    ...
    if state == 5 then
        ...
        -- local iconPath = moneyIcon[tonumber(id[1])]
        -- GetElement(self.m_root,"imgMoneyType_WndMail",WZUIImage):setFile(iconPath)
        -- GetElement(self.m_root,"txtPayNum_WndMail",WZUILabelTTF):setText(num[1])

        local strFormat = [[<T C="127,70,26" S="22" P="0">%s</T>]]
        local strDesc = string.format(strFormat,LocalStrings.MAIL_PAY)
        ...
        if costList then
            for i=1,#costList do
                ...
                local itemInfo = GDatatab_item["id_"..id]
                local tempFormat = [[<I Z="0.5">%s </I><T C="229,105,22" S="22" P="0">%s </T>]]
                strDesc = strDesc .. string.format(tempFormat,itemInfo.icon,num)
            end
        end
        local ftbMoneyType = GetElement(self.m_root,"ftbMoneyType_WndMail",WZUIFreeTextBox)
        ftbMoneyType:setShowText(strDesc)
    end
end
...
--        if tonumber(id[i]) ~= 161021 or level >= 100 then 
            ...
            if celElement ~= nil then 
                ...
                tabCon:setCellElement(nIndex-1,0,celElement)
...
function WndMail:onGetAnnex(element)
    ...
    if self.m_tCurOpenMail.mailType == 3 then
        local price = WZLuaVector_int_:create()
        ProtocolProcessorWndMail:MAIL_MallMailOperate(self.m_tCurOpenMail.mailId, 1, price)
    else
...
function WndMail:getInfoFromServer(nTag)
    ...
    elseif nTag == 1 then
        ...
        if self.m_tMailList and next(self.m_tMailList) then
            for j=1,#self.m_tMailList do
                if self.m_tMailList[j].mailType == self.n_editMailId then
                    table.remove(self.m_tMailList,j)
                    self.m_tCurOpenMail = nil
...
function WndMail:_createMailMenuList()
    ...
    self.m_root:enableSchedule("_createMailMenuList2", 0.01)
end
...
function WndMail:_deletMailFromList(nTag)
    ...
    WZLog("----TTTTTT----:", deleNum, #self.m_tMailList)
...
function WndMail:_setWndMailBtnVisible(tag)
    ...
    --商品邮件提示
    if tag==3 and (#self.m_tMailList == 0) then
        GetElement(self.m_root, "txtShopTips_WndMail", WZUILabelTTF):setVisible(true)
    else
        GetElement(self.m_root, "txtShopTips_WndMail", WZUILabelTTF):setVisible(false)
    end
    ...
    if tag == 3 then --商务箱
        GetElement(self.m_root, "scrollNotice_WndMail", WZUIMoveContainer):setAbsContentSize(GlobalMethod:CCSize(505,105))
        GetElement(self.m_root, "btnEditMail_WndMail", WZUIButton):setVisible(false)
        GetElement(self.m_root,"conReward_WndMail",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.5, 0.36))
        GetElement(self.m_root,"conEditMailBtnSel_WndMail",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.5,0.08))
    else
        ...
        GetElement(self.m_root,"conEditMailBtnSel_WndMail",WZUIContainer):setRelativePosition(GlobalMethod:ccp(0.5,0.0188))
    end
    --右边按钮相关
...
function WndMail:_adaptLanguage_ug(  )
    local txtRecv = GetElement(self.m_root,"txtRecv_WndMail",WZUILabelTTF)
    txtRecv:setDimensions(GlobalMethod:CCSize(150,0))
    txtRecv:setScale(0.6)
    local txtRecvSel = GetElement(self.m_root,"txtRecvOfGroupBox_WndMail",WZUILabelTTF)
    txtRecvSel:setScale(0.6)
    txtRecvSel:setDimensions(GlobalMethod:CCSize(150,0))
    local txtOutBox = GetElement(self.m_root,"txtOutBox_WndMail",WZUILabelTTF)
    txtOutBox:setScale(0.6)
    txtOutBox:setDimensions(GlobalMethod:CCSize(150,0))
    local txtSendOfGroup = GetElement(self.m_root,"txtSendOfGroupBox_WndMail",WZUILabelTTF)
    txtSendOfGroup:setScale(0.6)
    txtSendOfGroup:setDimensions(GlobalMethod:CCSize(150,0))
    local txtBusiness = GetElement(self.m_root,"txtBusiness_WndMail",WZUILabelTTF)
    txtBusiness:setScale(0.6)
    txtBusiness:setDimensions(GlobalMethod:CCSize(150,0))
    local txtBusiness2 = GetElement(self.m_root,"txtBusiness2_WndMail",WZUILabelTTF)
    txtBusiness2:setScale(0.6)
    txtBusiness2:setDimensions(GlobalMethod:CCSize(150,0))

    local txtSel = GetElement(self.m_root,"txtSelName_WndMail",WZUILabelTTF)
    txtSel:setRelativePosition(GlobalMethod:ccp(0.36,0.5))
    txtSel:setDimensions(GlobalMethod:CCSize(100,0))

    local txtBtnEdit = GetElement(self.m_root,"txtBtnEdit_WndMail",WZUILabelTTF)
    txtBtnEdit:setScale(0.7)
    txtBtnEdit:setDimensions(GlobalMethod:CCSize(170))
    ...
    local txtShopTips = GetElement(self.m_root,"txtShopTips_WndMail",WZUILabelTTF)
    txtShopTips:setScale(0.7)
    txtShopTips:setDimensions(GlobalMethod:CCSize(500,0))
