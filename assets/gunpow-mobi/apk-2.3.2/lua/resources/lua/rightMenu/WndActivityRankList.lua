...
function WndActivityRankList:_update()
    ...
    --夫妻同心战活动
    if self.m_nRankType == g_tGameActivityTypes.ACTIVITY_COUPLEFIGHTING then
        GetElement(self.m_root, "txtCouple_WndActivityRankList", WZUILabelTTF):setText(LocalStrings.COUPLE_FIGHTING_TOGETHER)
        GetElement(self.m_root, "txtWinNum_WndActivityRankList", WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.96,0.7))
        GetElement(self.m_root, "txtRankName_WndActivityRankList", WZUILabelTTF):setRelativePosition(GlobalMethod:ccp(0.05,0.7))
    end
