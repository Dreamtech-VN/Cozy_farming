...
function WndWeekCard:_update(  )
    ...
    local txtTimeWord = GetElement(self.m_root, "txtTimeWord_WndWeekCard", WZUILabelTTF)
    if nLastDay > 0 then
        conLeftTime:setVisible(true)
        txtTimeWord:setTextKey("WEEK_DAY")
    else
    ...
    local imgBK = GetElement(self.m_root, "imgBK_WndWeekCard", WZUIImage)
    imgBK:setFile("ui/newActivity/activity_pic_flk_zk.png")
    if self.m_nCardActivityState == 0 then 
        conLeftTime:setVisible(true)
        ...
        for i = 1, #tTempList do
            if tTempList[i].type == g_tGameActivityTypes.ACIVIITY_WEEKCARD_DISCOUNT then 
        ...
        if self.m_nBuyCardTimes <= 0 then 
            ...
            imgBK:setFile("ui/newActivity/activity_pic_hd_zkdz.png")
        else
...
function WndWeekCard:_adaptLanguage_pt()
    local txtBuy = GetElement(self.m_root,"txtBuy_WndWeekCard",WZUILabelTTF)
    txtBuy:setScale(0.7)
    txtBuy:setDimensions(GlobalMethod:CCSize(170))
    local txtTimeValue = GetElement(self.m_root,"txtTimeValue_WndWeekCard",WZUILabelTTF)
