...
function CellExchangePanel:showWindow()
    ...
    if self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_NEW_EXCHANGE then
        ...
        if conConSume then
            conConSume:setAbsContentSize(GlobalMethod:CCSize(640,340))
...
--@brief    点击兑换回调
--@brief    兑换的奖励Id
function CellExchangePanel:onClickExchange(rewardId, num)
    -- body
    WZLog("CellExchangePanel:onClickExchange", rewardId)
    self.m_ClickRewardId = rewardId

    self.m_nloadingId = MsgBoxManager:showLoadingBox()
    ProtocolProcessorWndActivityOnLine:send_ACTIVITY_ReceiveActivityReward(self.m_activityId, self.m_ClickRewardId, num)
end
...
function CellExchangePanel:_showTime()
    ...
    --展示图片
    local bIsDraw = false 
    if self.m_tips and self.m_tips[1] ~= nil then
        ...
        if nStart then
            ...
            if self.m_tips[2] ~= nil and (tonumber(self.m_tips[2]) == 6 or tonumber(self.m_tips[2]) == 7) then   --兑换称号
                ...
                -- if conRule then
                --     conRule:setAbsContentSize(GlobalMethod:CCSize(550,90))
                ...
                if self.m_nActivityType ~= g_tGameActivityTypes.ACTIVITY_NEW_EXCHANGE then
                    ...
                    --     else
                    --         conRule:setAbsContentSize(GlobalMethod:CCSize(550,160))
...
function CellExchangePanel:_showList()
    ...
    if not flconList then return end
    ...
    for i = 1, nCount do
        ...
        tItem.tRewardData = {}
        tItem.tConsumeData = {}
        if self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_EXCHANGE or self.m_nActivityType == g_tGameActivityTypes.ACIVIITY_OLD_EXCHANGE or self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_EXCHANGE_ONE or self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_EXCHANGE_TWO or self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_EXCHANGE_THREE or self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_EXCHANGE_FOUR then
            local tRewardData = {}
    ...
    for i = 1, nCount do
        if self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_EXCHANGE or self.m_nActivityType == g_tGameActivityTypes.ACIVIITY_OLD_EXCHANGE or self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_EXCHANGE_ONE or self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_EXCHANGE_TWO or self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_EXCHANGE_THREE or self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_EXCHANGE_FOUR then 
            ...
            if element and tNewObj then
                tNewObj:setData(tData[i].tRewardData, tData[i].tConsumeData, self.m_rewardCounts[i], self.m_rewardId[i], self.m_nActivityType)
                tNewObj:setCallBackFunc(self, self.onClickExchange)
                element:setTag(i - 1)
                element = WZUIContainer:luaTo(element)
                element:setRelativeSize(GlobalMethod:CCSize(1,0.3))
        ...
        elseif self.m_nActivityType == g_tGameActivityTypes.ACTIVITY_NEW_EXCHANGE then
            ...
            if element and tNewObj then
                ...
                element:setTag(i - 1)
                element = WZUIContainer:luaTo(element)
                element:setRelativeSize(GlobalMethod:CCSize(1,0.4))
                flconList:pushBack(element)
...
--@brief    更新子项消耗数量
function CellExchangePanel:updateCellObjConsume()
    for i=1,#self.m_tCellObj do
        self.m_tCellObj[i]:updateConsume()
    end
end
...
function CellExchangePanel:_adaptLanguage_vn(  )
    ...
    local txtDesc1 = GetElement(self.m_root, "txtDesc1", WZUIFreeTextBox)
    txtDesc1:setRelativePosition(GlobalMethod:ccp(-0.13,1))
end
--------------------------------------语言适配End-------------------------------------------