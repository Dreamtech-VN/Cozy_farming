	...
	for i=1,14 do
		...
		if tabItem.id == 203 then
			...
			if self.ratio < 10 then
				...
				txt:setRelativePosition(GlobalMethod:ccp(0.83,0.980658))
				if ProjConfig.LANGUAGE == "pt" or ProjConfig.LANGUAGE == "es" or ProjConfig.LANGUAGE == "en" then
...
function WndLuckyGift:_HasDraw(  )
	...
	if ProjConfig.LANGUAGE == "en" or ProjConfig.LANGUAGE == "es" or ProjConfig.LANGUAGE == "pt" then
		...
		txtOP:setRelativePosition(GlobalMethod:ccp(0.066666,0.223333))
		txtSale:setRelativePosition(GlobalMethod:ccp(0.839724,0.513938))
		discount:setScale(0.7)
	elseif ProjConfig.LANGUAGE == "tr" then
		labCnt:setScale(0.6)
...
function WndLuckyGift:onTips( tCell,tag,tData )
    ...
    WndItemInfo:showInfo(tCell.m_root,con,1,tData,false)
end
...
function WndLuckyGift:_starDraw(  )
	...
	elseif ProjConfig.LANGUAGE == "vn" or ProjConfig.LANGUAGE == "pt" 
		or ProjConfig.LANGUAGE == "es" or ProjConfig.LANGUAGE == "tr" or ProjConfig.LANGUAGE == "en" or ProjConfig.LANGUAGE == "ug" then
	...
	if self.m_CardId == 203 then
		...
		elseif ProjConfig.LANGUAGE == "ug" then
			...
			txt:setRelativePosition(GlobalMethod:ccp(0.903333,0.867182))
		end
		conSel:addChild(txt)
    else
...
function WndLuckyGift:onDesc( element )
	...
	if CacheCenter:getGameParam().gameStatus == "1" then 
		WndSingleMapDesc:showInterface(LocalStrings.LUCKYGIFT_DES2)
	else
...
function WndLuckyGift:_adaptLanguage_tr(  )
	...
	GetElement(self.m_root,"txtNextDraw_WndLuckyGift",WZUILabelTTF):setScale(0.7)
...
function WndLuckyGift:_adaptLanguage_ug(  )
	GetElement(self.m_root,"txtBtn1_WndLuckyGift",WZUILabelTTF):setScale(0.7)
	GetElement(self.m_root,"txtBtn2_WndLuckyGift",WZUILabelTTF):setScale(0.7)
	GetElement(self.m_root,"txtBtn3_WndLuckyGift",WZUILabelTTF):setScale(0.7)

	GetElement(self.m_root,"txtLucky1",WZUILabelTTF):setScale(0.62)
	local txtNextDraw = GetElement(self.m_root,"txtNextDraw_WndLuckyGift",WZUILabelTTF)
	txtNextDraw:setScale(0.5)
	txtNextDraw:setRelativePosition(GlobalMethod:ccp(-0.01,0.49026))
	local imgDim = GetElement(self.m_root,"imgDim_WndLuckyGift",WZUIImage)
	imgDim:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	imgDim:setRelativePosition(GlobalMethod:ccp(0,0.55))
	local txtDimNum = GetElement(self.m_root,"txtDimNum_WndLuckyGift",WZUILabelTTF)
	txtDimNum:setRelativePosition(GlobalMethod:ccp(-0.126551,0.5))
	local txtNum = GetElement(self.m_root,"txtNum_WndLuckyGift",WZUILabelTTF)
	txtNum:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	txtNum:setRelativePosition(GlobalMethod:ccp(-0.1,0.5))
	GetElement(self.m_root,"imgLine_WndLuckyGift",WZUIImage):setRelativePosition(GlobalMethod:ccp(-0.121588,0.55))
end
-------------------------------------语言适配End--------------------------------------------