...
function CellLoveLotteryBox:setData(luckNum, rewardSet, bUpdatePrg, lotteryCount, lotteryReward)
	-- body
	WZLog("CellLoveLotteryBox:setData", luckNum, type(self.m_tData))
	if self.m_tData == nil then 
		self.m_tData = {}
		self.m_tData.curTimes = luckNum
		self.m_tData.firstTimes = lotteryCount[1]
		self.m_tData.secondTimes = lotteryCount[2]
		self.m_tData.thirdTimes = lotteryCount[3]
		self.m_tData.lotteryReward = lotteryReward
		self.m_tData.lotteryCount = lotteryCount
	else
