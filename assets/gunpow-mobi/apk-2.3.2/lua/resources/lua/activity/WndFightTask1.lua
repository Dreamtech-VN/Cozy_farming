...
--@note		在这里做场景进入前的准备工作
function WndFightTask1:onEnter(element)
	self.m_root = element
	self:register()
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function WndFightTask1:onExit(element)
	self:_unInit()
	self:unregister()
end
function WndFightTask1:showInterface()
	local wndTask = WndFightTask1:createElement()
	if wndTask ~= nil then
	    WindowManager:addWindow(wndTask,WndFightTask1,nil,false)
	end
end

function WndFightTask1:register()
...
function WndFightTask1:unregister()
...
function WndFightTask1:onEnterTransitionDidFinish(element)
	WindowManagerAni:createAppearAction(self.m_root,true,"actionCallback",self)
end
function WndFightTask1:actionCallback()
	for i=1,2 do
		local tab = {}
	...
	ProtocolProcessorFestivalActivity:send_ACTIVITY2_GetTaskList(g_cityExtenInfo.activity7200, 2)
	self:setDayRedPoint(GlobalGame.g_tRedPointTypeList[127200])
	self:setGroupRedPoint(GlobalGame.g_tRedPointTypeList[117200])
end

function WndFightTask1:onBtnChangeTitle(element)
	...
	if tag == 2 and self.m_tTaskTypeData[2] == nil then
		ProtocolProcessorFestivalActivity:send_ACTIVITY2_GetTaskList(g_cityExtenInfo.activity7200, 1)
	end
	self.m_nCurIndex = tag
end
function WndFightTask1:setDayRedPoint(visible)
	visible = visible or false
	if not self.m_root then return end
	GetElement(self.m_root,"dayRedPoint",WZUIImage):setVisible(visible)
end
function WndFightTask1:setGroupRedPoint(visible)
	visible = visible or false
	if not self.m_root then return end
	GetElement(self.m_root,"groupRedPoint",WZUIImage):setVisible(visible)
end
function WndFightTask1:onBtnClose()
...
-------------------------------------私有方法模块Begin--------------------------------------
function WndFightTask1:_onGetTaskInfo(activityId, activityType, taskType, id, status, target, progress, progressCount, refreshTime)
	if activityId == tonumber(g_cityExtenInfo.activity7200) then
		...
		for i = 1, #self.m_tTaskTypeData[taskType] do
			local element, tLuaObj = CellFightTaskItem1:createElement()
...
function WndFightTask1:_onGetTaskResult(activityId, taskId)
	if tonumber(g_cityExtenInfo.activity7200) == activityId then
		local _type = nil
		for i=1,2 do
		...
		if _type and self.m_tCellFightTaskItem[_type] then
			...
			if _type == 1 then
				GlobalGame.g_tRedPointTypeList[127200] = _status
			elseif _type == 2 then
				GlobalGame.g_tRedPointTypeList[117200] = _status
			end
			self:setDayRedPoint(GlobalGame.g_tRedPointTypeList[127200])
			self:setGroupRedPoint(GlobalGame.g_tRedPointTypeList[117200])
			WndFightActivity:setFightTaskPoint(GlobalGame.g_tRedPointTypeList[127200] or GlobalGame.g_tRedPointTypeList[117200])
			SceneCity:setSceneMainIconRedPoint(FIGHT_ACTIVITY, GlobalGame.g_tRedPointTypeList[127200] or GlobalGame.g_tRedPointTypeList[117200])
		end
	end
end
...
-------------------------------------语言适配begin----------------------------------------
function WndFightTask1:_adaptLanguage_vn()
