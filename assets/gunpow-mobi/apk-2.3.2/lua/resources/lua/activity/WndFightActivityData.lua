--WndFightActivityData.lua
--@brief	WndFightActivity的数据模块
--@date		2021/06/21
--@author	hyx
--@note		战力提升
...
function WndFightActivity:_init()
...
function WndFightActivity:_unInit()
	self.m_root = nil
	self.m_tRoleData = nil
end
...
--@note		请仅用此方法创建场景
function WndFightActivity:createElement()
	if WndFightActivity.m_root ~= nil then
		WindowManager:removeWindow(WndFightActivity.m_root, WndFightActivity, true)
	end
	local element = WZUISystem:getInstance():createElement("WndFightActivity")
	assert(element, "WndFightActivity create element failed!")
	self:_init()
	return element
end
...
--@brief	反初始化表的成员变量
--@note		在退出场景时回调的onExit函数里面必须调用本函数
function CellFightItem:_unInit()
	self.m_root = nil
	self.m_nType = nil 
end

--@brief	创建控件
function CellFightItem:createElement()
...
function CellFightItem:setFightItemData(data, nType)
	self.m_nFightData = data
	self.m_nType = nType or 1
end
--@brief 	开始加载
function CellFightItem:onLoadData(element)
...
function CellFightItem:setData()
...
function CellFightItem:onBtnHead()
...
function CellFightItem:onItemClick(tCell,tag,tData)
	if tData == nil then
       return
    end
    WndItemInfo:onCloseClick()
    WZLog("CellFightItem:onItemClick", self.m_nType)
    local tempRoot = WndFightActivity.m_root
    if self.m_nType == 2 then 
