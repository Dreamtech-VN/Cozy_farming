--WndFightActivity.lua
--@brief	WndFightActivity的UI模块
--@date		2021/06/21
--@author	hyx
--@note		战力提升
...
--@note		在这里做场景进入前的准备工作
function WndFightActivity:onEnter(element)
	self.m_root = element
	self:register()
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function WndFightActivity:onExit(element)
	self:_unInit()
	self:unregister()
	ProtocolProcessorWndBag:unregAll1()
end

function WndFightActivity:showInterface()
	local wndFight = WndFightActivity:createElement()
	if wndFight ~= nil then
	    WindowManager:addWindow(wndFight,WndFightActivity,nil,false)
	end
end

function WndFightActivity:register()
...
function WndFightActivity:unregister()
...
function WndFightActivity:onEnterTransitionDidFinish(element)
	WindowManagerAni:createAppearAction(self.m_root,true,"actionCallback",self)
end
function WndFightActivity:actionCallback()
...
function WndFightActivity:setFightTaskPoint(visible)
	if not self.m_root then return end
	visible = visible or false
	GetElement(self.m_root,"fightTaskRedPoint",WZUIImage):setVisible(visible)
end
function WndFightActivity:onBtnRole()
...
function WndFightActivity:onBtnRule()
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	WndFourStarRuleDesc:showInterface(LocalStrings.ACTIVITY_TEXT51)
end
function WndFightActivity:onBtnTask()
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	WndFightTask:showInterface()
end
function WndFightActivity:onBtnClose()
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	WindowManager:removeWindow(self.m_root, self, true)
end

--@brief 	红点
function WndFightActivity:showRedDot()
	-- body
	local status = GlobalGame.g_tRedPointTypeList[127018] or GlobalGame.g_tRedPointTypeList[117018]
    self:setFightTaskPoint(status)

    WndFightTask:setDayRedPoint(GlobalGame.g_tRedPointTypeList[127018])
    WndFightTask:setGroupRedPoint(GlobalGame.g_tRedPointTypeList[117018])
end
-------------------------------------公有方法模块End----------------------------------------


-------------------------------------私有方法模块Begin--------------------------------------
function WndFightActivity:_onGetFightInfo(activityId,maxCount,count,status, rewardCounts, rewardItems,rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
...
function WndFightActivity:_onGetRankResultInfo(activityId, activityType, rankingType, myPoint, myRanking, rewardConfig, playerIds, ranks, points, nickname, headIds, 
