--WndFightActivity1Data.lua
--@brief	WndFightActivity1的数据模块
--@date		2021/06/21
--@author	hyx
--@note		战力提升
...
function WndFightActivity1:_init()
...
function WndFightActivity1:_unInit()
	self.m_root = nil
	self.m_tRoleData = nil
end
...
--@note		请仅用此方法创建场景
function WndFightActivity1:createElement()
	if WndFightActivity1.m_root ~= nil then
		WindowManager:removeWindow(WndFightActivity1.m_root, WndFightActivity1, true)
	end
	local element = WZUISystem:getInstance():createElement("WndFightActivity1")
	assert(element, "WndFightActivity1 create element failed!")
	self:_init()
	return element
end
...
--@brief	反初始化表的成员变量
--@note		在退出场景时回调的onExit函数里面必须调用本函数
function CellFightItem1:_unInit()
	self.m_root = nil
	self.m_nType = nil 
end

--@brief	创建控件
function CellFightItem1:createElement()
...
function CellFightItem1:setFightItemData(data, nType)
	self.m_nFightData = data
	self.m_nType = nType or 1
end
--@brief 	开始加载
function CellFightItem1:onLoadData(element)
...
function CellFightItem1:setData()
...
function CellFightItem1:onBtnHead()
...
function CellFightItem1:onItemClick(tCell,tag,tData)
	if tData == nil then
       return
    end
    WndItemInfo:onCloseClick()
    WZLog("CellFightItem1:onItemClick", self.m_nType)
    local tempRoot = WndFightActivity1.m_root
    if self.m_nType == 2 then 
