--WndFightActivity2.lua
--@brief	WndFightActivity2的UI模块
--@date		2021/06/21
--@author	hyx
--@note		战力提升
...
--@note		在这里做场景进入前的准备工作
function WndFightActivity2:onEnter(element)
	self.m_root = element
	self:register()
...
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function WndFightActivity2:onExit(element)
	self:_unInit()
	self:unregister()
	ProtocolProcessorWndBag:unregAll1()
end

function WndFightActivity2:showInterface()
	local wndFight = WndFightActivity2:createElement()
	if wndFight ~= nil then
	    WindowManager:addWindow(wndFight,WndFightActivity2,nil,false)
	end
end

function WndFightActivity2:register()
...
function WndFightActivity2:unregister()
...
function WndFightActivity2:onEnterTransitionDidFinish(element)
	WindowManagerAni:createAppearAction(self.m_root,true,"actionCallback",self)
end
function WndFightActivity2:actionCallback()
	ProtocolProcessorWndActivityOnLine:send_ACTIVITY_GetActivityInfo(g_cityExtenInfo.activity7018, 7018)
	ProtocolProcessorFestivalActivity:send_ACTIVITY2_GetRankingList(g_cityExtenInfo.activity7018, 1)

	local status = false
	if GlobalGame.g_tRedPointTypeList then
		status = GlobalGame.g_tRedPointTypeList[127018] or GlobalGame.g_tRedPointTypeList[117018]
	end
    self:setFightTaskPoint(status)
end

function WndFightActivity2:setFightTaskPoint(visible)
	if not self.m_root then return end
	visible = visible or false
	GetElement(self.m_root,"fightTaskRedPoint",WZUIImage):setVisible(visible)
end
function WndFightActivity2:onBtnRole()
...
function WndFightActivity2:onBtnRule()
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	WndFourStarRuleDesc:showInterface(LocalStrings.ACTIVITY_TEXT51)
end
function WndFightActivity2:onBtnTask()
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	WndFightTask2:showInterface()
end
function WndFightActivity2:onBtnClose()
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	WindowManager:removeWindow(self.m_root, self, true)
end

--@brief 	红点
function WndFightActivity2:showRedDot()
	-- body
	local status = GlobalGame.g_tRedPointTypeList[127018] or GlobalGame.g_tRedPointTypeList[117018]
    self:setFightTaskPoint(status)

    WndFightTask2:setDayRedPoint(GlobalGame.g_tRedPointTypeList[127018])
    WndFightTask2:setGroupRedPoint(GlobalGame.g_tRedPointTypeList[117018])
end
-------------------------------------公有方法模块End----------------------------------------


-------------------------------------私有方法模块Begin--------------------------------------
function WndFightActivity2:_onGetFightInfo(activityId,maxCount,count,status, rewardCounts, rewardItems,rewardItemsParamCount, startTime, endTime, content, rewardId, finishCondition, tips)
	if activityId == tonumber(g_cityExtenInfo.activity7018) then
...
function WndFightActivity2:_onGetRankResultInfo(activityId, activityType, rankingType, myPoint, myRanking, rewardConfig, playerIds, ranks, points, nickname, headIds, 
	...
	if activityId == tonumber(g_cityExtenInfo.activity7018) then
		...
		for i = 1, #tData do
			local element, tLuaObj = CellFightItem2:createElement()
