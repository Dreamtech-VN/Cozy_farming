--WndFightTask1Data.lua
--@brief	WndFightTask1的数据模块
--@date		2021/06/21
--@author	hyx
--@note		战力任务
...
function WndFightTask1:_unInit()
	self.m_root = nil
	self.m_nCurIndex = nil
	self.m_tTaskTitle = {}
	self.m_tTaskTypeView = {}
...
--@note		请仅用此方法创建场景
function WndFightTask1:createElement()
	if WndFightTask1.m_root ~= nil then
		WindowManager:removeWindow(WndFightTask1.m_root, WndFightTask1, true)
	end
	local element = WZUISystem:getInstance():createElement("WndFightTask1")
	assert(element, "WndFightTask1 create element failed!")
	self:_init()
	return element
end
...
--=========== 战力飞升榜任务子项 ===============
CellFightTaskItem1 = {}
function CellFightTaskItem1:_init()
	self.m_root = nil	 	  			--场景根节点
...
--@brief	反初始化表的成员变量
--@note		在退出场景时回调的onExit函数里面必须调用本函数
function CellFightTaskItem1:_unInit()
	self.m_root = nil
	self.m_tGoodItemCell = {}
	self.m_nTaskRewardId = nil
end
...
--@brief	创建控件
function CellFightTaskItem1:createElement()
...
function CellFightTaskItem1:setFightTaskData(index, data)
	self.m_nIndex = index
	self.m_tFightTaskData = data
end
--@brief 	开始加载
function CellFightTaskItem1:onLoadData(element)
...
function CellFightTaskItem1:setData()
...
function CellFightTaskItem1:setFightTaskItemData(index, data)
	...
	for i=1, #data.reward do
		...
		if self.m_tGoodItemCell[index][i] and self.m_tGoodItemCell[index][i].celElement and self.m_tGoodItemCell[index][i].tLuaObj then
			...
			tLuaObj:setItemClickFun(WndFightTask1,self.onItemClick)
			local _x = 45 + (i-1) * 85
...
function CellFightTaskItem1:onItemClick(tCell,tag,tData)
	if tData == nil then
       return
    end
    WndItemInfo:onCloseClick()
   	WndItemInfo:showInfo(tCell.m_root,WndFightTask1.m_root,1,tData,false,nil,true)
end
function CellFightTaskItem1:onBtnGet()
	SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)
	if self.m_nTaskRewardId then
		ProtocolProcessorFestivalActivity:send_ACTIVITY2_ReceiveTaskReward(tonumber(g_cityExtenInfo.activity7200), self.m_nTaskRewardId)
	end
end
--@return	新建的表实例对象
function CellFightTaskItem1:_new( )
	local tNewObj = {}
...
-------------------------------------语言适配begin----------------------------------------
function CellFightTaskItem1:_adaptLanguage_vn()
