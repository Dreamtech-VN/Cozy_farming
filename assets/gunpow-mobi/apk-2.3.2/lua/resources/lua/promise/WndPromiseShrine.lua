...
function WndPromiseShrine:setVisibleStatus(visible)
...
function WndPromiseShrine:_showGiftState()
	...
	if 0 > tonumber(tConfig.item_id) then 
		--设置超值改为提示文字
		-- self.m_labelBuyBtn:setUseSystemFont(true)
		...
		local tRechargeConfig = self:_getRechargeConfig()
		if not tRechargeConfig then return end
...
function WndPromiseShrine:_adaptLanguage_ug(  )
	GetElement(self.m_root,"labelBuyButton",WZUILabelTTF):setScale(0.7)

	local labelBuyBtn = GetElement(self.m_root, "labelBuyBtn", WZUILabelTTF)
	labelBuyBtn:setScale(0.7)
	labelBuyBtn:setDimensions(GlobalMethod:CCSize(150))

	GetElement(self.m_root, "txtOpenDate_WndPromiseShrine", labelOpenDate):setRelativePosition(GlobalMethod:ccp(0.34,0.5))
	local labelOpenDate = GetElement(self.m_root, "labelOpenDate", WZUILabelTTF)
	labelOpenDate:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	labelOpenDate:setRelativePosition(GlobalMethod:ccp(0.14,0.5))

	local labelCountdown = GetElement(self.m_root, "labelCountdown", WZUILabelTTF)
	labelCountdown:setAnchorPoint(GlobalMethod:ccp(1,0.5))
	labelCountdown:setRelativePosition(GlobalMethod:ccp(0.4,0.05))
	local labelCountdownTitle = GetElement(self.m_root, "labelCountdownTitle", WZUILabelTTF)
	labelCountdownTitle:setRelativePosition(GlobalMethod:ccp(0.83,0.05))

	local txtPrice1 = GetElement(self.m_root, "txtPrice1_WndPromiseShrine", WZUILabelTTF)
	txtPrice1:setScale(0.6)
	txtPrice1:setDimensions(GlobalMethod:CCSize(220))
	local txtPrice2 = GetElement(self.m_root, "txtPrice2_WndPromiseShrine", WZUILabelTTF)
	txtPrice2:setScale(0.6)
	txtPrice2:setDimensions(GlobalMethod:CCSize(220))
	local txtPrice3 = GetElement(self.m_root, "txtPrice3_WndPromiseShrine", WZUILabelTTF)
	txtPrice3:setScale(0.6)
	txtPrice3:setDimensions(GlobalMethod:CCSize(220))
	local txtPrice4 = GetElement(self.m_root, "txtPrice4_WndPromiseShrine", WZUILabelTTF)
	txtPrice4:setScale(0.6)
	txtPrice4:setDimensions(GlobalMethod:CCSize(220))

	local labelGiftName = GetElement(self.m_root,"labelGiftName",WZUIFreeTextBox)
	labelGiftName:setScale(0.7)
	labelGiftName:setMaxWidth(640)
	local labelDesc = GetElement(self.m_root, "labelDesc", WZUIFreeTextBox)
	labelDesc:setScale(0.7)
	labelDesc:setMaxWidth(640)
end
---------------------------------------语言适配End----------------------------------------