...
GlobalGame = 
{
    ...
    g_nProfessionTaskCount = 0, --职业任务未领取数量
    ...
    g_autoReturnActivity = false,    --弹回流活动活动标记
    g_bisLogined = false,
    ...
    g_autoReturneeActivity = true,  --进游戏自动弹回归活动标记

    g_autoSummerActivity = 1, --弹暑假活动标记
    g_autoSummerStartT = nil,  --暑假活动开始时间
    g_autoSummerEndT = nil , --暑假活动结束时间
    g_autoThankfulSign = true , --自动弹感恩打卡活动
    g_autoWelcomeBack = true, --自动弹欢迎回来活动
    g_autoSevenYear = true , --自动弹七周年签到活动
    ...
    --房间所属属性    17:夫妻争霸 18战略赛
    g_tRoomChannel = {BATTLE_CHANNEL_DZ = 1,BATTLE_CHANNEL_YL= 2,BATTLE_CHANNEL_PW = 3,BATTLE_CHANNEL_SJ = 4,BATTLE_CHANNEL_ZF = 5,BATTLE_CHANNEL_FF = 6,BATTLE_CHANNEL_GF= 7,BATTLE_CHANNEL_GZ = 8,BATTLE_CHANNEL_LS = 9,BATTLE_CHANNEL_LX = 10,BATTLE_CHANNEL_QS = 11,BATTLE_CHANNEL_WTB = 12,BATTLE_CHANNEL_YXT  = 13, BATTLE_CHANNEL_DTF = 15, BATTLE_CHANNEL_DZTZ = 16, BATTLE_CHANNEL_FQZB = 17, BATTLE_CHANNEL_ZLS = 18},
    --匹配模式
    ...
    G_ClownTreasure_Quick = 1,   --寻宝跳过动画 (0：不跳过，1：跳过)
    ...
    g_pvpZlsPunishTime = nil,               --战略赛惩罚时间
    g_pvpZlsPunishTimeCurServiceT = 0,      --获取战略赛惩罚时间时服务器的时间

    g_oppo_channel = "-1", --判断是否有oppo买量渠道标志1-自然用户 3-广告用户、广告id
    g_oppo_adId = "-1",    --判断是否有oppo买量渠道标志1-自然用户 3-广告用户、广告id
    g_oppo_isAmberPlayer = false, --判断是否是oppo琥珀大玩家
    g_oppo_isStartFromGameCenter = false, --判断游戏是否是从游戏中心启动
    ...
    g_questionOpen = false, --
    g_bIsAutoFightOpen = false,     --是否开启自动战斗
    g_saveAutoFightOpenState = false,     --自动战斗开启状态
    g_saveBigSkillType = 1,     --保存战斗中大招的索引 1->武器大招；2->皮肤大招
    g_bookRed = {mountRed = false,footRed = false,skinRed = false},
    g_bIsShowDelAccount = false,    --是否显示删除账号
    g_bIsShowDelRoleTipsInMainCity = false,    --全局保存一下是否在主城已经显示了删除角色弹窗
    g_nSpAddTimes = 3,          --挖坑赛怒气增长倍数
    g_xmlDoc = nil,             --全局定向动态资源下载DressAniDownloadConfig.xml缓存数据
    g_xmlDoc_All = nil,             --全局定向动态资源下载DressAniDownloadConfig_All.xml缓存数据
    g_bIsDownloadingResCheckAll = false,    --是否已经开始下载DressAniDownloadConfig_All.xml的资源
    g_bIsConnectQQHallWebSocket = false,    --是否已经连接过qq大厅的websocket通信，多次连接会出错
    g_bIsPushByServer = true,               --是否是登陆进主城，服务器推送的
    g_bSendEventPerMinite = false,          --每分钟发送一个pulse事件状态
    g_nLoginInCityTime = 0,                 --登录进入主城时间戳
}
...
GlobalGame.g_tPlayerInfo = 
{
    ...
    nPlayerStatus,      -- status : 角色状态,0、禁言,1、正常,2、注销
    nUnregisterTime,    --注销时间,单位:秒
    sClientIp,          --客户端玩家ip
}
...
function GlobalGame:load_plist_pack()
    ...
    if WZFileUtil:isFileExist("pack/common/pack_common_1.plist") then
        CCSpriteFrameCache:sharedSpriteFrameCache():removeSpriteFramesFromFile("pack/common/pack_common_1.plist")
    end
    if WZFileUtil:isFileExist("pack/common/pack_common_2.plist") then
        CCSpriteFrameCache:sharedSpriteFrameCache():removeSpriteFramesFromFile("pack/common/pack_common_2.plist")
        CCSpriteFrameCache:sharedSpriteFrameCache():removeSpriteFramesFromFile("pack/common/pack_common_3.plist")
    end
    if WZFileUtil:isFileExist("pack/common/pack_common_3.plist") then
        CCSpriteFrameCache:sharedSpriteFrameCache():removeSpriteFramesFromFile("pack/common/pack_common_3.plist")
    end
    if WZFileUtil:isFileExist("pack/common/pack_common_4.plist") then
        CCSpriteFrameCache:sharedSpriteFrameCache():removeSpriteFramesFromFile("pack/common/pack_common_4.plist")
    end
    ...
    if WZFileUtil:isFileExist("pack/city/pack_city_1.plist") then
        CCSpriteFrameCache:sharedSpriteFrameCache():removeSpriteFramesFromFile("pack/city/pack_city_1.plist")
    end
    if WZFileUtil:isFileExist("pack/city/pack_city_2.plist") then
    ...
    if WZFileUtil:isFileExist("pack/common/pack_common_2.plist") then
        CCSpriteFrameCache:sharedSpriteFrameCache():addSpriteFramesWithFile("pack/common/pack_common_2.plist")
        CCSpriteFrameCache:sharedSpriteFrameCache():addSpriteFramesWithFile("pack/common/pack_common_3.plist")
    end
    if WZFileUtil:isFileExist("pack/common/pack_common_3.plist") then
        CCSpriteFrameCache:sharedSpriteFrameCache():addSpriteFramesWithFile("pack/common/pack_common_3.plist")
    end
    if WZFileUtil:isFileExist("pack/common/pack_common_4.plist") then
        CCSpriteFrameCache:sharedSpriteFrameCache():addSpriteFramesWithFile("pack/common/pack_common_4.plist")
    end
    if WZFileUtil:isFileExist("pack/common/pack_"..ProjConfig.LANGUAGE.."_common_0.plist") then
...
function GlobalGame:reset()
    ...
    self.g_nProfessionTaskCount = 0 --职业任务未领取数量
    ...
    self.g_autoReturnActivity = false
    ...
    self.g_questionOpen = false 
    self.g_bIsAutoFightOpen = false
    self.g_autoReturneeActivity = true  --进游戏自动弹回归活动标记
    self.g_pvpPunishTime = nil          --排位赛惩罚时间
    self.g_pvpZlsPunishTime = nil       --战略赛惩罚时间
    self.g_saveAutoFightOpenState = false
    self.g_saveBigSkillType = 1
    self.g_serverTaskTime = 0
    g_QuickRedPackState = false 
    g_QuickRedPackState2 = false 
    self.g_nSpAddTimes = 3          --挖坑赛怒气增长倍数
    self.g_bIsPushByServer = true 
    self.g_autoThankfulSign = true 
    self.g_autoWelcomeBack = true
    self.g_autoSevenYear = true
    self.g_bSendEventPerMinite = false
    self.g_nLoginInCityTime = 0     --登录进入主城时间戳
end
