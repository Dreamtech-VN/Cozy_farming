--CellBattleTeachDialogData.lua
--@brief	CellBattleTeachDialog的数据模块
--@date		2015/09/18
--@author	莫剑峰
--@note		对话框

CellBattleTeachDialog = {
	-- 请在这里定义和初始化全局成员变量
	DIMG = "ui/teach/dialog_box_1.png",
	RIMG = "ui/teach/dialog_box_2.png",
	CIMG = "ui/teach/dialog_box_3.png",
...
function CellBattleTeachDialog:_init()
...
function CellBattleTeachDialog:_unInit()
	self.m_root = nil
	self.m_nOffsetX = 0
	self.m_nOffsetY = 0
	self.m_nShowTime = nil
...
--@note		Order为默认值CellBattleTeachDialog.ZORDER
function CellBattleTeachDialog:createDialog(sText,dir,nShowTime,tBackSender,tBackFunction,nOffsetX,nOffsetY,nMaxWidth)
	local tNewObj = self:_new()
	assert(tNewObj, "CellBattleTeachDialog table create failed!")
	tNewObj:_init()
	local element = WZUIContainer:create()
	assert(element, "CellBattleTeachDialog element create failed!")
	...
	tNewObj.m_nDir = dir or CellBattleTeachDialog.DIR_RIGHT
	tNewObj.m_tBackSender = tBackSender

	element:setLuaObjectIndex(tNewObj)

	element:setZOrder(CellBattleTeachDialog.ZORDER)
...
--@return	#2, 表的引用，也可以用element:getLuaObjectIndex()
--@note		Order为默认值CellBattleTeachDialog.ZORDER
function CellBattleTeachDialog:addDialog(tSender,tParent,sText,dir,nShowTime,tBackSender,tBackFunction,nOffsetX,nOffsetY,nMaxWidth,nScale,bJump,tTxtColor,bIsUpdatePos,tFollowObj, zOrder, tag)
	local tNewObj = self:_new()
	assert(tNewObj, "CellBattleTeachDialog table create failed!")
	tNewObj:_init()
	local element = WZUIContainer:create()
    ...
    WZLog("CellBattleTeachDialog:addDialog", tNewObj.m_bIsUpdatePos)
	return element,tNewObj
end
-------------------------------------公有方法模块End----------------------------------------
...
--@return	新建的表实例对象
function CellBattleTeachDialog:_new( )
	local tNewObj = {}
