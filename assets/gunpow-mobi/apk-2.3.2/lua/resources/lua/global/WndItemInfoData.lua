...
function WndItemInfo:_init()
    ...
    self.m_ItemExpSize = CCSize(220,80)
...
function WndItemInfo:_setTipDataByType(nType,tData,pt,tOther)
	...
	WZLog("_setTipDataByType::::",nType, Serialize(tData))
...
function WndItemInfo:setEquipData(tData,pt)
	...
	self.m_tEquip.type = tData.type or 0
	self.m_tEquip.origin = tData.origin
	if tData.randomBtnVisible ~= nil then 
		self.m_tEquip.randomBtnVisible = tData.randomBtnVisible
	end
	self:_updateInfo()
	tData = nil
	pt = nil
...
function WndItemInfo:_getPropertyData()
	...
	if self.m_tEquip.basicInfo.main_type == 7 then return end    
	...
	if self.m_tEquip.basicInfo.main_type == 1 and self.m_tEquip.basicInfo.sub_type == 51 then return end
	...
	elseif self.m_tEquip.basicInfo.main_type == 25 and (self.m_tEquip.basicInfo.sub_type == 1 or self.m_tEquip.basicInfo.sub_type == 3) then
		tPro = nil
		local backgroundcardInfo = GDatatab_backgroundcard["id_"..self.m_tEquip.basicInfo.value]
		if backgroundcardInfo then
			tPro = backgroundcardInfo.property
		end
	elseif self.m_tEquip.basicInfo.main_type == MAIN_PROPS_TYPE and self.m_tEquip.basicInfo.sub_type == 11 then --坐骑
		local tMountData = GDatatab_mounts["id_" .. self.m_tEquip.basicInfo.value]
		if tMountData then 
			tPro = GDatatab_item["id_".. tMountData.item_id].property
		end
	elseif self.m_tEquip.basicInfo.main_type == 20 then --皮肤
		local tSkinData = GDatatab_shape_skins["id_" .. tPro[1][1]]
		if tSkinData then 
			local tempPro = tSkinData.property
			tPro = {}
			for i = 1, tSkinData.quality do
				table.insert(tPro, tempPro[i])
			end
		end
	elseif self.m_tEquip.basicInfo.main_type == 23 then --足迹
		local tFootMarkData = GDatatab_footmark["id_" .. tPro[1][1]]
		if tFootMarkData then 
			tPro = tFootMarkData.property
		end
	end
	...
	for i,data in pairs(tPro) do
		...
		if mainType == 38 then --灵石之源
			...
			if self.m_tEquip.extraInfo.spriteStoneQuality then
				value2 = self.m_tEquip.extraInfo.spriteStoneQuality
			else
				value2 = LocalStrings.NEW_SHOP_1
			end
		...
		local stoneAttr = 0
		--武器减去宝石攻击力
		if (mainType == 4 and (subType == 0 or subType == 1) or mainType == 43 and (subType == 0 or subType == 4)) and self.m_tEquip.extraInfo.attackStone ~= nil and self.m_tEquip.extraInfo.attackStone ~= 0 then
			stoneAttr = GDatatab_item["id_"..self.m_tEquip.extraInfo.attackStone].property[1][2]
			value3 = value3 - stoneAttr
		end
		--手镯减去宝石防御
		if (mainType == 4 and (subType == 4) or mainType == 43 and (subType == 2 or subType == 3)) and self.m_tEquip.extraInfo.defendStone ~= nil and self.m_tEquip.extraInfo.defendStone ~= 0 then
			stoneAttr = GDatatab_item["id_"..self.m_tEquip.extraInfo.defendStone].property[1][2]
			value3 = value3 - stoneAttr
		end
		--宝物和勋章减去宝石生命
		if (mainType == 4 and (subType == 5 or subType == 6) or mainType == 43 and (subType == 1 or subType == 5)) and self.m_tEquip.extraInfo.hpStone ~= nil and self.m_tEquip.extraInfo.hpStone ~= 0 then
	...
	local tItem2 = {}
	if self.m_tEquip.basicInfo.main_type == 50 then 
		local tConfigData = self:getFlowerpotFieldProperty(self.m_tEquip.basicInfo.id)
		if tConfigData then 
...
function WndItemInfo:_getStoneData()
	...
	if self.m_tEquip.extraInfo.attackStone and self.m_tEquip.extraInfo.attackStone > 0 then--攻击宝石等级
		if self.m_tEquip.extraInfo.attackStone > 41000 and self.m_tEquip.extraInfo.attackStone < 50000 then 
			tStone[1] = {}
			tStone[1].value1 = self:_checkMagicStone(GetItemLocalData(self.m_tEquip.extraInfo.attackStone))
			tStone[1].bIsMagicStone = true
		else
			local title,desc,desc1 = self:_checkStone(GetItemLocalData(self.m_tEquip.extraInfo.attackStone))
			tStone[1] = self:_checkExp(title,desc,desc1,ccc3(127,70,26),ccc3(127,70,26),ccc3(5,180,0),20,20,20)
		end
		tStone[1].itemId = self.m_tEquip.extraInfo.attackStone
	else
		if self.m_tEquip.basicInfo.main_type == 43 then
			tStone[1] = self:_checkExp(LocalStrings.PET_EQUIPMENT_9[1]..":",LocalStrings.UNMOUNTED,"0",ccc3(127,70,26),ccc3(127,70,26),20,20,20)--"攻击玉石:未镶嵌"
		else
			tStone[1] = self:_checkExp(LocalStrings.ATTACK_STONE_1..":",LocalStrings.UNMOUNTED,"0",ccc3(127,70,26),ccc3(127,70,26),20,20,20)--"攻击宝石:未镶嵌"
		end
	end
	if self.m_tEquip.extraInfo.hpStone and self.m_tEquip.extraInfo.hpStone > 0 then --生命宝石等级
		if self.m_tEquip.extraInfo.hpStone > 41000 and self.m_tEquip.extraInfo.hpStone < 50000 then 
			tStone[2] = {}
			tStone[2].value1 = self:_checkMagicStone(GetItemLocalData(self.m_tEquip.extraInfo.hpStone))
			tStone[2].bIsMagicStone = true
		else
			local title,desc,desc1 = self:_checkStone(GetItemLocalData(self.m_tEquip.extraInfo.hpStone))
			tStone[2] = self:_checkExp(title,desc,desc1,ccc3(127,70,26),ccc3(127,70,26),ccc3(5,180,0),20,20,20)
		end
		tStone[2].itemId = self.m_tEquip.extraInfo.hpStone
	else
		if self.m_tEquip.basicInfo.main_type == 43 then
			tStone[2] = self:_checkExp(LocalStrings.PET_EQUIPMENT_9[2]..":",LocalStrings.UNMOUNTED,"0",ccc3(127,70,26),ccc3(127,70,26),20,20,20)--"生命玉石:未镶嵌"
		else
			tStone[2] = self:_checkExp(LocalStrings.HP_STONE..":",LocalStrings.UNMOUNTED,"0",ccc3(127,70,26),ccc3(127,70,26),20,20,20)--"生命宝石:未镶嵌"
		end
	end
	if self.m_tEquip.extraInfo.defendStone and self.m_tEquip.extraInfo.defendStone > 0 then--防御宝石等级
		if self.m_tEquip.extraInfo.defendStone > 41000 and self.m_tEquip.extraInfo.defendStone < 50000 then 
			tStone[3] = {}
			tStone[3].value1 = self:_checkMagicStone(GetItemLocalData(self.m_tEquip.extraInfo.defendStone))
			tStone[3].bIsMagicStone = true
		else
			local title,desc,desc1 = self:_checkStone(GetItemLocalData(self.m_tEquip.extraInfo.defendStone))
			tStone[3] = self:_checkExp(title,desc,desc1,ccc3(127,70,26),ccc3(127,70,26),ccc3(5,180,0),20,20,20)
		end
		tStone[3].itemId = self.m_tEquip.extraInfo.defendStone
	else
		if self.m_tEquip.basicInfo.main_type == 43 then
			tStone[3] = self:_checkExp(LocalStrings.PET_EQUIPMENT_9[3]..":",LocalStrings.UNMOUNTED,"0",ccc3(127,70,26),ccc3(127,70,26),20,20,20)--"防御玉石:未镶嵌"
		else
			tStone[3] = self:_checkExp(LocalStrings.DEFENSE_STONE_1..":",LocalStrings.UNMOUNTED,"0",ccc3(127,70,26),ccc3(127,70,26),20,20,20)--"防御宝石:未镶嵌"
		end
	end
	if self.m_tEquip.extraInfo.gongmingStone and self.m_tEquip.extraInfo.gongmingStone > 0 then--共鸣宝石等级
		...
		tStone[4] = self:_checkExp(title,desc,desc1,ccc3(127,70,26),ccc3(127,70,26),ccc3(5,180,0),20,20,20)
		tStone[4].itemId = self.m_tEquip.extraInfo.gongmingStone
	else
		if self.m_tEquip.basicInfo.main_type == 43 then
			tStone[4] = self:_checkExp(LocalStrings.PET_EQUIPMENT_9[4]..":",LocalStrings.UNMOUNTED,"0",ccc3(127,70,26),ccc3(127,70,26),20,20,20)--"共鸣玉石:未镶嵌"
		else
			tStone[4] = self:_checkExp(LocalStrings.NEWSTONE2..":",LocalStrings.UNMOUNTED,"0",ccc3(127,70,26),ccc3(127,70,26),20,20,20)--"共鸣宝石:未镶嵌"
		end
	end
...
--@brief	获取魔力石头文本
function WndItemInfo:_checkMagicStone(tStone)
	local sFormat = [[<T C="127,70,26" S="20" P="1">%s</T><T C="127,70,26" S="20" P="1">(%s</T><T C="5,180,0" S="20" P="1">+%d</T><T C="127,70,26" S="20" P="1"> %s</T><T C="5,180,0" S="20" P="1">+%d</T><T C="127,70,26" S="20" P="1">%s</T>]]
	local title = ""
	if tStone.sub_type == 1 then
		title = tStone.name .. ":"--攻击宝石
	elseif tStone.sub_type == 0 then
		title = tStone.name .. ":"--生命宝石
	elseif tStone.sub_type == 2 then
		title = tStone.name .. ":"--防御宝石
	end

	local desc1 = string.format(sFormat, title, ATTR_TITLE[tStone.property[1][1]], tStone.property[1][2], ATTR_TITLE[tStone.property[2][1]], tStone.property[2][2], ") ")
...
function WndItemInfo:_getSkillData()
	...
	if self.m_tEquip.extraInfo.skillId1 and self.m_tEquip.extraInfo.skillId1 > 0 then--添加技能1
		...
		tData[2] = self:_checkExp(remark,"","0",ccc3(127,70,26))
	else--"强化等级达到6级可获得!"
		tData[1] = self:_checkExp(LocalStrings.SKILL.."1:",LocalStrings.NONE,"0",ccc3(193,0,234))
		tData[2] = self:_checkExp(string.format(LocalStrings.STRENGTENRECV,6),"","0",ccc3(127,70,26))
	end
	if self.m_tEquip.extraInfo.skillId2 and self.m_tEquip.extraInfo.skillId2 > 0 then--添加技能2
		...
		tData[4] = self:_checkExp(remark,"","0",ccc3(127,70,26))
	else--"强化等级达到10级可获得!"
		tData[3] = self:_checkExp(LocalStrings.SKILL.."2:",LocalStrings.NONE,"0",ccc3(193,0,234))
		tData[4] = self:_checkExp(string.format(LocalStrings.STRENGTENRECV,10),"","0",ccc3(127,70,26))
	end
	return tData
end
...
--@brief 使用等级高度
function WndItemInfo:_getUseLevelH()
	local txt = WZUILabelTTF:luaTo(self.m_root:getChildElement("txtUseLvel_WndItemInfo"))
	if txt then
		return txt:getContentSize().height + 10
	else
		return 0
...
--@brief	时效时装属性提示和999转永久提示高度
function WndItemInfo:_getDress999AttH()
	local txt = WZUILabelTTF:luaTo(self.m_root:getChildElement("txtDressAtt999_WndItemInfo"))
	if txt then
		return txt:getContentSize().height
	else
		return 0
	end
end
...
function WndItemInfo:_getAllHeight()
	...
	h = h + lockH--]]
	local useLevelH = self:_getUseLevelH()
	h = h + useLevelH
	h = h + self.m_nLineH
	...
	local txt999H = self:_getDress999AttH() --时装属性提示语和永久时装获得
	h = h + txt999H
...
function WndItemInfo:_checkBtnPoint(pt)
	...
	local btnRandomPro = self.m_root:getChildElement("btnRandomPro_WndItemInfo")
	if btnRandomPro then
		local btn = WZUIButton:luaTo(btnRandomPro)
		local x = btn:getPositionX()
		local y = btn:getPositionY()
		local pt1 = btn:convertToNodeSpace(GlobalMethod:ccp(pt.x,pt.y))
		local btnSize = btn:getContentSize()
...
function WndItemInfo:_createColorTxt(data,sName,pt,nDimen,nDimenB,sNameB)
	...
	local color2 = data.color2 or ccc3(127,70,26)
	local color3 = ccc3(5,180,0) --data.color3 or ccc3(0,246,34)
	...
	-- txtA:setEnableStroke(true)
	-- txtA:setStrokeColor(GlobalMethod:ccc3(132,66,29))
	-- txtA:setStrokeSize(4)
	self.m_root:addChild(txtA)
	--已拥有和未获得文字
	local txtA1 = nil 
	if data.value1_1 then 
		txtA1 = WZUILabelTTF:create()
		txtA1:setFontSize(font1)
		txtA1:setText(data.value1_1)
		txtA1:setAnchorPoint(ccp(0,0.5))
		txtA1:setRelativePosition(GlobalMethod:ccp(1, 0.5))
		txtA1:setColor(data.color1_1 or color1)
		txtA:addChild(txtA1, 1)

		nDimen = 0
	end
	--时装可进阶/已进阶字样
	if data.value1_2 then
		local txtA2 = WZUILabelTTF:create()
		txtA2:setFontSize(font1)
		txtA2:setText(data.value1_2)
		txtA2:setAnchorPoint(ccp(0,0.5))
		txtA2:setRelativePosition(GlobalMethod:ccp(1, 0.5))
		txtA2:setColor(data.color1_2 or color1)
		if txtA1 then 
			txtA1:addChild(txtA2, 1)
		else
			txtA:addChild(txtA2, 1)
		end
...
function WndItemInfo:_createFreeText(desc,pt,sName,maxWidth,anchor)
	...
	WZLog("WndItemInfo:_createFreeText 000")
...
function WndItemInfo:_getItemQuality()
	...
	if self.m_tEquip.basicInfo.main_type == 38 and (self.m_tEquip.basicInfo.sub_type >= 9 and self.m_tEquip.basicInfo.sub_type <= 13) then
		...
		if quality_score then 
	        ...
	        return qualityPic[index]
	    end
	end


	local qualityPic = {"ui/common/common_scale9_lv.png","ui/common/common_scale9_lan.png","ui/common/common_scale9_zi.png","ui/common/common_scale9_cheng.png","ui/common/common_scale9_hong.png","ui/common/common_scale9_cai.png"}
    ...
    if self.m_tEquip.basicInfo.main_type == 5 then 
	    local _, _, bIsAdvance = GetDressAdvanceData(self.m_tEquip.basicInfo.id)
	    if bIsAdvance then 
	    	quality = self.m_tEquip.basicInfo.quality + 1
	    end
	end

	return qualityPic[quality]
end

--@brief	品质名称
--@param	nType:用来判断默认颜色
function WndItemInfo:_getItemNameColor(nType)
...
--@brief 	获取相应的花盆土坑属性
function WndItemInfo:getFlowerpotFieldProperty(itemId)
...
--@brief 	显示足迹特效
function WndItemInfo:_showFootMarkEffect()
	if self.m_tEquip.basicInfo.main_type ~= 23 then return end  --足迹
	local tFootMarkData = GDatatab_footmark["id_" .. self.m_tEquip.basicInfo.property[1][1]]
	if tFootMarkData then 
		tPro = tFootMarkData.property
	end
	local tData = {}
	tData.path = "armatures/footprint/" .. tFootMarkData.animation 
	tData.play = "wait"
	tData.loop = true 
	tData.ccp = GlobalMethod:ccp(0.9,0.65) 
	local nodeCon = self.m_root:getChildByTag(222)
	local spineEffect = createEffectSpine(nodeCon, tData)
end
-------------------------------------私有方法模块End----------------------------------------
