...
function YDPlayerAnimation:setMonsterId(sId, otherAnimation)
    ...
    local mosterName = otherAnimation and otherAnimation or skins.animation
...
function YDPlayerAnimation:downloadMonsterCallback(taskId,extraData,failed)
    WZLog("YDPlayerAnimation:downloadMonsterCallback",taskId,extraData,failed)
end
...
function YDPlayerAnimation:loadEffect()
    ...
    local bIsExist = CheckEffectFile(effectPath, true)
    if not bIsExist then
		local downloadInfo = self:getDownloadInfo(sIndex, self.m_sAninName .. "_effect")
	    if downloadInfo == nil then return end 

		DownloadManager:addDownloadTask(1000 + tonumber(self.m_bodyIndex),downloadInfo.url,downloadInfo.md5,sIndex,"downloadEffectCallback",self)
        return
    end
...
function YDPlayerAnimation:downloadEffectCallback(taskId,extraData,failed)
    WZLog("YDPlayerAnimation:downloadEffectCallback",taskId,extraData,failed)
end

function YDPlayerAnimation:_loadEffect()
	local sIndex = self:_formatIndex(tonumber(self.m_bodyIndex))
    local animName = self.m_sAninName .. "_body_" .. sIndex .."_effect"   --combatboy_body_036_effect
    local effectPath = "player/" .. self.m_sAninName .. "/effect/" .. animName
...
function YDPlayerAnimation:loadParticle()
    ...
    if ( sActionName == "walk" or sActionName == "wait" or sActionName == "walk2" or sActionName == "wait0" or sActionName == "run" or sActionName == "win" or sActionName == "win2" or sActionName == "walk3") and config.particle["body_".._sIndex] ~= nil then
        ...
        local particlePath = "armatures/player/particle/combat_".._sIndex.."_lizi"
        local particleExist = CheckEffectFile(particlePath, nil, true)
        if particleExist then 
	        local particle = CCParticleSystemQuad:create(particlePath .. ".plist")
	    	...
	    	local downloadInfo = self:getDownloadInfo(_sIndex, "combat_lizi")
		    if downloadInfo == nil then return end 

			DownloadManager:addDownloadTask(11000 + tonumber(_sIndex),downloadInfo.url,downloadInfo.md5,_sIndex,"downloadCombatLiziCallback",self)
	    end
    end
end

function YDPlayerAnimation:downloadCombatLiziCallback(taskId,extraData,failed)
    WZLog("YDPlayerAnimation:downloadCombatLiziCallback",taskId,extraData,failed)
end
...
function YDPlayerAnimation:_setHead(sIndex,sRanSeIndex_)
	...
	local animName = self.m_sAninName .. "_head_" .. _sIndex
    local pathName = "player/" .. self.m_sAninName .. "/" .. _sIndex .. "/" .. animName
    local existArmature = false
    if self.m_headRanSeIndex > 0 then 
    	local totalHeadName = pathName .. "_hs" .. self:_formatIndex(self.m_headRanSeIndex)
    	existArmature = CheckEffectFile(totalHeadName, true)
    else
...
function YDPlayerAnimation:_setArmatureArmatureHead(sIndex,sRanSeIndex_)
    ...
    local nTempIndex = tonumber(_sIndex)
    for i,v in ipairs(bone) do
        ...
        if bone ~= nil then            
            ...
            if nTempIndex and nTempIndex <= 400 then 
            	bone:setScale(0.8)
            elseif nTempIndex and nTempIndex <= 410 then 
            	bone:setScale(0.9)
            elseif nTempIndex and nTempIndex == 425 then 
            	bone:setScale(1.15)
            end
...
function YDPlayerAnimation:setMount(sIndex)
	...
	self.m_d_mountIndex = tonumber(sIndex)
    if self:isMonster() then return end
	if self.m_running ~= true then return end
	...
	if WZDataFile:getInstance():checkFileExist(spinePath .. ".json") then 
		...
		if bIsExist then
			spinePath = "player_sp/mount/" .. _sIndex .. "/" .. "mount_head_" .. _sIndex 
			bIsExist = CheckEffectFile(spinePath)
			if bIsExist then 
	...
	if self.m_spineMount == true then 
		...
		local existArmature = CheckEffectFile(path, true)
		if existArmature then 
			self:_setArmatureArmatureMount(sIndex)	
			return 
		end

		self:_setArmatureArmatureMount(1)

		local downloadInfo = self:getDownloadInfo(_sIndex, "mount")
	    if downloadInfo == nil then return end 

		DownloadManager:addDownloadTask(9000 + sIndex,downloadInfo.url,downloadInfo.md5,_sIndex,"downloadMountCallback",self)
	end
end

function YDPlayerAnimation:downloadMountCallback(taskId,extraData,failed)
    WZLog("YDPlayerAnimation:downloadMountCallback",taskId,extraData,failed)
    WZLog("YDPlayerAnimation:downloadMountCallback_1",self.m_d_mountIndex)
    if failed == 0 then 
		self:_setArmatureArmatureMount(self.m_d_mountIndex)	
    end
end
...
function YDPlayerAnimation:_setBody(sIndex,bBoy)
    ...
    if self.m_bodyRanSeIndex ~= nil and self.m_bodyRanSeIndex > 0 then 
...
function YDPlayerAnimation:setBodyRanSe(sIndex)
	--qq大厅渠道临时屏蔽染色
    if ProjConfig.CHANNEL_ID == 1118 or ProjConfig.CHANNEL_ID == 1119 or ProjConfig.CHANNEL_ID == 1120 then
	    sIndex = 0
    end
...
function YDPlayerAnimation:_setBodyRanSeArmature(sIndex)
    ...
    if self.m_bodyRanSeIndex > 0 then 
        ...
        if not WZDataFile:getInstance():checkFileExist(pathName .. ".png") then
            return
        end
...
function YDPlayerAnimation:_setFace(sIndex)
	...
	local animName = self.m_sAninName .. "_face_" .. _sIndex
	local pathName = "player/" .. self.m_sAninName .. "/" .. _sIndex .. "/" .. animName
	local existArmature = CheckEffectFile(pathName, true)
    if existArmature then 
		...
		WZLog("YDPlayerAnimation:_setFace not exist",nIndex) 
		self:_setFace(11)
...
function YDPlayerAnimation:setWing(sIndex)
	...
	self.m_d_wingIndex = tonumber(_sIndex)
    if self:isMonster() then return end
	if self.m_running ~= true then 
		return
	end

	local spinePathR = "player_sp/wing/" .. _sIndex .. "/wing_" .. _sIndex
	...
	if self.m_spineWing == true then 
		...
		local pathName = "player/wing/" .. "wing_" .. _sIndex 
		local existArmature = CheckEffectFile(pathName, true)
	    if existArmature then 
	        WZLog("YDPlayerAnimation:setWing exist",sIndex)
	        self:_setWing(sIndex)
	        return
	    end

	    local downloadInfo = self:getDownloadInfo(_sIndex, "wing")
	    if downloadInfo == nil then return end 

		DownloadManager:addDownloadTask(2000 + tonumber(sIndex),downloadInfo.url,downloadInfo.md5,_sIndex,"downloadWingCallback",self)
	end
end

function YDPlayerAnimation:downloadWingCallback(taskId,extraData,failed)
    WZLog("YDPlayerAnimation:downloadWingCallback",taskId,extraData,failed)
    WZLog("YDPlayerAnimation:downloadWingCallback_1",self.m_d_wingIndex)
    if failed == 0 then 
		self:_setWing(self.m_d_wingIndex)
    end
end

function YDPlayerAnimation:_setWing(sIndex)
	if self.m_running ~= true then return end
...
function YDPlayerAnimation:_setArmatureSpineWing(sIndex)
	local _sIndex = self:_formatIndex(sIndex)
	local pathL = "player_sp/wing/" .. _sIndex .. "/wing_" .. _sIndex
	local bIsExist = CheckEffectFile(pathL)
...
function YDPlayerAnimation:_setArmatureArmatureWing(sIndex)
	WZLog("YDPlayerAnimation:_setArmatureArmatureWing", sIndex)
	local _sIndex = self:_formatIndex(sIndex)
	local config = g_YDPlayerAnimation_Config[self.m_sAninName]
	local bones = config.bone
...
function YDPlayerAnimation:setGunBigSkill()
	...
	if not existArmature then 
		local sIndex = "0000"
        local downloadInfo = GetDownloadInfo(sIndex, "wuqi")
        if downloadInfo == nil then return end 

        DownloadManager:addDownloadTask(12000 + tonumber(sIndex),downloadInfo.url,downloadInfo.md5,sIndex,"downloadWuqiCallback",self)
		return 
	end
...
function YDPlayerAnimation:downloadWuqiCallback(taskId,extraData,failed)
    WZLog("YDPlayerAnimation:downloadWuqiCallback",taskId,extraData,failed)
    if failed == 0 then 
    	self:setGunBigSkill()
    end
end
...
function YDPlayerAnimation:setWeaponBigSkill(sIndex)
    ...
    self.m_d_wuqi_index = index
	local boneName = "wuqi2"
	local path = "player/wuqi/wuqi" .. index
	local existArmature = CheckEffectFile(path, true)
	if not existArmature then 
		local _sIndex = string.format("%04d", sIndex)	
        local downloadInfo = GetDownloadInfo(_sIndex, "wuqi")
        if downloadInfo == nil then return end 

        DownloadManager:addDownloadTask(12000 + tonumber(sIndex),downloadInfo.url,downloadInfo.md5,_sIndex,"downloadWuqiIndexCallback",self)
		return 
	end
...
function YDPlayerAnimation:downloadWuqiIndexCallback(taskId,extraData,failed)
    WZLog("YDPlayerAnimation:downloadWuqiIndexCallback",taskId,extraData,failed)
    if failed == 0 then 
    	self:setWeaponBigSkill(self.m_d_wuqi_index)
    end
end
...
function YDPlayerAnimation:getDownloadInfo(sIndex, aninName)
 ...
 --    if not GlobalGame.g_xmlDoc and ConfigExist then
	...
	-- end
 --    local sAnimName = aninName or self.m_sAninName
	-- if ConfigExist then
 ...
 --    	    local sex = element:attributeString("sex")
	-- 	--	WZLog("YDPlayerAnimation:getDownloadInfo_1", index, index == tostring(sIndex), sex == sAnimName)
