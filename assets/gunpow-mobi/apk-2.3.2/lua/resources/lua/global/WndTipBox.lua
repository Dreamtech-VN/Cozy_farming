...
function WndTipBox:_update()
	...
	local nPng = string.find(self.m_tMsgData.sMsgBody, ".png")
    if nPng then 
        local imgTip = createImage(self.m_tMsgData.sMsgBody, nil, nil, true, GlobalMethod:ccp(0.5, 0.5))
        local conTop = GetElement(self.m_root, "conTop_WndTipBox", WZUIContainer)
        conTop:addChild(imgTip)
        conTop:setAbsContentSize(GlobalMethod:CCSize(500, 80))
...
function WndTipBox:_adaptLanguage_ug(  )
    local txtTip = GetElement(self.m_root, "txtTip_WndTipBox", WZUILabelTTF)
    if txtTip then
