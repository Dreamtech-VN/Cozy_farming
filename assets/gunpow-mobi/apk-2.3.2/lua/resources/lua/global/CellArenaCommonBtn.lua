...
function CellArenaCommonBtn:onArenaAddHelpClick()
    ...
    if ScenePvpAmuse.m_root then 
        WndSingleMapDesc:showInterface1(LocalStrings.AMUSE_RULE)
    else
...
function CellArenaCommonBtn:onArenaAddInfoClick()
    ...
    if ScenePvp.m_root then
        ...
        if ProjConfig.LANGUAGE == "es" or ProjConfig.LANGUAGE == "pt" or ProjConfig.LANGUAGE == "en" or ProjConfig.LANGUAGE == "tr" or ProjConfig.LANGUAGE == "vn" then
            WndTips:show(con,ScenePvp.m_root,32,{},GlobalMethod:ccp(-420,20))
        else    
    ...
    if ScenePvpAmuse.m_root then
        local con = GetElement(self.m_root,"conArenaAddInfo_CellArenaCommonBtn",WZUIContainer)
        if ProjConfig.LANGUAGE == "es" or ProjConfig.LANGUAGE == "pt" or ProjConfig.LANGUAGE == "en" or ProjConfig.LANGUAGE == "tr" or ProjConfig.LANGUAGE == "vn" then
            WndTips:show(con,ScenePvpAmuse.m_root,32,{},GlobalMethod:ccp(-420,20))
        else 
...
--@brief    设置显示的按钮内容
function CellArenaCommonBtn:setBtnContent(bAmuseBtn)
    -- body
    if bAmuseBtn then 
        GetElement(self.m_root, "conHall_CellArenaCommonBtn", WZUIContainer):setVisible(false)
        GetElement(self.m_root, "conAmuse_CellArenaCommonBtn", WZUIContainer):setVisible(true)
    else
        GetElement(self.m_root, "conHall_CellArenaCommonBtn", WZUIContainer):setVisible(true)
        GetElement(self.m_root, "conAmuse_CellArenaCommonBtn", WZUIContainer):setVisible(false)
    end
end
-------------------------------------公有方法模块End----------------------------------------
...
function CellArenaCommonBtn:_adaptLanguage_ug( )
    local txtBtn1 = GetElement(self.m_root,"txtBtn1_CellArenaCommonBtn",WZUILabelTTF)
    txtBtn1:setScale(0.65)
    txtBtn1:setDimensions(GlobalMethod:CCSize(200))
    local txtBtn2 = GetElement(self.m_root,"txtBtn2_CellArenaCommonBtn",WZUILabelTTF)
    txtBtn2:setScale(0.65)
    txtBtn2:setDimensions(GlobalMethod:CCSize(200))
    local txtBtn3 = GetElement(self.m_root,"txtBtn3_CellArenaCommonBtn",WZUILabelTTF)
    txtBtn3:setScale(0.65)
    txtBtn3:setDimensions(GlobalMethod:CCSize(200))
    local txtBtn4 = GetElement(self.m_root,"txtBtn4_CellArenaCommonBtn",WZUILabelTTF)
    txtBtn4:setScale(0.65)
    txtBtn4:setDimensions(GlobalMethod:CCSize(200))
end
-------------------------------------语言适配End--------------------------------------------
