...
function WndStore:_init()
	...
	self.m_tLotteryData = nil 		--召唤商店
	self.m_tEnergyData = nil 		--能源商店
	self.m_tFansData = nil 			--越南粉丝商店
	self.m_tVIPData = nil 			--越南VIP商店
	self.m_tFlowerData = nil 		--越南鲜花商店
	self.m_tRankData = nil 			--越南排位商店
    ...
    self.m_rootAssistTable = nil 
    self.m_nAssistrefreshCount  = nil  --刷新次数
    self.m_tAssistData = nil
    self.m_nAssistnextRefreshTime = nil --下次自动刷新时间(秒数)
    self.m_blessRefreshCount = nil 	--祈福商店刷新次数
    self.m_blessRefreshTime = nil
    self.vipLimit29 = nil
    self.vipLimit30 = nil

	self.m_nLotteryRefreshTime = nil
	self.m_nLotteryNextRefreshTime = nil
	self.m_rootLotteryTable = nil
	self.m_LotteryRefreshCount = nil --召唤商店刷新次数
	self.m_nNum = 1
	self.m_limitNum = nil 
	self.m_nEnterDay = nil 		--打开界面日期

	self.m_rootFansTable = nil
	self.m_rootVIPTable = nil
	self.m_rootFlowerTable = nil
	self.m_rootRankTable = nil
	self.m_nfreshCount = {}		--刷新次数 [14]:粉丝商店 [15]:VIP商店 16:鲜花商店 17:排位商店
	self.vipLimitList = {}		--刷新消耗 [14]:粉丝商店 [15]:VIP商店 16:鲜花商店 17:排位商店
end
...
function WndStore:_unInit()
	...
	self.m_limitNum = nil 
	self.m_tEnergyData = nil 		--能源商店
	self.m_nEnterDay = nil 

	self.m_tFansData = nil
	self.m_tVIPData = nil
	self.m_tFlowerData = nil
	self.m_tRankData = nil
	self.m_rootFansTable = nil
	self.m_rootVIPTable = nil
	self.m_rootFlowerTable = nil
	self.m_rootRankTable = nil
	self.m_nfreshCount = nil
	self.vipLimitList = nil
end
...
function WndStore:setRuneStoreData(commodityIds,commodityNums,refreshTimes)
	-- body
	WZLog("WndStore:setRuneStoreData",Serialize(commodityIds),Serialize(commodityNums))
...
function WndStore:setCommunityShopData(tData)
	...
	if self.m_tCommunityShopData == nil then
		...
		-- 			v.itemNum = k.itemNum
		-- 			v.limitNum = k.limitNum
		-- 		end
		-- 	end
		-- end

		for _,v1 in pairs(GDatatab_guild_store) do
			local tempItem = CopyTable(v1)
			tempItem.itemNum = 0
			tempItem.id = tempItem.store
			for _,v2 in ipairs(tData) do
				if tempItem.id == v2.itemId then
					tempItem.itemNum = v2.itemNum
					tempItem.limitNum = v2.limitNum
					break 
				end
			end
			if tempItem.itemNum>0 or tempItem.display==1 then
				table.insert(self.m_tCommunityShopData,tempItem)
			end
		end
...
--祈福商店
function WndStore:setBlessShopData(tData,num,time)
	WZLog("WndStore:setBlessShopData")
	self.m_tBlessShopData = tData
	self.m_blessRefreshCount = num
	self.m_blessRefreshTime = time
...
--@brief 	粉丝商店,vip商店数据
function WndStore:setShopData(shopType, id, buyLimit, goodsStr, costItemStr, buyNum, totalRefreshTimes)
	local tStoreData = {}
	self.m_nfreshCount[self.m_nStoreType]  = totalRefreshTimes
	self:closeLoadingB()
	for i,v in ipairs(id) do
		local _string = string.sub(goodsStr[i],2,-2)
		local itemId = tonumber(SplitStringWithSeparator(_string,",")[1])
		local itemNum = tonumber(SplitStringWithSeparator(_string,",")[2])
		local _string = string.sub(costItemStr[i],2,-2)
		local costId = tonumber(SplitStringWithSeparator(_string,",")[1])
		local costNum = tonumber(SplitStringWithSeparator(_string,",")[2])
		local LeftNum = buyLimit[i] == -1 and buyLimit[i] or (buyLimit[i] - buyNum[i])

		local temp = {}
		table.insert(temp,v)
		table.insert(temp, itemNum)
		table.insert(temp, itemId)
		table.insert(temp, costId)
		table.insert(temp, costNum)
		table.insert(temp, LeftNum)
		table.insert(temp, buyLimit[i])
		table.insert(temp, shopType)

		table.insert(tStoreData,temp)
	end
	WZLog("WndStore:setShopData", self.m_nStoreType, Serialize(tStoreData))
	if self.m_nStoreType == 14 then
		self.m_tFansData = tStoreData
		self.m_rootFansTable:cleanTable()
		self.m_rootFansTable:setVisible(true)
		self:showStoreList()
		self:setButtomInfo()
	elseif self.m_nStoreType == 15 then
		self.m_tVIPData = tStoreData
		self.m_rootVIPTable:cleanTable()
		self.m_rootVIPTable:setVisible(true)
		self:showStoreList()
		self:setButtomInfo()
	elseif self.m_nStoreType == 16 then
		self.m_tFlowerData = tStoreData
		self.m_rootFlowerTable:cleanTable()
		self.m_rootFlowerTable:setVisible(true)
		self:showStoreList()
		self:setButtomInfo()
	elseif self.m_nStoreType == 17 then
		self.m_tRankData = tStoreData
		self.m_rootRankTable:cleanTable()
...
--显示相应的道具商店
--type : 1 竞技商店 2 公会商店 3 宠物商店  4 祈福商店  5卡牌商店 6 黑市商店 7 符文商店 8 装备商店 9 冒险商店 10 排位商店 11 助战商店 12 召唤商店 13 能源商店
--@param 	otherType:如果storeType所指的商店未开启，则打开改字段中的商店
function WndStore:showStoreByType(storeType,closeCallbackLua,closeCallbakcFun, otherType)
	...
	elseif storeType == 11 then
		if not self:checkAssistStoreIsOpen() then
			return
		end
	elseif storeType == 12 then
		if not self:LotteryStoreIsOpen() then
			return
		end
	elseif storeType == 13 then
...
--@brief 	检测协助商店是否开启
function WndStore:checkAssistStoreIsOpen()
	-- body
...
function WndStore:storeItemBuySuccess(stats)
	...
	if stats == 0 then
		MsgBoxManager:showTipBox(LocalStrings.SHOP_BUY_SUCCESS)
	elseif stats == 3 then
		MsgBoxManager:showTipBox(LocalStrings.GUILD_STORES_BUY_LIMIT)
	end
	...
	if self.m_nCurItemTag ~= nil and self.m_nCurItemTag  >= 0 then
		...
		if cellElement then
			if self.m_nStoreType == 1 or self.m_nStoreType == 3 or self.m_nStoreType == 7 or self.m_nStoreType == 8 or self.m_nStoreType == 9 or self.m_nStoreType == 10 or self.m_nStoreType == 11 or self.m_nStoreType == 12 or self.m_nStoreType == 14 or self.m_nStoreType == 15 or self.m_nStoreType == 16 or self.m_nStoreType == 17 then 
...
function WndStore:storeBlessBuySuccess()
	MsgBoxManager:showTipBox(LocalStrings.SHOP_BUY_SUCCESS)
	SoundManager:playEffectSound(SoundDefine.E_S_KILL_GOUMAICHENGGONG)
	if self.m_nCurItemTag ~= nil and self.m_nCurItemTag  >= 0 then
		local curListTable = self:getCurListTable()
...
function WndStore:setCurSelT()
	...
	elseif self.m_nStoreType == 11 then
		txtCurSel:setText(LocalStrings.BATTLE_HELP_TEXT11)
	elseif self.m_nStoreType == 12 then
		txtCurSel:setText(LocalStrings.LOTTERY_STORE)
	elseif self.m_nStoreType == 13 then
		txtCurSel:setText(LocalStrings.BREAK_TEXT1[4])
	end
end
...
--	local stringList = {localStrings.PVP_HALL_3,LocalStrings.EQUIPSTORE,localStrings.COMMUNITY_STORE,localStrings.PET_STORE,localStrings.BLESS_SHOP,localStrings.CARD_SHOP,LocalStrings.RUNE_STORE,LocalStrings.ADVENTURE_STORE,LocalStrings.QUALIFYING_SHOP}
	local stringList = {LocalStrings.QUALIFYING_SHOP, LocalStrings.ADVENTURE_STORE, LocalStrings.RUNE_STORE, LocalStrings.CARD_SHOP, LocalStrings.BLESS_SHOP, LocalStrings.PET_STORE, LocalStrings.COMMUNITY_STORE, LocalStrings.EQUIPSTORE, LocalStrings.PVP_HALL_3, LocalStrings.BATTLE_HELP_TEXT11, LocalStrings.LOTTERY_STORE, LocalStrings.BREAK_TEXT1[4], LocalStrings.STORE_TEXT1[1], LocalStrings.STORE_TEXT1[2], LocalStrings.STORE_TEXT1[3], LocalStrings.STORE_TEXT1[4]}
--	local btnIdList = {5,11,9,88,64,76,115,-2,23}
	local btnIdList = {23,98,116,76,64,88,9,11,5,1,206,218,-2,-3,-4,-5}
	...
	elseif self.m_nStoreType == 11 then
		tempType = btnIdList[startIndex+9]
	elseif self.m_nStoreType == 12 then
		tempType = btnIdList[startIndex+10]
	elseif self.m_nStoreType == 13 then
		tempType = btnIdList[startIndex+11]
	elseif self.m_nStoreType == 14 then
		tempType = btnIdList[startIndex+12]
	elseif self.m_nStoreType == 15 then
		tempType = btnIdList[startIndex+13]
	elseif self.m_nStoreType == 16 then
		tempType = btnIdList[startIndex+14]
	elseif self.m_nStoreType == 17 then
		tempType = btnIdList[startIndex+15]
	end
	...
	for i=1,storeCount do
		...
		con:setAbsContentSize(globalMethod:CCSize(232,64))
		...
		local norImage = WZUI9Image:create()
		norImage:setFile("ui/common/common_btn_04_1.png")

		local selImage = WZUI9Image:create()
		selImage:setFile("ui/common/common_btn_03_1.png")

		local disImage = WZUI9Image:create()
		disImage:setFile("ui/common/common_btn_03_1.png")

		local norLabel = WZUILabelTTF:create()
		norLabel:setLabelStyleKey("C1_F22_S4_C2")
		local selLabel = WZUILabelTTF:create()
		selLabel:setLabelStyleKey("C13_F22")
		local disLabel = WZUILabelTTF:create()
		disLabel:setLabelStyleKey("C13_F22")
		...
		if btnIdList[i] == -2 then
			norLabel:setText("")
			selLabel:setText("")
			disLabel:setText("")
			local imgTitle = WZUIImage:create()
			imgTitle:setFile("ui/shop/icon_shop_fc_vn.png")
			imgTitle:setUseOriginSize(true)
			btn:addChild(imgTitle)
		elseif btnIdList[i] == -3 then
			norLabel:setText("")
			selLabel:setText("")
			disLabel:setText("")
			local imgTitle = WZUIImage:create()
			imgTitle:setFile("ui/shop/icon_shop_vip_vn.png")
			imgTitle:setUseOriginSize(true)
			btn:addChild(imgTitle)
		elseif btnIdList[i] == -4 then
			norLabel:setText("")
			selLabel:setText("")
			disLabel:setText("")
			local imgTitle = WZUIImage:create()
			imgTitle:setFile("ui/shop/icon_shop_bxhht_vn.png")
			imgTitle:setUseOriginSize(true)
			btn:addChild(imgTitle)
		elseif btnIdList[i] == -5 then
			norLabel:setText("")
			selLabel:setText("")
			disLabel:setText("")
			local imgTitle = WZUIImage:create()
			imgTitle:setFile("ui/shop/icon_shop_event_vn.png")
			imgTitle:setUseOriginSize(true)
			btn:addChild(imgTitle)
		end

		if btnIdList[i] == tempType then
			WZLog("相等的值",i,btnIdList[i],tempType)
...
function WndStore:getCurListTable()
	...
	elseif self.m_nStoreType == 11 then
		return self.m_rootAssistTable
	elseif self.m_nStoreType == 12 then
		return self.m_rootLotteryTable
	elseif self.m_nStoreType == 13 then
		return self.m_rootEnergy
	elseif self.m_nStoreType == 14 then
		return self.m_rootFansTable
	elseif self.m_nStoreType == 15 then
		return self.m_rootVIPTable
	elseif self.m_nStoreType == 16 then
		return self.m_rootFlowerTable
	elseif self.m_nStoreType == 17 then
		return self.m_rootRankTable
	end
end
...
--@brief 	竞技商店
function WndStore:setAssistData(tData, refreshCount, nextRefreshTime)
	WZLog("WndStore:setAssistData")
	self.m_tAssistData = tData
	self.m_nAssistrefreshCount  = refreshCount  
    self.m_nAssistnextRefreshTime = nextRefreshTime 
    self.m_rootAssistTable:cleanTable()
    self:closeLoadingB()
    if self.m_nStoreType == 11 then
...
--@brief 	召唤商店
function WndStore:setLotteryData(tData, refreshCount, nextRefreshTime)
	-- body
	WZLog("WndStore:setLotteryData1",Serialize(tData))
	WZLog("WndStore1:setLotteryData1",refreshCount,nextRefreshTime)
	self.m_tLotteryData = tData
	self.m_LotteryRefreshCount = refreshCount
	self.m_nLotteryNextRefreshTime = nextRefreshTime
	self.m_rootLotteryTable:cleanTable()
	self:closeLoadingB()
	if self.m_nStoreType == 12 then
...
--@brief 	能源商店
function WndStore:setEnergyData(goodId, leftNum, priceId, priceNum, discount)
	-- body
	self.m_tEnergyData = {}
	self.m_tEnergyData.goodId = goodId
	self.m_tEnergyData.leftNum = leftNum
	self.m_tEnergyData.priceId = priceId
	self.m_tEnergyData.priceNum = priceNum
	self.m_tEnergyData.discount = discount - 100
	self.m_limitNum = leftNum
	self:closeLoadingB()
	if self.m_nStoreType == 13 then
...
--@brief 	能源商店
function WndStore:buyEnergyOK(result, stockNum)
	-- body
	local buyNum = self.m_tEnergyData.leftNum - stockNum
	self.m_tEnergyData.leftNum = stockNum
	self.m_limitNum = stockNum
	WndRewardShow:showById({self.m_tEnergyData.goodId},{buyNum})
	if self.m_root == nil then return end 
	...
	if self.m_nStoreType == 13 then
		...
		GetElement(self.m_root, "txtLeftNum_WndStore", WZUILabelTTF):setText(self.m_tEnergyData.leftNum)
	end
end
-------------------------------------公有方法模块End----------------------------------------
