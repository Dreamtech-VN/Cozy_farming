...
function CacheCenter:showInAll(mainType, subType)
	local show = false
	if ((mainType == 1 and subType == 42) or (mainType == 2 and subType ~= 49) or mainType == 4 or mainType == 6 or mainType == 7 or mainType == 12 or mainType == 3 or (mainType == 9 and (subType == 0 or subType == 1 or subType == 2 or subType == 3 or subType == 5)) or (mainType == 13 and subType ==2) or mainType == 20 or mainType == 23 or mainType == 24) or (mainType == 25 and subType ==2) or mainType == 35 or mainType == 39 then
		show = true
	end
	return show
end
...
function CacheCenter:getPropList()
	...
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subType = self.m_tPlayerItemList[i].subtype
		if (mainType == 2 and subType ~= 49) or mainType == 12 or mainType == 3 or (mainType == 13 and subType ==2) or mainType == 24 or (mainType == 25 and subType ==2) 
			or (mainType == 1 and subType == 42) or (mainType == 20 and subType == 1) or mainType == 39 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
	return tTempList
...
--@brief	获取装备列表
--@param 	bExceptRecommend 
function CacheCenter:getEquipList(bExceptRecommend)
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
...
--获取紫色装备列表
function CacheCenter:getVioletEquipList(index)
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	index = index or 1
	for i=1,#self.m_tPlayerItemList do
		...
		if index == 1 or index == 0 then
			if self.m_tPlayerItemList[i].subtype == 0 or self.m_tPlayerItemList[i].subtype == 1 then
				is_status = true
			end
		else
			if self.m_tPlayerItemList[i].subtype == index then
				is_status = true
			end
		end
		if mainType == 4 and not isUse and (self.m_tPlayerItemList[i].basicInfo.quality == 3 or self.m_tPlayerItemList[i].basicInfo.quality == 4) and 
			is_status == true and self.m_tPlayerItemList[i].extraInfo.starLevel == 0 and self.m_tPlayerItemList[i].extraInfo.strongLevel == 0 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
	table.sort( tTempList, function(a,b) return a.basicInfo.quality < b.basicInfo.quality end)
	return tTempList
end

--获取宠物装备升星消耗装备列表
function CacheCenter:getPetEquipCostList(index)
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subtype = self.m_tPlayerItemList[i].subtype
		local isUse = self.m_tPlayerItemList[i].isUse
		if mainType == 43 and subtype == index and not isUse and (self.m_tPlayerItemList[i].basicInfo.quality == 3 or self.m_tPlayerItemList[i].basicInfo.quality == 4)
			 and self.m_tPlayerItemList[i].extraInfo.starLevel == 0 and self.m_tPlayerItemList[i].extraInfo.strongLevel == 0 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
...
--@brief	获取人物和宠物宝石列表
function CacheCenter:getPlayerAndPetGemList()
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		if mainType == 6 or mainType == 44 or mainType == 46 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
	return tTempList
...
function CacheCenter:getSkinAndFootList()
	...
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subType = self.m_tPlayerItemList[i].subtype
		if mainType == 20 or (mainType == 9 and subType ==3) or mainType == 23 or mainType == 49 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
	return tTempList
...
function CacheCenter:getBagMaterialList()
	...
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subType = self.m_tPlayerItemList[i].subtype
		if mainType == 7 or (mainType == 9 and (subType == 0 or subType == 1 or subType == 2 or subType == 4 or subType == 5)) or mainType == 35 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
	return tTempList
...
--@brief	获取背包回收其他列表
function CacheCenter:getBagOthersListOfSale()
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subType = self.m_tPlayerItemList[i].subtype
		--if mainType == 37 or mainType == 38 or mainType == 47 then
		if mainType == 38 or mainType == 47 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
	return tTempList
...
--@brief	获取背包回收其他列表
function CacheCenter:getBagListWithType(type1, type2)
	if self.m_tPlayerItemList == nil then return end
	if type1 == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subType = self.m_tPlayerItemList[i].subtype
		if not type2 then
			if mainType == type1 then
				table.insert(tTempList, self.m_tPlayerItemList[i])
			end
		else
			if mainType == type1 and subType == type2 then
				table.insert(tTempList, self.m_tPlayerItemList[i])
			end			
		end
	end
...
function CacheCenter:getMaterialList()
	...
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subType = self.m_tPlayerItemList[i].subtype
		if mainType == 6 or mainType == 7 or (mainType == 9 and (subType == 1 or subType == 2 or subType == 4)) or mainType == 35 or mainType == 46 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
	return tTempList
...
function CacheCenter:getSyntheticMaterialList()
    ...
    for i=1,#self.m_tPlayerItemList do
		...
		if self.m_tPlayerItemList[i].maintype == 9 and (self.m_tPlayerItemList[i].subtype == 0 or self.m_tPlayerItemList[i].subtype == 4 or self.m_tPlayerItemList[i].subtype == 5) then
			local tItemInfo = GetItemLocalData(self.m_tPlayerItemList[i].id)
            if tItemInfo and tItemInfo.mix == 1 then
                table.insert(tMaterialList, CopyTable(self.m_tPlayerItemList[i]))
            end
        elseif self.m_tPlayerItemList[i].maintype == 35 and self.m_tPlayerItemList[i].subtype < 4 then 
        	local tItemInfo = GetItemLocalData(self.m_tPlayerItemList[i].id)
            if tItemInfo and tItemInfo.mix == 1 then
                table.insert(tMaterialList, CopyTable(self.m_tPlayerItemList[i]))
            end
...
--@brief	获取合成宝石列表 (包括人物宝石和宠物宝石)
function CacheCenter:getSyntheticGemList()
	if self.m_tPlayerItemList == nil then return end
    local tMaterialList = {}
    for i=1,#self.m_tPlayerItemList do
		if self.m_tPlayerItemList[i].maintype == 6 or self.m_tPlayerItemList[i].maintype == 44 or self.m_tPlayerItemList[i].maintype == 46 then
			local tItemInfo = GetItemLocalData(self.m_tPlayerItemList[i].id)
            if tItemInfo and tItemInfo.mix == 1 then
                table.insert(tMaterialList, CopyTable(self.m_tPlayerItemList[i]))
            end
...
--	WZLog("CacheCenter:getPlayerItemCountById =",itemId)
	...
	if basicInfo and basicInfo.main_type == 32 then 
		return CacheCenter:getPlayerHomeItemCountById(itemId)
	else
		if self.m_tPlayerItemList == nil then
			return 0
		end
		for i,data in pairs(self.m_tPlayerItemList) do
			if basicInfo ~= nil and (basicInfo.main_type == 5 or basicInfo.main_type == 13) then
				if data.basicInfo ~= nil and tonumber(data.basicInfo.id) == tonumber(itemId) then
					return data.lastTime
				end
			end
			if data.basicInfo ~= nil and tonumber(data.basicInfo.id) == tonumber(itemId) then
				if tonumber(data.basicInfo.stack) ~= 0 then
					return data.lastNum
				end
			end
		end
		...
		for i,data in pairs(self.m_tPlayerItemList) do
			if data.basicInfo ~= nil and tonumber(data.basicInfo.id) == tonumber(itemId) then
				if tonumber(data.basicInfo.stack) == 0 and data.isUse ~= true then
					num = num + data.lastNum
				end
			end
		end
		return num
...
--获取灵石数据
function CacheCenter:getMountStoneList()
	if self.m_tPlayerItemList == nil then return {} end
	local list = {}
	for i=1,#self.m_tPlayerItemList do
		if self.m_tPlayerItemList[i].maintype == 38 and (self.m_tPlayerItemList[i].subtype >= 1 and self.m_tPlayerItemList[i].subtype <= 8) then
			table.insert(list, CopyTable(self.m_tPlayerItemList[i]))
		end
	end
	return list
end
--获取灵石之源数据
function CacheCenter:getMountStoneSourceList()
	if self.m_tPlayerItemList == nil then return {} end
	local list = {}
	for i=1,#self.m_tPlayerItemList do
		if self.m_tPlayerItemList[i].maintype == 38 and (self.m_tPlayerItemList[i].subtype >= 9 and self.m_tPlayerItemList[i].subtype <= 14) then
			table.insert(list, CopyTable(self.m_tPlayerItemList[i]))
		end
	end
	return list
...
--@brief	切换到主城界面
function CacheCenter:jumpToIslandUnion()
...
function CacheCenter:updatePlayerInfo(key, value)
	...
	if key ~= nil and value ~= nil and self.m_tPlayerInfo ~= nil then
		for i=1,#key do
			...
			--职业Id
			if key[i] == "profession" then
				self.m_tPlayerInfo.professionId = tonumber(value[i])
			end
			--判断是否退出联盟，在联盟场景退出联盟，返回主城
			if key[i] == "leagueInfo" then
				if self.m_tPlayerInfo.leagueInfo ~= "" then
					local bClose = false 
					if value[i] == "" then 
						bClose = true 
					else 
						local tempJson = json.decode(value[i])
						if self.m_tPlayerInfo.unionInfo and self.m_tPlayerInfo.unionInfo.id > 0 and tempJson.id == 0 then 
							bClose = true
						end
					end
					if bClose then 
						if WndUnionList.m_root ~= nil then
							DelayCallFunction(self.jumpToIslandUnion,self,1.5)
						end
					end
				end
				if value[i] == "" then 
					self.m_tPlayerInfo.unionInfo = nil 
				else
					self.m_tPlayerInfo.unionInfo = json.decode(value[i])
				end
			end
			-----------------------------------------   更新直接赋值属性   -------------------------------------------------
			if key[i] ~= "allMountsMessage" and key[i] ~= "footMark" and key[i] ~= "allFootMark" then
				--越南判断是否是玩家首充
				if key[i] == "vipExp" then 
					if "vn" == ProjConfig.LANGUAGE and PassportSdkManager and PassportSdkManager.postGameInfoVn then
						if self.m_tPlayerInfo.vipExp == 0 then 
        					PassportSdkManager.m_sPurchaseType = "purchase_new"
						end
					end
        		end
			...
			elseif key[i] == "allFootMark" then  --越南版本，足迹数量太大，将足迹数据从102-1协议分离出来，通过这个协议推送
				self.m_tPlayerInfo.footMark = value
			end
			--更新修炼id和经验
			if key[i] == "xlId" then
			...
			if key[i] == "footMarkCityIds" or key[i] == "footMarkCityTimes" then
				local list = SplitStringWithSeparator(value[i], ",", nil, true)
				self.m_tPlayerInfo[key[i]] = CopyTable(list)
			end
			...
			if key[i] == "honourPoint" then
				self.m_tPlayerInfo.honourPoint = tonumber(value[i])
			end
			if key[i] == "shapeEquip" then
				self.m_tPlayerInfo.phantomEquipment = value[i]
			end
			if key[i] == "pastureId" then
				self.m_tPlayerInfo.pastureId = tonumber(value[i])
			end
			if key[i] == "masterId" then
				self.m_tPlayerInfo.masterId = tonumber(value[i])
			end
			if key[i] == "masterName" then
				self.m_tPlayerInfo.masterName = value[i]
			end
			if key[i] == "pupliInfo" then
				self.m_tPlayerInfo.pupliInfo = value[i]
			end
			if key[i] == "myMoralityLevel" then
				self.m_tPlayerInfo.myMoralityLevel = tonumber(value[i])
			end
			if key[i] == "soulInfo" then
				self.m_tPlayerInfo.soulInfoJson = value[i]
			...
			elseif key[i] == "blueVipInfo" then 
				self.m_tPlayerInfo.blueVipInfo = value[i]
				if self.m_tPlayerInfo.blueVipInfo ~= "" then 
			    	self.m_tPlayerInfo.qqHallData = json.decode(self.m_tPlayerInfo.blueVipInfo)
			    end
			end

			if key[i] == "petEquip" then
				if value[i] ~= "" then
			    	self.m_tPlayerInfo.petEquip = json.decode(value[i])
			    end
			end
			if key[i] == "shape" then
				if value[i] ~= "" then
					self.m_tPlayerInfo.shape = VectorToTable(json.decode(value[i]))
				end
			end
			if key[i] == "footMarkArr" then
				if value[i] ~= "" then
					self.m_tPlayerInfo.footMark = VectorToTable(json.decode(value[i]))
				end
			end
			if key[i] == "zlsJsonInfo" then
				if value[i] ~= "" then
					self.m_tPlayerInfo.zlsJsonInfo = VectorToTable(json.decode(value[i]))
				end
			end
		end
...
function CacheCenter:addPlayerItem(itemId, lastNum, lastTime, isUse, data, playerItemId, disappearTime)
	...
	if self.m_tPlayerFootItemList == nil then 
		self.m_tPlayerFootItemList = {}
	end
	if self.m_tPlayerMountItemList == nil then 
		self.m_tPlayerMountItemList = {}
	end
	
	local receiveTime = SystemTime:getServerTime()

	local bIsHaveDress = false 
	for i=0,itemId:size() - 1 do
		...
		if tTempItem.maintype == 5 and CacheCenter:isEquipedDecorationRedPoint() then
			CacheCenter:setRedState("btnBag",true)
			GlobalGame:getBtnRedPointEvent():dispatcher()
		elseif tTempItem.id == 20018 then 
		...
		WZLog("******** 弹礼包条件 ********", tTempItem.maintype, tTempItem.subtype, level, g_bIsShowWndDressUp)
		if tTempItem.basicInfo and tTempItem.maintype == 3 and (tTempItem.subtype == 0 or tTempItem.subtype == 1 or tTempItem.subtype == 7) and tonumber(level) >= tonumber(tTempItem.basicInfo.use_level) then
			--爱心许愿界面这里先不弹礼包打开提示
			if g_bIsShowWndDressUp == true then
...
--===========批量更新物品============
function CacheCenter:setCacheUpdataItem(playerItemId, splitCount, key, value)
	local data = {}
	local index = 1
	...
	for i=1,#playerItemId do
		...
		for m=1,splitCount[i] do
			table_insert(temp_key, key[index])
			table_insert(temp_value, value[index])
			index = index + 1
		end
		tab.key = temp_key
	...
	local bUpdateDecoration, bUpdateItem, bUpdateItem2 = false, false 
	for i=1,#data do
		local bUpdateDec, bUpdate, bUpdate2 = self:updateEachPlayerItems(data[i].id, data[i].key, data[i].value)
		if bUpdateDec then 
		...
		if bUpdate2 then 
			bUpdateItem2 = bUpdate2 
		end
	end

	self:callUpdateUIFunc(bUpdateDecoration, bUpdateItem, bUpdateItem2)
end
--@brief	更新物品列表缓存信息
--@param    playerItemId:物品唯一id，tKey:属性列表，tValue：属性值列表	
function CacheCenter:updatePlayerItems(playerItemId, tKey, tValue)
	WZLog("更新物品列表缓存信息",playerItemId,Serialize(tKey),Serialize(tValue))
	if playerItemId == nil or tKey == nil or tValue == nil then
		return
	end
	
	local bUpdateDecoration, bUpdateItem, bUpdatePetEquip = CacheCenter:updateEachPlayerItems(playerItemId, tKey, tValue)

	WZLog("CacheCenter:updatePlayerItems zero", bUpdateDecoration, bUpdateItem, bUpdatePetEquip)
	self:callUpdateUIFunc(bUpdateDecoration, bUpdateItem, bUpdatePetEquip)
end

--@brief 	调用监听回调
function CacheCenter:callUpdateUIFunc(bUpdateDecoration, bUpdateItem, bUpdatePetEquip)
	--更新时装装扮
	if bUpdateDecoration then
		self:_updateDecorationData()
	end
	--更新宠物装备
	if bUpdatePetEquip then
		self:updatePetEquipCacheData()
	end
	--更新钻石、金币数据
	self:_setMoneyList()
	if self.m_bStopUpdateNewData == nil or not self.m_bStopUpdateNewData then
...
--@brief	更新每个物品列表缓存信息
--@param    playerItemId:物品唯一id，tKey:属性列表，tValue：属性值列表	
function CacheCenter:updateEachPlayerItems(playerItemId, tKey, tValue)
	WZLog("updateEachPlayerItems",playerItemId,Serialize(tKey),Serialize(tValue))
	local mainType
	local tTempItem = nil 
	local bIsIncrease = false --道具数量是增加的还是减少的
	local level = CacheCenter:getPlayerInfo().level
	local bUpdateDecoration = false 
	local bUpdateItem = false 
	local bUpdatePetEquip = false

	WZLog("CacheCenter:updateEachPlayerItems zero",playerItemId)
	for i=1,#self.m_tPlayerItemList do
		if self.m_tPlayerItemList[i].playerItemId == playerItemId then
			...
			WZLog("CacheCenter:updateEachPlayerItems one", self.m_tPlayerItemList[i].id, self.m_tPlayerItemList[i].lastTime)

			if mainType == 38 then --灵石的时候
				self.m_tPlayerItemList[i].extraInfo = {}
			end
			for j=1,#tKey do
				if self.m_tPlayerItemList[i].basicInfo and self.m_tPlayerItemList[i].basicInfo.main_type == 5 then
				...
				if self.m_tPlayerItemList[i][tKey[j]] ~= nil then
					...
					if tKey[j] == "itemId" then
						...
						if WndAscending.m_root ~= nil and WndAscending.m_nCurTab ~= 2 and WndAscending.m_nCurTab ~= 7 then
							WndRewardShow:showById({tValue[j]},{1})
						end
					else
						if tKey[j] == "randAttr" then
							self.m_tPlayerItemList[i].extraInfo[tKey[j]] = json.decode(tValue[j])
						else
			...
			if tTempItem ~= nil and tTempItem.maintype == 3 and (tTempItem.subtype == 0 or tTempItem.subtype == 1 or tTempItem.subtype == 7) and tonumber(level) >= tonumber(tTempItem.basicInfo.use_level) then
				--爱心许愿界面这里先不弹礼包打开提示
			...
			--货币类型不更新物品
			if mainType ~= 1 or (tTempItem ~= nil and (tTempItem.basicInfo.id == 97 or tTempItem.basicInfo.id == 62 or tTempItem.basicInfo.id == 75 or tTempItem.basicInfo.id == 78 or tTempItem.basicInfo.id == 80 or (tTempItem.basicInfo.main_type == 1 and tTempItem.basicInfo.sub_type == 50))) then
				if self.m_nUpdatePlayerItem == nil then self.m_nUpdatePlayerItem = 0 end
				self.m_nUpdatePlayerItem = self.m_nUpdatePlayerItem + 1
				bUpdateItem = true
			end
			--更新装扮数据
			if mainType == 5 then
				bUpdateDecoration = true 
			end
			--更新宠物装备数据
			if mainType == 43 then
				bUpdatePetEquip = true 
			end
			if tTempItem and tTempItem.basicInfo.id == 20018 then --黑洞弹碎片，刷新技能入口红点
				CacheCenter:setSkillRed() 
				GlobalGame:getBtnRedPointEvent():dispatcher()
			end
			break
	...
	return bUpdateDecoration, bUpdateItem, bUpdatePetEquip
end
...
--@brief	根据物品id获取宠物数量
function CacheCenter:getPetCountByItemId(itemId)
	local count = 0
	for i=1,#self.m_tPlayerPetInfo do
		if self.m_tPlayerPetInfo[i].itemId == itemId and self.m_tPlayerPetInfo[i].isInUsed == false then
			count = count + self.m_tPlayerPetInfo[i].num
		end
	end
	return count
end
...
function CacheCenter:getShopItems(fCallback,tData)
	local bIntegrity = true --是否完整
	if g_nShopCachePage and g_tShopCacheCount then
		for i=1,g_nShopCachePage do
			if g_tShopCacheCount[i] ~= 1 then
				bIntegrity = false
				break
			end
		end
	...
	if bIntegrity and self.m_tShopItems ~= nil and #self.m_tShopItems > 0 then
...
--@brief	重新获取商城商品列表缓存信息
function CacheCenter:refreshShopItems(fCallback,tData)
	table.insert(self.m_tShopItemsCallBack,{fCallback,tData})
	ProtocolProcessorWndShop:send_MALL_GetMallList()
end
			...
			for i,value in pairs(self.m_tAchieList) do
				for j=1,#value.childList do
					if id:get(k) == value.childList[j].id and value.childList[j].status == 0 then 
		                ...
		                --重新排序子成就列表
		                if value.id == 1 then 
		                	table.sort(value.childList, sortSubAchiListTwo)
		                else
...
function CacheCenter:getOriginalMount(mountsId)
	if GDatatab_mounts["id_"..mountsId] then
...
function CacheCenter:mountDescRedPoint()
	...
	if not CacheCenter:getPlayerInfo() then return end
...
--@brief  拥有战马的数据列表
function CacheCenter:setMountsData(mountsId, upgradeLevel, advancedLevel, blessingValue, property, isPlay, state,upgradeBlessingValue, collectStatus)
--	WZLog("CacheCenter:setMountsData",Serialize(VectorToTable(property)))
    ...
    -- 同步服务器的坐骑最新信息
    if WndMounts.seeTick then 
    	WZLog("CacheCenter:setMountsData", WZThread:getUTickCount() - WndMounts.seeTick)
    end
...
--			WZLog("华丽恐惧石", data.id, mountsId:get(nIndex))
			local tMountUpgrade = GDatatab_mounts_upgrade["id_"..upgradeLevel:get(nIndex)]
			local tempUpgradeBless = math.floor((upgradeBlessingValue:get(nIndex) / tMountUpgrade.blessing) * 1000) / 10 --乘1000向下取整后除10是为了保留1位小数

			local tMountAdvanced = GDatatab_mounts_advanced["id_"..(advancedLevel:get(nIndex) + 1)]
			local tempAdvancedBless = 0
			if tMountAdvanced then
				tempAdvancedBless = math.floor((blessingValue:get(nIndex) / tMountAdvanced.blessing) * 1000) / 10 --乘1000向下取整后除10是为了保留1位小数
			end
			...
			data.blessingValue = tempAdvancedBless
			data.upgradeBless = tempUpgradeBless
			...
			data.collectStatus = collectStatus:get(nIndex) or 0
		end
    end
    if WndMounts.seeTick then 
    	WZLog("CacheCenter:setMountsData 000", WZThread:getUTickCount() - WndMounts.seeTick)
    end
    CacheCenter:mountDescRedPoint()
	if WndMounts.m_root then WndMounts:initAllMountsData() end
end
...
function CacheCenter:updateMountsInfoOK(mountsId, upgradeLevel,advancedLevel, blessingValue, property, isPlay,originType,isResult,state,upgradeBlessingValue,collectStatus)
    WZLog("CacheCenter:updateMountsInfoOK", mountsId, isPlay,upgradeBlessingValue,collectStatus)
	for i,data in pairs(self.m_tMounts) do
		if tonumber(data.id) == tonumber(mountsId) then
			local tMountUpgrade = GDatatab_mounts_upgrade["id_"..upgradeLevel]
			local tempUpgradeBless = math.floor((upgradeBlessingValue / tMountUpgrade.blessing) * 1000) / 10 --乘1000向下取整后除10是为了保留1位小数

			local tMountAdvanced = GDatatab_mounts_advanced["id_"..(advancedLevel + 1)]
			local tempAdvancedBless = 0
			if tMountAdvanced then
				tempAdvancedBless = math.floor((blessingValue / tMountAdvanced.blessing) * 1000) / 10 --乘1000向下取整后除10是为了保留1位小数
			end
...
--@brief 	领取师徒任务奖励后更新师德值
function CacheCenter:updateMorality(exp,level)
	self.m_tMasterInfo.moralityLevel = level
	self.m_tMasterInfo.moralityExp = exp
end
...
--@brief 	获取密友数据
function CacheCenter:getBestFriendData()
	-- body
	local bestFriendData = {}
	if self.m_tFriend == nil then 
	...
	for i = 1, #self.m_tFriend do
		if self.m_tFriend[i].bBestFriend == 1 then
			table.insert(bestFriendData,self.m_tFriend[i])
		end
	end
...
--@brief 	获取登陆已经触发的礼包
function CacheCenter:getFiveTypePackageList()
	-- body
	return self.m_tFiveTypePackageList
end

--@brief 	重置登陆已经触发的礼包
function CacheCenter:resetFiveTypePackageList()
	-- body
...
--@brief 	获取足迹特殊属性
function CacheCenter:getFootMapProps()
	return self.m_tStarsSpecialAttr
end
...
--@brief	注册宠物装备更新回调函数
--@param  tObserver：被注册观察者表对象
function CacheCenter:registerUpdatePetEquipObserver(tObserver)
	if self.m_tPetEquitObservers == nil then
		self.m_tPetEquitObservers = {}
	end
	--已经注册,直接返回
	for i=1,#self.m_tPetEquitObservers do
		if self.m_tPetEquitObservers[i] == tObserver then
			do return end
		end
	end
	table.insert(self.m_tPetEquitObservers, tObserver)
end

--@brief	取消注册宠物装备更新回调函数
--@param  tObserver：被注册观察者表对象
function CacheCenter:unregisterUpdatePetEquipObserver(tObserver)
	if self.m_tPetEquitObservers ~= nil then
		for i=1,#self.m_tPetEquitObservers do
			if self.m_tPetEquitObservers[i] == tObserver then
				table.remove(self.m_tPetEquitObservers, i)
				do return end
			end
		end
	end
...
--@brief	更新宠物装备列表缓存数据变化
function CacheCenter:updatePetEquipCacheData()
	WZLog("CacheCenter:updatePetEquipCacheData")
	if self.m_tPetEquitObservers ~= nil then
		for i=1,#self.m_tPetEquitObservers do
			if self.m_tPetEquitObservers[i].updatePetEquipCacheData ~= nil then
				self.m_tPetEquitObservers[i]:updatePetEquipCacheData()
			end
		end
	end
end
...
--宠物装备方案
--@brief 	获取玩家保存的套装数据
function CacheCenter:getPetEquipSchemeData()
	return self.m_tPetEquipSchemeData
end

--@brief 	获取当前玩家使用的宠物装备方案
function CacheCenter:getCurPetEquipScheme()
	local nUsingScheme = 1
	for i=1,#self.m_tPetEquipSchemeData do
		if self.m_tPetEquipSchemeData[i].bIsUsed == true then
			nUsingScheme = self.m_tPetEquipSchemeData[i].id
		end
	end
	return nUsingScheme
end
...
--宠物装备注册
--@brief	注册宠物装备方案更新回调函数
--@param  tObserver：被注册观察者表对象
function CacheCenter:registerPetEuqipSchemeObserver(tObserver)
	if self.m_tPetEquipSchemeObservers == nil then
		self.m_tPetEquipSchemeObservers = {}
	end
	--已经注册,直接返回
	for i=1,#self.m_tPetEquipSchemeObservers do
		if self.m_tPetEquipSchemeObservers[i] == tObserver then
			do return end
		end
	end
	table.insert(self.m_tPetEquipSchemeObservers, tObserver)
end

--@brief	取消注册宠物装备方案更新回调函数
--@param  tObserver：被注册观察者表对象
function CacheCenter:unregisterPetEuqipSchemeObserver(tObserver)
	if self.m_tPetEquipSchemeObservers ~= nil then
		for i=1,#self.m_tPetEquipSchemeObservers do
			if self.m_tPetEquipSchemeObservers[i] == tObserver then
				table.remove(self.m_tPetEquipSchemeObservers, i)
				do return end
			end
		end
	end
...
--@brief 多套宠物装备方案回调
function CacheCenter:updatePetEuqipSchemeData(nType)
	if self.m_tPetEquipSchemeObservers ~= nil then
		for i=1,#self.m_tPetEquipSchemeObservers do
			if self.m_tPetEquipSchemeObservers[i].updatePetEuqipSchemeData ~= nil then
				self.m_tPetEquipSchemeObservers[i]:updatePetEuqipSchemeData(nType)
			end
		end
	end
end
...
--@brief	注册玩家技能方案更新回调函数
--@param  tObserver：被注册观察者表对象
function CacheCenter:registerUpateSkillSuitObserver(tObserver)
	if self.m_tSkillSuitObservers == nil then
		self.m_tSkillSuitObservers = {}
	end
	--已经注册,直接返回
	for i=1,#self.m_tSkillSuitObservers do
		if self.m_tSkillSuitObservers[i] == tObserver then
			do return end
		end
	end
	table.insert(self.m_tSkillSuitObservers, tObserver)
end

--@brief	取消注册玩家技能方案更新回调函数
--@param  tObserver：被注册观察者表对象
function CacheCenter:unregisterUpateSkillSuitObserver(tObserver)
	if self.m_tSkillSuitObservers ~= nil then
		for i=1,#self.m_tSkillSuitObservers do
			if self.m_tSkillSuitObservers[i] == tObserver then
				table.remove(self.m_tSkillSuitObservers, i)
				do return end
			end
		end
	end
...
--@brief 多套技能方案回调
function CacheCenter:updateSkillSuitData()
	-- body
	if self.m_tSkillSuitObservers ~= nil then
		for i=1,#self.m_tSkillSuitObservers do
			if self.m_tSkillSuitObservers[i].updateSkillSuitData ~= nil then
				self.m_tSkillSuitObservers[i]:updateSkillSuitData()
			end
		end
	end
end
				...
				for key, v in pairs(tKey) do
					...
					if self.m_tPlayerItemList[i][key] ~= nil then
						...
						if key == "itemId" then
							...
							if key == "randAttr" then
								self.m_tPlayerItemList[i].extraInfo[key] = json.decode(v)
							else
...
function CacheCenter:updatePlayerHomeItems(playerItemId, itemId, tKey, tValue)
	...
	for i=1,#self.m_tPlayerHomeItemList do
		if self.m_tPlayerHomeItemList[i].maintype == 31 or (self.m_tPlayerHomeItemList[i].maintype == 40 and self.m_tPlayerHomeItemList[i].subtype == 2) then
...
function CacheCenter:getHomeBagData()
	...
	local tTempList = {}
	for i = 1, #self.m_tPlayerHomeItemList do
		if self.m_tPlayerHomeItemList[i].maintype ~= 31 and not (self.m_tPlayerHomeItemList[i].maintype == 40 and self.m_tPlayerHomeItemList[i].subtype == 2) then
...
function CacheCenter:getPlayerPetByItemId(petItemId)
	...
	for i=1,#(self.m_tPlayerPetInfo) do
		local nTempItemId = self.m_tPlayerPetInfo[i].itemId
		local basicInfo = GDatatab_item["id_" .. nTempItemId]
		local nTempItemId2 = nil 
		if basicInfo and basicInfo.quality == 4 then 
			nTempItemId2 = self:getPetItemIdBeforeYellow(nTempItemId)
		end
		if nTempItemId == petItemId or (nTempItemId2 and nTempItemId2 == petItemId) then 
...
-- 获取双人爬塔副本的信息
function CacheCenter:getDoubleTowerCopyData()
    if not self.m_tDoubleTowerCopyData then
        ProtocolProcessorGlobal:send_BOSSMAPROOM_GetTwoTowerInfo()
        return nil
    end
    if self.m_tDoubleTowerCopyData.timeStr then
    	local strTime = self.m_tDoubleTowerCopyData.timeStr
    	local timeStruct = os.date("*t",os.time())
	    local timeStr = string.format("%d%d%02d",timeStruct.year,timeStruct.month,timeStruct.day)
	    if timeStr > strTime then
	    	ProtocolProcessorGlobal:send_BOSSMAPROOM_GetTwoTowerInfo()
    ...
    return self.m_tDoubleTowerCopyData
end

--@brief 	获取职业数据
function CacheCenter:getProfessionData()
	-- body
	return self.m_tProfessionData
end

--@brief 	获取橙色宠物对应的紫宠的物品ID
function CacheCenter:getPetItemIdBeforeYellow(itemId)
	-- body
	for i, value in pairs(GDatatab_pet_advance_evo) do
...
--@brief 	获取染色
function CacheCenter:getColorById(itemId)
    if self.m_tPlayerItemList == nil then return end
	local list = self.m_tPlayerItemList
	local color = 0
...
--@brief 	根据物品Id获取商品数据（只针对时装，如果想用作其他看看要加什么限制，物品id有可能不是唯一）
function CacheCenter:getShopDataByItemId(itemId)
	if self.m_tShopItems == nil then
		return -1
	end
	for k,v in pairs(self.m_tShopItems) do
...
--获取副本里面的重置次数 hyx
-- id: 关卡id
function CacheCenter:getSingleResetCount(id)
	if self.m_tSingleResertData then
		return self.m_tSingleResertData[id]
	end
	return nil
end

--@brief 	获取技能方案
function CacheCenter:getSkillSuit()
	-- body
	return self.m_tSkillSuit
end

--@brief 	获取皮肤图鉴领取状态
function CacheCenter:getSkinStatus()
	-- body
	return self.m_tSkinStatus
end

--@brief 	获取红点状态
function CacheCenter:getRedDotStatus()
	-- body
	return self.m_tRedPointInfo
end

--@brief 	收集信息
function CacheCenter:updateBookStates(rtype, id)
	-- body
	if rtype == 1 then
		for k,v in pairs(self.m_tMounts) do
...
--@brief 	获取小孩辅助技能
function CacheCenter:getKidAssistSkillData()
	-- body
	return self.m_tAssistSkill.kidSkill
end

--@brief 	获取坐骑辅助技能
function CacheCenter:getMountAssistSkillData()
	-- body
	return self.m_tAssistSkill.mountSkill
end

--@brief 	获取辅助技能红点
function CacheCenter:getAssistSkillRed()
	--小孩技能红点
	self.m_bIsAssistSkillRed = false 
	local kidSkill = self.m_tAssistSkill.kidSkill
	if kidSkill and kidSkill.kidId > 0 then 
		local useSkillNum = 0 
		local bHaveNullGrid = false 
		for i, v in pairs(kidSkill.skillId) do
			if v == 0 then 
				bHaveNullGrid = true 
			elseif v > 0 then 
		...
		local unlockNum = 0
		for i = 1, #kidSkill.unlockSkillNum do
			if kidSkill.unlockSkillNum[i] > 0 then 
	...
	--坐骑技能红点
	local mountSkill = self.m_tAssistSkill.mountSkill
	if mountSkill and mountSkill.mountId > 0 then 
		local useSkillNum = 0 
		local bHaveNullGrid = false 
		for i, v in pairs(mountSkill.skillId) do
			if v == 0 then 
				bHaveNullGrid = true 
			elseif v > 0 then 
				useSkillNum = useSkillNum + 1 
		...
		local unlockNum = 0
		for i = 1, #mountSkill.unlockSkillNum do
			if mountSkill.unlockSkillNum[i] > 0 then 
				unlockNum = unlockNum + 1
			end
		end
		if useSkillNum < unlockNum and bHaveNullGrid then 
	...
	return self.m_bIsAssistSkillRed
end

--@brief 	获取使用中的灵石数据
function CacheCenter:getUsingMountStoneList()
	if self.m_tPlayerItemList == nil then return {} end
	local list = {}
	for i=1,#self.m_tPlayerItemList do
		if self.m_tPlayerItemList[i].maintype == 38 and (self.m_tPlayerItemList[i].subtype >= 1 and self.m_tPlayerItemList[i].subtype <= 8) and self.m_tPlayerItemList[i].isUse then
			table.insert(list, CopyTable(self.m_tPlayerItemList[i]))
		end
	end
	return list
...
--@brief 	锻造红点
function CacheCenter:updateStrengthenRed()
    local tData = WndStrengthen:getRecommendEquip2()
    --镶嵌
    local bIsRed = false
    for i = 1, #tData do
		if CheckButtonOpen(43,1) and tData[i].basicInfo and tData[i].basicInfo.time_limit == -1 and tData[i].isUse and tData[i].extraInfo 
				and (tData[i].extraInfo.hpStone == nil or tData[i].extraInfo.attackStone == nil or tData[i].extraInfo.defendStone == nil or (tData[i].extraInfo.gongmingStone == nil and CheckButtonOpen(195,true))
				or tData[i].extraInfo.hpStone == 0 or tData[i].extraInfo.attackStone == 0 or tData[i].extraInfo.defendStone == 0 or (tData[i].extraInfo.gongmingStone == 0 and CheckButtonOpen(195,true)) ) then
			bIsRed = true
			break
		end
	end
	if bIsRed == true then
		CacheCenter:setRedState("gemMounting_btnStrong",true)
	else
		CacheCenter:setRedState("gemMounting_btnStrong",false)
	end

    CacheCenter:updateRedPoint("right", SceneCity.m_tWndBottomBar, nil)
end

--@brief 	获取玩家使用的信息框特效物品Id
function CacheCenter:getPlayerInfoRectEffectItemId()
	-- body
	if self.m_tPlayerItemList == nil then return 0 end

	for i=1, #self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subtype = self.m_tPlayerItemList[i].subtype
		local isUse = self.m_tPlayerItemList[i].isUse
		if mainType == 2 and isUse and subtype == 49 then
			return self.m_tPlayerItemList[i].id
		end
	end
...
--@brief 	获取玩家使用的头像框特效物品Id
function CacheCenter:getPlayerHeadEffectItemId()
	-- body
	if self.m_tPlayerItemList == nil then return 0 end
	...
	for i=1, #self.m_tPlayerItemList do
		...
		local subtype = self.m_tPlayerItemList[i].subtype
		local isUse = self.m_tPlayerItemList[i].isUse
		if mainType == 40 and isUse and subtype == 1 then
			return self.m_tPlayerItemList[i].id
		end
	end
...
--@brief 	获取玩家小孩使用的头像框特效物品Id
function CacheCenter:getKidUsingHeadEffectItemId()
	-- body
	if self.m_tPlayerItemList == nil then return 0 end
	...
	for i=1, #self.m_tPlayerItemList do
		...
		local subtype = self.m_tPlayerItemList[i].subtype
		local isUse = self.m_tPlayerItemList[i].isUse
		if mainType == 40 and isUse and subtype == 2 then
			return self.m_tPlayerItemList[i].id
		end
	end
...
--@brief 	获取玩家拥有的头像框特效物品Id
function CacheCenter:getPlayerOwnHeadEffect()
	-- body
	local tempList = {}
	if self.m_tPlayerItemList == nil then return tempList end

	for i=1, #self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subtype = self.m_tPlayerItemList[i].subtype
		if mainType == 40 and subtype == 1 then
			table.insert(tempList, self.m_tPlayerItemList[i])
		end
	end
...
--@brief 	获取玩家拥有的信息框特效物品Id
function CacheCenter:getPlayerOwnInfoRectEffect()
	-- body
	local tTempList = {}
	if self.m_tPlayerItemList == nil then return tTempList end

	for i=1, #self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subtype = self.m_tPlayerItemList[i].subtype
		if mainType == 2 and subtype == 49 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
...
--@brief 	获取玩家拥有的小孩头像框特效物品Id
function CacheCenter:getPlayerOwnKidHeadEffect(kidId)
	-- body
	local tempList = {}
	if self.m_tPlayerHomeItemList == nil then return tempList end

	for i=1, #self.m_tPlayerHomeItemList do
		local mainType = self.m_tPlayerHomeItemList[i].maintype
		local subtype = self.m_tPlayerHomeItemList[i].subtype
		local isUse = self.m_tPlayerHomeItemList[i].isUse
		local childId = self.m_tPlayerHomeItemList[i].childId
		if mainType == 40 and subtype == 2 then
			local bAdd = true 
			if kidId then 
				bAdd = false 
				if childId == 0 or (isUse and kidId == childId) then 
					bAdd = true
				end
			end
			if bAdd then 
				table.insert(tempList, self.m_tPlayerHomeItemList[i])
			end
		end
	end
...
--@brief 	获取玩家的突破等级Id
function  CacheCenter:getPlayerBreakLvId()
	-- body
	return self.m_tPlayerInfo.levelBreachId
end

--@brief 	设置玩家的突破等级Id
function  CacheCenter:setPlayerBreakLvId(levelBreachId)
	-- body
	self.m_tPlayerInfo.levelBreachId = levelBreachId
end

--判断玩家ID是否是玩家的徒弟
function CacheCenter:judgeIsPupilById(nId)
	local bIsPupil = false 
	if self.m_tPlayerInfo.pupliInfo == nil or self.m_tPlayerInfo.pupliInfo == "" or self.m_tPlayerInfo.pupliInfo == "{}" then return bIsPupil end
	local tPupilList = json.decode(self.m_tPlayerInfo.pupliInfo)
	for i = 1, #tPupilList do
		if tonumber(tPupilList[i].playerId) == nId then 
			return true
		end 
	end
...
--@brief	获取攻击特效爆炸效果列表
function CacheCenter:getExplosionItemList()
	if self.m_tPlayerItemList == nil then return end
	local tExplosionItemList = {}
	for i=1,#self.m_tPlayerItemList do
		if self.m_tPlayerItemList[i].maintype == 42 then
			table.insert(tExplosionItemList, self.m_tPlayerItemList[i])
		end
	end
	return tExplosionItemList
end
--@brief 	获取活动投放勋章数据
function CacheCenter:getActivityMedalData()
	if self.m_tPlayerItemList == nil then return end
	local tMedalData = {}
	for i=1,#self.m_tPlayerItemList do
		if self.m_tPlayerItemList[i].maintype == 1 and self.m_tPlayerItemList[i].subtype == 59 then
			tMedalData[tostring(self.m_tPlayerItemList[i].id)] = self.m_tPlayerItemList[i].lastNum
		end
	end
	return tMedalData
end
...
--@brief 	获取玩家vip称号
function CacheCenter:getPlayerVipTitle()
	for i=1,#self.m_tAchieList do           
		if self.m_tAchieList[i].id == 1 then 
			return self.m_tAchieList[i].childList
		end
	end
...
--@brief	获取人物装备和宠物装备列表
--@param 	bExceptRecommend 
function CacheCenter:getPlayerAndPetEquipList(bExceptRecommend)
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local isUse = self.m_tPlayerItemList[i].isUse
		if mainType == 4 or mainType == 43 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
	if not bExceptRecommend then 
...
--@brief	获取宠物装备数据列表
function CacheCenter:getPetsEquipmentList()
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
...
--@brief	获取使用中的宠物装备数据列表
function CacheCenter:getUsingPetsEquipList()
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subType = self.m_tPlayerItemList[i].subtype
		if mainType == 43 and self.m_tPlayerItemList[i].isUse == true then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
...
--@brief	获取宠物装备宝石列表
function CacheCenter:getPetEquipGemsList()
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
...
--@brief	获取物品列表缓存信息
function CacheCenter:getPlayerItemByPlayerItemId(playerItemId)
	if self.m_tPlayerItemList ~= nil then
		for i=1,#self.m_tPlayerItemList do
			WZLog("CacheCenter:getPlayerItemByPlayerItemId", i, self.m_tPlayerItemList[i].playerItemId)
			if self.m_tPlayerItemList[i].playerItemId == playerItemId then
				return self.m_tPlayerItemList[i]
			end
		end
...
--@brief	获取足迹星辰宝石列表
function CacheCenter:getFootStarMapGemList()
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
...
--@brief	获取度假村精灵列表
function CacheCenter:getHVSpiritList()
	if self.m_tPlayerItemList == nil then return end
	local tTempList = {}
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subType = self.m_tPlayerItemList[i].subtype
		if mainType == 49 and subType == 1 then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
	return tTempList
...
--@brief 	获取度假村花盆数据
--@param 	fieldType:土坑类型1：土坑；2：水坑；3：小屋；4：水车
function CacheCenter:getHVFlowerpots(fieldType)
	local tTempList = {}
	if self.m_tPlayerItemList == nil then return tTempList end
	...
	for i=1,#self.m_tPlayerItemList do
		local mainType = self.m_tPlayerItemList[i].maintype
		local subType = self.m_tPlayerItemList[i].subtype
		if mainType == 50 and subType == fieldType then
			table.insert(tTempList, self.m_tPlayerItemList[i])
		end
	end
...
--@brief 	获取物品表中所有的花盆
function CacheCenter:getAllFlowerpot()
	if self.m_tAllFlowerpot == nil then 
		self.m_tAllFlowerpot = {}
		for i, value in pairs(GDatatab_item) do
			if value.main_type == 50 then 
				table.insert(self.m_tAllFlowerpot, value)
			end
		end
	end

	return self.m_tAllFlowerpot
end

--@brief	通过物品Id获取玩家物品个数
--@return   玩家物品个数
function CacheCenter:caculatePlayerItemCountById(itemId)
	local lastNum = 0 
	if itemId == nil then
		return lastNum
	end
	local key = "id_" .. itemId 
	local basicInfo = GDatatab_item[key]
	if basicInfo.main_type == 32 then 
		return CacheCenter:getPlayerHomeItemCountById(itemId)
	else
		if self.m_tPlayerItemList == nil then
			return lastNum
...
--@brief	通过物品Id获取玩家足迹物品个数
--@return   玩家可激活足迹物品个数
function CacheCenter:getPlayerFootItemCountById(itemId)
--	WZLog("CacheCenter:getPlayerFootItemCountById =",itemId)
	if itemId == nil then
		return 0
	end
	local key = "id_" .. itemId 
	...
	for i,data in pairs(self.m_tPlayerFootItemList) do
		if data.basicInfo ~= nil and tonumber(data.basicInfo.id) == tonumber(itemId) then
			return data.lastNum
		end
	end
...
--@brief	通过物品Id获取玩家坐骑卡物品个数
--@return   玩家可激活坐骑物品个数
function CacheCenter:getPlayerMountItemCountById(itemId)
--	WZLog("CacheCenter:getPlayerMountItemCountById =",itemId)
	if itemId == nil then
		return 0
	end
	local key = "id_" .. itemId 
	...
	for i,data in pairs(self.m_tPlayerMountItemList) do
		if data.basicInfo ~= nil and tonumber(data.basicInfo.id) == tonumber(itemId) then
			return data.lastNum
		end
	end
...
--@brief	获取联盟信息
function CacheCenter:getUnionInfo()
	return self.m_tUnionInfo 
end

--@brief 	获取已进阶的时装套装Id数据
function CacheCenter:getDressAdvanceId()
	return self.m_tHavedAdvanceDressIds or {}
end

--@brief 	获取配置的可进阶的时装套装Id数据
function CacheCenter:getAllConfigDressAdvanceIds()
	--每次登录，只执行一次
	if self.m_tConfigAdvanceDressIds == nil then 
		self.m_tConfigAdvanceDressIds = {}
		for i, value in pairs(GDatatab_enchanting) do
			if value.property3 ~= -1 and value.cost ~= -1 then 
				table.insert(self.m_tConfigAdvanceDressIds, value.id)
			end
		end
	end

	return self.m_tConfigAdvanceDressIds 
end

--@brief 	获取已进阶的翅膀Id数据
function CacheCenter:getWingAdvanceId()
	return self.m_tHavedAdvanceWingIds or {}
end

--@brief 	获取配置的可进阶的翅膀Id数据
function CacheCenter:getAllConfigWingAdvanceIds()
	--每次登录，只执行一次
	if self.m_tConfigAdvanceWingIds == nil then 
		self.m_tConfigAdvanceWingIds = {}
		for i, value in pairs(GDatatab_enchanting) do
			if value.wing_property ~= -1 and value.wing_cost ~= -1 then 
				table.insert(self.m_tConfigAdvanceWingIds, value.id)
			end
		end
	end

	return self.m_tConfigAdvanceWingIds 
end