...
function WndGemLibrary:_updateDetail(nTag)
    ...
    if ftxtDesc1 then
        local sFormat = [[<T C="233,166,62" S="20" P="1">%s: </T><T C="233,166,62" S="20" P="1">%s</T>]]
    ...
    if ftxtDesc2 then
        local sFormat = [[<T C="233,166,62" S="20" P="1">%s: </T><I Z="0.5" P="1">%s</I><T C="233,166,62" S="20" P="1">%d</T>]]
    ...
    if ftxtDesc3 then
        ...
        if tData.basicInfo.id >= 2155 and tData.basicInfo.id <= 2160 then 
            sFormat = [[<T C="127,70,26" S="20" P="1">%s: </T><T C="127,70,26" S="20" P="1">%s</T>]]
            ftxtDesc3:setShowText(string.format(sFormat, LocalStrings.GET_ACCESS, LocalStrings.DIGGEM_TEXT43))
        else
            if tData.sLevel == tData.eLevel then
                sFormat = [[<T C="127,70,26" S="20" P="1">%s: </T><T C="127,70,26" S="20" P="1">Lv%d%s</T>]]
    ...
    if tData.sLevel == 999 then
        ftxtDesc3:setVisible(false)
    else
        ftxtDesc3:setVisible(true)
    end
end
...
function WndGemLibrary:_adaptLanguage_vn(  )
    GetElement(self.m_root, "ftxtDesc2_WndGemLibrary", WZUIFreeTextBox):setRelativePosition(GlobalMethod:ccp(0.05,0.33))
    GetElement(self.m_root, "ftxtDesc3_WndGemLibrary", WZUIFreeTextBox):setRelativePosition(GlobalMethod:ccp(0.05,0.25))
end
...
function WndGemLibrary:_adaptLanguage_ug(  )
    local ftxtDesc1 = GetElement(self.m_root, "ftxtDesc1_WndGemLibrary", WZUIFreeTextBox)
    ftxtDesc1:setScale(0.8)
    ftxtDesc1:setMaxWidth(280)
    ftxtDesc1:setRelativePosition(GlobalMethod:ccp(0.05,0.55))
