...
function WndDigGem:_init()
    ...
    self.m_tRemainsList = nil           --遗迹之光事件列表
    self.m_tRemainsEleList = nil        --遗迹之光事件列表对象
    self.m_nMapSize = 0                 --遗迹之光事件容量
    self.m_nShareTime = nil             --遗迹分享cd
    self.m_nMapSize = nil
    self.m_nComplete = nil

    self.m_tRoleDataList = {}           --人物数据列表
    self.m_nHireNum = 0                 --受雇人数
    self.m_tRoleList = {}               --受雇人数据列表
    self.playing = false                --是否在播放表情动画
    self.operateId = nil                --选中人物的id
    self.operateIndex = nil             --选中人物的下标
    self.m_tHireFriendList = {}         --雇佣好友数据列表
    self.playFaceRole = nil             --需要播放表情的玩家
...
function WndDigGem:_unInit()
    ...
    self.m_tRemainsList = nil           --遗迹之光事件列表
    self.m_tRemainsEleList = nil        --遗迹之光事件列表对象
    self.m_nMapSize = nil               --遗迹之光事件容量
    self.m_nShareTime = nil             --遗迹分享cd
    self.m_nMapSize = nil
    self.m_nComplete = nil

    self.m_tRoleDataList = nil
    self.m_nHireNum = nil
    self.m_tRoleList = nil
    self.playing = nil
    self.operateId = nil
    self.operateIndex = nil
    self.m_tHireFriendList = nil
    self.playFaceRole = nil
end
...
function WndDigGem:createElement()
    if WndDigGem.m_root ~= nil then
        WindowManager:removeWindow(WndDigGem.m_root, WndDigGem, true)
    end
...
--@brief    设置数据
function WndDigGem:setData(useTool, remainTime, level, exp, toolId, remainToolTime, item, num, toolTime, buyGemCoinTimes, mapId, mapNum, mapStatus, overTime, reward, taskId, status, target, complete, mapSize)
    -- body
...
--@brief    雇佣人形象
function WndDigGem:setRoleData(hireNum, playerId, playerName, playerLevel, playerSex, playerHeadId, playerFaceId, playerBodyId, playerWingId, playerHeadcolour, playerBodycolour, playerPet, hireMoodValue, hireEndTime)
    self.m_nHireNum = hireNum
    self.m_tRoleDataList = {}
    for i=1,#playerId do
        local tempTable = {}
        tempTable.playerId = playerId[i]
        tempTable.playerName = playerName[i]
        tempTable.playerLevel = playerLevel[i]
        tempTable.playerSex = playerSex[i]
        tempTable.playerHeadId = playerHeadId[i]
        tempTable.playerFaceId = playerFaceId[i]
        tempTable.playerBodyId = playerBodyId[i]
        tempTable.playerWingId = playerWingId[i]
        tempTable.playerHeadcolour = playerHeadcolour[i]
        tempTable.playerBodycolour = playerBodycolour[i]
        tempTable.playerPet = playerPet[i]
        tempTable.hireMoodValue = hireMoodValue[i]
        tempTable.hireEndTime = hireEndTime[i]
        table.insert(self.m_tRoleDataList,tempTable)
    end
...
--@brief    获得格式化时间 "%s小时%s分钟"或"%s分钟"
--@param    剩余时间(秒)
function WndDigGem:getStrFormatTime(nTime)
    local totalMin = math.ceil(nTime / 60) --向上取整分钟
    local m = totalMin % 60
    local h = math.floor(totalMin / 60)

    local strTime = ""
    if h == 0 then
        strTime = m..LocalStrings.MINUTE1
    else
        strTime = h..LocalStrings.HOUR1..m..LocalStrings.MINUTE1
    end
    return strTime
end

--@brief    设置日志数据
function WndDigGem:setLogData(logtype, time, name, itemCount, itemId, itemNum, miningExp, miningLevel)
    -- body
    self.m_tLogList = {}
    self:_stopLoading()
    ...
    for i = 1, #logtype do
        ...
        tItem.itemData = {}
        for j=1,itemCount[i] do
            local tempItem = {}
            tempItem.itemId = itemId[itemIndex]
            tempItem.itemNum = itemNum[itemIndex]
            table.insert(tItem.itemData,tempItem)
            itemIndex = itemIndex + 1
        end

        if logtype[i] == 1 then --挖矿开始,将持续%s(时间)
            tItem.name = self:getStrFormatTime(miningExp[i])
        elseif logtype[i] == 2 then --收获%s(物品*x,物品*x,...)
            -- local gemInfo = GDatatab_item["id_" .. tItem.itemData[1].itemId]
            -- tItem.name = gemInfo.name
            tItem.name = ""
            for k=1,#tItem.itemData do
                local tItemInfo = GDatatab_item["id_" .. tItem.itemData[k].itemId]
                tItem.name = tItem.name.."["..tItemInfo.name.."]*"..tItem.itemData[k].itemNum
                if k ~= #tItem.itemData then
                    tItem.name = tItem.name .. ","
                end
            end
        elseif logtype[i] == 7 then --挖到%s(遗迹,遗迹,...)
            -- local gemInfo = GDatatab_dig_map["id_" .. tItem.itemData[1].itemId]
            -- tItem.name = gemInfo.map_name
            tItem.name = ""
            for k=1,#tItem.itemData do
                local tItemInfo = GDatatab_dig_map["id_" .. tItem.itemData[k].itemId]
                tItem.name = tItem.name..""..tItemInfo.map_name..""
                if k ~= #tItem.itemData then
                    tItem.name = tItem.name .. ","
                end
            end
        elseif logtype[i] == 8 or logtype[i] == 9 then --8:我雇佣%s(玩家),持续%s(时间) 9:%s(玩家)雇佣我,持续%s(时间)
            tItem.name = name[i]
            tItem.param = self:getStrFormatTime(miningExp[i])
        elseif logtype[i] == 10 or logtype[i] == 11 or logtype[i] == 12 then --%s(玩家)给我飞吻,面包,皮鞭
            tItem.name = name[i]
        elseif logtype[i] == 13 then --挖矿等级上升到%s
            tItem.name = miningLevel[i]
        elseif logtype[i] == 14 or logtype[i] == 15 then --14:我从%s(玩家),偷到%s 15:%s(玩家)从我,偷到%s
            tItem.name = name[i]
            tItem.param = ""
            for k=1,#tItem.itemData do
                local tItemInfo = GDatatab_item["id_" .. tItem.itemData[k].itemId]
                tItem.param = tItem.param.."["..tItemInfo.name.."]*"..tItem.itemData[k].itemNum
                if k ~= #tItem.itemData then
                    tItem.param = tItem.param .. ","
                end
            end
        elseif logtype[i] == 16 then --雇员%s挖到%s
            tItem.name = name[i]
            tItem.param = ""
            for k=1,#tItem.itemData do
                local tItemInfo = GDatatab_item["id_" .. tItem.itemData[k].itemId]
...
--@brief    设置遗迹之光数据
function WndDigGem:setRemainsData(mapId, mapNum, mapStatus, overTime, complete, mapSize, shareTime)
    self:_stopLoading()

    self.m_nMapSize = mapSize
    self.m_nComplete =  complete
    self.m_nShareTime = shareTime

    self.m_tRemainsList = {}
    for i = 1, #mapId do
        local tRemains = {}
        tRemains.mapId = mapId[i]
        tRemains.mapNum = mapNum[i]
        tRemains.mapStatus = mapStatus[i]
        tRemains.overTime = overTime[i]
        table.insert(self.m_tRemainsList, tRemains)
    end
...
--@brief    设置雇佣好友数据
function WndDigGem:getHireFriendListOk(playerId, isOnline, playerName, sex, vipLevel, faceItemId, headItemId, headColor, minerLevel, hireId, hireEndTime, logtype, logTime)
    self.m_tHireFriendList = {}
    local tMyHire = {} --被我雇佣的好友
    local tNoHire = {} --没被雇佣的好友
    local tOtherHire = {} --被其他人雇佣的好友

    for i=1,#playerId do
        local tPlayDatas = {}
        tPlayDatas.playerId = playerId[i]
        tPlayDatas.isOnline = isOnline[i]
        tPlayDatas.playerName = playerName[i]
        tPlayDatas.sex = sex[i]
        tPlayDatas.vipLevel = vipLevel[i]
        tPlayDatas.faceItemId = faceItemId[i]
        tPlayDatas.headItemId = headItemId[i]
        tPlayDatas.headColor = headColor[i]
        tPlayDatas.minerLevel = minerLevel[i]
        tPlayDatas.hireId = hireId[i]
        tPlayDatas.hireEndTime = hireEndTime[i]
        tPlayDatas.logtype = logtype[i]
        tPlayDatas.logTime = logTime[i]

        if tPlayDatas.hireId == 0 or tPlayDatas.hireEndTime < SystemTime:getServerTime() then --没被雇佣
            table.insert(tNoHire, tPlayDatas)
        else
            if tPlayDatas.hireId == CacheCenter:getPlayerInfo().id then
                table.insert(tMyHire, tPlayDatas)
            else
                table.insert(tOtherHire, tPlayDatas)
            end
        end
    ...
    local funcSort = function(a,b)
        if a.minerLevel ~= b.minerLevel then
            return a.minerLevel > b.minerLevel
        else
    ...
    table.sort( tMyHire, funcSort )
    table.sort( tNoHire, funcSort )
    table.sort( tOtherHire, funcSort )

    self.m_tHireFriendList = CopyTable(tNoHire)
    for i=#tMyHire,1,-1 do
        table.insert(self.m_tHireFriendList, 1, tMyHire[i])
    end
    for i=1,#tOtherHire do
        table.insert(self.m_tHireFriendList, tOtherHire[i])
    end
end
...
function WndDigGem:getShareMapOk(result)
    if result == 0 then
...
--@brief    互动结果
function WndDigGem:getHireInteractOk(result, playerId, interactType, moodChange)
    if result == 0 then
        for i=1,self.m_nHireNum do
            if self.m_tRoleList[i].playerId == playerId then

                self.moodChange = moodChange
                self.playFaceRole = self.m_tRoleList[i]

                local uiNode = self.playFaceRole.uiNode
                if moodChange > 0 then
                    self:playFaceAnimation(21, uiNode)
                elseif moodChange < 0 then
                    self:playFaceAnimation(17, uiNode)
                else
                    self.playing = false
                end
            else
...
function WndDigGem:getChasingThiefOk()
    MsgBoxManager:showTipBox(LocalStrings.DIGGEM_TEXT65)
end
