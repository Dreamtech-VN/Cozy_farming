...
function WndTransaction:onEnterTransitionDidFinish(element)
	...
	--取商品列表
	WndTransaction:sendGetCommodityList(ids)
...
function WndTransaction:sendGetCommodityList(element, data)
    local ids = vIds or WZLuaVector_int_:create()
	local nSubType = self.m_nCurType
	local nQuality = self.m_nCurQuality
	if self.m_nCurType == 5 and self.m_nCurQuality == 1 then
		nSubType = 4
		nQuality = 3
	end
	ProtocolProcessorTransaction:send_TRANSACTION_GetCommodityList(nSubType, nQuality, ids)
end
...
function WndTransaction:show()
	...
	WindowManager:addWindow(wnd, WndTransaction, nil, nil, true, true)
end
...
function WndTransaction:onTab(element)
	...
	local tips = GetElement(self.m_root,"tips_WndTransaction",WZUIButton)
...
function WndTransaction:update()
	...
	local leftTitles = {LocalStrings.TRANSACTION12,LocalStrings.DIGGEM_TEXT1,LocalStrings.DIGGEM_TEXT1,LocalStrings.DIGGEM_TEXT1}
	local rightTitles = {LocalStrings.TRANSACTION13,LocalStrings.TRANSACTION13,LocalStrings.BAGTIP3,LocalStrings.TRANSACTION13}
...
function WndTransaction:updateBag()
	...
	freeCon:setAbsContentSize(GlobalMethod:CCSize(236,395))
	if self.m_nRightTag == 3 then
		freeCon:setAbsContentSize(GlobalMethod:CCSize(236,320))
	end
...
function WndTransaction:updateLeft1()
    ...
    local freeCon = GetElement(self.m_root, "freeConLeft_WndTransaction", WZUIFreeListContainer)
	freeCon:setAbsContentSize(GlobalMethod:CCSize(220,395))
	freeCon:updateRelativeSize()
    freeCon:removeAll()
	removeShowPanelNullTip(freeCon)
...
function WndTransaction:updateRight2()
	...
	local txtRight2SoldNum = GetElement(self.m_root,"txtRight2SoldNum_WndTransaction",WZUILabelTTF)
	local miningShopLimit = CacheCenter:getGameParam().miningShopLimit
	if self.m_nTodaySaleCount and miningShopLimit then
		txtRight2SoldNum:setText(LocalStrings.PRODUCTS_SOLD_TODAY..":"..self.m_nTodaySaleCount.."/"..miningShopLimit)
	end
    self.m_nDayOfMonth = os.date("%d")
    txtRight2SoldNum:enableSchedule("_caculateTime", 1)
end

--@brief    计时器跨天刷新出售数据
function WndTransaction:_caculateTime()
    if self.m_nDayOfMonth ~= os.date("%d") then
	    self.m_nDayOfMonth = os.date("%d")
	    ProtocolProcessorTransaction:send_TRANSACTION_GetSaleList()
    end
end
...
function WndTransaction:_adaptLanguage_ug(  )
	GetElement(self.m_root,"txtTab1_WndTransaction",WZUILabelTTF):setScale(0.6)
	GetElement(self.m_root,"txtTab1Sel_WndTransaction",WZUILabelTTF):setScale(0.6)
	GetElement(self.m_root,"txtTab3_WndTransaction",WZUILabelTTF):setScale(0.5)
	GetElement(self.m_root,"txtTab3Sel_WndTransaction",WZUILabelTTF):setScale(0.5)

	local txtQuickSelect1 = GetElement(self.m_root,"txtQuickSelect1_WndTransaction",WZUILabelTTF)
	txtQuickSelect1:setScale(0.7)
	txtQuickSelect1:setDimensions(GlobalMethod:CCSize(170))
	local txtQuickSelect2 = GetElement(self.m_root,"txtQuickSelect2_WndTransaction",WZUILabelTTF)
	txtQuickSelect2:setScale(0.7)
	txtQuickSelect2:setDimensions(GlobalMethod:CCSize(170))
	local txtRefresh = GetElement(self.m_root,"txtRefresh",WZUILabelTTF)
	txtRefresh:setScale(0.7)
	txtRefresh:setDimensions(GlobalMethod:CCSize(170))

	GetElement(self.m_root,"leftSubTitle",WZUILabelTTF):setScale(0.8)
	
	local txtRecycle = GetElement(self.m_root,"txtRecycle",WZUIFreeTextBox)
	txtRecycle:setScale(0.8)
	txtRecycle:setMaxWidth(400)

	local txtRight3 = GetElement(self.m_root,"txtRight3_WndTransaction",WZUILabelTTF)
	txtRight3:setScale(0.6)

	GetElement(self.m_root,"txtCon4_WndTransaction",WZUILabelTTF):setDimensions(GlobalMethod:CCSize(360))

	local txtC4T1 = GetElement(self.m_root,"txtC4T1_WndTransaction",WZUILabelTTF)
	txtC4T1:setScale(0.7)
	txtC4T1:setRelativePosition(GlobalMethod:ccp(0.81,0.475))
	local txtC4T2 = GetElement(self.m_root,"txtC4T2_WndTransaction",WZUILabelTTF)
	txtC4T2:setScale(0.7)
	txtC4T2:setRelativePosition(GlobalMethod:ccp(0.81,0.475))
	local txtC4T3 = GetElement(self.m_root,"txtC4T3_WndTransaction",WZUILabelTTF)
	txtC4T3:setScale(0.7)
	txtC4T3:setRelativePosition(GlobalMethod:ccp(0.81,0.475))
