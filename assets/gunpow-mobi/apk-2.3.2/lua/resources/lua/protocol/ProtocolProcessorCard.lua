...
function ProtocolProcessorCard:regAll()
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_CARD, Protocol.CARD_GetCardMesOk, "ProtocolProcessorCard:parse_CARD_GetCardMesOk", "vivivivivsvsviviivii")
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_CARD, Protocol.CARD_OpenCardSetOk, "ProtocolProcessorCard:parse_CARD_OpenCardSetOk", "iviviiiii")
    ...
    --@brief    兑换卡魂（CARD_ExchangeCardSoulOk = 17）
    self:regProtocolCallbackFunction( Protocol.MAIN_CARD, Protocol.CARD_ExchangeCardSoulOk, "ProtocolProcessorCard:parse_CARD_ExchangeCardSoulOk", "iiii")
    --@brief    瞻仰卡魂（CARD_RevereCardSoulOk = 19）
    self:regProtocolCallbackFunction( Protocol.MAIN_CARD, Protocol.CARD_RevereCardSoulOk, "ProtocolProcessorCard:parse_CARD_RevereCardSoulOk", "ii")
    ...
    --@brief    兑换卡魂（CARD_ExchangeCardSoul = 16）错误处理(S->C)
    self:regProtocolCallbackFunction( Protocol.MAIN_CARD, Protocol.CARD_ExchangeCardSoul, "ProtocolProcessorCard:send_CARD_ExchangeCardSoul_ErrorProcess", "is")
    --@brief    瞻仰卡魂（CARD_RevereCardSoul = 18）错误处理(S->C)
    self:regProtocolCallbackFunction( Protocol.MAIN_CARD, Protocol.CARD_RevereCardSoul, "ProtocolProcessorCard:send_CARD_RevereCardSoul_ErrorProcess", "is")
...
function ProtocolProcessorCard:send_CARD_OpenCardSet(cardSetId ,batch)
    WZLog("send_CARD_OpenCardSet",cardSetId,batch)
...
--@brief    兑换卡魂（CARD_ExchangeCardSoul = 16）
function ProtocolProcessorCard:send_CARD_ExchangeCardSoul(cardSetId, num)
    WZLog("send_CARD_ExchangeCardSoul", cardSetId, num)
    local sender = Protocol:getSender( Protocol.MAIN_CARD, Protocol.CARD_ExchangeCardSoul )
    if sender==nil then WZLog("sender == nil") return end

    sender:writeInt(cardSetId)  -- 卡套id
    sender:writeInt(num)    -- 兑换张数
    SendProtocol(sender,false) --true:showLoading
end

--@brief    瞻仰卡魂（CARD_RevereCardSoul = 18）
function ProtocolProcessorCard:send_CARD_RevereCardSoul()
    WZLog("send_CARD_RevereCardSoul")
    local sender = Protocol:getSender( Protocol.MAIN_CARD, Protocol.CARD_RevereCardSoul )
    if sender==nil then WZLog("sender == nil") return end
...
--------------------------服务器到客户端协议回调方法模块----------------------------------
--@brief    获取卡牌信息（CARD_GetCardMesOk = 2）
function ProtocolProcessorCard:parse_CARD_GetCardMesOk(itemId, level, num, shopId, shopItem, shopPrice, shopRebate, shopStatus, refreshCount, lookItemId, cardSoulBuffTime)
    -- itemId : 物品Id
    -- level : 等级
    -- num : 数量
    -- position : 商店位置
    ...
    WZLog("ProtocolProcessorCard:parse_CARD_GetCardMesOk",
        ...
        "\nlookItemId =",Serialize(VectorToTable(lookItemId)),
        "\ncardSoulBuffTime =",cardSoulBuffTime
    )
    WndCard:setData(VectorToTable(itemId), VectorToTable(level), VectorToTable(num), VectorToTable(lookItemId), cardSoulBuffTime)
end
...
--@brief    开启宝箱（CARD_OpenCardSetOk = 12）
function ProtocolProcessorCard:parse_CARD_OpenCardSetOk(code, itemId, itemNum, cdTime, openNum, batch)
    -- code : 宝箱开启状态0成功1道具不足
    -- itemId : 获得物品id
    -- itemNum : 获得物品数量
    -- cdTime : 宝箱CD（秒）
...
--@brief    兑换卡魂（CARD_ExchangeCardSoulOk = 17）
function ProtocolProcessorCard:parse_CARD_ExchangeCardSoulOk(result, cardSetId, changeNum, cardSoulNum)
    -- result : 兑换结果:0、成功,1、卡片不足
    -- cardSetId : 卡套id
    -- changeNum : 兑换数量
    -- cardSoulNum : 获得卡魂数量
    WZLog("ProtocolProcessorCard:parse_CARD_ExchangeCardSoulOk", result, cardSetId, changeNum, cardSoulNum)
    WndCard:exchangeCardSoulOk(result, cardSetId, changeNum, cardSoulNum)
end

--@brief    瞻仰卡魂（CARD_RevereCardSoulOk = 19）
function ProtocolProcessorCard:parse_CARD_RevereCardSoulOk(buffTime, result)
    -- buffTime : 如果大于0表示当前buff所剩余的时间
    -- result : 0、成功,1、buff正生效中
    WZLog("ProtocolProcessorCard:parse_CARD_RevereCardSoulOk", buffTime, result)
    WndCard:revereCardSoulOk(buffTime, result)
end
...
--@brief    兑换卡魂（CARD_ExchangeCardSoul = 16）错误处理函数(S->C)
--@param    nFlag:标志位
--@param    sMessage:错误信息
--@note 在此对协议错误进行相应处理
function ProtocolProcessorCard:send_CARD_ExchangeCardSoul_ErrorProcess(nFlag, sMessage)
    WZLog("ProtocolProcessorCard:parse_CARD_ExchangeCardSoul_ErrorProcess")
    ProtocolErrorProcessor:errorProcess(Protocol.MAIN_CARD, Protocol.CARD_ExchangeCardSoul, nflag, sMessage)
end

--@brief    瞻仰卡魂（CARD_RevereCardSoul = 18）错误处理函数(S->C)
--@param    nFlag:标志位
--@param    sMessage:错误信息
--@note 在此对协议错误进行相应处理
function ProtocolProcessorCard:send_CARD_RevereCardSoul_ErrorProcess(nFlag, sMessage)
    WZLog("ProtocolProcessorCard:parse_CARD_RevereCardSoul_ErrorProcess")
    ProtocolErrorProcessor:errorProcess(Protocol.MAIN_CARD, Protocol.CARD_RevereCardSoul, nflag, sMessage)
end
