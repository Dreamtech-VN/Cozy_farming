...
function ProtocolProcessorSceneBossRoom:regAll()
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_BOSSMAPROOM, Protocol.BOSSMAPROOM_MakePair , "ProtocolProcessorSceneBossRoom:parse_BOSSMAPROOM_MakePair", "is")

	--@brief	副本随机配对成功同步对战玩家数据（BOSSMAPROOM_MakePairOk = 25）
	self:regProtocolCallbackFunction( Protocol.MAIN_BOSSMAPROOM, Protocol.BOSSMAPROOM_MakePairOk, "ProtocolProcessorSceneBossRoom:parse_BOSSMAPROOM_MakePairOk", "iiivivsvsvsvsvivivivivivivivivivivivivivivivivivivivivivivsvivivsvivivivivnvivivivivsvivivivsvivsvsvsvivivivi")
	...
	--@brief	发起助战（BOSSMAPROOM_Assist= 35）错误处理(S->C)
	self:regProtocolCallbackFunction( Protocol.MAIN_BOSSMAPROOM, Protocol.BOSSMAPROOM_Assist, "ProtocolProcessorSceneBossRoom:send_BOSSMAPROOM_Assist_ErrorProcess", "is" )
end
...
function ProtocolProcessorSceneBossRoom:send_BOSSMAPROOM_Invite(roomId, playerId, interfaceId)
	WZLog("send_BOSSMAPROOM_Invite",roomId, playerId ,assist)
	...
	sender:writeInt( interfaceId ) --界面id (目前用于师徒改版1:组队副本)
...
--@brief	发起助战（BOSSMAPROOM_Assist= 35）
function ProtocolProcessorSceneBossRoom:send_BOSSMAPROOM_Assist(roomId)
	WZLog("send_BOSSMAPROOM_Assist")
	local sender = Protocol:getSender( Protocol.MAIN_BOSSMAPROOM, Protocol.BOSSMAPROOM_Assist )
	if sender==nil then WZLog("sender == nil") return end
...
function ProtocolProcessorSceneBossRoom:parse_BOSSMAPROOM_MakePairOk(battleId,mapId, playerCount, playerId, serverId, 
	...
	petLevel, colour, bodyColour,footmark, professionId, professionSkill, playerCamp, mountId, childId, childName, childSex, 
	...
	-- professionId : 职业
	-- professionSkill : 激活的职业技能
	-- mountId : 出战的坐骑ID，没有的传0【162新增】
    -- childId : 出战的孩子ID，没有的传0【162新增】
    -- childName : 孩子的名字，没有的传""空字符串【162新增】
    -- childSex : 孩子性别【162新增】
    -- childImage : 孩子的形象["A头,A脸,A身","B头,B脸,B身","C头,C脸,C身"] 没有的传""空字符串【162新增】
    -- assistSkillIds : 助战技ids["前3孩子技能后3孩子技能","1,2,3,4,0,-1"]【1,2,3为孩子技能ID，4为坐骑技能，0表示空技能槽，-1表示技能槽未解锁】【162新增】
	...
	local copyLua = SceneBossRoom
	if WndSingleCopyInfo.m_root then --岛主挑战
		copyLua = WndSingleCopyInfo
	else
		if locData and locData.map_num == 0 then
			copyLua = SceneMarryCopy
		else
			local floorData = GDatatab_grouptower_map["id_" .. mapId]
			if floorData then 
				copyLua = WndDoubleTowerRoom
			end
		end
	end
	copyLua:receiveMakePairOk(VectorToTable(battleId),
		VectorToTable(mapId),
		...
		VectorToTable(professionId),
		VectorToTable(professionSkill),
		VectorToTable(playerCamp),
		VectorToTable(mountId), 
		VectorToTable(childId), 
		VectorToTable(childName), 
		VectorToTable(childSex), 
		VectorToTable(childImage), 
		VectorToTable(assistSkillIds),
		VectorToTable(defaultShapeBigSkill),
		VectorToTable(blastEffect),
		VectorToTable(extPropertyKey),
		VectorToTable(extPropertyValue),
		VectorToTable(extPropertyCount)
	)
...
--@brief	副本通知所有玩家正在随机配对对战用户（BOSSMAPROOM_MakePair = 24）
function ProtocolProcessorSceneBossRoom:parse_BOSSMAPROOM_MakePair(roomId, sMsg)
	-- roomId : 房间Id
	...
	elseif SceneBossRoom.m_root then 
		MsgBoxManager:showTipBox(sMsg)
		SceneBossRoom:receiveMakePairring(roomId)
	elseif WndDoubleTowerRoom.m_root then 
		MsgBoxManager:showTipBox(sMsg)
		WndDoubleTowerRoom:receiveMakePairring(roomId)
	elseif WndSingleCopyInfo.m_root then --岛主挑战
		MsgBoxManager:showTipBox(sMsg)
	end
end
...
function ProtocolProcessorSceneBossRoom:parse_BOSSMAPROOM_QuitRoomOk(isInitiative,bDissolution)
    -- isInitiative : 是否主动退出
    -- bDissolution : 是否解散房间
	WZLog("ProtocolProcessorSceneBossRoom:parse_BOSSMAPROOM_QuitRoomOk ",isInitiative,bDissolution)
  ...
  --       end
		...
		if WndDoubleTowerRoom.m_root then 
			WndDoubleTowerRoom:exitMulRoom()
		elseif WndSingleCopy.m_root then --岛主挑战
			WndSingleCopyInfo:exitMulRoom(isInitiative)
		else
...
function ProtocolProcessorSceneBossRoom:parse_BOSSMAPROOM_MakePairFail()
	...
	WndDoubleTowerRoom:receiveMakePairFail()
	WndSingleCopyInfo:receiveMakePairFail()
end
...
function ProtocolProcessorSceneBossRoom:send_BOSSMAPROOM_UpdateRoom_ErrorProcess(nFlag, sMessage)
    ...
    MsgBoxManager:showTipBox(sMessage)
    SceneBossRoom:resetSelDiffercenty()
end
...
--@brief	发起助战（BOSSMAPROOM_Assist = 35）错误处理函数(S->C)
--@param	nFlag:标志位
--@param	sMessage:错误信息
--@note	在此对协议错误进行相应处理
function ProtocolProcessorSceneBossRoom:send_BOSSMAPROOM_Assist_ErrorProcess(nFlag, sMessage)
	WZLog("ProtocolProcessorSceneBossRoom:send_BOSSMAPROOM_Assist_ErrorProcess")
	ProtocolErrorProcessor:errorProcess(Protocol.MAIN_BOSSMAPROOM, Protocol.BOSSMAPROOM_Assist, nflag, sMessage)
end
