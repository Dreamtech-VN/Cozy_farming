...
function ProtocolProcessorSceneWorldBoss:regAll()
	...
	--@brief	获取房间状态成功	
	self:regProtocolCallbackFunction( Protocol.MAIN_WORLDBOSS, Protocol.WORLDBOSS_GetRoomStateOk, "ProtocolProcessorSceneWorldBoss:parse_WORLDBOSS_GetRoomStateOk", "iiivivsvivivivtvivisiiiiiiiii")
	--@brief	开始战斗失败	
	...
	--@brief	开始战斗成功
	self:regProtocolCallbackFunction( Protocol.MAIN_WORLDBOSS, Protocol.WORLDBOSS_MakePairOK , "ProtocolProcessorSceneWorldBoss:parse_WORLDBOSS_MakePairOk", "iiivivsvsvsvivivivivivivivivivivivivivivivivivivivivivivivsvivivsvivivivivivivivnvivivivivsvivivsvivsvsvsvivivivi")
	--@brief	发送结算信息	
	self:regProtocolCallbackFunction( Protocol.MAIN_WORLDBOSS, Protocol.WORLDBOSS_SendSettlementInfo, "ProtocolProcessorSceneWorldBoss:parse_WORLDBOSS_SendSettlementInfo", "siibis")
...
function ProtocolProcessorSceneWorldBoss:parse_WORLDBOSS_SendOpenState(mapId, state, time, overTime)
	...
	if WndChallengeEntrance.m_root then
...
--@brief	获取房间状态成功	
function ProtocolProcessorSceneWorldBoss:parse_WORLDBOSS_GetRoomStateOk(mapId, bossBloodMax, bossBloodCurrent,rankPlayerId, rankPlayerName, rankHurt,  headId,faceId,sex,headColor,vipLevel,   hurt, cdTime, accelerateCost, inspire, bossLevel, myRank, dimaCDTime, goldCDTime,bossState,openTime)
	-- mapId : 地图id
    ...
    if SceneWorldBoss.m_root then
		SceneWorldBoss:setEnterRoomData( mapId, bossBloodMax, bossBloodCurrent,rankPlayerId, rankPlayerName, rankHurt,  headId,faceId,sex,headColor,vipLevel,  hurt, cdTime, accelerateCost, inspire, bossLevel, myRank, dimaCDTime, goldCDTime,bossState,openTime)
...
function ProtocolProcessorSceneWorldBoss:parse_WORLDBOSS_MakePairOk(battleId, mapId, playerCount, playerId, playerName, 
	...
	guaiAtk,guaiLv,petLevel, colour, bodyColour,footmark, professionId, professionSkill, mountId, childId, childName, childSex, childImage, assistSkillIds, defaultShapeBigSkill, blastEffect, extPropertyKey, extPropertyValue, extPropertyCount)
	-- battleId : 战斗组Id
	-- mapId : 地图id
	-- playerCount : 玩家数量
	-- playerId : 所有玩家id
...
WZLog("parse_WORLDBOSS_MakePairOk",
...
"\ninsptre =",				Serialize(VectorToTable(insptre)),
...
"\nguaiAtk =",				Serialize(VectorToTable(guaiAtk)),
"\nguaiLv =",				Serialize(VectorToTable(guaiLv)),
