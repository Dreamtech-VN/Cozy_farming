...
function ProtocolProcessorWorldTeamBossRoom:regAll()
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_TEAMWORLDBOSS, Protocol.TEAMWORLDBOSS_MakePairOk, "ProtocolProcessorWorldTeamBossRoom:parse_TEAMWORLDBOSS_MakePairOk", "iiivivsvsvsvsvivivivivivivivivivivivivivivivivivivivivivivivsvivivsvivivivsvsvivnvivivivivsvivivsvivsvsvsvivivivi")
	--@brief	进入房间成功（TEAMWORLDBOSS_EnterRoomOk = 6）
	self:regProtocolCallbackFunction( Protocol.MAIN_TEAMWORLDBOSS, Protocol.TEAMWORLDBOSS_EnterRoomOk, "ProtocolProcessorWorldTeamBossRoom:parse_TEAMWORLDBOSS_EnterRoomOk", "issiiiivbvivivsvivbvivivivivsvivivivivsvsvivissssssvivivivivivivivivivivi")
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_TEAMWORLDBOSS, Protocol.TEAMWORLDBOSS_GetHurtRankOk, "ProtocolProcessorWorldTeamBossRoom:parse_TEAMWORLDBOSS_GetHurtRankOk", "vivsvbvsvivivtviviviviis")
...
function ProtocolProcessorWorldTeamBossRoom:send_TEAMWORLDBOSS_Invite(roomId, playerId, interfaceId)
...
function ProtocolProcessorWorldTeamBossRoom:parse_TEAMWORLDBOSS_EnterRoomOk(roomId, passWord, roomName, playerNumMode, mapId, wnersId, playerNum, seatUsed, playerId, serviceId, playerName, playerLevel, playerReady, playerSex, playerEquipment, playerEquipmentLevel, vipLevel, player_title, qualifyingLevel, zsleve, playerStar, playerFighting, pet, extranInfo, playerHeadColour, playerBodyColour, mentoringStr, coupleStr, chumStr, coupleNum, chumNum, mentoringNum, winNum, playNum, tournamentExp, skillId, matchLevel, matchscore, joinTimes, winTimes, continuousWinTimes, inspire, openStatus)
	...
	SceneWorldTeamBossRoom:receiveEnterRoomOk(
		...
		mentoringStr,coupleStr,chumStr,coupleNum,chumNum,mentoringNum,VectorToTable(matchLevel),VectorToTable(matchscore),VectorToTable(joinTimes),VectorToTable(winTimes),VectorToTable(continuousWinTimes),VectorToTable(serviceId),VectorToTable(inspire),VectorToTable(openStatus))
end
...
function ProtocolProcessorWorldTeamBossRoom:parse_TEAMWORLDBOSS_GetRoomStateOk(mapId, bossNowHp, diamondCDTime, goldCDTime, bossState, openTime, inspire, challengeNum, leaveNum, maxHp)
	...
	SceneWorldTeamBossRoom:setEnterRoomData( mapId, tonumber(bossNowHp), inspire, diamondCDTime, goldCDTime, bossState, openTime, challengeNum, leaveNum, tonumber(maxHp))
end
...
--[[	WZLog("ProtocolProcessorWorldTeamBossRoom:parse_TEAMWORLDBOSS_GetHurtRankOk",
	...
	if SceneWorldTeamBossRoom.m_root then
		SceneWorldTeamBossRoom:setHurtList(VectorToTable(playerNum), VectorToTable(playerId), VectorToTable(playerName), VectorToTable(sex), VectorToTable(headId), VectorToTable(faceId), VectorToTable(headColor), VectorToTable(vipLevel), VectorToTable(hurt), VectorToTable(level), myRank, tonumber(myHurt))
	end
	if WndTowerRank.m_root then
		WndTowerRank:getTeamWorldbossRankOk(VectorToTable(playerId), VectorToTable(playerName), VectorToTable(cross), VectorToTable(hurt), VectorToTable(headId), VectorToTable(faceId), VectorToTable(sex), VectorToTable(headColor), VectorToTable(vipLevel), VectorToTable(level), VectorToTable(playerNum), myRank, tonumber(myHurt))
	end
end
...
function ProtocolProcessorWorldTeamBossRoom:parse_TEAMWORLDBOSS_BuyChallengeNumOk(leaveNum)
	-- leaveNum : 剩余挑战次数
	WZLog("ProtocolProcessorWorldTeamBossRoom:parse_TEAMWORLDBOSS_BuyChallengeNumOk", leaveNum)

	SceneWorldTeamBossRoom:updateChallengeTimes(leaveNum)
end
------------------------------------------------------------------------------------------------
