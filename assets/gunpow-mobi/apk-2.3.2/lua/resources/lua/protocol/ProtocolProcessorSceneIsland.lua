...
function ProtocolProcessorSceneIsland:regAll()
    ...
    --@brief	调研提交（PLAYER_Investigate = 115）错误处理(S->C)
    self:regProtocolCallbackFunction( Protocol.MAIN_PLAYER, Protocol.PLAYER_Investigate, "ProtocolProcessorSceneIsland:send_PLAYER_Investigate_ErrorProcess", "is" )
    --@brief	调研提交（PLAYER_InvestigateOk = 116）
    self:regProtocolCallbackFunction( Protocol.MAIN_PLAYER, Protocol.PLAYER_InvestigateOk, "ProtocolProcessorSceneIsland:parse_PLAYER_InvestigateOk", "i")
end
...
--@brief	调研提交（PLAYER_Investigate = 115）
function ProtocolProcessorSceneIsland:send_PLAYER_Investigate(titleId, option )
	WZLog("send_PLAYER_Investigate")
	local sender = Protocol:getSender( Protocol.MAIN_PLAYER, Protocol.PLAYER_Investigate )
	if sender==nil then WZLog("sender == nil") return end

	sender:writeInts( titleId )	-- 题目Id
	sender:writeStrings( option )	-- 对应下标的选择
	SendProtocol(sender,false) --true:showLoading
end
...
--@brief	调研提交（PLAYER_ InvestigateOk = 116）
function ProtocolProcessorSceneIsland:parse_PLAYER_InvestigateOk(code)
	-- code : 0成功 1失败
	-- itemCount : 奖励的数量
	-- itemIcon : 奖励物品的图标
	WZLog("ProtocolProcessorSceneIsland:parse_PLAYER_InvestigateOk", code)
    WndAnswerSurvey:submitOk(code)
end
-------------------------------------协议错误处理方法模块--------------------------------------
...
--@brief	调研提交（PLAYER_Investigate = 115）错误处理函数(S->C)
--@param	nFlag:标志位
--@param	sMessage:错误信息
--@note	在此对协议错误进行相应处理
function ProtocolProcessorSceneIsland:send_PLAYER_Investigate_ErrorProcess(nFlag, sMessage)
	WZLog("ProtocolProcessorSceneIsland:send_PLAYER_Investigate_ErrorProcess")
	ProtocolErrorProcessor:errorProcess(Protocol.MAIN_PLAYER, Protocol.PLAYER_Investigate, nflag, sMessage)
end
