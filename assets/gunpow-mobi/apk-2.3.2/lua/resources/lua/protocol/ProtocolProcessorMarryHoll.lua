...
function ProtocolProcessorMarryHoll:regAll()
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WEDDING_GetWedListOK, "ProtocolProcessorMarryHoll:parse_WEDDING_GetWedListOK", "vivtvtvsvsvivivbvivivivivivivivivivivivivivivtvivii")
...
--@brief	返回婚礼列表（WEDDING_GetWedListOK = 17）		
function ProtocolProcessorMarryHoll:parse_WEDDING_GetWedListOK(weddingHallId, wedStatus, marryType, manName, womanName, startDate, endDate, usePassword, manFaceId, womanFaceId, manId, womanId, womanColour, manColour, womanbodyColour, manbodyColour, womanbody, manbody, womanhead, manhead, manServerId, womanServerId, progress, host, guest, divorceTime)
	-- weddingHallId : 婚礼id
	...
	-- guest : 宾客数量
	-- divorceTime : 离婚冷却时间戳
	WZLog("ProtocolProcessorMarryHoll:parse_WEDDING_GetWedListOKaaa =",Serialize(VectorToTable(manhead)), divorceTime)
	WndMarryManager:closeLoading()
	if SceneWeddingChurch.m_root then
	    SceneWeddingChurch.m_bBacking = false
	end
	-- if weddingHallId:size() == 0 then 
	-- 	MsgBoxManager:showTipBox(LocalStrings.NOT_WEDDING_LIST)
	-- 	return 
	-- end
	SceneWeddingDaily:SendWedList(weddingHallId, wedStatus, marryType, manName, womanName, startDate, endDate, usePassword, manFaceId, womanFaceId, manId, womanId, womanColour, manColour, womanbodyColour, manbodyColour, womanbody, manbody, womanhead, manhead, manServerId, womanServerId, progress, host, guest, divorceTime)
end
