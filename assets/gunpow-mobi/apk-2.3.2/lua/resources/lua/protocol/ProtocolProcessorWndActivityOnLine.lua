...
function ProtocolProcessorWndActivityOnLine:regAll()
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_ACTIVITY, Protocol.ACTIVITY_GetActivityListInfoOK, "ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_GetActivityListInfoOK", "vivsviviivivivivsvivsvii")
    ...
    --@brief    领取奖励成功
    self:regProtocolCallbackFunction( Protocol.MAIN_ACTIVITY, Protocol.ACTIVITY_ReceiveActivityRewardOK, "ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_ReceiveActivityRewardOK", "viviii")
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_RANK, Protocol.RANK_GetRankRecordOK, "ProtocolProcessorWndActivityOnLine:parse_RANK_GetRankRecordOK", "tvtvivivsvivivtvivsvsvsvsvsvsvsvtvsvivsvivivs")
    ...
    --@brief    历届榜首（RANK_GetRankingTopList = 11）错误处理(S->C)
    self:regProtocolCallbackFunction( Protocol.MAIN_RANK, Protocol.RANK_GetRankingTopList, "ProtocolProcessorWndActivityOnLine:send_RANK_GetRankingTopList_ErrorProcess", "is")
    --@brief    历届榜首（RANK_GetRankingTopListOk = 12）
    self:regProtocolCallbackFunction( Protocol.MAIN_RANK, Protocol.RANK_GetRankingTopListOk, "ProtocolProcessorWndActivityOnLine:parse_RANK_GetRankingTopListOk", "ivivivsvivivivtvnvnvivivivsvivivsvi")
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_ACTIVITY, Protocol.ACTIVITY_GetWelfareCardActivityInfoOk, "ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_GetWelfareCardActivityInfoOk", "iiiivivii")
    ...
    --@brief    疯狂翻倍（ACTIVITY_CrazyDouble = 123）        错误处理(S->C)
    self:regProtocolCallbackFunction( Protocol.MAIN_ACTIVITY, Protocol.ACTIVITY_CrazyDouble, "ProtocolProcessorWndActivityOnLine:send_ACTIVITY_CrazyDouble_ErrorProcess", "is" )
    --@brief    获取疯狂翻倍奖励（ACTIVITY_CrazyDoubleRewrad= 124）       
    self:regProtocolCallbackFunction( Protocol.MAIN_ACTIVITY, Protocol.ACTIVITY_CrazyDoubleRewrad, "ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_CrazyDoubleRewrad", "ivivivi")

    --@brief    oppo会员福利（ACTIVITY_OppoWelfare = 125）        错误处理(S->C)
    self:regProtocolCallbackFunction( Protocol.MAIN_ACTIVITY, Protocol.ACTIVITY_OppoWelfare, "ProtocolProcessorWndActivityOnLine:send_ACTIVITY_OppoWelfare_ErrorProcess", "is" )
...
function ProtocolProcessorWndActivityOnLine:send_ACTIVITY_GetActivityInfo(activityId ,activityType)
    ...
    sender:writeInt( tonumber(activityType))
    SendProtocol(sender,false) --true:showLoading
end

--@brief    获取活动列表等信息
function ProtocolProcessorWndActivityOnLine:send_ACTIVITY_GetActivityListInfo(nType)
    ...
    sender:writeInt(nType or -1)  --活动大类：0->活动；1->福利；2->周年活动；3->暑假活动；4->小推车活动；5->世界杯活动；6->回流活动；10->新年活动；11->回归活动；12->中秋主题活动；13->蓝钻特权；20->精彩推荐；21->蓝钻特权；22->大厅特权
    SendProtocol(sender,false) --true:showLoading
end
...
function ProtocolProcessorWndActivityOnLine:send_ACTIVITY_ReceiveActivityReward(activityId, rewardId, count)
    ...
    sender:writeInt( tonumber(activityId) )  -- 活动id
    sender:writeInt( tonumber(rewardId) ) -- 奖励id(第N个奖励)
...
--@brief    历届榜首（RANK_GetRankingTopList = 11）
function ProtocolProcessorWndActivityOnLine:send_RANK_GetRankingTopList(rankingType, teamPlayerInfoType)
    WZLog("send_RANK_GetRankingTopList", rankingType, teamPlayerInfoType)
    local sender = Protocol:getSender( Protocol.MAIN_RANK, Protocol.RANK_GetRankingTopList )
    if sender==nil then WZLog("sender == nil") return end

    sender:writeInt(rankingType)    -- 排行榜类型 鲜花榜:42(男) 43(女) 
    sender:writeInt(teamPlayerInfoType) -- 返回组队上榜信息 0:默认 1: 返回玩家playerIds,nicknames,points,serverId, 2:不需要返回玩家个人贡献
    SendProtocol(sender,false) --true:showLoading
end
...
--@brief    疯狂翻倍（ACTIVITY_CrazyDouble = 123）        
function ProtocolProcessorWndActivityOnLine:send_ACTIVITY_CrazyDouble(taskId )
    WZLog("send_ACTIVITY_CrazyDouble",taskId)
    local sender = Protocol:getSender( Protocol.MAIN_ACTIVITY, Protocol.ACTIVITY_CrazyDouble )
    if sender==nil then WZLog("sender == nil") return end
...
--@brief    oppo会员福利（ACTIVITY_OppoWelfare = 125）        
function ProtocolProcessorWndActivityOnLine:send_ACTIVITY_OppoWelfare( )
    WZLog("send_ACTIVITY_OppoWelfare")
    local sender = Protocol:getSender( Protocol.MAIN_ACTIVITY, Protocol.ACTIVITY_OppoWelfare )
    if sender==nil then WZLog("sender == nil") return end
...
-------------------------------------协议接收处理方法模块--------------------------------------
--@brief    获取活获取活动列表等信息成功
function ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_GetActivityListInfoOK(activityId, title, startTime, endTime, serverTime , types, type2, param1, param2, ui_id, ui_res, sortValue, nActMainType)
    -- activityId : 活动id
    -- title : 活动标题
    -- startTime : 生效开始时间
    -- endTime : 生效结束时间
    ...
    -- ui_res : 活动的资源
    -- sortValue : 排序
    -- nActMainType : --活动大类：0->活动；1->福利；2->周年活动；3->暑假活动；4->小推车活动；5->世界杯活动；6->回流活动；9->琥珀专属活动；10->新年活动；
    --11->回归活动；12:中秋活动 20->精彩推荐；
    -- bJumpReturneeAct : true 列表中有type2 = 6的活动
        WZLog("ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_GetActivityListInfoOK",
        ...
        "\nserverTime=",Serialize(VectorToTable(serverTime)) ,
        "\ntypes=",Serialize(VectorToTable(types)),
        "\ntype2=",Serialize(VectorToTable(type2)),
    ...
    local types = VectorToTable(types)
    local type2 = VectorToTable(type2)
    local activityId = VectorToTable(activityId)
    local title = VectorToTable(title)
    local startTime = VectorToTable(startTime)
    local endTime = VectorToTable(endTime)
    local ui_id = VectorToTable(ui_id)
    local ui_res = VectorToTable(ui_res)
    GlobalGame:getGameEventDispathcer():Dispatch(WndNationalEvent.WndNationalEvent_GetActivityTitleName,types,type2,activityId,title,startTime,endTime,ui_id,ui_res)
    
    if WndGameActivity.m_bNewOnlineState == 1 and nActMainType == 0 then 
        getNewOnlineRewardState1(VectorToTable(activityId), VectorToTable(types))
        return 
    end

    getMondayPlanCardState(VectorToTable(types))

    --大厅特权
    if WndLobbyPrivilegesAct.m_root ~= nil then
        WndLobbyPrivilegesAct:GetActivityListInfoOK(VectorToTable(activityId), VectorToTable(title), VectorToTable(startTime), VectorToTable(endTime), serverTime , VectorToTable(types), VectorToTable(type2))
        return
    end
    --拍卖行
    if WndAuctionHouseAct.m_root ~= nil then
        WndAuctionHouseAct:GetActivityListInfoOK(VectorToTable(activityId), VectorToTable(title), VectorToTable(startTime), VectorToTable(endTime), serverTime , VectorToTable(types), VectorToTable(type2))
        return
    end
    getMondayPlanCardState(VectorToTable(types))
    
    --拍卖行
    if WndReturneeActivity.m_root ~= nil and (nActMainType == 6 or nActMainType == -1) then
        WndReturneeActivity:GetActivityListInfoOK(VectorToTable(activityId), VectorToTable(title), VectorToTable(startTime), VectorToTable(endTime), serverTime , VectorToTable(types), VectorToTable(type2))
    end
	--首充活动
    ...
    -- --空间
    -- if WndSpaceSendFlower.m_root and (nActMainType == -1 or nActMainType == 0) then 
    ...
    -- end
	--活动
    if WndGameActivity.m_root and nActMainType == 0 then
        WndGameActivity:GetActivityListInfoOK(VectorToTable(activityId), VectorToTable(title), VectorToTable(startTime), VectorToTable(endTime), serverTime , VectorToTable(types), VectorToTable(type2), VectorToTable(sortValue))
        -- return
    end
    --oppo琥珀大玩家专属福利活动
    if WndAmberPlayer.m_root and (nActMainType == 9 or nActMainType == -1) then
        WndAmberPlayer:GetActivityListInfoOK(VectorToTable(activityId), VectorToTable(title), VectorToTable(startTime), VectorToTable(endTime), serverTime , VectorToTable(types), VectorToTable(type2))
        -- return
    end
	--新年活动
    if WndNewActivity.m_root and (nActMainType == 10 or nActMainType == -1) then
	...
	--夏日活动
    if WndSumVacAct.m_root and (nActMainType == 3 or nActMainType == -1) then
	...
	--福利
    if WndWelfare.m_root and (nActMainType == 1 or nActMainType == 20 or nActMainType == -1) then
        WndWelfare:GetWelfareListInfoOK(VectorToTable(activityId), VectorToTable(title), VectorToTable(startTime), VectorToTable(endTime), serverTime , VectorToTable(types), VectorToTable(type2), VectorToTable(sortValue))
        return
    end

    --代言人活动
    if WndApartmentAct.m_root and (nActMainType == 4 or nActMainType == -1) then
    ...
    if WndFootballActivity.m_root and (nActMainType == 5 or nActMainType == -1) then
...
--@brief    获取活动详细内容成功
function ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_GetActivityInfoOK(activityId, content, tips, startTime, endTime, serverTime, rewardId, status, rewardItems, rewardItemsParamCount, rewardCounts,count,maxCount,finishCondition)
    -- activityId : 活动id
    -- content               : 活动内容
    -- tips : 提示内容
    -- startTime : 生效开始时间
    ...
    WZLog("ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_GetActivityInfoOK"
        ,"\nactivityId=",Serialize(VectorToTable(activityId))
        ,"\ncontent=",Serialize(VectorToTable(content))
        ,"\ntips=",Serialize(VectorToTable(tips))
        ,"\nstartTime=",Serialize(VectorToTable(startTime))
        ,"\nendTime=",Serialize(VectorToTable(endTime))
        ,"\nserverTime=",Serialize(VectorToTable(serverTime))
        ,"\nrewardId=",Serialize(VectorToTable(rewardId))
        ,"\nstatus=",Serialize(VectorToTable(status))
        ,"\nrewardItems=",Serialize(VectorToTable(rewardItems))
        ,"\nrewardItemsParamCount=",Serialize(VectorToTable(rewardItemsParamCount))
        ,"\nrewardCounts=",Serialize(VectorToTable(rewardCounts))
        ,"\ncount=",Serialize(VectorToTable(count))
        ,"\nmaxCount=",Serialize(VectorToTable(maxCount))
        ,"\nfinishCondition=",Serialize(VectorToTable(finishCondition)))

    -- 变废为宝活动
    if WndSellList.m_root then
        WndSellList:getActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
        return
    end

    GlobalGame:getGameEventDispathcer():Dispatch(WndNationalEvent.WndNationalEvent_FestivalLogin, activityId, maxCount, count, VectorToTable(status),VectorToTable(rewardCounts),
                                                VectorToTable(rewardItems),VectorToTable(rewardItemsParamCount), startTime, endTime, content, VectorToTable(rewardId), VectorToTable(finishCondition),VectorToTable(tips))

	getNewOnlineRewardState2( VectorToTable(rewardItems), VectorToTable(rewardCounts), count, VectorToTable(finishCondition), VectorToTable(status), activityId)

    --大厅特权
    if WndLobbyPrivilegesAct.m_root then
        WndLobbyPrivilegesAct:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
        return
    end

    if WndReturneeActivity.m_root then
        WndReturneeActivity:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
        return
    end

	if CellRechargePanelActivity.m_root ~= nil then
		CellRechargePanelActivity:getDetailInfoOk(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
	end

    if WndGameActivity.m_root then
        WndGameActivity:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
    end

    --oppo琥珀大玩家专属福利活动
    if WndAmberPlayer.m_root then
        WZLog("--ProtocolProcessorWndActivityOnLine:WndAmberPlayer--")
        WndAmberPlayer:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
    end

    if WndWelfare.m_root then
        WndWelfare:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
    end
    if WndNewActivity.m_root then
        WZLog("--Protocol--")
        WndNewActivity:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
        return
    end
    if WndSumVacAct.m_root then
        WndSumVacAct:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
        -- return
    end

	--代言人活动
    if WndApartmentAct.m_root then
        WndApartmentAct:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
        return
    end

    if WndFootballActivity.m_root then
        WZLog("--Protocol--11")
        WndFootballActivity:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
        return
    end

    --新框架活动
    if WndFrameActivity.m_root then
        WndFrameActivity:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
        return 
    end

    --限定活动
    if WndActivitySpecificSales.m_root then 
        WndActivitySpecificSales:GetActivityInfoOK(activityId, content, VectorToTable(tips) , startTime, endTime, serverTime, VectorToTable(rewardId), VectorToTable(status), VectorToTable(rewardItems), VectorToTable(rewardItemsParamCount), VectorToTable(rewardCounts),count,maxCount,VectorToTable(finishCondition))
        return
    end
end

--@brief    领取奖励成功
function ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_ReceiveActivityRewardOK(rewardItems, rewardCount, type, rewardId)
    -- rewardItems : 物品id
    -- rewardCount : 物品数量
    if type == 0 then
        WndRewardShow:showById(VectorToTable(rewardItems),VectorToTable(rewardCount))
    end
    WZLog("ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_ReceiveActivityRewardOK", Serialize(VectorToTable(rewardItems)), Serialize(VectorToTable(rewardCount)), type)
    GlobalGame:getGameEventDispathcer():Dispatch(WndNationalEvent.WndNationalEvent_GetInfo,VectorToTable(rewardItems),VectorToTable(rewardCount), type, rewardId)
    
    if CellMondayPlanCard.m_root ~= nil then
        CellMondayPlanCard:getRewardOk(VectorToTable(rewardItems),VectorToTable(rewardCount),type)
    end
    if CellRechargePanelActivity.m_root ~= nil then
    ...
    if WndGameActivity.m_root ~= nil then
        WndGameActivity:GetRewardOk(VectorToTable(rewardItems),VectorToTable(rewardCount),type, rewardId)
    end
    if WndReturneeActivity.m_root then
        WndReturneeActivity:GetRewardOk(VectorToTable(rewardItems),VectorToTable(rewardCount),type)
    end
    if WndWelfare.m_root then
    ...
    if WndFrameActivity.m_root ~= nil then
        WndFrameActivity:GetRewardOk(VectorToTable(rewardItems),VectorToTable(rewardCount),type)
    end
    if WndAmberPlayer.m_root ~= nil then
        WndAmberPlayer:GetRewardOk(VectorToTable(rewardItems),VectorToTable(rewardCount),type)
    end
...
function ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_GetFightingKingInfoOk(session, playerId, rank, worshipTimes, totlaWorshipTimes, fighting, name, faceId, headId, sex, level, vipLevel, headColor, bodyId, bodyColor, windId, crossServer,rewardRank,reward)
    ...
    WZLog("ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_GetFightingKingInfoOk", 
        "\n session =",Serialize(VectorToTable(session)), 
        "\n playerId =",Serialize(VectorToTable(playerId)), 
        "\n rank =",Serialize(VectorToTable(rank)), 
        "\n worshipTimes =",Serialize(VectorToTable(worshipTimes)), 
        "\n totlaWorshipTimes =",Serialize(VectorToTable(totlaWorshipTimes)), 
        ...
        "\n windId =",Serialize(VectorToTable(windId)), 
        "\n crossServer =",Serialize(VectorToTable(crossServer)),
        "\n rewardRank =",Serialize(VectorToTable(rewardRank)),
    ...
    if WndWelfare.m_root then 
         WndWelfare:GetMonthFightingListOK_newServer(session, VectorToTable(playerId), VectorToTable(rank), VectorToTable(worshipTimes), VectorToTable(totlaWorshipTimes), VectorToTable(fighting), VectorToTable(name), VectorToTable(faceId), VectorToTable(headId), VectorToTable(sex), 
        VectorToTable(level), VectorToTable(vipLevel), VectorToTable(headColor), VectorToTable(bodyId), VectorToTable(bodyColor), VectorToTable(windId), VectorToTable(crossServer), VectorToTable(rewardRank), VectorToTable(reward))
    end
    if WndFlowerRank.m_root then 
         WndFlowerRank:GetFightingKingInfoOk(session, VectorToTable(playerId), VectorToTable(rank), VectorToTable(worshipTimes), VectorToTable(totlaWorshipTimes), VectorToTable(fighting), VectorToTable(name), VectorToTable(faceId), VectorToTable(headId), VectorToTable(sex), 
        VectorToTable(level), VectorToTable(vipLevel), VectorToTable(headColor), VectorToTable(bodyId), VectorToTable(bodyColor), VectorToTable(windId), VectorToTable(crossServer), VectorToTable(rewardRank), VectorToTable(reward))
    end
end

--@brief    排行榜信息（RANK_GetRankRecordOK = 2）
function ProtocolProcessorWndActivityOnLine:parse_RANK_GetRankRecordOK(rankType, trendRank, ranking, playerId, name, faceId, headId, sex, level, param1, param2, param3, param4, param5, param6, param7, vipLevel, param8, headColor, param9, headEffectId, wifeHeadEffectId, qqHallInfo)
    -- rankType : 排行榜类型
    ...
    if WndMonthFighting.m_root then 
        WndMonthFighting:setRankListData(ranking, playerId, name, faceId, headId, sex, level, param1, param2, param3, param4, param5, param6, param7, rankType, trendRank, vipLevel, param8, headColor, param9, headEffectId, qqHallInfo)
    end
    GlobalGame:getGameEventDispathcer():Dispatch(NewVipEvent.NewVipEvent_VipRankInfo, rankType, playerId, ranking, name, level, faceId, headId, sex, 
                                                param1, param3, param5, param7, headColor,param6, param8, param2, vipLevel, headEffectId, qqHallInfo)
end
...
function ProtocolProcessorWndActivityOnLine:parse_RANK_GetPlayerRankOK(myRank, rankValue, rankExp, myTrendRank, rankType, canWorship)
    ...
    if WndFlowerRank.m_root then
        WndFlowerRank:GetPlayerRankOK(myRank, rankValue, rankExp, myTrendRank, rankType, canWorship)
    end
end

--@brief    历届榜首（RANK_GetRankingTopListOk = 12）
function ProtocolProcessorWndActivityOnLine:parse_RANK_GetRankingTopListOk(rankingType, playerIds, points, nicknames, headIds, headColors, faceIds, sexs, vipLevels, levels, bodyIds, bodyColors, windIds, title, serverId, seasonNum, blueVip, sessions)
    -- rankingType : 排行榜类型
    -- playerIds : 玩家id
    -- points : 积分
    -- nicknames : 昵称
    ...
    -- blueVip : QQ蓝钻信息
    -- sessions : 排行榜多次结算排行榜
    WZLog("ProtocolProcessorWndActivityOnLine:parse_RANK_GetRankingTopListOk", 
...
--@brief    福利卡活动信息（ACTIVITY_GetWelfareCardActivityInfoOk = 63）
function ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_GetWelfareCardActivityInfoOk(progress, num, endTime, rewardStatus, itemId, itemNum, activityId)
    -- progress : 活动是否在进行. 0-进行中 1-已结束
    -- num : 购买次数
    -- endTime : 活动结束时间. 秒.
    -- rewardStatus : 奖励状态 周一卡新增 0无奖励|不可领取,1可领取,2已领取
    -- itemId : 奖励id 周一卡新增
    -- itemNum : 奖励数量 周一卡新增
    -- activityId : 活动id 周一卡新增
    WZLog("ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_GetWelfareCardActivityInfoOk",progress, num, endTime, rewardStatus, Serialize(VectorToTable(itemId)), Serialize(VectorToTable(itemNum)), activityId)
    if WndFreeca.m_root then 
        WndFreeca:GetCardActivityInfoOK(progress, num, endTime, rewardStatus, VectorToTable(itemId), VectorToTable(itemNum), activityId)
    end
end
...
--@brief    获取疯狂翻倍奖励（ACTIVITY_CrazyDoubleRewrad= 124）       
function ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_CrazyDoubleRewrad(taskId, dice, itemId, itemNum)
    -- taskId : 任务id
    -- dice : 骰子
    -- itemId : 物品id
    -- itemNum : 物品数量
    WZLog("ProtocolProcessorWndActivityOnLine:parse_ACTIVITY_CrazyDoubleRewrad",taskId, Serialize(VectorToTable(dice)), Serialize(VectorToTable(itemId)), Serialize(VectorToTable(itemNum)))
    WndCrazyDoubling:getCrazyDoubleRewrad(taskId, VectorToTable(dice), VectorToTable(itemId), VectorToTable(itemNum))
end
...
--@brief    历届榜首（RANK_GetRankingTopList = 11）错误处理函数(S->C)
--@param    nFlag:标志位
--@param    sMessage:错误信息
--@note 在此对协议错误进行相应处理
function ProtocolProcessorWndActivityOnLine:send_RANK_GetRankingTopList_ErrorProcess(nFlag, sMessage)
    WZLog("ProtocolProcessorWndActivityOnLine:parse_RANK_GetRankingTopList_ErrorProcess")
    ProtocolErrorProcessor:errorProcess(Protocol.MAIN_RANK, Protocol.RANK_GetRankingTopList, nflag, sMessage)
end
...
--@brief    疯狂翻倍（ACTIVITY_CrazyDouble = 123）        错误处理函数(S->C)
--@param    nFlag:标志位
--@param    sMessage:错误信息
--@note 在此对协议错误进行相应处理
function ProtocolProcessorWndActivityOnLine:send_ACTIVITY_CrazyDouble_ErrorProcess(nFlag, sMessage)
    WZLog("ProtocolProcessorWndActivityOnLine:send_ACTIVITY_CrazyDouble_ErrorProcess")
    ProtocolErrorProcessor:errorProcess(Protocol.MAIN_ACTIVITY, Protocol.ACTIVITY_CrazyDouble, nflag, sMessage)
end
