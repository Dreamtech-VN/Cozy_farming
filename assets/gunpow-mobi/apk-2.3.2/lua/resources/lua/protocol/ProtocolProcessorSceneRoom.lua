...
function ProtocolProcessorSceneRoom:regAll()
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_ROOM, Protocol.ROOM_MakePairOk, "ProtocolProcessorSceneRoom:parse_ROOM_MakePairOk", "iiiiiivivivsvsvsvsvivivivivivivivivivivivivivivivivivivivivivivivivsvivivsvivivivivivivivsvsvnvivivbviivivsvivivsvivsvsvsvivivivi")
...
--@brief	邀请战斗
function ProtocolProcessorSceneRoom:send_ROOM_Invite(roomId, playerId, interfaceId)
	...
	sender:writeInt( interfaceId )	--interfaceId:师徒的时候竞技邀请 
	SendProtocol(sender,false) --true:showLoading
end
...
--@brief	随机配对对战用户
function ProtocolProcessorSceneRoom:send_ROOM_MakePair(roomId,channel,schedule,battleMode,numMode)
	WZLog("send_ROOM_MakePair =",roomId,channel,schedule,battleMode,numMode)
	...
	sender:writeInt( numMode or 0 )	-- 人数模式[178+ 战略赛需要根据人数模式判断是否处于开放日期]
	SendProtocol(sender,false) --true:showLoading
end
...
extPropertyKey, extPropertyValue, extPropertyCount)
	...
	roomLua:receiveMakePairOk(VectorToTable(battleId), battleMode, battleChannle, schedule,
		...
		VectorToTable(petLevel),VectorToTable(colour),VectorToTable(bodyColour),VectorToTable(isCaptain),
		VectorToTable(footmark), monsterId, VectorToTable(professionId), VectorToTable(professionSkill), 
		VectorToTable(mountId), VectorToTable(childId), VectorToTable(childName), VectorToTable(childSex), 
		VectorToTable(childImage), VectorToTable(assistSkillIds), VectorToTable(defaultShapeBigSkill), VectorToTable(blastEffect),
