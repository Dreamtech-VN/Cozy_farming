...
function ProtocolProcessorWndBag:regAll1()
	--@brief	获取角色信息（PLAYER_GetPlayerInfoOk = 2）
	self:regProtocolCallbackFunction( Protocol.MAIN_PLAYER, Protocol.PLAYER_GetPlayerInfoOk, "ProtocolProcessorWndBag:parse_PLAYER_GetPlayerInfoOk", "isisssiiiiissbvivssiiisvsiiinsisiissviiiissiisitisviviiiviiiiiiiiiiivsisviviisiisssiisiiissiiivsssviiisssssiivsiisiivivissvsssss")
	--@brief	获取指定玩家的数据信息（PLAYER_GetPlayerDataOk = 25）
	self:regProtocolCallbackFunction( Protocol.MAIN_PLAYER, Protocol.PLAYER_GetPlayerDataOk, "ProtocolProcessorWndBag:parse_PLAYER_GetPlayerDataOk", "iivs")

	--@brief	目标服务器状态（PLAYER_ServerError = 85）
	self:regProtocolCallbackFunction( Protocol.MAIN_PLAYER, Protocol.PLAYER_ServerError, "ProtocolProcessorWndBag:parse_PLAYER_ServerError", "is")
	--@brief	获取指定玩家的数据信息（PLAYER_GetPlayerData = 24）错误处理(S->C)
	self:regProtocolCallbackFunction( Protocol.MAIN_PLAYER, Protocol.PLAYER_GetPlayerData, "ProtocolProcessorWndBag:send_PLAYER_GetPlayerData_ErrorProcess", "is")
end

function ProtocolProcessorWndBag:unregAll1()
	--@brief	获取角色信息（PLAYER_GetPlayerInfoOk = 2）
	self:unregProtocolCallbackFunction( Protocol.MAIN_PLAYER, Protocol.PLAYER_GetPlayerInfoOk, "ProtocolProcessorWndBag:parse_PLAYER_GetPlayerInfoOk", "isisssiiiiissbvivssiiisvsiiinsisiissviiiissiisitisviviiiviiiiiiiiiiivsisviviisiisssiisiiissiiivsssviiisssssiivsiisiivivissvsssss")
	--@brief	获取指定玩家的数据信息（PLAYER_GetPlayerDataOk = 25）
	self:unregProtocolCallbackFunction( Protocol.MAIN_PLAYER, Protocol.PLAYER_GetPlayerDataOk, "ProtocolProcessorWndBag:parse_PLAYER_GetPlayerDataOk", "iivs")

	--@brief	目标服务器状态（PLAYER_ServerError = 85）
	self:unregProtocolCallbackFunction( Protocol.MAIN_PLAYER, Protocol.PLAYER_ServerError, "ProtocolProcessorWndBag:parse_PLAYER_ServerError", "is")
	--@brief	获取指定玩家的数据信息（PLAYER_GetPlayerData = 24）错误处理(S->C)
	self:unregProtocolCallbackFunction( Protocol.MAIN_PLAYER, Protocol.PLAYER_GetPlayerData, "ProtocolProcessorWndBag:send_PLAYER_GetPlayerData_ErrorProcess", "is")
end
...
function ProtocolProcessorWndBag:send_PLAYER_GetPlayerInfo(playerId )
	WZLog("send_PLAYER_GetPlayerInfo",playerId)
...
--@brief	获取指定玩家的数据信息（PLAYER_GetPlayerData = 24）
function ProtocolProcessorWndBag:send_PLAYER_GetPlayerData(playerId, dataType)
	WZLog("send_PLAYER_GetPlayerData")
	local sender = Protocol:getSender( Protocol.MAIN_PLAYER, Protocol.PLAYER_GetPlayerData )
	if sender==nil then WZLog("sender == nil") return end

	sender:writeInt(playerId)	-- 玩家id
	sender:writeInt(dataType)	-- 数据类型【1=坐骑数据 | 2=足迹数据】
	SendProtocol(sender,false) --true:showLoading
end
-------------------------------------服务器到客户端协议回调方法模块--------------------------------------
--@brief	获取角色信息（PLAYER_GetPlayerInfoOk = 2）
function ProtocolProcessorWndBag:parse_PLAYER_GetPlayerInfoOk(id, name, sex, title, guildName, position, level, vipLevel, winNum, playNum, fighting, 
	mateName, signature, isFriend, itemId, extranInfo, property, strongSuitId, starSuitId, mosaicSuitId, petMessage, mountsMessage, tournamentLevel, 
	segmentId, totemLevel, loveLevel, loveSkill, moralityLevel, masterName, itemSuitId, itemSuitNum, snsValue, rankMatchMessage, starsoulId, guildLevel, 
	spaceSex, giftNum, distance, headScul, mentoring, couple, useMountsMessage, tournamentIntegral, marryFlag, serverId, prayInfo, xlId, xlExp, headColor,
	bodyColor, isUse, chum, shapeId, shapeLevel, showShape, awakeSoulLevel, awakeStep, itemSuitId2, itemSuitNum2, homeLevel, sheerLuxury, footMark, 
	shapeSkillId, awakeSkillId, runeItemId, runeItemNum, obtainNum, cardMessage, bgId, showMes, coupleMes, childMes, careBuffProp, careToday, thumbUpNum, 
	badgeInfo, professionId, myMaxSegmentLevel, masterId, remarkName, ylJsonInfo, honourPoint, itemSuitStrongNum, itemSuitStarNum, shape, 
	shapeFetterProperties, soulInfo, rpIds, wedBufLevel, wedBufTime, loveSkill2, professionAttr1, professionAttr2,vipMedal,phantomEquipment,pastureId,
	spriteStoneFp, spriteStoneInfo, friendliness, infoBarItemId,pupliInfo, myMoralityLevel, headEffectId, footMarkCityIds, footMarkCityTimes, medalInfo, 
	blueVipInfo, petEquip, runeResonateAdd, cardSoulBuffAdd, guildBaptismAdd, zlsJsonInfo)
	-- id : Id
	-- name : 名称
	-- sex : 性别
	-- title : 称号
	...
	-- loveSkill : 夫妻技能json字符串｛技能类型：技能id｝ 非同阵营
	-- moralityLevel : 师德等级
	-- masterName : 师傅名称
	-- itemSuitId : 套装id
	-- itemSuitNum : n件套
	...
	-- badgeInfo : 成就徽章数据
	-- professionId : 玩家职业Id
	-- myMaxSegmentLevel : 玩家历史最高排位等级
	-- masterId : 玩家师傅Id(没师傅=0)
	-- remarkName : 备注名字
	-- ylJsonInfo : 娱乐竞技信息
	-- honourPoint
	...
	WZLog("ProtocolProcessorWndBag:parse_PLAYER_GetPlayerInfoOk", wedBufLevel, wedBufTime, blueVipInfo, zlsJsonInfo)
	if WndCheckOther.m_root == nil then
		WZLog("ProtocolProcessorWndBag:parse_PLAYER_GetPlayerInfoOk 222", wedBufLevel, wedBufTime, blueVipInfo)
		local wnd = WndCheckOther:createElement()
		WndCheckOther.m_nPlayerId = id
		WindowManager:addWindow(wnd, WndCheckOther, false, nil, nil, true)
	end

    if WndCheckOther.m_root then
        WndCheckOther:setPlayerInfo(id, name, sex, title, guildName, position, level, vipLevel, winNum, playNum, fighting, mateName, signature, 
        	isFriend, itemId, extranInfo, property, strongSuitId, starSuitId, mosaicSuitId, petMessage, mountsMessage, tournamentLevel, segmentId, 
        	totemLevel, loveLevel, loveSkill,moralityLevel,masterName, itemSuitId, itemSuitNum, snsValue, rankMatchMessage, starsoulId, guildLevel, 
        	spaceSex, giftNum, distance, headScul, mentoring, couple, useMountsMessage, tournamentIntegral, marryFlag, serverId, prayInfo, xlId, xlExp, 
        	headColor, bodyColor, isUse, chum, shapeId, shapeLevel, showShape, awakeSoulLevel, awakeStep, itemSuitId2, itemSuitNum2, homeLevel, 
        	sheerLuxury, footMark, shapeSkillId, awakeSkillId, runeItemId, runeItemNum, obtainNum, cardMessage, bgId, showMes, coupleMes, childMes, 
        	careBuffProp, careToday, thumbUpNum, badgeInfo, professionId, myMaxSegmentLevel, masterId, remarkName, ylJsonInfo, honourPoint, shape, 
        	shapeFetterProperties, soulInfo, rpIds, wedBufLevel, wedBufTime, loveSkill2, professionAttr1, professionAttr2, vipMedal, 
        	phantomEquipment, pastureId,spriteStoneFp,spriteStoneInfo, friendliness, infoBarItemId, pupliInfo, myMoralityLevel, headEffectId, 
        	footMarkCityIds, footMarkCityTimes, medalInfo, blueVipInfo, petEquip, runeResonateAdd, cardSoulBuffAdd, guildBaptismAdd, zlsJsonInfo)
    end
end
...
function ProtocolProcessorWndBag:parse_PLAYER_ServerError(status, tip)
	...
	-- GlobalGame:getGameEventDispathcer():Dispatch(bottomMeneEvent.WndBottomMeneEvent_GetHouseHeadMessage, status, tip)
...
--@brief	获取指定玩家的数据信息（PLAYER_GetPlayerDataOk = 25）
function ProtocolProcessorWndBag:parse_PLAYER_GetPlayerDataOk(playerId, dataType, jsonData)
	-- playerId : 玩家id
	-- dataType : 数据类型【1=坐骑数据 | 2=足迹数据 | 3=皮肤数据】dataType=6(60000 + 总批数*100 + 批次)
	-- json : 内容json字符串
	WZLog("ProtocolProcessorWndBag:parse_PLAYER_GetPlayerDataOk",playerId, dataType, jsonData:size())
	WndCheckOther:setMountsAndFootsData(playerId, dataType, VectorToTable(jsonData))
end
-------------------------------------协议错误处理方法模块--------------------------------------
...
--@brief	获取指定玩家的数据信息（PLAYER_GetPlayerData = 24）错误处理函数(S->C)
--@param	nFlag:标志位
--@param	sMessage:错误信息
--@note	在此对协议错误进行相应处理
function ProtocolProcessorWndBag:send_PLAYER_GetPlayerData_ErrorProcess(nFlag, sMessage)
	WZLog("ProtocolProcessorWndBag:parse_PLAYER_GetPlayerData_ErrorProcess")
	ProtocolErrorProcessor:errorProcess(Protocol.MAIN_PLAYER, Protocol.PLAYER_GetPlayerData, nflag, sMessage)
end