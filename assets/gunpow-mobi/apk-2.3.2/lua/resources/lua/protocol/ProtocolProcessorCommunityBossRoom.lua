...
function ProtocolProcessorCommunityBossRoom:regAll()
	--@brief	获取公会副本信息
	self:regProtocolCallbackFunction( Protocol.MAIN_GUILD, Protocol.GUILD_GetGuildBossInfoOk, "ProtocolProcessorCommunityBossRoom:parse_GUILD_GetGuildBossInfoOk", "iiiiivivsviiisviii")
	--@brief	获取公会副本排行榜
	...
	--@brief	开始战斗成功
	self:regProtocolCallbackFunction( Protocol.MAIN_GUILD, Protocol.GUILD_MakePairOk, "ProtocolProcessorCommunityBossRoom:parse_GUILD_MakePairOk", "iiivivsvsvsvivivivivivivivivivivivivivivivivivivivivivivivsvivivsvivivivsvivivnvivivivivsvivivsvivsvsvsvivivivi")
...
--@brief	获取公会副本信息
function ProtocolProcessorCommunityBossRoom:parse_GUILD_GetGuildBossInfoOk(sectionId, boosId, boosHP, hurtAdd,playerNum,cheerId, cheerName, cheerCost, playTimes, todayGain, weekHurt, reward, bossMaxHp, contribution)
	-- sectionId : 挑战中章节ID
	-- boosId : 挑战中BossID
	-- boosHP : Boss当前血量
	-- hurtAdd : 当前伤害加成
	...
	-- reward : 已领取的周奖励ID
	-- contribution : 个人贡献
	WZLog("ProtocolProcessorCommunityBossRoom:parse_GUILD_GetGuildBossInfoOk =",sectionId,boosHP,bossMaxHp)
	local data = {
		sectionId = sectionId,
		bossId = boosId,
		bossHp = boosHP,
	...
	SceneCommunityMain:updateGuildBossInfo(data)
end
...
--@brief	开始战斗成功
function ProtocolProcessorCommunityBossRoom:parse_GUILD_MakePairOk(battleId, mapId, playerCount, playerId, playerName, playerTitle, playerGuild, playerLevel, playerSex, maxHP, maxPF, maxSP, attack, critRate, defence, injuryFree, wreckDefense, reduceCrit, power, armor, constitution, agility, lucky, inspire, headId, faceId, bodyId, weaponId, wingId, item_id, petSkill, playerBuffCount, buffId, petAnimation, petGift, guaiBattleId, guaiId, guaiMaxHP, guaiNowHP, guaiAttack, petAdvancedLevel, colour, bodycolour, footmark, professionId, professionSkill, mountId, childId, childName, childSex, childImage, assistSkillIds, defaultShapeBigSkill, blastEffect, extPropertyKey, extPropertyValue, extPropertyCount)
	-- battleId : 战斗组Id
	-- mapId : 地图id
	...
	WZLog("ProtocolProcessorCommunityBossRoom:parse_GUILD_MakePairOk ",battleId, mapId, playerCount, Serialize(VectorToTable(playerId)), Serialize(VectorToTable(playerName)), Serialize(VectorToTable(playerTitle)), Serialize(VectorToTable(playerGuild)), Serialize(VectorToTable(playerLevel)), Serialize(VectorToTable(playerSex)), Serialize(VectorToTable(maxHP)), Serialize(VectorToTable(maxPF)), Serialize(VectorToTable(maxSP)),
        ...
        "\npetLevel:", Serialize(VectorToTable(colour)),Serialize(VectorToTable(bodycolour)),Serialize(VectorToTable(defaultShapeBigSkill)),Serialize(VectorToTable(blastEffect)),Serialize(VectorToTable(extPropertyKey)),Serialize(VectorToTable(extPropertyValue)),Serialize(VectorToTable(extPropertyCount)))
	...
	SceneCommunityBossInfo:receiveMakePairOk(battleId, mapId, playerCount,
		...
		VectorToTable(guaiAttack), VectorToTable(weaponSkill), VectorToTable(petAdvancedLevel),VectorToTable(colour),VectorToTable(bodycolour), VectorToTable(footmark), 
		VectorToTable(professionId), VectorToTable(professionSkill), VectorToTable(mountId), VectorToTable(childId), VectorToTable(childName), VectorToTable(childSex), 
		VectorToTable(childImage), VectorToTable(assistSkillIds), VectorToTable(defaultShapeBigSkill), VectorToTable(blastEffect), VectorToTable(extPropertyKey), VectorToTable(extPropertyValue), VectorToTable(extPropertyCount))
end
...
function ProtocolProcessorCommunityBossRoom:send_GUILD_GuildInspire_ErrorProcess(nFlag, sMessage)
	...
	SceneCommunityCopy:closeLoading()
	if sMessage == "3" then
