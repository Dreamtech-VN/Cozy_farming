...
function ProtocolProcessorSceneCommunity:regAll()
	...
	--@brief	获取公会副本boss鼓舞排行（GUILD_GetGuildBossInspireRank = 99）错误处理(S->C)
	self:regProtocolCallbackFunction( Protocol.MAIN_GUILD, Protocol.GUILD_GetGuildBossInspireRank, "ProtocolProcessorSceneCommunity:send_GUILD_GetGuildBossInspireRank_ErrorProcess", "is" )
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_GUILD, Protocol.GUILD_GetGuildHallOk, "ProtocolProcessorSceneCommunity:parse_GUILD_GetGuildHallOk", "isiiisiiiiivivivivsvivivivivivivivivivivivivtiviviviiivivssviiiisit")
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_GUILD, Protocol.GUILD_TotemPayOk, "ProtocolProcessorSceneCommunity:parse_GUILD_TotemPayOk", "i")
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_GUILD, Protocol.GUILD_LearnGuildSkillOk, "ProtocolProcessorSceneCommunity:parse_GUILD_LearnGuildSkillOk", "ii")
	...
	--@brief	获取公会副本boss鼓舞排行（GUILD_GetGuildBossInspireRankOk = 100）158+
	self:regProtocolCallbackFunction( Protocol.MAIN_GUILD, Protocol.GUILD_GetGuildBossInspireRankOk, "ProtocolProcessorSceneCommunity:parse_GUILD_GetGuildBossInspireRankOk", "iivivsvivivivivivivivi")
end 
...
function ProtocolProcessorSceneCommunity:send_GUILD_LearnGuildSkill(id,num )
	WZLog("send_GUILD_LearnGuildSkill")
	local sender = Protocol:getSender( Protocol.MAIN_GUILD, Protocol.GUILD_LearnGuildSkill )
	if sender==nil then WZLog("sender == nil") return end
	WZLog("学习公会技能（GUILD_LearnGuildSkill = 42）",id,num)
	sender:writeInt( id )	-- 技能ID
	sender:writeInt( num )	--学习次数
	SendProtocol(sender,false) --true:showLoading
end
...
--@brief	获取公会副本boss鼓舞排行（GUILD_GetGuildBossInspireRank = 99）
function ProtocolProcessorSceneCommunity:send_GUILD_GetGuildBossInspireRank(guildId, mapId )
	WZLog("send_GUILD_GetGuildBossInspireRank", guildId, mapId)
	local sender = Protocol:getSender( Protocol.MAIN_GUILD, Protocol.GUILD_GetGuildBossInspireRank )
	if sender==nil then WZLog("sender == nil") return end
...
function ProtocolProcessorSceneCommunity:parse_GUILD_GetGuildHallOk(guildId, guildName, guildLevel, prestige, members, desc, setting, totemLevel, schoolLevel, storeLevel, newApply, id, headId, faceId, name, level, post, donate, totalDonate, loginTime, isOnline, buyDonateTime, totemPayTime, sex, weekDonate, lastDonate, allDonate, vipLevel, maxDelCount, headColor, rmLevel, tournamentLevel, rankMatchLevel, examine, fight, donateTime, declaration, limitDonate, joinVipLevel, guildwarStage, qualification, presidentInfo, isXili, noticeStatus)
	...
	-- sex : 性别
	-- presidentInfo : 会长信息
	-- noticeStatus : 公会公告审核字段0、待审核，1、未通过，2、已通过
	WZLog("ProtocolProcessorSceneCommunity:parse_GUILD_GetGuildHallOk", guildwarStage, qualification,isXili,noticeStatus,desc)
	CacheCenter:setGuildInfo(guildId, guildName, guildLevel, prestige, members, desc, setting, totemLevel, schoolLevel, storeLevel, newApply, VectorToTable(id), VectorToTable(level), VectorToTable(post), VectorToTable(donate), VectorToTable(totalDonate), VectorToTable(buyDonateTime), VectorToTable(totemPayTime), VectorToTable(sex), VectorToTable(weekDonate), VectorToTable(lastDonate), VectorToTable(allDonate), VectorToTable(vipLevel), examine, joinVipLevel, guildwarStage, qualification, presidentInfo, isXili, noticeStatus)
...
--@brief	图腾瞻仰（GUILD_TotemPayOk = 39）
function ProtocolProcessorSceneCommunity:parse_GUILD_TotemPayOk(ntype)
	WZLog("ProtocolProcessorSceneCommunity:parse_GUILD_TotemPayOk",ntype)
	if ntype == 2 then
		MsgBoxManager:showTipBox(LocalStrings.COMMUNITYINFO250)
	else 
		MsgBoxManager:showTipBox(LocalStrings.COMMUNITYINFO102)
	end
	if SceneCommunityTotem then
		SceneCommunityTotem:learnSuccess(ntype)
	end
	if SceneCommunityMain and SceneCommunityMain.m_root then
		SceneCommunityMain:closeLoading()
	end
...
--@brief	学习公会技能（GUILD_LearnGuildSkillOk = 43）
function ProtocolProcessorSceneCommunity:parse_GUILD_LearnGuildSkillOk(id,num)
	WZLog("ProtocolProcessorSceneCommunity:parse_GUILD_LearnGuildSkillOk")
	--取消圆圈的转动效果
	MsgBoxManager:stopLoadingBoxByMsgId(SceneCommunitySkill.m_nLoadingCircleId)
	WZLog("学习公会技能（GUILD_LearnGuildSkillOk = 43）",num)
	if num > 1 then
		SceneCommunitySkill:updateUpLog(num)
	end
end
...
function ProtocolProcessorSceneCommunity:parse_GUILD_RequestGuildTaskOk(playerid, faceid, headid, level, itime, vipLevel, job, online, msgList, name, sex, idList, totalCount, currCount, headColor, refresh, taskType)
	...
	-- totalCount : 任务总量
	-- currCount : 工会当前完成量
	-- refresh : 是否有刷新权限 普通玩家返回0;可以发布任务的玩家返回1
...
--@brief	获取公会副本boss鼓舞排行（GUILD_GetGuildBossInspireRankOk = 100）158+
function ProtocolProcessorSceneCommunity:parse_GUILD_GetGuildBossInspireRankOk(mapId, totalCheerCost, playerId, name, cost, rank, faceId, headId, headColor, sex, level, vipLevel)
	-- mapId : 地图id
	-- totalCheerCost : 鼓舞总消耗
	-- playerId : 玩家ID
	-- rank : 排名
	-- name : 玩家名称
	-- faceId : 脸ID
	...
	-- vipLevel : 玩家VIP等级
	-- cost : 玩家鼓舞消耗的钻石
	WZLog("ProtocolProcessorSceneCommunity:parse_GUILD_GetGuildBossInspireRankOk")
	SceneCommunityCopy:setInspireData(VectorToTable(playerId), VectorToTable(rank), VectorToTable(name), VectorToTable(faceId), VectorToTable(headId), VectorToTable(headColor), VectorToTable(sex), VectorToTable(level), VectorToTable(vipLevel), VectorToTable(cost), totalCheerCost)
end
-------------------------------------协议错误处理方法模块--------------------------------------
...
--@brief	获取公会副本boss鼓舞排行（GUILD_GetGuildBossInspireRank = 99）错误处理函数(S->C)
--@param	nFlag:标志位
--@param	sMessage:错误信息
--@note	在此对协议错误进行相应处理
function ProtocolProcessorSceneCommunity:send_GUILD_GetGuildBossInspireRank_ErrorProcess(nFlag, sMessage)
	WZLog("ProtocolProcessorSceneCommunity:send_GUILD_GetGuildBossInspireRank_ErrorProcess")
	ProtocolErrorProcessor:errorProcess(Protocol.MAIN_GUILD, Protocol.GUILD_GetGuildBossInspireRank, nflag, sMessage)
end