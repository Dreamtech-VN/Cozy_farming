...
function ProtocolProcessorWndShop:regAll()
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_MALL, Protocol.MALL_MallList, "ProtocolProcessorWndShop:parse_MALL_MallList", "vivivsvbvbvbvnvsvivivsvivbvivsvsvivtvbviii")
    ...
    self:regProtocolCallbackFunction( Protocol.MAIN_MALL, Protocol.MALL_GetLuckDrawInfoOk, "ProtocolProcessorWndShop:parse_MALL_GetLuckDrawInfoOk", "viviviiviviiiivivivivs")
    --@brief	商城抽奖（MALL_LuckDrawResult = 34）
    self:regProtocolCallbackFunction( Protocol.MAIN_MALL, Protocol.MALL_LuckDrawResult, "ProtocolProcessorWndShop:parse_MALL_LuckDrawResult", "vivi")
    ...
    --@brief    获得商城抽奖额外奖励完成（MALL_GetMallLuckRewardOk = 55）
    self:regProtocolCallbackFunction( Protocol.MAIN_MALL, Protocol.MALL_GetMallLuckRewardOk, "ProtocolProcessorWndShop:parse_MALL_GetMallLuckRewardOk", "vivii")
    --@brief   修改商城抽奖信息（MALL_UpdateMallLuckInfo = 56）
    self:regProtocolCallbackFunction( Protocol.MAIN_MALL, Protocol.MALL_UpdateMallLuckInfo, "ProtocolProcessorWndShop:parse_MALL_UpdateMallLuckInfo", "vivivivivii")
	--@brief    新增限时兑换商品（MALL_AddTimeMall = 57）
    self:regProtocolCallbackFunction( Protocol.MAIN_MALL, Protocol.MALL_AddTimeMall, "ProtocolProcessorWndShop:parse_MALL_AddTimeMall", "iiiiiiisi")
    --@brief   更新限时兑换商品数据（MALL_UpdateTimeMallInfo = 58）
    self:regProtocolCallbackFunction( Protocol.MAIN_MALL, Protocol.MALL_UpdateTimeMallInfo, "ProtocolProcessorWndShop:parse_MALL_UpdateTimeMallInfo", "iii")
    ...
    --@brief    获得商城抽奖额外奖励（MALL_GetMallLuckReward = 54）错误处理(S->C)
    self:regProtocolCallbackFunction( Protocol.MAIN_MALL, Protocol.MALL_GetMallLuckReward, "ProtocolProcessorWndShop:send_MALL_GetMallLuckReward_ErrorProcess", "is" )
end
...
function ProtocolProcessorWndShop:send_MALL_BuyItems(count, mallId, paytype, buyType, sex, targetPlayerId, price)
	WZLog("send_MALL_BuyItems", Serialize(VectorToTable(count)), Serialize(VectorToTable(mallId)), paytype, buyType, tostring(sex), tostring(targetPlayerId), Serialize(VectorToTable(price)))
    ...
    sender:writeInts( price )   -- 价格,服务端用来判断价格是否一致
	SendProtocol(sender,false) --true:showLoading
end
...
function ProtocolProcessorWndShop:send_MALL_MallOperate(operateType, mallId, count, operatePlayerId, message, price)
...
function ProtocolProcessorWndShop:send_MALL_GetLuckDrawInfo()
...
function ProtocolProcessorWndShop:send_MALL_LuckDraw(drawNum )
...
--@brief    获得商城抽奖额外奖励（MALL_GetMallLuckReward = 54）
function ProtocolProcessorWndShop:send_MALL_GetMallLuckReward(id )
    WZLog("send_MALL_GetMallLuckReward")
    local sender = Protocol:getSender( Protocol.MAIN_MALL, Protocol.MALL_GetMallLuckReward )
    if sender==nil then WZLog("sender == nil") return end
...
-------------------------------------服务器到客户端协议回调方法模块--------------------------------------
--@brief	商城商品（MALL_MallList = 2）
function ProtocolProcessorWndShop:parse_MALL_MallList(id, itemId, itemName, isHot, isNew, isVip, discount, mainType, moneyId, floorPrice, agingPrice, limitLeave,isOnSale,transaction,ad,newad, moneyId2, suit, isPromotion, discountTime, pageSum, pageCount)
    -- id : 商城ID
    -- itemId : 物品id
    -- itemName : 物品名称
    -- isHot : 是否热卖
    ...
    CacheCenter:setShopItems(id, itemId, itemName, isHot, isNew, isVip, discount, mainType, moneyId, floorPrice, agingPrice, limitLeave,isOnSale,transaction,ad,newad, moneyId2, suit, isPromotion, discountTime, pageSum, pageCount)
end
...
--@brief	获取商城抽奖列表信息（MALL_GetLuckDrawInfoOk = 32））
function ProtocolProcessorWndShop:parse_MALL_GetLuckDrawInfoOk(itemId, itemNum, rare, luckValue, drawNum, drawPrice, type, updateTime, eNum, eId, eStatus, extTimes, extReward)
    -- itemId : 物品id
    -- itemNum : 物品数量
    -- rare : 是否稀有物品 0普通物品 1稀有物品 2珍贵物品
    -- luckValue : 当前幸运值
    -- drawNum : 抽奖次数
    -- drawPrice : 抽奖价格
    -- type : 商城抽奖类型，2粉钻，1蓝钻
    ...
    -- eNum : 抽奖总次数
    -- eId : 奖励id
    -- eStatus : 额外奖励领取状态 0不可领取 1可领取 2已领取
    -- extTimes : 额外奖励目标次数
	-- extReward : 额外奖励
	WZLog("ProtocolProcessorWndShop:parse_MALL_GetLuckDrawInfoOk",itemId:size(), itemNum:size(), rare:size(), luckValue, drawNum:size(), drawPrice:size(), type, updateTime, eNum, eId:size(), eStatus:size(), extTimes:size(), extReward:size())
	WndShopLottery:setData7(VectorToTable(itemId), VectorToTable(itemNum), VectorToTable(rare), luckValue, VectorToTable(drawNum), VectorToTable(drawPrice), type, updateTime, eNum, VectorToTable(eId), VectorToTable(eStatus), VectorToTable(extTimes), VectorToTable(extReward))
end

--@brief	商城抽奖（MALL_LuckDrawResult = 34）
function ProtocolProcessorWndShop:parse_MALL_LuckDrawResult(itemId, itemNum)
	-- itemId : 物品id
	-- itemNum : 物品数量
	WZLog("ProtocolProcessorWndShop:parse_MALL_LuckDrawResult")
	WndShopLottery:showLotteryResult(VectorToTable(itemId), VectorToTable(itemNum))
end
...
--@brief    获得商城抽奖额外奖励完成（MALL_GetMallLuckRewardOk = 55）
function ProtocolProcessorWndShop:parse_MALL_GetMallLuckRewardOk(itemId, itemNum, extId)
    WZLog("ProtocolProcessorWndShop:parse_MALL_GetMallLuckRewardOk")

    WndShopLottery:getExtendRewardOK(VectorToTable(itemId), VectorToTable(itemNum), extId)
end

--@brief    修改商城抽奖信息（MALL_UpdateMallLuckInfo = 56）
function ProtocolProcessorWndShop:parse_MALL_UpdateMallLuckInfo(itemId, itemNum, rare, drawNum, drawPrice, updateTime)
    -- itemId : 物品id
    -- itemNum : 物品数量
    -- rare : 是否稀有物品 0普通物品 1稀有物品 2珍贵物品
    -- drawNum : 抽奖次数
    -- drawPrice : 抽奖价格
    -- updateTime : 距离更新奖池时间
    WZLog("ProtocolProcessorWndShop:parse_MALL_UpdateMallLuckInfo",drawNum:size())
    WndShopLottery:setUpdateData7(VectorToTable(itemId), VectorToTable(itemNum), VectorToTable(rare), VectorToTable(drawNum), VectorToTable(drawPrice), updateTime)
end

--@brief   新增限时兑换商品（MALL_AddTimeMall = 57）
function ProtocolProcessorWndShop:parse_MALL_AddTimeMall(mallId, itemId, itemNum, limitNum, totalLimitNum, costId, costNum, mainType, endTime)
    -- mallId : 商品id
    -- itemId : 物品id
    -- itemNum : 物品数量
    -- limitNum : 剩余限购数量
    ...
    -- mainType : 大类 格式：1,2
    -- endTime : 下架时间戳
    WZLog("ProtocolProcessorWndShop:parse_MALL_AddTimeMall")
    CacheCenter:addNewGoods(mallId, itemId, itemNum, limitNum, totalLimitNum, costId, costNum, mainType, endTime)
end

--@brief   更新限时兑换商品数据（MALL_UpdateTimeMallInfo = 58）
function ProtocolProcessorWndShop:parse_MALL_UpdateTimeMallInfo(operateType, mallId, limitNum)
    -- operateType : 1->更新限购数量, 2->删除商品
    -- mallId : 商品id
    -- limitNum : 剩余限购数量

    WZLog("ProtocolProcessorWndShop:parse_MALL_UpdateTimeMallInfo")
    CacheCenter:updateExchangeGoodsInfo(operateType, mallId, limitNum, true)
end
-------------------------------------协议错误处理方法模块--------------------------------------
...
function ProtocolProcessorWndShop:send_MALL_BuyItems_ErrorProcess(nFlag, sMessage)
    ...
    if WndShop.m_root then
        CacheCenter:refreshShopItems(WndShop.getShopItemsListCallBack,WndShop)
    end
end
...
--@brief    获得商城抽奖额外奖励（MALL_GetMallLuckReward = 54）错误处理函数(S->C)
--@param    nFlag:标志位
--@param    sMessage:错误信息
--@note 在此对协议错误进行相应处理
function ProtocolProcessorWndShop:send_MALL_GetMallLuckReward_ErrorProcess(nFlag, sMessage)
    WZLog("ProtocolProcessorWndShop:send_MALL_GetMallLuckReward_ErrorProcess")
    ProtocolErrorProcessor:errorProcess(Protocol.MAIN_MALL, Protocol.MALL_GetMallLuckReward, nflag, sMessage)
end
