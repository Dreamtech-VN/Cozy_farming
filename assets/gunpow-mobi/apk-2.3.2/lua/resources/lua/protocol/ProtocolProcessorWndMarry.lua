...
function ProtocolProcessorWndMarry:regAll()
	--@brief	获取婚姻状况成功
	self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WEDDING_GetMaritalStatusOK, "ProtocolProcessorWndMarry:parse_WEDDING_GetMaritalStatusOK", "tistiii")

	--@brief	获取求婚信
	self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WEDDING_GetMarryRecordList, "ProtocolProcessorWndMarry:parse_WEDDING_GetMarryRecordList", "vivsvtvivivivivivivi")
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WEDDING_GetMarryInfoOK, "ProtocolProcessorWndMarry:parse_WEDDING_GetMarryInfoOK", "ssiniiiiiiiiiiiiiiiiii")
	...
	self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WWEDDING_GetRemoveEngageStatusOk, "ProtocolProcessorWndMarry:parse_WWEDDING_GetRemoveEngageStatusOk", "iivivivtvsvivivivivstvii")

	--@brief	离婚孩子分配结果（WEDDING_GetDivorceResultOk = 95）
	self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WEDDING_GetDivorceResultOk, "ProtocolProcessorWndMarry:parse_WEDDING_GetDivorceResultOk", "vivsvivsvtvivivivivsvi")
	--@brief	获取孩子分配信息（WEDDING_GetAllotChildInfoOk = 96）
	self:regProtocolCallbackFunction( Protocol.MAIN_WEDDING, Protocol.WEDDING_GetAllotChildInfoOk, "ProtocolProcessorWndMarry:parse_WEDDING_GetAllotChildInfoOk", "iiivivtvsvivivivivivsvi")
...
function ProtocolProcessorWndMarry:parse_WEDDING_SendCardOK()
	...
	WndMarryBetrothed:getWeddingSendCardOK()
end
...
-------------------------------------服务器到客户端协议回调方法模块--------------------------------------
--@brief	获取婚姻状况成功
function ProtocolProcessorWndMarry:parse_WEDDING_GetMaritalStatusOK(marryStatus, coupleId, coupleName, weddingType,wedTime, divorceTime, divorceCDTime)
	...
	-- divorceTime : 离婚冷却时间戳
	-- divorceCDTime：当前是已婚状态的话，返回如果离婚的话，下次结婚所需的冷却时间
	WZLog("ProtocolProcessorWndMarry:parse_WEDDING_GetMaritalStatusOK",marryStatus, coupleId, coupleName, weddingType, wedTime, divorceTime, divorceCDTime)

	 --关闭加载框
	WZLog("WndMarryManager.m_nLoadingId = ",WndMarryManager.m_nLoadingId)
	WndMarryManager:closeLoading()
	WndMarryManager:getMarryStatusOK(marryStatus, coupleId, coupleName, weddingType, wedTime, divorceTime, divorceCDTime)
end
...
--@brief	获取求婚信
function ProtocolProcessorWndMarry:parse_WEDDING_GetMarryRecordList(marryRecordId, sendPlayerName, marryType,sendTime,headIds,faceIds,levels,playerId,headColors, serverId)
	-- marryMailId : 求婚信的Id
	-- sendPlayerName : 求婚人名称
	-- marryType : 类型【1鲜花，2戒指，3银行，4钥匙】
	WZLog("ProtocolProcessorWndMarry:parse_WEDDING_GetMarryRecordList")
    WndMarryManager:closeLoading()
    WndMarryManager:getMarryLetterOK(marryRecordId, sendPlayerName, marryType,sendTime,headIds,faceIds,levels,playerId,headColors, serverId)
...
--@brief	夫妻关系结果（WEDDING_GetMarryInfoOK = 45）
function ProtocolProcessorWndMarry:parse_WEDDING_GetMarryInfoOK(manName, womanName, loveExp, loveLevel, giftNum, manHeadId, manFaceId, manBodyId, manWingId, womanHeadId, womanFaceId, womanBodyId, womanWingId, partnerId, manHeadColor, manBodyColor, womanHeadColor, womanBodyColor, manServerId, womanServerId, manSkillEndTime, womanSkillEndTime)
	-- manName : 男方名称
	-- womanName : 女方名称
	-- loveExp : 恩爱经验
	-- loveLevel : 恩爱等级
	...
	-- womanWingId : 翅膀id
	-- partnerId：伴侣ID
	-- manSkillEndTime : 男方技能失效时间
	-- womanSkillEndTime : 女方技能失效时间
	WZLog("ProtocolProcessorWndMarry:parse_WEDDING_GetMarryInfoOK =", manSkillEndTime, womanSkillEndTime)
	WndMarryManager:closeLoading()
	if SceneMarryWedding.m_root then
		SceneMarryWedding:receivePropInfo(manName,womanName,loveExp,loveLevel,giftNum,manHeadId, manFaceId, manBodyId, manWingId, womanHeadId, womanFaceId, womanBodyId, womanWingId,partnerId,manHeadColor,manBodyColor,womanHeadColor,womanBodyColor, manServerId, womanServerId, manSkillEndTime, womanSkillEndTime)
	end
	if SceneMarryCopy.m_root then
		SceneMarryCopy:receivePropInfo(manName,womanName,loveExp,loveLevel,giftNum,manHeadId, manFaceId, manBodyId, manWingId, womanHeadId, womanFaceId, womanBodyId, womanWingId,partnerId,manHeadColor,manBodyColor,womanHeadColor,womanBodyColor, manServerId, womanServerId, manSkillEndTime, womanSkillEndTime)
	end
...
--@brief	获取离婚信息（WWEDDING_GetRemoveEngageStatusOk = 94）
function ProtocolProcessorWndMarry:parse_WWEDDING_GetRemoveEngageStatusOk(fatherDevote, motherDevote, ownerId, childId, sex, childName, headId, faceId, level, childFight, childProp, needPay, headEffectId, divorceCDTime)
	-- fatherDevote : 父亲贡献度
	-- motherDevote : 母亲贡献度
	-- ownerId : 孩子所属玩家Id
	-- childId : 小孩Id
	...
	-- childFight : 小孩战力
	-- childProp : 小孩属性
	-- needPay : 是否需要支付离婚费用 0不需要 1需要支付 
	-- headEffectId : 小孩头像框Id
	-- divorceCDTime : 离婚后需要等待多少时间后才能再婚
	WZLog("ProtocolProcessorWndMarry:parse_WWEDDING_GetRemoveEngageStatusOk")
	WndMarryManager:closeLoading()
	SceneMarryWedding:getKidDataOK(fatherDevote, motherDevote, VectorToTable(ownerId), VectorToTable(childId), VectorToTable(sex), VectorToTable(childName), VectorToTable(headId), VectorToTable(faceId), VectorToTable(level), VectorToTable(childFight), VectorToTable(childProp), needPay, VectorToTable(headEffectId), divorceCDTime)
end

--@brief	离婚孩子分配结果（WEDDING_GetDivorceResultOk = 95）
function ProtocolProcessorWndMarry:parse_WEDDING_GetDivorceResultOk(playerId, playerName, childId, childName, sex, headId, faceId, level, childFight, childProp, headEffectId)
	-- playerId : 玩家Id
	-- playerName : 玩家名字
	-- childId : 小孩Id
	-- sex : 小孩性别
	...
	WndParentsCare:receiveDivorceResult(VectorToTable(playerId), VectorToTable(playerName), VectorToTable(childId), VectorToTable(childName), VectorToTable(sex), VectorToTable(headId), VectorToTable(faceId), VectorToTable(level), VectorToTable(childFight), VectorToTable(childProp), VectorToTable(headEffectId))
end

--@brief	获取孩子分配信息（WEDDING_GetAllotChildInfoOk = 96）
function ProtocolProcessorWndMarry:parse_WEDDING_GetAllotChildInfoOk(fatherDevote, motherDevote, otherSelectChildId, childId, sex, childName, headId, faceId, level, ownerId, childFight, childProp, headEffectId)
	-- fatherDevote : 父亲贡献度
	-- motherDevote : 母亲贡献度
	-- ownerId : 孩子所属玩家Id 
	...
	WndParentsCare:receiveDivorceData(fatherDevote, motherDevote, VectorToTable(ownerId), VectorToTable(childId), VectorToTable(sex), VectorToTable(childName), VectorToTable(headId), VectorToTable(faceId), VectorToTable(level), VectorToTable(otherSelectChildId), VectorToTable(childFight), VectorToTable(childProp), VectorToTable(headEffectId))
end
-------------------------------------协议错误处理方法模块--------------------------------------
