...
function ProtocolProcessorWndLeague:parse_HERO_ReadyFightOK(teamId, teamName, photoURL, declaration, playerId, faceId, headId, bodyId, wingId, captain, score, fightNum, winNum, readyPlayerId, watchPlayerId, rank, sex, pet, status, timeMes, openTime, level, name, lastFight, itemId, extraInfo, kictNum, outNum, picStatue, canFight, readyed, fight, viceCaptain, headColor, bodyColor, mentoringStr, coupleStr, chumStr, coupleNum, chumNum, mentoringNum, matchLevel, matchscore, joinTimes, winTimes, continuousWinTimes)
	...
	-- watchPlayerId : 观战成员
	-- rank : 排名
	WZLog("ProtocolProcessorWndLeague:parse_HERO_ReadyFightOK",
		"\nteamId = ",Serialize(VectorToTable(teamId)),
		"\nteamName = ",Serialize((teamName)),
		"\nphotoURL = ",Serialize((photoURL)),
		...
		"\ncaptain = ",Serialize(VectorToTable(captain)),
		"\nscore = ",Serialize(VectorToTable(score)),
		"\nfightNum = ",Serialize(VectorToTable(fightNum)),
		"\nwinNum = ",Serialize(VectorToTable(winNum)),
		"\nreadyPlayerId = ",Serialize(VectorToTable(readyPlayerId)),
		"\nwatchPlayerId = ",Serialize(VectorToTable(watchPlayerId)),
		...
		"\ntimeMes = ",Serialize((timeMes)),
		"\nopenTime = ",Serialize((openTime)),
		"\nlevel = ",Serialize(VectorToTable(level)),
		"\nname = ",Serialize(VectorToTable(name)),
		"\nlastFight = ",Serialize(VectorToTable(lastFight)),
		"\nitemId = ",Serialize(VectorToTable(itemId)),
		"\nextraInfo = ",Serialize(VectorToTable(extraInfo)),
		"\nkictNum = ",Serialize(VectorToTable(kictNum)),
		"\noutNum = ",Serialize(VectorToTable(outNum)),
		"\npicStatue = ",Serialize(VectorToTable(picStatue)),
		"\ncanFight = ",Serialize((canFight)),
		"\nreadyed = ",Serialize(VectorToTable(readyed)),
		"\nfight = ",Serialize(VectorToTable(fight)),
		"\nviceCaptain = ",Serialize(VectorToTable(viceCaptain)),
		"\nheadColor = ",Serialize(VectorToTable(headColor)),
		"\nbodyColor = ",Serialize(VectorToTable(bodyColor)),
		"\nmentoringStr = ",Serialize((mentoringStr)),
		"\ncoupleStr = ",Serialize((coupleStr)),
		"\nchumStr = ",Serialize((chumStr)),
		"\ncoupleNum = ",Serialize((coupleNum)),
		"\nchumNum = ",Serialize((chumNum)),
		"\nmentoringNum = ",Serialize((mentoringNum)),
...
--@brief	32强战况（HERO_TeamSelectRankOk = 76））
function ProtocolProcessorWndLeague:parse_HERO_TeamSelectRankOk(firstStartTime, firstEndTime, secondStartTime, secondEndTime, thirdStartTime, thirdEndTime, teamId, successStatus, url)
	...
	-- teamId : 队伍id
	-- successStatus : 队伍三轮胜负情况 0-失败 1-胜利 2-弃权 最后一位0-淘汰 1-晋级
	-- url : 队伍图标
...
function ProtocolProcessorWndLeague:parse_HERO_InvitationReadyOk(msg)
	...
	if SceneRoom.m_root == nil and SceneBossRoom.m_root == nil and SceneBattle.m_root == nil and SceneBattleLoading.m_root == nil and not WndChat.m_bRecording and not SceneHall:getMatchState() and not WndTowerScroll.m_root and WndDoubleTowerRoom.m_root == nil  then 
