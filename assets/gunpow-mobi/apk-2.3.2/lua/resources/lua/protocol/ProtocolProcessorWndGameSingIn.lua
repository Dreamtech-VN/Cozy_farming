...
function ProtocolProcessorWndGameSingIn:regAll()
	...
	--@brief	获取签到状态OK
	self:regProtocolCallbackFunction( Protocol.MAIN_TASK, Protocol.TASK_GetSignStatusOk, "ProtocolProcessorWndGameSingIn:parse_TASK_GetSignStatusOk", "ibblbiivivi")
	...
	--@brief	领取累计签到奖励错误处理(S->C)
	self:regProtocolCallbackFunction( Protocol.MAIN_TASK, Protocol.TASK_ReceiveSignTotaReward, "ProtocolProcessorWndGameSingIn:send_TASK_ReceiveSignTotaReward_ErrorProcess", "is" )
	--@brief	领取累计签到奖励返回
	self:regProtocolCallbackFunction( Protocol.MAIN_TASK, Protocol.TASK_TASK_ReceiveSignTotaRewardOk, "ProtocolProcessorWndGameSingIn:parse_TASK_TASK_ReceiveSignTotaRewardOk", "ii")
...
--@brief	领取累计签到奖励
function ProtocolProcessorWndGameSingIn:send_TASK_ReceiveSignTotaReward(day )
	WZLog("send_TASK_ReceiveSignTotaReward")
	local sender = Protocol:getSender( Protocol.MAIN_TASK, Protocol.TASK_ReceiveSignTotaReward )
	if sender==nil then WZLog("sender == nil") return end

	sender:writeInt( day )	-- 点击的宝箱累计天数
	SendProtocol(sender,false) --true:showLoading
end
...
--@brief	获取签到状态OK
function ProtocolProcessorWndGameSingIn:parse_TASK_GetSignStatusOk(days, sign, vipSign, nCurrentTime, monthReward, reissueTimes, todaySignNum, totaRewardDays, totaReWardStatus)
	-- days : 累计签到天数
	...
	if WndGameSingIn.m_root ~= nil then 
		WndGameSingIn:GetSingInDataOK( days, sign ,vipSign , nCurrentTime, monthReward, reissueTimes, todaySignNum, VectorToTable(totaRewardDays), VectorToTable(totaReWardStatus))
	else 
...
--@brief	领取累计签到奖励返回
function ProtocolProcessorWndGameSingIn:parse_TASK_TASK_ReceiveSignTotaRewardOk(day, status)
	-- day : 点击的宝箱累计天数
	-- status : 0 成功 1 不可领取 2 已领取
	WZLog("ProtocolProcessorWndGameSingIn:parse_TASK_TASK_ReceiveSignTotaRewardOk")
	if status == 0 then
		GlobalGame:getBattleEventDispatcher():Dispatch("GAMESINGIN_TOTLE_GET", day)
	end
end
...
--@brief	领取累计签到奖励错误处理函数(S->C)
--@param	nFlag:标志位
--@param	sMessage:错误信息
--@note	在此对协议错误进行相应处理
function ProtocolProcessorWndGameSingIn:send_TASK_ReceiveSignTotaReward_ErrorProcess(nFlag, sMessage)
	WZLog("ProtocolProcessorWndGameSingIn:send_TASK_ReceiveSignTotaReward_ErrorProcess")
	ProtocolErrorProcessor:errorProcess(Protocol.MAIN_TASK, Protocol.TASK_ReceiveSignTotaReward, nflag, sMessage)
end