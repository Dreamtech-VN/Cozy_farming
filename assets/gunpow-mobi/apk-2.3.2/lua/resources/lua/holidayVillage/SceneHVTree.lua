...
--@note		在这里做场景进入前的准备工作
function SceneHVTree:onEnter(element)
	self.m_root = element

	ChangeChatChannel(Chat_Channel_HOLIDAYVILLAGE)
	ProtocolProcessorHolidayVillage:regAll()
	self:_initFieldData()
	GlobalGame:getGameEventDispathcer():Add(HolidayVEvent.HolidayVEvent_GetDivineTreeInfo,self._getDivineInfo,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Add(HolidayVEvent.HolidayVEvent_ChooseFruit,self._chooseFruit,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Add(HolidayVEvent.HolidayVEvent_SpeedFruit,self._speedFruit,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Add(HolidayVEvent.HolidayVEvent_HostInfo,self.setHostData,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Add(HolidayVEvent.HolidayVEvent_AllPits,self.setFieldAndPlantData,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Add(HolidayVEvent.HolidayVEvent_PitOpResult,self.operateResult,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Add(HolidayVillageEvent.HolidayVillageEvent_Visitors,self.setVisitorsData,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Add(HolidayVillageEvent.HolidayVillageEvent_Store,self.setStoreData,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Add(HolidayVEvent.HolidayVEvent_WishResult,self.wishResult,SceneHVTree)
end

--@brief	退出场景时被调用的函数
--@param	element:表绑定的UI节点引用
--@note		在这里做场景退出前的清理工作
function SceneHVTree:onExit(element)
	GlobalGame:getGameEventDispathcer():Remove(HolidayVEvent.HolidayVEvent_GetDivineTreeInfo,self._getDivineInfo,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Remove(HolidayVEvent.HolidayVEvent_ChooseFruit,self._chooseFruit,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Remove(HolidayVEvent.HolidayVEvent_SpeedFruit,self._speedFruit,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Remove(HolidayVEvent.HolidayVEvent_HostInfo,self.setHostData,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Remove(HolidayVEvent.HolidayVEvent_AllPits,self.setFieldAndPlantData,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Remove(HolidayVEvent.HolidayVEvent_PitOpResult,self.operateResult,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Remove(HolidayVillageEvent.HolidayVillageEvent_Visitors,self.setVisitorsData,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Remove(HolidayVillageEvent.HolidayVillageEvent_Store,self.setStoreData,SceneHVTree)
    GlobalGame:getGameEventDispathcer():Remove(HolidayVEvent.HolidayVEvent_WishResult,self.wishResult,SceneHVTree)
    if self.m_root then 
...
function SceneHVTree:onEnterTransitionDidFinish(element)
	self.m_root:enableSchedule("_setSceneState", 10)
	self:_setSceneState()
...
--@brief 关闭界面
function SceneHVTree:onClickClose()
    WZLog("SceneHVTree:onClickClose")
	SoundManager:playEffectSound(SoundDefine.E_S_CLOSE_WIN)
	if self.m_tHostInfo then
        WZLog("SceneHVTree:onExit", self.m_tHostInfo.playerId, self.m_nPlayerId) 
...
--@brief    触摸开始回调
function SceneHVTree:onTouchBegin(element, pt, nIdx)
    -- body
...
--@brief    点击建筑物回调
function SceneHVTree:onClickBuildingCallBack(element, tCell, tData)
    -- body
...
--@brief    挖坑回调
function SceneHVTree:digCallBack(tGridData)
    WZLog("SceneHVTree:digCallBack", self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1)
...
--@brief    播种回调
function SceneHVTree:sowCallBack(tGridData)
    WZLog("SceneHVTree:sowCallBack", self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1, self.m_tSeedData.basicInfo.id)
...
--@brief    浇水回调
function SceneHVTree:waterCallBack(tGridData)
    WZLog("SceneHVTree:waterCallBack", self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1)
    self:_createOperateAni(self.m_nOperateType, tGridData)
    ProtocolProcessorHolidayVillage:send_HOLIDAYVILLAGE_PitOp(self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1, 0)
end

--@brief    捕捉回调
function SceneHVTree:catchCallBack(tGridData)
    WZLog("SceneHVTree:catchCallBack", self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1)
    self:_createOperateAni(self.m_nOperateType, tGridData)
    ProtocolProcessorHolidayVillage:send_HOLIDAYVILLAGE_PitOp(self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1, 0)
end

--@brief    收获/偷取回调
function SceneHVTree:collectCallBack(tGridData)
    WZLog("SceneHVTree:collectCallBack", self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1)
    self:_createOperateAni(self.m_nOperateType)
    ProtocolProcessorHolidayVillage:send_HOLIDAYVILLAGE_PitOp(self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1, 0)
end

--@brief    施肥回调
function SceneHVTree:fertilizerCallBack(tGridData)
    WZLog("SceneHVTree:fertilizerCallBack", self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1, self.m_tSeedData.basicInfo.id)
    self:_createOperateAni(self.m_nOperateType)
    ProtocolProcessorHolidayVillage:send_HOLIDAYVILLAGE_PitOp(self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1, self.m_tSeedData.basicInfo.id)
end

--@brief    扩展回调
function SceneHVTree:extendFieldCallBack(tGridData)
    WZLog("SceneHVTree:extendFieldCallBack", self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1)
    self:_createOperateAni(self.m_nOperateType)
    ProtocolProcessorHolidayVillage:send_HOLIDAYVILLAGE_PitOp(self.m_nPlayerId, self.m_nOperateType, tGridData.fieldId - 1, 0)
end

--@brief    点击度假村土坑按钮回调
function SceneHVTree:onClickHVBtn(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)

    local nTag = element:getTag()
    WZLog("SceneHVTree:onClickHVBtn", nTag)
    WndHVOperate:onClickOperateCallBack(nTag, element)
    WndHVOperate:_hideOperateBtn()
end

--@brief    点击精灵神树回调
function SceneHVTree:onClickTree(element)
    SoundManager:playEffectSound(SoundDefine.E_S_CLICK_BTN)

    WndHVDivineTree:showInterface()
end
-------------------------------------公有方法模块End----------------------------------------
...
-------------------------------------私有方法模块Begin--------------------------------------
--@brief     添加顶部货币栏
function SceneHVTree:_addTop()
    -- body
    local celElement, tNewObj = CellTopHandle:createElement()
    tNewObj:setTopData("ui/common/common_icon_sssl.png", SceneHVTree, SceneHVTree.onClickClose, true, nil, false,nil, {goldType = 18}, false, false)
...
function SceneHVTree:_setSceneState()
    ...
    local imgBg = GetElement(self.m_root, "imgBg_hvTreeScene", WZUIImage)
    local imgFg = GetElement(self.m_root, "imgFg_hvTreeScene", WZUIImage)
    local spineLake = GetElement(self.m_root, "spineLake_hvTreeScene", WZUISpine)

    if self.m_nSceneState == 0 then 
        if imgBg then 
            imgBg:setFile("ui/holidayVillage/djc_swty_bg_01.png")
        end
        if imgFg then 
            imgFg:setFile("ui/holidayVillage/djc_swty_bg_01_a.png")
        end
        if spineLake then 
            spineLake:play("wait1", true)
        end
    elseif self.m_nSceneState == 1 then 
        if imgBg then 
            imgBg:setFile("ui/holidayVillage/djc_swty_bg_01_1.png")
        end
        if imgFg then 
            imgFg:setFile("ui/holidayVillage/djc_swty_bg_01_a_1.png")
        end
        if spineLake then 
            spineLake:play("wait2", true)
        end
    end
end

--@brief    显示操作窗口
function SceneHVTree:createOperateWin()
    -- body
    --添加操作窗口
    local multiTouchPanel = GetElement(self.m_root, "multiTouchPanel_SceneHVTree", WZUIMultiTouchPanel)
    local element = WndHVOperate:createElement()
    WndHVOperate:setWinType(1)
    multiTouchPanel:addChild(element)
end

--@brief 	初始化数据
function SceneHVTree:_initData()
	self.m_tIsRoleMove = {}
...
function SceneHVTree:createAni()
	-- body
	--显示夫妻形象
    if self:isMyHolidayVillage() then 
	   self:createHostRole()
...
--@brief    延时创建建筑
function SceneHVTree:delayCreateBuilding()
...
--@brief    创建家园建筑
function SceneHVTree:_createBuilding()
    -- body
...
--@brief    创建某一建筑
function SceneHVTree:updateBuildingSchedule(element,dt)
...
--@brief    创建某一建筑
function SceneHVTree:_createOneBuilding(indexX, indexY,con)
    -- body
    local conForBuilding = con 

    if self.m_tGridList[indexX][indexY].configId > 0 then
    	WZLog("SceneHVTree:_createOneBuilding", self.m_tGridList[indexX][indexY].configId, indexX, indexY)
        ...
        local celElement, tNewObj = CellHVBuilding:createElement(1)
...
--@brief    初始化家园底图
function SceneHVTree:_initMap()
    -- body
    WZLog("SceneHVTree:_initMap")
    local conForMap = GetElement(self.m_root, "conForMap_hvTreeScene", WZUIContainer)
...
--@brief 	创建度假村主人的形象
function SceneHVTree:createHostRole(bRemove, hostData)
	...
	if self:isMyHolidayVillage() then
    	...
    	if element and tNewObj then
            ...
            element:setZOrder((tGridBySex[1] - 1) + (HVTREE_ROW - tGridBySex[2]) * HVTREE_ROW)
...
--@brief    创建拜访者形象
function SceneHVTree:createVisiting()
    -- body
    if self.m_root == nil then return end
...
--@brief    创建一个访客玩家
function SceneHVTree:_createOneVisitor(i, visitorData)
    local conForBuilding = GetElement(self.m_root, "conForBuilding_hvTreeScene", WZUIContainer)
    local tVisitorGrid = {}
...
--@brief    创建精灵形象
function SceneHVTree:createSpirit()
    if self.m_root == nil then return end

    WZLog("SceneHVTree:createSpirit", Serialize(self.m_tSpiritList))
...
--@brief    创建一个精灵
function SceneHVTree:_createOneSpirit(i, visitorData)
    local conForBuilding = GetElement(self.m_root, "conForBuilding_hvTreeScene", WZUIContainer)
    local tSpiritGrid = {}
...
--@brief    角色小孩移动
function SceneHVTree:roleAndKidMove(element, delta)
    self:_setSceneState()
...
--@brief    移动计算
function SceneHVTree:_addBySpeed(tCell, step, tGridData, tNextGridData, roleIndex)
    -- body
...
--    WZLog("SceneHVTree:_addBySpeed 000", originPositionX, originPositionY)
...
--    WZLog("SceneHVTree:_addBySpeed 000", nCurPositionX, nCurPositionY)

    tCell.m_root:setAbsPosition(GlobalMethod:ccp(nCurPositionX, nCurPositionY))
end

--@brief    创建种子图标
function SceneHVTree:_createOperateIcon(operateType, tData)
    local conForBuilding = GetElement(self.m_root, "conForBuilding_hvTreeScene", WZUIContainer)
    local nTag = 99999
    local element = conForBuilding:getChildByTag(nTag)
    if not element then 
...
--@brief    创建可用肥料列表
function SceneHVTree:_createFertilizerList(operateType, bAdd, gridData)
    local conForBuilding = GetElement(self.m_root, "conForBuilding_hvTreeScene", WZUIContainer)
    local nTag = 99990
    local element = conForBuilding:getChildByTag(nTag)
    if not bAdd then 
    ...
    if not element and bAdd then 
        ...
        element:setTag(nTag)
        WndHVFertilizerList:setData(gridData, SceneHVTree)
...
function SceneHVTree:_createOperateAni(operateType, tGridData)
    local conForBuilding = GetElement(self.m_root, "conForBuilding_hvTreeScene", WZUIContainer)
    local nTag = 99991
    local element = conForBuilding:getChildByTag(nTag)
    if not element then 
...
--@brief    检测触摸点
function SceneHVTree:checkPointInBtn(pt)
    if self.m_root == nil then return end
    local conForBuilding = GetElement(self.m_root, "conForBuilding_hvTreeScene", WZUIContainer)
    local nTag = 99989
    local element = conForBuilding:getChildByTag(nTag)
...
--@brief    自动计算作物的成长剩余时间
function SceneHVTree:_updatePlantTime(delta)
    if self.m_root == nil then return end 
            ...
            if configId > 0 and configId < 20 then 
                ...
                if plantId > 0 and bIsWater then 
                    if plantStatus < PLANT_STATUS.MATURITY and nCurTime >= plantGroupTime and plantGroupTime > 0 then 
                        ...
                        --更新地块的作物状态
                        local nTag = indexX * HVTREE_ROW + indexY + 1
                        local conForBuilding = GetElement(self.m_root, "conForBuilding_hvTreeScene", WZUIContainer)
                        if conForBuilding:getChildByTag(nTag) then 
                            local element = conForBuilding:getChildByTag(nTag)
                            element = WZUIContainer:luaTo(element)
...
--@brief    更新待扩展地块牌子上的文字
function SceneHVTree:_updateExtendFieldWords(hvLevel)
...
function SceneHVTree:_updatePlantBtnAndDetail(fieldData)
...
--@brief    显示自己和伴侣互动动画
function SceneHVTree:showCoupleAnimation()
    local nCoupleId = 0
...
--@brief    更新精灵睡眠状态
function SceneHVTree:updateSpiritSleep(nIdx)
