...
HVMAP_SIZEY = 22
HVTREE_ROW = 56
HVTREE_REAL_WIDTH = math.ceil(math.sqrt(HVMAP_SIZEX*HVMAP_SIZEX/4 + HVMAP_SIZEY * HVMAP_SIZEY/4))
HVTREE_WIDTH = HVMAP_SIZEX * HVTREE_ROW
HVTREE_HEIGHT = HVMAP_SIZEY * HVTREE_ROW
...
function SceneHVTree:_init()
	...
	self.m_nFieldPosOffset = 15 		--地块索引偏移
	self.m_tWishData = nil 				--今日已许愿次数，上一次许愿时间戳
end


--@brief	反初始化表的成员变量
--@note		在退出场景时回调的onExit函数里面必须调用本函数
function SceneHVTree:_unInit()
	self.m_root = nil
	self.m_topCellLua = nil 
	self.m_tDivineTreeInfo = nil 
	self.m_nSceneState = nil 			--0=白天；1=傍晚
	self.m_tHostInfo = nil 
	self.m_nPlayerId = nil 				--农场主Id
	self.m_tHostInfo = nil 				--农场主信息
	self.m_tVisitorsList = nil 			--访问者信息
	...
	self.m_scheduleId2 = nil  			--延时显示偷取奖励
	self.m_scheduleId3 = nil  			--延时显示松土奖励
	self.m_tHarvestField = nil 			--收获的土坑数据,用于获取地块，展示收获后的固定奖励
	self.m_tOperateReward = nil 		--操作奖励
	self.m_nSleepValue = nil 			--精灵饱食度少于这个值播放睡觉动作
	self.m_tSleepPosition = nil			--精灵睡眠时位置
	self.m_tCellFieldObjList = nil		--存放土坑水坑对象列表
	self.m_nFieldPosOffset = nil  
	self.m_tWishData = nil 				--今日已许愿次数，上一次许愿时间戳
end
...
--@note		请仅用此方法创建场景
function SceneHVTree:createElement()
	local element = WZUISystem:getInstance():createElement("SceneHVTree")
	assert(element, "SceneHVTree create element failed!")
	self:_init()
	return element
end

--@brief 	外部接口
function SceneHVTree:showInterface()
	local nTempId = playerId or CacheCenter:getPlayerInfo().id 
	if self.m_root and self.m_nPlayerId == nTempId then return end 
	if self.m_root then 
		WZLog("SceneHVTree:showInterface", self.m_nPlayerId)
		ProtocolProcessorHolidayVillage:send_HOLIDAYVILLAGE_HolidayVillageOp(false, self.m_nPlayerId)
	end

	local scene = SceneHVTree:createElement()
	if scene then 
...
function SceneHVTree:setHostData(hostPlayerId, hostName, hostHeadId, hostFaceId, hostHeadColor, hostHeadEffectId, hostSex, hostVipLevel, hostServerId, hostLvel, hvLevel, hvExp, hvCurEnergy, achieId, hvCoolValue, houseId, waterWheelId)
	--农场主信息
	self.m_tHostInfo = {}
	...
	WZLog("SceneHVTree:setHostData", Serialize(self.m_tHostInfo))
	WndHVOperate:_showPlayerInfo()
end

--@brief 	设置土坑、作物数据
--@param 	synType : 同步类型，0-全部，1-增，2-更新，3-删
function SceneHVTree:setFieldAndPlantData(playerId, synType,fieldPos, fieldStatus, fieldLv, plantId, plantStatus, plantPests, plantGroupTime, leftNum, extraInfo, totalNum, refines, waterings, digs, exps, fertilizerIds, reduceNumes, opType, flowerpotIds)
	WZLog("SceneHVTree:setFieldAndPlantData", self.m_nPlayerId, playerId, synType, Serialize(fieldPos), Serialize(fieldStatus), Serialize(fieldLv), Serialize(extraInfo), opType)
	if self.m_nPlayerId == playerId then 
		WZLog("SceneHVTree:setFieldAndPlantData Zero", SystemTime:getServerTime())
		if synType == 0 then 
			for i = 1, #fieldPos do
				WZLog("SceneHVTree:setFieldAndPlantData one", plantId[i], plantStatus[i], plantGroupTime[i], plantPests[i], leftNum[i], totalNum[i], exps[i], fertilizerIds[i], waterings[i], digs[i], reduceNumes[i])
		...
		elseif synType == 2 then 
			for i = 1, #fieldPos do
				WZLog("SceneHVTree:setFieldAndPlantData two", plantId[i], plantStatus[i], plantGroupTime[i], plantPests[i], leftNum[i], totalNum[i], exps[i], fertilizerIds[i], waterings[i], digs[i], reduceNumes[i])
				if fieldPos[i] >= 15 then 
					self:setOneBuildingData(plantId[i], plantStatus[i], plantPests[i], fieldStatus[i], fieldPos[i] + 1, fieldLv[i], plantGroupTime[i], leftNum[i], extraInfo[i], totalNum[i], refines[i], waterings[i], digs[i], exps[i], fertilizerIds[i], reduceNumes[i], flowerpotIds[i])
...
--@brief 	获取度假村主人信息
function SceneHVTree:getHostInfo()
	return self.m_tHostInfo
end

--@brief 	判断触摸点是否在建筑内
function SceneHVTree:judgePtInBuilding(tData)
	-- body
	...
	WZLog("SceneHVTree:judgePtInBuilding", self.m_bIsPtInBuilding)
end

--@brief 	设置某一格数据
function SceneHVTree:setOneBuildingData(plantId, plantStatus, plantPests, fieldStatus, fieldId, fieldLv, plantGroupTime, leftNum, extraInfo, totalNum, refine, waterings, digs, exp, fertilizerId, reduceNumes, flowerpotId)
	-- body
...
function SceneHVTree:setNextExtendField()
	--初始化土地块数据
	local bIsSet = false 
	local bIsSet2 = false 
	local nIndex1 = 999 
	local nIndex2 = 999 
	for i, configId in pairs(self.m_tConfigId) do
	...
	WZLog("SceneHVTree:setNextExtendField", nIndex1, nIndex2)
	for i, configId in pairs(self.m_tConfigId) do
		local indexX = self.m_tFieldPos[i][1]
		local indexY = self.m_tFieldPos[i][2]
		if configId == 1 then 
			if self.m_tGridList[indexX + 1][indexY + 1].fieldStatus <= 0 and i == nIndex1 then 
				if not bIsSet then 
		...
		elseif configId == 2 then 
			if self.m_tGridList[indexX + 1][indexY + 1].fieldStatus <= 0 and i == nIndex2 then 
				if not bIsSet2 then 
					self.m_tGridList[indexX + 1][indexY + 1].fieldStatus = 0
					bIsSet2 = true 
				end
...
--@brief 	获取仓库数据
function SceneHVTree:setStoreData(warehouseType, itemIds, nums, synType)
...
function SceneHVTree:getItemCountByItemId(id)
	local nNum = 0 
...
--@brief 	根据仓库中的肥料数据
function SceneHVTree:getFertilizerData()
	local tData = {} 
...
--@brief 	获取选中的土坑信息
function SceneHVTree:getClickFieldData()
	return self.m_clickInfo
end

--@brief 	获取度假村主人Id
function SceneHVTree:getHostId()
	return self.m_nPlayerId
end

--@brief 	获取场景状态
function SceneHVTree:getSceneState()
	return self.m_nSceneState
end

--@brief 	设置操作类型
function SceneHVTree:setOperateType(operateType, seedData, fieldData)
	self.m_nOperateType = operateType
	if operateType == 1 then --播种
		self.m_tSeedData = seedData
		SceneHVTree:sowCallBack(self.m_clickInfo.tData)
	...
	elseif operateType == 2 then --肥料
	 	...
	 	if fieldData.plantId > 0 and fieldData.bIsWater then 
            SceneHVTree:fertilizerCallBack(fieldData)
        else
...
--@brief 	获取操作类型
function SceneHVTree:getOperateType()
	return self.m_nOperateType
end

--@brief 	操作结果
function SceneHVTree:operateResult(opType, result, itemIds, nums)
	if result == 0 then 
		if opType == 1 then --播种
...
--@brief 	延时显示偷取奖励
function SceneHVTree:delayShowReward()
	local self = SceneHVTree
...
--@brief 	延时显示松土奖励
function SceneHVTree:delayShowLooseReward()
	local self = SceneHVTree
...
--@brief 	设置待扩展地块Cell值
function SceneHVTree:setExtendFieldCell(tCell, tCell2)
...
--@brief 	是否自己的度假村
function SceneHVTree:isMyHolidayVillage()
...
--@brief 	显示收获固定奖励
function SceneHVTree:showFixedReward()
	if self.m_root == nil then return end 
	if self.m_tHarvestField == nil then return end 
	--获取收获的土坑
	local conForBuilding = GetElement(self.m_root, "conForBuilding_hvTreeScene", WZUIContainer)
	local indexX, indexY = self.m_tHarvestField.indexX, self.m_tHarvestField.indexY
	local nTag = (indexX - 1) * HVTREE_ROW + indexY
...
--@brief 	访客数据
function SceneHVTree:setVisitorsData(visitorIds, visitorSexs, visitorNames, visitorFaceIds, visitorHeadIds, visitorHeadColors, visitorBodyIds, visitorBodyColors, visitorWingIds, visitorServerIds, visitorLevels, visitorVipLevels, visitorTitle, visitorFootMark, visitorBlueVipInfo, spirits, synType)
	do return end 
	if synType == 0 then 
	...
	WZLog("SceneHVTree:setVisitorsData one", synType)
	if synType == 3 then 
				...
				if self.m_tCellVisitorRole then 
					for j = 1, #self.m_tCellVisitorRole do
						...
						if tItem and tItem.playerId == visitorIds[i] then 
							WZLog("SceneHVTree:setVisitorsData two")
		...
		for i = 1, #visitorIds do
			...
			--	WZLog("SceneHVTree:setVisitorsData five")
						...
						if self.m_tCellVisitorRole then --如果原来有隐藏的，则使用原来隐藏的玩家的形象，不用重新创建
							for j = 1, #self.m_tCellVisitorRole do
								...
								if not bVisible then 
									...
									bAddNew = false 
									WZLog("SceneHVTree:setVisitorsData three")
									break 
								end
							end
						end
						if bAddNew then --需新创建一个形象
							WZLog("SceneHVTree:setVisitorsData four")
			...
			if synType == 0 then
				...
				for j=1,#spiritIds do
					...
					if self.m_tCellSpiritRoles then
						for k = 1, #self.m_tCellSpiritRoles do
							...
							if tItem and tItem.spiritId == spiritIds[j] then --找到相同的更新
								WZLog("SceneHVTree:setVisitorsData spirit_update")
					...
					if bAddNew then --没找到就新创建一个
						WZLog("SceneHVTree:setVisitorsData spirit_Add")
						table.insert(self.m_tSpiritList, tData)
				...
				if self.m_tCellSpiritRoles then
					for j = 1, #self.m_tCellSpiritRoles do
						...
						if bDelOld then
							WZLog("SceneHVTree:setVisitorsData spirit_Delect")
...
--@brief 	许愿结果
function SceneHVTree:wishResult(itemId, itemNum, result, wishingTimes)
	if result == 0 then 
		WndHVOperate:wishResult(itemId, itemNum, wishingTimes)
	end
end
-------------------------------------公有方法模块End----------------------------------------
...
-------------------------------------私有方法模块Begin--------------------------------------
--@brief 	获取神树等级配置
function SceneHVTree:getDivineTreeConfigByLv(level)
...
--@brief 	选择果实回调
function SceneHVTree:_chooseFruit(opType, fruitId, index, itemId, num)
	if opType == 1 then 
		local fruitData = GDatatab_holiday_tree_fruit["id_" .. fruitId]
		self.m_tDivineTreeInfo.fruits[index + 1].fruitId = fruitId
		self.m_tDivineTreeInfo.fruits[index + 1].endTime = SystemTime:getServerTime() + fruitData.time / 1000
...
--@brief 	加速果实/果树回调
function SceneHVTree:_speedFruit(opType, itemId, itemNum, index)
...
function SceneHVTree:generateRandomGrid()
	-- body
	self.m_tRandomGrid = {}
	local mMaxNum = self.m_nMaxVisitorCount + 1 + self.m_nMaxSpiritCount
	while #self.m_tRandomGrid < mMaxNum do 
		local nIndexX = math.random(1, HVTREE_ROW)
		local nIndexY = math.random(1, HVTREE_ROW)
		local bFind = false 
		while not bFind do
			...
			if bFind then 
				...
				nIndexX = math.random(1, HVTREE_ROW)
				nIndexY = math.random(1, HVTREE_ROW)
			end
		end
	end

	WZLog("SceneHVTree:generateRandomGrid", Serialize(self.m_tRandomGrid))
end

--@brief 	根据格子X,Y索引，计算绝对坐标
--@param 	tBasicData:建筑表数据
function SceneHVTree:_getAbsPosition(indexX, indexY, tBasicData)	-- body
    ...
    local startY = HVTREE_HEIGHT / 2 - (indexX - 1) * gapY
...
--@brief 	初始化格子数据
function SceneHVTree:_initGridData()
	--初始化格子数据
	self.m_tGridList = {} 	
	local gapX = HVMAP_SIZEX / 2 
    local gapY = HVMAP_SIZEY / 2 			--格子数据
	for i = 1, HVTREE_ROW do 
		self.m_tGridList[i] = {}
		local startX = 0 + (i - 1) * gapX
        local startY = HVTREE_HEIGHT / 2 - (i - 1) * gapY
		for j = 1, HVTREE_ROW do
			local tItem = {}
			local nPosX = startX + j * gapX
			local nPosY = startY + (j - 1) * gapY
			if nPosY > HVTREE_HEIGHT*9/16 + HVMAP_SIZEY then --上部不可跑动
				tItem.configId = 0
			elseif nPosY < 23*gapY then --下部不可跑动
				tItem.configId = 0
			elseif nPosX < 21*gapX then --左边
				tItem.configId = 0
			elseif nPosX > HVTREE_WIDTH - 21*gapX then --右边
				tItem.configId = 0
			else
	...
	for i, configId in pairs(self.m_tConfigId) do
		...
		WZLog("SceneHVTree:_initGridData", configId, indexX, indexY)
		local tBasicData = nil 
		if configId > 0 and configId < 20 then 
	...
	WZLog("SceneHVTree:_initGridData", Serialize(self.m_tGridList))
end

--@brief    检测移动间隔时间,为了确保只有一个小孩开始移动,因为多个小孩一起移动比较卡
function SceneHVTree:checkMoveRandomTime()
...
function SceneHVTree:reGenerateOneGrid(tCellRoleObj)
	-- body
	local nIndexX = math.random(1, HVTREE_ROW)
	local nIndexY = math.random(1, HVTREE_ROW)
	...
	if tCellRoleObj then
		...
		nIndexX = math.random(math.max(tGridData[1]-offset,0), math.min(tGridData[1]+offset,HVTREE_ROW))
		nIndexY = math.random(math.max(tGridData[2]-offset,0), math.min(tGridData[2]+offset,HVTREE_ROW))
	end
...
--@brief 	获取格子信息
function SceneHVTree:GetGroundInfo(indexX, indexY)
	-- body
	local cellFloor = nil 
...
function SceneHVTree:findPathFinish(tPathNode, nIndex)
	-- body
...
--		WZLog("SceneHVTree:findPathFinish", Serialize(tPathNode))
	end
end

--@brief 	开始寻路
function SceneHVTree:startFindPath(targetX, targetY, roleIndex)
	-- body
	AStarPathfinding:SetRowAndColumnAmount(HVTREE_ROW, HVTREE_ROW)
...
function SceneHVTree:sortBuilding()
	...
	for i = 1, #self.m_tGridList do
		for j = 1, #self.m_tGridList[i] do
			if self.m_tGridList[i][j].configId > 0 then 
				...
				tItem.listSortIdMin = HVTREE_ROW - nTempIndexY
				tItem.lineSortIdMin = tItem.indexX
				tItem.listSortIdMax = HVTREE_ROW - tItem.indexY
	...
	--获取玩家和小孩的格子
--	WZLog("SceneHVTree:sortBuilding", type(self.m_tCellHost))
...
--	WZLog("SceneHVTree:sortBuilding", Serialize(tGridData))
	SceneHVTree:FurnitureSort2(tGridData)
end

--对所有的物品排序显示
function SceneHVTree:FurnitureSort2(cellList)
	local temp = {}
	...
	if #temp == 0 then
--		LogDebug("SceneHVTree:FurnitureSort temp is nil 000000000000")
		return
	end
			...
			if k.listSortIdMin == v[1] and k.lineSortIdMin == v[2] and  k.listSortIdMax == v[3] and k.lineSortIdMax == v[4] then
				if k.configId > 0 then 
					if k.configId >= 9999999 - self.m_nMaxVisitorCount - self.m_nMaxSpiritCount and k.configId <= 9999999 then 
						...
						local nTag = (k.indexX - 1) * HVTREE_ROW + k.indexY
...
--@brief 	获取一块可播种的土坑
function SceneHVTree:getNullField()
	--初始化土地块数据
	for i, configId in pairs(self.m_tConfigId) do
		local indexX = self.m_tFieldPos[i][1]
		local indexY = self.m_tFieldPos[i][2]
...
--@brief 	初始化数据
function SceneHVTree:_initFieldData()
	self.m_tConfigId = {}
	self.m_tFieldPos = {} 
	...
	for i, value in pairs(GDatatab_holiday_position) do
		if value.id >= 16 and value.id <= 24 then 
...
--@brief 	获取神树信息
function SceneHVTree:_getDivineInfo(lvl, exp, index, ids, endTimes, wishingTimes)
	...
	if self.m_tDivineTreeInfo then 
		if lvl ~= self.m_tDivineTreeInfo.level then 
			...
			nextLvConfig = self:getDivineTreeConfigByLv(lvl)
	...
	if bIsNeedUpdate then 
		local spineTree = GetElement(self.m_root, "spineTree_hvTreeScene", WZUISpine)
		if spineTree and nextLvConfig then 
			spineTree:play(nextLvConfig.action, true)
		end
	end
	WZLog("SceneHVTree:_getDivineInfo", wishingTimes[1], wishingTimes[2])
	if self.m_tWishData == nil then 
		self.m_tWishData = wishingTimes
		WndHVOperate:_setWishBtnState(0, wishingTimes[1], wishingTimes[2])
	else
		if self.m_tWishData[1] ~= wishingTimes[1] or self.m_tWishData[2] ~= wishingTimes[2] then 
			self.m_tWishData = wishingTimes
			WndHVOperate:_setWishBtnState(0, wishingTimes[1], wishingTimes[2])
		end
	end
