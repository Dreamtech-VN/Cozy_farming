...
IPDhttpServer = 
{
    ...
    isNewUser = false,--0=新用户 true，1=老用户 false
}
...
function IPDhttpServer:ipdCallback(nTaskID, sData, bFinish, bFailed)
    ...
    if tData.isNewUser and tonumber(tData.isNewUser) == 0 then
        self.isNewUser = true
    end
    WZLog("IPDhttpServer:ipdCallback self.isNewUser =", self.isNewUser)
    -- 需要更新整个包
    ...
    -- 进入选服
    g_nCreateRoleNum = tData.roleNum or 0
...
function IPDhttpServer:initGameNotice(data)
    ...
    --qq大厅新用户直接进入游戏
    if (isChannelQQHall() or isChannelFlashHall() or isChannel360Hall() or isYLGYLoginChannel()) and IPDhttpServer.isNewUser == true and WndLoginSelect.m_root ~= nil then
        WZLog("IPDhttpServer:initGameNotice ===> qqhall newuser auto entergame.")
        WndLoginSelect:OnBtnEnterGame()
        return
    end
